...
#include "hisi_vcodec_vdec_dbg.h"
#include "securec.h"
#include "dbg.h"
...
static ssize_t vdec_show_setting_value(char *buf, ssize_t start_pos, ssize_t buf_capacity, const struct vdec_setting_info *setting_info)
{
	size_t i;
	ssize_t ret;
	ssize_t filled_len = start_pos;
...
static void vdec_store_setting_value(const void *start_addr, const char *key, const int key_len, const char *value)
{
	...
	for (i = 0; i < ARRAY_SIZE(setting_options); i++) {
		...
		if (strlen(option->name) == key_len &&
			...
			case SETTING_OPT_TYPE_UINT64: {
				long long val = 0;
				if (strict_strtoll(value, 0, &val) < 0)
					return;
