...
static void cmdlist_put_head(struct cmdlist_head *head)
{
	if (head < g_cmdlist_manager.head) {
		HI_FATAL_VENC("current head is invalid");
		return;
	}
...
static int32_t cmdlist_encode(int32_t core_id, uint32_t type)
{
	...
	if (wait_idx >= CMDLIST_MAX_HEAD_NUM) {
		HI_FATAL_VENC("no new node is added to the list");
		ret = HI_FAILURE;
		goto exit;
	}

	if (cur_idx < CMDLIST_MAX_HEAD_NUM) {
		HI_FATAL_VENC("cur num is %u, core id %u",
...
static void cmdlist_encode_handle(struct work_struct *work)
{
	...
	if (!g_cmdlist_manager.bit_map) {
		if (venc_regulator_disable_by_low_power() != HI_SUCCESS)
			HI_WARN_VENC("power off regulator failed");
		return;
	}

	venc_get_clock_info(&info);

	if (venc_regulator_update(&info) != HI_SUCCESS) {
		HI_ERR_VENC("update regulator status failed");
		return;
	}
	...
	uint32_t *reg_base, U_FUNC_VCPI_INTSTAT int_status)
{
	...
	list_for_each_entry_safe(cmdlist_node, tmp_cmdlist_node, &head->list, list) {
		...
		if (int_status.bits.vcpi_int_vedu_timeout ||
			...
			HI_WARN_VENC("current isr status %x", int_status.u32);
		} else {
...
static void cmdlist_encode_done(struct venc_context *ctx)
{
	...
	if (ctx->status != VENC_BUSY) {
		...
		HI_FATAL_VENC("current core status invalid, core status: %d", ctx->status);
		return;
	}
...
#endif
		if (osal_del_timer(&ctx->timer, false) != HI_SUCCESS)
...
static void cmdlist_encode_timeout(unsigned long core_id)
{
	...
	HI_WARN_VENC("coreid:%d encode timeout", (int32_t)core_id);

	ret = venc_check_coreid((int32_t)core_id);
	if (ret != HI_SUCCESS) {
		HI_ERR_VENC("CORE_ERROR:invalid core ID is %d", (int32_t)core_id);
		return;
	}
