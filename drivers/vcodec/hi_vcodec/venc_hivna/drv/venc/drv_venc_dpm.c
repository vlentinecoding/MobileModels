...
static int dpm_venc_enable(const struct venc_config *venc_config)
{
	...
	HI_DBG_VENC("enable dpm_venc success");
	return HI_SUCCESS;
}
...
static void dpm_venc_disable(void)
{
	...
	HI_DBG_VENC("disable dpm_venc success");
}
...
void venc_dpm_freq_select(venc_clk_t clk_type)
{
	unsigned int val;
	if (!g_dpm_venc_inited) {
		HI_DBG_VENC("dpm_venc is disabled");
		return;
	}
...
void venc_dpm_init(struct venc_config *venc_config)
{
	...
	if (!g_dpm_venc_ops.module_enabled) {
		HI_DBG_VENC("dpm_venc is not enabled");
		return;
	}
	...
	mutex_unlock(&g_dpm_venc_state_lock);
	HI_DBG_VENC("dpm_venc is successfully init");
}

void venc_dpm_deinit(void)
{
	if (!g_dpm_venc_inited) {
		HI_DBG_VENC("dpm_venc is not init");
		return;
	}
	...
	HI_DBG_VENC("dpm_venc is successfully deinit");
}
