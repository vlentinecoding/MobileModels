...
#if   defined(CONFIG_HISI_LB_GEMINI)
...
static void set_quota(struct lb_plc_info *policy, unsigned int quota_value)
{
	gid quota_set;
	gid allc_set;
...
#endif
	writel(CMO_CFG_INT_MASK, (d->base + CMO_CFG_INT_CLR));
...
#endif
	writel(CMO_CFG_INT_MASK, (d->base + CMO_CFG_INT_EN));
