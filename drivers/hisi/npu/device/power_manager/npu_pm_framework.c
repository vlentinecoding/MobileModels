...
void npu_pm_add_idle_timer(struct npu_dev_ctx *dev_ctx)
{
	...
	if ((bitmap_get(dev_ctx->pm.work_mode, NPU_NONSEC) == 0) ||
...
int npu_pm_powerdown_proc(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode, u32 delta_subip)
{
	int ret = 0;
	int fail_flag = false;
	int subip;
	...
	for (subip_idx = 0; subip_idx < NPU_SUBIP_MAX; subip_idx++) {
		...
		if (bitmap_get(delta_subip, subip) &&
			(bitmap_get(curr_subip_state, subip) == 1)) {
			subip_action = npu_pm_get_subip_action(&dev_ctx->pm, subip);
			ret = subip_action->power_down(work_mode, delta_subip,
				(void *)proc_ctx);
			if (ret != 0) {
				fail_flag = true;
			...
			curr_subip_state &= (~(1U << (unsigned int)subip));
			if ((subip == NPU_AICORE0) &&
				(bitmap_get(delta_subip, NPU_AICORE1)))
...
int npu_pm_interframe_idle_powerdown(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode)
{
	int ret;
	u32 delta_subip;
...
int npu_pm_powerup_proc(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode, u32 delta_subip)
{
	int ret = 0;
	int subip;
	...
	for (subip_idx = 0; subip_idx < NPU_SUBIP_MAX; subip_idx++) {
		...
		if (bitmap_get(delta_subip, subip) &&
			(bitmap_get(curr_subip_set, subip) == 0)) {
			subip_action = npu_pm_get_subip_action(&dev_ctx->pm, subip);
			ret = subip_action->power_up(work_mode, delta_subip,
				(void **)proc_ctx);
			cond_goto_error(ret != 0, POWER_UP_FAIL, ret, ret,
				"subip power up fail : subip %d ret = %d\n", subip, ret);
			curr_subip_set |= (1 << (unsigned int)subip);
			if ((subip == NPU_AICORE0) &&
				(bitmap_get(delta_subip, NPU_AICORE1)))
...
POWER_UP_FAIL:
	...
	if (npu_rdr_exception_is_count_exceeding(
		(u32)RDR_EXC_TYPE_NPU_POWERUP_FAIL) == 0 && ret != -NOSUPPORT)
...
static int npu_pm_interframe_idle_powerup(struct npu_proc_ctx *proc_ctx,
...
int npu_pm_check_and_interframe_powerup(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, u32 workmode)
{
	int ret = 0;

	if (dev_ctx->plat_info != NULL &&
		dev_ctx->plat_info->dts_info.feature_switch[NPU_FEATURE_INTERFRAME_IDLE_POWER_DOWN]) {
		if (workmode == NPU_NONSEC) {
			if (npu_pm_interframe_check_need_powerup(dev_ctx, workmode)) {
				if ((dev_ctx->plat_info->dts_info.feature_switch[NPU_FEATURE_NPU_AUTOFS] == 1) &&
					(dev_ctx->power_stage == NPU_PM_UP))
					npu_autofs_disable();
				ret = npu_pm_interframe_idle_powerup(
					proc_ctx, dev_ctx, workmode);
			}
		}
	}
	...
	struct npu_dev_ctx *dev_ctx, u32 work_mode)
{
	...
	if (bitmap_get(curr_work_mode_set, work_mode) != 0) {
		...
		ret = npu_pm_check_and_interframe_powerup(proc_ctx, dev_ctx,
			work_mode);
		cond_goto_error(ret != 0, fail, ret, ret,
			"npu interframe powerup failed\n");
		mutex_unlock(&dev_ctx->npu_power_up_off_mutex);
		return 0;
	}
	...
	cond_goto_error(ret != 0, fail, ret, ret,
		"fail to enter workwode: %d, ret = %d\n", work_mode, ret);
	...
	struct npu_dev_ctx *dev_ctx, u32 workmode)
{
	...
	if (workmode == NPU_NONSEC && atomic_read(&dev_ctx->pm.task_ref_cnt) != 0) {
		npu_drv_err("some tasks have not been recycled, count = %d\n",
			atomic_read(&dev_ctx->pm.task_ref_cnt));
		npu_task_set_clear(dev_ctx);
	}
	return ret;
}
...
int npu_pm_powerdown_pre_proc(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode)
{
	int ret;
	...
	u32 goal_work_mode_set =
		(dev_ctx->pm.work_mode & (~(1U << (unsigned int)work_mode)));
...
int npu_pm_powerdown_post_proc(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode)
{
	int aicore_result;
	u32 *ts_status = NULL;
...
int npu_pm_powerup_pre_proc(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode)
{
...
int npu_pm_powerup_post_proc(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode)
{
	int ret;
	int aicore_result;
	...
	if ((work_mode != NPU_SEC) && (dev_ctx->pm.work_mode == 0)) {
		dev_ctx->heart_beat.hwts_exception_callback =
			npu_exception_report_proc;
...
static void npu_pm_set_power_on(int work_mode)
{
	if (work_mode == NPU_SEC)
		npu_plat_set_npu_power_status(DRV_NPU_POWER_ON_SEC_FLAG);
	else
...
int npu_pm_powerdown(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode)
{
	int ret;
	u32 delta_subip;
	...
	delta_subip = npu_pm_get_delta_subip_set(&dev_ctx->pm, work_mode,
		POWER_DOWN);
	ret = npu_pm_powerdown_proc(proc_ctx, dev_ctx, work_mode, delta_subip);
	if (ret != 0)
...
int npu_pm_powerup(struct npu_proc_ctx *proc_ctx,
	struct npu_dev_ctx *dev_ctx, int work_mode)
{
	int ret;
	u32 delta_subip;
	...
	struct npu_dev_ctx *dev_ctx)
{
	...
	for (workmode_idx = 0; workmode_idx < NPU_WORKMODE_MAX; workmode_idx++) {
		if (bitmap_get(curr_work_mode_set, workmode_idx) != 0) {
			goal_work_mode_set &= (~(1U << (unsigned int)workmode_idx));
			ret = npu_pm_powerdown(proc_ctx, dev_ctx, workmode_idx);
			cond_goto_error(ret != 0, fail, ret, ret,
				"npu power down fail : ret = %d, in workmode = %d\n",
				ret, workmode_idx);
...
static void npu_pm_idle_work(struct work_struct *npu_idle_work)
{
	...
	proc_ctx = (struct npu_proc_ctx *)power_manager->private_data[1];
	cond_return_void(proc_ctx == NULL, "proc_ctx is null\n");
	...
	int pm_ops)
{
	...
	if (pm_ops == POWER_UP) {
		...
		for (mode_idx = 0; mode_idx < NPU_WORKMODE_MAX; mode_idx++) {
			if (bitmap_get(handle->work_mode, mode_idx) &&
				(mode_idx != work_mode)) {
