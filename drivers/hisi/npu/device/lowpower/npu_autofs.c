...
void __iomem *g_npu_autofs_crg_vaddr;

void npu_autofs_npubus_on(void)
{
	...
	npu_autofs_reg_update(
		(SOC_NPUCRG_PERI_AUTODIV2_ADDR(g_npu_autofs_crg_vaddr)), 0x0, 0x1);
	npu_autofs_reg_update((SOC_NPUCRG_PEREN1_ADDR(g_npu_autofs_crg_vaddr)),
		0x100, 0x100);
}

void npu_autofs_npubus_off(void)
{
	npu_autofs_reg_update((SOC_NPUCRG_PERDIS1_ADDR(g_npu_autofs_crg_vaddr)),
		0x100, 0x100);
	npu_autofs_reg_update(
		(SOC_NPUCRG_PERI_AUTODIV2_ADDR(g_npu_autofs_crg_vaddr)), 0x1, 0x1);
}

void npu_autofs_npubuscfg_on(void)
{
	...
	npu_autofs_reg_update(
		(SOC_NPUCRG_PERI_AUTODIV1_ADDR(g_npu_autofs_crg_vaddr)), 0x0, 0x1);
	npu_autofs_reg_update((SOC_NPUCRG_PEREN1_ADDR(g_npu_autofs_crg_vaddr)),
		0x200, 0x200);
}

void npu_autofs_npubuscfg_off(void)
{
	npu_autofs_reg_update((SOC_NPUCRG_PERDIS1_ADDR(g_npu_autofs_crg_vaddr)),
		0x200, 0x200);
	npu_autofs_reg_update(
		(SOC_NPUCRG_PERI_AUTODIV1_ADDR(g_npu_autofs_crg_vaddr)), 0x1, 0x1);
}

void npu_autofs_npubus_aicore_on(void)
{
	...
	npu_autofs_reg_update(
		(SOC_NPUCRG_PERI_AUTODIV5_ADDR(g_npu_autofs_crg_vaddr)), 0x0, 0x1);
	npu_autofs_reg_update((SOC_NPUCRG_PEREN1_ADDR(g_npu_autofs_crg_vaddr)),
		0x800, 0x800);
}

void npu_autofs_npubus_aicore_off(void)
{
	npu_autofs_reg_update((SOC_NPUCRG_PERDIS1_ADDR(g_npu_autofs_crg_vaddr)),
		0x800, 0x800);
	npu_autofs_reg_update(
		(SOC_NPUCRG_PERI_AUTODIV5_ADDR(g_npu_autofs_crg_vaddr)), 0x1, 0x1);
}
...
{
	if (g_npu_autofs_crg_vaddr == NULL) {
		npu_drv_info("g_npu_autofs_crg_vaddr is NULL\n");
		return;
	}
