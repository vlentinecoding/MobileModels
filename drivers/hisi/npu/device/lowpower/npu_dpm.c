...
/* npu pcr related */
void __iomem *acpu_peri_crg_base_vir_addr;
void __iomem *acpu_npu_crg_base_vir_addr;
void __iomem *acpu_npu_sctrl_base_vir_addr;
void __iomem *acpu_npu_pcr_base_vir_addr;
		...
		for (i = 0; i < NPU_DPM_NUM; i++) {
			...
			if (dpm_addr->vir_addr != NULL) {
				...
				for (j = 0; j < NPU_DPMONITOR_BUSY_CNT_REG_NUM; j++) {
					...
					npu_dpm_ops.dpm_counter_table[count_reg_num_updated++] +=
						val;
				}
				/* step16 */
	...
	for (index = 0; index < NPU_DPM_NUM; index++) {
		...
		if (dpm_addr->vir_addr == NULL) {
			...
			if (dpm_addr->vir_addr == NULL) {
				...
				npu_drv_err("ioremap g_npu_dpm_addr %d fail\n", index);
				mutex_unlock(&dpm_npu_lock);
				return;
			}
		}
