...
static int npu_load_hwts_sqe(npu_model_desc_t *model_desc, int stream_idx,
	...
	struct npu_entity_info *hwts_sq_sub_info)
{
	...
	// verify hwts_sqe
	ret = npu_verify_hwts_sqe(stream_buf_addr, stream_len);
	if (ret != 0) {
		vfree(stream_buf_addr);
...
static int npu_load_model(npu_model_desc_t *model_desc, u8 devid)
{
	...
	for (idx = 0; idx < model_desc->stream_cnt; idx++) {
		...
		// V200 useless field, should use modelExecute task field
		stream_info->smmu_substream_id = 1;
		...
		sqe_num = npu_load_hwts_sqe(model_desc, idx, model_desc_info,
			hwts_sq_sub_info);
		if (sqe_num <= 0) {
