...
void write_reg32_readback(u64 addr, u32 value)
{
	...
	npu_drv_info("addr:0x%lx,write value:0x%x, read value:0x%x",
...
void write_reg64_readback(u64 addr, u64 value)
{
	...
	npu_drv_info("addr:0x%lx,write value:0x%lx, read value:0x%lx",
		addr, value, read_value);
}
...
void hwts_log_reg_init(u8 devid, u64 hwts_base)
{
	...
	ring_manager =
		&(profiling_info->head.manager.ring_buff[PROF_HWTS_LOG_CHANNEL]);
	...
	aicore_enable = profiling_info->head.manager.cfg.info.aicore.en;
	if (aicore_enable & AICORE_PROF_ENABLE_TASK_LOG)
		hwts_dfx_log_ctrl.reg.task_log_en = 1;

	if (aicore_enable & AICORE_PROF_ENABLE_BLOCK_LOG)
		hwts_dfx_log_ctrl.reg.block_log_en = 1;

	if (aicore_enable & AICORE_PROF_ENABLE_BLOCK_PMU)
		hwts_dfx_log_ctrl.reg.pmu_log_en = 1;

	hwts_dfx_log_ctrl.reg.log_buf_length = (u16)(log_buf_length - 1);
	hwts_dfx_log_ctrl.reg.log_af_threshold = (u16)(log_buf_length - 1);
...
void hwts_profiling_reg_init(u8 devid, u64 hwts_base)
{
	...
	ring_manager =
		&(profiling_info->head.manager.ring_buff[PROF_HWTS_PROFILING_CHANNEL]);
	cond_return_void(ring_manager == NULL,
		"ring_manager is null, channel = %d\n", PROF_HWTS_PROFILING_CHANNEL);
	...
	hwts_dfx_profile_ctrl.reg.profile_buf_length = (u16)(profile_buf_length - 1);
	hwts_dfx_profile_ctrl.reg.profile_af_threshold = (u16)(profile_buf_length - 1);
...
void hwts_profiling_init(u8 devid)
{
	...
	hwts_addr = (u64)(uintptr_t)plat_info->dts_info.reg_vaddr[NPU_REG_HWTS_BASE];
...
int npu_hwts_query_aic_task_config(u8 do_print)
{
	u64 value;
	u64 hwts_base = npu_hwts_get_base_addr();
	...
	read_uint64(value, SOC_NPU_HWTS_HWTS_FREE_AI_CORE_BITMAP_ADDR(hwts_base));
	npu_drv_warn("HWTS_FREE_AI_CORE_BITMAP = 0x%llx\n", value);
	read_uint64(value, SOC_NPU_HWTS_HWTS_AIC_BLK_FSM_SEL_ADDR(hwts_base));
	npu_drv_warn("HWTS_AIC_BLK_FSM_SEL = 0x%llx\n", value);
	read_uint64(value, SOC_NPU_HWTS_HWTS_AIC_BLK_FSM_STATE_ADDR(hwts_base));
	npu_drv_warn("HWTS_AIC_BLK_FSM_STATE = 0x%llx\n", value);
	read_uint64(value, SOC_NPU_HWTS_HWTS_L2_OWNER_ADDR(hwts_base));
...
int npu_hwts_query_sdma_task_config(u8 do_print)
{
	uint32_t idx;
	u64 value[4];
	u64 hwts_base = npu_hwts_get_base_addr();
	...
	read_uint64(value[0],
		SOC_NPU_HWTS_HWTS_SDMA_NS_SQ_BASE_ADDR_CFG_ADDR(hwts_base));
	npu_drv_warn("HWTS_SDMA_NS_SQ_BASE_ADDR_CFG = 0x%llx\n", value[0]);
	read_uint64(value[0],
		SOC_NPU_HWTS_HWTS_FREE_SDMA_SQ_BITMAP_ADDR(hwts_base));
	npu_drv_warn("HWTS_FREE_SDMA_SQ_BITMAP = 0x%llx\n", value[0]);

	read_uint64(value[0], SOC_NPU_HWTS_HWTS_SDMA_BLK_FSM_SEL_ADDR(hwts_base));
	npu_drv_warn("HWTS_SDMA_BLK_FSM_SEL = 0x%llx\n", value[0]);
	read_uint64(value[0],
		SOC_NPU_HWTS_HWTS_SDMA_BLK_FSM_STATE_ADDR(hwts_base));
	npu_drv_warn("HWTS_SDMA_BLK_FSM_STATE = 0x%llx\n", value[0]);

	read_uint64(value[0], SOC_NPU_HWTS_SDMA_GLB_SETTING1_ADDR(hwts_base));
	read_uint64(value[1], SOC_NPU_HWTS_SDMA_GLB_SETTING2_ADDR(hwts_base));
	read_uint64(value[2], SOC_NPU_HWTS_SDMA_GLB_SETTING3_ADDR(hwts_base));
	npu_drv_warn("SDMA_GLB_SETTING[0~2] = 0x%llx 0x%llx 0x%llx\n",
			value[0], value[1], value[2]);

	for (idx = 0; idx < 4; idx++) {
		read_uint64(value[0],
			SOC_NPU_HWTS_SDMA_SQCQ_DB_ADDR(hwts_base, idx));
		read_uint64(value[1],
			SOC_NPU_HWTS_SDMA_CQE_STATUS0_ADDR(hwts_base, idx));
		read_uint64(value[2],
			SOC_NPU_HWTS_SDMA_CQE_STATUS1_ADDR(hwts_base, idx));
	...
	struct hwts_interrupt_info *interrupt_info, u8 do_print)
{
	...
	read_uint64(glb_ctrl0.value, SOC_NPU_HWTS_HWTS_GLB_CTRL0_ADDR(hwts_base));
	read_uint64(glb_ctrl1.value, SOC_NPU_HWTS_HWTS_GLB_CTRL1_ADDR(hwts_base));
	read_uint64(glb_ctrl2.value, SOC_NPU_HWTS_HWTS_GLB_CTRL2_ADDR(hwts_base));
	...
	u16 channel_id, struct sq_exception_info *sq_info, u8 do_print)
{
	...
	read_uint64(sq_cfg1.value,
		SOC_NPU_HWTS_HWTS_SQ_CFG1_ADDR(hwts_base, channel_id));
	if (do_print != 0)
	...
	read_uint64(sqcq_misc_state.value,
		SOC_NPU_HWTS_SQCQ_FSM_MISC_STATE_ADDR(hwts_base, channel_id));
	if (do_print != 0)
		npu_drv_warn(
...
int npu_hwts_query_ispnn_info(u16 channel_id)
{
	...
	/* HWTS_HW_HS */
	read_uint64(value, SOC_NPU_HWTS_HW_HS_GLB_CFG_ADDR(hwts_base));
	npu_drv_warn("CH = %u HW_HS_GLB_CFG = 0x%llx\n", channel_id, value);
	read_uint64(value,
		SOC_NPU_HWTS_HW_HS_CHANNEL_CFG_ADDR(hwts_base, channel_id));
	npu_drv_warn("CH = %u HW_HS_CHANNEL_CFG = 0x%llx\n", channel_id, value);
	read_uint64(value,
		SOC_NPU_HWTS_HW_HS_CHANNEL_CTRL_ADDR(hwts_base, channel_id));
	npu_drv_warn("CH = %u HW_HS_CHANNEL_CTRL = 0x%llx\n", channel_id, value);
	read_uint64(value, SOC_NPU_HWTS_HW_HS_SHADOW0_ADDR(hwts_base, channel_id));
	npu_drv_warn("CH = %u HW_HS_SHADOW0 = 0x%llx\n", channel_id, value);
	read_uint64(value, SOC_NPU_HWTS_HW_HS_SHADOW1_ADDR(hwts_base, channel_id));
	npu_drv_warn("CH = %u HW_HS_SHADOW1 = 0x%llx\n", channel_id, value);
	read_uint64(value, SOC_NPU_HWTS_HW_HS_DFX0_ADDR(hwts_base, channel_id));
