	...
	u8 aic_id, u64 query_addr, u64 *buffer, u32 buffer_len)
{
	...
	read_uint64(dbg_data1.value, SOC_NPU_AICORE_DBG_DATA1_ADDR(base));
	buffer[1] = dbg_data1.reg.dbg_data1;

	read_uint64(dbg_data2.value, SOC_NPU_AICORE_DBG_DATA2_ADDR(base));
	buffer[2] = dbg_data2.reg.dbg_data2;

	read_uint64(dbg_data3.value, SOC_NPU_AICORE_DBG_DATA3_ADDR(base));
	...
	u8 aic_id, struct aicore_exception_info *aic_info, u8 do_print)
{
	...
	read_uint64(aic_error_mask.value,
		SOC_NPU_AICORE_AIC_ERROR_MASK_ADDR(base));
