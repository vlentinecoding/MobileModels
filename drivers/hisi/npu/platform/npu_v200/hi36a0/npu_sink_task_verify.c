...
typedef int (*verify_hwts_task_func)(npu_rt_task_t *hwts_task);

int verify_task_comm_field(npu_rt_task_t *hwts_task)
{
	...
	if (hwts_task->stream_id < NPU_MAX_NON_SINK_STREAM_ID ||
		hwts_task->stream_id >= NPU_MAX_STREAM_ID) {
...
int verify_aicore_task(npu_rt_task_t *hwts_task)
{
	if (hwts_task->u.kernel_task.block_dim == 0 ||
		hwts_task->u.kernel_task.block_dim > UINT16_MAX) {
...
int verify_ph_task(npu_rt_task_t *hwts_task)
{
	return 0;
}

int verify_label_switch_task(npu_rt_task_t *hwts_task)
{
	if (hwts_task->stream_id >=
...
int verify_label_goto_task(npu_rt_task_t *hwts_task)
{
	if (hwts_task->stream_id >=
		(NPU_MAX_NON_SINK_STREAM_ID + NPU_MAX_LONG_HWTS_SQ_NUM) &&
...
int verify_evt_rec_task(npu_rt_task_t *hwts_task)
{
	if (hwts_task->u.event_task.event_id >= NPU_MAX_HWTS_EVENT_ID) {
...
int verify_wait_evt_task(npu_rt_task_t *hwts_task)
{
	if (hwts_task->u.event_task.event_id >= NPU_MAX_HWTS_EVENT_ID) {
		npu_drv_err("invalid event_id:%u, task_id:%u\n",
			hwts_task->u.event_task.event_id, hwts_task->task_id);
...
int verify_wite_value_task(npu_rt_task_t *hwts_task)
{
	return 0;
}

int verify_memcpy_task(npu_rt_task_t *hwts_task)
{
...
int npu_verify_hwts_sqe(void *stream_buf_addr, u64 ts_sq_len)
{
