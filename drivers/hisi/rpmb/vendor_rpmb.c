...
static void rpmb_report_hievent(void)
{
	...
	if ((hisi_getcurtime() - dmd_time) < RPMB_DMD_TIME_INTERVAL) {
			   ...
			   struct rpmb_operation *rpmb_ops)
{
	...
	g_mspc_end_time = hisi_getcurtime();
...
void vendor_rpmb_active(void)
{
	...
	g_mspc_start_time = hisi_getcurtime();
...
int get_rpmb_init_status(void)
{
	...
	for (i = 0; i < WAIT_INIT_TIMES; i++) {
		if (rpmb_support_device == BOOT_DEVICE_EMMC)
			...
			ret = (int32_t)ksys_access(EMMC_RPMB_BLOCK_DEVICE_NAME,
						  0);
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
			ret = (int32_t)ksys_access(UFS_RPMB_BLOCK_DEVICE_NAME,
						  0);
#else
