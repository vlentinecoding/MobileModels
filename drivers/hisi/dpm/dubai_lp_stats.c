 ...
 *
 */
#include <linux/hisi/dubai_lp_stats.h>
...
static int send_ipc_2_ddr(union ipc_data *msg, union ipc_data *ack)
{
	...
	ret = RPROC_SYNC_SEND(HISI_ACPU_LPM3_MBX_2, (mbox_msg_t *)msg,
			      DDR_VOTE_MSG_LEN,
...
{
	u32 id, freq, offset, id_offset, ddr_freq_num;
	int logic_id, ret;
	...
	for (id = 0; id < g_ddr_vote_num; id++) {
		id_offset = id * (ID_LEN + ddr_freq_num);
		logic_id = get_ddr_vote_id(id_offset);
		if (logic_id < 0)
			continue;
		for (freq = 0; freq < ddr_freq_num; freq++) {
			offset = id * ddr_freq_num + freq;
...
static int get_ddr_state_data(struct ddr_state *ddr_state_data)
{
	 ...
	 */
	ddr_freq_num = MAX_DDR_FREQ_NUM;
	for (id = 0; id < g_ddr_state_num; id++) {
		id_offset = id * (ID_LEN + ddr_freq_num);
		...
		for (freq = 0; freq < ddr_freq_num; freq++) {
			...
			if (ret != EOK) {
				pr_err("copy g_ddr_lp_stats_table %u error\n", id);
				return ret;
			}
		}
	}
