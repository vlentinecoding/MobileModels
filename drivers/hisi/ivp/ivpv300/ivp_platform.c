...
struct ivp_dync_dma_mem {
	void *vaddr;
	dma_addr_t phy_addr;
};

static struct ivp_dync_dma_mem g_ivp_dync_addr = {
	.vaddr = NULL,
	.phy_addr = 0
};
...
void ivp_check_and_wait_wfi(struct ivp_device *ivp_devp)
{
	...
	ivp_info("ivp core wfi value 0x%x", (waiti & IVP_DSP_PWAITMODE));
...
static int ivp_get_dynimic_buff(struct ivp_device *ivp_devp)
{
	...
	/*lint -restore*/
	if (!g_ivp_dync_addr.vaddr) {
		g_ivp_dync_addr.vaddr = dma_alloc_coherent(&ivp_comm->ivp_pdev->dev,
			ivp_comm->dynamic_mem_size, &g_ivp_dync_addr.phy_addr, GFP_KERNEL);
		if (!g_ivp_dync_addr.vaddr || !g_ivp_dync_addr.phy_addr) {
			ivp_err("get vaddr_memory failed");
			return -EINVAL;
		}
	}
	ivp_comm->vaddr_memory = g_ivp_dync_addr.vaddr;
	ivp_comm->sects[BASE_SECT_INDEX].acpu_addr = g_ivp_dync_addr.phy_addr >> IVP_MMAP_SHIFT;
...
void ivp_free_memory_section(struct ivp_device *ivp_devp)
{
	...
	if (g_ivp_dync_addr.vaddr) {
		dma_free_coherent(&ivp_comm->ivp_pdev->dev,
			...
			g_ivp_dync_addr.vaddr, g_ivp_dync_addr.phy_addr);
		g_ivp_dync_addr.vaddr = NULL;
		g_ivp_dync_addr.phy_addr = 0;
	}
...
int ivp_poweroff_pri(struct ivp_device *ivp_devp)
{
	...
	ivp_check_and_wait_wfi(ivp_devp);
	...
	struct ivp_device *ivp_devp)
{
	...
	ivp_info("get ultra low clk rate: %u", ivp_devp->ultra_low_clk_rate);
...
static ivp_get_clk_rate(struct ivp_device *ivp_devp, struct ivp_common *ivp_comm)
{
...
int ivp_change_clk(struct ivp_device *ivp_devp, unsigned int level)
{
	...
	ivp_comm->clk_usrsetrate = ivp_get_clk_rate(ivp_devp, ivp_comm);
	/* Set Clock rate */
