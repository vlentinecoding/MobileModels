...
int hisi_jpu_register(struct hisi_jpu_data_type *hisijd)
{
	...
	hisijd->jpgd_drv = hipp_common_register(JPEGD_UNIT, &hisijd->pdev->dev);
	jpu_err_if_cond((hisijd->jpgd_drv == NULL), "Failed: hipp_common_register\n");
#endif
...
static int do_cfg_smmuv3_on_ippcomm(const struct hisi_jpu_data_type *hisijd)
{
	int ret;
	jpu_check_null_return(hisijd->jpgd_drv, -EINVAL);
	jpu_check_null_return(hisijd->jpgd_drv->enable_smmu, -EINVAL);
	jpu_check_null_return(hisijd->jpgd_drv->setsid_smmu, -EINVAL);
	jpu_check_null_return(hisijd->jpgd_drv->set_pref, -EINVAL);

	ret = hisijd->jpgd_drv->enable_smmu(hisijd->jpgd_drv);
	if (ret != 0) {
		hisi_jpu_err("enable_smmu failed:%d\n", ret);
		return ret;
	}

	ret = hisijd->jpgd_drv->setsid_smmu(hisijd->jpgd_drv,
		hisijd->jpgd_secadapt_prop.swid[0], MAX_SECADAPT_SWID_NUM,
		hisijd->jpgd_secadapt_prop.sid, hisijd->jpgd_secadapt_prop.ssid_ns);
	if (ret != 0) {
		hisi_jpu_err("setsid_smmu failed:%d\n", ret);
	...
	ret = hisijd->jpgd_drv->set_pref(hisijd->jpgd_drv,
		hisijd->jpgd_secadapt_prop.swid[0], MAX_SECADAPT_SWID_NUM);
	if (ret != 0) {
		hisi_jpu_err("set_pref failed:%d\n", ret);
		if ((hisijd->jpgd_drv->disable_smmu(hisijd->jpgd_drv)) != 0)
			hisi_jpu_err("failed to disable smmu\n");
	}
...
#if defined(CONFIG_HISI_FB_V600)
	if ((hisijd->jpgd_drv == NULL) || (hisijd->jpgd_drv->power_up == NULL) ||
		(hisijd->jpgd_drv->set_jpgclk_rate == NULL)) {
		hisi_jpu_err("hisijd->jpgd_drv is NULL\n");
		return -EINVAL;
	}

	ret = hisijd->jpgd_drv->power_up(hisijd->jpgd_drv);
	if (ret != 0) {
		hisi_jpu_info("power_up failed:%d\n", ret);
		return ret;
	}

	/* set clk rate 640M */
	ret = hisijd->jpgd_drv->set_jpgclk_rate(hisijd->jpgd_drv, CLK_RATE_TURBO);
	if (ret != 0) {
		hisi_jpu_err("set_jpgclk_rate fail, change to set low temp rate %d\n", ret);
		/* low temp, when clk set fail, set clk rate to 480M */
		ret = hisijd->jpgd_drv->set_jpgclk_rate(hisijd->jpgd_drv, CLK_RATE_NORMAL);
		if (ret != 0) {
...
#if defined(CONFIG_HISI_FB_V600)
	if ((hisijd->jpgd_drv == NULL) || (hisijd->jpgd_drv->set_jpgclk_rate == NULL) ||
		(hisijd->jpgd_drv->power_dn == NULL)) {
		hisi_jpu_err("hisijd->jpgd_drv is NULL\n");
		return -EINVAL;
	}

	/* set clk rate 104M */
	ret = hisijd->jpgd_drv->set_jpgclk_rate(hisijd->jpgd_drv, CLK_RATE_OFF);
	if (ret != 0) {
		hisi_jpu_err("set_jpgclk_rate fail %d\n", ret);
		return ret;
	}

	ret = hisijd->jpgd_drv->power_dn(hisijd->jpgd_drv);
	if (ret != 0)
