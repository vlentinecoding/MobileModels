	...
	struct device_node *np)
{
	...
	hisijd->secadapt_base = of_iomap(np, JPEG_SECADAPT_REG);
	jpu_check_null_return(hisijd->secadapt_base, -ENXIO);
...
#if defined(CONFIG_HISI_FB_V600)
static int hisi_jpu_get_secadapt_prop(struct hisi_jpu_data_type *hisijd,
	struct device_node *np)
{
