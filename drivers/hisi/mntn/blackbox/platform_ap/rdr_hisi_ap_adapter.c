...
#include <linux/hisi/lpcpu_mntn_l3cache_ecc.h>
#include <securec.h>
...
#ifndef CONFIG_HISI_EARLY_PANIC
...
#ifdef CONFIG_PM
void rdr_hisiap_root_backup(void)
{
	if (g_rdr_ap_root)
...
void rdr_hisiap_root_restore(void)
{
	if (g_rdr_ap_root)
...
void save_logbuff_memory(void)
{
	...
	if (rdr_save_logbuff_memory(dst_str, SRC_LOGBUFFER_MEMORY)) {
...
int save_exception_info(void *arg)
{
	 ...
	 * it indicates that the function is invoked in rdr_dfx_ap_dump
	 */
	if (!arg)
		return ret;
...
int __init rdr_hisiap_init(void)
{
	...
	powerkey_register_notifier(&rdr_hisiap_powerkey_block);
...
void rdr_log_buf_notify_bl31(void)
{
	...
	unsigned long long logbuf_addr = virt_to_phys(hisirdr_ex_log_buf);
	unsigned int logbuf_size = hisirdr_ex_log_buf_len;
	unsigned long long log_first_idx_addr = virt_to_phys(hisirdr_ex_log_first_idx);
	unsigned long long log_next_idx_addr = virt_to_phys(hisirdr_ex_log_next_idx);
	...
	printk("hisi_ap_ealry_panic happen, will notify bl31\n");
	flush_logbuff_range();
