...
int rdr_hisee_thread(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		case HISEE_STATE_HISEE_EXC:
			...
			if (diaginfo.mod_id == (u32)MODID_HISEE_EXC_BUS)
				mspc_report_rpmb();
			/*
			...
			if (diaginfo.mod_id == (u32)MODID_HISEE_EXC_ROM)
				(void)mspc_load_image2ddr();
#ifdef CONFIG_MSPC_SUPPORT_DCS
			if (diaginfo.mod_id == (u32)MODID_HISEE_EXC_DCS)
				mspc_dcs_data_load();
#endif
...
#if defined(CONFIG_HISEE_MNTN_TEST) || defined(CONFIG_MSPC)
/*
