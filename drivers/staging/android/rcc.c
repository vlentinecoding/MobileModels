...
static int rcc_thread(void *unused)
{
	...
	while (true) {
		...
		if (rcc->full_clean_flag) {
#ifdef CONFIG_RCC_STR_SUPPORT
			full_clean_sleep_count = 0;
			while (full_clean_sleep_count++ < RCC_SLEEP_TIME) {
				ssleep(1);
				try_to_freeze();
			}
#else
...
static void rcc_setup(struct rcc_module *rcc)
{
	...
	if (nr_total > 3 * TOTAL_RAM_PAGES_1G) { /* >3g */
		rcc->anon_pages_min = RCC_ANON_PAGE_MIN + 2 * RCC_ANON_PAGE_RAM_GAP;
	} else if (nr_total > 2 * TOTAL_RAM_PAGES_1G) { /* 3g */
