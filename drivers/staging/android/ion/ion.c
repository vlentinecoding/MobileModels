...
#include <linux/mem_track/mm_ion_dump.h>
#endif

#define ION_ALLOC_MAX_LATENCY_US	(500 * 1000) /* 500ms */
					    ...
					    unsigned long flags)
{
	...
	etime = ktime_get();
	buffer->buffer_trace.heap_alloc1_cost = ktime_us_delta(etime, stime);
	buffer->buffer_trace.heap_drain_cost = 0;
	buffer->buffer_trace.heap_alloc2_cost = 0;
	...
	if (ret) {
		...
		ret = heap->ops->allocate(heap, buffer, len, align,
					  flags);
		etime = ktime_get();
		buffer->buffer_trace.heap_drain_cost = ktime_us_delta(mtime, stime);
		buffer->buffer_trace.heap_alloc2_cost = ktime_us_delta(etime, mtime);
		if (ret)
			goto err2;
	}
...
static void __print_cur_mem_state(void)
{
	pr_info("inactive_file:%lu, isolated_file:%lu, "
		"isolated_anon:%lu, free pages:%lu, "
		"kernel_stack:%luKB, free_cma:%lu\n",
			     ...
			     unsigned int flags)
{
	...
	if (len > (SZ_2M * 100)) { /* if len over 200M, print info */
		pr_info("%s: len > 200MB\n", __func__);
		__print_cur_mem_state();
	}
	...
	htime = ktime_get();
	htimedelta = ktime_us_delta(htime, stime);
	...
	etime = ktime_get();
	timedelta = ktime_us_delta(etime, stime);
	buffer->buffer_trace.alloc_sema_cost = htimedelta;
	buffer->buffer_trace.alloc_total_cost = timedelta;
	if (timedelta > ION_ALLOC_MAX_LATENCY_US) {
		pr_err("%s: alloc size=%zu, heap_id_mask=0x%x, "
			"flags=0x%x, htime=%lld, time=%lld, "
			"alloc1=%lld, drain=%lld, alloc2=%lld\n",
			__func__, len, heap_id_mask, flags, htimedelta, timedelta,
			buffer->buffer_trace.heap_alloc1_cost,
			buffer->buffer_trace.heap_drain_cost,
			buffer->buffer_trace.heap_alloc2_cost);
		__print_cur_mem_state();
#ifdef CONFIG_MM_PAGE_TRACE
		mm_ion_proecss_info();
#endif
