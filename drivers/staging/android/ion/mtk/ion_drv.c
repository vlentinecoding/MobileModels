...
#define MAX_WATER_MARK (SZ_1M * 220) /* 220MB */
...
static long ion_query_arch_type(struct ion_client *client, unsigned int cmd,
			  unsigned long arg, int from_kernel)
{
	struct ion_arch_data arch_data;

	if (from_kernel)
		return -EINVAL;

	arch_data.arch_type = ARCH_TYPE_MTK;
	return copy_to_user((void __user *)arg, &arch_data,
					sizeof(struct ion_arch_data));
}
#endif
		       ...
		       unsigned long arg, int from_kernel)
{
	...
	case ION_CMD_MULTIMEDIA:
		...
		break;
#ifdef CONFIG_HW_SECMEM
	case ION_CMD_QUERY_ARCH_TYPE:
		ret = ion_query_arch_type(client, cmd, arg, from_kernel);
		break;
#endif
	}
	ION_FUNC_LEAVE;
...
static int ion_drv_probe(struct platform_device *pdev)
{
	...
	if (sysfs_create_group(&pdev->dev.kobj, &sys_pool_attr_group))
