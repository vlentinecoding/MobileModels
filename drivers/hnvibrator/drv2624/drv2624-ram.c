...
#define OL_LRA_PERIOD_SCALE_NS 		(49230)
...
static void drv2624_parse_dt(struct device *dev, struct drv2624_data *pDRV2624)
{
	...
	int rc = 0;
	unsigned int value = 0, sampleTime = 300;
	...
	if (pPlatData->msActuator.mnActuatorType == LRA) {
		...
		dev_info(pDRV2624->dev, "ti,lra-frequency = %dHz ti,lra-period = %dms\n",
			pPlatData->msActuator.mnLRAFreq, pPlatData->msActuator.mnLRAPeriod);

		rc = of_property_read_u32(np, "ti,sample-time", &sampleTime);
		if (rc) {
			dev_err(pDRV2624->dev,
				"Looking up %s property in node %s failed %d\n",
		...
		dev_info(pDRV2624->dev, "ti,rated-voltage=%d, R=0x%02x\n",
			sampleTime, pPlatData->msActuator.mnSampleTimeReg);
		...
		pPlatData->msActuator.mnRatedVoltage =
		    value * int_sqrt(1000000-(4*sampleTime+300)*pPlatData->msActuator.mnLRAFreq)/2058;
		dev_info(pDRV2624->dev, "ti,rated-voltage=%d, R=0x%02x\n",
...
static void drv2624_ram_load(const struct firmware *pFW, void *pContext)
{
	...
	if (!pFW || !pFW->data || !pDRV26xx) {
		dev_info(pDRV26xx->dev, "%s:%u:Failed to read firmware\n",
			__func__, __LINE__);
		return;
	}
	...
	cur++;
	wvfm->rated_Voltage = buf[cur];
	cur++;
	wvfm->overDrive_Voltage = buf[cur];
	cur++;
	wvfm->lra_f0 = buf[cur];
	cur ++;
	...
	dev_info(pDRV26xx->dev, "rated_Voltage = 0x%02x\n", wvfm->rated_Voltage);
	dev_info(pDRV26xx->dev, "overDrive_Voltage = 0x%02x\n", wvfm->overDrive_Voltage);
...
static void drv2624_init(struct drv2624_data *pDRV2624)
{
	...
	drv2624_reg_write(pDRV2624, DRV2624_R0X1F_RATED_VOLTAGE,
		pActuator->mnRatedVoltage);

	drv2624_reg_write(pDRV2624, DRV2624_R0X20_OD_CLAMP,
	...
	if (pActuator->mnActuatorType == LRA) {
		...
		openLoopPeriod =
			(unsigned short)((unsigned int)1000000000 /
					(24619 * pActuator->mnLRAFreq));

		drv2624_set_bits(pDRV2624, DRV2624_R0X2E_OL_LRA_PERIOD_H, 0x03,
...
static int drv2624_get_calibration_result(struct drv2624_data *pDRV2624)
{
	...
	if(lra_reg) {
		...
		ol_lra_reg = (OL_LRA_PERIOD_FACTOR*lra_reg*LRA_PERIOD_SCALE_NS)/OL_LRA_PERIOD_SCALE_NS + 1;
		pDRV2624->mAutoCalData.mnOL_LraPeriod.msb =
		...
		if(pDRV2624) {
			...
			switch (pDRV2624->mAutoCalData.mnDoneFlag) {
				case DRV2624_R0X01_STATUS_DIAG_RESULT_NOK:
					len = scnprintf(buf, 32, "0");
					break;
				case 0xFF:
					len = scnprintf(buf, 32, "non-calibration!\n");
					break;
				default:
					len = scnprintf(buf, 32, "1");
					break;
			}
		}else {
			...
			switch (pDRV2624->mAutoCalData.mnDoneFlag) {
				...
				default: {
					int f0 = (pDRV2624->mAutoCalData.mnOL_LraPeriod.msb << 8
						| pDRV2624->mAutoCalData.mnOL_LraPeriod.lsb) * OL_LRA_PERIOD_SCALE_NS;
					if(f0 != 0) {
						f0 /= 10;
						f0 = OL_LRA_PERIOD_SCALE_UNIT / f0;
						len = scnprintf(buf, 32, "%d\n", f0);
					} else {
...
static DEVICE_ATTR(vibrator_calib, 0664, drv2624_calib_show, drv2624_calib_store);
...
static DEVICE_ATTR(cali_val, 0664, drv2624_f0_data_show, NULL);
...
static int __init drv2624_i2c_init(void)
{
	...
	pr_info("%s:drv2624 driver i2c_init\n", __func__);
