...
#ifdef CONFIG_HONOR_RTC
static void rtc_alarm_task(struct work_struct *work)
{
	struct pm8xxx_rtc *rtc =
		container_of(work, struct pm8xxx_rtc, alarm_task);
	dev_warn(rtc->rtc_dev, "rtc_alarm_task kobject_uevent\n");
	kobject_uevent(&rtc->rtc_dev->kobj, KOBJ_CHANGE);
}
#endif /* CONFIG_HONOR_RTC */
...
#ifdef CONFIG_HONOR_RTC
	schedule_work(&rtc_dd->alarm_task);
#endif /* CONFIG_HONOR_RTC */
rtc_alarm_handled:
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_HONOR_RTC
	INIT_WORK(&rtc_dd->alarm_task, rtc_alarm_task);
#endif /* CONFIG_HONOR_RTC */
