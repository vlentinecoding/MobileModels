...
static int lcdkit_backlight_ic_read_byte(struct lcdkit_bl_ic_device *pbl_ic,
	u8 reg, u8 *pdata)
{
	int ret;
...
static int lcdkit_backlight_ic_write_byte(struct lcdkit_bl_ic_device *pbl_ic,
	u8 reg, u8 data)
{
	int ret;
...
static int lcdkit_backlight_ic_update_bit(struct lcdkit_bl_ic_device *pbl_ic,
...
static int lcdkit_backlight_ic_config_reg(struct lcdkit_bl_ic_device *pbl_ic,
	u8 reg, u8 mask, u8 data)
{
	int ret = 0;
...
int lcdkit_backlight_ic_set_hidden_reg(void)
{
	...
	ret = lcdkit_backlight_ic_write_byte(plcdkit_bl_ic,
		plcdkit_bl_ic->bl_config.security_reg_enable_cmd.cmd_reg,
		plcdkit_bl_ic->bl_config.security_reg_enable_cmd.cmd_val);
	if (ret < 0) {
		LCDKIT_ERR("lcdkit backlight ic set security reg enable failed\n");
		goto out;
	}
	ret = lcdkit_backlight_ic_read_byte(plcdkit_bl_ic,
		plcdkit_bl_ic->bl_config.hidden_reg_cmd.cmd_reg, &readval);
	if (ret < 0) {
		LCDKIT_ERR("lcdkit backlight ic read hidden reg failed\n");
		goto out;
	}

	if (plcdkit_bl_ic->bl_config.hidden_reg_val_mask !=
	...
	for (i = 0; i < plcdkit_bl_ic->bl_config.num_of_init_cmds; i++) {
		...
		case 0:
			ret = lcdkit_backlight_ic_read_byte(plcdkit_bl_ic,
				...
				&(plcdkit_bl_ic->bl_config.init_cmds[i].cmd_val));
			break;
		case 1:
			ret = lcdkit_backlight_ic_write_byte(plcdkit_bl_ic,
				plcdkit_bl_ic->bl_config.init_cmds[i].cmd_reg,
		...
		case 2:
			ret = lcdkit_backlight_ic_update_bit(plcdkit_bl_ic,
				plcdkit_bl_ic->bl_config.init_cmds[i].cmd_reg,
				plcdkit_bl_ic->bl_config.init_cmds[i].cmd_mask,
				plcdkit_bl_ic->bl_config.init_cmds[i].cmd_val);
			break;
		default:
			break;
...
int lcdkit_backlight_ic_set_brightness(unsigned int level)
{
...
#if defined (CONFIG_HUAWEI_DSM)
	if ((level == 0) && (bl_backlight_off_flag == false)) {
	...
	level_msb = (level >> plcdkit_bl_ic->bl_config.bl_lsb_reg_cmd.val_bits) &
		plcdkit_bl_ic->bl_config.bl_msb_reg_cmd.cmd_mask;
	LCDKIT_DEBUG("[backlight] level_lsb is 0x%x , level_msb is 0x%x \n",
		level_lsb, level_msb);
	if (plcdkit_bl_ic->bl_config.bl_lsb_reg_cmd.val_bits != 0) {
		ret = lcdkit_backlight_ic_write_byte(plcdkit_bl_ic,
			plcdkit_bl_ic->bl_config.bl_lsb_reg_cmd.cmd_reg, level_lsb);
		if(ret < 0) {
	...
	ret = lcdkit_backlight_ic_write_byte(plcdkit_bl_ic,
		plcdkit_bl_ic->bl_config.bl_msb_reg_cmd.cmd_reg, level_msb);
	if (ret < 0) {
		LCDKIT_ERR("set backlight ic brightness failed!\n");
#if defined (CONFIG_HUAWEI_DSM)
		s_brightness_set_fail_count++;
		if (s_brightness_set_fail_count >=
			DMD_SET_BRIGHTNESS_FAIL_EXPIER_COUNT) {
			if (lcd_dclient && !dsm_client_ocuppy(lcd_dclient)) {
				dsm_client_record(lcd_dclient,
					"set backlight ic brightness failed! backlight_level = %d, cmd_reg = 0x%x, chip_name = %s.\n",
...
#if defined (CONFIG_HUAWEI_DSM)
	if ((level != 0) && (bl_backlight_off_flag == true)) {
...
int lcdkit_backlight_ic_enable_brightness(void)
{
	...
	if (plcdkit_bl_ic == NULL)
		return -1;
	ret = lcdkit_backlight_ic_update_bit(plcdkit_bl_ic,
		plcdkit_bl_ic->bl_config.bl_enable_cmd.cmd_reg,
		plcdkit_bl_ic->bl_config.bl_enable_cmd.cmd_mask,
		plcdkit_bl_ic->bl_config.bl_enable_cmd.cmd_val);
	if (ret < 0)
...
int lcdkit_backlight_ic_disable_brightness(void)
{
	...
	if (plcdkit_bl_ic == NULL)
		return -1;
	ret = lcdkit_backlight_ic_update_bit(plcdkit_bl_ic,
		plcdkit_bl_ic->bl_config.bl_disable_cmd.cmd_reg,
		plcdkit_bl_ic->bl_config.bl_disable_cmd.cmd_mask,
		plcdkit_bl_ic->bl_config.bl_disable_cmd.cmd_val);
	if (ret < 0)
...
int lcdkit_backlight_ic_disable_device(void)
{
	...
	if(plcdkit_bl_ic == NULL)
		return -1;
	ret = lcdkit_backlight_ic_update_bit(plcdkit_bl_ic,
		plcdkit_bl_ic->bl_config.disable_dev_cmd.cmd_reg,
		plcdkit_bl_ic->bl_config.disable_dev_cmd.cmd_mask,
		plcdkit_bl_ic->bl_config.disable_dev_cmd.cmd_val);
	if(ret < 0)
...
void lcdkit_parse_backlight_fault_check_param(struct device_node *pnp)
{
	...
	if(of_property_read_u32(pnp, tmp_buf, &tmp_32) || (tmp_32 == 0)){
...
ssize_t lcdkit_backlight_set_normal_mode(void)
{
	...
	if (tmp->bl_enhance_hrdtimer_time != 0)
...
static int lcdkit_backlight_ic_remove(struct i2c_client *client)
{
	if (plcdkit_bl_ic != NULL)
	{
		...
		if (plcdkit_bl_ic->bl_config.bl_wq_support)
		{
			if (plcdkit_bl_ic->bl_config.bl_enhance_hrdtimer_time != 0)
				hrtimer_cancel(&plcdkit_bl_ic->bl_resume_hrtimer);
			if (plcdkit_bl_ic->bl_resume_wq != NULL)
			{
