 ...
 * output: return the result of getting lcd model
...
/*
 * function:  this function is used to get the type of LCD
 * output: return the result of this function
 * @buf: get the type of lcd
 */

static ssize_t lcdkit_display_type_show(char* buf)
{
	int lcd_display_type;
 ...
 * input: the value of inversion mode
 * @pdata: this void point is used to converte to fb data struct.
 ...
 * input: the result of checking the status reg of LCD
 * @pdata: this void point is used to converte to fb data struct.
 * output: return the result of this function
 ...
 * input: the buffer to store gram status
 * @pdata: this void point is used to converte to fb data struct.
 * output: return the result of this function
 * @buf: output the check result, "OK" means SUCCESS, "FAIL" means FAIL
 */
static ssize_t lcdkit_gram_check_show(void* pdata, char* buf)
{
	...
	if (checksum_result && (lcdkit_info.panel_infos.checksum_pic_num == 2))
...
static ssize_t lcdkit_dynamic_sram_check_show(void* pdata, char* buf)
{
	...
	for (i = 0; i < LCDKIT_CHECKSUM_SIZE; i++) {
		...
		*data = 0x73 + i;
		lcdkit_dsi_rx(pdata, (rd + i), 0,
			&lcdkit_info.panel_infos.dynamic_sram_check_cmds);
...
/* mipi detect */
/* function: mipi detect
 * input: the buffer to store mipi detect
 * @pdata: this void point is used to converte to fb data struct.
 * output: the result of mipi detect
