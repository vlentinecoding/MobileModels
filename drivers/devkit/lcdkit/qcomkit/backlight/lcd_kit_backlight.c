...
static void lcd_kit_backlight_parse_uinit_cmds_config(struct device_node *np,
	...
	unsigned char reg, unsigned char data)
{
	...
	if (ret < 0)
		...
		LCD_DEV_DEBUG("register 0x%x value = 0x%x\n", reg, data);
	return ret;
}
	...
	const char *buf, size_t size)
{
	...
	if ((dev == NULL) || (attr == NULL) || (buf == NULL)) {
		LCD_ERR("input invalid\n");
		return size;
	}
	ret = sscanf(buf, "reg=0x%x",&reg_addr);
	if (ret < 0) {
		LCD_ERR("check input fail\n");
		return -EINVAL;
	}
	reg = reg_addr;
	return size;
	...
	const char *buf, size_t size)
{
	...
	if (ret < 0) {
		LCD_ERR("backlight i2c update register error\n");
		return ret;
	}
	return size;
}
	...
	const char *propertyname, struct backlight_ic_cmd *pcmd)
{
	...
	if ((np == NULL) || (propertyname == NULL) || (pcmd == NULL)) {
		LCD_ERR("input parameter is NULL\n");
		return LCD_FAIL;
	}
	...
	pcmd->cmd_mask = buf[3];
	pcmd->delay = buf[4];
	LCD_INFO("%s: type is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	lcd_kit_parse_backlight_param(np, "backlight_ic_uinit_vsp_index",
		&pbl_config->uinit_vsp_index);
	lcd_kit_parse_backlight_param(np, "backlight_ic_uinit_vsn_index",
		&pbl_config->uinit_vsn_index);
	...
	if (pbl_config->pull_down_boost_support)
		lcd_kit_backlight_parse_uinit_cmds_config(np, pbl_config);
}
	...
	const char *propertyname, struct backlight_reg_info *pcmd)
{
	...
	pcmd->cmd_val = buf[2];
	pcmd->cmd_mask = buf[3];
	LCD_INFO("%s: val_bits is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	if (ret) {
		LCD_ERR("get kernel init cmds config fail\n");
		kfree(buf);
		buf = NULL;
		return;
	}
	for (i = 0; i < pbl_config->num_of_init_cmds; i++) {
		...
		LCD_INFO("init i is %d, tpye is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	memset(&pbl_config->uinit_cmds[0], 0, LCD_BACKLIGHT_UINIT_CMD_NUM);
	lcd_kit_parse_backlight_param(np, "backlight_ic_num_of_kernel_uinit_cmd",
		&pbl_config->num_of_uinit_cmds);
	if (!pbl_config->num_of_uinit_cmds) {
		LCD_ERR("uinit cmd number is 0\n");
		return;
	}
	lcd_kit_backlight_propname_cat(tmp_buf, "backlight_ic_kernel_uinit_cmd",
		sizeof(tmp_buf));
	prop = of_find_property(np, tmp_buf, NULL);
	if (prop == NULL) {
		LCD_ERR("kernel init cmds is not config\n");
	...
	tmp_val = pbl_config->num_of_uinit_cmds;
	if (prop->length !=
		tmp_val * BACKLIGHT_IC_CMD_LEN * sizeof(unsigned int)) {
		LCD_ERR("init cmds number is not right\n");
		return;
	...
	for (i = 0; i < pbl_config->num_of_uinit_cmds; i++) {
		pbl_config->uinit_cmds[i].ops_type =
			buf[i * BACKLIGHT_IC_CMD_LEN];
		pbl_config->uinit_cmds[i].cmd_reg =
			buf[i * BACKLIGHT_IC_CMD_LEN + 1];
		pbl_config->uinit_cmds[i].cmd_val =
			buf[i * BACKLIGHT_IC_CMD_LEN + 2];
		pbl_config->uinit_cmds[i].cmd_mask =
			buf[i * BACKLIGHT_IC_CMD_LEN + 3];
		pbl_config->uinit_cmds[i].delay =
			buf[i * BACKLIGHT_IC_CMD_LEN + 4];
		LCD_INFO("uinit i is %d, tpye is 0x%x  reg is 0x%x  val is 0x%x   mask is 0x%x\n",
		 i, pbl_config->uinit_cmds[i].ops_type,
		 pbl_config->uinit_cmds[i].cmd_reg,
		 pbl_config->uinit_cmds[i].cmd_val,
		 pbl_config->uinit_cmds[i].cmd_mask);
	}
	...
	struct lcd_kit_backlight_info *pbl_config)
{
	...
	if (ret < 0) {
		LCD_ERR("parse base config fail\n");
		return;
	}
	...
	unsigned char *pos_votage, unsigned char *neg_votage)
{
	...
	for (i = 0; i < ARRAY_SIZE(backlight_bias_config); i++) {
		...
		if (j == backlight_bias_config[i].len) {
			LCD_INFO("not found vsp voltage, use default\n");
		...
		if (j == backlight_bias_config[i].len) {
			LCD_INFO("not found neg voltage, use default voltage\n");
	...
	if (i == ARRAY_SIZE(backlight_bias_config)) {
		LCD_INFO("not found right voltage, use default voltage\n");
		*pos_votage = DEFAULT_VOL_VAL;
		*neg_votage = DEFAULT_VOL_VAL;
	}
}
...
static int lcd_kit_backlight_ic_config(void)
{
	...
	LCD_INFO("backlight ic config\n");
	if (pbacklight_dev == NULL) {
		LCD_ERR(" backlight device is NULL\n");
		return LCD_FAIL;
	}
	...
	for (i = 0; i < pbacklight_dev->bl_config.num_of_init_cmds; i++) {
		...
		if (ret < 0) {
			LCD_ERR("operation  reg 0x%x fail\n",
				pbacklight_dev->bl_config.init_cmds[i].cmd_reg);
			return ret;
		}
...
{
	int ret;
	static int vpos_bak = 0;
	static int vneg_bak = 0;
	...
	if (vpos_bak != vpos && vneg_bak != vneg) {
	...
	if (ret < 0) {
		LCD_ERR("set config register failed");
		return ret;
	}

	LCD_INFO("set_bias is successful\n");
	return ret;
}

static void lcd_kit_backlight_disable(void)
{
	LCD_INFO("backlight ic disable enter\n");
	if (pbacklight_dev->bl_config.hw_en) {
		if(pbacklight_dev->bl_config.hw_en_pull_low)
			gpio_set_value(pbacklight_dev->bl_config.hw_en_gpio,
...
static int lcd_kit_backlight_set_ic_disable(void)
{
	...
	if (ret < 0)
		LCD_ERR("write REG_BL_ENABLE fail\n");
	...
	if (ret < 0)
		LCD_ERR("update_bits REG_BL_BRIGHTNESS_LSB fail\n");

	ret = lcd_kit_backlight_write_byte(pbacklight_dev,
		pbacklight_dev->bl_config.bl_msb_reg_cmd.cmd_reg, 0);
	if (ret < 0)
		LCD_ERR("write REG_BL_BRIGHTNESS_MSB fail!\n");
...
static int lcd_kit_backlight_set_bias_power_down(int vpos, int vneg)
{
	...
	if (vpos_bak != vpos && vneg_bak != vneg) {
		vsp_index = pbacklight_dev->bl_config.uinit_vsp_index;
		vsn_index = pbacklight_dev->bl_config.uinit_vsn_index;
		lcd_kit_backlight_get_bias_value(vpos, vneg, &vsp_vol, &vsn_vol);
		pbacklight_dev->bl_config.uinit_cmds[vsp_index].cmd_val = vsp_vol;
		pbacklight_dev->bl_config.uinit_cmds[vsn_index].cmd_val = vsn_vol;
		vpos_bak = vpos;
		vneg_bak = vneg;
	}

	for (i = 0; i < pbacklight_dev->bl_config.num_of_uinit_cmds; i++) {
		ret = lcd_kit_backlight_write_byte(pbacklight_dev,
			pbacklight_dev->bl_config.uinit_cmds[i].cmd_reg,
			pbacklight_dev->bl_config.uinit_cmds[i].cmd_val);
		if (ret < 0) {
			LCD_ERR("write reg 0x%x fail\n",
				pbacklight_dev->bl_config.uinit_cmds[i].cmd_reg);
			return ret;
		}
		if (pbacklight_dev->bl_config.uinit_cmds[i].delay)
			mdelay(pbacklight_dev->bl_config.uinit_cmds[i].delay);
	}

	LCD_INFO("lcd_kit_pull_boost is successful\n");
	return ret;
}
	...
	int last_level, int level)
{
	...
	if (ret < 0) {
		LCD_ERR("%s: read backlight ic ovp reg fail\n");
		return;
	}
	if ((val & pchip->bl_config.bl_ovp_flag_cmd.cmd_mask) ==
		pchip->bl_config.bl_ovp_flag_cmd.cmd_val) {
		LCD_INFO("backlight ic check ovp fault val is 0x%x\n", val);
		if (lcd_dclient == NULL)
			return;
		ret = dsm_client_ocuppy(lcd_dclient);
		if (ret != 0) {
			LCD_ERR("dsm_client_ocuppy fail, ret=%d\n", ret);
			return;
		}
	...
	int last_level, int level)
{
	...
	if (ret < 0) {
		LCD_ERR("read backlight ic ocp reg fail\n");
		return;
	}
	if ((val & pchip->bl_config.bl_ocp_flag_cmd.cmd_mask) ==
		pchip->bl_config.bl_ocp_flag_cmd.cmd_val) {
		LCD_INFO("backlight ic check ocp fault val is 0x%x\n", val);
		if (lcd_dclient == NULL)
			return;
		ret = dsm_client_ocuppy(lcd_dclient);
		if (ret != 0) {
	...
	int last_level, int level)
{
	...
	if (ret < 0) {
		LCD_ERR("read backlight ic tsd reg fail\n");
		return;
	}
	if ((val & pchip->bl_config.bl_tsd_flag_cmd.cmd_mask) ==
		pchip->bl_config.bl_tsd_flag_cmd.cmd_val) {
		LCD_INFO("backlight ic check tsd fault val is 0x%x\n", val);
		if (lcd_dclient == NULL)
			return;
		ret = dsm_client_ocuppy(lcd_dclient);
		if (ret != 0) {
	...
	int level)
{
	...
	if (pchip->bl_config.fault_check_enable &&
		...
		(last_level > 0 && level == 0))) {
		LCD_INFO("start backlight ic fault check\n");
...
int lcd_kit_set_backlight(unsigned int bl_level)
{
	...
	if(last_level == bl_level) {
		return ret;
	}
	last_level = bl_level;
	if (pbacklight_dev == NULL) {
		LCD_ERR("init fail, return\n");
		return ret;
	}
	if (down_trylock(&(pbacklight_dev->sem))) {
		LCD_INFO("Now in test mode\n");
		return ret;
	}
	...
	if (pbacklight_dev->bl_config.bl_ctrl_mod == BL_PWM_I2C_MODE) {
		LCD_INFO("backlight ic ctrl by mipi pwm, level is %u\n", level);
	...
	if (ret < 0) {
		LCD_ERR("set backlight ic brightness msb failed!\n");
		goto i2c_error;
	}
...
#endif
	up(&(pbacklight_dev->sem));
	LCD_INFO("lcd kit set backlight exit succ\n");
	return ret;

i2c_error:
	up(&(pbacklight_dev->sem));
	LCD_ERR("lcd kit set backlight exit fail\n");
	return ret;
}

int lcd_kit_able_backlight(bool enable)
{
	int ret = LCD_FAIL;
	...
	down(&(pbacklight_dev->sem));

	if (enable == 0) {
		ret = lcd_kit_backlight_write_byte(pbacklight_dev,
			pbacklight_dev->bl_config.bl_disable_cmd.cmd_reg,
			pbacklight_dev->bl_config.bl_disable_cmd.cmd_val);
		if (ret < 0) {
			LCD_ERR("set backlight ic brightness disenable failed!\n");
			goto i2c_error;
		}
	}else {
		ret = lcd_kit_backlight_write_byte(pbacklight_dev,
			pbacklight_dev->bl_config.bl_enable_cmd.cmd_reg,
			pbacklight_dev->bl_config.bl_enable_cmd.cmd_val);
		if (ret < 0) {
			LCD_ERR("set backlight ic brightness enable failed!\n");
			goto i2c_error;
		}
	}
	up(&(pbacklight_dev->sem));
	LCD_INFO("lcd kit %d backlight exit succ\n",enable);
	return ret;

i2c_error:
	up(&(pbacklight_dev->sem));
	LCD_ERR("lcd kit disable backlight exit fail\n");
	return ret;
}

static struct lcd_kit_bl_ops bl_ops = {
	.set_backlight = lcd_kit_set_backlight,
	.en_backlight = lcd_kit_able_backlight,
	.name = "bl_common",
};
	...
	const struct i2c_device_id *id)
{
	...
	LCD_INFO("enter\n");
	if (client == NULL) {
		LCD_ERR("client is null\n");
		return -EINVAL;
	}
	adapter = client->adapter;
	if (adapter == NULL) {
		LCD_ERR("adapter is NULL pointer\n");
		return -EINVAL;
	}
	...
	if (pbacklight_dev == NULL) {
		LCD_ERR("Failed to allocate memory\n");
		return -ENOMEM;
	}
	...
	if (backlight_class != NULL) {
		...
		if (IS_ERR(pbacklight_dev->dev)) {
			LCD_ERR("Unable to create device; errno = %ld\n", PTR_ERR(pbacklight_dev->dev));
			...
			if (ret) {
				LCD_ERR("Create backlight sysfs group node failed!\n");
...
{
	if (pbacklight_dev == NULL) {
		LCD_ERR("pbacklight_dev is null\n");
		return -EINVAL;
	}
	if (g_sysfs_created)
...
static int __init lcd_kit_backlight_init(void)
{
	...
	LCD_INFO("backlight_ic_name is %s\n", backlight_ic_name);
	...
	if (IS_ERR(backlight_class)) {
		LCD_ERR("Unable to create backlight class, errno = %ld\n", PTR_ERR(backlight_class));
		backlight_class = NULL;
	}
