...
{
	return lcd_inversion_get_mode(buf);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!pinfo->fact_info->inversion.support) {
		LCD_INFO("not support inversion\n");
		return count;
	}
	switch (val) {
	case COLUMN_MODE:
		if(plat_ops->send_column_inversion_cmd) {
			ret = plat_ops->send_column_inversion_cmd(pinfo->panel);
		}
		LCD_INFO("ret = %d", ret);
		break;
	case DOT_MODE:
		if(plat_ops->send_dot_inversion_cmd) {
			ret = plat_ops->send_dot_inversion_cmd(pinfo->panel);
		}
		LCD_INFO("ret = %d", ret);
		break;
	default:
	...
	pinfo->fact_info->inversion.mode = (int)val;
	LCD_INFO("inversion.support = %d, inversion.mode = %d\n",
		pinfo->fact_info->inversion.support, pinfo->fact_info->inversion.mode);
	return count;
}
...
int lcd_inversion_get_mode(char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE, "%d\n", pinfo->fact_info->inversion.mode);
	return ret;
}

static void parse_inversion(struct lcd_fact_info *fact_info,
	struct dsi_parser_utils *utils)
{
	/* lcd-inversion-support */
	fact_info->inversion.support = utils->read_bool(utils->data,
		"qcom,mdss-dsi-panel-lcd-inversion-enabled");
...
int factory_init(struct dsi_panel *panel, struct panel_info *pinfo)
{
	...
	parse_inversion(pinfo->fact_info, utils);
	return 0;
}
