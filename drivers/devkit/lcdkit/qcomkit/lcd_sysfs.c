...
/* panel bl max nit */
static char blmaxnit_info[LCD_BLMAXNIT_LEN] = "default";
...
static int lcd_get_project_id(struct panel_info *pinfo, char *oem_data, int len)
{
	...
	if(!plat_ops || !plat_ops->read_project_id) {
		LCD_ERR("pointer is null\n");
		return LCD_FAIL;
	}
	if (!pinfo->oeminfo.project_id.support) {
		LCD_INFO("not support project id\n");
		return LCD_FAIL;
	}
	if (!pinfo->power_on) {
	...
	plat_ops->read_project_id(pinfo->panel, read_value, OEM_INFO_SIZE_MAX - 1);
	...
	memcpy_s(pinfo->oeminfo.project_id.id, PROJECTID_LEN + 1, read_value, strlen(read_value)+1);
	LCD_INFO("read lcd project_id:%s", pinfo->oeminfo.project_id.id);
	return LCD_OK;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	panel = pinfo->panel;
	panel->bl_config.bl_actual_max_nit = maxnit;
...
static int  panel_maxnit_cmdline_parse(char *arg)
{
	int len;
	int i;
	maxnit = 0;
	...
	for( i = 0; i < len ; i++) {
		 if (arg[i] >= '0' && arg[i] <= '9') {
			maxnit = (arg[i] - '0') + maxnit * 10;
		}
	}
	LCD_INFO("panel_maxnit_cmdline_parse is %d\n", maxnit);
	return LCD_OK;
}

static int __init early_parse_panel_maxnit_cmdline(char *arg)
{
	int len;
	...
	memset(blmaxnit_info, 0, LCD_BLMAXNIT_LEN);
	len = strlen(arg);
	if (len > LCD_BLMAXNIT_LEN)
		len = LCD_BLMAXNIT_LEN - 1;
	memcpy(blmaxnit_info, arg, len);
	blmaxnit_info[LCD_BLMAXNIT_LEN - 1] = 0;
	panel_maxnit_cmdline_parse(blmaxnit_info);
	LCD_INFO("early_parse_panel_maxnit_cmdline\n");
	return LCD_OK;
}

early_param("panel_maxnit", early_parse_panel_maxnit_cmdline);
