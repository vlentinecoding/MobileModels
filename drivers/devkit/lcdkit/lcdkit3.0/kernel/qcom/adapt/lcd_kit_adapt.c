...
int qcom_mipi_dsi_cmd_tx(struct dsi_display *display, struct dsi_ctrl *dsi_ctrl,
	struct lcd_kit_dsi_cmd_desc *cmds, int link_state )
{
	...
	if ((display->ctrl_count > 1) && (!(cmd.msg.flags & MIPI_DSI_MSG_UNICAST)) &&
		(!(common_info->close_dual_dsi_sn))) {
		ret = dsi_display_broadcast_cmd(display, &cmd.msg);
	} else {
...
int qcom_mipi_dsi_cmds_transfer(struct dsi_display *display,
	struct dsi_ctrl *dsi_ctrl, unsigned char *out,
	...
	unsigned int len, struct lcd_kit_dsi_panel_cmds *cmds)
{
	...
	ret = qcom_mipi_dsi_cmds_transfer(display, m_ctrl->ctrl, out, len, cmds);
	if (ret  != LCD_KIT_OK)
...
int lcd_kit_dsi_cmds_tx(void *hld, struct lcd_kit_dsi_panel_cmds *cmds)
{
	...
	for (i = 0; i < cmds->cmd_cnt; i++)
		ret = qcom_mipi_dsi_cmd_tx(display, m_ctrl->ctrl, &cmds->cmds[i],
			cmds->link_state);
	dsi_display_cmd_engine_disable(display);
release_clk_ctrl:
	dsi_display_clk_ctrl(display->dsi_clk_handle, DSI_ALL_CLKS, DSI_CLK_OFF);
