...
int lcd_kit_enter_ddic_alpha(struct hisi_fb_data_type *hisifd, int alpha)
{
	int ret = LCD_KIT_OK;

	if (common_info->ddic_alpha.enter_alpha_cmds.cmds == NULL) {
		LCD_KIT_ERR("enter_alpha_cmds.cmds is NULL");
		return LCD_KIT_FAIL;
	}

	/* Sets the upper eight bits of the alpha. */
	common_info->ddic_alpha.enter_alpha_cmds.cmds[0].payload[1] =
		((alpha >> 8) & 0x0f) | 0x10;

	/* Sets the lower eight bits of the alpha. */
	common_info->ddic_alpha.enter_alpha_cmds.cmds[0].payload[2] = alpha & 0xff;
	ret = lcd_kit_dsi_cmds_tx(hisifd,
		&common_info->ddic_alpha.enter_alpha_cmds);
	if (ret != LCD_KIT_OK)
		LCD_KIT_ERR("lcd_kit_dsi_cmds_tx error");

	return ret;
}

int lcd_kit_exit_ddic_alpha(struct hisi_fb_data_type *hisifd, int alpha)
{
	int ret = LCD_KIT_OK;
...
u32 get_lcd_kit_alpha_support(void)
{
	return common_info->ddic_alpha.alpha_support;
}

u32 get_force_delta_bl_update_support(void)
{
