	...
	struct drm_file *file_priv)
{
	...
	if (common_info && common_info->sn_code.support) {
		...
		errno_t rc = memcpy_s(param->sn_code, sizeof(param->sn_code), lcd_kit_info.sn_code, sizeof(lcd_kit_info.sn_code));
		if (rc != EOK) {
			LCD_KIT_ERR("failed to memcpy_s sn_code\n");
		}
	}
