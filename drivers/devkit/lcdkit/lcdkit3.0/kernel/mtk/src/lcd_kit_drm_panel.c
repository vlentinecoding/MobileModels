...
static int lcm_disable(struct drm_panel *panel)
{
	...
	/* send status after close aod */
	send_aod_status(1);
	if (common_info->esd.acc_support)
		esd_check_enable_acc(false);
#endif
...
static int lcm_enable(struct drm_panel *panel)
{
	...
	if (disp_info->alpm_state == ALPM_OUT) {
		send_aod_status(2);
		if (common_info->esd.acc_support) {
			esd_check_enable_acc(true);
			common_info->esd.screen_status = 0;
		}
	}
#endif
	LCD_KIT_INFO("exit\n");
	...
	void *handle, unsigned int level)
{
	...
	if (common_info->backlight.need_prepare) {
		for (int i = 0; i < common_info->backlight.prepare.cmd_cnt; i++) {
	...
	if (ret < 0)
		...
		(strcmp("sns,tcs3701,1", sensor_name.name) == 0) || (strcmp("sns,stk3a6x,l", sensor_name.name) == 0))
		save_light_to_sensorhub(level, level);
#endif
	return LCD_KIT_OK;
}
...
static int lcm_change_fps_check(unsigned int vrefresh)
{
	long delta_time;
	struct timeval tv;
	...
	if (vrefresh == LCD_KIT_FPS_60
		|| disp_info->fps.fps_check_panel_on_delay <= 0
		|| disp_info->fps.fps_can_update_flag)
		return MTK_MODE_STATE_OK;

	memset(&record_tv, 0, sizeof(struct timeval));
	if (disp_info->quickly_sleep_out.support) {
	...
	ret = lcd_kit_get_thp_afe_status(&afe_tv);
	if (!ret) {
		LCD_KIT_ERR("lcd_kit_get_thp_afe_status is false, can not change fps!\n");
	...
	if (delta_time < disp_info->fps.fps_check_panel_on_delay) {
		LCD_KIT_ERR("panel on delay for fw is 0x%x ms, can not change fps\n", delta_time);
		return MTK_MODE_STATE_FAIL;
	} else {
	...
	unsigned int dst_mode, enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	...
	if (disp_info->fps.switch_delay_ms != 0) {
		if ((cur_m->vrefresh > dst_m->vrefresh) &&
			(stage == BEFORE_DSI_POWERDOWN)) {
			mode_switch_fps_change(panel, AFTER_DSI_POWERON, lcd_fps_scence);
			disp_info->fps.current_fps = dst_m->vrefresh;
			mdelay(disp_info->fps.switch_delay_ms);
			return LCD_KIT_OK;
		}
	}
...
void lcdkit_cwb_copy_done(void *buffer, int type)
{
	if (share_mem_virt == NULL) {
		LCD_KIT_ERR("lcdkit_cwb share_mem_virt is null\n");
		return;
	}
	u8 *data = buffer;
...
extern void mtk_drm_cwb_enable_wrapper(int x, int y, int w, int h);
...
#if defined CONFIG_MACH_MT6877
	g_lcdkit_cwb_width = (int)share_memory_param->width;
	g_lcdkit_cwb_height = (int)share_memory_param->height;
	mtk_drm_cwb_enable_wrapper((int)share_memory_param->x,
		(int)share_memory_param->y,
		(int)share_memory_param->width,
		(int)share_memory_param->height);
#else
...
#if (defined CONFIG_XHUB_ROUTER) || (defined CONFIG_MACH_MT6877)
...
static struct mtk_panel_funcs g_ext_funcs = {
	...
	.change_fps_check = lcm_change_fps_check,
};
...
static void lcm_init_ext_params(void)
{
	...
	struct lcd_kit_array_data *hbm_timing = NULL;
	...
	if (disp_info->fps.support) {
		...
		for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
			index = disp_info->fps.panel_support_fps_list.buf[i];
			timing = &disp_info->fps.fps_dsi_timming[index];
			hbm_timing = &disp_info->fps.hbm_timming[index];
			...
			if (timing == NULL || timing->buf == NULL) {
				...
				if (hbm_timing != NULL && hbm_timing->buf != NULL) {
					ext_params_list[index].vsync_time = hbm_timing->buf[0];
					ext_params_list[index].hbm_deadline = hbm_timing->buf[1];
				} else {
					ext_params_list[index].vsync_time = 0;
					ext_params_list[index].hbm_deadline = 0;
				}
				ext_params_list[index].hbm_en_time = disp_info->hbm_en_times;
				ext_params_list[index].hbm_dis_time = disp_info->hbm_dis_times;
				/* phy_timcon */
		...
		ext_params.vsync_time = pinfo->hbm_vsync_time;
		ext_params.hbm_deadline = pinfo->hbm_deadline;
...
extern void mtk_set_mipi_volt(unsigned int val);

void lcd_kit_set_mipi_volt(void)
{
	struct mtk_panel_info *pinfo = &lcd_kit_info;
	if (pinfo->mipi.rg_vrefsel_vcm_adjust != 0) {
		mtk_set_mipi_volt(pinfo->mipi.rg_vrefsel_vcm_adjust);
	}
}

void lcd_kit_set_kernel_init_level(struct device_node *np, int *level)
{
	static u32 bl_level = 0;

	if (np == NULL && level == NULL)
		return;
	if (np != NULL && bl_level == 0) {
		OF_PROPERTY_READ_U32_DEFAULT(np, "kernel_init_bl_level", &bl_level, 0);
	}
	if (level != NULL && bl_level != 0) {
		*level = (int)bl_level;
		LCD_KIT_INFO("%u\n", bl_level);
	}
}
...
int lcd_kit_init(void)
{
	...
	lcd_kit_set_kernel_init_level(np, NULL);
