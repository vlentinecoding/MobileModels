...
#ifdef CONFIG_DRM_MEDIATEK
		ret = lcd_kit_dsi_cmds_tx_compact(
			&disp_info->alpm.high_light_cmds, true);
#else
...
#ifdef CONFIG_DRM_MEDIATEK
		ret = lcd_kit_dsi_cmds_tx_compact(
			&disp_info->alpm.low_light_cmds, true);
#else
		ret = lcd_kit_dsi_cmds_extern_tx(
...
#ifdef CONFIG_DRM_MEDIATEK
		ret = lcd_kit_dsi_cmds_tx_compact(
			&disp_info->alpm.middle_light_cmds, true);
#else
		ret = lcd_kit_dsi_cmds_extern_tx(
...
void lcd_kit_pinfo_init(struct device_node *np, struct mtk_panel_info *pinfo)
{
	...
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,hbm-vsync-time",
			&pinfo->hbm_vsync_time, 0);
	OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,hbm-deadline",
			&pinfo->hbm_deadline, 0);
	...
	if (common_info->esd.support) {
		...
		OF_PROPERTY_READ_U32_DEFAULT(np, "lcd-kit,sensor-esd-vol-support",
			&common_info->esd.acc_support, 0);
		if (common_info->esd.acc_support) {
			lcd_kit_parse_dcs_cmds(np, "lcd-kit,reduce-vol-for-esd-cmds",
				"lcd-kit,esd-vol-cmds-state",
				&common_info->esd.reduce_vol_cmds);
			lcd_kit_parse_dcs_cmds(np, "lcd-kit,recover-vol-for-esd-cmds",
				"lcd-kit,esd-vol-cmds-state",
				&common_info->esd.recover_vol_cmds);
		}
	}
...
void lcd_kit_parse_fps(struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,fps-switch-delay-ms",
		&disp_info->fps.switch_delay_ms, 0);
	...
	for (i = 0; i < disp_info->fps.panel_support_fps_list.cnt; i++) {
		...
		if (index == LCD_FPS_SCENCE_60) {
			...
			lcd_kit_parse_array_data(np, "lcd-kit,fps-normal-60-hbm-timming",
				&disp_info->fps.hbm_timming[index]);
			/* hopping */
			...
			lcd_kit_parse_array_data(np, "lcd-kit,fps-high-60-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
			lcd_kit_parse_array_data(np, "lcd-kit,fps-high-60-hbm-timming",
				&disp_info->fps.hbm_timming[index]);
			/* hopping */
			if (disp_info->fps.hop_support)
			...
			lcd_kit_parse_array_data(np, "lcd-kit,fps-90-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
			lcd_kit_parse_array_data(np, "lcd-kit,fps-90-hbm-timming",
				&disp_info->fps.hbm_timming[index]);
			/* hopping */
			if (disp_info->fps.hop_support)
			...
			lcd_kit_parse_array_data(np, "lcd-kit,fps-120-dsi-timming",
				&disp_info->fps.fps_dsi_timming[index]);
			lcd_kit_parse_array_data(np, "lcd-kit,fps-120-hbm-timming",
				&disp_info->fps.hbm_timming[index]);
			/* hopping */
			if (disp_info->fps.hop_support)
...
void lcd_kit_parse_util(struct device_node *np)
{
	...
	lcd_kit_parse_u32(np, "lcd-kit,hbm-en-times",
		&disp_info->hbm_en_times, 0);
	lcd_kit_parse_u32(np, "lcd-kit,hbm-dis-times",
		&disp_info->hbm_dis_times, 0);
	/* lcd_color_mode set*/
	...
	/* debug wr lock */
	lcd_kit_parse_u32(np, "lcd-kit,mtk-debug-wr-lock-support",
		&disp_info->mtk_debug_wr_lock_support, 0);
	lcd_kit_parse_u32(np, "lcd-kit,mtk-eof-timing-support",
		&disp_info->mtk_eof_timing_support, 0);
}
	...
	uint32_t *expect_val, int cnt, uint32_t compare_mode)
{
	...
	if (compare_mode == PCD_COMPARE_MODE_EQUAL) {
		...
		if (exp_pcd_value_mask != 0) {
			if (((uint32_t)read_val[0] & exp_pcd_value_mask) < expect_val[0])
				return LCD_KIT_FAIL;
		} else if ((uint32_t)read_val[0] < expect_val[0])
			return LCD_KIT_FAIL;
...
static int set_mipi_bl_level(struct hbm_type_cfg hbm_source, uint32_t level)
{
	...
	if (hbm_source.source == HBM_FOR_FP) {
		...
		if (common_info->backlight.need_prepare)
			cb_tx(hbm_source.dsi, hbm_source.cb, hbm_source.handle,
...
static int process_done = 1;

static void lcd_reduce_vol_for_esd_fn(struct work_struct *work)
{
	int ret;
	char err_info[DMD_ERR_INFO_LEN] = {0};
	...
	LCD_KIT_INFO("+\n");
	(void)work;
	ret = lcd_kit_dsi_cmds_extern_tx(&common_info->esd.reduce_vol_cmds);

	ret = snprintf(err_info, DMD_ERR_INFO_LEN,
		"esd trigger lcd_reduce_vol_for_esd\n");
	lcd_dmd_report_err(DSM_LCD_ESD_RECOVERY_NO, err_info,
		DMD_ERR_INFO_LEN);

	LCD_KIT_INFO("-\n");
	process_done = 1;
}
static DECLARE_DELAYED_WORK(lcd_reduce_vol_for_esd, lcd_reduce_vol_for_esd_fn);

static void lcd_recover_vol_for_esd_fn(struct work_struct *work)
{
	int ret;
	char err_info[DMD_ERR_INFO_LEN] = {0};
	...
	(void)work;
	LCD_KIT_INFO("+\n");
	ret = lcd_kit_dsi_cmds_extern_tx(&common_info->esd.recover_vol_cmds);

	ret = snprintf(err_info, DMD_ERR_INFO_LEN,
		"esd recover lcd_recover_vol_for_esd\n");
        lcd_dmd_report_err(DSM_LCD_ESD_RECOVERY_NO, err_info,
                DMD_ERR_INFO_LEN);
...
static DECLARE_DELAYED_WORK(lcd_recover_vol_for_esd, lcd_recover_vol_for_esd_fn);

void trigger_lcd_esd(void)
{
	if (!common_info->esd.acc_support)
		return;

	LCD_KIT_DEBUG("+\n");
	if (lcm_get_panel_state() && process_done &&
		(common_info->esd.screen_status == 0)) {
		schedule_delayed_work(&lcd_reduce_vol_for_esd, 0);
		common_info->esd.screen_status = 1;
	}
	LCD_KIT_DEBUG("-\n");
}

void recover_lcd_esd(void)
{
	if (!common_info->esd.acc_support)
		return;
	...
	if (lcm_get_panel_state() && process_done &&
		(common_info->esd.screen_status == 1)) {
		schedule_delayed_work(&lcd_recover_vol_for_esd, 0);
		common_info->esd.screen_status = 0;
	}
	LCD_KIT_DEBUG("-\n");
}

bool acc_esd_support(void)
{
	if (common_info->esd.acc_support)
		return true;
	return false;
