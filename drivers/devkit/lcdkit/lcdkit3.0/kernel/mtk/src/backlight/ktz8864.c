...
static void ktz8864_get_target_voltage(int *boost, int *vpos, int *vneg)
{
	...
	for (i = 0; i < tal_size; i++) {
		...
		if (voltage_table[i].voltage == power_hdl->ktz8864_bl_boost) {
			*boost = voltage_table[i].value;
			break;
		}
	}
...
static int ktz8864_chip_init(struct ktz8864_chip_data *pchip)
{
	...
	if (bl_info.bl_only == 0) {
		...
		if (boost_target > 0) {
			if (ktz8864_reg[8] != boost_target)
				ktz8864_reg[8] = boost_target;
		}
