...
static struct lm36274_voltage lm36274_vol_table[] = {
	...
	{ 6100000, LM36274_VOL_610 },
	{ 6300000, LM36274_VOL_630 },
...
static void lm36274_get_target_voltage(int *boost, int *vpos, int *vneg)
{
	int i;
	...
	for (i = 0; i < tal_size; i++) {
		/* set boost bias voltage value */
		if (lm36274_vol_table[i].voltage == power_hdl->lm36274_bl_boost) {
			*boost = lm36274_vol_table[i].value;
			break;
		}
	}

	if (i >= tal_size) {
		LCD_KIT_INFO("not found boost bias voltage, use default voltage\n");
	}
	...
	LCD_KIT_INFO("*boost_target=0x%x,*vpos_target=0x%x,*vneg_target=0x%x\n", *boost, *vpos, *vneg);
...
static int lm36274_chip_init(struct lm36274_chip_data *pchip)
{
	...
	if (bl_info.bl_only == 0) {
		lm36274_get_target_voltage(&boost_target, &vpos_target, &vneg_target);
		/* 8  is the position of boost in lm36274_reg */
		/* 9  is the position of vsp in lm36274_reg */
		/* 10 is the position of vsn in lm36274_reg */
		if (boost_target > 0) {
			if (lm36274_reg[8] != boost_target)
				lm36274_reg[8] = boost_target;
		}
