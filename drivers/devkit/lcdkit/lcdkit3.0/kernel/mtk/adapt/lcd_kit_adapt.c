...
#ifdef CONFIG_DRM_MEDIATEK
bool get_lcd_kit_te_after_esd_cabc_support(void) {
	if (disp_info->mtk_eof_timing_support)
		return true;
	return false;
}

bool get_lcd_kit_debug_wr_support(void) {
	if (disp_info->mtk_debug_wr_lock_support)
		return true;
	return false;
}

void mtk_get_crtc_lock_for_debug_wr(void) {
	if (disp_info->mtk_debug_wr_lock_support)
		mtk_get_crtc_commit_lock_ex();
}

void mtk_get_crtc_unlock_for_debug_wr(void) {
	if (disp_info->mtk_debug_wr_lock_support)
		mtk_get_crtc_commit_unlock_ex();
}
	...
	int cnt, int link_state, bool lock_state)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		if (lock_state)
			if (get_lcd_kit_debug_wr_support())
	...
	struct lcd_kit_dsi_panel_cmds *cmds, unsigned int len)
{
	...
	mtk_get_crtc_lock_for_debug_wr();
	for (i = 0; i < cmds->cmd_cnt; i++) {
		if (lcd_kit_cmd_is_write(&cmds->cmds[i])) {
			...
			if (ret < 0) {
				...
				mtk_get_crtc_unlock_for_debug_wr();
				return LCD_KIT_FAIL;
			}
		} else {
	...
	bool lock_state)
{
	...
	ret = mtk_ddic_dsi_send_cmd(&cmd_msg, true, lock_state);
	if (ret != 0) {
