...
static void ktz8864_get_target_voltage(int *boost, int *vpos, int *vneg)
{
	int i;
	...
	if ((vpos == NULL) || (vneg == NULL)) {
        ...
        for (i = 0; i < tal_size; i++) {
                /* set boost bias value */
                if (ktz8864_vol_table[i].voltage == power_hdl->ktz8864_bl_boost) {
                        *boost = ktz8864_vol_table[i].value;
                        break;
                }
        }
...
static int ktz8864_parse_dts(void)
{
	...
	if(bl_info.bl_only == 0) {
		ktz8864_get_target_voltage(&boost_target, &vpos_target, &vneg_target);
		/* 8  is the position of boost in ktz8864_reg */
		/* 9  is the position of vsp in ktz8864_reg */
		/* 10 is the position of vsn in ktz8864_reg */
		if (boost_target > 0) {
			if (bl_info.ktz8864_reg[8] != boost_target)
				bl_info.ktz8864_reg[8] = boost_target;
		}
