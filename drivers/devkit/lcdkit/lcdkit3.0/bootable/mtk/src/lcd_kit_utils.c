...
static void lcd_kit_parse_power(const void *fdt, int nodeoffset)
{
	...
	if (power_hdl->lcd_iovcc.buf == NULL)
		lcd_kit_parse_dts_array(fdt, nodeoffset, "lcd-kit,lcd-iovcc",
			&power_hdl->lcd_iovcc);
	/* ktz8864 bl boost bias*/
	if (power_hdl->ktz8864_bl_boost == NULL)
		lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,ktz8864-bl-boost",
			&power_hdl->ktz8864_bl_boost, 0);
	/* lm36274 bl boost bias*/
	if (power_hdl->lm36274_bl_boost == NULL)
		lcd_kit_get_dts_u32_default(fdt, nodeoffset, "lcd-kit,lm36274-bl-boost",
			&power_hdl->lm36274_bl_boost, 0);
	/* vsp */
...
static void lcd_kit_parse_power_seq(const void *fdt, int nodeoffset)
{
	...
	if (common_info->lk_power_on_early)
		lcd_kit_parse_dts_arrays(fdt, nodeoffset,
