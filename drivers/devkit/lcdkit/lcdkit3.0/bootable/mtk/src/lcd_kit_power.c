...
#define LDO_BEGIN 1
#define LDO_MAX 5
#define BUCK_OFFSET_BASE 1000000
#define BUCK_OFFSET_MT6315_MIN BUCK_OFFSET_BASE
#define BUCK_OFFSET_MT6315_MAX (BUCK_OFFSET_MT6315_MIN + 100)

static struct str_ldo_desc g_ldo_enable[] = {
	/* ldo num, volt_value, enable_addr, enable_data, enable_mask, enable_shift */
#ifdef MTK_5G_B_MT6360_MT6315
#if defined(CONFIG_MACH_MT6877)
	{ 1000004, 1193750, MT6315_PMIC_RG_BUCK_VBUCK4_EN_ADDR,
	0x1, MT6315_PMIC_RG_BUCK_VBUCK4_EN_MASK,
	MT6315_PMIC_RG_BUCK_VBUCK4_EN_SHIFT },
#endif
#endif
	...
	{ 2, 1800000, 0x11, 0x1, 0x1, 0x6 },
	{ 5, 3000000, 0x0b, 0x1, 0x1, 0x6 },
#endif
...
static struct str_ldo_desc g_ldo_volt[] = {
...
#ifdef MTK_5G_B_MT6360_MT6315
#if defined(CONFIG_MACH_MT6877)
	{ 1000004, 1193750, MT6315_PMIC_RG_BUCK_VBUCK4_VOSEL_ADDR,
	0xBF, MT6315_PMIC_RG_BUCK_VBUCK4_VOSEL_MASK,
	MT6315_PMIC_RG_BUCK_VBUCK4_VOSEL_SHIFT },
#endif
#endif
#ifdef MTK_MT6358_LDO_SUPPORT
	/* ldo1 3v */
	...
	{ 2, 1800000, 0x15, 0x4, 0xf, 0x4 },
	{ 5, 3000000, 0x0f, 0x3, 0x7, 0x4 },
#endif
#if defined(CONFIG_MACH_MT6885) && defined(MTK_MT6360_PMIC_SUPPORT)
	/* ldo6 1v8 */
...
int mt6315_ap_power_ctrl(unsigned int addr,
		unsigned char val, unsigned short mask, unsigned short shift)
{
#ifdef MTK_5G_B_MT6360_MT6315
#if defined(CONFIG_MACH_MT6877)
	int ret;
	struct mt6315_chip *chip = mt6315_find_chip(MT6315_S6);
	if (chip == NULL) {
		LCD_KIT_ERR("can not find MT6315_S6\n");
		return LCD_KIT_FAIL;
	}
	ret = mt6315_write_field_byte(chip->spmi_dev, addr, val, mask, shift);
	if (ret != 0) {
		LCD_KIT_ERR("MT6315_S6 write fail\n");
		return LCD_KIT_FAIL;
	}
	return LCD_KIT_OK;
#else
...
static int lcd_kit_pmu_buck_ctrl(uint32_t array_index, uint32_t power_num)
{
	int ret;
	if (power_num > BUCK_OFFSET_MT6315_MIN &&
		power_num < BUCK_OFFSET_MT6315_MAX) {
		ret = mt6315_ap_power_ctrl(g_ldo_volt[array_index].ldo_addr,
			g_ldo_volt[array_index].ldo_data,
			g_ldo_volt[array_index].ldo_mask,
			g_ldo_volt[array_index].ldo_shift);
		if (ret != 0) {
			LCD_KIT_ERR("mt6315 volt set fail\n");
			return LCD_KIT_FAIL;
		}
		ret = mt6315_ap_power_ctrl(g_ldo_enable[array_index].ldo_addr,
			g_ldo_enable[array_index].ldo_data,
			g_ldo_enable[array_index].ldo_mask,
			g_ldo_enable[array_index].ldo_shift);
		if (ret != 0) {
			LCD_KIT_ERR("mt6315 enable fail\n");
			return LCD_KIT_FAIL;
		}
	} else {
		LCD_KIT_ERR("unknown power_num:%d\n", power_num);
		return LCD_KIT_FAIL;
	}
	LCD_KIT_INFO("[vol]power_num=%d,ldo_addr = 0x%x,"
			"ldo_data = 0x%x,ldo_mask = 0x%x, ldo_shift = 0x%x\n",
			power_num,
			g_ldo_volt[array_index].ldo_addr,
			g_ldo_volt[array_index].ldo_data,
			g_ldo_volt[array_index].ldo_mask,
			g_ldo_volt[array_index].ldo_shift);
	LCD_KIT_INFO("[enable]power_num=%d,ldo_addr = 0x%x,"
			"ldo_data = 0x%x,ldo_mask = 0x%x, ldo_shift = 0x%x\n",
			power_num,
...
int lcd_kit_pmu_ctrl(uint32_t type, uint32_t enable)
{
	...
	if (power_num > BUCK_OFFSET_BASE)
		return lcd_kit_pmu_buck_ctrl(array_index, power_num);
