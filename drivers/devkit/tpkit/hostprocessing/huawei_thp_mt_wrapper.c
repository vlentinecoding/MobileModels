...
static int thp_mt_wrapper_ioctl_get_res_scale(unsigned long arg)
{
	...
	if (copy_to_user(res_scale, &cd->res_scale, sizeof(cd->res_scale))) {
		thp_log_err("%s:copy res_scale failed\n", __func__);
		return -EFAULT;
	}
...
#endif
#if ((defined CONFIG_HUAWEI_SHB_THP) || (defined CONFIG_HUAWEI_THP_MTK) || (IS_ENABLED(CONFIG_TP_QCOM_7XX_8XX)))
...
static int thp_ioctl_set_stylus3_connect_status(unsigned long arg)
{
	...
	if (cd->support_pen_wakeup_gesture ||
		((cd->send_bt_status_to_fw) && (cd->support_dual_chip_arch)))
	...
	unsigned long arg)
{
	...
	case INPUT_MT_WRAPPER_IOCTL_GET_RES_SCALE:
		ret = thp_mt_wrapper_ioctl_get_res_scale(arg);
		break;
...
int thp_mt_wrapper_init(void)
{
	...
	if (cd->support_b_protocol) {
		...
		thp_log_info("%s:type b protocol init\n", __func__);
	}
