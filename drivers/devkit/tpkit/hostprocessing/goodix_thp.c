...
static unsigned int g_goodix_esd_status = 0;
	...
	while (retry < SPI_READ_RETRY_TIME) {
		...
		if (ret < 0) {
			...
			if ((gdix_set_mode.lowpower_mode == 1) && cd->support_lowpower_no_irq) {
				thp_log_err("In lowpower state\n");
				g_goodix_esd_status = 1;
			}
			if (cd->suspended) {
				retry++;
				msleep(SPI_DELAY_MS);
...
exit:
	thp_log_info("%s: command state ack info: 0x%x\n", __func__, cmd_ack);
	return ret;
}
	...
	struct thp_core_data *cd)
{
	...
	if (!rc) {
		...
		thp_log_info("%s: support_lowpower_no_irq %u\n", __func__, value);
	}
	return rc;
}
...
static int touch_driver_communication_check_for_9966(struct thp_device *tdev)
{
	...
	thp_log_info("rom_pid:%s\n", temp_buf);
	thp_log_info("rom_vid:%*ph\n", (int)sizeof(module_info_for_gt9966.rom_vid),
		module_info_for_gt9966.rom_vid);
	memcpy(temp_buf, module_info_for_gt9966.patch_pid, sizeof(module_info_for_gt9966.patch_pid));
	thp_log_info("patch_pid:%s\n", temp_buf);
	thp_log_info("patch_vid:%*ph\n", (int)sizeof(module_info_for_gt9966.patch_vid),
...
{
	return touch_driver_switch_cmd(tdev, CMD_AOD_EVENT_SWITCH_GT9897S, state,
	...
	u8 state)
{
	...
	if ((state == 0) && cd->support_lowpower_no_irq && g_goodix_esd_status) {
		touch_driver_switch_cmd(tdev, 0xC0, ENABLE, SEND_COMMAND_MULTIPLE_RETRY);
		thp_log_err("status reset\n");
		msleep(200);
		ret = touch_driver_suspend_ap(tdev);
		g_goodix_esd_status = 0;
		return ret;
	}
	...
	struct thp_udfp_data *udfp_data)
{
	...
	if (cd->support_lowpower_no_irq && g_goodix_esd_status) {
		thp_log_err("%s: No processing in lowpower state\n", __func__);
		return -EFAULT;
	}
	...
	if ((ret < 0) || le32_data_check(frame_buf, sizeof(frame_buf))) {
		...
		if (tdev->thp_core->event_anomaly_count > EVENT_ANOMALY_MAX_COUNT) {
			thp_log_err("event_anomaly_count++ %u, self recovery\n",
				tdev->thp_core->event_anomaly_count);
...
#else
int goodix_driver_module_init(void)
#endif
{
	int rc;
