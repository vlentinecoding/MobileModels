...
#if ((defined CONFIG_FB) && (!IS_ENABLED(CONFIG_HUAWEI_THP_QCOM)) && (!defined CONFIG_HUAWEI_THP_MTK))
#include "hisi_fb.h"
#endif
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case STYLUS_GESTURE_WAKEUP_ENABLE:
		if (cd->support_pen_wakeup_gesture) {
			thp_log_info("%s:stylus gesture setting enable\n", __func__);
	...
	case STYLUS_GESTURE_WAKEUP_DISABLE:
		if (cd->support_pen_wakeup_gesture) {
			thp_log_info("%s:stylus gesture setting disable\n", __func__);
