	...
	int panel_index)
{
	...
	if ((cd->multi_screen_fold == true) && (panel_index == 0)) {
		thp_log_info("%s: in fold status, not support main panel resume, return\n", __func__);
		return 0;
	}
#endif
	thp_log_info("%s: called, index:%d, pm_type:%u\n", __func__,
...
const char *thp_get_vendor_name(void)
{
	...
	if (cd && cd->support_multi_panel_attach)
		return cd->thp_dev->multi_vendor_name;
...
static int thp_pinctrl_init(struct thp_device *dev)
{
	int rc;
	struct thp_core_data *cd = thp_get_core_data();
