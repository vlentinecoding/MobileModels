...
&& ((!defined CONFIG_LCD_KIT_DRIVER) && (!defined CONFIG_LCD_QCOM_DRV)))
#if (!IS_ENABLED(CONFIG_TP_QCOM_7XX_8XX))
#include "dsi_panel.h"
#endif
#endif

#if (!defined(CONFIG_LCD_KIT_DRIVER) && !defined(CONFIG_LCD_QCOM_DRV))
#include "../../lcdkit/lcdkit1.0/core/common/lcdkit_tp.h"
#endif
#if (!IS_ENABLED(CONFIG_HUAWEI_THP_QCOM)) && (!defined CONFIG_HUAWEI_THP_MTK)
...
#if (defined(CONFIG_HUAWEI_THP_MTK) || (IS_ENABLED(CONFIG_HUAWEI_THP_QCOM)))
...
extern int  syna_driver_module_init(void);
extern int  goodix_driver_module_init(void);
extern int nova_driver_module_init(void);
extern int ilitek_driver_module_init(void);
#endif
...
static bool touch_recovery_mode;
static bool g_touch_factory_mode = false;
int thp_spi_irq_flag;
EXPORT_SYMBOL(thp_spi_irq_flag);
#endif
static bool irq_storm_flag;
...
#define RUNMODE_TYPE  "androidboot.swtype"

#ifndef CONFIG_HONOR_GKI_V1
static int __init early_parse_recovery_cmdline(char *p)
{
	if (p != NULL)
		touch_recovery_mode = (strncmp(p, RECOVERY_ENTER,
			strlen(RECOVERY_ENTER)) == 0) ? true : false;
	return 0;
}
early_param("enter_recovery", early_parse_recovery_cmdline);

static int __init early_parse_runmode_type_cmdline(char *runmode_type)
{
	if (runmode_type) {
		if (!strncmp(runmode_type, "factory", strlen("factory")))
			g_touch_factory_mode = true;
		thp_log_info("%s, runmode_type = %u\n", runmode_type,
			g_touch_factory_mode);
	}
	return 0;
}
early_param(RUNMODE_TYPE, early_parse_runmode_type_cmdline);
#endif

#if ((defined CONFIG_HUAWEI_THP_MTK) || (IS_ENABLED(CONFIG_TP_QCOM_7XX_8XX)))
...
static long thp_ioctl_set_resume_status(unsigned long arg)
{
	struct thp_core_data *cd = thp_get_core_data();
	unsigned long status = arg;

	thp_log_info("%s:set resume status %lu\n", __func__, status);
	switch (status) {
	case THP_AFE_NOTIFY_RESUME_FINISH:
		if (cd->wait_afe_screen_on_support) {
			thp_log_info("%s: afe notify resume status\n", __func__);
			thp_wakeup_screenon_waitq(cd);
		}
		break;
	default:
	...
	unsigned long arg)
{
	...
	case THP_IOCTL_CMD_SET_RESUME_STATUS:
		ret = thp_ioctl_set_resume_status(arg);
		break;
...
#define QCOM_FINGER_EVENT_OFFSET 5
#if ((defined CONFIG_HUAWEI_THP_MTK) || (IS_ENABLED(CONFIG_TP_QCOM_7XX_8XX)))
static void send_event_to_fingerprint_ud(struct thp_udfp_data udfp_data)
{
	...
	udfp_data.tpud_data.udfp_event += QCOM_FINGER_EVENT_OFFSET;
	thp_log_debug("%s send screen off fingerprint event\n", __func__);
#endif
...
static void thp_irq_thread_suspend_process(struct thp_core_data *cd)
{
	...
	if (need_work_in_suspend_switch(cd) &&
		...
		thp_log_info("%s: gesture wakeup value is %u\n", __func__,
			gesture_wakeup_value);
...
#if ((defined CONFIG_HUAWEI_THP_MTK) || (IS_ENABLED(CONFIG_HUAWEI_THP_QCOM)))
__attribute__((weak)) int power_event_nc_register(unsigned int type,
	struct notifier_block *nb)
{
	return 0;
}

__attribute__((weak)) int power_event_nc_unregister(unsigned int type,
	struct notifier_block *nb)
{
	return 0;
}
	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	if (!rc) {
		...
		thp_log_info("%s:support_suspend_cs_low configed: %u\n",
		 __func__, value);
	}
	cd->res_scale = 1;
#ifndef CONFIG_HONOR_GKI_V1
	if (!touch_recovery_mode) {
		...
		if (!rc) {
			...
			thp_log_info("%s:support_dynamic_resolution configed: %u\n",
				__func__, value);
		}
		if (cd->support_dynamic_resolution) {
			rc = of_property_read_u32(thp_node, "res_scale", &value);
			if (!rc) {
				if (g_touch_factory_mode)
					cd->res_scale = 1;
				else
					cd->res_scale = value;
				thp_log_info("%s:res_scale:%u\n", __func__,
					cd->res_scale);
			}
		}
	}
	...
	struct thp_core_data *cd)
{
	...
	thp_log_debug("%s:Enter!\n", __func__);
	cd->support_fixed_2nd_poweroff = 0;
	rc = of_property_read_u32(thp_node, "support_fixed_2nd_poweroff", &value);
	if (!rc) {
		cd->support_fixed_2nd_poweroff = value;
		thp_log_info("%s:support_fixed_2nd_poweroff configed %u\n",
				__func__, value);
	}
	cd->support_send_cmd_when_suspend = 0;
	rc = of_property_read_u32(thp_node, "support_send_cmd_when_suspend", &value);
	if (!rc) {
		cd->support_send_cmd_when_suspend = value;
		thp_log_info("%s:support_send_cmd_when_suspend configed %u\n",
				__func__, value);
	}
	...
	if (!rc) {
		...
		thp_log_info("%s:support_pen_wakeup_gesture %u\n", __func__, value);
	}
...
static void thp_prox_add_suspend(struct thp_core_data *cd, bool enable)
{
	...
	if (cd->reset_status_in_suspend_mode) {
		thp_log_info("[Proximity_feature] %s second power down\n",
			__func__);
	} else if (!cd->support_fixed_2nd_poweroff) {
		pinctrl_select_state(cd->pctrl, cd->mtk_pinctrl.reset_low);
		thp_log_info("%s tp rst has been pulled down\n", __func__);
	}
	pinctrl_select_state(cd->pctrl, cd->mtk_pinctrl.cs_low);
#endif
	/* disable the irq */
...
/*
 * This function is supplied for lcd driver to actively keep tp reset low in
 * several ms
 */
int thp_keep_reset_low(uint32_t msec)
{
	...
	if (!cd || !cd->support_lcd_do_tp_reset) {
		thp_log_err("%s: lcd can not keep tp reset low\n", __func__);
		return 0;
	}

#ifndef CONFIG_HUAWEI_THP_MTK
	gpio_direction_output(cd->gpios.rst_gpio, THP_RESET_LOW);
	mdelay(msec);
	gpio_set_value(cd->gpios.rst_gpio, THP_RESET_HIGH);
#else
	pinctrl_select_state(cd->pctrl, cd->mtk_pinctrl.reset_low);
	mdelay(msec);
	pinctrl_select_state(cd->pctrl, cd->mtk_pinctrl.reset_high);
#endif
	thp_log_info("%s: keep tp reset low for %u ms", __func__, msec);
	return 0;
}
...
static int thp_probe(struct spi_device *sdev)
{
	...
	thp_log_info("%s: in\n", __func__);
#if (!(IS_ENABLED(CONFIG_TP_QCOM_7XX_8XX)) || defined(CONFIG_LCD_QCOM_DRV))
#if ((defined(CONFIG_LCD_KIT_DRIVER) || defined(CONFIG_LCD_QCOM_DRV)) \
&& IS_ENABLED(CONFIG_HUAWEI_THP_QCOM))
#ifndef CONFIG_HONOR_GKI_V1
	if (!lcd_kit_get_ops()) {
	...
	if (thp_core->fast_booting_solution) {
		...
		return rc;
#else
#if IS_ENABLED(CONFIG_HUAWEI_THP_SYNAPTICS)
	syna_driver_module_init();
#endif
#if IS_ENABLED(CONFIG_HUAWEI_THP_GOODIX)
	goodix_driver_module_init();
#endif
#if IS_ENABLED(CONFIG_HUAWEI_THP_NOVATECH)
	nova_driver_module_init();
#endif
#if IS_ENABLED(CONFIG_HUAWEI_THP_ILITEK)
	ilitek_driver_module_init();
#endif
#endif
	}
	thp_log_info("%s:out\n", __func__);
...
#if ((defined THP_CHARGER_FB) && (!IS_ENABLED(CONFIG_HUAWEI_THP_QCOM)) && \
