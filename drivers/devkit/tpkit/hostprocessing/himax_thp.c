...
static uint8_t *g_thp_project_id;
static uint8_t *g_thp_cg_color;
static uint8_t *g_thp_sensor_id;
...
static uint32_t touch_driver_thp_check_crc(struct thp_device *tdev,
...
static int touch_driver_projectid(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (type == STR_TYPE) {
		g_thp_project_id = kcalloc((len + 1),
			sizeof(*g_thp_project_id), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_thp_project_id = kcalloc(len,
			sizeof(*g_thp_project_id), GFP_KERNEL);
	} else {
	...
	if (g_thp_project_id) {
		memcpy(g_thp_project_id,
			buffer + MOVE_8BIT, len);
		thp_log_info("%s: project id: ", __func__);
		if (type == STR_TYPE) {
			g_thp_project_id[len] = '\0';
			thp_log_info("%s", g_thp_project_id);
		} else {
...
static int touch_driver_cgcolor(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (type == STR_TYPE) {
		g_thp_cg_color = kcalloc((len + 1),
			sizeof(*g_thp_cg_color), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_thp_cg_color = kcalloc(len,
			sizeof(*g_thp_cg_color), GFP_KERNEL);
	} else {
	...
	if (g_thp_cg_color) {
		memcpy(g_thp_cg_color,
			buffer + MOVE_8BIT, len);
		thp_log_info("%s: cg_color: ", __func__);
		if (type == STR_TYPE) {
			g_thp_cg_color[len] = '\0';
			thp_log_info("%s", g_thp_cg_color);
		} else {
			for (j = 0; j < len; j++)
				thp_log_info("0x%02X",
					*(g_thp_cg_color +
...
static int touch_driver_sensorid(uint8_t type, uint32_t len, uint8_t *buffer)
{
	...
	if (type == STR_TYPE) {
		g_thp_sensor_id = kcalloc((len + 1),
			sizeof(*g_thp_sensor_id), GFP_KERNEL);
	} else if (type == NUM_TYPE) {
		g_thp_sensor_id = kcalloc(len,
			sizeof(*g_thp_sensor_id), GFP_KERNEL);
	} else {
	...
	if (g_thp_sensor_id) {
		memcpy(g_thp_sensor_id,
			buffer + MOVE_8BIT, len);
		thp_log_info("%s: sensor id: ", __func__);
		if (type == STR_TYPE) {
			g_thp_sensor_id[len] = '\0';
			thp_log_info("%s", g_thp_sensor_id);
		} else {
			for (j = 0; j < len; j++)
				thp_log_info("0x%02X",
					*(g_thp_sensor_id +
...
static int touch_driver_read_panel_info(struct thp_device *tdev)
{
	...
	for (i = DATA_0; i < INFO_SECTION_NUM; i++) {
		...
		if (touch_driver_thp_check_crc(tdev,
...
int touch_driver_83102_chip_detect(struct thp_device *tdev)
{
	...
	/* Get tp_color */
	if (g_thp_cg_color != NULL)
		cypress_ts_kit_color[0] = *g_thp_cg_color;
