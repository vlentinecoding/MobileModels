...
#define DOUBLE_CLICK_GESTURE_ID 0x7F
#define WAKE_UP_ONLY_HAND 0x00
#define WAKE_UP_PEN_HAND 0x01
#define WAKE_UP_ONLY_PEN 0x02
#define PEN_ENABLE 0x01
#define PEN_DISABLE 0x00

uint8_t g_lpwg_subtype = PEN_ENABLE;
uint8_t g_lpwg_type = WAKE_UP_PEN_HAND;
...
static int touch_driver_init(struct thp_device *tdev)
{
	...
	thp_log_info("%s: send_cmd_retry_delay_ms %u\n", __func__,
		cd->send_cmd_retry_delay_ms);
	rc = of_property_read_u32(nova_node, "support_deepsleep_mode",
	...
	uint8_t *r_buf, uint8_t *w_buf)
{
	...
	if ((tdev->thp_core->support_vendor_ic_type == N9Z_SERIES) &&
		(tdev->thp_core->pen_supported) &&
		((r_buf[6] == STYLUS_GESTURE_ID) &&
		(((~r_buf[6] + 1) & 0xFF) == r_buf[7]))) {
		return STYLUS_EVENT;
	} else if ((r_buf[6] == DOUBLE_CLICK_GESTURE_ID) &&
		(((~r_buf[6] + 1) & 0xFF) == r_buf[7])) {
	...
	unsigned int *gesture_wakeup_value)
{
	...
	if (tdev->thp_core->easy_wakeup_info.off_motion_on == true) {
		...
		*gesture_wakeup_value = TS_DOUBLE_CLICK;
		if ((tdev->thp_core->support_vendor_ic_type == N9Z_SERIES) &&
			(tdev->thp_core->pen_supported) && (ret == STYLUS_EVENT))
...
static int touch_driver_send_fw_cmd(struct thp_device *tdev, uint8_t cmd)
{
	...
	thp_log_info("read stamp value: %x, cmd: %x, type: %x, subtype: %x\n",
		stamp_val, cmd, g_lpwg_type, g_lpwg_subtype);
	...
	w_buf[3] = 0x00;
	w_buf[4] = 0x00;
	if (tdev->thp_core->support_pen_wakeup_gesture) {
		w_buf[3] = g_lpwg_type;
		w_buf[4] = g_lpwg_subtype;
	}
	w_buf[5] = stamp_val - 1;
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	if (pt_test_mode) {
		if (cd->support_deepsleep_mode) {
			thp_log_info("%s: turn to deepsleep mode\n", __func__);
			(void)touch_driver_send_fw_cmd(tdev,
				NVT_CMD_ENTER_CUSTOMIZED_DP_DSTB);
		} else {
...
static int touch_driver_nt_bt_handler(struct thp_device *tdev, bool delay_enable)
{
	...
	thp_log_info("%s: status:%u, stylus_gesture_status:%u ,subtype:%u, sleep_mode:%u\n",
...
#else
int nova_driver_module_init(void)
#endif
{
	int rc;
	struct thp_core_data *cd = thp_get_core_data();
