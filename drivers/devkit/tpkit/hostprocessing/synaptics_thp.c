	...
	struct device_node *thp_node, struct thp_core_data *cd)
{
	...
	if (!rc) {
		...
		thp_log_info("%s:support_ignore_first_irq_after_reset %u\n",
			__func__, value);
	}
	...
	cd->support_lcd_do_tp_reset = 0;
	rc = of_property_read_u32(thp_node, "support_lcd_do_tp_reset",
		&value);
	if (!rc) {
		cd->support_lcd_do_tp_reset = value;
		thp_log_info("%s:support_lcd_do_tp_reset configed %u\n",
			__func__, value);
	}
	return 0;
}
...
static int touch_driver_power_on(struct thp_device *tdev)
{
	...
	if (cd->support_vendor_ic_type == SYNA3909)
		thp_log_debug("%s: reset high without delay\n", __func__);
	else
...
static int touch_driver_chip_detect_for_tddi(struct thp_device *tdev)
{
	...
	unsigned char *fnnum = spi_read_buf;
	int rc;
	...
	if (rc < 0)
		thp_log_err("%s: spi write failed\n", __func__);
	rc = touch_driver_spi_read(tdev->thp_core->sdev, fnnum, RMI_CMD_LEN);
	if (rc < 0)
		thp_log_err("%s:spi read failed\n", __func__);
	thp_bus_unlock();
	if ((fnnum[0] != UBL_FN_NUMBER) && (fnnum[0] != MESSAGE_MARKER)) {
		thp_log_err("%s: fnnum[0] error: 0x%02x\n", __func__, fnnum[0]);
		rc = -ENODEV;
		goto exit;
	}
	thp_log_info("%s: fnnum[0] info: 0x%02x\n", __func__, fnnum[0]);
	return 0;
exit:
	return rc;
}
	...
	char *frame_buf, unsigned int len)
{
	...
	if (tdev->thp_core->support_ignore_first_irq_after_reset &&
		...
		thp_log_err("%s:ignore_irq_after_reset count is %u\n",
...
static int touch_driver_resume(struct thp_device *tdev)
{
	...
	if (tdev->thp_core->self_control_power) {
		...
		if (gesture_status == TS_GESTURE_MODE &&
			...
			if (tdev->thp_core->support_ignore_first_irq_after_reset)
				ignore_irq_after_reset = IGNORE_IRQ_COUNT;
			/* keep TP rst  high before LCD  reset hign */
			if (!tdev->thp_core->support_lcd_do_tp_reset)
				gpio_direction_output(tdev->gpios->rst_gpio, GPIO_HIGH);
		}
#else
		...
		if (gesture_status == TS_GESTURE_MODE &&
			...
			if (tdev->thp_core->support_reset_low_pinctrl_for_mtk) {
				pinctrl_select_state(tdev->thp_core->pctrl,
					tdev->thp_core->mtk_pinctrl.reset_low);
				mdelay(10);  /* 10ms sequence delay */
				/*First irq will blur the screen so ignore it*/
				if (tdev->thp_core->support_ignore_first_irq_after_reset)
					ignore_irq_after_reset = IGNORE_IRQ_COUNT;
			}
			pinctrl_select_state(tdev->thp_core->pctrl,
	...
	u8 state)
{
	...
	delta_time /= 1000;
	if (delta_time < 10) {
		thp_log_info("%s: lowpower switch too fast [%ld ms]\n",
			__func__, delta_time);
...
#else
int syna_driver_module_init(void)
#endif
{
	int rc;
