...
int cts_start_fw_short_opentest(const struct cts_device *cts_dev)
{
	...
	if (ret) {
		...
		TS_LOG_DEBUG("Send CTS_CMD_SHORT_OPEN_TEST SUCCESS\n");
		return 0;
	}
}
...
int cts_get_shortdata(const struct cts_device *cts_dev, void *buf)
{
	...
	TS_LOG_INFO("Get short test data");
	...
	if (i == 2000) {
		TS_LOG_ERR("Get data ready flag timeout");
		return -ETIMEDOUT;
	}

	do {
		n_channels = cts_dev->fwdata.rows + cts_dev->fwdata.cols;
		if (n_channels > 16) {
			TS_LOG_ERR("invaild channels data = %d\n", n_channels);
			return -EINVAL;
		}
		ret = cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_SHORT_DATA,
			temp,  n_channels * 4);
		if (ret) {
...
int cts_get_opendata(const struct cts_device *cts_dev, void *buf)
{
	...
	TS_LOG_INFO("Get open test data");
    /** - Wait data ready flag set */
	for (i = 0; i < 200; i++) {
		mdelay(10);
		ret = cts_get_short_test_status(cts_dev, &status);
...
int cts_charger_status_rec(struct cts_device *cts_dev)
{
	int ret = NO_ERR;
	struct chipone_ts_data *cts_data =
	...
	if (!cts_data->use_huawei_charger_detect) {
		TS_LOG_ERR("Charger rec with use_huawei_charger_detect = false\n");
		return -EINVAL;
	}
	if (!cts_data->tskit_data->ts_platform_data->feature_info.charger_info.charger_supported) {
		TS_LOG_ERR("Charger rec with charger_supported = false\n");
		return -EINVAL;
	}
	TS_LOG_INFO("Charger rec write charger_switch: %u\n",
		cts_data->tskit_data->ts_platform_data->feature_info.charger_info.charger_switch);
	cts_lock_device(&cts_data->cts_dev);
	ret = cts_send_command(&cts_data->cts_dev,
		cts_data->tskit_data->ts_platform_data->feature_info.charger_info.charger_switch ?
		CTS_CMD_CHARGER_ATTACHED : CTS_CMD_CHARGER_DETACHED);
	cts_unlock_device(&cts_data->cts_dev);

	if (ret)
		TS_LOG_ERR("Charger rec, send %s to device failed %d(%s)\n",
			cts_data->tskit_data->ts_platform_data->feature_info.charger_info.charger_switch?
			"CMD_CHARGER_ATTACHED" :
...
int cts_start_device(struct cts_device *cts_dev)
{
	...
	if (cts_charger_status_rec(cts_dev))
		TS_LOG_ERR("TS CHARGER STATUS REC FAIL\n");
