...
int cts_init_testlimits_from_csvfile(void)
{
	...
	if (ret) {
		...
		TS_LOG_INFO("deviation_test_priv_param.frames:%u", data[0]);
	}
	...
	ret = ts_kit_parse_csvfile(file_path,
		CTS_CSV_NOISE_TEST_FRAME_IGNORE, data, 1, 1);
	if (ret) {
		TS_LOG_ERR("Failed to read noise_test_frame_ignore from csvfile:%d", ret);
		return ret;
	} else {
		cts_data->noise_test_priv_param.frames_ignore = (u8) (data[0]);
		TS_LOG_INFO("noise_test_priv_param.frames_ignore:%u", data[0]);
	}
	ret =
	...
	ret = ts_kit_parse_csvfile(file_path,
		CTS_CSV_MUTUAL_DEVIATION_LIMIT_M, data, rows, columns - 1);
	if (ret) {
		TS_LOG_ERR("Failed to read mutual deviation data limit max from csvfile:%d", ret);
		return ret;
	} else {
		for (i = 0; i < rows * (columns -1); i++)
			cts_data->deviation_test_param.max_limits_col[i] = (u16)(data[i]);
	}

	ret = ts_kit_parse_csvfile(file_path,
		CTS_CSV_MUTUAL_DEVIATION_LIMIT_N, data, rows - 1, columns);
	if (ret) {
		TS_LOG_ERR("Failed to read mutual deviation data limit max from csvfile:%d", ret);
		return ret;
	} else {
		for (i = 0; i < (rows-1) * columns; i++)
			cts_data->deviation_test_param.max_limits_row[i] = (u16)(data[i]);
	}

	ret = ts_kit_parse_csvfile(file_path, CTS_CSV_SRAM_TEST_ENABLE, data, 1, 1);
	if (ret) {
		TS_LOG_ERR("Failed to read sram test enable from csvfile:%d", ret);
		return ret;
	} else {
		cts_data->test_sram = (u8)(data[0]);
		TS_LOG_INFO("test_sram enable:%u", data[0]);
	}
...
static int init_factory_test_param(void)
{
	...
	if (cts_data->test_deviation) {
		...
		test_param->max_row = test_param->max_limits_row;
		test_param->max_col = test_param->max_limits_col;
		test_param->num_invalid_node =  cts_data->invalid_node_num;
		test_param->invalid_nodes = cts_data->invalid_nodes;
		if (cts_data->dump_factory_test_data_to_console) {
			test_param->flags |=
		...
		if (cts_data->dump_factory_test_data_to_buffer) {
			test_param->flags |=
			    CTS_TEST_FLAG_DUMP_TEST_DATA_TO_USERSPACE;
			ret = alloc_test_data_mem(test_param, 4 * rows * cols *
	...
	if (cts_data->test_noise) {
		...
		if (cts_data->dump_factory_test_data_to_buffer) {
			test_param->flags |=
			    CTS_TEST_FLAG_DUMP_TEST_DATA_TO_USERSPACE;
			ret = alloc_test_data_mem(test_param, 2 * nodes *
				(FREQ_HOP_NUM_MAX * STORE_NOISE_FRAME_NUM));
			if (ret)
				return ret;
			test_param->test_data_wr_size =
...
static void print_factory_test_result_to_console(void)
{
	...
	if (cts_data->test_sram)
		print_test_result_to_console("Sram",
			cts_data->sram_test_result);
...
static void do_factory_test(void)
{
	...
	if (cts_data->test_sram) {
		cts_data->sram_test_result = 0;
		ret = cts_test_sram(&cts_data->cts_dev,
			&cts_data->sram_test_param);
		if (ret)
			cts_data->sram_test_result = 1; /* sram fail flag */
	}
	if (cts_data->test_noise) {
...
static void cts_put_test_result_newformat(struct ts_rawdata_info_new *info)
{
	...
	if (cts_data->test_rawdata) {
		...
		if (cts_data->rawdata_test_data_size) {
			data = (u16 *)cts_data->rawdata_test_param.test_data_buf;
	...
	if (cts_data->test_deviation) {
		/* 2p 2f */
		ret = chipone_alloc_newnode(&pts_node, 2 * info->tx*info->rx);
		if(ret < 0) {
		...
		if (cts_data->deviation_test_data_size) {
			...
			for (i = 0; i < (2 * info->tx * info->rx); i++)  /* only 1 frame */
				pts_node->values[i] = data[i];
		}
		pts_node->size = 2 * info->tx * info->rx;
	...
	if (cts_data->test_noise) {
		/* mul noise, 3p 3f */
		ret = chipone_alloc_newnode(&pts_node,
			FREQ_HOP_NUM_MAX * STORE_NOISE_FRAME_NUM *
			(info->tx * info->rx));
		if(ret < 0) {
		...
		if (cts_data->noise_test_data_size) {
			...
			for (j = 0; j < (FREQ_HOP_NUM_MAX * STORE_NOISE_FRAME_NUM); j++)
				for (i = 0; i < info->tx * info->rx; i++) /*only 1 frame*/
					pts_node->values[(info->tx * info->rx) * j + i ] =
						data[single_frame_size * j + i ];
		}
		pts_node->size =
			FREQ_HOP_NUM_MAX * STORE_NOISE_FRAME_NUM * (info->tx * info->rx);
		pts_node->testresult = cts_data->noise_test_result ? 'F' : 'P';
		pts_node->typeindex = 3;
		strncpy(pts_node->test_name, "Mutual Noise test",
				sizeof(pts_node->test_name) - 1);
		if (pts_node->testresult == CAP_TEST_FAIL_CHAR)
			strncpy(pts_node->tptestfailedreason, cts_data->failedreason,
				TS_RAWDATA_FAILED_REASON_LEN - 1);
	...
	if (cts_data->test_sram) {
		...
		ret = chipone_alloc_newnode(&pts_node, info->tx * info->rx);
		if(ret < 0) {
			TS_LOG_ERR("sram malloc failed node\n");
			return ;
		}
		pts_node->size = 0;
		pts_node->testresult = cts_data->sram_test_result ? 'F' : 'P';
		pts_node->typeindex = 4;
		strncpy(pts_node->test_name, "sram",
				sizeof(pts_node->test_name) - 1);
		if (pts_node->testresult == CAP_TEST_FAIL_CHAR)
			strncpy(pts_node->tptestfailedreason, cts_data->failedreason,
				TS_RAWDATA_FAILED_REASON_LEN - 1);
	...
	if (cts_data->test_short) {
		...
		if (cts_data->short_test_data_size) {
			short_data = (s32 *)cts_data->short_test_param.test_data_buf;
			for (i = 0; i < (info->tx + info->rx); i++)
	...
	if (cts_data->test_noise) {
		/* self noise, 8p 8f */
		ret = chipone_alloc_newnode(&pts_node, (info->tx + info->rx)); /* Scap only once */
		if(ret < 0) {
		...
		if (cts_data->noise_test_data_size) {
			data = (s16 *)cts_data->noise_test_param.test_data_buf;
			for (j = 0; j < STORE_NOISE_FRAME_NUM; j++) /* Scap only once */
				for (i = 0; i < (info->tx + info->rx); i++) /*only 1 frame*/
					pts_node->values[(info->tx + info->rx) * j + i] =
						data[single_frame_size * j + (info->tx * info->rx) + i];
		}
		pts_node->size = info->tx + info->rx;
...
static int chipone_get_capacitance_test_type(struct ts_test_type_info *info)
{
	...
	case TS_ACTION_WRITE:
		break;
	default:
		TS_LOG_ERR("%s:invalid op_action:%d\n",
...
output_result_str:
	snprintf(info->result, TS_RAWDATA_RESULT_MAX,
		 "0%c-1%c-2%c-3%c-4%c-5%c-6%c-7%c-8%c-9%c-%s-%s-%s\n",
				...
				int row_size)
{
	...
	if (cts_data->test_sram) {
		print_test_result_to_seq_file(m, "sram",
			cts_data->sram_test_result);
		if (cts_data->dump_factory_test_data_to_buffer &&
			((cts_data->sram_test_result > 0) ||
			!cts_data->print_factory_test_data_only_fail))
				print_touch_data_to_seq_file(m,
					cts_data->sram_test_param.test_data_buf,
					cts_data->sram_test_priv_param.frames, rows, cols);
	}
	if (cts_data->test_noise) {
		print_test_result_to_seq_file(m,
			"MCap Noise", cts_data->noise_test_result);
		print_test_result_to_seq_file(m,
			"Scap Noise", cts_data->selfnoise_test_result);
		if (cts_data->dump_factory_test_data_to_buffer &&
		    (cts_data->noise_test_result > 0 ||
		    !cts_data->print_factory_test_data_only_fail)) {
			print_touch_data_to_seq_file(m,
				cts_data->noise_test_param.test_data_buf,
				(FREQ_HOP_NUM_MAX * STORE_NOISE_FRAME_NUM), rows + 2, cols);
		}
	}
	if (cts_data->test_open) {
...
static int cts_chip_suspend(void)
{
	...
	TS_LOG_INFO("Chip suspend with sleep_mode: %d, aod_mode: %d\n",
		wakeup_info->sleep_mode, wakeup_info->aod_mode);
	...
	if (cts_is_gesture_wakeup_enabled(&cts_data->cts_dev)) {
		TS_LOG_INFO("Suspend with gesture wakeup\n");
