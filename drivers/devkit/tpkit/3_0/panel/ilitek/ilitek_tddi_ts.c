...
#ifdef CONFIG_HONOR_DEVKIT_SPRD_3_0
	if (!ilits->use_pinctrl)
		return 0;
#endif
...
int ilitek_tddi_fw_update_boot(char *file_name)
{
	...
	if (ilits && ilits->ts_dev_data && ilits->chip) {
		(void)snprintf(ilits->ts_dev_data->project_id, MAX_STR_LEN,
			"%s", ilits->project_id);
		(void)snprintf(ilits->ts_dev_data->version_name, MAX_STR_LEN,
			"0x%x", ilits->chip->fw_ver);
	}
	return 0;
}
...
static unsigned char *chip_roi_rawdata(void)
{
	...
	if (ilits->roi_completion_disable){
