...
int ilitek_parse_dts(struct device_node *np)
{
	...
	ret = of_property_read_u32(dts_np, ILITEK_DTS_ROI_COMP, &ilits->roi_completion_disable);
	if (ret) {
		TS_LOG_INFO("get roi_completion_disable failed, use default: %d\n",
			ILITEK_DTS_DEF_ROI_COMP);
		ilits->roi_completion_disable = ILITEK_DTS_DEF_ROI_COMP;
	}
	TS_LOG_INFO("roi_completion_disable = %d\n", ilits->roi_completion_disable);
