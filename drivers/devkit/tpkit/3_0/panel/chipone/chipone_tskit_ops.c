...
#define CHIPONE_TOUCH_SWITCH_SEG_COUNT 2
#define SOPER_LANDSCAPE_LEFT 0
#define SOPER_LANDSCAPE_RIGHT 1
	...
	struct ts_kit_device_data *data)
{
	...
	struct ts_kit_platform_data *pd = NULL;
	int ret;
	...
	pd = data->ts_platform_data;

	ret = of_property_read_u32(of_node, "use-pinctrl", (u32 *)&cts_data->use_pinctrl);
	if (ret) {
		TS_LOG_ERR("Get use_pinctrl from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
		cts_data->use_pinctrl = 0;
	}
	TS_LOG_INFO(" - %-32s: %c\n", "Use pinctrl",
		cts_data->use_pinctrl ? 'Y' : 'N');

	ret = of_property_read_u32(of_node, "use-huawei-charger-detect", (u32 *)&cts_data->use_huawei_charger_detect);
	if (ret) {
		TS_LOG_ERR("Get use-honor-charger-detect from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
		cts_data->use_huawei_charger_detect = 0;
	}
	...
	ret = of_property_read_u8(of_node, "charger_supported",
		(u8 *)&pd->feature_info.charger_info.charger_supported);
	if (ret) {
		TS_LOG_ERR("Get charger_supported from dts failed %d(%s)\n",
			ret, cts_strerror(ret));
		pd->feature_info.charger_info.charger_supported = 1;
	}
	TS_LOG_INFO(" - %-32s: %c\n", "Charger supported",
		pd->feature_info.charger_info.charger_supported ? 'Y' : 'N');
	...
	ret = of_property_read_s32(of_node, "check_status_watchdog_timeout",
		(s32 *)&data->check_status_watchdog_timeout);
	if (ret) {
...
static int chipone_chip_detect(struct ts_kit_platform_data *data)
{
	...
	if (data->chip_data == NULL) {
		TS_LOG_ERR("Detect chip with chip_data = NULL\n");
		return -EINVAL;
	}
	if (data->chip_data->cnode == NULL) {
...
enum TOUCH_MESSAGE_PRINT_LEVEL {
	PRINT_TOUCH_MESSAGE_NONE = 0,
	PRINT_TOUCH_MESSAGE_BRIEF,
	PRINT_TOUCH_MESSAGE_DETAIL,
	TOUCH_MESSAGE_PRINT_LEVEL_MAX,
};

/*	module parameter in /sys/module/chipone/parameter/
 *	touch_message_print_level
 *	E.g. "echo 1 > /sys/module/chipone/parameter/
 *	touch_message_print_level" will
 *	set touch_message_print_level to PRINT_TOUCH_MESSAGE_BRIEF
 *	"echo 2 >  /sys/module/chipone/parameter/touch_message_print_level" will
 *	set touch_message_print_level to PRINT_TOUCH_MESSAGE_DETAIL
 *	USE command "cat /dev/kmsg | grep -iE "Msgs|Touch" to filter kernel
 *	message out.
 */
static int touch_message_print_level = PRINT_TOUCH_MESSAGE_NONE;
module_param(touch_message_print_level, int, 0644);
/* Convert touch event to string*/
static const char *touch_event_type_2_str(u8 type)
{
	static const char * const touch_event_type_str[] = {
		"NONE", "DOWN", "MOVE", "STAY", "UUPP"
	};
	if (type < ARRAY_SIZE(touch_event_type_str))
		return touch_event_type_str[type];
	else
		return "BAD!";
}
/* Following array to define whether print touch event */
static bool touch_event_print_flag[TOUCH_MESSAGE_PRINT_LEVEL_MAX][5] = {
	...
	struct cts_device_touch_msg *msgs, int num)
{
	...
	/* Print separator for the touch message*/
	if (touch_message_print_level > PRINT_TOUCH_MESSAGE_NONE) {
		TS_LOG_INFO("++ Touch Msgs point number == %d ++\n", num);
	}
	...
	for (i = 0; i < num; i++) {
		...
		/*Print touch event if necessary*/
		if (touch_message_print_level < TOUCH_MESSAGE_PRINT_LEVEL_MAX &&
			msgs[i].event < 5 &&
			touch_event_print_flag[touch_message_print_level][msgs[i].event]) {
			TS_LOG_INFO("Touch[%d]:%s,[%3u,%4u]\n", msgs[i].id,
			touch_event_type_2_str(msgs[i].event), x, y);
		}
...
static int chipone_fw_update_boot(char *file_name)
{
	...
	if (cts_data && cts_data->tskit_data) {
		(void)snprintf(cts_data->tskit_data->version_name,
				MAX_STR_LEN, "0x%x",
...
#ifdef CONFIG_CTS_EARJACK_DETECT
	if (cts_data->cts_dev.fwdata.supp_headphone_cable_reject)
...
static int chipone_charger_switch(struct ts_charger_info *info)
{
	...
	if (cts_data == NULL) {
		TS_LOG_ERR("%s, cts_data = NULL\n", __func__);
		return -EINVAL;
	}
	if (cts_data->tskit_data)
		pd = cts_data->tskit_data->ts_platform_data;
	if (pd == NULL) {
		TS_LOG_ERR("%s, pd = NULL\n", __func__);
		return -EINVAL;
	}

	if (!cts_data->use_huawei_charger_detect) {
		TS_LOG_ERR(
			"Charger switch with use_huawei_charger_detect = false\n");
		return -EINVAL;
	}

	if (!pd->feature_info.charger_info.charger_supported) {
	...
	case TS_ACTION_READ:
		...
		ret = cts_fw_reg_readb(&cts_data->cts_dev,
			CTS_DEVICE_FW_REG_CHARGER_STATUS,
			&info->charger_switch);
		cts_unlock_device(&cts_data->cts_dev);
		if (ret) {
			TS_LOG_ERR(
				"Get charger status from device failed %d(%s)\n",
				ret, cts_strerror(ret));
		}
		return ret;
...
int chipone_chip_check_status(void)
{
	...
	cts_lock_device(&cts_data->cts_dev);
	ret = cts_fw_reg_readw(&cts_data->cts_dev,
		CTS_DEVICE_FW_REG_CHIP_TYPE, &fwid);
	cts_unlock_device(&cts_data->cts_dev);
	if (ret) {
...
static bool chipone_switch_supported(unsigned int stype)
{
	unsigned int i;

	if (cts_data->tskit_data->scene_type_num == 0)
		return false;
	for (i = 0; i < cts_data->tskit_data->scene_type_num; i++) {
		if (stype == cts_data->tskit_data->scene_support_array[i])
			return true;
	}
	return false;
}

static int chipone_landscape_scene_switch(unsigned int soper)
{
	int ret;

	if (soper == SOPER_LANDSCAPE_LEFT) {
		ret = cts_send_command(&cts_data->cts_dev,
			CTS_CMD_LANDSCAPE_LEFT);
	} else if (soper == SOPER_LANDSCAPE_RIGHT) {
		ret = cts_send_command(&cts_data->cts_dev,
			CTS_CMD_LANDSCAPE_RIGHT);
	} else {
		ret = -EINVAL;
		TS_LOG_ERR("%s, unknown soper:%u\n", __func__, soper);
	}
	return ret;
}

static void chipone_chip_touch_switch(void)
{
	char in_data[MAX_STR_LEN] = {0};
	unsigned int stype, soper, sparam;
	unsigned int i, cnt;
	struct ts_kit_device_data *dev_data = NULL;
	int ret = -EINVAL;

	if (cts_data == NULL || cts_data->tskit_data == NULL) {
		TS_LOG_ERR("%s, cts_data is NULL\n", __func__);
		return;
	}
	dev_data = cts_data->tskit_data;
	memcpy(in_data, dev_data->touch_switch_info, MAX_STR_LEN - 1);
	...
	if (cnt != CHIPONE_TOUCH_SWITCH_SEG_COUNT) {
		TS_LOG_ERR("%s: segment count wrong:%u\n", __func__, cnt);
		goto out;
	}

	if (sscanf(in_data, "%u,%u,%u", &stype, &soper, &sparam) <= 0) {
		TS_LOG_ERR("%s: sscanf error\n", __func__);
		goto out;
	}
	TS_LOG_INFO("stype=%u, soper=%u, sparam=%u\n", stype, soper, sparam);

	if (!chipone_switch_supported(stype)) {
		TS_LOG_ERR("%s, stype:%u, not supported!\n", __func__, stype);
		goto out;
	}

	switch (stype) {
	case TS_SCENE_PORTRAIT:
		if (soper != 0 || sparam != 0) {
			TS_LOG_ERR("%s, soper or sparam invalid!\n", __func__);
			goto out;
		}
		ret = cts_send_command(&cts_data->cts_dev, CTS_CMD_PORTRAIT);
		break;
	case TS_SCENE_LANDSCAPE:
		if (sparam != 0) {
			TS_LOG_ERR("%s, sparam invalid!\n", __func__);
			goto out;
		}
		ret = chipone_landscape_scene_switch(soper);
		break;
	default:
...
struct ts_device_ops chipone_ts_device_ops = {
	...
	.chip_touch_switch = chipone_chip_touch_switch,
};
