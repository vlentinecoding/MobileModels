	...
	struct device_node *np)
{
	...
	if (ret) {
		TS_LOG_ERR("Parse detect psp failed %d", ret);
		...
		if (psp < 0) {
			TS_LOG_ERR("Parse detect psp: '%s' invalid", psp_str);
			cd_data->psp = CFG_CTS_DEF_CHGR_DET_PSY_PROP;
		} else {
			cd_data->psp = psp;
		}
	...
	if (ret) {
		TS_LOG_ERR("Parse detect psp poll interval failed %d", ret);
	}
...
static int start_charger_detect(struct cts_charger_detect_data *cd_data)
{
	...
	TS_LOG_INFO("Start detect type: %d(%s)",
	...
	if(cd_data->type == CTS_CHGR_DET_TYPE_PSY_NOTIFY) {
		...
		if (ret) {
			TS_LOG_ERR("Register notifier failed: %d", ret);
			return ret;
		}
	...
	if(cd_data->type == CTS_CHGR_DET_TYPE_POLL_PSP) {
		if (!schedule_delayed_work(&cd_data->psp_poll_work,
...
static int stop_charger_detect(struct cts_charger_detect_data *cd_data)
{
	...
	TS_LOG_INFO("Stop detect type: %d(%s)",
		cd_data->type, charger_detect_type_str(cd_data->type));
	...
	if(cd_data->type == CTS_CHGR_DET_TYPE_POLL_PSP) {
		if (!cancel_delayed_work_sync(&cd_data->psp_poll_work)) {
			TS_LOG_ERR("Cancel poll psp work while NOT pending");
		}
		cd_data->running = false;
...
static int get_charger_state(struct cts_charger_detect_data *cd_data)
{
	...
	if (ret < 0) {
		TS_LOG_ERR("Get state from psy: '%s' prop: %d(%s) failed %d(%s)",
			cd_data->psy_name, cd_data->psp,
			power_supply_prop_str(cd_data->psp),
	...
	TS_LOG_DEBUG("State: %s", charger_state_str(cd_data->state));
...
static void set_dev_charger_state_work(struct work_struct *work)
{
	...
	if (ret) {
		TS_LOG_ERR("Set dev charger state to %s failed %d",
	...
	unsigned long action, void *data)
{
	...
	if (nb == NULL || data == NULL) {
		TS_LOG_ERR("PSY notify with notifier(%p) or data(%p) = NULL",
			nb, data);
		return NOTIFY_DONE;
	}

	if (action != PSY_EVENT_PROP_CHANGED) {
		TS_LOG_DEBUG("Notify event %ld not care", action);
		return NOTIFY_DONE;
	}
	...
	if (strcmp(POWER_SUPPLY_NAME(psy), cd_data->psy_name) != 0) {
		TS_LOG_DEBUG("Notify from power supply '%s' not care",
	...
	if (cd_data->state != prev_state) {
		...
		if (!schedule_work(&cd_data->set_charger_state_work)) {
			TS_LOG_ERR("Set device charger state work is PENDING");
		}
	}
...
{
	TS_LOG_INFO("Enable detect type: %d(%s)",
		cd_data->type, charger_detect_type_str(cd_data->type));
...
static int disable_charger_detect(struct cts_charger_detect_data *cd_data)
{
	...
	TS_LOG_INFO("Disable detect type: %d(%s)",
		cd_data->type, charger_detect_type_str(cd_data->type));

	ret = stop_charger_detect(cd_data);
	if (ret) {
		TS_LOG_ERR("Disable detect failed %d", ret);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	TS_LOG_INFO("Read sysfs '"CHARGER_DET_SYSFS_GROUP_NAME"/%s'",
		attr->attr.name);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	TS_LOG_INFO("Write sysfs '"CHARGER_DET_SYSFS_GROUP_NAME"/%s' size %zu",
	...
	if (ret) {
		TS_LOG_ERR("%s charger detect failed %d",
	...
	enum cts_charger_detect_type type)
{
	...
	TS_LOG_INFO("Switch detect type to %d(%s)",
		type, charger_detect_type_str(type));

	if (type >= CTS_CHGR_DET_TYPE_MAX) {
		TS_LOG_ERR("Switch detect type %d invalid", type);
		return -EINVAL;
	}

	if (cd_data->type == type) {
		TS_LOG_ERR("Switch detect type equal");
		return 0;
	}
	...
	if (running) {
		...
		if (ret) {
			TS_LOG_ERR("Start detect failed %d", ret);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (type < 0) {
		TS_LOG_ERR("Invalid charger detect type: '%s'", argv[0]);
		return -EINVAL;
	}

	ret = switch_charger_detect_type(cd_data, type);
	if (ret) {
		TS_LOG_ERR("Switch charger detect type failed %d", ret);
		return ret;
	}

	return count;
}
static DEVICE_ATTR(type, S_IWUSR | S_IRUSR | S_IRGRP,
...
static DEVICE_ATTR(state, S_IRUSR | S_IRGRP, charger_state_show, NULL);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strcasecmp(cd_data->psy_name, argv[0]) != 0) {
		if ((psy = power_supply_get_by_name(argv[0])) == NULL) {
			TS_LOG_ERR("Power supply '%s' not found", argv[0]);
			return -EINVAL;
		}
	} else {
		same_psy = true;
	...
	if (argc > 1) {
		...
		if (psp < 0) {
			TS_LOG_ERR("Power supply property '%s' not valid",
	...
	if (argc > 2) {
		...
		if (ret) {
			TS_LOG_ERR("Arg psp poll interval is invalid");
			ret = -EINVAL;
			goto put_power_supply;
		}
	}
	...
	if (!same_psy) {
		...
		if (psy_name == NULL) {
			TS_LOG_ERR("Dup power supply name failed");
			ret = -ENOMEM;
			goto put_power_supply;
		}
...
int cts_init_charger_detect(struct chipone_ts_data *cts_data)
{
	...
	if (cd_data == NULL) {
		TS_LOG_ERR("Alloc charger detect data failed");
		return -ENOMEM;
	}
...
#ifdef CONFIG_CTS_OF
	ret = parse_charger_detect_dt(cd_data, cts_data->tskit_data->cnode);
#else /* CONFIG_CTS_OF */
#ifdef CFG_CTS_DEF_CHGR_DET_ENABLE
	cd_data->enable = true;
#else /* CFG_CTS_DEF_CHGR_DET_ENABLE */
	...
	TS_LOG_INFO("Detect: %sABLED", cd_data->enable ? "EN" : "DIS");
	TS_LOG_INFO("  Type	: %s",  charger_detect_type_str(cd_data->type));
	TS_LOG_INFO("  PSY Name: %s, prop: %d(%s)", cd_data->psy_name,
		cd_data->psp, power_supply_prop_str(cd_data->psp));
	TS_LOG_INFO("  Poll Int: %dms", cd_data->psp_poll_interval);

	ret = init_charger_detect(cd_data);
	if (ret) {
		TS_LOG_ERR("Init detect failed %d", ret);
		goto free_psy_name;
	}

#ifdef CONFIG_CTS_SYSFS
	TS_LOG_INFO("Create sysfs attr group '%s'", CHARGER_DET_SYSFS_GROUP_NAME);
...
int cts_deinit_charger_detect(struct chipone_ts_data *cts_data)
{
	...
	if (cd_data == NULL) {
		TS_LOG_ERR("Deinit detect with charger_detect_data = NULL");
		return 0;
	}
	...
	if (cd_data->psy_name) {
		TS_LOG_INFO("Kfree power supply name");
		kfree(cd_data->psy_name);
...
int cts_is_charger_attached(struct chipone_ts_data *cts_data, bool *attached)
{
	...
	cd_data = cts_data->charger_detect_data;
	if (cd_data == NULL) {
		TS_LOG_ERR("Get state with charger_detect_data = NULL");
		return -ENODEV;
	}
	...
	TS_LOG_INFO("Get curr state: %s", charger_state_str(cd_data->state));
...
int cts_start_charger_detect(struct chipone_ts_data *cts_data)
{
	...
	cd_data = cts_data->charger_detect_data;
	if (cd_data == NULL) {
		TS_LOG_ERR("Start detect with charger_detect_data = NULL");
		return -ENODEV;
	}
...
int cts_stop_charger_detect(struct chipone_ts_data *cts_data)
{
	...
	cd_data = cts_data->charger_detect_data;
	if (cd_data == NULL) {
		TS_LOG_ERR("Stop detect with charger_detect_data = NULL");
		return -ENODEV;
	}
