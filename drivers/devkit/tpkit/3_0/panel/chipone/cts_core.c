...
#define GET_GESTURE_INFO_RETRY_COUNT 30
#define GET_GESTURE_INFO_RETRY_DELAY 5
		...
		void *gesture_info, bool trace_point)
{
	...
	do {
		...
		if (ret == 0)
			break;
		retry_count++;
		if (retry_count >= GET_GESTURE_INFO_RETRY_COUNT)
			break;
		mdelay(GET_GESTURE_INFO_RETRY_DELAY);
	} while (true);
...
int cts_get_compensate_cap(struct cts_device *cts_dev, u8 *cap)
{
	...
	/* Disable idle mode if enabled */
	ret = cts_fw_reg_readb(cts_dev,
		CTS_DEVICE_FW_REG_FLAG_BITS, &idle_mode_reg);
	if (ret) {
		TS_LOG_ERR("Get idle mode enable reg failed %d", ret);
		return ret;
	}

	if (idle_mode_reg & BIT(0)) {
		TS_LOG_INFO("Disable idle mode");

		ret = cts_fw_reg_writeb(cts_dev,
			CTS_DEVICE_FW_REG_FLAG_BITS, idle_mode_reg & (~BIT(0)));
		if (ret) {
			TS_LOG_ERR("Write idle mode reg failed %d", ret);
			return ret;
		}

		ret = cts_send_command(cts_dev, CTS_CMD_QUIT_GESTURE_MONITOR);
		if (ret) {
			TS_LOG_ERR("Send cmd QUIT_GESTURE_MONITOR failed %d",
				ret);
			goto recovery_idle_mode;
		}
...
recovery_idle_mode:
	if (idle_mode_reg & BIT(0)) {
		...
		TS_LOG_INFO("Re-enable idle mode");

		r = cts_fw_reg_writeb(cts_dev,
			CTS_DEVICE_FW_REG_FLAG_BITS, idle_mode_reg);
		if (r) {
			TS_LOG_ERR("Write idle mode reg failed %d", ret);
			goto recovery_device;
		}
	}
