	...
	int reg_addr, int *value)
{
	...
	if ((kit_priv == NULL) || (kit_priv->i2c_num <= 0)) {
		hwlog_err("smartpakit_get_misc_priv: Invalid smartpakit_priv param");
		return -EINVAL;
	}
	...
	if ((i2c_priv == NULL) || (value == NULL)) {
		hwlog_err("get_i2c_priv_by_addr: invalid input arg, i2c_addr0x%x\n", i2c_addr);
		ret = -EINVAL;
		goto err_out;
	}
	...
	if ((kit_priv->ioctl_ops == NULL) ||
		(kit_priv->ioctl_ops->read_regs == NULL)) {
		    hwlog_err("ioctl_ops or i2c_reg_ctl is NULL\n");
		    ret = -ECHILD;
		    goto err_out;
	}
	...
	int mask, int value)
{
	...
	i2c_priv = get_i2c_priv_by_addr(kit_priv, i2c_addr);
	if (i2c_priv == NULL) {
		hwlog_err("get_i2c_priv_by_addr: invalid i2c addr 0x%x\n", i2c_addr);
		ret = -EINVAL;
		goto err_out;
	}
