...
struct ana_hs_core_data {
	...
	int phy_ldo_vol;
	/* use pinctrl to control mos */
	bool pinctrl_used;
	struct device *dev;
};

static struct ana_hs_core_data *g_data;
static int g_ana_hs_state;
...
static void pinctrl_mic_gnd_swap(bool flag)
{
	struct pinctrl *p = NULL;
	struct pinctrl_state *pinctrl_switch = NULL;
	int ret;

	if (g_data->bus_type != ANA_HS_PINCTRL) {
		return;
	}

	p = devm_pinctrl_get(g_data->dev);
	if (IS_ERR(p)) {
		hwlog_info("could not get pinctrl dev\n");
		return;
	}

	if (flag) {
		pinctrl_switch = pinctrl_lookup_state(p, "mic_on");
		if (IS_ERR(pinctrl_switch)) {
			hwlog_info("could not get mic_on state\n");
			return;
		}

		ret = pinctrl_select_state(p, pinctrl_switch);
		if (ret) {
			hwlog_info("could not set pins to mic_on state\n");
			return;
		}
	} else {
		pinctrl_switch = pinctrl_lookup_state(p, "mic_off");
		if (IS_ERR(pinctrl_switch)) {
			hwlog_info("could not get mic_off state\n");
			return;
		}

		ret = pinctrl_select_state(p, pinctrl_switch);
		if (ret) {
			hwlog_info("could not set pins to mic_off state\n");
			return;
		}
	}
}
...
static void plugin_regulator_config(void)
{
	if (!g_data || !g_data->regulator_used) {
		hwlog_info("%s: g_data is null or ldo not used\n", __func__);
		return;
	}

	if ((g_data->phy_ldo == NULL) && !ana_hs_prepare_regulator()) {
		hwlog_err("%s: regulator used but get failed\n", __func__);
		return;
	}
...
static void plugin_pinctrl_config(void)
{
	int ret;
	struct pinctrl *p = NULL;
	struct pinctrl_state *pinctrl_switch = NULL;
	if (!g_data || !g_data->pinctrl_used) {
		hwlog_info("%s: g_data is null or ldo not used\n", __func__);
		return;
	}
	...
	pinctrl_switch = pinctrl_lookup_state(p, "ldo_on");
	if (IS_ERR(pinctrl_switch)) {
		hwlog_info("could not get ldo_on state\n");
		return;
	}

	ret = pinctrl_select_state(p, pinctrl_switch);
	if (ret) {
		hwlog_info("could not set pins to ldo_on state\n");
		return;
	}

	hwlog_info("%s: pinctrl config\n", __func__);
}
...
static void plugin_work(struct work_struct *work)
{
	...
	if (g_data->switch_antenna)
		hw_usb_send_event(ANA_AUDIO_IN_EVENT);

	plugin_regulator_config();
	plugin_pinctrl_config();
...
static void plugout_pinctrl_config(void)
{
	int ret;
	struct pinctrl *p = NULL;
	struct pinctrl_state *pinctrl_switch = NULL;
	...
	pinctrl_switch = pinctrl_lookup_state(p, "ldo_off");
	if (IS_ERR(pinctrl_switch)) {
		hwlog_info("could not get ldo_off state\n");
		return;
	}

	ret = pinctrl_select_state(p, pinctrl_switch);
	if (ret) {
		hwlog_info("could not set pins to ldo_off state\n");
		return;
	}
	...
	while (count--) {
		if (g_data->ana_hs_in == ANA_HS_PLUG_IN) {
			...
			/* notify the phone: usb analog hs plug out. */
				hw_usb_send_event(ANA_AUDIO_OUT_EVENT);
			break;
		}
		mdelay(50);
	}
...
int ana_hs_core_get_analog_state(void)
{
	return g_ana_hs_state;
}

static int ana_hs_core_pluged_state(void)
{
	int ana_hs_state = ana_hs_core_get_analog_state();
...
void ana_hs_core_mic_gnd_swap(void)
{
	...
	if (g_data->bus_type == ANA_HS_I2C) {
		i2c_mic_gnd_swap();
	} else if (g_data->bus_type == ANA_HS_PINCTRL){
		pinctrl_mic_gnd_swap(true);
	} else {
		gpio_mic_gnd_swap();
	}
}
...
static int ana_hs_get_regulator(struct device *dev)
{
	...
	ret = of_property_read_s32(node, "ana_hs_switch_vol",
		&g_data->phy_ldo_vol);
	if (ret != 0) {
		hwlog_warn("phy_ldo_vol use default\n");
...
static int ana_hs_get_pinctrl(struct device *dev)
{
	struct device_node *node = dev->of_node;
	...
	if (of_property_read_bool(node, "pinctrl_used"))
		g_data->pinctrl_used = true;

	if (!g_data->regulator_used) {
		hwlog_info("%s: phy_ldo pinctrl not used\n", __func__);
		return -1;
	}
...
static int ana_hs_core_probe(struct platform_device *pdev)
{
	...
	g_data->wake_lock = wakeup_source_register(dev, "ana_hs_core");
	if (!g_data->wake_lock) {
	...
	g_data->pinctrl_used = false;
	ret = ana_hs_get_pinctrl(dev);
	if (ret < 0)
		hwlog_info("%s: get pinctrl fail, maybe not needed\n", __func__);
