			...
			size_t *recv_size, unsigned long flags)
{
	...
	do {
		if (cap_table_entry->rx_empty && (flags & HH_MSGQ_NONBLOCK))
			return -EAGAIN;
		if (flags & HH_MSGQ_TIMEOUT) {
			flags &= ~HH_MSGQ_TIMEOUT;
			ret = wait_event_interruptible_timeout(cap_table_entry->rx_wq, 
					!cap_table_entry->rx_empty, msecs_to_jiffies(60000));
			if (ret == 0) {
				pr_err("%s: msgq wait timeout\n", __func__);
				return 0x12345678;
			} else if (ret < 0) {
				pr_err("%s: msgq wait ret:0x%x\n", __func__, ret);
				return ret;
			}
