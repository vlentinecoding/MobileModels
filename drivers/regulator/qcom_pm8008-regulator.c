...
#ifdef CONFIG_LOG_EXCEPTION
static DEFINE_MUTEX(g_pm8008_hiview_lock);

#define HIVIEW_MAX_CONTENT_LEN 256
struct pm8008_hievent_info {
	int error_no;
	char content[HIVIEW_MAX_CONTENT_LEN];
};

struct pm8008_hievent_info g_pm8008_err_info;
//static bool g_pm8008_hiview_flag;
void pm8008_hiview_handle(int error_no, const char *error_info)
{
	errno_t ret;

	ret = memset_s(&g_pm8008_err_info, sizeof(g_pm8008_err_info), 0, sizeof(g_pm8008_err_info));
	if (ret != EOK) {
		pr_err("g_pm8008_err_info memset_s fail, ret = %d\n", ret);
		return;
	}

	g_pm8008_err_info.error_no = error_no;
	if(error_info) {
		if(strncpy_s(g_pm8008_err_info.content, sizeof(g_pm8008_err_info.content),
			error_info, sizeof(g_pm8008_err_info.content) - 1) < 0)
			pr_err("strncpy_s fail\n");
	}
}

void pm8008_hiview_report(struct pm8008_hievent_info *pm8008_info)
{
	struct hiview_hievent *hi_event = NULL;
	if (!pm8008_info) {
		pr_err("pm8008_info is null\n");
		return;
	}

	mutex_lock(&g_pm8008_hiview_lock);
	hi_event = hiview_hievent_create(pm8008_info->error_no);
	if (!hi_event) {
		pr_err("create hievent fail");
		mutex_unlock(&g_pm8008_hiview_lock);
		return;
	}

	hiview_hievent_put_string(hi_event, "CONTENT", pm8008_info->content);

	if (hiview_hievent_report(hi_event) <= 0) {
		pr_err("pm8008 hiview report failed");
	...
	pr_info("pm8008 hiview report succ");
}

void pm8008_hiview_error_info(u16 reg)
{
	char error_info[HIVIEW_MAX_CONTENT_LEN] = {0};

	if (snprintf_s(error_info, sizeof(error_info),
		sizeof(error_info) - 1, "pm8008_write, failed to write 0x%04x\n", reg) < 0)
		pr_err("pm8008_hiview error_info snprintf_s fail");
	pm8008_hiview_handle(DSM_CAMPMIC_I2C_ERROR_NO, error_info);
	pm8008_hiview_report(&g_pm8008_err_info);
}
#endif
