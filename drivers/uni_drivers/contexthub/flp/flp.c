...
#define FLP_4K_SIZE        (0x1000)

#ifdef CONFIG_HUAWEI_FLP_DIAG
#define FLP_DIAG_MAX_CMD_LEN 64
#endif

#define DDR_SHMEMEXT_ADDR_AP
#define DDR_SHMEMEXT_SIZE
...
#define OFFLINE_WIFI_MMAP_SIZE    (0x600000)
...
#if (defined CONFIG_HUAWEI_FLP_GEOFENCE) || (defined CONFIG_HUAWEI_FLP_WIFIFENCE) || (defined CONFIG_HUAWEI_FLP_CELLFENCE)
...
static int get_common_data_from_mcu(const pkt_header_t *head)
{
	...
	list_for_each(pos, &g_flp_dev.list) {//lint !e838
		...
		case SUB_CMD_FLP_WIFENCE_GET_SIZE_REPORT_REQ:
			...
			break;
		case SUB_CMD_FLP_REQUEST_GPS_REQ:
			if (flp_generate_netlink_packet(flp_port, data, len, FLP_GENL_CMD_REQUEST_GPS) != 0)
				pr_err("flp:%s cmd[0x%x] handle error\n", __func__, head->cmd);
			break;
		default:
...
{
	int ret;
	struct flp_report_data_handle_work *w =
		container_of(work, struct flp_report_data_handle_work, worker);
	const pkt_header_t *pkt = (pkt_header_t *)w->data;

	ret = flp_handle_report_data(pkt);
	if (ret < 0)
			...
			unsigned long action, void *data)
{
	...
	case XHUB_RECOVERY_3RD_DOING:
		...
		break;
	default:
		pr_err("register_xhub_recovery_notifier err\n");
		break;
	}
	return 0;
}
...
{
	if (g_flp_dev.service_type & IOMCU_APP_FLP)
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CLOSE_REQ, 0, NULL, (size_t)0, NULL, true);
	return 0;
}
...
static int flp_stop_service_type(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_COMMON_STOP_SERVICE_REQ,
					    (char *)&service_id, sizeof(service_id), &rd, true);
	if (ret != 0) {
	...
	if (0 == (g_flp_dev.service_type & IOMCU_APP_FLP))
		(void)inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CLOSE_REQ, 0, NULL, (size_t)0, NULL, true);
...
static int flp_iomcu_template (flp_port_t *flp_port, unsigned long arg, unsigned int iomcu_sub_cmd)
{
	...
	return inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, iomcu_sub_cmd,
				    usr, (unsigned long)cfence_hdr.len, NULL, true);
}
/*
 * cellfence命令处理
 */
...
static int flp_common_inject_location(unsigned long arg)
{
	...
	return inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
		SUB_CMD_FLP_INJECT_LOCATION_REQ, (char *)&inject_data, sizeof(iomcu_location),
		NULL, true);
}
...
static int flp_common_push_location_status(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_BATCH_PUSH_GNSS_REQ,
					(char *)&flp_port->location_status, sizeof(loc_source_status), NULL, true);
...
static int flp_common_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	case FLP_IOCTL_COMMON_SLEEP:
		...
		huawei_softtimer_delete(&flp_port->sleep_timer);
		ret = huawei_softtimer_modify(&flp_port->sleep_timer, data);
		if (ret == 0) {
			huawei_softtimer_add(&flp_port->sleep_timer);
		}
		break ;
...
static int flp_shmem_prepare_data(unsigned short int cmd, const void __user *buf, unsigned int length,
	void **out_buf, unsigned int *out_buf_size)
{
	...
	if (!out_buf || !out_buf_size) {
		pr_err("%s out buf or out_buf_size is null\n", __func__);
		return -EINVAL;
	}
	...
	pr_info("%s cmd:0x%x,transid:%u iomcu_len:%u\n", __func__,
		cmd, iomcu_data->transid, iomcu_len);
	*out_buf = iomcu_data;
	*out_buf_size = iomcu_len;
	return 0;
			...
			unsigned int length)
{
	...
	ret = flp_shmem_prepare_data(cmd, buf, length, &send_data, &send_len);
	if (ret) {
		pr_err("%s prepare data fail\n", __func__);
		return -EINVAL;
	}

#ifdef CONFIG_CONTEXTHUB_UNIFIED_TRANSFER
	ret = inputhub_wrapper_pack_and_send_cmd_unified(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_SHMEM_REQ,
		send_data, send_len, NULL, true);
#else
	#ifdef CONFIG_INPUTHUB_30
	if (iomcu_len > INPUTHUB_WRAPPER_MAX_LEN)
		ret = shmem_send(TAG_FLP, send_data, (unsigned int)send_len);
	else
		ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
			SUB_CMD_FLP_SHMEM_REQ, (const char*)send_data, send_len, NULL, true);
...
static int flp_open_send_status_flp(flp_port_t *flp_port)
{
	...
	if (!(flp_port->channel_type & IOMCU_APP_FLP)) {
		ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_OPEN_REQ, 0, NULL, (size_t)0, NULL, true);
		if (ret != 0) {
...
#ifdef CONFIG_HUAWEI_FLP_GEOFENCE
/*
...
static int flp_ioctl_modify_geofence(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_MODIFY_GEOF_REQ,
			     (char *)&modify_config, sizeof(geofencing_option_info_t), NULL, true);
	return ret;
}
...
static int flp_geofence_get_location(flp_port_t *flp_port, unsigned long arg)
{
	...
	inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_GEOF_GET_LOCATION_REQ, NULL, (size_t)0, NULL, true);
	return 0;
}
...
#endif /* CONFIG_HUAWEI_FLP_GEOFENCE end */

/*lint +e438*/
#ifdef CONFIG_HUAWEI_FLP_BATCHING
...
static int __flp_location_ioctl(flp_port_t *flp_port, unsigned int cmd, unsigned long arg)
{
	...
	case FLP_IOCTL_BATCHING_STOP:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_STOP_BATCHING_REQ,
				(char *)&data, sizeof(int), NULL, true);
	...
	case FLP_IOCTL_BATCHING_UPDATE:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_UPDATE_BATCHING_REQ,
			(char *)&flp_port->gps_batching_config, sizeof(batching_config_t), NULL, true);
		break;
	case FLP_IOCTL_BATCHING_LASTLOCATION:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_GET_BATCHED_LOCATION_REQ,
				(char *)&data, sizeof(int), NULL, true);
		break ;
	case FLP_IOCTL_BATCHING_FLUSH:
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_FLUSH_LOCATION_REQ, NULL,
				(size_t)0, NULL, true);
		break;
	...
	case FLP_IOCTL_BATCHING_MULT_LASTLOCATION:
		...
		inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_BATCHING_MULT_LASTLOCATION_REQ,
				(char *)&data, sizeof(int), NULL, true);
		break ;
...
#endif /* CONFIG_HUAWEI_FLP_BATCHING end */

/*lint -e438*/
#ifdef CONFIG_HUAWEI_FLP_CELLFENCE
/*
...
static int celltrajectory_cfg(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
			SUB_CMD_FLP_CELLTRAJECTORY_CFG_REQ, (char *)&cfg, sizeof(cfg), NULL, true);
	if (ret != 0) {
...
static int celltrajectory_request(flp_port_t *flp_port)
{
	...
	return inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
				SUB_CMD_FLP_CELLTRAJECTORY_REQUEST_REQ, NULL, (size_t)0, NULL, true);
}
...
static int flp_cellbatching_cfg(flp_port_t *flp_port, unsigned long arg)
{
	...
	if ((flp_port->channel_type & IOMCU_APP_FLP) == 0) {
		ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_OPEN_REQ,
			0, NULL, (size_t)0, NULL, true);
		if (ret != 0) {
	...
	ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
	SUB_CMD_FLP_CELL_CELLBATCHING_CFG_REQ, (char *)&ct_cfg, sizeof(cell_batching_start_config_t), NULL, true);
	if (ret != 0) {
		if (!(g_flp_dev.service_type & IOMCU_APP_FLP))  {
			(void)inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CLOSE_REQ,
				0, NULL, (size_t)0, NULL, true);
		}
		pr_err("[%s]GEOFENCE_RM_REQ error\n", __func__);
		return ret;
...
static int flp_cellbatching_request(flp_port_t *flp_port)
{
	...
	return inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ,
				    SUB_CMD_FLP_CELL_CELLBATCHING_REQ, NULL, (size_t)0, NULL, true);
}
...
#endif /* CONFIG_HUAWEI_FLP_CELLFENCE */

#ifdef CONFIG_HUAWEI_FLP_WIFIFENCE
/*
...
static int flp_oper_wififence(flp_port_t *flp_port, unsigned long arg, unsigned int cmd)
{
	...
	inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, cmd,
			     (char *)cmd_data, (size_t)config.length, NULL, true);
	kfree(cmd_data);
	return 0;
}
...
static int flp_diag_send_cmd(flp_port_t *flp_port, unsigned long arg)
{
	...
	ret = inputhub_wrapper_pack_and_send_cmd(TAG_FLP, CMD_CMN_CONFIG_REQ, SUB_CMD_FLP_DIAG_SEND_CMD_REQ,
				(char *)(data_buf + sizeof(unsigned int)), (size_t)data_len, NULL, true);
	if (ret != 0) {
		pr_err("[%s]GEOFENCE_RM_REQ error\n", __func__);
		goto FLP_DIAG_EXIT;
	}
...
#endif /* CONFIG_HUAWEI_FLP_DIAG end */
/*lint -e732*/
/*
 * flp ioctl消息处理
 */
...
static int flp_open(struct inode *inode, struct file *filp)/*lint -e715*/
{
	...
	huawei_softtimer_create(&flp_port->sleep_timer,
	...
	mutex_unlock(&g_flp_dev.recovery_lock);
	mutex_unlock(&g_flp_dev.lock);
	wakeup_source_init(&flp_port->wlock, "huawei_flp");
...
static int flp_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	reserve_size = get_flp_reserve_mem_size();
	page = get_flp_reserve_mem_phys();
	if (page == 0 || reserve_size < FLP_4K_SIZE) {
		pr_err("[%s]:line[%d] reserve mem phys null, reserve size[%lx]\n",
			__func__, __LINE__, reserve_size);
		return -EINVAL;
	}

	/* 4K address alignment */
	if (IS_ALIGNED(page, FLP_4K_SIZE) != true) {
		page = ALIGN(page, FLP_4K_SIZE);
		reserve_size -= FLP_4K_SIZE;
	}

	offset = offset >> MMAP_OFFSET_CHECK_BIT;
	if (OFFLINE_TYPE_CELL == offset) {
		if (OFFLINE_CELL_MMAP_SIZE < size || size > reserve_size) {
			pr_err("[%s]:line[%d] size.0x%lx.\n", __func__, __LINE__, size);
			return -EINVAL;
		}
	} else if (OFFLINE_TYPE_WIFI == offset) {
		if (OFFLINE_WIFI_MMAP_SIZE < size || (OFFLINE_CELL_MMAP_SIZE + size) > reserve_size) {
			pr_err("[%s]:line[%d] size.0x%lx.\n", __func__, __LINE__, size);
			return -EINVAL;
		}
		page += OFFLINE_CELL_MMAP_SIZE;
	} else {
...
/*lint +e826*/
/*lint -e785 -e64*/
static const struct file_operations huawei_flp_fops = {
	.owner          = THIS_MODULE,
	.llseek         = no_llseek,
...
*******************************************************************************************/
static struct miscdevice huawei_flp_miscdev = {
	.minor =    MISC_DYNAMIC_MINOR,
	.name =     "flp",
	.fops =     &huawei_flp_fops,
};
/*lint +e785 +e64*/

/*******************************************************************************************
Function:       huawei_flp_register
Description:
Data Accessed:  no
Data Updated:   no
Input:          void
...
int huawei_flp_register(void)
{
	...
	register_xhub_recovery_notifier(&sensor_reboot_notify);
	...
	ret = misc_register(&huawei_flp_miscdev);
	if (ret != 0)    {
		pr_err("cannot register huawei flp err=%d\n", ret);
		goto destroy_wq;
	}
	pr_info("huawei_flp_register success\n");
	return 0;
...
EXPORT_SYMBOL_GPL(huawei_flp_register);
/*lint +e655*/

/*******************************************************************************************
Function:       huawei_flp_unregister
...
Return:        void
*******************************************************************************************/
 int huawei_flp_unregister(void)
{
	destroy_workqueue(g_flp_dev.wq);
	(void)inputhub_wrapper_unregister_event_notifier(TAG_FLP,
		CMD_SHMEM_AP_RECV_REQ, flp_shmem_resp);
	...
	genl_unregister_family(&flp_genl_family);
	misc_deregister(&huawei_flp_miscdev);
	return 0;
}
EXPORT_SYMBOL_GPL(huawei_flp_unregister);
