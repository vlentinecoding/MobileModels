...
static int ar_send_cmd_through_shmem(ar_hdr_t *head, const char  *buf, size_t count)
{
	...
	if (len > sizeof(ar_hdr_t) && buf != NULL) {
					  ...
					  unsigned int subtype, const char *buf, size_t count)
{
	...
	return inputhub_wrapper_pack_and_send_cmd_unified(cmd_tag, cmd_type, sub_cmd, buf, count, NULL, false);
#else
...
static void fifo_in(ar_data_buf_t *pdata, char *data, unsigned int len, unsigned int align)
{
	...
	if ((pdata == NULL) || (data == NULL) || (pdata->data_buf == NULL)) {
		ctxhub_err("hismart:[%s]:pdata[%pK] data[%pK] parm is null\n", __func__, pdata, data);
		return ;
	}
	...
	if ((pdata->data_count + len) > pdata->buf_size) {
		...
		if (deta % align) {
			ctxhub_err("hismart:[%s]:line[%d]fifo_in data not align\n", __func__, __LINE__);
...
static int  env_close_cmd(ar_port_t *ar_port, int force)
{
	...
	if (!(FLP_ENVIRONMENT & ar_port->channel_type) || !(FLP_ENVIRONMENT & g_ar_dev.service_type)) {
		ctxhub_info("hismart:[%s]:line[%d] channel_type[0x%x] service_type[0x%x]\n", __func__, __LINE__, ar_port->channel_type, g_ar_dev.service_type);
		goto ENV_CLOSE_ERR;
	}
...
STATIC int env_open_cmd(ar_port_t *ar_port)
{
	...
	if (ret) {
		ctxhub_err("hismart:[%s]:line[%d] hub not support env, open env app[%d]\n", __func__, __LINE__, ret);
		goto ENV_INIT_ERR;
	}
	ar_port->channel_type |= FLP_ENVIRONMENT;
	g_ar_dev.service_type |= FLP_ENVIRONMENT;
...
static int  env_disable_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	mutex_lock(&g_ar_dev.lock);
	if (!(FLP_ENVIRONMENT & ar_port->channel_type) || !(FLP_ENVIRONMENT & g_ar_dev.service_type)) {
		ctxhub_err("hismart:[%s]:line[%d] Env has not open, channel[0x%x] service[0x%x]\n", __func__, __LINE__, ar_port->channel_type, g_ar_dev.service_type);
		ret = -EPERM;
		goto ENV_DIS_ERR;
	}
...
int get_ar_data_from_mcu(const pkt_header_t *head)
{
	...
	list_for_each(pos, &g_ar_dev.list) {
		...
		for (i = 0; i < pd->context_num; i++) {
			if (pcd >= data_tail) {
				ctxhub_err("hismart:[%s]:line[%d] break[%d][%pK]\n", __func__, __LINE__, i, pcd);
				break;
			}
...
static int get_env_data_from_mcu(const pkt_header_t *head)
{
	...
	if (NULL == ar_port) {
		ctxhub_err("hismart:[%s]:line[%d] env_port must init first\n", __func__, __LINE__);
		goto ENV_DATA_FIN;
	}
	...
	for (i = 0; i < pd->context_num; i++) {
		...
		if (AR_ENVIRONMENT_END <= pevent->context) {
			ctxhub_err("hismart:[%s]:line[%d] pevent->context[%d]idx[%d]\n", __func__, __LINE__, pevent->context, i);
			goto ENV_DATA_FIN;
		}

		if (pevent->buf_len > CONTEXT_PRIVATE_DATA_MAX) {
			ctxhub_err("hismart:[%s]:line[%d] type[%d]len[%d] private data too large.idx[%d]\n", __func__, __LINE__, pevent->event_type, pevent->buf_len, i);
			goto ENV_DATA_FIN;
		}

		port_etype = g_ar_dev.envdev_priv.env_enable[pevent->context].head.event_type;
		ctxhub_info("hismart:[%s]:line[%d] num[%u]type[%u]ctt[%u]msec[%llu]len[%u] channel_type[0x%x] confident[%u][%u]\n", __func__, __LINE__,
	...
	ret = fifo_out(pdata, data, pdata->data_count);
	if (ret) {
		ctxhub_err("hismart:[%s]:line[%d] fifo_out ERR\n", __func__, __LINE__);
		nlmsg_free(skb);
		goto ENV_DATA_FIN;
	}
	HUAWEI_AR_DEBUG_DUMP_OPT(HUAWEI_AR_DEBUG_DUMP, "env: ", DUMP_PREFIX_ADDRESS,
...
static int state_v2_cmd(struct read_info *rd, unsigned long arg)
{
	...
	if ((0 == usr_len) || (usr_len > (MAX_PKT_LENGTH - sizeof(ar_state_t)))) {
		ctxhub_err("hismart:[%s]:line[%d] length err[%lu] buflen_limit[%lu]\n", __func__, __LINE__, usr_len, MAX_PKT_LENGTH - sizeof(ar_state_t));
		ret = -EINVAL;
		goto STATE_V2_ERR;
	}
	ctxhub_info("hismart:[%s]:line[%d] usr_len[%lu]\n", __func__, __LINE__, usr_len);
...
static int env_state_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	if (ret) {
		ctxhub_err("hismart:[%s]:line[%d] ar_send_cmd_from_kernel_response err[%d]\n", __func__, __LINE__, rd.errno);
		goto ENV_STATE_ERR;
	}

	ret = state_v2_cmd(&rd, arg);
	if (ret < 0) {
		ctxhub_err("hismart:[%s]:line[%d] state_v2_cmd err[%d]\n", __func__, __LINE__, ret);
		goto ENV_STATE_ERR;
	}
...
static int  env_init_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	if (ret) {
		ctxhub_err("hismart:[%s]:line[%d] env_open_cmd err\n", __func__, __LINE__);
		return ret;
	}
	...
	if (copy_from_user((void *)&env_init,  (void __user *)((uintptr_t)arg), sizeof(env_init_t))) {
		ctxhub_err("hismart:[%s]:line[%d] copy_from_user error env_init\n", __func__, __LINE__);
		ret = -EDOM;
		goto ENV_INIT_ERR;
	}

	if (AR_ENVIRONMENT_END <= env_init.context) {
		ctxhub_err("hismart:[%s]:line[%d] context too big[%d]\n", __func__, __LINE__, env_init.context);
		ret = -EINVAL;
		goto ENV_INIT_ERR;
	}
...
static int  env_data_cmd(ar_port_t *ar_port, unsigned long arg)
{
	...
	if (copy_from_user((void *)&download,  (void __user *)((uintptr_t)arg), sizeof(env_data_download_t))) {
		ctxhub_err("hismart:[%s]:line[%d] copy_from_user  download error \n", __func__, __LINE__);
		ret = -EDOM;
		goto ENV_DATA_FIN;
	}

	if (download.len > DATABASE_DATALEN || !download.len || download.bufaddr == NULL) {
		ctxhub_err("hismart:[%s]:line[%d] copy_from_user len to big[%d] \n", __func__, __LINE__, download.len);
		ret = -E2BIG;
		goto ENV_DATA_FIN;
	}
	...
	if (ret)
		ctxhub_err("hismart:[%s]:line[%d] shmem_send error \n", __func__, __LINE__);
#endif
...
static int ar_common_ioctl_open_service(void)
{
	...
	if (g_ar_dev.service_type & FLP_ENVIRONMENT) {
		ret = ar_send_cmd_from_kernel_response(TAG_ENVIRONMENT, CMD_CMN_OPEN_REQ, 0, NULL, (size_t)0, &rd);
		if (ret) {
			ctxhub_err("hismart:[%s]:line[%d] ar_send_cmd_from_kernel_response ret[%d]\n", __func__, __LINE__, ret);
			return ret;
		}
...
static int ar_common_ioctl(ar_port_t *ar_port, unsigned int cmd, unsigned long arg)
{
	...
	if (FLP_IOCTL_COMMON_RELEASE_WAKELOCK != cmd) {
		if (copy_from_user(&data, (void *)((uintptr_t)arg), sizeof(unsigned int))) {
			ctxhub_err("hismart:[%s]:line[%d] flp_ioctl copy_from_user error[%d]\n", __func__, __LINE__, ret);
			return -EFAULT;
		}
	}
	...
	case FLP_IOCTL_COMMON_SLEEP:
		ctxhub_info("hismart:[%s]:line[%d] start timer %d\n", __func__, __LINE__, data);
		/*if timer is running just delete it ,then restart it*/
		huawei_softtimer_delete(&ar_port->sleep_timer);
		mutex_lock(&g_ar_dev.lock);
		ret = huawei_softtimer_modify(&ar_port->sleep_timer, data);
		mutex_unlock(&g_ar_dev.lock);
		if (!ret) {
			huawei_softtimer_add(&ar_port->sleep_timer);
		}
		break ;
	...
	case FLP_IOCTL_COMMON_RELEASE_WAKELOCK:
		...
		break;
	default:
		ctxhub_err("hismart:[%s]:line[%d] ar_common_ioctl input cmd[0x%x] error\n", __func__, __LINE__, cmd);
		return -EFAULT;
	}
	return 0;
}
...
static int ar_open(struct inode *inode, struct file *filp)/*lint -e715*/
{
	...
	if (count > 32) {
		ctxhub_err("hismart:[%s]:line[%d] ar_open clinet limit\n", __func__, __LINE__);
		ret = -EACCES;
		goto ar_open_ERR;
	}
	...
	huawei_softtimer_create(&ar_port->sleep_timer, ar_sleep_timeout, (unsigned long)((uintptr_t)ar_port), 0);
	INIT_WORK(&ar_port->work, ar_timerout_work);

	list_add_tail(&ar_port->list, &g_ar_dev.list);
	wakeup_source_init(&ar_port->wlock, "huawei_ar");
...
static int database_req(const pkt_header_t *head)
{
	...
	if (0 == dbase->len || HISMART_BLOCK_SIZE*HISMART_BLOCK_SIZE < dbase->len) {
		ctxhub_err("hismart:[%s]:line[%d] len[%d] is err\n", __func__, __LINE__, dbase->len);
		return 0;
	}
	...
	ctxhub_info("hismart:[%s]:line[%d] len[%d]\n", __func__, __LINE__, dbase->len);
	HUAWEI_AR_DEBUG_DUMP_OPT(HUAWEI_AR_DEBUG_DUMP, "dbase: ", DUMP_PREFIX_ADDRESS,
	...
	HUAWEI_AR_DEBUG_DUMP_OPT(HUAWEI_AR_DEBUG_DUMP, "data: ", DUMP_PREFIX_ADDRESS,
		16, 1, dbase, dbase->len, true);
...
static int __init ar_init(void)
{
	...
	if (ret) {
		ctxhub_err("hismart:[%s]:line[%d] cannot register huawei ar ret[%d]\n", __func__, __LINE__, ret);
		goto AR_PROBE_GEN;
	}
	...
	if (g_dts_status == SENSORHUB_DTS_ENABLED) {
		...
		register_xhub_recovery_notifier(&ar_reboot_notify);
	}
	return ret;
...
static int huawei_ar_pm_suspend(struct device *dev)
{
	return 0;
}
 ...
 * 驱动resume回调
 */
static int huawei_ar_pm_resume(struct device *dev)
{
	struct list_head    *pos = NULL;
	ar_port_t      *ar_port = NULL;
	unsigned int cnt = 0;
...
/*lint -e785*/
struct dev_pm_ops huawei_ar_pm_ops = {
#ifdef CONFIG_PM_SLEEP
	.suspend = huawei_ar_pm_suspend,
	.resume  = huawei_ar_pm_resume,
#endif
};
...
/*lint +e785*/
/*lint -e64*/
static int __init huawei_ar_init(void)
{
	return platform_driver_register(&generic_ar_platdrv);
}
/*lint +e64*/
static void __exit huawei_ar_exit(void)
{
	platform_driver_unregister(&generic_ar_platdrv);
}
/*lint -e528 -esym(528,*) */
late_initcall_sync(huawei_ar_init);
module_exit(huawei_ar_exit);
