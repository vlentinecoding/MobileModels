...
static void free_alloc_list(struct list_head *head)
{
			    ...
			    struct list_head *head, u32 cmd_to_ta)
{
	...
	if (i < nents) {
		pr_err("[%s], invalid nents %u or head!\n", __func__, nents);
		ret = -EINVAL;
		goto out;
	}
			...
			struct ion_seccg_heap *seccg_heap, unsigned int count)
{
	...
	if (seccg_heap->heap_attr == HEAP_SECURE_TEE) {
		...
		change_secpage_range(virt, size, 0);
	...
	return 0;
err_out2:
	change_secpage_range(virt, size, 1);
			 ...
			 struct ion_buffer *buffer, unsigned long size)
{
	...
	if (seccg_heap->heap_attr == HEAP_NORMAL) {
		...
		__dma_unmap_area(page_address(page), size, DMA_FROM_DEVICE);
	}
			      ...
			      struct ion_buffer *buffer)
{
	...
	if (!(buffer->flags & ION_FLAG_SECURE_BUFFER)) {
		...
		if (buffer->flags & ION_FLAG_CACHED)
			__dma_unmap_area(page_address(page), buffer->size, DMA_BIDIRECTIONAL);
	}
...
int __seccg_fill_watermark(struct ion_seccg_heap *seccg_heap)
{
	...
	(void)memset_s(page_address(pg), size, 0x0, size); /* unsafe_function_ignore: memset */
	gen_pool_free(seccg_heap->pool, page_to_phys(pg), size);
