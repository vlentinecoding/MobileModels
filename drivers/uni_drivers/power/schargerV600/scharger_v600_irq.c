...
static void hi6526_mask_all_irq(struct hi6526_device_info *di)
{
	/* mask all irq output  */
	(void)hi6526_write_mask(di, CHG_IRQ_MASK_ALL_ADDR,
		CHG_IRQ_MASK_ALL_MSK,
		CHG_IRQ_MASK_ALL_SHIFT,
		CHG_IRQ_MASK_ALL_MSK);
}

static void hi6526_mask_pd_irq(struct hi6526_device_info *di)
{
	/* mask pd irq output  */
	(void)hi6526_write_mask(di, CHG_IRQ_MASK_ALL_ADDR, CHG_IRQ_MASK_PD_MSK,
		CHG_IRQ_MASK_ALL_SHIFT, CHG_IRQ_MASK_PD_MSK);
	/* mask irq_cc_ov irq_cc_ovp and irq_cc_uv output */
	(void)hi6526_write_mask(di, CHG_IRQ_MASK_5,
...
{
	/* buck irq unmask */
	(void)hi6526_write(di, CHG_IRQ_MASK_0, CHG_IRQ_MASK_0_VAL);
	(void)hi6526_write(di, CHG_IRQ_MASK_1, CHG_IRQ_MASK_1_VAL);
	(void)hi6526_write(di, CHG_IRQ_MASK_2, CHG_IRQ_MASK_2_VAL);
	(void)hi6526_write(di, CHG_IRQ_MASK_3, CHG_IRQ_MASK_3_VAL);
	(void)hi6526_write(di, CHG_IRQ_MASK_4, CHG_IRQ_MASK_4_VAL);

	/* except irq_cc_ov irq_cc_ovp and irq_cc_uv */
	(void)hi6526_write_mask(di, CHG_IRQ_MASK_5, CHG_IRQ_MASK_5_IRQ_MSK,
		CHG_IRQ_MASK_5_IRQ_SHIFT, CHG_IRQ_MASK_5_VAL);
	(void)hi6526_write(di, CHG_IRQ_MASK_6, CHG_IRQ_MASK_6_VAL);
	(void)hi6526_write(di, CHG_IRQ_MASK_7, CHG_IRQ_MASK_7_VAL);

	/* FCP MASK */
	(void)hi6526_write(di, CHG_FCP_IRQ5_MASK_ADDR, 0x03);
	(void)hi6526_write(di, 0x9b, 0xff);
	(void)hi6526_write(di, 0x9c, 0xff);
...
static void hi6526_direct_dmd_report(struct hi6526_device_info *di, u32 fault_type, struct nty_data *data)
{
#ifdef CONFIG_DIRECT_CHARGER
	struct atomic_notifier_head *dc_fault_notifier_list = NULL;

	if ((di == NULL) || !(di->chg_mode == SC || di->chg_mode == LVC))
		return;

	if (di->chg_mode == SC)
		sc_get_fault_notifier(&dc_fault_notifier_list);
	else
		lvc_get_fault_notifier(&dc_fault_notifier_list);

	atomic_notifier_call_chain(dc_fault_notifier_list,
		fault_type, data);
#endif
}
...
void hi6526_direct_charge_fault_handle(struct hi6526_device_info *di, u32 lvc_sc_irq_state)
{
	...
	hi6526_direct_dmd_report(di, fault_type, data);
	ret = hi6526_lvc_enable(0, di);
	if (ret != 0)
		scharger_err("hi6526_lvc_enable err\n");
	ret = hi6526_sc_enable(0, di);
	if (ret != 0)
		scharger_err("hi6526_sc_enable err\n");
}

static void hi6526_chip_overtemp_handle(struct hi6526_device_info *di)
{
	...
	hi6526_direct_dmd_report(di, DC_FAULT_TDIE_OTP, data);
}

static void hi6526_buck_vbat_ovp_handle(struct hi6526_device_info *di)
{
	int i, ret;
	u8 vbat_ovp_cnt = 0;
	u8 irq_st0 = 0;
	...
	for (i = 0; i < 5; i++) { /* status number is 5 */
		ret = hi6526_read(di, CHG_IRQ_STATUS0, &irq_st0);
...
static void hi6526_buck_fault_handle(struct hi6526_device_info *di, u32 buck_irq_state)
{
	...
	if (buck_irq_state & FAULT_BUCK_VBAT_OVP)
		hi6526_buck_vbat_ovp_handle(di);
...
static void hi6526_force_clear_all_irq(struct hi6526_device_info *di)
{
	int i, ret;
	u8 value = 0;
	...
	for (i = 0; i < irq_reg_num; i++) {
		ret = hi6526_read(di, CHG_BUCK_IRQ_ADDR + i, &value);
		if (ret)
			scharger_inf("%s i2c read error i %d\n", __func__, i);

		if (value) {
			scharger_inf("%s:irq flag%d:0x%x\n", __func__, i, value);
			ret = hi6526_write(di, CHG_BUCK_IRQ_ADDR + i, value);
			if (ret)
...
again:
	ret = hi6526_read(di, CHG_IRQ_ADDR, &irq_state);
	scharger_inf("%s_role_%d %d:irq_state:0x%x\n", __func__, di->param_dts.ic_role,
		retry_time, irq_state);

	if (irq_state & CHG_BUCK_IRQ) {
		/* 3: totally 3 bytes to be read */
		(void)hi6526_read_block(di, CHG_BUCK_IRQ_ADDR,
			(u8 *)&buck_irq_state, 3);
		scharger_inf(
			"%s_role_%d:CHG_BUCK_IRQ,irq_state:0x%x,buck_irq_state:0x%6.6x\n",
			__func__, di->param_dts.ic_role, irq_state, buck_irq_state);
		buck_irq_state &= BUCK_IRQ_MASK;
		hi6526_buck_fault_handle(di, buck_irq_state);
		(void)hi6526_write_block(di, CHG_BUCK_IRQ_ADDR,
			(u8 *)&buck_irq_state, 3);
	}
	if (irq_state & CHG_LVC_SC_IRQ) {
		(void)hi6526_read_block(di, CHG_LVC_SC_IRQ_ADDR,
			(u8 *)&lvc_sc_irq_state, 3);
		scharger_err(
			"%s_role_%d:CHG_LVC_SC_IRQ,irq_state:0x%x,lvc_sc_irq_state:0x%6.6x\n",
			__func__, di->param_dts.ic_role, irq_state, lvc_sc_irq_state);
		lvc_sc_irq_state &= LVC_SC_IRQ_MASK;
		hi6526_direct_charge_fault_handle(di, lvc_sc_irq_state);
		(void)hi6526_write_block(di, CHG_LVC_SC_IRQ_ADDR,
			(u8 *)&lvc_sc_irq_state, 3);
	}
	if (irq_state & CHG_OTHERS_IRQ) {
		(void)hi6526_read_block(di, CHG_OTHERS_IRQ_ADDR,
			(u8 *)&others_irq_state, 3);
		scharger_inf(
			"%s_role_%d:CHG_OTHERS_IRQ,irq_state:0x%x,others_irq_state:0x%6.6x\n",
			__func__, di->param_dts.ic_role, irq_state, others_irq_state);
		hi6526_others_irq2str(others_irq_state);

		if (others_irq_state & OTHERS_OTP_IRQ_MASK)
			hi6526_chip_overtemp_handle(di);

		if (others_irq_state & OTHERS_VBAT_LV_IRQ_MASK)
			hi6526_vbat_lv_handle(di);

		others_irq_state &= OTHERS_IRQ_MASK;
		(void)hi6526_write_block(di, CHG_OTHERS_IRQ_ADDR,
			(u8 *)&others_irq_state, 3);
	}
	if (irq_state & CHG_FCP_IRQ) {
		/* 2: totally 2 bytes to be read */
		ret = ret || hi6526_read_block(di, CHG_FCP_IRQ_ADDR1,
			(u8 *)&fcp_irq_state1, 2);
		ret = ret || hi6526_read_block(di, CHG_FCP_IRQ_ADDR2,
			(u8 *)&fcp_irq_state2, 2);
		ret = ret || hi6526_read(di, CHG_FCP_IRQ_ADDR3, &fcp_irq_state3);

		scharger_inf("%s_role_%d:CHG_FCP_IRQ, irq_state:0x%x, fcp_irq_state1:0x%x, fcp_irq_state2:0x%x, fcp_irq_state3:0x%x\n",
			__func__, di->param_dts.ic_role, irq_state, fcp_irq_state1,
		...
		ret = ret || hi6526_write_block(di, CHG_FCP_IRQ_ADDR1,
			(u8 *)&fcp_irq_state1, 2);
		ret = ret || hi6526_write_block(di, CHG_FCP_IRQ_ADDR2,
			(u8 *)&fcp_irq_state2, 2);
		ret = ret || hi6526_write(di, CHG_FCP_IRQ_ADDR3, fcp_irq_state3);
	}
	if (ret)
	...
	if (gpio_val == 0) {
		...
		if (retry_time > IRQ_RETRY_TIME) {
			scharger_err("%s_role_%d:gpio is low,retry time %d MAX\n",
				__func__, di->param_dts.ic_role, retry_time);
			/* clear all irq */
			hi6526_force_clear_all_irq(di);
		} else {
			retry_time++;
			goto again;
		}
...
static void hi6526_irq_clear(struct hi6526_device_info *di)
{
	int i, ret;
	u8 irq_state = 0;
	u8 val = 0;
	...
	for (i = 0; i < 8; i++) { /* 8 continuous regs to be read */
		ret = ret || hi6526_read(di, CHG_BUCK_IRQ_ADDR + i, &val);
		if (val) {
			scharger_err("[%s]:irq_state=0x%x, irq[%d]=0x%x\n",
				__func__, irq_state, i, val);
			ret = ret || hi6526_write(di, CHG_BUCK_IRQ_ADDR + i, val);
		}
	}
	if (ret)
