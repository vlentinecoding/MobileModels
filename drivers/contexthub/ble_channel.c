...
static int ble_shmem(hal_config_t *config,unsigned short int shmem_cmd)
{
	int ret;
...
static int ble_ioctl_get(unsigned long arg, unsigned int subcmd,struct read_info *rd){
	...
	if (subcmd == SUB_CMD_BLE_GET_ABILITY_REQ) {
		pr_info("[%s]: get ability %d.\n", __func__, *(int *)rd->data);
		if (ret == 0) {
...
static long ble_channel_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct read_info rd;
	if (file == NULL) {
		pr_err("[%s]: ble_channel_ioctl parameter error.\n", __func__);
		return -EINVAL;
	}
	...
	case BLE_IOCTL_GET_DEVICE_LIST:
		...
		break;
	case BLE_IOCTL_GET_CAPABILITY:
		pr_info("[%s]: enter into ble_ioctl_get.\n", __func__);
		ret = (long)ble_ioctl_get(arg, SUB_CMD_BLE_GET_ABILITY_REQ, &rd);
		break;
...
static int __init ble_channel_register(void)
{
	...
	pr_info("[%s]: ble_channel_register success.\n", __func__);
	return 0;
err:
