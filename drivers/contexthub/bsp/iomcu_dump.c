...
static int write_bbox_log_info_to_sharemem(struct rdr_register_module_result *rdr_info)
{
	...
	pr_info("%s: bbox log addr: low 0x%x, high 0x%x, log len: 0x%x\n",
		__func__, (u32)(rdr_info->log_addr), (u32)((rdr_info->log_addr) >> 32), rdr_info->log_len);
	return 0;
}
...
static int write_sh_dump_file(void)
{
	...
	if ((g_sensorhub_extend_dump_buff != NULL) && (g_smplat_scfg != NULL)) {
		dzh = (struct dump_zone_head *)g_sensorhub_extend_dump_buff;
...
static int rdr_sh_thread(void *arg)
{
	...
	while (1) {
		...
		if (g_enable_dump && g_smplat_scfg != NULL) {
...
static int __iom3_need_recovery(int modid, enum exp_source f, bool rdr_reset)
{
	...
	if (old_state == IOM3_RECOVERY_IDLE) {
		...
		if (f > SH_FAULT_INTERNELFAULT && g_smplat_scfg != NULL)
		...
		if (g_enable_dump && g_dump_extend_size && !g_sensorhub_extend_dump_buff) {
			...
			pr_warn("%s alloc pages logic %pK phy addr 0x%lx\n", __func__,
					   g_sensorhub_extend_dump_buff,
					   virt_to_phys(g_sensorhub_extend_dump_buff));

			if (g_sensorhub_extend_dump_buff  && g_smplat_scfg != NULL) {
...
{
	iomcu_rdr_exit();
	wakeup_source_trash(&g_iom3_rec_wl);
	if (g_iom3_rec_wq != NULL) {
		destroy_workqueue(g_iom3_rec_wq);
		g_iom3_rec_wq = NULL;
	}
	(void)blocking_notifier_chain_unregister(&iom3_recovery_notifier_list, &recovery_notify);
}
