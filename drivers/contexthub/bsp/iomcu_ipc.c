...
int iomcu_route_recv_mcu_data(const char *buf, unsigned int length)
{
	...
	if (head->hw_trans_mode == IOMCU_TRANS_SHMEM_TYPE) {
		...
		notifier_work->data =
		kzalloc(head->length + sizeof(struct pkt_header), GFP_ATOMIC);
		if (notifier_work->data == NULL) {
			kfree(notifier_work);
...
{
	pr_info("\ninputhub_max_latency_msecs: %llums\n"
		"tagcmd_max_latency_msecs:tag %x,%x,%x,%llums",
		g_iomcu_ipcmntn.inputhub_max_latency_msecs,
		g_iomcu_ipcmntn.tagcmd.tag, g_iomcu_ipcmntn.tagcmd.cmd,
		g_iomcu_ipcmntn.tagcmd.tranid, g_iomcu_ipcmntn.tagcmd_max_latency_msecs);
}
