				   ...
				   unsigned int max_cores)
{
	...
	struct device_node *cpu_cluster_node;
	u32 current_buf[32] = {0};
...
#ifdef CONFIG_CPU_FREQ_POWER_STAT
	sprintf(param_name, "honor,cpu_cluster_%d", cluster_id);

	pr_err("honor_debug: %s, %d, compatible: %s\n", __func__, __LINE__, param_name);

	cpu_cluster_node = of_find_compatible_node(NULL, NULL, param_name);
	if (!cpu_cluster_node) {
		pr_err("honor_power: %s, %d find cluster node %s error\n", __func__, __LINE__, param_name);
		goto err;
	}

	len = of_property_count_u32_elems(cpu_cluster_node, "honor,cpu-current-table");
	if (len <= 0) {
		pr_err("honor_power: %s, %d find cluster table error\n", __func__, __LINE__);
		goto err;
	}

	ret = of_property_read_u32_array(cpu_cluster_node, "honor,cpu-current-table", current_buf, len);
	if (ret)
		pr_err("honor_power: %s, %d find cluster table error\n", __func__, __LINE__);

	for (i = 0; i < len; i++)
		c->table[i].electric_current = current_buf[i];
