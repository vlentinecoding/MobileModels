...
#include "../../../soc/qcom/pil-q6v5-mss/pil_subsys_log.h"
#endif
	...
	struct file_info *info)
{
	...
	MHI_ERR("%s\n", sfr_buf);
#ifdef CONFIG_SUBSYS_CRASH_REASON_UPLOAD
	if (get_logusertype_flag() == COMMERCIAL_USER || get_logusertype_flag() == OVERSEA_COMMERCIAL_USER) {
		report_subsys_crash_log("wlan", sfr_buf, strlen(sfr_buf));
	}
#endif
err:
	kfree(sfr_buf);
