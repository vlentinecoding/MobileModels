...
#ifdef CONFIG_RAINBOW_HIMNTN
	if (!console_enabled && !strcmp(id->compatible, "qcom,msm-geni-console"))
		return 0;
#endif
...
#ifdef CONFIG_RAINBOW_HIMNTN
static bool is_msm_geni_console_device(struct device *dev)
{
	const struct of_device_id *id = of_match_device(msm_geni_device_tbl, dev);
	if (!id) {
		dev_err(dev, "%s: No matching device found\n", __func__);
		return -ENODEV;
	}
	dev_dbg(dev, "%s: %s\n", __func__, id->compatible);

	return !strcmp(id->compatible, "qcom,msm-geni-console");
}
#endif
...
#ifdef CONFIG_RAINBOW_HIMNTN
	if (!console_enabled && is_msm_geni_console_device(&pdev->dev)) {
		pr_info("Skip console remove\n");
		return 0;
	}
#endif
...
#ifdef CONFIG_RAINBOW_HIMNTN
	if (!console_enabled && is_msm_geni_console_device(dev)) {
		pr_info("Skip console runtime suspend\n");
		return 0;
	}
#endif
...
#ifdef CONFIG_RAINBOW_HIMNTN
	if (!console_enabled && is_msm_geni_console_device(dev)) {
		pr_info("Skip console runtime resume\n");
		return 0;
	}
#endif
...
static int __init msm_geni_serial_init(void)
{
	...
	int i;
#ifdef CONFIG_RAINBOW_HIMNTN
	console_enabled = cmd_himntn_item_switch(HIMNTN_ID_UART_LOG_SWITCH);
#endif
