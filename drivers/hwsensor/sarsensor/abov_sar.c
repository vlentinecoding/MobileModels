...
//#include <linux/abov_sar.h> /* main struct, interrupt,init,pointers */
...
static void touchProcess(pabovXX_t this)
{
	...
	if (this && pDevice) {
		...
		if (unlikely((buttons == NULL) || (input_ch0 == NULL))) {
		...
		for (counter = 0; counter < numberOfButtons; counter++) {
			...
			case IDLE: /* Button is being in far state! */
				...
				} else if ((i & pCurrentButton->mask) == (pCurrentButton->mask & 0x01)) {
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	client->dev.platform_data = pplatData;
        ...
        if (IS_ERR(pplatData->cap_vdd)) {
			...
			ret = regulator_set_voltage(pplatData->cap_vdd, 2800000, 2800000);
			if (ret) {
			...
			ret = regulator_set_load(pplatData->cap_vdd, 100000);
			if (ret) {
				regulator_put(pplatData->cap_vdd);
				LOG_DBG("%s: Error %d enable regulator\n",
						__func__, ret);
	...
	if (this) {
		...
		this->statusFunc = touchProcess; /* RELEASE_STA T&& TOUCH_STAT */
		...
		LOG_INFO("PYL: Begin to create capsense directory\n");
