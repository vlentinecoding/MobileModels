...
static bool tcs3408_vdd_enable;
static struct regulator *tcs3408_ldo = NULL;
...
static void tcs3707_report_data(int value[])
{
	UINT8 i;
	char data[6] = {'\0'};
	tcs3408_channel_data[0] = '\0';
	for (i = 0; i < 5; i++) {
		sprintf(data, "%d", value[i]);
		hwlog_info("%s tcs3408_channel_data : %s\n", __func__, data);
		strcat(tcs3408_channel_data, data);
		strcat(tcs3408_channel_data, "u");
	}
	hwlog_info("%s now the tcs3408_channel_data : %s\n", __func__, tcs3408_channel_data);
}
...
static INT32 tcs3408_chip_get_atime(struct color_chip *chip)
{
	UINT8 atime = 0;
	UINT8 itime = 0;
	...
	color_sensor_get_byte(chip->client, TCS3707_ATIME_REG, &atime);
	itime = (UINT8)(((atime + 1) * (DEFAULT_ASTEP + 1)) / TCS3408_ASTEP_SCALE);
	hwlog_info("%s: now the atime = %d, itime = %dms\n", __func__, atime, itime);
	return atime;
}

static UINT32 tcs3408_chip_get_itime(struct color_chip *chip)
{
	UINT8 atime = 0;
	UINT32 itime = 0;
...
void tcs3707_als_timer_wrk(struct timer_list *data)
{
...
void tcs3707_flc_timer_wrk(struct timer_list *data)
{
	//struct color_chip *chip = (struct color_chip *)data;
...
static void tcs3408_store_calibrate_result(UINT32 value[])
{
	UINT8 i;
	char data[6] = {'\0'};
	memset(tcs3408_calibrate_result, 0, sizeof(char) * 64);
	if (report_calibrate_result)
		tcs3408_calibrate_result[0] = '1';
	else
		tcs3408_calibrate_result[0] = '0';
	tcs3408_calibrate_result[1] = 'u';
	for (i = 0; i < 5; i++) {
		sprintf(data, "%d", value[i]);
		hwlog_info("%s tcs3408_calibrate_result : %s\n", __func__, data);
		strcat(tcs3408_calibrate_result, data);
		strcat(tcs3408_calibrate_result, "u");
	}
	hwlog_info("%s now the tcs3408_calibrate_result : %s\n", __func__, tcs3408_calibrate_result);
}

char *tcs3707_show_calibrate(struct color_chip *chip,
	struct at_color_sensor_output_t *out)
{
	UINT32 i = 0;
	UINT32 cali_ratio[5] = {0};
	...
	cali_ratio[0] = chip->cali_ctx.cal_c_rst[4];
	cali_ratio[1] = chip->cali_ctx.cal_r_rst[4];
	cali_ratio[2] = chip->cali_ctx.cal_g_rst[4];
	cali_ratio[3] = chip->cali_ctx.cal_b_rst[4];
	cali_ratio[4] = chip->cali_ctx.cal_w_rst[4];
	tcs3408_store_calibrate_result(cali_ratio);
	return tcs3408_calibrate_result;
}
...
static char *tcs3408_chip_sensor_report_data(void)
{
	hwlog_info("%s report channel_data : %s\n", __func__, tcs3408_channel_data);
	return tcs3408_channel_data;
}
...
int tcs3707_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	pr_info("addr=0x%x\n", client->addr);
	tcs3408_ldo = devm_regulator_get(&client->dev, "colorsensor-vdd");
	if (IS_ERR_OR_NULL(tcs3408_ldo)) {
		pr_err("regulator get fail\n");
	}
	ret = regulator_enable(tcs3408_ldo);
	if (ret != 0) {
		pr_err("regulator enable fail\n");
	} else {
		tcs3408_vdd_enable = true;
	}
	...
	timer_setup(&chip->work_timer, tcs3707_als_timer_wrk, 0);
	...
	timer_setup(&chip->fd_timer, tcs3707_flc_timer_wrk, 0);
	...
	chip->color_chip_sensor_report_data = tcs3408_chip_sensor_report_data;
	chip->color_chip_get_atime = tcs3408_chip_get_atime;
	chip->color_chip_get_itime = tcs3408_chip_get_itime;
...
int ams_tcs3707_remove(struct i2c_client *client)
{
	...
	if (tcs3408_vdd_enable) {
		regulator_disable(tcs3408_ldo);
		regulator_put(tcs3408_ldo);
	}
