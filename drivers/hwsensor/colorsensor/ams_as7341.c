 ...
 * Date:    2020.07.20
 * Description: ams_as7324 driver module
 *
...
static bool as7341_vdd_enable;
static struct regulator *as7341_ldo = NULL;
...
static void as7341_get_optimized_gain_itime(UINT32 maximum_adc, UINT32 highest_adc,
	UINT8 *p_gain, UINT32 *p_itime_ms, UINT8 *p_saturation)
{
	UINT32 gain_change;
	UINT8  old_gain = *p_gain;
	...
	if (highest_adc >= maximum_adc) {
		/* saturation detected */
		if (*p_itime_ms > AS7341_ITIME_DEFAULT) {
			*p_itime_ms -= AS7341_ITIME_STEP;
			hwlog_info("%s: decrease integration time to %u\n", __func__, *p_itime_ms);
		} else {
			if (*p_gain > LOW_AUTO_GAIN_VALUE)
				*p_gain /= AUTO_GAIN_DIVIDER;
			else
				*p_gain = LOWEST_GAIN_ID;
			hwlog_info("%s: decrease gain to %u\n", __func__, *p_gain);
		}
		...
		*p_saturation = IS_SATURATION;
		...
		hwlog_info("%s: maximum_adc %u highest_adc %u gain_change %u gain %u itime %u\n",
					__func__, maximum_adc, highest_adc, gain_change, *p_gain, *p_itime_ms);
		if (gain_change == 0) {
			/* value is in (80%, 100%) maximum adc range, almost saturation */
			if (*p_itime_ms > AS7341_ITIME_DEFAULT) {
				*p_itime_ms -= AS7341_ITIME_STEP;
				hwlog_info("%s: decrease integration time to %u\n", __func__, *p_itime_ms);
			} else {
			...
			/* value is in [0%, 80%] maximum adc range, increase the integration time or gain */
			if (old_gain == MAX_GAIN_ID) {
				/* make sure there is no saturation */
				if (highest_adc < 1024) {
					if (*p_itime_ms < AS7341_ITIME_MAX) {
						*p_itime_ms += AS7341_ITIME_STEP;
						hwlog_info("%s: increase integration time to %u\n", __func__, *p_itime_ms);
					}
				} else if (highest_adc > 4096) {
					if (*p_itime_ms > AS7341_ITIME_DEFAULT) {
						*p_itime_ms -= AS7341_ITIME_STEP;
						hwlog_info("%s: decrease integration time to %u\n", __func__, *p_itime_ms);
					}
				...
				hwlog_info("%s: increase gain to %u\n", __func__, *p_gain);
			}
		}
...
static void as7341_report_data(int value[])
{
	UINT8 i;
	char data[AS7341_CHDATA_LEN] = {'\0'};
	char channel_data_temp[MAX_CHDATA_STR_LEN] = {0};
	errno_t ret;

	for (i = 0; i < AMS_REPORT_DATA_LEN; i++) {
		ret = memset_s(data, AS7341_CHDATA_LEN, 0, AS7341_CHDATA_LEN);
		if (ret != EOK) {
			hwlog_err("%s data memset_s error, ret = %d\n", __func__, ret);
			return;
		}
		ret = sprintf_s(data, sizeof(data), "%du", value[i]);
		if (ret < 0) {
			hwlog_err("%s data sprintf_s error, ret = %d\n", __func__, ret);
			return;
		}
		hwlog_debug("%s channel_data_temp : %s\n", __func__, data);
		strcat(channel_data_temp, data);
	}
	ret = memcpy_s(channel_data, sizeof(channel_data), channel_data_temp, sizeof(channel_data_temp));
	if (ret != EOK) {
		hwlog_err("%s channel_data memcpy_s error, ret = %d\n", __func__, ret);
	}
	hwlog_info("%s now the channel_data : %s\n", __func__, channel_data);
}
...
static INT32 as7341_chip_get_atime(struct color_chip *chip)
{
	UINT8 atime = 0;
	...
	color_sensor_get_byte(chip->client, AS7341_ATIME_REG, &atime);
	hwlog_debug("%s: now the atime = %d\n", __func__, atime);
	return atime;
}

static UINT32 as7341_chip_get_itime(struct color_chip *chip)
{
    UINT8 atime = 0;
    UINT32 itime = 0;

    if (!chip) {
        hwlog_err("%s: chip is NULL\n", __func__);
        return -EFAULT;
    }
    color_sensor_get_byte(chip->client, AS7341_ATIME_REG, &atime);
    // calc itime
    // itime = (atime +1) * (astep + 1) * 2.78us
    // 2000/720 = 2.78
    struct as7341_ctx * as7341_ctx = (struct as7341_ctx *)chip->device_ctx;
    if (!as7341_ctx) {
        hwlog_err("%s: as7341_ctx is NULL\n", __func__);
        return -EFAULT;
    }
    itime = (atime + 1) * (as7341_ctx->alg_ctx.astep + 1) * ITIME_MULTIPLIER / ITIME_DIVIDER / MS_2_US;
    hwlog_debug("%s: now the itime = %u\n", __func__, itime);
    return itime;
}
...
static INT32 as7341_als_get_gain(const void *ctx)
{
	...
	hwlog_debug("now the gain val = %d\n", gain);
	return gain;
}
...
static void as7341_log_buffer(UINT8 *p_name, UINT32 *p_buf, UINT8 size)
{
	...
	hwlog_info("%s : %u, %u, %u, %u, %u, %u, %u, %u, %u, %u\n", p_name,
		p_buf[BUF_DATA_0], p_buf[BUF_DATA_1], p_buf[BUF_DATA_2], p_buf[BUF_DATA_3],
		p_buf[BUF_DATA_4], p_buf[BUF_DATA_5], p_buf[BUF_DATA_6], p_buf[BUF_DATA_7],
		p_buf[BUF_DATA_8], p_buf[BUF_DATA_9]);
}
	...
	uint8_t *p_smux)
{
	...
	int ret;
	uint8_t i, j, msg_num;
	uint8_t buffer[SMUX_NO_RAM_I2C_NUM] = { AS7341_REGADDR_CHAINCMD, CHAIN_CMD_SHIFT_ALS_6BITS };
...
static bool as7341_handle_smux_block(struct as7341_ctx *ctx, bool cal_mode)
{
	...
	if (channel_block == 0) {
		...
		if (cal_mode) {
			...
			gain_id = as7341_als_get_saved_gain(ctx);
			old_gain_id = gain_id;
			itime_ms = ctx->alg_ctx.itime_ms;
			old_itime_ms = itime_ms;
			as7341_get_optimized_gain_itime(as7341_als_get_fullscale(ctx),
				maximum, &gain_id, &itime_ms, &saturation);
			if (old_itime_ms != itime_ms) {
				hwlog_info("integration time adjust - old: %u, new: %u, sat: %u\n",
					old_itime_ms, itime_ms, saturation);
				as7341_als_set_itime(ctx, itime_ms);
			}
			if (old_gain_id != gain_id) {
				hwlog_info("gain adjust - old: %u, new: %u, sat: %u\n",
					old_gain_id, gain_id, saturation);
				as7341_als_set_gain(ctx, gain_id);
			}
...
static void adjust_raw_values_on_itime(struct as7341_ctx *ctx, UINT32 itime_ms)
{
	int i;
	...
	if (ctx->alg_ctx.meas_steps == STEP_FAST_MEASURE) {
		for (i = 0; i < AMS_REPORT_DATA_LEN; i++)
			ctx->alg_ctx.data[i] =
				(ctx->alg_ctx.data[i] * AS7341_ITIME_MAX * 11) /
				(AS7341_ITIME_FAST * 10); // convert raw data by 1.1
	} else {
		for (i = 0; i < AMS_REPORT_DATA_LEN; i++) {
			ctx->alg_ctx.data[i] =
				((ctx->alg_ctx.data[i] * AS7341_ITIME_MAX) + (itime_ms / 2)) / itime_ms;
		}
	}
	...
	UINT16 *p_adc_data)
{
	if (old_smux == SMUX_OTHER) {
		ctx->alg_ctx.data[CHAN_F1] = p_adc_data[BUF_DATA_0];
		ctx->alg_ctx.data[CHAN_F2] = p_adc_data[BUF_DATA_1];
		ctx->alg_ctx.data[CHAN_F3] = p_adc_data[BUF_DATA_2];
		ctx->alg_ctx.data[CHAN_F8] = p_adc_data[BUF_DATA_3];
		ctx->alg_ctx.data[CHAN_NR] = p_adc_data[BUF_DATA_4];
	} else {
		ctx->alg_ctx.data[CHAN_F4] = p_adc_data[BUF_DATA_0];
		ctx->alg_ctx.data[CHAN_F5] = p_adc_data[BUF_DATA_1];
		ctx->alg_ctx.data[CHAN_F6] = p_adc_data[BUF_DATA_2];
		ctx->alg_ctx.data[CHAN_F7] = p_adc_data[BUF_DATA_3];
		ctx->alg_ctx.data[CHAN_CL] = p_adc_data[BUF_DATA_4];
	}
	as7341_log_buffer("7341_SAVE_DATA", ctx->alg_ctx.data,
		AMS_REPORT_DATA_LEN);
}
...
static bool as7341_handle_als(struct as7341_ctx *ctx, bool cal_mode)
{
	...
	old_itime_ms = ctx->alg_ctx.itime_ms;
	...
	if (ctx->alg_ctx.smux_mode == SMUX_DEFAULT) {
		hwlog_info("CH0:F4 %u, F5 %u, F6 %u, F7 %u, CL %u, GAIN %u ITIME %u\n",
			adc_data[BUF_DATA_0], adc_data[BUF_DATA_1], adc_data[BUF_DATA_2],
			adc_data[BUF_DATA_3], adc_data[BUF_DATA_4],
			((struct as7341_ctx *)ctx)->alg_ctx.gain_id,
			((struct as7341_ctx *)ctx)->alg_ctx.itime_ms);
	} else {
		hwlog_info("CH1:F1 %u, F2 %u, F3 %u, F8 %u, NR %u, GAIN %u ITIME %u\n",
			adc_data[BUF_DATA_0], adc_data[BUF_DATA_1], adc_data[BUF_DATA_2],
			adc_data[BUF_DATA_3], adc_data[BUF_DATA_4],
			((struct as7341_ctx *)ctx)->alg_ctx.gain_id,
			((struct as7341_ctx *)ctx)->alg_ctx.itime_ms);
	...
	if (!re_enable) {
		...
		if (ctx->alg_ctx.als_update)
			adjust_raw_values_on_itime(ctx, old_itime_ms);
	}

	if (ctx->alg_ctx.enabled)
		as7341_set_field(ctx->handle, AS7341_ENABLE_REG, AEN, AEN);
	hwlog_info("%s als_update: %u, re_enable: %d\n", __func__,
...
{
	struct color_chip *chip = container_of(data, struct color_chip, work_timer);
	if (!chip) {
		hwlog_err("%s: Pointer is NULL\n", __func__);
		return;
	}
...
{
	struct color_chip *chip = container_of(data, struct color_chip, fd_timer);
	if (!chip) {
		hwlog_err("%s: Pointer is NULL\n", __func__);
		return;
	}
...
static ssize_t as7341_als_set_enable(struct color_chip *chip, uint8_t en)
{
	...
	hwlog_info("%s: autozero_offset: %d, itime_ms :%d, jiffies :%d\n", __func__,
		autozero_offset, ((struct as7341_ctx *)chip->device_ctx)->alg_ctx.itime_ms, jiffies);
...
static void as7341_report_als(struct color_chip *chip)
{
	...
	hwlog_info("raw data whitebalanced: %u, %u, %u\n",
		raw_data[BUF_DATA_0], raw_data[BUF_DATA_1], raw_data[BUF_DATA_2]);
	...
	hwlog_info("color_report_val: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, curr gain: %u\n",
		color_report_val[BUF_DATA_1], color_report_val[BUF_DATA_2],
		color_report_val[BUF_DATA_3], color_report_val[BUF_DATA_4],
		color_report_val[BUF_DATA_5], color_report_val[BUF_DATA_6],
		color_report_val[BUF_DATA_7], color_report_val[BUF_DATA_8],
		color_report_val[BUF_DATA_9], color_report_val[BUF_DATA_10],
		curr_gain);
}
...
static void as7341_store_calibrate_result(UINT32 value[])
{
	UINT8 i;
	char data[6] = {'\0'};
	char *buf = &calibrate_result[BUF_DATA_2];
	memset(calibrate_result, 0, sizeof(char) * AS7341_CALIBRATE_RESULT_LEN);
	if (report_calibrate_result)
		calibrate_result[0] = '1';
	else
		calibrate_result[0] = '0';
	calibrate_result[1] = 'u';
	for (i = 0; i < AS7341_DATA_CHANNEL; i++) {
		sprintf(data, "%d", value[i]);
		hwlog_info("%s calibrate_result : %s\n", __func__, data);
		strcat(buf, data);
		strcat(buf, "u");
	}
	hwlog_info("%s now the calibrate_result : %s\n", __func__, calibrate_result);
}

char *as7341_show_calibrate(struct color_chip *chip,
	struct at_color_sensor_output_t *out)
{
	...
	for (i = 0; i < CAL_STATE_GAIN_LAST; i++) {
		...
		hwlog_info("%s i = %d: cali= %u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
			__func__,
			as7341_als_gain_lvl[i], out->cali_rst[BUF_DATA_0][i],
			out->cali_rst[BUF_DATA_1][i], out->cali_rst[BUF_DATA_2][i],
			out->cali_rst[BUF_DATA_3][i], out->cali_rst[BUF_DATA_4][i],
			out->cali_rst[BUF_DATA_5][i], out->cali_rst[BUF_DATA_6][i],
			out->cali_rst[BUF_DATA_7][i], out->cali_rst[BUF_DATA_8][i],
			out->cali_rst[BUF_DATA_9][i]);
	}
	as7341_store_calibrate_result(ctx->cal_ctx.ratio);
	...
	struct at_color_sensor_input_t *in)
{
	...
	hwlog_info("%s get tar_f1_f2_f3_clear_nir = %u, %u, %u, %u, %u\n",
		__func__, in->reserverd[BUF_DATA_0], in->reserverd[BUF_DATA_1],
		in->reserverd[BUF_DATA_2], in->reserverd[BUF_DATA_8],
		in->reserverd[BUF_DATA_9]);
...
static void as7341_initialize_calibration(struct as7341_ctx *ctx)
{
	...
	hwlog_info("CALIB %u, %u, %u, %u, %u, %u, %u, %u, %u, %u\n",
		ctx->cal_ctx.ratio[BUF_DATA_0],
		ctx->cal_ctx.ratio[BUF_DATA_1],
		ctx->cal_ctx.ratio[BUF_DATA_2],
		ctx->cal_ctx.ratio[BUF_DATA_3],
		ctx->cal_ctx.ratio[BUF_DATA_4],
		ctx->cal_ctx.ratio[BUF_DATA_5],
		ctx->cal_ctx.ratio[BUF_DATA_6],
		ctx->cal_ctx.ratio[BUF_DATA_7],
		ctx->cal_ctx.ratio[BUF_DATA_8],
		ctx->cal_ctx.ratio[BUF_DATA_9]);
}
...
static char *as7341_chip_vendor(void)
{
	return "colorsensor";
}

static int as7341_chip_version(void)
{
	return AS7341_VERSION;
}

static int as7341_chip_type(void)
{
	return AS7341_TYPE;
}

static char *as7341_chip_max_range(void)
{
	return "0";
}

static char *as7341_chip_resolution(void)
{
	return "0";
}

static char *as7341_chip_sensor_report_data(void)
{
	hwlog_debug("%s report channel_data : %s\n", __func__, channel_data);
	return channel_data;
}

static int as7341_chip_min_delay(void)
{
	return AS7341_MIN_DELAY;
}

static int as7341_chip_fifo_reserved_event_count(void)
{
	return AS7341_FIFO_RESERVED_EVENT_COUNT;
}

static int as7341_chip_fifo_max_event_count(void)
{
	return AS7341_MAX_EVENT_COUNT;
}
...
static void smux_init(struct as7341_ctx *ctx)
{
	...
	channels[BUF_DATA_0] = SMUX_F4;
	channels[BUF_DATA_1] = SMUX_F5;
	channels[BUF_DATA_2] = SMUX_F6;
	channels[BUF_DATA_3] = SMUX_F7;
	channels[BUF_DATA_4] = SMUX_CLEAR;
	channels[BUF_DATA_5] = SMUX_FLICKER;
	if (as7341_create_smux_buffer(channels, SMUX_CHANNELS, smux_buffer[0],
		SMUX_BUF_LEN))
		hwlog_info("failed as7341_create_smux_buffer 0\n");

	channels[BUF_DATA_0] = SMUX_F1;
	channels[BUF_DATA_1] = SMUX_F2;
	channels[BUF_DATA_2] = SMUX_F3;
	channels[BUF_DATA_3] = SMUX_F8;
	channels[BUF_DATA_4] = SMUX_NIR;
	channels[BUF_DATA_5] = SMUX_FLICKER;
...
static void configure_functions(struct color_chip *chip)
{
	...
	chip->color_chip_vendor = as7341_chip_vendor;
	chip->color_chip_version = as7341_chip_version;
	chip->color_chip_type = as7341_chip_type;
	chip->color_chip_max_range = as7341_chip_max_range;
	chip->color_chip_resolution = as7341_chip_resolution;
	chip->color_chip_sensor_report_data = as7341_chip_sensor_report_data;
	chip->color_chip_min_delay = as7341_chip_min_delay;
	chip->color_chip_fifo_reserved_event_count = as7341_chip_fifo_reserved_event_count;
	chip->color_chip_fifo_max_event_count = as7341_chip_fifo_max_event_count;
	chip->color_chip_get_atime = as7341_chip_get_atime;
	chip->color_chip_get_itime = as7341_chip_get_itime;
...
int as7341_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	if (!client)
		return -EFAULT;
	pr_info("addr=0x%x\n", client->addr);
	as7341_ldo = devm_regulator_get(&client->dev, "colorsensor-vdd");
	if (IS_ERR_OR_NULL(as7341_ldo)) {
		pr_err("regulator get fail\n");
	}
	ret = regulator_enable(as7341_ldo);
	if (ret != 0) {
		pr_err("regulator enable fail\n");
	} else {
		as7341_vdd_enable = true;
	}
	...
	udelay(AS7341_DELAY_200US);
	for (i = 0; i <= AS7341_RETRY; i++) {
		udelay(AS7341_DELAY_50US);
		if (as7341_get_chipid(chip->client) == AMS_UNKNOWN_DEVICE) {
			hwlog_info("%s failed: AMS_UNKNOWN_DEVICE\n", __func__);
		} else {
			hwlog_info("%s retry[%d]: AMS GOT DEVICE\n", __func__, i);
			break;
		}
	}
	if (i > AS7341_RETRY) {
		hwlog_info("%s retry failed: AMS_UNKNOWN_DEVICE\n", __func__);
		goto id_failed;
	}
...
int ams_as7341_remove(struct i2c_client *client)
{
	...
	if (as7341_vdd_enable) {
		regulator_disable(as7341_ldo);
		regulator_put(as7341_ldo);
	}
