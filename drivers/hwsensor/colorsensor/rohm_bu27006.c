...
static uint32_t const bu27006_als_itime[BU27006_RGB_GAIN_SIZE] = {
	0,
	50,
	100,
	200
...
static INT32 bu27006_chip_get_atime(struct color_chip *chip)
{
	UINT8 atime = 0;
	...
	color_sensor_get_byte(chip->client, BU27006_RGB_GAIN_TIME, &atime);
	hwlog_info("%s: now the atime = %d, itime = %dms\n", __func__, atime,
		bu27006_als_itime[atime & BU27006_ALS_ITIME_MASK]);
	return atime;
}

static UINT32 bu27006_chip_get_itime(struct color_chip *chip)
{
	UINT8 atime = 0;
	UINT32 itime = 0;
	int index = 0;
	...
	color_sensor_get_byte(chip->client, BU27006_RGB_GAIN_TIME, &atime);
	index = atime & BU27006_ALS_ITIME_MASK;
	if (index > 3) {
		hwlog_err("%s: error bu27006_als_itime out, atime = %d, itime = %dms\n", __func__, atime, itime);
		return itime;
	}
	itime = bu27006_als_itime[index];
	hwlog_debug("%s: now the atime = %u, itime = %ums\n", __func__, atime, itime);
	return itime;
}
...
static void bu27006_report_data(int value[])
{
	UINT8 i;
	char data[6] = {'\0'};
	bu27006_channel_data[0] = '\0';
	for (i = 0; i < 4; i++) {
		sprintf(data, "%d", value[i]);
		hwlog_info("%s bu27006_channel_data : %s\n", __func__, data);
		strcat(bu27006_channel_data, data);
		strcat(bu27006_channel_data, "u");
	}
	hwlog_info("%s now the bu27006_channel_data : %s\n", __func__, bu27006_channel_data);
}
...
static int bu27006_get_flc_data(struct bu27006_ctx *ctx)
{
	...
	if ((flc_report_logcount % FLC_LOG_REPORT_NUM) == 0) {
		for (i = 0; i < ROHM_FLICKER_FIFO_SIZE; i++)
			hwlog_debug("bu27006 fifo_data[i=%3d] = %d\n",
...
void bu27006_als_timer_wrk(struct timer_list *data)
{
...
void bu27006_flc_timer_wrk(struct timer_list *data)
{
...
static void bu27006_als_report(struct color_chip *chip)
{
	...
	bu27006_report_data(report_value);
...
static void bu27006_store_calibrate_result(UINT32 value[])
{
	UINT8 i;
	char data[6] = {'\0'};
	memset(bu27006_calibrate_result, 0, sizeof(char) * 64);
	if (report_calibrate_result)
		bu27006_calibrate_result[0] = '1';
	else
		bu27006_calibrate_result[0] = '0';
	bu27006_calibrate_result[1] = 'u';
	for (i = 0; i < 4; i++) {
		sprintf(data, "%d", value[i]);
		hwlog_info("%s bu27006_calibrate_result : %s\n", __func__, data);
		strcat(bu27006_calibrate_result, data);
		strcat(bu27006_calibrate_result, "u");
	}
	hwlog_info("%s now the bu27006_calibrate_result : %s\n", __func__, bu27006_calibrate_result);
}

char *bu27006_als_show_calibrate(struct color_chip *chip,
	struct at_color_sensor_output_t *out_para)
{
	int32_t i;
	UINT32 cali_ratio[4] = {0};
	...
	cali_ratio[0] = chip->cali_ctx.cal_ir_rst[3];
	cali_ratio[1] = chip->cali_ctx.cal_x_rst[3];
	cali_ratio[2] = chip->cali_ctx.cal_y_rst[3];
	cali_ratio[3] = chip->cali_ctx.cal_z_rst[3];
	bu27006_store_calibrate_result(cali_ratio);
	return bu27006_calibrate_result;
}
...
static char *bu27006_chip_sensor_report_data(void)
{
	hwlog_info("%s report channel_data : %s\n", __func__, bu27006_channel_data);
	return bu27006_channel_data;
}
static struct regulator *bu27006_ldo = NULL;
static bool bu27006_vdd_enable;
	...
	const struct i2c_device_id *idp)
{
	...
	bu27006_ldo = devm_regulator_get(&client->dev, "colorsensor-vdd");
	if (IS_ERR_OR_NULL(bu27006_ldo)) {
		pr_err("regulator get fail\n");
	}
	ret = regulator_enable(bu27006_ldo);
	if (ret != 0) {
		pr_err("regulator enable fail\n");
	} else {
		bu27006_vdd_enable = true;
	}
...
*/
	timer_setup(&p_chip->work_timer, bu27006_als_timer_wrk, 0);
	add_timer(&p_chip->work_timer);
	INIT_WORK(&p_chip->als_work, bu27006_als_work);

	timer_setup(&p_chip->fd_timer, bu27006_flc_timer_wrk, 0);
	add_timer(&p_chip->fd_timer);
	INIT_WORK(&p_chip->fd_work, bu27006_flc_work);
	...
	p_chip->color_chip_sensor_report_data = bu27006_chip_sensor_report_data;
	p_chip->color_chip_get_atime = bu27006_chip_get_atime;
	p_chip->color_chip_get_itime = bu27006_chip_get_itime;
...
static int bu27006_remove(struct i2c_client *client)
{
	...
	if (bu27006_vdd_enable) {
		regulator_disable(bu27006_ldo);
		regulator_put(bu27006_ldo);
	}
