...
static ssize_t sensors_near_far_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sensors_classdev *sensors_cdev = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "%d\n",
			sensors_cdev->near_far);
}
...
static DEVICE_ATTR(near_far, 0444, sensors_near_far_show, NULL);
