					 ...
					 unsigned long u_volt, u32 *static_power)
{
	...
	if (cpu_coeff == 0)
		return platform_cluster_get_static_power(nr_cpus, cluster,
				u_volt, static_power);
#endif
...
int of_parse_ipa_sensor_index_table(void)
{
	...
	ret = of_property_read_u32(np, "huawei,ipa_sensor_num",
				   &g_ipa_sensor_num);
	if (ret != 0) {
...
#ifdef CONFIG_HW_THERMAL_GPU_HOTPLUG
	/* GPU */
	hw_thermal_gpu_hotplug_check(&sensor_val[g_ipa_sensor_num - 1]);
#endif
#ifdef CONFIG_HW_THERMAL_NPU_HOTPLUG
