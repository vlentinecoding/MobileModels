...
static void rt9759_report_i2c_error(struct rt9759_device_info *di)
{
	...
	rt9759_fault_event_notify(DC_FAULT_I2C_ERROR, data);
	return;
}
...
static int rt9759_reg_reset(struct rt9759_device_info *di)
{
	...
	hwlog_info("[ic:%d]reg_reset [%x]=0x%x\n", di->ic_role, RT9759_CONTROL_REG, reg);
	return 0;
}
...
static int rt9759_fault_clear(struct rt9759_device_info *di)
{
	...
	hwlog_info("[ic:%d]fault_flag [%x]=0x%x\n", di->ic_role, RT9759_FLT_FLAG_REG, reg);
	return 0;
}

static int rt9759_vac_insert_control(int enable, void *dev_data)
{
	...
	hwlog_info("%s: [ic:%d]enable:%d\n", __func__, di->ic_role, enable);
	reg = enable > 0 ? 0xFF : 0x7F; /* 0xFF:enable, 0x7F:disable */
	rt9759_write_byte(di, RT9759_TEST_MODE_REG_0, 0x69); /* enable test mode */
	rt9759_write_byte(di, RT9759_TEST_MODE_REG_1, 0x96);

	rt9759_write_byte(di, RT9759_VAC_INSERT_REG, reg); /* disable vac insert */

	rt9759_write_byte(di, RT9759_TEST_MODE_REG_0, 0x69); /* disable test mode*/
	rt9759_write_byte(di, RT9759_TEST_MODE_REG_1, 0x90);
...
static int rt9759_charge_enable(int enable, void *dev_data)
{
	...
	hwlog_info("[ic:%d]charge_enable [%x]=0x%x enable:%d\n", di->ic_role, RT9759_CHRG_CTL_REG, reg, enable);
	return 0;
}
...
static int rt9759_adc_enable(int enable, void *dev_data)
{
	...
	hwlog_info("[ic:%d]adc_enable [%x]=0x%x\n", di->ic_role, RT9759_ADC_CTRL_REG, reg);
	return 0;
}
...
static int rt9759_discharge(int enable, void *dev_data)
{
	...
	hwlog_info("[ic:%d]discharge [%x]=0x%x\n", di->ic_role, RT9759_BUS_OVP_REG, reg);
	...
	int data)
{
	...
	hwlog_info("[ic:%d]config_switching_frequency [%x]=0x%x, [%x]=0x%x\n",
		di->ic_role, RT9759_CONTROL_REG, freq, RT9759_CHRG_CTL_REG, freq_shift);
...
static int rt9759_reg_init(struct rt9759_device_info *di)
{
	...
	ret += rt9759_config_ibus_ocp_threshold_ma(di, di->ibus_ocp_th);
	...
	ret += rt9759_write_mask(di, RT9759_STAT_FLAG_MASK_REG,
			RT9759_IBUS_UCP_FALL_DEGLTICH_MASK, RT9759_IBUS_UCP_FALL_DEGLTICH_SHIFT,
			RT9759_IBUS_UCP_FALL_5MS);
...
static int rt9759_charge_init(void *dev_data)
{
	...
	di->device_id = rt9759_get_device_id(dev_data);
	if (di->device_id == -1)
		return -1;
...
static void rt9759_interrupt_work(struct work_struct *work)
{
	...
	ret |= rt9759_read_byte(di, RT9759_CONVERTER_STATE_REG,
		&converter_state);
	if (ret)
	...
	struct rt9759_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"r_comp", &di->r_comp, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ibus_ocp_th", &di->ibus_ocp_th, RT9759_IBUS_OCP_THRESHOLD_INIT);
}
...
static struct dc_ic_ops rt9759_sysinfo_ops = {
	...
	.ic_vac_insert_control = rt9759_vac_insert_control,
};
...
static struct dc_ic_ops rt9759_aux_sysinfo_ops = {
	.dev_name = "rt9759_aux",
	.ic_init = rt9759_charge_init,
	.ic_exit = rt9759_charge_exit,
	.ic_enable = rt9759_charge_enable,
...
static struct dc_batinfo_ops rt9759_aux_batinfo_ops = {
	.init = rt9759_batinfo_init,
	.exit = rt9759_batinfo_exit,
	.get_bat_btb_voltage = rt9759_get_vbat_mv,
	.get_bat_package_voltage = rt9759_get_vbat_mv,
...
static struct power_tz_ops rt9759_aux_temp_sensing_ops = {
	.get_raw_data = rt9759_get_raw_data,
};
...
static void rt9759_init_ops_dev_data(struct rt9759_device_info *di)
{
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		...
		rt9759_aux_temp_sensing_ops.dev_data = (void *)di;
...
static int rt9759_ops_register(struct rt9759_device_info *di)
{
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		...
		ret = dc_ic_ops_register(SC_MODE, di->ic_role,
			&rt9759_aux_sysinfo_ops);
		ret |= dc_batinfo_ops_register(SC_MODE, di->ic_role,
			&rt9759_aux_batinfo_ops);
	}
	if (ret) {
	...
	if (di->ic_role == CHARGE_IC_TYPE_MAIN) {
		ret = power_tz_ops_register(&rt9759_temp_sensing_ops,
			"bq25970");
		ret |= power_log_ops_register(&rt9759_log_ops);
	} else {
		ret = power_tz_ops_register(&rt9759_aux_temp_sensing_ops,
			"bq25970_aux");
