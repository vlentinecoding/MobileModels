	...
	void *dst)
{
	...
	int ret;
	char info[RT9426A_WRITE_INFO_LEN] = {0};

	ret = regmap_raw_read(chip->regmap, reg, dst, len);
	if (ret < 0 ) {
		hwlog_err("RT9426A block read 0x%02x fail\n", reg);
		snprintf(info, RT9426A_WRITE_INFO_LEN, "RT9426A fuel gauge communication fail");
...
static void rt9426a_update_info(struct rt9426a_chip *chip)
{
	...
	if (((ret & 0x0800) >> 11) == 1) {
		hwlog_debug("OCV table define by User\n");
	} else {
		if (regval == 0)
	...
	if (((regval & 0x0800) >> 11) == 1)
		hwlog_debug("User_Define_Table (IN USE)\n");
	else
		hwlog_info("User_Define_Table (NOT IN USE)\n");
	if (((regval & 0x0040) >> 6) == 1)
		hwlog_debug("Battery_Status (Inserted)\n");
	else
...
static int rt9426a_i2c_chipid_check(struct rt9426a_chip *chip)
{
	...
	if (val != RT9426A_DEVICE_ID) {
		hwlog_err("dev_id not match cur=0x%x expected=0x%x\n", val, RT9426A_DEVICE_ID);
		return -ENODEV;
	}
