...
static int hl7227_chip_init_ex(int type)
{
	int ret;
	u8 mode_val = HL1512_FWD_MOD_INIT_VAL;
	u8 current_limit_val = HL1512_CUR_LIMIT_FWD_VAL_HL7227;

	if (type == HL1512_CHARGING_FWD) {
		mode_val = HL1512_FWD_MOD_INIT_VAL;
		current_limit_val = HL1512_CUR_LIMIT_FWD_VAL_HL7227;
	} else if (type == HL1512_CHARGING_REV) {
		mode_val = HL1512_REV_MOD_INIT_VAL;
		current_limit_val = HL1512_CUR_LIMIT_REV_VAL_HL7227;
	} else if (type == HL1512_CHARGING_REV_CP) {
		mode_val = HL1512_REV_PMID_INIT_VAL;
		current_limit_val = HL1512_CUR_LIMIT_REV_CP_VAL;
	}
	...
	ret = hl1512_write_byte(HL1512_INIT_REGA7, HL1512_INIT_REGA7_VAL);
	/* control reg */
	ret += hl1512_write_byte(HL1512_CONTROL_REG, HL1512_CONTROL_REG_VAL_HL7227);
	/* pmid vout ov uv */
	ret += hl1512_write_byte(HL1512_PMID_VOUT_OV_UV_REG,
		HL1512_PMID_VOUT_FWD_VAL);
	/* vout ov */
	ret += hl1512_write_byte(HL1512_VOUT_OV_REG, HL1512_VOUT_OV_REG_FWD_VAL_HL7227);
	/* wd and clock */
	ret += hl1512_write_byte(HL1512_WD_CLOCK_REG, HL1512_WD_CLOCK_VAL);
	/* current ilimit */
	ret += hl1512_write_byte(HL1512_CUR_LIMIT_REG, current_limit_val);
	/* init 58 reg */
	ret += hl1512_write_byte(HL1512_INIT_REG58, HL1512_INIT_REG58_VAL_HL7227);
	/* temp alarm th */
	ret += hl1512_write_byte(HL1512_TEMP_ALARM_TH_REG,
		HL1512_TEMP_ALARM_TH_REG_VAL);
	/* mode config reg */
...
static int hl1512_chip_init(void *dev_data)
{
	...
	if (di->chip_id == HL1512X_DEVICE_ID_HL1512)
		return hl1512_chip_init_ex(HL1512_CHARGING_FWD);
	else if (di->chip_id == HL1512X_DEVICE_ID_HL7227)
		return hl7227_chip_init_ex(HL1512_CHARGING_FWD);
	else {
		hwlog_err("not recognized chip_id 0x%x\n", di->chip_id);
		return -EIO;
	}
}
...
static int hl1512_reverse_chip_init(void *dev_data)
{
	...
	if (di->chip_id == HL1512X_DEVICE_ID_HL1512)
		...
		ret = hl7227_chip_init_ex(HL1512_CHARGING_REV);
	else {
		hwlog_err("not recognized chip_id 0x%x\n", di->chip_id);
		return -EIO;
	}
...
static int hl1512_set_rvs_bp_mode(void *dev_data)
{
	int ret;
...
static int hl1512_reverse_cp_chip_init(void *dev_data)
{
	...
	if (di->chip_id == HL1512X_DEVICE_ID_HL1512)
		...
		ret = hl7227_chip_init_ex(HL1512_CHARGING_REV_CP);
	else {
		hwlog_err("not recognized chip_id 0x%x\n", di->chip_id);
		return -EIO;
	}
...
static struct charge_pump_ops hl1512_ops = {
	...
	.set_rvs_bp_mode = hl1512_set_rvs_bp_mode,
