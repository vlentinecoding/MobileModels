...
static int stwlc88_tx_set_bridge(int v_ask, enum wltx_bridge_type type)
{
	switch (type) {
...
static int stwlc88_tx_get_ploss(s32 *ploss)
{
	...
	if (!ploss) {
		hwlog_err("get_ploss: para null\n");
		return -WLC_ERR_PARA_NULL;
	}

	ret = stwlc88_read_word(ST88_TX_TFRD_PWR_ADDR, &ptx);
	ret += stwlc88_read_word(ST88_TX_RCVD_PWR_ADDR, &prx);
	if (ret)
		return ret;
...
static int stwlc88_tx_get_ploss_id(u8 *id)
{
...
static void stwlc88_tx_ept_handler(struct stwlc88_dev_info *di)
{
	...
	case ST88_TX_EPT_SRC_RX:
		di->ept_type &= ~ST88_TX_EPT_SRC_RX;
		hwlog_info("%s: ST88_TX_EPT_SRC_RX\n", __func__);
		break;
...
int stwlc88_tx_ops_register(void)
{
	...
	ret = wireless_tx_ops_register(&g_stwlc88_tx_ops);
	ret += wltx_ic_ops_register(&g_stwlc88_tx_ic_ops, WLTRX_IC_TYPE_MAIN);
