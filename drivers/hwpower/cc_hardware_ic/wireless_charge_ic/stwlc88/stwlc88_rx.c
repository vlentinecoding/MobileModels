...
static void stwlc88_set_mode_cfg(int vset)
{
	...
	if (di->product_isolation == 0) {
		if (vset <= RX_HIGH_VOUT)
			...
			stwlc88_ask_mode_cfg(ST88_BOTH_CAP_NEGATIVE);
		else
			stwlc88_ask_mode_cfg(ST88_CAP_BC_NEGATIVE);
	} else if (di->product_isolation == 1) {
		if (vset <= RX_HIGH_VOUT)
			stwlc88_ask_mode_cfg(ST88_BOTH_CAP_POSITIVE);
		else if (vset < RX_HIGH_VOUT3)
			stwlc88_ask_mode_cfg(ST88_BOTH_CAP_NEGATIVE);
		else
			stwlc88_ask_mode_cfg(ST88_CAP_B_NEGATIVE);
	}
}
...
static struct wireless_charge_device_ops g_stwlc88_rx_ops = {
	...
	.is_pwr_good            = stwlc88_is_pwr_good,
};
...
int stwlc88_rx_ops_register(void)
{
	...
	ret = wireless_charge_ops_register(&g_stwlc88_rx_ops);
	ret += wlrx_ic_ops_register(&g_stwlc88_rx_ic_ops, WLTRX_IC_TYPE_MAIN);
