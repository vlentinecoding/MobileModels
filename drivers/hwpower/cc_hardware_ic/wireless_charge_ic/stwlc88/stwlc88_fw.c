...
static int stwlc88_fw_hw_erase_ftp(void)
{
	int ret;
...
static int stwlc88_fw_hang_process(void)
{
	...
	return stwlc88_fw_hw_erase_ftp();
}
...
static int stwlc88_fw_abnormal_dc_power_check(u8 *op_mode)
{
	int ret;
...
static int stwlc88_fw_check_dc_power(void)
{
	...
	if (stwlc88_fw_abnormal_dc_power_check(&op_mode))
		return -WLC_ERR_MISMATCH;
...
static int stwlc88_fw_write_ftp_sector(const u8 *data, unsigned int len, u8 sector_id)
{
	...
	remainder = len % ST88_FTP_SECTOR_SEGMENT_SIZE;
	quotient = len / ST88_FTP_SECTOR_SEGMENT_SIZE;
	for (i = 0; i < quotient; i++)
		ret |= stwlc88_write_block(ST88_AUX_DATA_ADDR + (i * ST88_FTP_SECTOR_SEGMENT_SIZE), wr_buff + (i * ST88_FTP_SECTOR_SEGMENT_SIZE), ST88_FTP_SECTOR_SEGMENT_SIZE);
	if (quotient != 0)
		ret |= stwlc88_write_block(ST88_AUX_DATA_ADDR + (quotient * ST88_FTP_SECTOR_SEGMENT_SIZE), wr_buff + (quotient * ST88_FTP_SECTOR_SEGMENT_SIZE), remainder);
	if (ret) {
