...
static int mt5735_qi_send_ask_msg(u8 cmd, u8 *data, int data_len, void *dev_data)
{
	...
	ret = mt5735_write_word_mask(MT5735_RX_CMD_ADDR,
		MT5735_RX_CMD_SEND_MSG_RPLY,
		MT5735_RX_CMD_SEND_MSG_RPLY_SHIFT, MT5735_RX_CMD_VAL);
	if (ret) {
	...
	void *dev_data)
{
	int ret;
	u8 header;
	u32 reg_val = MT5735_TX_CMD_SEND_MSG;
	u8 write_data[MT5735_SEND_MSG_DATA_LEN] = { 0 };
...
{
	int ret;
	u32 reg_val = MT5735_RX_CMD_SET_RPP_24BIT;

	ret = mt5735_write_byte(MT5735_RX_RP_PMAX_ADDR,
		pmax * MT5735_RX_RP_VAL_UNIT);
	ret += mt5735_write_block(MT5735_RX_CMD_ADDR, (u8 *)&reg_val,
		MT5735_RX_CMD_LEN);
	if (ret) {
...
static struct qi_protocol_ops g_mt5735_qi_ops = {
	.chip_name           = "mt5735",
...
{
	return qi_protocol_ops_register(&g_mt5735_qi_ops);
}
