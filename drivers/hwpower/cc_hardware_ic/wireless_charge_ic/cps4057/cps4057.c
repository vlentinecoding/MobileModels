	...
	struct device_node *np)
{
	...
	if (di->irq_no_suspend) {
		flags = IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND;
	} else {
		flags = IRQF_TRIGGER_FALLING;
	}

	hwlog_err("irq_init: flags %u\n", flags);
	if (request_irq(di->irq_int, cps4057_interrupt, flags, "cps4057_irq", di)) {
