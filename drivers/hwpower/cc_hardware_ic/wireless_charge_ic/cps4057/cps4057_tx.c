...
static int cps4057_tx_set_bridge(int v_ask, enum wltx_bridge_type type)
{
...
static int cps4057_tx_get_cep(u8 *cep)
{
	return cps4057_read_byte(CPS4057_TX_CEP_ADDR, cep);
}

static int cps4057_tx_get_duty(u8 *duty)
{
	int ret;
	u16 pwm_duty = 0;
...
static int cps4057_tx_get_ptx(u32 *ptx)
{
	return cps4057_read_word(CPS4057_TX_PTX_ADDR, (u16 *)ptx);
}

static int cps4057_tx_get_prx(u32 *prx)
{
	return cps4057_read_word(CPS4057_TX_PRX_ADDR, (u16 *)prx);
}

static int cps4057_tx_get_ploss(s32 *ploss)
{
	return cps4057_read_word(CPS4057_TX_PLOSS_ADDR, (u16 *)ploss);
}

static int cps4057_tx_get_ploss_id(u8 *id)
{
...
static void cps4057_tx_ept_handler(struct cps4057_dev_info *di)
{
	...
	case CPS4057_TX_EPT_SRC_RX_EPT:
		di->ept_type &= ~CPS4057_TX_EPT_SRC_RX_EPT;
		hwlog_info("%s: CPS4057_TX_EPT_SRC_RX_EPT\n", __func__);
		break;
...
int cps4057_tx_ops_register(void)
{
	...
	ret = wireless_tx_ops_register(&g_cps4057_tx_ops);
	ret += wltx_ic_ops_register(&g_cps4057_tx_ic_ops, WLTRX_IC_TYPE_MAIN);
