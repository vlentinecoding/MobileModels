	...
	if (di->product_isolation == 0) {
		if (vset <= RX_HIGH_VOUT) {
			...
			if (!power_cmdline_is_factory_mode() && (vset >= RX_HIGH_VOUT2))
				...
				cps4057_ask_mode_cfg(CPS4057_RX_CMALL_EN_VAL, CPS4057_RX_CM_NEGTIVE);
		}
	} else {
		if (vset <= RX_HIGH_VOUT) {
			cps4057_ask_mode_cfg(CPS4057_RX_CMALL_EN_VAL,
			...
			if (!power_cmdline_is_factory_mode() && (vset >= RX_HIGH_VOUT2))
				cps4057_ask_mode_cfg(CPS4057_RX_CMC_EN_VAL, CPS4057_RX_CM_POSITIVE);
			else
				cps4057_ask_mode_cfg(CPS4057_RX_CMALL_EN_VAL, CPS4057_RX_CM_POSITIVE);
		}
	}
}
...
static int cps4057_rx_get_type_by_txver(int txver)
{
	int type;
	int data = txver;
...
static int cps4057_rx_get_index(struct cps4057_dev_info *di, int tx_type)
{
	int index;
	int type;

	for (index = 0; index < CPS4057_FOD_PARA_MULTI_TX; index++) {
		//kstrtoint(di->multi_tx[index].tx_version, POWER_BASE_HEX, &txver);
		type = cps4057_rx_get_type_by_txver(di->multi_tx[index].tx_version);
		if (tx_type == type)
			return index;
	}
	return -1;
...
static int cps4057_rx_set_rx_fod(struct cps4057_dev_info *di, char* rx_fod_index, u8 **rx_fod)
{
	int index;

	for (index = 0; index < CPS4057_FOD_PARA_GROUP; index++) {
		if (strcmp(di->rx_fod_para_info[index].tx_fod_index, rx_fod_index) == 0) {
				*rx_fod = di->rx_fod_para_info[index].rx_fod;
				break;
		}
	}
	if (index >= CPS4057_FOD_PARA_GROUP)
		return -1;
	return 0;
}

static int cps4057_rx_init_fod_coef_multi_tx(struct cps4057_dev_info *di, int tx_type)
{
	int tx_vset;
	int ret;
	u8 *rx_fod = NULL;
	int index = 0;
	char *rx_fod_index = NULL;

	tx_vset = cps4057_rx_get_tx_vout_reg();
	hwlog_info("[init_fod_coef] tx_vout_reg: %dmV\n", tx_vset);

	if (tx_vset < 9000) {
		index = cps4057_rx_get_index(di, tx_type);
		if (index < 0) {
			hwlog_err("%s: no match tx version\n", __func__);
			return -WLC_ERR_MISMATCH;
		}
		rx_fod_index = di->multi_tx[index].rx_fod_5v;
		(void)cps4057_rx_set_rx_fod(di, rx_fod_index, &rx_fod);
	}

	else if (tx_vset < 15000) {
		index = cps4057_rx_get_index(di, tx_type);
		if (index < 0) {
			hwlog_err("%s: no match tx version\n", __func__);
			return -WLC_ERR_MISMATCH;
		}
		rx_fod_index = di->multi_tx[index].rx_fod_9v;
		(void)cps4057_rx_set_rx_fod(di, rx_fod_index, &rx_fod);
	}
		...
		rx_fod_index = di->multi_tx[index].rx_fod_15v;
		(void)cps4057_rx_set_rx_fod(di, rx_fod_index, &rx_fod);
	}
	else
		return -WLC_ERR_MISMATCH;
...
static int cps4057_rx_chip_init(int init_type, int tx_type)
{
	...
	case ADAPTER_5V * WL_MVOLT_PER_VOLT:
		...
		if (!di->support_multi_tx)
			...
			ret += cps4057_rx_init_fod_coef_multi_tx(di, tx_type);
...
static struct wireless_charge_device_ops g_cps4057_rx_ops = {
	...
	.is_pwr_good            = cps4057_is_pwr_good,
};
...
int cps4057_rx_ops_register(void)
{
	...
	ret = wireless_charge_ops_register(&g_cps4057_rx_ops);
	ret += wlrx_ic_ops_register(&g_cps4057_rx_ic_ops, WLTRX_IC_TYPE_MAIN);
