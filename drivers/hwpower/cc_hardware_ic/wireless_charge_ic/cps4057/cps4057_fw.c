...
static int cps4057_fw_check_mtp_crc(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (crc == mtp_info[di->product_isolation].mtp_crc)
			return 0;
	}
...
static int cps4057_fw_check_mtp_version(void)
{
	...
	if (info.mtp_ver != mtp_info[di->product_isolation].mtp_version)
		return -WLC_ERR_MISMATCH;
	...
	const u8 *data, int len)
{
	...
	while (remaining > 0) {
		...
		quotient = size_to_wr / CPS4057_FW_PAGE_SEGMENT_SIZE;
		remainder = size_to_wr % CPS4057_FW_PAGE_SEGMENT_SIZE;
		for (i = 0; i < quotient; i++)
			ret |= cps4057_write_block(addr + (i * CPS4057_FW_PAGE_SEGMENT_SIZE), wr_buff + (i * CPS4057_FW_PAGE_SEGMENT_SIZE), CPS4057_FW_PAGE_SEGMENT_SIZE);
		if (remainder != 0)
			ret |= cps4057_write_block(addr + (quotient * CPS4057_FW_PAGE_SEGMENT_SIZE), wr_buff + (quotient * CPS4057_FW_PAGE_SEGMENT_SIZE), remainder);
		if (ret) {
...
static int cps4057_fw_write_mtp_data(void)
{
	...
	while (remaining > 0) {
		...
		ret = cps4057_fw_write_sram_data(CPS4057_SRAM_MTP_BUFF0,
			mtp_info[di->product_isolation].cps4057_mtp + offset, wr_size);
		if (ret) {
