...
static const struct adapter_protocol_device_data g_scp_protocol_dev_data[] = {
	...
	{ PROTOCOL_DEVICE_ID_SC8551S, "sc8551s" },
};
...
static int scp_protocol_check_adp_type(int mode, int value)
{
	...
	if ((mode & ADAPTER_SUPPORT_SCP_B_LVC) &&
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_40W_DL:
			return ADAPTER_TYPE_10V4A_DL;
		default:
			break;
		}
	}
	...
	if (mode & ADAPTER_SUPPORT_SCP_B_SC) {
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_100W:
			return ADAPTER_TYPE_20V5A;
		default:
			break;
		}
	}
...
static bool scp_protocol_is_ignore_cable_check(void)
{
	struct scp_protocol_dev *l_dev = NULL;
...
static int scp_protocol_detect_adapter_support_mode_by_0x7e(int *mode, int *ignore_cable)
{
	int value[BYTE_ONE] = { 0 };
	...
	scp_protocol_get_max_voltage(&volt);
	...
	if (value[0] & SCP_PROTOCOL_ADP_TYPE0_AB_MASK) {
		...
		if (value[0] & SCP_PROTOCOL_ADP_TYPE0_B_SC_MASK) {
			*mode |= ADAPTER_SUPPORT_SCP_B_SC;
			if (volt > SCP_PROTOCOL_HSC_VOLTAGE_TH)
				*mode |= ADAPTER_SUPPORT_SCP_B_HSC;
			ret = SCP_DETECT_SUCC;
		}
	...
	if (value[0] & SCP_PROTOCOL_ADP_TYPE0_IGNORE_CABLE)
		*ignore_cable = 1; /* no need check cable */
...
static int scp_protocol_detect_adapter_support_mode(int *mode)
{
	...
	ret = scp_protocol_detect_adapter_support_mode_by_0x7e(&support_mode, &ignore_cable);
	if (ret == SCP_DETECT_SUCC) {
		*mode = support_mode;
		l_dev->info.ignore_cable = ignore_cable;
...
static int scp_protocol_get_chip_vendor_id(int *id)
{
	...
	case SCP_PROTOCOL_VENDOR_ID_IWATT:
		...
		*id = ADAPTER_CHIP_IWATT;
		break;
	case SCP_PROTOCOL_VENDOR_ID_ROCKCHIP:
		hwlog_info("adapter chip is rockchip\n");
		*id = ADAPTER_CHIP_ROCKCHIP;
		break;
	default:
...
static struct adapter_protocol_ops adapter_protocol_scp_ops = {
	...
	.is_ignore_cable_check = scp_protocol_is_ignore_cable_check,
};
