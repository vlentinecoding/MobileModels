...
static int scp_protocol_get_reg7e_rw_error_flag(void)
{
	struct scp_protocol_dev *l_dev = NULL;
...
static void scp_protocol_set_reg7e_rw_error_flag(int flag)
{
	struct scp_protocol_dev *l_dev = NULL;
...
static int scp_protocol_reg_read(int reg, int *val, int num)
{
	...
	if (ret < 0) {
		if ((reg != SCP_PROTOCOL_ADP_TYPE0) &&
			(reg != SCP_PROTOCOL_POWER_CURVE_NUM))
...
static int scp_protocol_reg_multi_read(int reg, int *val, int num)
{
	int i;
	int ret;
	...
	if (!l_ops->reg_multi_read) {
		...
		return scp_protocol_reg_read(reg, val, num);
	}
...
static int scp_protocol_check_adp_type(int mode, int value)
{
	if (mode & ADAPTER_SUPPORT_SCP_B_LVC) {
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_22P5W_BANK:
	...
	if ((mode & ADAPTER_SUPPORT_SCP_B_LVC) &&
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_40W_2:
		case SCP_PROTOCOL_ADP_B_TYPE1_NR_40W:
		case SCP_PROTOCOL_ADP_B_TYPE1_NR_40W_1:
		case SCP_PROTOCOL_ADP_B_TYPE1_NR_40W_2:
		case SCP_PROTOCOL_ADP_B_TYPE1_NR_40W_3:
		case SCP_PROTOCOL_ADP_B_TYPE1_PS_C_40W:
		case SCP_PROTOCOL_ADP_B_TYPE1_PS_C_40W_1:
		case SCP_PROTOCOL_ADP_B_TYPE1_PS_A_40W:
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_MJR_66W:
		case SCP_PROTOCOL_ADP_B_TYPE1_MJR_66W_1:
			return ADAPTER_TYPE_11V6A;
		case SCP_PROTOCOL_ADP_B_TYPE1_66W_CAR:
		case SCP_PROTOCOL_ADP_B_TYPE1_66W_CAR_1:
			return ADAPTER_TYPE_11V6A_CAR;
		case SCP_PROTOCOL_ADP_B_TYPE1_66W_BANK:
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_A_22P5W:
			return ADAPTER_TYPE_HPR_A_10V2P25A;
		case SCP_PROTOCOL_ADP_B_TYPE1_JLR_135W:
	...
	if (mode & ADAPTER_SUPPORT_SCP_B_SC) {
		...
		case SCP_PROTOCOL_ADP_B_TYPE1_PS_C_22P5W:
		case SCP_PROTOCOL_ADP_B_TYPE1_PS_A_22P5W:
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_A_22P5W_1:
			return ADAPTER_TYPE_10V2P25A;
		/* check 10v2.25a-bank */
		case SCP_PROTOCOL_ADP_B_TYPE1_22P5W_BANK_1:
			return ADAPTER_TYPE_10V2P25A_BANK;
		/* check 10v2.25a-car */
		case SCP_PROTOCOL_ADP_B_TYPE1_22P5W_CAR:
		case SCP_PROTOCOL_ADP_B_TYPE1_22P5W_CAR_1:
			return ADAPTER_TYPE_10V2P25A_CAR;
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_C_66W:
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_C_66W_1:
			return ADAPTER_TYPE_HPR_C_11V6A;
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_C_40W:
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_C_40W_1:
			return ADAPTER_TYPE_HPR_C_10V4A;
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_A_66W:
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_A_66W_1:
			return ADAPTER_TYPE_HPR_A_11V6A;
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_A_40W:
		case SCP_PROTOCOL_ADP_B_TYPE1_HPR_A_40W_1:
			return ADAPTER_TYPE_HPR_A_10V4A;
		case SCP_PROTOCOL_ADP_B_TYPE1_HHR_90W:
		case SCP_PROTOCOL_ADP_B_TYPE1_HHR_90W_1:
...
static int scp_protocol_get_power_curve_num(int *num)
{
	int value[BYTE_ONE] = { 0 };

	/* check reg 0x7e read result */
	if (scp_protocol_get_reg7e_rw_error_flag())
		return -1;

	/* reg: 0x8f */
	if (scp_protocol_reg_read(SCP_PROTOCOL_POWER_CURVE_NUM, value, BYTE_ONE))
		return -1;
...
static int scp_protocol_get_power_curve(int *val, unsigned int size)
{
	unsigned int i;
	int num;
	int value[SCP_PROTOCOL_POWER_CURVE_SIZE] = { 0 };

	if (!val || (size > SCP_PROTOCOL_POWER_CURVE_SIZE))
		return -1;

	num = (size < BYTE_SIXTEEN) ? size : BYTE_SIXTEEN;
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_POWER_CURVE_BASE0, value,
		num))
		return -1;

	if (size > BYTE_SIXTEEN) {
		num = size - BYTE_SIXTEEN;
		if (scp_protocol_reg_multi_read(SCP_PROTOCOL_POWER_CURVE_BASE1,
			&value[BYTE_SIXTEEN], num))
			return -1;
	}

	for (i = 0; i < size; i++) {
		if (i % 2 == 0)
			val[i] = value[i] * SCP_PROTOCOL_POWER_CURVE_VOLT_STEP;
		else
			val[i] = value[i] * SCP_PROTOCOL_POWER_CURVE_CUR_STEP;
	}

	return 0;
}

static void scp_protocol_detect_undetach_cable(unsigned int value)
{
	struct scp_protocol_dev *l_dev = scp_protocol_get_dev();

	if (!l_dev)
		return;

	l_dev->info.is_undetach_cable = value & SCP_PROTOCOL_UNDETACH_CABLE;
}
...
static int scp_protocol_detect_adapter_support_mode_by_0x7e(int *mode)
{
	...
	scp_protocol_set_reg7e_rw_error_flag(NO_RW_ERROR_FLAG);

	/* reg: 0x7e */
	if (scp_protocol_reg_read(SCP_PROTOCOL_ADP_TYPE0, value, BYTE_ONE)) {
		scp_protocol_set_reg7e_rw_error_flag(RW_ERROR_FLAG);
	...
	if (value[0] & SCP_PROTOCOL_ADP_TYPE0_AB_MASK) {
		...
		scp_protocol_detect_undetach_cable(value[0]);
...
static int scp_protocol_get_output_voltage_d(int *volt)
{
	...
	/* reg: 0xa8 & 0xa9 */
	if (scp_protocol_reg_multi_read(SCP_PROTOCOL_READ_VOUT_HBYTE,
		value, BYTE_TWO))
		return -1;
...
static int scp_protocol_get_encrypt_completed(void)
{
	int i;
	int value[BYTE_ONE] = { 0 };
	...
	for (i = 0; i < RETRY_THREE; i++) {
		if (scp_protocol_reg_read(SCP_PROTOCOL_ENCRYPT_INFO, value,
			BYTE_ONE))
			break;
	...
	int size)
{
	...
	if (scp_protocol_get_encrypt_completed()) {
		ret = -1;
		goto fail_encrypt;
	}
...
static bool scp_protocol_is_undetach_cable(void)
{
	struct scp_protocol_dev *l_dev = scp_protocol_get_dev();
...
static struct adapter_protocol_ops adapter_protocol_scp_ops = {
	...
	.get_chip_serial_num = scp_protocol_get_serial_no_id,
	...
	.get_power_curve_num = scp_protocol_get_power_curve_num,
	.get_power_curve = scp_protocol_get_power_curve,
	...
	.is_undetach_cable = scp_protocol_is_undetach_cable,
};
