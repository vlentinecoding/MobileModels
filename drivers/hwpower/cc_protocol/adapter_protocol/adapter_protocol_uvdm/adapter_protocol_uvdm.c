...
static int __init uvdm_protocol_init(void)
{
	...
	ret = power_event_nc_register(POWER_NT_UVDM, &l_dev->nb);
	if (ret)
		goto fail_free_mem;
...
fail_unregister_notifier:
	power_event_nc_unregister(POWER_NT_UVDM, &l_dev->nb);
fail_free_mem:
	kfree(l_dev);
...
static void __exit uvdm_protocol_exit(void)
{
	...
	power_event_nc_unregister(POWER_NT_UVDM, &g_uvdm_protocol_dev->nb);
