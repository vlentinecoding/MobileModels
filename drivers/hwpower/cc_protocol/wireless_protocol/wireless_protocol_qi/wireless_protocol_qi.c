...
#include <huawei_platform/power/wireless/wireless_tx_auth.h>
...
static const struct wireless_protocol_device_data g_qi_protocol_dev_data[] = {
	...
	{ WIRELESS_DEVICE_ID_CPS4035, "cps4035" },
	{ WIRELESS_DEVICE_ID_MT5728, "mt5728" },
};
...
static int qi_protocol_send_msg_only(u8 cmd, u8 *para, int para_len, int retrys)
{
	int i;
	int ret;
	struct qi_protocol_ops *l_ops = NULL;
...
static int qi_protocol_auto_send_fsk_msg(u8 cmd, u8 *para, int para_len, int retrys)
{
	int i;
	int ret;
	struct qi_protocol_ops *l_ops = NULL;
...
static int qi_protocol_send_rx_event(u8 rx_evt)
{
	int retry = WIRELESS_RETRY_ONE;

	if (qi_protocol_send_msg_ack(QI_CMD_SEND_RX_EVT,
...
static int qi_protocol_send_tx_alarm(u8 *alarm, int len)
{
	int retry = WIRELESS_RETRY_ONE;
	...
	if (qi_protocol_auto_send_fsk_msg(QI_CMD_TX_ALARM, alarm, len, retry))
		return -1;
...
static int qi_protocol_set_motor_position(u8 motorPosition)
{
	int retry = WIRELESS_RETRY_ONE;
	struct qi_protocol_dev *l_dev = NULL;
	...
	if (qi_protocol_send_msg_ack(QI_CMD_SET_MOTOR_POSITION,
		&motorPosition, QI_PARA_MOTOR_POSITION_LEN, retry)) {
		hwlog_err("%s: failed\n", __func__);
		return -1;
	}
...
static int qi_protocol_set_rpp_format_post(u8 pmax, int mode)
{
	struct qi_protocol_ops *l_ops = NULL;
...
static int qi_protocol_set_rpp_format(u8 pmax)
{
	...
	if (qi_protocol_set_rpp_format_post(pmax, WIRELESS_RX))
		return -1;
...
static int qi_protocol_get_tx_type(int *type)
{
	...
	case QI_TX_TYPE_CP62R:
		*type = WIRELESS_TX_TYPE_CP62R;
		break;
	case QI_TX_TYPE_CP60_1:
		*type = WIRELESS_TX_TYPE_CP60_1;
		break;
	case QI_TX_TYPE_CP60_2:
		*type = WIRELESS_TX_TYPE_CP60_2;
		break;
	case QI_TX_TYPE_CP62_1:
		*type = WIRELESS_TX_TYPE_CP62_1;
		break;
	case QI_TX_TYPE_CP62_2:
		*type = WIRELESS_TX_TYPE_CP62_2;
		break;
	case QI_TX_TYPE_CK030_1:
		*type = WIRELESS_TX_TYPE_CK030_1;
		break;
	case QI_TX_TYPE_CK030_2:
		*type = WIRELESS_TX_TYPE_CK030_2;
		break;
	case QI_TX_TYPE_AP62L:
		*type = WIRELESS_TX_TYPE_AP62L;
		break;
	case QI_TX_TYPE_AP62C:
		*type = WIRELESS_TX_TYPE_AP62C;
		break;
	default:
...
static int qi_protocol_get_tx_capability(struct wireless_protocol_tx_cap *cap)
{
	...
	// BYTE2
	cap->support_motor_move = data2[QI_TX_EXTRA_CAP_ATTR2] &
		QI_PARA_TX_EXT_CAP_SUPPORT_MOTOR_MOVE_MASK;

	hwlog_info("tx_extra_cap: [support] fan=%d tec=%d fod_status=%d fop_range=%d support_motor_move=%d.\n",
		cap->support_fan, cap->support_tec, cap->support_fod_status,
		cap->support_fop_range, cap->support_motor_move);
...
static struct wireless_protocol_ops wireless_protocol_qi_ops = {
	...
	.set_motor_position = qi_protocol_set_motor_position,
};
...
static int qi_protocol_handle_ask_packet_cmd_0x02(u8 *data)
{
	...
	case QI_CMD_GET_RX_EPT:
		power_event_notify(POWER_NT_LIGHTSTRAP, POWER_NE_LIGHTSTRAP_EPT, NULL);
		break;
	default:
		break;
	}
...
/* 0x18 + 0x09 + checksum */
static int qi_protocol_handle_ask_packet_cmd_0x09(void)
{
	u8 para  = QI_ACK_TX_PAD_TYPE;

	if (qi_protocol_send_fsk_msg(QI_CMD_GET_TX_TYPE, &para,
		QI_ACK_TX_TYPE_LEN, WIRELESS_RETRY_ONE)) {
		hwlog_err("send fsk 0x09 error\n");
		return -1;
	}

	hwlog_info("ask_packet_cmd_0x09\n");
	return 0;
}
...
static int qi_protocol_handle_ask_packet_cmd_0x0a(u8 *data)
{
	...
	power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_ASK_SET_VTX, &tx_vset);
	if (qi_protocol_send_fsk_ack_msg())
...
static int qi_protocol_handle_ask_packet_cmd_0x1a(u8 *data)
{
	power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_ASK_RX_EVT,
		&data[QI_ASK_PACKET_DAT1]);
	if (qi_protocol_send_fsk_ack_msg())
...
static int qi_protocol_handle_ask_packet_cmd_0x3b(u8 *data)
{
	...
	if (tx_id == QI_HANDSHAKE_ID_HW) {
		...
		power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_HANDSHAKE_SUCC, NULL);
	}
...
{
	power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_GET_TX_CAP, NULL);
...
static int qi_protocol_handle_ask_packet_cmd_0x43(u8 *data)
{
	...
	wireless_get_qi_charge_done_type(data[QI_ASK_PACKET_DAT1]);
	qi_protocol_send_fsk_ack_msg();

	if (chrg_state & QI_CHRG_STATE_DONE) {
		hwlog_info("tx received rx charge-done event\n");
		power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_CHARGEDONE, NULL);
	}
...
static int qi_protocol_handle_ask_packet_cmd_0x55(u8 *data)
{
	...
	qi_protocol_send_fsk_ack_msg();
	power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_GET_RX_MAX_POWER, &rx_pmax);
...
static int qi_protocol_handle_ask_packet_cmd_0x0e(u8 *data)
{
	...
	power_event_notify(POWER_NT_LIGHTSTRAP,
		POWER_NE_LIGHTSTRAP_GET_PRODUCT_TYPE, &product_type);
	power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_GET_RX_PRODUCT_TYPE, &product_type);
...
static int qi_protocol_handle_ask_packet_cmd_0x54(u8 *data)
{
	...
	power_event_notify(POWER_NT_WLTX, POWER_NE_WLTX_ACC_DEV_CONNECTED, NULL);
...
/* 0x18 + 0x6b + checksum */
static int qi_protocol_handle_ask_packet_cmd_0x6b(void)
{
	u8 data = QI_ACK_RPP_FORMAT_24BIT;

	return qi_protocol_send_fsk_msg(QI_CMD_GET_RPP_FORMAT,
		&data, QI_FSK_RPP_FORMAT_LEN, WIRELESS_RETRY_ONE);
}

/* 0x28 + 0x6c + pmax + checksum */
static int qi_protocol_handle_ask_packet_cmd_0x6c(u8 *data)
{
	u8 pmax;

	(void)qi_protocol_send_fsk_ack_msg();
	pmax = (data[QI_ASK_PACKET_DAT1] >> QI_PARA_RX_PMAX_SHIFT) *
		QI_PARA_RX_PMAX_UNIT;

	hwlog_info("received pmax=%dw\n", pmax);
	return qi_protocol_set_rpp_format_post(pmax, WIRELESS_TX);
}
...
static int qi_protocol_handle_ask_packet_hdr_0x18(u8 *data)
{
	...
	case QI_CMD_GET_TX_TYPE:
		return qi_protocol_handle_ask_packet_cmd_0x09();
	...
	case QI_CMD_GET_RPP_FORMAT:
		return qi_protocol_handle_ask_packet_cmd_0x6b();
	default:
...
static int qi_protocol_handle_ask_packet_hdr_0x28(u8 *data)
{
	...
	case QI_CMD_SEND_RX_EVT:
		return qi_protocol_handle_ask_packet_cmd_0x1a(data);
	...
	case QI_CMD_SET_RX_MAX_POWER:
		return qi_protocol_handle_ask_packet_cmd_0x6c(data);
	default:
...
static void qi_protocol_handle_fsk_tx_alarm(u8 *pkt)
{
	u8 header;
	...
	power_event_notify(POWER_NT_WLRX, POWER_NE_WLRX_TX_ALARM, &tx_alarm);
	(void)qi_protocol_send_msg_only(QI_CMD_ACK, NULL, 0, WIRELESS_RETRY_THREE);
}
...
static void qi_protocol_handle_fsk_packet(u8 *pkt, int len)
{
	...
	case QI_CMD_TX_ALARM:
		...
		qi_protocol_handle_fsk_tx_alarm(pkt);
		break;
	case QI_CMD_ACK_BST_ERR:
		hwlog_info("received cmd with tx boost err\n");
		power_event_notify(POWER_NT_WLRX, POWER_NE_WLRX_TX_BST_ERR, NULL);
		break;
	default:
		break;
	}
