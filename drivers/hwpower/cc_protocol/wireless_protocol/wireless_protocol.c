...
int wireless_send_tx_alarm(int prot, u8 *alarm, int len)
{
	...
	return l_ops->send_tx_alarm(alarm, len);
}
...
int wlc_set_motor_position(int prot, u8 position)
{
	...
	if (!l_ops->set_motor_position) {
		hwlog_err("set_motor_position is null\n");
		return -1;
	}
	return l_ops->set_motor_position(position);
}
