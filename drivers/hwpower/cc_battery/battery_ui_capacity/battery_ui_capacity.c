...
void bat_ui_capacity_restart_work(void)
{
	...
	if (di)
		mod_delayed_work(system_power_efficient_wq, &di->update_work,
			msecs_to_jiffies(BUC_WORK_INTERVAL_NORMAL));
	...
	int *curr_capacity)
{
	if (*curr_capacity > di->chg_force_full_soc_thld) {
		...
		if (di->chg_force_full_count * di->monitoring_interval >=
			di->chg_force_full_wait_time) {
			hwlog_info("wait out full time curr_capacity=%d, interval = %d\n",
				curr_capacity, di->monitoring_interval);
...
static int bat_ui_capacity_select_interval(struct bat_ui_cap_interval_para *para,
	int ui_capacity, int default_value)
{
	int i;
	...
	for (i = 0; i < BUC_WORK_INTERVAL_LEVEL; i++) {
		if ((para[i].min_soc < ui_capacity) && (ui_capacity <= para[i].max_soc))
			return para[i].interval;
	}

	return default_value;
}

static void bat_ui_capacity_set_work_interval(struct bat_ui_capacity_device *di)
{
	if (!di->monitoring_interval ||
		(di->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) ||
		(di->charge_status == POWER_SUPPLY_STATUS_NOT_CHARGING))
		di->monitoring_interval =
		bat_ui_capacity_select_interval(di->discharging_interval_para, di->ui_capacity,
			BUC_WORK_INTERVAL_NORMAL);

	if (di->charge_status == POWER_SUPPLY_STATUS_CHARGING)
		di->monitoring_interval =
		bat_ui_capacity_select_interval(di->charging_interval_para, di->ui_capacity,
			BUC_WORK_INTERVAL_CHARGING);;
	...
	int curr_capacity)
{
	...
	if ((di->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) ||
		(di->charge_status == POWER_SUPPLY_STATUS_NOT_CHARGING)) {
		if (di->ui_prev_capacity < curr_capacity) {
			hwlog_err("discharging capacity can not increase\n");
			return -1;
		}
	}
	...
	if ((di->charge_status == POWER_SUPPLY_STATUS_CHARGING) &&
		...
		if (di->ui_prev_capacity > curr_capacity) {
			hwlog_err("charging capacity can not decline\n");
			return -1;
		}
	}
...
static void bat_ui_capacity_data_init(struct bat_ui_capacity_device *di)
{
	...
	di->chg_force_full_wait_time = CAPACITY_FULL_CHECK_TIME;
...
static void bat_ui_vth_correct_dts(struct bat_ui_capacity_device *di)
{
	...
	int data_len = sizeof(di->vth_soc_calibration_data);
...
static int bat_ui_capacity_suspend(struct device *dev)
{
	struct bat_ui_capacity_device *di = dev_get_drvdata(dev);
...
static int bat_ui_capacity_resume(struct device *dev)
{
	return 0;
}

static void bat_ui_capacity_complete(struct device *dev)
{
	struct bat_ui_capacity_device *di = dev_get_drvdata(dev);
...
static const struct dev_pm_ops bat_ui_capacity_pm_ops = {
		.suspend  = bat_ui_capacity_suspend,
		.resume   = bat_ui_capacity_resume,
		.complete = bat_ui_capacity_complete,
};
#else
