...
static int bat_fault_probe(struct platform_device *pdev)
{
	...
	ret = power_event_nc_register(POWER_NT_COUL, &di->coul_event_nb);
	if (ret)
		goto fail_free_mem;
...
static int bat_fault_remove(struct platform_device *pdev)
{
	...
	power_event_nc_unregister(POWER_NT_COUL, &di->coul_event_nb);
	kfree(di);
	g_bat_fault_dev = NULL;
	return 0;
}
...
static int bat_fault_suspend(struct device *dev)
{
	bat_fault_update_cutoff_vol(true);
	return 0;
}

static int bat_fault_resume(struct device *dev)
{
	return 0;
}
