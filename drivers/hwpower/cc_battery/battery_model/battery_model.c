...
int bat_model_get_vaild_bat_id(int *bat_id)
{
	int id;
	const char *brand;

	id = bat_model_id_index();
	brand = bat_model_get_brand();
	hwlog_info("bat model brand is %s\n", brand);

	if (brand && strcmp(brand, "default")) {
		hwlog_info("get vaild battery id\n", __func__);
		*bat_id = id;
		return 0;
	} else {
		hwlog_info("bat id used default\n", __func__);
		return -1;
	}
}
