...
#define HLTHERM_RUNTEST_TEMP             25
...
static int multi_btb_get_temp_with_comp(int index, int temp_without_comp)
{
	...
	hwlog_debug("sensor:%d,raw:%d,temp:%d\n", index, temp_without_comp, temp_with_comp);
	return temp_with_comp;
}

static bool multi_btb_init_temp(struct multi_btb_temp_info *di);
static void multi_btb_update_temp(struct multi_btb_temp_info *di)
{
	int i, raw, temp;

	if (!di->is_init) {
		if (multi_btb_init_temp(di))
			di->is_init = true;
		hwlog_info("try get sensor\n");
		return;
	}
	...
	for (i = 0; i < di->sensor_num; i++) {
		...
		hwlog_debug("update temp%d:%s,raw:%d,temp:%d\n", i,
...
{
	int i;
	/* first config sensor must start earlier than multi_btb */
	for (i = 0; i < di->sensor_num; i++) {
		di->sensor_tab[i].raw = power_temp_get_average_value(di->sensor_tab[i].name);
		if (di->sensor_tab[i].raw == POWER_TEMP_INVALID_TEMP) {
			hwlog_err("init temp%d:%s,ntc:%d fail\n",
				i, di->sensor_tab[i].name, di->sensor_tab[i].raw);
			return false;
		}
...
static int multi_btb_temp_sync(enum bat_temp_id id, int *temp)
{
	...
	if ((di->sensor_num <= 0) || ((int)id >= di->sensor_num) || !di->is_init)
		id = BTB_TEMP_0;

	batt_temp = di->sensor_tab[id].temp;

	hwlog_debug("temp_sync%d temp:%d from cache\n", id, batt_temp);
	*temp = batt_temp / THOUSAND_UNIT;
	return 0;
#endif
}
...
static int multi_btb_temp_probe(struct platform_device *pdev)
{
	...
	if (!di->is_init && multi_btb_init_temp(di)) {
		 di->is_init = true;
	}
...
late_initcall(multi_btb_temp_init);
