...
static int g_cycle_count_base = 0;
static int g_cycle_count_offset = 0;
...
static int __init batt_parse_cycle_count_cmdline(char *p)
{
	int ret;

	if (!p)
		return 0;
	ret = sscanf(p, "%x,%x", &g_cycle_count_base, &g_cycle_count_offset);
	hwlog_info("%s ret=%d g_cycle_count_base:%d , g_cycle_count_offset:%d \n", __func__, ret, g_cycle_count_base , g_cycle_count_offset);
	return ret;
}
early_param("cyclecountoffset", batt_parse_cycle_count_cmdline);
...
static int bat_core_set_voltage_max_now(int value)
{
	...
	int set_value = (int)(value/BAT_CORE_UNIT_UTOM);
	int dec;
	struct bat_core_device *di = g_bat_core_dev;

	if (!di || (set_value > max)) {
		hwlog_err("bat_core_set_voltage_max_now error set_value=%d max=%d\n", set_value, max);
		return -EPERM;
	}

	di->data.voltage_max_now = value;
	dec = max - set_value;
	hwlog_info("bat_core_set_voltage_max_now set_value=%d max=%d\n", set_value, max);
...
static int bat_core_temp(struct bat_core_device *di)
{
	int temp = coul_interface_get_battery_temperature(di->config.coul_type);
	static int last_temp;
	static int is_init = 0;

	if (temp == -EPERM) {
		hwlog_err(" get bat_core_temp fail\n");
		return temp;
	}
	...
	if (!is_init) {
		last_temp = temp;
		is_init = 1;
		hwlog_info("is_init%d temp%d\n", last_temp, temp);
		return temp;
	}

	if ((temp - last_temp) > TEMP_DIFF) {
		last_temp = last_temp + TEMP_DIFF;
		hwlog_info("temp  %d > last temp %d\n", temp, last_temp);
		return last_temp;
	} else if((last_temp - temp) > TEMP_DIFF) {
		last_temp = last_temp - TEMP_DIFF;
		hwlog_info("temp %d < last temp%d\n", temp, last_temp);
		return last_temp;
	}
...
static void bat_core_update_data(struct bat_core_device *di)
{
	...
	exist = coul_interface_is_battery_exist(di->config.coul_type);
	fcc = coul_interface_get_battery_fcc(di->config.coul_type);
	capacity = bat_ui_capacity();
	level = bat_core_update_capacity_level(capacity);
	...
	if (!exist) {
		di->data.health = POWER_SUPPLY_HEALTH_UNKNOWN;
		di->data.capacity_level = POWER_SUPPLY_CAPACITY_LEVEL_UNKNOWN;
		di->data.temp_now = POWER_SUPPLY_DEFAULT_TEMP;
	} else {
		di->data.temp_now = bat_core_temp(di);
		di->data.cycle_count = coul_interface_get_battery_cycle(di->config.coul_type);
	}
...
static int bat_core_suspend(struct device *dev)
{
	struct bat_core_device *di = dev_get_drvdata(dev);
...
static int bat_core_resume(struct device *dev)
{
	return 0;
}

static void bat_core_complete(struct device *dev)
{
	struct bat_core_device *di = dev_get_drvdata(dev);
...
static const struct dev_pm_ops bat_core_pm_ops = {
		.suspend  = bat_core_suspend,
		.resume   = bat_core_resume,
		.complete = bat_core_complete,
};
#else
...
#ifdef CONFIG_PM
#ifndef CONFIG_I2C_OPERATION_IN_COMPLETE
	.suspend = bat_core_suspend,
	.resume = bat_core_resume,
#endif
#endif /* CONFIG_PM */
