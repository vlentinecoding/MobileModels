...
static int power_cali_reset(const char *name)
{
	struct power_cali_ops *ops = power_cali_find_ops(name);
	int ret;

	if (!ops || !ops->reset) {
		hwlog_err("ops or ops->reset is null\n");
		return -EINVAL;
	}
	ret = ops->reset(ops->dev_data);
	return ret;
}
...
static struct power_sysfs_attr_info power_cali_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_wo(power_cali, 0220, POWER_CALI_SYSFS_RESET, reset),
};
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case POWER_CALI_SYSFS_COUL_SAVE:
		if (power_cali_save_data("coul") < 0)
			return -EINVAL;
		break;
	case POWER_CALI_SYSFS_RESET:
		power_cali_reset("coul");
		break;
	default:
		break;
	}
