...
#define TEMP_LOW_THRESHOLD          11000
...
int power_get_compensation_value(int raw, struct common_comp_data *data)
{
	...
	hwlog_debug("refer:%d, without_comp:%d, with_comp:%d\n",
...
int power_get_smooth_compensation_value(struct smooth_comp_data *data)
{
	...
	if (current_comp - data->last_comp > data->max_delta)
		...
		temp_val = data->last_comp - data->max_delta;
		if (current_raw > TEMP_LOW_THRESHOLD) {
			current_comp = temp_val > TEMP_LOW_THRESHOLD ? temp_val : data->last_comp;
			hwlog_info("abnormal temp_val= %d\n", temp_val);
		}
	}
