...
static const char * const g_power_if_sysfs_type_table[POWER_IF_SYSFS_END] = {
	...
	[POWER_IF_SYSFS_ENABLE_HV_CHARGE] = "enable_hv_charge",
};
	...
	char *value_str)
{
	...
	case POWER_IF_SYSFS_RTB_SUCCESS:
		...
		break;
	case POWER_IF_SYSFS_ENABLE_HV_CHARGE:
		if (l_ops->set_enable_hv_charge) {
			ret = l_ops->set_enable_hv_charge(value);
		}
		break;
	default:
		break;
...
static struct power_sysfs_attr_info power_if_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_rw(power_if, 0640, POWER_IF_SYSFS_ENABLE_HV_CHARGE, enable_hv_charge),
};
