...
static const char * const g_power_if_sysfs_type_table[POWER_IF_SYSFS_END] = {
	...
	[POWER_IF_SYSFS_FG_TERM] = "fg_term",
};
	...
	unsigned int *value)
{
	...
	case POWER_IF_SYSFS_VBUS:
		...
		break;
	case POWER_IF_SYSFS_FG_TERM:
		if (l_ops->get_fg_term_status) {
			ret = l_ops->get_fg_term_status((int *)value);
			hwlog_info("get fg term status=%d\n", *value);
		} else {
			*value = 1; /* 1:fuel guage charging full */
			ret = POWER_IF_ERRCODE_PASS;
		}
	default:
		break;
	}
...
static struct power_sysfs_attr_info power_if_sysfs_field_tbl[] = {
	...
	power_sysfs_attr_rw(power_if, 0440, POWER_IF_SYSFS_FG_TERM, fg_term),
};
