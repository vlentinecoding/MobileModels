...
static struct blocking_notifier_head g_power_event_nh[POWER_NT_END];
...
static const char * const g_power_event_ne_table[POWER_NE_END] = {
	...
	[POWER_NE_START_CHARGING] = "start_charging",
	[POWER_NE_STOP_CHARGING] = "stop_charging",
	[POWER_NE_SUSPEND_CHARGING] = "suspend_charging",
	...
	[POWER_NE_LIGHTSTRAP_GET_PRODUCT_TYPE] = "lightstrap_get_product_type",
	[POWER_NE_LIGHTSTRAP_EPT] = "lightstrap_ept",
	/* section: for otg */
	[POWER_NE_OTG_SC_CHECK_STOP] = "otg_sc_check_stop",
	[POWER_NE_OTG_SC_CHECK_START] = "otg_sc_check_start",
	...
	[POWER_NE_WLC_CERT_SUCC] = "wlc_cert_succ",
	...
	[POWER_NE_WLTX_PING_OCP_OVP] = "wltx_ping_ocp_ovp",
	...
	[POWER_NE_WLTX_DEV_ATTCH_EVT] = "wltx_dev_attch_evt",
	[POWER_NE_WLTX_DEV_KP_BT_EVT] = "wltx_dev_bt_evt",
	[POWER_NE_WLTX_DEV_KP_CHAN_SWITCH] = "wltx_dev_kp_chan_switch",
...
static void power_event_nc_init(void)
{
	int i;

	for (i = 0; i < POWER_NT_END; i++)
		BLOCKING_INIT_NOTIFIER_HEAD(&g_power_event_nh[i]);
}

int power_event_nc_cond_register(unsigned int type, struct notifier_block *nb)
{
	if (type >= POWER_NT_END) {
	...
	return blocking_notifier_chain_cond_register(&g_power_event_nh[type], nb);
}
...
int power_event_nc_register(unsigned int type, struct notifier_block *nb)
{
	...
	return blocking_notifier_chain_register(&g_power_event_nh[type], nb);
}
...
int power_event_nc_unregister(unsigned int type, struct notifier_block *nb)
{
	...
	return blocking_notifier_chain_unregister(&g_power_event_nh[type], nb);
}
...
void power_event_notify(unsigned int type, unsigned long event, void *data)
{
	...
	hwlog_info("receive event type=%u event=%lu,%s\n",
		type, event, power_event_get_ne_name(event));
	blocking_notifier_call_chain(&g_power_event_nh[type], event, data);
}

#ifdef CONFIG_SYSFS
static struct attribute *power_event_sysfs_attrs[] = {
	NULL,
};
