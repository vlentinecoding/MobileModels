...
static const char * const g_power_log_device_id_table[] = {
	...
	[POWER_LOG_DEVICE_ID_AW32280_AUX] = "aw32280_aux",
	[POWER_LOG_DEVICE_ID_RT9426A] = "rt9426a",
	[POWER_LOG_DEVICE_ID_CW2217] = "cw2217",
	[POWER_LOG_DEVICE_ID_ADSP] = "adsp",
};
	...
	struct device_attribute *attr, char *buf)
{
	...
	case POWER_LOG_SYSFS_HEAD:
		if (!l_dev->ops[l_dev->dev_id])
			return -EINVAL;

		if (power_log_operate_ops(l_dev->ops[l_dev->dev_id],
			POWER_LOG_DUMP_LOG_HEAD, buf, PAGE_SIZE - 1))
			return -EINVAL;
	...
	case POWER_LOG_SYSFS_CONTENT:
		...
		if (power_log_operate_ops(l_dev->ops[l_dev->dev_id],
			POWER_LOG_DUMP_LOG_CONTENT, buf, PAGE_SIZE - 1))
			return -EINVAL;
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case POWER_LOG_SYSFS_DEV_ID:
		...
		l_dev->dev_id = power_log_get_device_id(dev_id);
		if ((l_dev->dev_id < POWER_LOG_DEVICE_ID_BEGIN) ||
			(l_dev->dev_id >= POWER_LOG_DEVICE_ID_END) ||
			!l_dev->ops[l_dev->dev_id]) {
			l_dev->dev_id = 0;
			hwlog_err("unable to parse input:%s\n", buf);
			return -EINVAL;
		}
