...
static void soc_decimal_init_fifo(struct soc_decimal_dev *l_dev)
{
	...
	ui_soc = power_platform_get_battery_ui_capacity() * l_dev->info.para.base;
...
static u32 soc_decimal_calculate_soc(struct soc_decimal_dev *l_dev)
{
	...
	if (calc.curr_samples < calc.samples) {
		...
		else if ((calc.init_soc >= 90 * calc.base) &&
			(calc.init_soc < 100 * calc.base))
			calc.curr_soc = calc.init_soc + (calc.curr_samples / 5);
		else
			calc.curr_soc = 100 * calc.base; /* Greater than 100, display 100 */
	...
	struct soc_decimal_dev *l_dev)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ui_offset", &l_dev->ui_offset, POWER_PLATFOR_SOC_UI_OFFSET);
...
static int soc_decimal_probe(struct platform_device *pdev)
{
	...
	ret = power_event_nc_register(POWER_NT_SOC_DECIMAL, &l_dev->nb);
	if (ret)
		goto fail_free_mem;
	ret = power_event_nc_register(POWER_NT_CHARGING, &l_dev->nb);
	if (ret)
...
fail_regist_event:
	power_event_nc_unregister(POWER_NT_SOC_DECIMAL, &l_dev->nb);
fail_free_mem:
	kfree(l_dev);
...
static int soc_decimal_remove(struct platform_device *pdev)
{
	...
	power_event_nc_unregister(POWER_NT_SOC_DECIMAL, &l_dev->nb);
	power_event_nc_unregister(POWER_NT_CHARGING, &l_dev->nb);
