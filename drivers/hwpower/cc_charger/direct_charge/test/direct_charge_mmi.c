...
static int dc_mmi_hsc_result(struct dc_mmi_data *di)
{
	struct direct_charge_device *l_di = direct_charge_get_di();

	if (!l_di || (l_di->working_mode != HSC_MODE))
		return di->hsc_succ_flag;
	...
	if (direct_charge_get_stage_status() < DC_STAGE_CHARGING)
		return di->hsc_succ_flag;
	dc_mmi_set_succ_flag(HSC_MODE, DC_ERROR_PROT_CHECK);
	if ((di->prot_state & di->dts_para.expt_prot) != di->dts_para.expt_prot)
		return di->hsc_succ_flag;
	if (di->dts_para.multi_sc_test) {
		dc_mmi_set_succ_flag(HSC_MODE, DC_ERROR_OPEN_MULTI_SC);
		if (dc_mmi_multi_sc_test())
			return di->hsc_succ_flag;
	}
	if (di->dts_para.ibat_th) {
		dc_mmi_set_succ_flag(HSC_MODE, DC_ERROR_CUR_TEST);
		if (dc_mmi_ibat_test(di))
			return di->hsc_succ_flag;
	}

	dc_mmi_set_succ_flag(HSC_MODE, DC_SUCC);
	return 0;
}
...
static void dc_mmi_init(struct device *dev)
{
	...
	di->hsc_succ_flag = DC_ERROR_ADAPTER_DETECT;
	...
	ret = power_event_nc_register(POWER_NT_DC, &di->dc_mmi_nb);
	if (ret)
		goto free_mem;
...
unregister_notifier:
	power_event_nc_unregister(POWER_NT_DC, &di->dc_mmi_nb);
free_mem:
	devm_kfree(dev, di);
	g_dc_mmi_di = NULL;
}
...
static void dc_mmi_exit(struct device *dev)
{
	...
	power_event_nc_unregister(POWER_NT_DC, &g_dc_mmi_di->dc_mmi_nb);
