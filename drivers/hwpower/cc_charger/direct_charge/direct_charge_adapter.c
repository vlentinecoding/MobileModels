...
{
	adapter_set_usbpd_enable(g_prot, enable);
}
...
unsigned int dc_update_adapter_support_mode(void)
{
	unsigned int adp_mode = ADAPTER_SUPPORT_SCP_B_LVC +
		ADAPTER_SUPPORT_SCP_B_SC + ADAPTER_SUPPORT_SCP_B_HSC;
...
{
	return adapter_detect_adapter_support_mode(g_prot, mode);
}

static int dc_wait_adapter_antifake(void)
{
	struct direct_charge_device *l_di = direct_charge_get_di();
...
int dc_check_adapter_antifake(void)
{
	...
	if (max_cur == 0) {
		snprintf(tmp_buf, sizeof(tmp_buf), "dc_get_adapter_max_current 0\n");
		dc_fill_eh_buf(l_di->dsm_buff, sizeof(l_di->dsm_buff), DC_EH_APT_ANTIFAKE,tmp_buf);
		return -1;
	}
	...
	if (ret) {
		snprintf(tmp_buf, sizeof(tmp_buf), "dapter_auth_encrypt fail\n");
		dc_fill_eh_buf(l_di->dsm_buff, sizeof(l_di->dsm_buff), DC_EH_APT_ANTIFAKE,tmp_buf);
		goto fail_check;
	}
	...
	else if (l_di->working_mode == HSC_MODE)
		n_data.event = "AUTH_DCCT=3";
	else
		n_data.event = NULL;
	n_data.event_len = 11; /* length of AUTH_DCCT=2 */
	power_event_report_uevent(&n_data);
	ret = dc_wait_adapter_antifake();
...
{
	return adapter_is_ignore_cable_check(ADAPTER_PROTOCOL_SCP);
}
