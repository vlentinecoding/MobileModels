...
static void dc_report_antifake_result(struct direct_charge_device *di, int res)
{
	int ret;
	int len;
	int vendor_id = 0;
	...
	ret = adapter_get_adp_type(g_prot, &adapter_type);
	if (ret)
		hwlog_info("get adapter type fail\n");

	len = snprintf(auth_buf, POWER_EVENT_NOTIFY_SIZE - 1,
		"BMS_EVT=EVT_ADAPTER_AUTH_UPDATE@vendor_id:%d,serial_no:%d,protocol:scp,adp_type:%d,auth_state:%d\n",
		vendor_id, serial_num, adapter_type, antifake_state);
	if (len <= 0)
		return;
...
end_check:
	...
	dc_report_antifake_result(l_di, ret);
	return ret;
}
