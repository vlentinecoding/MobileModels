...
static int dc_ps_probe(struct platform_device *pdev)
{
	...
	if (g_scp_ps_by_5vboost)
		(void)direct_charge_pps_ops_register(&scp_ps_5vboost_ops);
	else if (g_scp_ps_by_charger)
		(void)direct_charge_pps_ops_register(&scp_ps_charger_ops);
	else
		(void)direct_charge_pps_ops_register(&scp_ps_dummy_ops);
