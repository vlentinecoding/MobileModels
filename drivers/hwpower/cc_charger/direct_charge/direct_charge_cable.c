...
#define HSC_RESIST_IBUS_TH        2000
#define CTC_EMARK_CURR_5A         5000

/* for OB adapter */
#define OB_RETRY_TIMES 3
#define VALID_OB_VOLTAGE_RANGE 200
...
static void dc_detect_std_cable(void)
{
	...
	bool is_ignore_cable_check = dc_is_ignore_cable_check();
	...
	if ((adapter_type == ADAPTER_TYPE_10V2A) ||
		(adapter_type == ADAPTER_TYPE_10V2P25A) || is_ignore_cable_check) {
...
int dc_calculate_path_resistance(int *rpath)
{
	...
	for (i = 0; i < retry; ++i) {
		...
		/* if adapter vendor is OB, judge the validity of Vadapt */
		if (l_di->adaptor_vendor_id == SCP_PROTOCOL_VENDOR_ID_ON_BRIGHT) {
			for (j = 0; j < OB_RETRY_TIMES; j++) {
				if (dc_get_adapter_voltage(&tmp.vadapt))
					return -1;
				if (abs(tmp.vadapt - l_di->adaptor_vset) <= VALID_OB_VOLTAGE_RANGE)
					break;
			}
		} else {
			if (dc_get_adapter_voltage(&tmp.vadapt))
...
int dc_calculate_second_path_resistance(void)
{
	...
	if (l_di->working_mode == HSC_MODE)
		resist_ibus = HSC_RESIST_IBUS_TH;
	else
		resist_ibus = SEC_RESIST_IBUS_TH;
	if (!l_di || (l_di->ls_ibus < resist_ibus))
		return -1;
	...
	snprintf(tmp_buf, sizeof(tmp_buf),
		"f_Rpath=%d, S_Rpath=%d, Vapt=%d, Vbus=%d, Ibus=%d, Vapt0=%d, Vbus0=%d, Ibus0=%d\n",
