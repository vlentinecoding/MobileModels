...
static void dc_parse_temp_cv_para(struct device_node *np,
	struct direct_charge_device *di)
{
	int row, col, len;
	...
	for (row = 0; row < len / DC_TEMP_CV_TOTOL; row++) {
		...
		di->temp_cv_para[row].temp_min = idata[col];
		col = row * DC_TEMP_CV_TOTOL + DC_TEMP_CV_MAX;
		di->temp_cv_para[row].temp_max = idata[col];
		col = row * DC_TEMP_CV_TOTOL + DC_TEMP_CV_CUR;
		di->temp_cv_para[row].cv_curr = idata[col];
	}
}
	...
	struct direct_charge_device *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"lvc_in_aux_mode", &di->lvc_in_aux_mode, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"is_align_caculation_work", &di->is_align_caculation_work, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sbu_unsafe_sc_enable", (u32 *)&di->sbu_unsafe_sc_enable, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"path_ibus_th", (u32 *)&di->path_ibus_th, MIN_CURRENT_FOR_RES_DETECT);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
	...
	struct direct_charge_device *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"cam_flash_enable_path", &di->cam_flash_enable_path, 0);
	power_gpio_config_output(np, "flash_ovp_off", "flash_ovp_off",
		&di->flash_ovp_off_gpio, 0);
}
