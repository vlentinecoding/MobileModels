...
static int dc_enable_multi_ic(struct direct_charge_device *di, int enable)
{
	...
	dc_ic_set_sc_mode(di->working_mode, mode, di->sc_mode);
...
static void dc_change2singlepath(struct direct_charge_device *di)
{
	...
	direct_charge_select_comp_param(di);
}
...
static void dc_change2multipath(struct direct_charge_device *di)
{
	...
	if (di->sysfs_mainsc_enable_charger ^ di->sysfs_auxsc_enable_charger) {
		hwlog_err("%s:single sc flag is set\n", __func__);
		return;
	}
...
int dc_open_wired_channel(void)
{
	...
	if (l_di->need_wired_sw_off)
		return wired_chsw_set_wired_channel(WIRED_CHANNEL_MAIN, WIRED_CHSW_CLIENT_USB, WIRED_CHANNEL_RESTORE);
...
int dc_close_wired_channel(void)
{
	...
	if (l_di->need_wired_sw_off)
		return wired_chsw_set_wired_channel(WIRED_CHANNEL_ALL, WIRED_CHSW_CLIENT_USB, WIRED_CHANNEL_CUTOFF);
...
void dc_open_aux_wired_channel(void)
{
	...
	if (l_di->multi_ic_mode_para.support_multi_ic &&
		...
		wired_chsw_set_wired_channel(WIRED_CHANNEL_AUX, WIRED_CHSW_CLIENT_USB, WIRED_CHANNEL_RESTORE);
...
void dc_close_aux_wired_channel(void)
{
	...
	if (l_di->multi_ic_mode_para.support_multi_ic)
		wired_chsw_set_wired_channel(WIRED_CHANNEL_AUX, WIRED_CHSW_CLIENT_USB, WIRED_CHANNEL_CUTOFF);
}
...
void dc_select_charge_path(void)
{
	...
	if (dc_get_ic_ibus(l_di->working_mode, l_di->cur_mode, &ibus))
		return;
...
int dc_switch_charging_path(unsigned int path, int hv_flag)
{
	...
	case PATH_LVC:
	case PATH_SC:
		return direct_charge_switch_path_to_dc_charging(hv_flag);
	default:
		return -1;
	}
}
