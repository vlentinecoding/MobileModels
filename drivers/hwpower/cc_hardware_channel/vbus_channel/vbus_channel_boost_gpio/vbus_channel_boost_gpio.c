	...
	unsigned int user)
{
	if (l_dev->otg_sc_check_enable && (user == VBUS_CH_USER_PD))
		power_event_notify(POWER_NT_OTG, POWER_NE_OTG_SC_CHECK_START,
...
{
	if (l_dev->otg_sc_check_enable && (user == VBUS_CH_USER_PD))
		power_event_notify(POWER_NT_OTG, POWER_NE_OTG_SC_CHECK_STOP,
			NULL);
}
...
static void boost_gpio_otg_ocp_work(struct work_struct *work)
{
	...
	power_event_notify(POWER_NT_OTG, POWER_NE_OTG_OCP_HANDLE, &l_dev->otg_ocp_int);
}
...
static int boost_gpio_start_config(int flag)
{
	...
	wired_chsw_set_wired_channel(WIRED_CHANNEL_ALL, WIRED_CHSW_CLIENT_BOOST, WIRED_CHANNEL_CUTOFF);
