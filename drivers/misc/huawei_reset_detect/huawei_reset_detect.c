				  ...
				  unsigned long event, void *ptr)
{
	...
	pr_info(RESET_DETECT_TAG "apanic_handler enters \n");
...
static int __init reset_magic_address_get(void)
{
	...
	unsigned long magic_number = RESET_MAGIC_HW_RESET;
	struct resource dts_res_tmpinfo;
	...
	if (!of_address_to_resource(np, 0, &dts_res_tmpinfo))
	{
		pr_err(RESET_DETECT_TAG "DTS AP reset_magic_addr=%p \n", dts_res_tmpinfo.start);
	}
	...
	pr_err(RESET_DETECT_TAG "Ioremap AP reset_magic_addr=%p \n", reset_magic_addr);
	magic_number = raw_readl(reset_magic_addr);
	pr_err(RESET_DETECT_TAG "AP reset MAGIC NUM 0x%lx\n", magic_number);
...
static int is_tz_reset(unsigned int magic_number) {
	switch (magic_number) {
		...
		case TZBSP_ERR_FATAL_MON_PREFETCH_ABT:
		case TZBSP_ERR_FATAL_MON_DATA_ABT:
		case TZBSP_ERR_FATAL_QSEE_PREFETCH_ABT:
		case TZBSP_ERR_FATAL_QSEE_DATA_ABT:
		case TZBSP_ERR_FATAL_QSEE_UNDEF_ABT:
		case TZBSP_ERR_FATAL_QSEE_STACK_CHK_FAILED:
		case TZBSP_ERR_FATAL_MON_STACK_CHK_FAILED:
		case TZBSP_ERR_FATAL_MON_ASSERT_FAILED:
...
static int set_tz_reset_magic_number(void)
{
			...
			&& (is_tz_reset(tz_reset_reason) ))
	{
		...
		pr_err(RESET_DETECT_TAG "%s:written tz reset"
	...
	else if (RESET_MAGIC_HW_RESET == magic_number)
	{
		 ...
		 * reboot*/
		pr_err(RESET_DETECT_TAG "%s:its a HARD RESET",
				__func__);
	}
	else
	{
		pr_err(RESET_DETECT_TAG "%s:its an HLOS triggered reset",
				__func__);
	}
...
static int tz_reset_reason_address_get(void)
{
	...
	if (!np)
	{
		pr_err(RESET_DETECT_TAG "reset_detect:unable to find DT imem"
				" tz_reset_reason_addr node\n");
		return -1;
	}
	if (!of_address_to_resource(np, 0, &dts_res_tmpinfo))
	{
		pr_err(RESET_DETECT_TAG "DTS TZ reset_magic_addr=%p \n", dts_res_tmpinfo.start);
	}
	...
	pr_err(RESET_DETECT_TAG "Ioremap TZ reset_magic_addr=%p \n", tz_reset_reason_addr);

	tz_reset_reason = raw_readl(tz_reset_reason_addr);
	pr_err(RESET_DETECT_TAG "TZ reset reason: %lx\n", tz_reset_reason);
