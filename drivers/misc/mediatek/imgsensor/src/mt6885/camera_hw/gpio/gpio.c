...
struct GPIO_PINCTRL gpio_pinctrl_list_cam[
	...
	{"vcama1_en_1"},
	{"vcama1_en_0"},
	{"5v_boost_1"}, /* config camera 5v boost */
	{"5v_boost_0"}, /* config camera 5v boost */
};
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	if (pin == IMGSENSOR_HW_PIN_5V_BOOST) {
		boost_5v_enable(gpio_state, BOOST_CTRL_CAMERA);
		pr_info("config 5v boost %d", gpio_state);
		return IMGSENSOR_RETURN_SUCCESS;
	}
