...
void set_dummy(struct camkit_sensor_params *params)
{
	...
	LOG_INF("[%s] frame_length:%u\n", params->sensor_name,
		aec_ctrl.frame_length);
	...
	uint32 shutter, uint32 frame_length)
{
	...
	LOG_INF("[%s] input shutter = %u, input fl = %u\n",
		params->sensor_name, shutter, frame_length);
	...
	if ((params->sensor_info.need_extra_vts_offset == 1) &&
		...
		(sensor_ctrl->frame_length - sensor_ctrl->last_shutter <
			aec_info->extra_vts_offset)) {
		LOG_INF("[%s] frame_length(%u) < last_shutter(%u) + extra_vts_offset(%u), extend frame_length\n",
	...
	LOG_INF("[%s] shutter = %u(0x%x), fl = %u(0x%x)\n", params->sensor_name,
		shutter, shutter, aec_ctrl.frame_length, aec_ctrl.frame_length);
