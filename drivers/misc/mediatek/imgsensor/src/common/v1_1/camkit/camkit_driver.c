...
#define PDAF_HFPS_INDEX 0x03
#define PDAF_CUSTOM1_INDEX 5
	...
	uint8 mode, struct camkit_mode_info *mode_info)
{
	...
	params->sensor_ctrl.last_shutter = 0;
	spin_unlock(&camkit_lock);
}
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (!mode_info || !setting || !params) {
		LOG_ERR("get settings info by scenario failed\n");
		return ERROR_DRIVER_INIT_FAIL;
	}

	update_sensor_ctrl(params, mode, mode_info);
	LOG_INF("[%s] Enter scenario_id:%d\n", params->sensor_name, scenario_id);
	...
	unsigned long long *feature_data)
{
	...
	case MSDK_SCENARIO_ID_CUSTOM3:
		err = memcpy_s((void *)PDAFinfo, sizeof(struct SET_PD_BLOCK_INFO_T),
			(void *)&params->pdaf_info_scenario[PDAF_HFPS_INDEX],
			sizeof(struct SET_PD_BLOCK_INFO_T));
		if (err != EOK) {
			LOG_INF("memcpy_s PDAFinfo failed!! err is %d\n", err);
			return;
	...
	case MSDK_SCENARIO_ID_CUSTOM1:
		err = memcpy_s((void *)PDAFinfo, sizeof(struct SET_PD_BLOCK_INFO_T),
			(void *)&params->pdaf_info_scenario[PDAF_CUSTOM1_INDEX],
			sizeof(struct SET_PD_BLOCK_INFO_T));
		if (err != EOK) {
			LOG_INF("memcpy_s custom1 PDAFinfo failed!! err is %d\n", err);
			return;
		}
		break;
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
