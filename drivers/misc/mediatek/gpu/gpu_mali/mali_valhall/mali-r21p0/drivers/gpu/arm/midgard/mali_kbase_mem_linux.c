...
static vm_fault_t kbase_cpu_vm_fault(struct vm_fault *vmf)
{
	...
	if (aliased)
		kbase_get_compressed_alloc(aliased->alloc, 0, aliased->alloc->nents);
	else
		kbase_get_compressed_alloc(map->alloc, map_start_pgoff,
...
{
#ifdef CONFIG_GPU_GMC_GENERIC
	bool may_compressed = (map->gpu_alloc != NULL) && (map->gpu_alloc->reg != NULL);
#endif
	...
	if (may_compressed) {
		...
		unsigned long page_index = ((size_t)map->gpu_pages - (size_t)start_page) / sizeof(*start_page);
		kbase_gpu_vm_lock(kctx);
