...
static uint64_t g_ui64InBusy;
static uint64_t g_ui64InTime;
static int g_num;
#endif
	...
	if (dvfs_margin_mode & 0x10) {
		...
		if (t_gpu > t_gpu_target_hd) { /* must set to max. margin */
			...
			if (margin_low_bound > dvfs_margin_value*10)
				margin_low_bound = dvfs_margin_value*10;

			if (margin_low_bound < dvfs_margin_low_bound*10)
				margin_low_bound = dvfs_margin_low_bound*10;
		}
...
#ifdef CONFIG_MTK_GPU_OPP_STATS_SUPPORT
void ged_dvfs_reset_opp_cost(int oppsize, uint64_t reset)
{
	int i;
...
int ged_dvfs_init_opp_cost(void)
{
	...
	g_num = oppsize;
	ged_dvfs_reset_opp_cost(oppsize, 0);
