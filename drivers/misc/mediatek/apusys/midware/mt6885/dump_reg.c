...
void dump_gals_reg(bool dump_vpu)
{
	...
	for (i = 0; i < TOTAL_DBG_MUX_COUNT; ++i) {
		...
		printk("%s %s: %08x \n", __func__, value_table[i].name, gals_reg[i]);
	}
}
...
void apusys_reg_dump(void)
{
	...
	for (i = 0; i < SEGMENT_COUNT; ++i) {
		...
		if (range_table[i].base == 0x190F0000 || range_table[i].base == 0x190F1000) {
			print_hex_dump(KERN_DEBUG, range_table[i].name, DUMP_PREFIX_OFFSET, 32, 1, apu_top + offset, size, 1);
		}
	}
