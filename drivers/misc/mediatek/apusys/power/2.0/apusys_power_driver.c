...
(defined(CONFIG_MACH_MT6893) && defined(CONFIG_MTK_PLAT_MT6885_EMULATION))
	char *pwr_ptr;
...
#define DUMP_BASE (0x10000000)
#define DUMP_SIZE (0x10000)
static void dump_debug_reg(void)
{
	void __iomem *apu_dump = NULL;
	uint32_t reg_val = 0x0;

	apu_dump = ioremap_nocache (DUMP_BASE, DUMP_SIZE);

	reg_val = ioread32(apu_dump + 0x1228);
	pr_info("%s INFRA2APU_APB_TX port_rdy = 0x%08x \n", __func__, reg_val);

	reg_val = ioread32(apu_dump + 0x12E4);
	pr_info("%s INFRA2APU_APB_RX port_rdy = 0x%08x \n", __func__, reg_val);

	reg_val = ioread32(apu_dump + 0x1220);
	pr_info("%s INFRA2APU APB_TX = 0x%08x \n", __func__, reg_val);

	reg_val = ioread32(apu_dump + 0x12C0);
	pr_info("%s INFRA2APU APB_RX = 0x%08x \n", __func__, reg_val);
...
int apu_device_power_suspend(enum DVFS_USER user, int is_suspend)
{
	...
	if (ret) {
		hal_config_power(PWR_CMD_DUMP_FAIL_STATE, VPU0, NULL);
		printk("APUPWR_OFF_FAIL", "user:%d, is_suspend:%d\n",
			user, is_suspend);
		printk("%s %d is_power_debug_lock:%d, fixed_opp:%d\n",
			__func__, __LINE__,
			is_power_debug_lock, fixed_opp);
		apusys_reg_dump();
		dump_debug_reg();
