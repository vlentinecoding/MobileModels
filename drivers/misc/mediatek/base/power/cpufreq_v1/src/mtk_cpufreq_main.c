...
static void ppm_limit_callback(struct ppm_client_req req)
{
	...
	/*
	 * check dvfs_limit_enabled, if enabled, ppm dvfs limit will not set
	 * to policy
	 */
	if (mt_ppm_dvfs_limit_callback_locked()) {
		return;
	}
#endif
...
static const char *find_supply_name(struct device *dev)
{
	...
	if (!cpu) {
		pp = of_find_property(np, "proc1-supply", NULL);
		if (pp) {
			name = "proc1";
			goto node_put;
		}
	...
	pp = of_find_property(np, "proc2-supply", NULL);
	if (pp) {
		name = "proc2";
		goto node_put;
	}
...
static int _mt_cpufreq_target_index(struct cpufreq_policy *policy, unsigned int index)
{
	...
	int ret = 0;
	unsigned int freqs[MAX_RETRY+1] = { 0 };
	...
	trace_cpufreq_hw_target_index(policy->cpu, policy->cur, next_freq,
			cpu_dvfs_get_freq_by_idx(p, p->idx_opp_tbl),
			next_freq, count, freq_changed, freqs);
					 ...
					 unsigned long event, void *data)
{
	...
	if (policy->cpu == 0 && max == policy->cpuinfo.max_freq
		&& policy->max == policy->cpuinfo.max_freq) {
		int idx = 0;
		idx = cpufreq_frequency_table_target(policy, max, CPUFREQ_RELATION_C);
		max = policy->freq_table[idx-1].frequency;
		tag_pr_debug("frequency table index %d\n",idx);
	}
#endif
	if (policy->max > max)
	...
	trace_cpufreq_limit_sspm_thermal(policy->cpu, p->idx_opp_ppm_limit,
					 max);
