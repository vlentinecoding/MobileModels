...
static unsigned int g_user_set_idx = LIMIT_IDX_DEFAULT;
					...
					enum mt_gpufreq_kicker kicker)
{
	...
	if (g_user_set_idx != LIMIT_IDX_DEFAULT)
		target_idx = g_user_set_idx;
...
static int mt_gpufreq_var_dump_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "gpu_loading = %d\n",
			gpu_loading);
	seq_printf(m, "gpu_user_set_idx = %d\n",
			g_user_set_idx);
...
static int mt_gpufreq_current_freq_proc_show(struct seq_file *m, void *v)
{
	if (mt_get_ckgen_freq(hf_fmfg_ck))
		seq_printf(m, "%d\n", g_cur_opp_freq);
	else
		seq_printf(m, "0\n");
	return 0;
...
/*
 * PROCFS : show current keeping max frequency state
 */
static int mt_gpufreq_max_freq_proc_show(struct seq_file *m, void *v)
{
	unsigned int keep_max_freq_idx;

	mutex_lock(&mt_gpufreq_limit_table_lock);
	keep_max_freq_idx = limit_table[KIR_PROC].upper_idx;
	mutex_unlock(&mt_gpufreq_limit_table_lock);

	if (keep_max_freq_idx != LIMIT_IDX_DEFAULT)
		seq_printf(m, "%d\n",
				   g_opp_table[keep_max_freq_idx].gpufreq_khz);
	else
		seq_printf(m, "%d\n",
...
/*
 * PROCFS : keeping max frequency setting
 * 0 : free run
 * 1 : keep max frequency
 */
static ssize_t mt_gpufreq_max_freq_proc_write(struct file *file,
		const char __user *buffer, size_t count, loff_t *data)
{
	char buf[64];
	unsigned int len = 0;
	unsigned int value = 0;
	unsigned int i = 0;
	unsigned int keep_min_freq_idx;
	int ret = -EFAULT;
	...
	mutex_lock(&mt_gpufreq_limit_table_lock);
	keep_min_freq_idx = limit_table[KIR_PROC].lower_idx;
	mutex_unlock(&mt_gpufreq_limit_table_lock);
	if (kstrtouint(buf, 10, &value) == 0) {
		for (i = g_segment_max_opp_idx;
			i <= g_segment_min_opp_idx;
			...
			if (value == g_opp_table[i].gpufreq_khz) {
				ret = 0;
				mt_gpufreq_update_limit_idx(
					KIR_PROC, i, keep_min_freq_idx);
				mt_gpufreq_target(g_cur_opp_idx - g_segment_max_opp_idx, KIR_PROC);
				break;
			}
		}
	}
...
/*
 * PROCFS : show current keeping min frequency state
 */
static int mt_gpufreq_min_freq_proc_show(struct seq_file *m, void *v)
{
	unsigned int keep_min_freq_idx;
	...
	if (keep_min_freq_idx != LIMIT_IDX_DEFAULT)
		seq_printf(m, "%d\n",
				   g_opp_table[keep_min_freq_idx].gpufreq_khz);
	else
		seq_printf(m, "%d\n",
 ...
 * 1 : keep max frequency
 */
static ssize_t mt_gpufreq_min_freq_proc_write(struct file *file,
		const char __user *buffer, size_t count, loff_t *data)
{
	char buf[64];
	unsigned int len = 0;
...
/*
 * PROCFS : show user set gpu frequency
 */
static int mt_gpufreq_user_set_freq_proc_show(struct seq_file *m, void *v)
{
	if (g_user_set_idx != LIMIT_IDX_DEFAULT)
		seq_printf(m, "%d\n", g_opp_table[g_user_set_idx].gpufreq_khz);
	else
		seq_puts(m, "0\n");
...
/*
 * PROCFS : user set gpu frequency
 */
static ssize_t mt_gpufreq_user_set_freq_proc_write(struct file *file,
		const char __user *buffer, size_t count, loff_t *data)
{
	char buf[64];
	unsigned int len;
	...
	if (value == 0) {
		g_user_set_idx = LIMIT_IDX_DEFAULT;
		ret = 0;
	} else {
		for (i = g_segment_max_opp_idx; i <= g_segment_min_opp_idx; i++) {
			if (value == g_opp_table[i].gpufreq_khz) {
				g_user_set_idx = i;
				ret = 0;
				mt_gpufreq_target(i, KIR_POLICY);
				break;
			}
		}
	}
...
/*
 * PROCFS : show gpu available frequencies
 */
static int mt_gpufreq_available_freq_proc_show(struct seq_file *m, void *v)
{
	int i;

	for (i = g_segment_min_opp_idx; i > g_segment_max_opp_idx; i--)
...
PROC_FOPS_RW(gpufreq_max_freq);
PROC_FOPS_RW(gpufreq_min_freq);
PROC_FOPS_RO(gpufreq_current_freq);
PROC_FOPS_RW(gpufreq_user_set_freq);
PROC_FOPS_RO(gpufreq_available_freq);
...
static int __mt_gpufreq_create_procfs(void)
{
	...
	const struct pentry entries[] = {
		...
		PROC_ENTRY(gpufreq_current_freq),
		PROC_ENTRY(gpufreq_opp_freq),
		PROC_ENTRY(gpufreq_max_freq),
		PROC_ENTRY(gpufreq_min_freq),
		PROC_ENTRY(gpufreq_user_set_freq),
		PROC_ENTRY(gpufreq_available_freq),
		...
		unsigned int vsram_gpu_old, unsigned int vsram_gpu_new)
{
	...
	trace_clock_set_rate("gpu", g_cur_opp_freq, 0);
...
static void __mt_devfreq_target(unsigned long t_freq)
{
	unsigned int idx;

	idx = mt_gpufreq_get_limited_idx_by_freq(t_freq);
	mt_gpufreq_target(idx, KIR_THERMAL);
}

#include "hw_ipa_gpu.c"

static void __mt_gpufreq_init_ipa(struct platform_device *pdev)
{
	...
	if (ret != 0) {
		gpufreq_pr_info("@%s: add opp table failed\n", __func__);
		return;
	}

	hw_gpu_devfreq_init(dev);

	gpufreq_pr_info("@%s end\n", __func__);
}
