...
static bool accdet_out_only;
#define NEED_DISABLE_MICBIAS_RUNTIME 0
...
static void cat_register(char *buf)
{
	...
	strncat(buf, accdet_log_buf, strlen(accdet_log_buf));
#elif defined CONFIG_ACCDET_SUPPORT_EINT1
	ret = sprintf(accdet_log_buf, "[ccdet EINT1 support][MODE_%d]regs:\n",
		accdet_dts.mic_mode);
	if (ret < 0)
		pr_notice("sprintf failed\n");
	strncat(buf, accdet_log_buf, strlen(accdet_log_buf));
#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
	ret = sprintf(accdet_log_buf, "[Accdet BI support][MODE_%d] regs:\n",
		accdet_dts.mic_mode);
	if (ret < 0)
		pr_notice("sprintf failed\n");
	strncat(buf, accdet_log_buf, strlen(accdet_log_buf));
	...
	const char *buf, size_t count)
{
	...
	addr = strsep(&val, ",");
	if (addr)
		ret = kstrtou32(addr, 16, (unsigned int *)&addr_tmp);
	if (val)
		ret = kstrtou32(val, 16, (unsigned int *)&value_tmp);
...
static inline void headset_plug_out(void)
{
	...
	pmic_write_mclr(PMIC_RG_AUDPREAMPLGAIN_ADDR,
	PMIC_RG_AUDPREAMPLINPUTSEL_SHIFT,
	0x03);
...
{
#if NEED_DISABLE_MICBIAS_RUNTIME
	u32 cur_AB, eintID;
...
cur_AB = pmic_read(PMIC_ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET;
	...
	if (cable_type == HEADSET_MIC) {
		...
		pr_info("%s more than 6s, cur_AB:0x%x c_type:0x%x, do nothing\n",
				__func__, cur_AB, cable_type);
#else
	pr_info("%s %d dis micbias do nothing\n", __func__, __LINE__);
#endif
}
#endif /* end of #if PMIC_ACCDET_KERNEL */
...
static inline void check_cable_type(void)
{
	...
	case PLUG_OUT:
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			if (eint_accdet_sync_flag) {
				...
				headset_type = ACCDET_NORMAL_3POLE;
				pr_info("accdet headset 3-pole\n");
			} else
			...
			if (eint_accdet_sync_flag) {
				...
				headset_type = ACCDET_NORMAL_4POLE;
				pr_info("accdet headset 4-pole\n");
			} else
				pr_info("accdet headset has been plug-out\n");
			mutex_unlock(&accdet_eint_irq_sync_mutex);
	...
	pr_info("accdet cur cable type:[%s], status switch:[%s]->[%s] %x\n",
		accdet_report_str[cable_type], accdet_status_str[pre_status],
		accdet_status_str[accdet_status], accdet_status);
}
...
#endif
{
	...
	if (eint_accdet_sync_flag) {
		if (pre_cable_type != cable_type) {
			send_accdet_status_event(cable_type, 1);
		} else if (accdet_out_only == true) {
			send_accdet_status_event(cable_type, 0);
			pre_cable_type = NO_DEVICE;
			cable_type = NO_DEVICE;
		}
	} else
	...
	pr_info("%s() report cable_type done %x\n", __func__, accdet_out_only);
