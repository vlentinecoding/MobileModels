...
#define COUL_TBATCAL_MIN_B      (-100000)
#define COUL_TBATCAL_MAX_B      100000
#define COUL_TBATCAL_MIN_A      800000
#define COUL_TBATCAL_MAX_A      1300000
...
static int pmic_batt_vol_update(int vol)
{
	int vol_a = 0;
	int vol_b = 0;

	if (coul_cali_get_para(COUL_CALI_MODE_MAIN, COUL_CALI_PARA_VOL_A, &vol_a))
		return vol;

	if (coul_cali_get_para(COUL_CALI_MODE_MAIN, COUL_CALI_PARA_VOL_B, &vol_b))
		return vol;

	if ((vol_b < COUL_TBATCAL_MIN_B) || (vol_b > COUL_TBATCAL_MAX_B))
		vol_b = 0;

	if ((vol_a >= COUL_TBATCAL_MIN_A) && (vol_a <= COUL_TBATCAL_MAX_A)) {
		vol = ((s64)vol_a * vol) / COUL_DEFAULT_VAL;
