	...
	char *reg_head, int size)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_mt6360_reg_addr); i++) {
		snprintf(buff, sizeof(buff), "R[0x%02X] ",
			g_mt6360_reg_addr[i]);
		len += strlen(buff);
		if (len < size)
...
static int mt6360_hw_dump_log_head(char *buffer, int size, void *dev_data)
{
	if (!buffer || !g_mpci || !g_mpci->chg_dev)
		return -1;

	return mt6360_get_register_head(g_mpci->chg_dev, buffer, size);
}
	...
	char *reg_value, int size)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_mt6360_reg_addr); i++) {
		...
		snprintf(buff, sizeof(buff), "0x%-6.2x", regval);
		len += strlen(buff);
		if (len < size)
...
static int mt6360_hw_dump_log_content(char *buffer, int size, void *dev_data)
{
	if (!buffer || !g_mpci || !g_mpci->chg_dev)
		return -1;

	return mt6360_hw_dump_register(g_mpci->chg_dev, buffer, size);
}
...
static struct power_log_ops mt6360_log_ops = {
	.dev_name = "mt6360",
	.dump_log_head = mt6360_hw_dump_log_head,
	.dump_log_content = mt6360_hw_dump_log_content,
};
...
&& !defined(CONFIG_TCPC_CLASS)
	...
	mt6360_log_ops.dev_data = (void *)mpci;
	ret = power_log_ops_register(&mt6360_log_ops);
	if (ret)
		dev_err(mpci->dev, "power_log_ops_register fail\n");
