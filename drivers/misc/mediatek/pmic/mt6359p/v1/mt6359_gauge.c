	...
	int *data)
{
	...
	if (*fg_is_charging == true)
		...
		bm_err("[%s] final current=%d (ratio=%d), PMIC_FG_CURRENT_OUT=0x%x,"
			"r_fg_value=%d, DEFAULT_R_FG=%d, UNIT_FGCURRENT=%d\n", __func__,
			dvalue, gauge_dev->fg_cust_data->car_tune_value, PMIC_FG_CURRENT_OUT,
			gauge_dev->fg_cust_data->r_fg_value, DEFAULT_R_FG, UNIT_FGCURRENT);
