...
signed int battery_get_bat_voltage(void)
{
	...
	if (!power_proxy_get_bat_voltage(&val))
		return val / 1000; /* 1000: uv to mv */
...
signed int battery_get_bat_current(void)
{
	...
	if (!power_proxy_get_bat_current(&curr_val))
		return curr_val * 10; /* 10: unit convert to 0.1mA */
...
signed int battery_get_uisoc(void)
{
	...
	if (!power_proxy_get_ui_capacity(&soc))
		return soc;
...
signed int battery_get_bat_temperature(void)
{
	...
	if (!power_proxy_get_bat_temperature(&temperature))
		return temperature / 10; /* 10: unit convert */
