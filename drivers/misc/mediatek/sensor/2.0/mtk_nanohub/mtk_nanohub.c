...
static int mock_test_flags[ID_SENSOR_MAX];
static int mock_test_val[ID_SENSOR_MAX];
...
static int mtk_nanohub_set_data_type_to_hub(int sensor_id,
	struct custom_cmd * cmd)
{
	...
	req.req.sensorType = sensor_id;
	req.req.action = SENSOR_HUB_SET_DATA_TYPE;
	req.req.data[0] = cmd->data[3];

	err = mtk_nanohub_req_send(&req);
	if (err < 0 || sensor_id != req.rsp.sensorType ||
		req.rsp.action != SENSOR_HUB_SET_DATA_TYPE) {
...
/* get data from scp */
static int mtk_nanohub_get_data_to_hub(int sensor_id,
	int32_t *arr, int size)
{
	int err;
	int i;
	union SCP_SENSOR_HUB_DATA req;
	int8_t *point = NULL;
	/* data offest 12 again struct int8_Data */
	int offest = 12;
	int32_t *start = NULL;

	req.req.sensorType = sensor_id;
	req.req.action = SENSOR_HUB_GET_DATA;

	err = mtk_nanohub_req_send(&req);
	if (err < 0 || sensor_id != req.rsp.sensorType ||
		req.rsp.action != SENSOR_HUB_GET_DATA) {
		pr_err("%s fail, err=%d, sensor_id=%d, action=%d\n", __func__, err, req.rsp.sensorType, req.rsp.action);
		/* return error */
		return -1;
	}
	point = (int8_t *)&req.get_data_rsp.data.int8_Data;
	start = (int32_t *)(point + offest);
	for (i = 0; i < size; i++)
		arr[i] = start[i];
	return err;
}
...
int mtk_nanohub_enable_to_hub(uint8_t sensor_id, int enabledisable)
{
	...
	if (atomic_read(&power_status) == SENSOR_POWER_UP) {
		...
		if (ret < 0)
			pr_err("fail enable: [%d,%d,%d]\n", sensor_id, cmd.cmd, ret);
	}
	if (!enabledisable)
...
void mtk_nanohub_power_up_loop(void *data)
{
	...
	mutex_unlock(&sensor_state_mtx);
#ifdef CONFIG_HUAWEI_SENSORS_2_0
	if (honor_pick_up_status()) {
		honor_motion_cmd_internal();
		pr_notice("restore pick up\n");
	}
#endif
		...
		int sensor_type, struct custom_cmd *cust_cmd)
{
	...
	if (cust_action == CUST_CMD_CALI) {
	...
	} else if (cust_action == CUST_CMD_SET_DATA_TYPE) {
		return mtk_nanohub_set_data_type_to_hub(type_to_id(sensor_type), cust_cmd);
	} else if (cust_action == CUST_CMD_GET_DATA) {
		int need_size = 3;
		...
		return mtk_nanohub_cmd_control_to_hub(type_to_id(sensor_type),
			cust_cmd);
	} else {
...
static void sensor_mock_report_to_manager(struct data_unit_t *data,
	struct hf_manager_event *event)
{
	if (data->sensor_type >= ID_SENSOR_MAX || data->sensor_type < 0)
		return;
	if (mock_test_flags[data->sensor_type] == 1)
		event->word[0] = mock_test_val[data->sensor_type];
}
...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		...
		case ID_LIGHT:
			...
			if (count_als_log > LOG_SIZE) {
				pr_info("als lux = %d, rawdata1 = %d, rawdata2 = %d\n",
					event.word[0], event.word[1], event.word[2]);
				count_als_log = 0;
			}
			count_als_log++;
			sensor_mock_report_to_manager(data, &event);
			break;
		case ID_PROXIMITY:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
...
static ssize_t single_sensor_amock_test_store(struct device_driver *ddri,
	const char *buf, size_t count)
{
	int id = 0;
	int mock_test_flag = 0;
	int single_mock_val = 0;

	if (!buf)
		goto err_out;
	if (sscanf(buf, "%d,%d,%d", &id, &mock_test_flag, &single_mock_val) != 3) {
		pr_err("invalid content: '%s', length = %zu\n", buf, count);
		goto err_out;
	}
	...
	mock_test_flags[id] = mock_test_flag;
	mock_test_val[id] = single_mock_val;
	pr_info("%s id = %d, test_flag = %d, mock_val = %d\n", __func__,
		id, mock_test_flag, single_mock_val);
err_out:
	return count;
}
...
static DRIVER_ATTR_WO(single_sensor_amock_test);

static struct driver_attribute *mtk_nanohub_attrs[] = {
	&driver_attr_trace,
	&driver_attr_state,
	&driver_attr_single_sensor_amock_test,
};
