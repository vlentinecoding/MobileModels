...
char default_client_proc_comm[] = "vendor.huawei.h";
static bool light_key;
		...
		struct hf_manager_event *event)
{
	...
	if (unlikely(hf_fifo->buffull == true)) {
		if (event->sensor_type == SENSOR_TYPE_LIGHT)
			pr_info("%s hf_fifo->buffull is true enter\n", __func__);
		...
		if (hang_time >= max_hang_time) {
			...
			hf_fifo->tail = 0;
			if (event->sensor_type == SENSOR_TYPE_LIGHT)
				pr_info("%s hang_time >= max_hang_time enter\n", __func__);
		...
		struct hf_manager_event *event)
{
	...
	if (unlikely(event->sensor_type >= SENSOR_TYPE_SENSOR_MAX)) {
		pr_err("Report failed, %u exceed max\n",
			event->sensor_type);
		...
		struct hf_manager_event *event)
{
	...
	if (event->sensor_type == SENSOR_TYPE_LIGHT)
		pr_info("%s enter event->action = %d\n", __func__, event->action);
	switch (event->action) {
	case DATA_ACTION:
		if ((event->sensor_type == SENSOR_TYPE_LIGHT) || (event->sensor_type == SENSOR_TYPE_PROXIMITY))
			pr_info("%s enter enable = %d timestamp = %lld start_time = %lld, sensor_type = %d, proc_comm = %s\n",
				__func__, request->enable, event->timestamp,
				request->start_time, event->sensor_type, client->proc_comm);
		...
		else if (event->sensor_type == SENSOR_TYPE_LIGHT &&
			!strncmp(client->proc_comm, default_client_proc_comm, sizeof(default_client_proc_comm))) {
			if (!light_key) {
				light_key = true;
				err = hf_manager_report_event(client, event);
				break;
		...
		struct hf_manager_event *event)
{
	...
	list_for_each_entry(client, &core->client_list, list) {
		...
		if (err != 0)
			pr_info("%s hf_manager_distinguish_event fail err = %d\n", __func__, err);
	}
...
#if !defined(CONFIG_XHUB_ROUTER)
extern void hf_client_set_mag_status(uint8_t action);
#endif
		...
		struct hf_manager_cmd *cmd)
{
	...
	case HF_MANAGER_SENSOR_DISABLE:
		...
		if (err < 0)
			...
			hf_client_set_mag_status(cmd->action);
#endif
		break;
	...
	case HF_MANAGER_SENSOR_ENABLE_CALI:
		...
		if (!err)
			client->request[sensor_type].cali = true;
		break;
	...
	case HF_MANAGER_SENSOR_SELFTEST:
		...
		if (!err)
			client->request[sensor_type].test = true;
		break;
		...
		char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (count != 0 && count < sizeof(struct hf_manager_event)) {
		pr_info("%s fail, count = %d, event_size = %d\n",
			__func__, (unsigned int)count, sizeof(struct hf_manager_event));
		return -EINVAL;
	}

	for (;;) {
		if (hf_fifo->head == hf_fifo->tail) {
			pr_info("%s fifo head = tail fail\n", __func__);
			return 0;
		}
		if (count == 0) {
			pr_info("%s break count = 0\n", __func__);
			break;
		}
		while (read + sizeof(event) <= count &&
			...
			read += sizeof(event);
			if (event.sensor_type == SENSOR_TYPE_LIGHT)
				pr_info("%s copy_to_user success\n", __func__);
		}
		if (read) {
			if (event.sensor_type == SENSOR_TYPE_LIGHT)
				pr_info("%s read = %d\n", __func__, (unsigned int)read);
		}
			break;
	}
	return read;
