...
#define AF_DRVNAME                 "LC898219AF_DRV"
#define AF_I2C_SLAVE_ADDR          (0xE4 >> 1)

#define LC898219_CHIP_ID           0xA5
#define LC898219_CHIP_ID_REG       0xF0
/* download */
#define LC898219_DWN_REG           0xE0
#define LC898219_DWN_START         0x1
#define LC898219_DWN_WAKE_UP       0x0
#define LC898219_DWN_WAKE_UP_REG   0xB3
#define LC898219_DWN_WAKE_UP_MASK  0xE0
#define LC898219_DWN_WAIT_MS       0x1
#define LC898219_DWN_TRY_TIMES     100
/* set code */
#define LC898219_CODE_MAX          1023
#define LC898219_CODE_MIN          0
#define LC898219_CODE_H_REG        0x84
#define LC898219_CODE_L_REG        0x85
/* hall */
#define LC898219_HALL_REG             0x0A
#define LC898219_HALL_OPEN_LOOP       0x8000
#define LC898219_HALL_INVAL           0
#define LC898219_HALL_CLOSE_LOOP_MASK 0x3FFF
#define LC898219_HALL_CLOSE_LOOP_MAX  0x3FF0
#define LC898219_HALL_CODE_MASK       0x03FF

/* scaling function */
#define SCALING_FUNCTION_OFF          0x0
#define SCALING_FUNCTION_MASK         0x3
#define LC898219_SCALING_FUNCTION_REG 0x81
#define LC898219_I2C_STATUS_REG       0xF0


/* vcm standby mode */
#define LC898219_MODE_SET_REG          0x96
#define LC898219_STANDBY_SET_VAL       0x80
#define LC898219_STANDBY_READ_REG      0x81
#define LC898219_STANDBY_READ_VAL      0x80
#define LC898219_WAKEUP_SET_VAL        0x1
#define LC898219_INIT_DOWNLOAD_REG     0xE0
#define LC898219_INIT_DOWNLOAD_VAL     0x1
#define LC898219_WAKEUP_SUCC_REG       0xB3
#define LC898219_WAKEUP_SUCC_VAL       0xE0
#define VCM_INIT_DELAY                 0x6
#define VCM_READ_DELAY                 0x1
#define VCM_COUNT_TRY_TIMES            20
#define LC898219_CODE_MID              650
#define LC898219_POSITIVE_INTERVAL     150
#define LC898219_NEGATIVE_INTERVAL     0xFF69
#define LC898219_SET_POS_REG_DELAY     2
#define LC898219_REAL_CODE_TRY_TIMES   10
#define LC898219_COMPARE_REG           0x0C
...
static int init_af(void)
{
	...
	/* Driver IC check */
	loge_if_ret(af_read_reg(LC898219_CHIP_ID_REG, &data));
	if (data != LC898219_CHIP_ID) {
		log_err("lc898219 check error data = 0x%x", data);
		return -1;
	}

	/* Init data download */
	if (af_write_reg(LC898219_DWN_REG, LC898219_DWN_START) != 0) {
	...
	do {
		mdelay(LC898219_DWN_WAIT_MS);
		loge_if_ret(af_read_reg(LC898219_DWN_WAKE_UP_REG, &data));
	} while ((data & LC898219_DWN_WAKE_UP_MASK) != LC898219_DWN_WAKE_UP &&
		cnt++ < LC898219_DWN_TRY_TIMES);

	if (cnt > LC898219_DWN_TRY_TIMES) {
		log_err("lc898219 download failed");
		return -1;
	}
	 ...
	 */
	loge_if_ret(af_read_reg(LC898219_SCALING_FUNCTION_REG, &scaling_func));
	...
	if ((scaling_func & SCALING_FUNCTION_MASK) == SCALING_FUNCTION_OFF) {
		...
		if (af_write_reg(LC898219_SCALING_FUNCTION_REG, scaling_func) != 0) {
			lens_hiview_handle(DSM_CAMERA_ACTUATOR_INIT_FAIL,
				g_ic_name, g_module_name, VCM_WRITE_I2C_FAIL);
			log_err("i2c send fail\n");
			return -1;
...
static inline int move_af(unsigned long position)
{
	...
	if (af_write_reg_16bit_data(LC898219_CODE_H_REG, dest_code) != 0) {
...
long lc898219af_ioctl(struct file *a_file, unsigned int a_command,
...
int lc898219af_release(struct inode *a_inode, struct file *a_file)
{
	log_info("Start\n");
...
int lc898219af_set_i2c_client(struct i2c_client *af_i2c_client,
...
int lc898219af_get_file_name(unsigned char *file_name)
{
