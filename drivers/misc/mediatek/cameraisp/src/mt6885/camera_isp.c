...
void isp_hiview_handle(int error_no, unsigned int err_status, unsigned int csi_index)
{
	struct mipi_err_info_t mipi_err_info;

	if (is_reach_max_report_num(csi_index))
		return;

	if (csi_index >= array_size(HwP1Done_cnt) ||
		csi_index >= array_size(g1stSof)) {
		LOG_NOTICE("csi_index:%u is invalid\n", csi_index);
		return;
	}

	mipi_err_info.err_status = err_status;
	mipi_err_info.err_mask = TG_GBERR_ST;
	mipi_err_info.frame_cnt = HwP1Done_cnt[csi_index];
	mipi_err_info.is_stream_on = g1stSof[csi_index];

	isp_hiview_report(error_no, csi_index, &mipi_err_info);
}
	...
	if (ErrStatus) {
		...
		case ISP_IRQ_TYPE_INT_CAM_A_ST:
			...
			if (ErrStatus & TG_GBERR_ST)
				isp_hiview_handle(DSM_CAMERA_SOF_ERR, ErrStatus, CSIA);
			break;
		case ISP_IRQ_TYPE_INT_CAM_B_ST:
			...
			if (ErrStatus & TG_GBERR_ST)
				isp_hiview_handle(DSM_CAMERA_SOF_ERR, ErrStatus, CSIB);
			break;
		case ISP_IRQ_TYPE_INT_CAM_C_ST:
			...
			if (ErrStatus & TG_GBERR_ST)
				isp_hiview_handle(DSM_CAMERA_SOF_ERR, ErrStatus, CSIC);
			break;
		case ISP_IRQ_TYPE_INT_CAMSV_0_ST:
