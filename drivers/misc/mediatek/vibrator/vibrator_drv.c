...
#ifdef CONFIG_HUAWEI_DSM
	/* report dmd when vibr ocp happened */
	ret = power_dsm_dmd_report(POWER_DSM_VIBRATOR_IRQ,
		DSM_PMIC_VIBRATOR_IRQ_OCP_NO, "VIBR OC happened");
	if (ret)
		pr_err("%s, report vibr oc dmd fail\n", __func__);
#endif
}
