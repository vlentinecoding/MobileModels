...
static struct timeval temp_time;
extern int thp_spi_irq_flag;
#endif
				...
				struct spi_transfer *xfer)
{
	...
	if (mas->use_fixed_timeout)
		...
		xfer_timeout =
			500 * msecs_to_jiffies(DIV_ROUND_UP(xfer->len * 8,
...
#ifndef CONFIG_HONOR_GKI_V1
static void get_timestamp_spi(struct timeval *tv)
{
...
static int spi_geni_suspend(struct device *dev)
{
	...
	dev_err(geni_mas->dev, "spi_geni_suspend usage_count = %d\n",
		atomic_read(&geni_mas->dev->power.usage_count));
	if (!pm_runtime_status_suspended(dev)) {
		GENI_SE_ERR(geni_mas->ipc, true, dev,
			":%s: runtime PM is active\n", __func__);
#ifndef CONFIG_HONOR_GKI_V1
		if (thp_spi_irq_flag == 0) {
			if (count == 0) {
				get_timestamp_spi(&start_time);
				dev_err(geni_mas->dev,"start time = %ld s sum = %d", start_time.tv_sec, sum);
				count++;
			} else {
				get_timestamp_spi(&temp_time);
				dev_err(geni_mas->dev,"start time = %ld s, temp time = %ld s sum = %d",
					start_time.tv_sec, temp_time.tv_sec, sum);
				if ((start_time.tv_sec != 0) && (temp_time.tv_sec - start_time.tv_sec) >= 58 &&
					(temp_time.tv_sec - start_time.tv_sec) <= 62) {
						start_time = temp_time;
						sum++;
				} else {
					dev_err(geni_mas->dev, "delta time is error");
					sum = 0;
					start_time.tv_sec = 0;
					count = 0;
					thp_spi_irq_flag = 0;
		...
		if (sum >= 2) {
			dev_err(geni_mas->dev,"problem occure");
