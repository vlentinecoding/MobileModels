...
#define BATTERY_NAME_STR  "bk_battery" /* honor custom */
#else
#define BATTERY_NAME_STR  "battery"
#endif

#ifdef CONFIG_LOG_EXCEPTION
#define HIVIEW_MAX_CONTENT_LEN 256
static DEFINE_MUTEX(g_flash_hiview_lock);
struct flash_hievent_info {
	u32 error_no;
...
#ifdef CONFIG_LOG_EXCEPTION
	u8              open_short_mask;
	struct work_struct flash_hiview_work;
	struct flash_hievent_info flash_err_info;
#endif
};
...
#ifdef CONFIG_LOG_EXCEPTION
static void qti_flash_hiview_handle(u32 error_no, const char *error_info, struct flash_hievent_info *flash_info)
{
	errno_t ret;
	if (!flash_info) {
		pr_err("flash_info is null\n");
		return;
	}

	ret = memset_s(flash_info->content, sizeof(flash_info->content), 0, sizeof(flash_info->content));
	if (ret != EOK) {
		pr_err("flash_err_info memset_s fail, ret = %d\n", ret);
		return;
	}

	flash_info->error_no = error_no;
	if(error_info) {
		if(strncpy_s(flash_info->content, sizeof(flash_info->content),
			error_info, sizeof(flash_info->content) - 1) < 0)
			pr_err("strncpy_s fail\n");
	}
}

static void qti_flash_hiview_report(struct flash_hievent_info *flash_info)
{
	...
	mutex_lock(&g_flash_hiview_lock);
	hi_event = hiview_hievent_create(flash_info->error_no);
	if (!hi_event) {
		pr_err("create hievent fail");
		mutex_unlock(&g_flash_hiview_lock);
		return;
	}

	hiview_hievent_put_string(hi_event, "CONTENT", flash_info->content);

	if (hiview_hievent_report(hi_event) <= 0) {
		pr_err("flash hiview report failed");
	...
	pr_info("flash hiview report succ");
}

static void qti_flash_hiview_work(struct work_struct *work)
{
	struct qti_flash_led *led =
		container_of(work, struct qti_flash_led, flash_hiview_work);
	if (!led) {
		pr_err("led is null\n");
		return;
	}
	qti_flash_hiview_report(&(led->flash_err_info));
}
#endif
...
#ifdef CONFIG_LOG_EXCEPTION
	if (rc < 0) {
		if (snprintf_s(error_info, sizeof(error_info),
			sizeof(error_info) - 1, "qti_flash_led_write, Failed to write to 0x%04X rc = %d\n",
			(led->base + offset), rc) < 0)
			pr_err("error_info snprintf_s fail");
		qti_flash_hiview_handle(DSM_FLASH_I2C_ERROR_NO, error_info, &(led->flash_err_info));
		schedule_work(&led->flash_hiview_work);
	}
#endif
					...
					u16 offset, u8 mask, u8 data)
{
	...
	if (rc < 0) {
		if (snprintf_s(error_info, sizeof(error_info),
			sizeof(error_info) - 1, "qti_flash_led_masked_write, Failed to update bits from 0x%04X, rc = %d\n",
			(led->base + offset), rc) < 0)
			pr_err("error_info snprintf_s fail");
		qti_flash_hiview_handle(DSM_FLASH_I2C_ERROR_NO, error_info, &(led->flash_err_info));
		schedule_work(&led->flash_hiview_work);
			...
			int *max_current_ma)
{
	...
	rc = qti_battery_charger_get_prop(BATTERY_NAME_STR, BATTERY_RESISTANCE,
		&rbatt_uohm);
	if (rc < 0) {
		pr_err("Failed to get %s resistance, rc=%d\n",
			BATTERY_NAME_STR, rc);
		return rc;
	}
	...
	if (!led->batt_psy)
		led->batt_psy = power_supply_get_by_name(BATTERY_NAME_STR);
...
#ifdef CONFIG_LOG_EXCEPTION
	if (led_status1 & led->open_short_mask) {
		if (snprintf_s(error_info, sizeof(error_info),
			sizeof(error_info) - 1, "LED channel open/short fault detected led_status1 = 0x%02x\n", led_status1) < 0)
			pr_err("error_info snprintf_s fail");
		pr_err("LED channel open/short fault detected led_status1 = 0x%02x\n", led_status1);
		qti_flash_hiview_handle(DSM_FLASH_OPEN_SHOTR_ERROR_NO, error_info, &(led->flash_err_info));
		schedule_work(&led->flash_hiview_work);
	}
	if (led_status2 & FLASH_LED_VPH_PWR_LOW) {
		if (snprintf_s(error_info, sizeof(error_info),
			sizeof(error_info) - 1, "too low vphone voltage detected led_status2 = 0x%02x\n", led_status2) < 0)
			pr_err("error_info snprintf_s fail");
		pr_err("too low vphone voltage detected led_status2 = 0x%02x\n", led_status2);
		qti_flash_hiview_handle(DSM_FLASH_UNDER_VOLTAGE_LOCKOUT_ERROR_NO, error_info, &(led->flash_err_info));
		schedule_work(&led->flash_hiview_work);
	}
#endif
...
#ifdef CONFIG_LOG_EXCEPTION
	rc = of_property_read_u8(node, "open-short-mask", &mask);
	if (rc < 0)
		pr_err("Failed to find open-short-mask in node %s, rc = %d\n",
			node->full_name, rc);
	led->open_short_mask = mask;
	pr_info("open_short_mask 0x%x\n", led->open_short_mask);
#endif
	return 0;
...
#ifdef CONFIG_LOG_EXCEPTION
	INIT_WORK(&led->flash_hiview_work, qti_flash_hiview_work);
#endif
...
static int qti_flash_led_remove(struct platform_device *pdev)
{
	...
	for (i = 0; (i < led->num_fnodes); i++)
		led_classdev_flash_unregister(&led->fnode[i].fdev);
#ifdef CONFIG_LOG_EXCEPTION
	cancel_work_sync(&led->flash_hiview_work);
#endif
	return 0;
}
