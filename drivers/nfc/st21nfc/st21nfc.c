...
#define ST_NFC_DMD_PROBE_TIMER (10 * HZ)  /* delay to probe dmd */
...
static struct timer_list st_nfc_dmd_probe_timer;
static nfc_aidInfo_t g_activeAidVal;
...
#ifdef CONFIG_HUAWEI_DSM
static struct dsm_dev st_dsm_nfc = {
	.name = "st_dsm_nfc",
	.device_name = NULL,
	.ic_name = NULL,
	.module_name = NULL,
...
static int g_nfc_qcom_activated;
...
static void st_nfc_record_dmd_probe_timerhandler(struct timer_list *t)
{
	del_timer(&st_nfc_dmd_probe_timer);

	if (get_nfc_dmd_probe_flag() == 0)
		st_nfc_record_dmd_info(NFC_DMD_I2C_READ_ERROR_NO, "ST21NFC-Error-in-Probe-Error");
}
...
void st_set_nfc_qcom_activated(void)
{
	int ret = -1;

	ret = nfc_get_dts_config_u32("st_nfc_qcom_activated", "st_nfc_qcom_activated", &g_nfc_qcom_activated);
	if (ret != 0) {
		g_nfc_qcom_activated = 1;
...
{
	int ret = -1;
	ret = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE - 1, sizeof(g_nfc_qcom_activated) + 1, "%d\n",
	    (unsigned char)g_nfc_qcom_activated);
	if (ret < 0)
...
static int32_t compare_active_cmd(const char* send_buf, size_t len, size_t count)
{
	...
	if (count <= HEADER_LEN + CMD_LEN || len < count || (send_buf[HEADER_LEN + 1] & 0xF0) != active_cmd[0])
		return active_ret;

	// compare 0x8xF00101xx4F
	active_ret = memcmp(&send_buf[HEADER_LEN + 2], &active_cmd[1], HEADER_LEN - 1) // compare 0xF00101
		|| memcmp(&send_buf[HEADER_LEN + CMD_LEN], &active_cmd[CMD_LEN - 1], 1); // compare 0x4F
				 ...
				 size_t count, loff_t *offset)
{
	...
	for (j = 0; j < send_times; j++) {
		...
		if (is_active_cmd) {
			...
			mutex_lock(&st21nfc_dev->platform_data.read_mutex);
			if (j == 1 && g_activeAidVal.type == 0
				&& data_buf[HEADER_LEN + DATA_LENGTH * j + INDEX_ACTIVE_FLAG] == 0x4F) {
				pr_info("%s: active flag no set, change 4F->FF\n", __func__);
				data_buf[HEADER_LEN + DATA_LENGTH * j + INDEX_ACTIVE_FLAG] = 0xFF;
			}
			mutex_unlock(&st21nfc_dev->platform_data.read_mutex);
	...
	if (is_active_cmd) {
		mutex_lock(&st21nfc_dev->platform_data.read_mutex);
		g_activeAidVal.type = 0;
		pr_info("%s : reset active flag\n", __func__);
		mutex_unlock(&st21nfc_dev->platform_data.read_mutex);
	}
	return ret;
}
			      ...
			      unsigned long arg)
{
	...
	case ST21NFC_SET_RESET_LOW:
		...
		gpio_set_value(st21nfc_dev->platform_data.reset_gpio, 0);
		break;
	case ST21NFC_SET_ACTIVE_FLAG:
		pr_info("%s set active aid flag\n", __func__);
		mutex_lock(&st21nfc_dev->platform_data.read_mutex);
		if(copy_from_user(&g_activeAidVal, (struct nfc_aidInfo_t *)arg, sizeof(struct nfc_aidInfo))) {
			ret = - EFAULT;
			hwlog_err("%s : ST21NFC_SET_ACTIVE_FLAG copy_from_user fail\n", __func__);
			mutex_unlock(&st21nfc_dev->platform_data.read_mutex);
			break;
		}
		pr_info("%s : type = %d\n", __func__, g_activeAidVal.type);
		mutex_unlock(&st21nfc_dev->platform_data.read_mutex);
		break;
	default:
...
static int nfc_parse_dt(struct device *dev, struct st21nfc_platform_data *pdata)
{
	...
	np = of_find_compatible_node(NULL, NULL, "qcom,st21nfc");
...
//
...
#ifdef CONFIG_HUAWEI_DSM
	if (!nfc_dclient)
		nfc_dclient = dsm_register_client(&st_dsm_nfc);
#endif
	...
	st_set_nfc_card_num();
	st_set_nfc_qcom_activated();
	st_set_nfc_nxp_config_name();
	st_set_nfc_ese_num();
	if ((st_set_nfc_spi_bus() == 0) && (st_set_nfc_clkreq_gpio() == 0))
		write_conf_to_tee();

    /* set probe flag: st21nfc success */
    set_nfc_dmd_probe_flag(NFC_ST21NFC_PROBE_SUCCESS);
...
err_free_buffer:
	kfree(st21nfc_dev);
err_exit:
	/* set probe flag: st21nfc failed */
	set_nfc_dmd_probe_flag(NFC_ST21NFC_PROBE_FAIL);
	timer_setup(&st_nfc_dmd_probe_timer, st_nfc_record_dmd_probe_timerhandler, 0);
	st_nfc_dmd_probe_timer.expires = jiffies + ST_NFC_DMD_PROBE_TIMER;
	add_timer(&st_nfc_dmd_probe_timer);
...
static struct of_device_id st21nfc_of_match[] = {
	{ .compatible = "qcom,st21nfc", },  //it's same as the compatible of nfc in dts.
	{}
};
