...
#define ST21NFC_SET_ACTIVE_FLAG       _IO(ST21NFC_MAGIC, 0x0c)
...
typedef struct nfc_aidInfo {
    uint32_t type; // type == 1 enable or type == 0 disable aid active
    uint32_t aidLen; // aid length
    uint8_t  cardAid[MAX_UID_AID_LEN]; // aid string to be active
} nfc_aidInfo_t;
