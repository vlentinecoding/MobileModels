...
static void print_hex_string(int count, char *buf, int len)
{
	int ret = 0;
	int i;
	...
	char *print_str = NULL;
	if (count > MAX_BUFFER_SIZE || count <= 0 ||
		count > len || buf == NULL) {
...
#else
    memset(print_str, 0x0, buffer_len);
#endif
	for (i = 0; i < count; i++) {
		...
		ret = snprintf_s(&print_str[i * 2], buffer_len - i * 2, 2,
			"%02X", buf[i]);
#else
		ret = snprintf(&print_str[i * 2], 2, "%02X", buf[i]);
#endif
		if (ret < 0) {
			pr_err("%s:snprintf_s fail, i=%d.\n", __func__, i);
			break;
		}
	}
	if (ret > 0)
		...
		size_t count, loff_t *offset)
{
	...
	for(retry = 0; retry < NFC_TRY_NUM; retry++) {
		...
		pr_err("%s : retry = %d, ret = %d, expect count = %3d\n",
			__func__, retry, ret, (int)send_length);
		if(ret == (int)send_length) {
			print_hex_string(send_length, send_buf, send_length);
			isSuccess = true;
			break;
		} else {
			if (retry > 0)
...
{
	// this code is not used on qcom, just for mtk
	/*
		...
		const struct i2c_device_id *id)
{
	...
	nfc_dev_type = get_dev_name_type();
	pr_info("%s get_dev_name_type = %d.\n", __func__, nfc_dev_type);
	if (nfc_dev_type == 1)
...
int nfc_i2c_dev_suspend(struct device *dev)
{
    struct i2c_client *i2c_client_dev = container_of(dev, struct i2c_client, dev);
    struct pn547_dev *pn547_dev = i2c_get_clientdata(i2c_client_dev);

    pn547_enable_irq(pn547_dev);
    pn547_enable_irq_wake(pn547_dev);
    pr_debug("%s pn547_enable_irq_wake success.\n", __func__);
    return 0;
}

int nfc_i2c_dev_resume(struct device *dev)
{
    struct i2c_client *i2c_client_dev = container_of(dev, struct i2c_client, dev);
    struct pn547_dev *pn547_dev = i2c_get_clientdata(i2c_client_dev);
    ...
    pn547_disable_irq_wake(pn547_dev);
    pn547_disable_irq(pn547_dev);
    pr_debug("%s pn547_disable_irq_wake success.\n", __func__);
    return 0;
}
