...
static int mt_i2c_do_transfer(struct mt_i2c *i2c)
{
	...
	if (i2c->irq_stat & (I2C_HS_NACKERR | I2C_ACKERR |
		...
		if (i2c->ext_data.isEnable ==  false ||
			...
			pr_info_ratelimited("addr:0x%x,ext_data skip more log\n",
				i2c->addr);

		if ((i2c->irq_stat & (I2C_HS_NACKERR | I2C_ACKERR)))
			pr_info_ratelimited("addr:0x%x,ACK error\n", i2c->addr);
		...
		if ((i2c->irq_stat & I2C_TRANSAC_COMP) && i2c->ch_offset &&
			...
			pr_info_ratelimited("trans done with error");
			return -EREMOTEIO;
		}
...
{
	int ret = -1;
	u32 secure_reg = I2C3_SECURE_DEFAULT_REG;
	...
	ret = of_property_read_u32(np, "secure_reg", &secure_reg);
	if (ret != 0)
		secure_reg = I2C3_SECURE_DEFAULT_REG;

	pr_info("[i2c] i2c secure mode is %u\n", i2c->secure_mode);

	if (i2c->secure_mode == 1) {
		set_secure_slave_addr(0, secure_reg);
	}
	return 0;
}
