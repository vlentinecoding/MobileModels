...
static int process_get_ion_size(const struct task_struct *task)
{
	...
	for (fdt = files_fdtable(files); n < fdt->max_fds; n++) {
		...
		struct file *f = rcu_dereference_check_fdtable(files, fdt->fd[n]);
...
static struct proc_meminfo *process_info_add(const struct task_struct *task, int tsize)
{
...
static int process_mem_show(struct seq_file *m, void *v)
{
	...
	for_each_process(p) {
		...
		task_size = get_mm_rss(task->mm) + get_mm_counter(task->mm, MM_SWAPENTS);
		...
		list_for_each_entry(temp_info, &dump_class, node_task) {
			...
			if (task_size > temp_info->task_size) {
				dump_info = process_info_add(task, task_size);
				if (dump_info)
					list_add_tail(&dump_info->node_task, &temp_info->node_task);
				break;
				...
				dump_info = process_info_add(task, task_size);
				if (dump_info)
					list_add(&dump_info->node_task, &temp_info->node_task);
				break;
			}
		}
t_unlock:
...
static int __init process_mem_procfs_init(void)
{
	...
	process_cache = kmem_cache_create("pinfo_cache", sizeof(struct proc_meminfo), 0, 0, NULL);
	if (!process_cache)
		return -ENOMEM;
