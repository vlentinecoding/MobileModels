...
static int set_prop_input_current_max(struct huawei_battery_info *info, int curr)
{
	...
	return power_vote_set(USB_ICL_VOTE_OBJECT, ICL_USER_VOTER, true, curr);
}
...
static void charger_handle_event(struct huawei_battery_info *info, enum charger_event_type event)
{
	switch (event) {
	case START_SINK:
...
static int charger_event_check(enum charger_event_type new_event)
{
	...
	if (info->event == CHARGER_MAX_EVENT)
		return 1;

	switch (new_event) {
	case START_SINK:
		if (info->event == STOP_SINK)
			ret = 1;
		break;
	case STOP_SINK:
		if (info->event == START_SINK)
			ret = 1;
		break;
	default:
		pr_err("%s: error event:%d\n", __func__, new_event);
...
static void charger_event_work(struct work_struct *work)
{
	...
	while (!charger_event_queue_isempty(&info->event_queue)) {
		spin_lock_irqsave(&(info->event_spin_lock), flags);
		event = charger_event_dequeue(&info->event_queue);
		spin_unlock_irqrestore(&(info->event_spin_lock), flags);

		charger_handle_event(info, event);
	}

	spin_lock_irqsave(&(info->event_spin_lock), flags);
	charger_event_queue_clear_overlay(&info->event_queue);
	spin_unlock_irqrestore(&(info->event_spin_lock), flags);
	pr_info("%s-\n", __func__);
}

void charger_sink_event(enum charger_event_type event)
{
	unsigned long flags;
	struct huawei_battery_info *info = g_info;
	...
	if (!charger_event_check(event)) {
		pr_info("last event: [%s], event [%s] was rejected\n",
			charger_event_type_string(info->event),
	...
	if (!charger_event_enqueue(&info->event_queue, event)) {
		if (!queue_work(system_power_efficient_wq, &info->event_work))
			pr_err("schedule event_work wait:%d\n", event);
		power_wakeup_lock(info->chg_ws, false);
	} else {
		pr_err("%s: can't enqueue event:%d\n", __func__, event);
	}

	if (event == STOP_SINK)
		charger_event_queue_set_overlay(&info->event_queue);
...
static int huawei_battery_probe(struct platform_device *pdev)
{
	...
	if (charger_event_queue_create(&info->event_queue, MAX_EVENT_COUNT))
		goto INIT_STATUS_FAIL;

	spin_lock_init(&info->event_spin_lock);
	INIT_WORK(&info->event_work, charger_event_work);
...
PREP_PROP_FAIL:
	charger_event_queue_destroy(&info->event_queue);
INIT_STATUS_FAIL:
PARSE_DT_FAIL:
