...
static int huawei_charger_set_rt_current(struct charge_device_info *di, int val)
{
	...
	data[GLINK_DATA_ZERO] = HLOS_AGGREGATOR_RT;
	...
	hwlog_info("[%s]:set iin_rt_curr %d\n", __func__, iin_rt_curr);
	return 0;
}
...
static int huawei_charger_set_iin_thermal_limit(unsigned int value)
{
	...
	data[GLINK_DATA_ZERO] = HLOS_AGGREGATOR_THERMAL;
	data[GLINK_DATA_ONE] = AGGREGATOR_VOTE_ENABLE;
...
static int huawei_charger_set_ichg_thermal_limit(unsigned int value)
{
...
int huawei_charger_set_ichg_jeita_limit(unsigned int value)
{
	...
	data[GLINK_DATA_ZERO] = HLOS_AGGREGATOR_JEITA;
	data[GLINK_DATA_ONE] = AGGREGATOR_VOTE_ENABLE;
	data[GLINK_DATA_TWO] = value;
...
static int huawei_charger_set_fcp_input_current(unsigned int value)
{
	u32 id = POWER_GLINK_PROP_ID_SET_USB_ICL;
	u32 data[GLINK_DATA_THREE];

	data[GLINK_DATA_ZERO] = HLOS_AGGREGATOR_FCP_ICL;
	data[GLINK_DATA_ONE] = AGGREGATOR_VOTE_ENABLE;
	data[GLINK_DATA_TWO] = value;
...
{
	int fcp_pd_support_icl = power_glink_support_fcp_pd_icl();

	if (fcp_pd_support_icl == 1) {
		huawei_charger_set_fcp_input_current(value);
...
static void update_iin_thermal(struct charge_device_info *di)
{
	...
	hwlog_info("update iin_thermal current: %d, type: %d",
...
static int huawei_charger_set_adaptor_change(struct charge_device_info *di, int val)
{
	...
	hwlog_info("[%s]:set adpator volt %d\n", __func__, val);
	return 0;
}
					...
					const char *buf, size_t count)
{
	...
	case CHARGE_SYSFS_IIN_THERMAL:
		...
		if (power_supply_check_psy_available("huawei_charge", &di->chg_psy)) {
			...
			hwlog_info("THERMAL set input current = %ld\n", val);
		}
		break;
	...
	case CHARGE_SYSFS_IIN_RUNNINGTEST:
		...
		hwlog_info("THERMAL set running test iin_rt = %d\n", di->sysfs_data.iin_rt);
		break;
	/* running test charging/discharge function */
	case CHARGE_SYSFS_IIN_RT_CURRENT:
		if ((kstrtol(buf, INPUT_NUMBER_BASE, &val) < 0) ||
		...
		hwlog_info("THERMAL set rt test iin_rt = %d\n", di->sysfs_data.iin_rt_curr);
		break;
	...
	case CHARGE_SYSFS_ENABLE_CHARGER:
		...
		if (power_supply_check_psy_available("battery", &di->batt_psy)) {
			power_supply_set_int_property_value_with_psy(di->batt_psy,
				POWER_SUPPLY_PROP_CHARGING_ENABLED, !!val);
			di->chrg_config = val;
			hwlog_info("ENABLE_CHARGER set chrg_config = %d\n", di->chrg_config);
		}
		break;
#endif
	case CHARGE_SYSFS_WATCHDOG_DISABLE:
...
static int dcp_set_enable_charger(unsigned int val)
{
	...
	hwlog_info("%s usb_in=%d, wls_in=%d\n", __func__, usb_in, wls_in);
	if (usb_in || wls_in) {
		if (val)
	...
	hwlog_info("ENABLE_CHARGER set %d\n", g_di->chrg_config);
	return 0;
}
#else
static int dcp_set_enable_charger(unsigned int val)
{
	...
	if (charge_support_thirdparty_buck()) {
		buck_sysfs_set_dcp_enable_charger(val);
		g_di->chrg_config = val;
		hwlog_info("ENABLE_CHARGER set %d\n", g_di->chrg_config);
	} else if (power_supply_check_psy_available("battery", &g_di->batt_psy)) {
...
static int dcp_set_iin_thermal(unsigned int index, unsigned int value)
{
	...
	if (charge_support_thirdparty_buck())
		return buck_sysfs_set_dcp_iin_thermal_array(index, value);
...
{
	unsigned int cur = (val > 0) ? val : DCP_IBAT_CURRENT;

	return huawei_charger_set_ichg_thermal_limit(cur);
}
#else
static int dcp_set_vterm_dec(unsigned int val)
{
