...
**********************************************************/
#if (KERNEL_VERSION(4, 14, 0) <= LINUX_VERSION_CODE)
__attribute__((weak)) void fg_dump_register(char *reg, int size)
{
	UNUSED(reg);
	UNUSED(size);
	pr_debug("%s: use weak\n", __func__);
...
__attribute__((weak)) void fg_get_register_head(char *reg, int size)
{
	UNUSED(reg);
	UNUSED(size);
	pr_debug("%s: use weak\n", __func__);
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	case FUELGUAGE_SYSFS_GAUGELOG_HEAD:
		...
		/*
		* reg_head has 2 part:1 fg reg head 2 ocv batt_type and other
		* param the leath MAX_SIZE = CHARGELOG_SIZE / 2
		*/
#if (KERNEL_VERSION(4, 14, 0) <= LINUX_VERSION_CODE)
		memset(di->sysfs_data.reg_head, 0, CHARGELOG_SIZE);
		fg_get_register_head(di->sysfs_data.reg_head, MAX_SIZE);
#elif (KERNEL_VERSION(4, 9, 0) <= LINUX_VERSION_CODE)
	...
	case FUELGUAGE_SYSFS_GAUGELOG:
		...
		/*
		* reg_value has 2 part:1 fg reg head 2 ocv batt_type and other
		* param the leath MAX_SIZE = CHARGELOG_SIZE / 2
		*/
#if (KERNEL_VERSION(4, 14, 0) <= LINUX_VERSION_CODE)
		memset(di->sysfs_data.reg_value, 0, CHARGELOG_SIZE);
		fg_dump_register(di->sysfs_data.reg_value, MAX_SIZE);
...
MODULE_DESCRIPTION("huawei fuelguage module driver");
