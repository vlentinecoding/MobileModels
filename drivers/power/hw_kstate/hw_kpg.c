...
static int pg_cb(channel_id src, packet_tag tag, const char *data, size_t len)
{
	...
	/* set wakelock */
	memset(&pg_cmd, 0, sizeof(pg_command));
	memcpy(&pg_cmd, data, len<(sizeof(pg_command)-1)?len:(sizeof(pg_command)-1));
	...
	case CANCEL_ALL_WAKELOCK_TIMEOUT:
		...
		break;
	case STOP_WAKELOCK:
		ret = wake_unlock_by_name(pg_cmd.name);
		break;
	case STOP_ALL_KERNEL_WAKELOCK:
		ret = wake_unlock_all(MAX_KERNEL_WAKELOCK_TIME);
		break;
	case FREEZED_PID: {
