...
#define WINDOW_LEN                   7
#define UI_OFFSET                    50
#define LOW_CAPACITY_CHECK_VOLT      3450
#define LOW_CAPACITY_CHECK_THRESHOLD 300
#define DAEMON_UI_SOC_FULL 10000
static int g_soc_algo_factor;
static unsigned int capacity_sum;
static unsigned int capacity_filter[WINDOW_LEN];
static bool first;
static int g_daemon_ui_soc;
static int g_low_capacity_volt_check_en;
...
static void battery_rm_lock(void)
{
	mutex_lock(&gm.bat_rm.rm_lock);
}

static void battery_rm_unlock(void)
{
	mutex_unlock(&gm.bat_rm.rm_lock);
}

static int battery_rm_get_consume(int last_coulomb, int cur_coulomb)
{
	return (cur_coulomb - last_coulomb) / UNIT_TRANS_10;
}

static void battery_rm_handle_gauge_reset(int coulomb)
{
	bm_info("mtk-battery %s rm %d, last %d, cur %d\n",
		__func__, gm.bat_rm.rm, gm.bat_rm.last_coulomb, coulomb);

	battery_rm_lock();

	gm.bat_rm.rm += battery_rm_get_consume(gm.bat_rm.last_coulomb, coulomb);
	gm.bat_rm.last_coulomb = 0;

	battery_rm_unlock();
}

static void battery_rm_handle_set_rm(int fcc, int capacity, int coulomb)
{
#ifdef CONFIG_HUAWEI_DUBAI
	static int fcc_last = -1;

	if (fcc_last == -1)
		fcc_last = fcc;
	if (fcc_last != fcc) {
		HWDUBAI_LOGE("DUBAI_TAG_BATTERY_ADJUST", "delta_soc=-1 delta_uah=%d sleep_uah=-1",
			(fcc - fcc_last) * UNIT_TRANS_100);
		fcc_last = fcc;
	}
#endif
	bm_info("mtk-battery %s fcc %d rm %d capacity %d coulomb %d\n",
		__func__, fcc, gm.bat_rm.rm, capacity, coulomb);

	battery_rm_lock();

	// update rm
	gm.bat_rm.rm = fcc * capacity / UNIT_TRANS_100000;
	gm.bat_rm.last_coulomb = coulomb;

	battery_rm_unlock();
}

int battery_get_capacity(struct battery_data *data)
{
	int capacity;
	if (gm.fixed_uisoc != 0xffff)
		capacity = gm.fixed_uisoc;
	else
		capacity = data->BAT_CAPACITY;
	return capacity;
}

int battery_get_rm(struct battery_data *data)
{
	int capacity;
	int coulomb;
	...
	coulomb = gauge_get_coulomb();

	bm_debug("mtk-battery %s rm %d, last %d, cur %d\n",
		__func__, gm.bat_rm.rm, gm.bat_rm.last_coulomb, coulomb);

	if (gm.bat_rm.rm == 0) {
		capacity = battery_get_capacity(data);
		gm.bat_rm.rm = gm.fcc * capacity / UNIT_TRANS_100000;
	} else {
		gm.bat_rm.rm += battery_rm_get_consume(gm.bat_rm.last_coulomb, coulomb);
	}
...
int gauge_reset_hw(void)
{
	...
	battery_rm_handle_gauge_reset(gauge_get_coulomb());
...
void fg_custom_init_from_dts(struct platform_device *dev)
{
	...
	fg_read_dts_val(np, "CAR_TUNE_VALUE", &(fg_cust_data.car_tune_value),
		UNIT_TRANS_10);
	gm.car_tune_default_value = fg_cust_data.car_tune_value;
	...
	fg_read_dts_val(np, "low_capacity_volt_check_en",
		&g_low_capacity_volt_check_en, 1);
	bm_err("g_low_capacity_volt_check_en is %d\n", g_low_capacity_volt_check_en);
	fg_read_dts_val(np, "soc_algo_factor", &g_soc_algo_factor, 1);
	bm_info("g_soc_algo_factor is %d\n", g_soc_algo_factor);
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_KERNEL_UISOC:
	{
		...
		if (g_soc_algo_factor) {
			/* soc = soc + 1.5*(1-soc/100), soc unit 1%, daemon_ui_soc unit 0.01% */
			daemon_ui_soc += g_soc_algo_factor * (DAEMON_UI_SOC_FULL - daemon_ui_soc) / 1000;
			if (daemon_ui_soc > DAEMON_UI_SOC_FULL)
				daemon_ui_soc = DAEMON_UI_SOC_FULL;
		}
		...
		if (first == false) {
			...
			if (g_low_capacity_volt_check_en && (daemon_ui_soc < LOW_CAPACITY_CHECK_THRESHOLD) &&
				(battery_get_bat_voltage() >= LOW_CAPACITY_CHECK_VOLT)) {
				daemon_ui_soc = LOW_CAPACITY_CHECK_THRESHOLD;
				bm_err("low_capacity_volt_check set daemon_ui_soc %d\n", daemon_ui_soc);
			}
	...
	case FG_DAEMON_CMD_SET_BATTERY_CAPACITY:
	{
		...
		battery_rm_handle_set_rm(param.data[4], param.data[6], gauge_get_coulomb());
	}
	break;
	default:
