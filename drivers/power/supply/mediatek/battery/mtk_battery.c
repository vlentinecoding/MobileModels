...
/* add for meta tool----------------------------------------- */
#define COUL_DEFAULT_VAL                1000000
#define COUL_TBATICAL_MIN_A             800000
#define COUL_TBATICAL_MAX_A             1300000
	...
	union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		val->intval = battery_get_capacity(data);
	...
	case POWER_SUPPLY_PROP_CAPACITY_RM:
		val->intval = battery_get_rm(data);
		break;
	default:
		ret = -EINVAL;
		break;
...
static int mt6359_set_current_gain(unsigned int cur_a, void *dev_data)
{
	int val;

	val = gm.car_tune_default_value;
	fg_cust_data.car_tune_value = ((s64)val * cur_a) / COUL_DEFAULT_VAL;

	return 0;
}

static struct coul_cali_ops mt6359_cali_ops = {
	.dev_name = "main",
	.set_current_gain = mt6359_set_current_gain,
};

static void battery_update_car_tune(void)
{
	int val = 0;

	if (coul_cali_get_para(COUL_CALI_MODE_MAIN, COUL_CALI_PARA_CUR_A, &val))
		return;

	if ((val >= COUL_TBATICAL_MIN_A) && (val <= COUL_TBATICAL_MAX_A))
		mt6359_set_current_gain(val, NULL);
}
...
/*****************************/
	...
	/* update car_tune_value */
	coul_cali_ops_register(&mt6359_cali_ops);
	battery_update_car_tune();
	/* Power supply class */
#if !defined(CONFIG_MTK_DISABLE_GAUGE)
	battery_main.psy =
