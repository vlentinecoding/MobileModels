	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		return 0;
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		last_chg_type = mtk_chg->chg_type;
	...
	if ((mtk_chg->chg_type == WIRELESS_CHARGER) ||
		((last_chg_type == WIRELESS_CHARGER) &&
	...
	if (!cti->ignore_usb) {
		...
		if ((mtk_chg->chg_type == STANDARD_HOST) ||
			...
			mt_usb_connect();
		else if (!wl_ignore_usb)
			mt_usb_disconnect();
		else
			pr_info("%s: ignore in wireless situation\n", __func__);
	}
...
static bool is_charger_plug_in(struct tcp_ny_typec_state typec_state)
{
	...
	if (typec_state.old_state == TYPEC_UNATTACHED &&
		...
		if (g_ignore_vbus_only_event) {
			pr_info("%s: ignore ATTACHED_NORP_SRC\n", __func__);
			return false;
		}
		return true;
	}
...
static bool is_charger_plug_out(struct tcp_ny_typec_state typec_state)
{
	if ((typec_state.old_state == TYPEC_ATTACHED_SNK ||
		typec_state.old_state == TYPEC_ATTACHED_CUSTOM_SRC) &&
		typec_state.new_state == TYPEC_UNATTACHED)
		return true;

	if (typec_state.old_state == TYPEC_ATTACHED_NORP_SRC &&
		typec_state.new_state == TYPEC_UNATTACHED) {
		if (g_ignore_vbus_only_event) {
			pr_info("%s: ignore UNATTACHED\n", __func__);
			return false;
		}
		return true;
	}
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (is_charger_plug_in(noti->typec_state)) {
		...
		} else if (is_charger_plug_out(noti->typec_state)) {
			if (cti->tcpc_kpoc) {
