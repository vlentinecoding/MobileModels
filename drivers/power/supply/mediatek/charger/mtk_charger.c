...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	/* set CV and iterm after temperature changed */
	/* STANDARD_HOST use another cv iterm para */
	/* In normal range, we adjust CV and iterm dynamically */
	if (info->chr_type == STANDARD_HOST)
		is_sdp = true;

	if (sw_jeita->sm == TEMP_ABOVE_T4) {
		sw_jeita->cv = is_sdp ? info->data.sdp_jeita_temp_above_t4_cv :
			info->data.jeita_temp_above_t4_cv;
		sw_jeita->iterm = 0;
	} else if (sw_jeita->sm == TEMP_T3_TO_T4) {
		sw_jeita->cv = is_sdp ? info->data.sdp_jeita_temp_t3_to_t4_cv :
			info->data.jeita_temp_t3_to_t4_cv;
		sw_jeita->iterm = is_sdp ? info->data.sdp_jeita_temp_t3_to_t4_iterm :
			info->data.jeita_temp_t3_to_t4_iterm;
	} else if (sw_jeita->sm == TEMP_T2_TO_T3) {
		sw_jeita->cv = 0;
		sw_jeita->iterm = is_sdp ? info->data.sdp_jeita_temp_t2_to_t3_iterm :
			info->data.jeita_temp_t2_to_t3_iterm;
	} else if (sw_jeita->sm == TEMP_T1_TO_T2) {
		sw_jeita->cv = is_sdp ? info->data.sdp_jeita_temp_t1_to_t2_cv :
			info->data.jeita_temp_t1_to_t2_cv;
		sw_jeita->iterm = is_sdp ? info->data.sdp_jeita_temp_t1_to_t2_iterm :
			info->data.jeita_temp_t1_to_t2_iterm;
	} else if (sw_jeita->sm == TEMP_T0_TO_T1) {
		sw_jeita->cv = is_sdp ? info->data.sdp_jeita_temp_t0_to_t1_cv :
			info->data.jeita_temp_t0_to_t1_cv;
		sw_jeita->iterm = is_sdp ? info->data.sdp_jeita_temp_t0_to_t1_iterm :
			info->data.jeita_temp_t0_to_t1_iterm;
	} else if (sw_jeita->sm == TEMP_BELOW_T0) {
		sw_jeita->cv = is_sdp ? info->data.sdp_jeita_temp_below_t0_cv :
			info->data.jeita_temp_below_t0_cv;
		sw_jeita->iterm = 0;
	} else {
	...
	chr_err("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d iterm:%d\n",
		sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
		sw_jeita->cv, sw_jeita->iterm);
}
...
static bool mtk_is_charger_on(struct charger_manager *info)
{
	...
	if (chr_type == CHARGER_UNKNOWN) {
		...
		if (info->cable_out_cnt > 0) {
			info->clear_cable_out_cnt_status = true;
			mtk_charger_plug_out(info);
			mtk_charger_plug_in(info, chr_type);
			mutex_lock(&info->cable_out_lock);
			info->cable_out_cnt--;
			mutex_unlock(&info->cable_out_lock);
			info->clear_cable_out_cnt_status = false;
		}
	}
	...
	if (count >= 3) {
		if (boot_mode == META_BOOT || boot_mode == ADVMETA_BOOT ||
			...
			chr_info("boot_mode = %d, bypass battery check\n",
				boot_mode);
		else if (!info->data.ignore_batt_check) {
...
static int charger_routine_thread(void *arg)
{
	...
	while (1) {
		...
		(void)dc_set_ic_buck_enable(SC_MODE, CHARGE_MULTI_IC, 0);
...
static void mtk_charger_usb_jeita_data_dt(struct device_node *np,
	struct charger_manager *info)
{
	u32 val;

	if (of_property_read_u32(np, "sdp_battery_cv", &val) >= 0)
		info->data.sdp_battery_cv = val;
	else {
		chr_err("use default sdp BATTERY_CV:%d\n", BATTERY_CV);
		info->data.sdp_battery_cv = info->data.battery_cv;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_above_t4_cv", &val) >= 0) {
		chr_info("set sdp_jeita_temp_above_t4_cv:%d\n", val);
		info->data.sdp_jeita_temp_above_t4_cv = val;
	} else {
		chr_err("not config sdp_jeita_temp_above_t4_cv, set to default");
		info->data.sdp_jeita_temp_above_t4_cv = info->data.jeita_temp_above_t4_cv;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t3_to_t4_cv", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t3_to_t4_cv:%d\n", val);
		info->data.sdp_jeita_temp_t3_to_t4_cv = val;
	} else {
		chr_err("not config sdp_jeita_temp_t3_to_t4_cv, set to default");
		info->data.sdp_jeita_temp_t3_to_t4_cv = info->data.jeita_temp_t3_to_t4_cv;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t2_to_t3_cv", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t2_to_t3_cv:%d\n", val);
		info->data.sdp_jeita_temp_t2_to_t3_cv = val;
	} else {
		chr_err("not config sdp_jeita_temp_t2_to_t3_cv, set to default");
		info->data.sdp_jeita_temp_t2_to_t3_cv = info->data.jeita_temp_t2_to_t3_cv;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t1_to_t2_cv", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t1_to_t2_cv:%d\n", val);
		info->data.sdp_jeita_temp_t1_to_t2_cv = val;
	} else {
		chr_err("not config sdp_jeita_temp_t1_to_t2_cv, set to default");
		info->data.sdp_jeita_temp_t1_to_t2_cv = info->data.jeita_temp_t1_to_t2_cv;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t0_to_t1_cv", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t0_to_t1_cv:%d\n", val);
		info->data.sdp_jeita_temp_t0_to_t1_cv = val;
	} else {
		chr_err("not config sdp_jeita_temp_t0_to_t1_cv, set to default");
		info->data.sdp_jeita_temp_t0_to_t1_cv = info->data.jeita_temp_t0_to_t1_cv;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_below_t0_cv", &val) >= 0) {
		chr_info("set sdp_jeita_temp_below_t0_cv:%d\n", val);
		info->data.sdp_jeita_temp_below_t0_cv = val;
	} else {
		chr_err("not config sdp_jeita_temp_below_t0_cv, set to default");
		info->data.sdp_jeita_temp_below_t0_cv = info->data.jeita_temp_below_t0_cv;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t0_to_t1_iterm", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t0_to_t1_iterm:%d\n", val);
		info->data.sdp_jeita_temp_t0_to_t1_iterm = val;
	} else {
		chr_err("not config sdp_jeita_temp_t0_to_t1_iterm, set to default");
		info->data.sdp_jeita_temp_t0_to_t1_iterm = info->data.jeita_temp_t0_to_t1_iterm;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t1_to_t2_iterm", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t1_to_t2_iterm:%d\n", val);
		info->data.sdp_jeita_temp_t1_to_t2_iterm = val;
	} else {
		chr_err("not config sdp_jeita_temp_t1_to_t2_iterm, set to default");
		info->data.sdp_jeita_temp_t1_to_t2_iterm = info->data.jeita_temp_t1_to_t2_iterm;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t2_to_t3_iterm", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t2_to_t3_iterm:%d\n", val);
		info->data.sdp_jeita_temp_t2_to_t3_iterm = val;
	} else {
		chr_err("not config sdp_jeita_temp_t2_to_t3_iterm, set to default");
		info->data.sdp_jeita_temp_t2_to_t3_iterm = info->data.jeita_temp_t2_to_t3_iterm;
	}

	if (of_property_read_u32(np, "sdp_jeita_temp_t3_to_t4_iterm", &val) >= 0) {
		chr_info("set sdp_jeita_temp_t3_to_t4_iterm:%d\n", val);
		info->data.sdp_jeita_temp_t3_to_t4_iterm = val;
	} else {
		chr_err("not config sdp_jeita_temp_t3_to_t4_iterm, set to default");
		info->data.sdp_jeita_temp_t3_to_t4_iterm = info->data.jeita_temp_t3_to_t4_iterm;
	}
}
				...
				struct device *dev)
{
	...
	/* for wireless charge */
	info->data.wireless_charger_input_current = WIRELESS_CHARGER_INPUT_CURRENT;
	info->data.wireless_charger_current = WIRELESS_CHARGER_CURRENT;
	...
	if (of_property_read_u32(np, "jeita_temp_t0_to_t1_iterm", &val) >= 0) {
		chr_info("set jeita_temp_t0_to_t1_iterm:%d\n", val);
		info->data.jeita_temp_t0_to_t1_iterm = val;
	} else {
		chr_err("not config jeita_temp_t0_to_t1_iterm, set to 0");
		info->data.jeita_temp_t0_to_t1_iterm = 0;
	}

	if (of_property_read_u32(np, "jeita_temp_t1_to_t2_iterm", &val) >= 0) {
		chr_info("set jeita_temp_t1_to_t2_iterm:%d\n", val);
		info->data.jeita_temp_t1_to_t2_iterm = val;
	} else {
		chr_err("not config jeita_temp_t1_to_t2_iterm, set to 0");
		info->data.jeita_temp_t1_to_t2_iterm = 0;
	}

	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_iterm", &val) >= 0) {
		chr_info("set jeita_temp_t2_to_t3_iterm:%d\n", val);
		info->data.jeita_temp_t2_to_t3_iterm = val;
	} else {
		chr_err("not config jeita_temp_t2_to_t3_iterm, set to 0");
		info->data.jeita_temp_t2_to_t3_iterm = 0;
	}

	if (of_property_read_u32(np, "jeita_temp_t3_to_t4_iterm", &val) >= 0) {
		chr_info("set jeita_temp_t3_to_t4_iterm:%d\n", val);
		info->data.jeita_temp_t3_to_t4_iterm = val;
	} else {
		chr_err("not config jeita_temp_t3_to_t4_iterm, set to 0");
		info->data.jeita_temp_t3_to_t4_iterm = 0;
	}
	mtk_charger_usb_jeita_data_dt(np, info);
#ifdef CONFIG_HLTHERM_RUNTEST
	...
	if (of_property_read_u32(np, "ignore_batt_check", &val) >= 0)
		info->data.ignore_batt_check = val;
