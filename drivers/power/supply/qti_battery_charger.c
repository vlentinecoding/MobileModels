...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
#define DPM_SBU_DMD_INTERVAL	(24 * 60 * 60) /* s */
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	[USB_INPUT_CURRENT_NOW] = POWER_SUPPLY_PROP_INPUT_CURRENT_NOW,
	[USB_TYPEC_CC_ORIENTATION] = POWER_SUPPLY_PROP_TYPEC_CC_ORIENTATION,
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
static struct timespec64 ts64_dmd_last;
#endif /* CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION */
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	/* online get usbswitch result finally */
	if (prop == POWER_SUPPLY_PROP_ONLINE) {
		pval->intval = bcdev->usb_online;
		return 0;
	}
	if (prop == POWER_SUPPLY_PROP_REAL_TYPE) {
		pr_err("charge_type = %d\n", bcdev->charge_type);
		pval->intval = bcdev->charge_type;
		return 0;
	}
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if (prop == POWER_SUPPLY_PROP_REAL_TYPE) {
		bcdev->charge_type = pval->intval;
		pr_err("set charge_type = %d\n", bcdev->charge_type);
		return 0;
	}
#endif
					...
					struct class_attribute *attr, char *buf)
{
	struct battery_chg_dev *bcdev = container_of(c, struct battery_chg_dev,
						battery_class);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	char dsm_buf[POWER_DSM_BUF_SIZE_0256] = { 0 };
	...
	if (pst->prop[USB_MOISTURE_DET_STS]) {
		...
		ts64_dmd_interval.tv_sec = DPM_SBU_DMD_INTERVAL;
		ts64_dmd_interval.tv_nsec = 0;
		ts64_dmd_sum = timespec64_add_safe(ts64_dmd_last, ts64_dmd_interval);
		if (ts64_dmd_sum.tv_sec == TIME_T_MAX) {
			pr_info("time overflow happend when add 24 hours\n");
		} else if (timespec64_compare(&ts64_dmd_sum, &ts64_dmd_now) < 0) {
			snprintf(dsm_buf, POWER_DSM_BUF_SIZE_0256 - 1,
				"sbu water detect trigger vbus_status %d\n", bcdev->usb_online);
			power_dsm_dmd_report(POWER_DSM_BATTERY, ERROR_NO_WATER_CHECK_IN_USB, dsm_buf);
			ktime_get_coarse_real_ts64(&ts64_dmd_last);
		}
	}
