...
#define HIZ_CURRENT (100)
#define IIN_CURR_MAX (2000)
void charge_set_hiz_enable(int hiz_mode)
{
    ...
    if (hiz_mode == 1) {
        save_iin_curr = get_propety_int(g_di->batt_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_MAX);
    ...
    rc = set_propety_int(g_di->batt_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_MAX, &val);
    if (rc < 0)
...
static int get_usb_temp_value(void)
{
    ...
    int sum = 0;
    int temp = 0;
    static int usb_temp = NORMAL_TEMP;
    ...
    for (i = 0; i < sample_num; ++i)
    {
        ...
        if (adc_temp >= 0)
        {
            if(filter_mode)//true:use filter mode to skip abnormal voltage jump
            {
                if(adc_temp > ERROR_TUSB_THR)
                {
                    usb_temp = adc_temp;
                }
                else
                {
                    pr_info("[uscp]:tusb more 125 degree adc_temp = %d usb_temp = %d\n",adc_temp,usb_temp);
                    adc_temp = usb_temp;
                }
            }
            sum += adc_temp;
	...
	if ((tusb >= di->uscp_threshold_tusb) && (tdiff >= di->open_mosfet_temp)) {
		if (protect_dmd_notify_enable) {
			ret = power_dsm_dmd_report(POWER_DSM_USCP,
				...
				"usb short happened,tusb = %d,tbatt = %d\n", tusb, tbatt);
			if (ret)
...
static void check_ntc_error(void)
{
	...
	if (temp > TUSB_TEMP_UPPER_LIMIT || temp < TUSB_TEMP_LOWER_LIMIT) {
		...
		ret = power_dsm_dmd_report(POWER_DSM_USCP, ERROR_NO_USB_SHORT_PROTECT_NTC,
			"ntc error happened\n");
		if (ret)
			pr_info("ntc error report dmd fail\n");
...
static int uscp_probe(struct platform_device *pdev)
{
    ...
    if (!huawei_adc_init_done(di->np_node)) {
        dev_err(di->dev, "honor adc not init yet, defer\n");
        ret = -EPROBE_DEFER;
        goto free_mem;
    }
    ...
    filter_mode = of_property_read_bool(np,"usb_vol_filter_mode");
    pr_info("filter_mode = %d\n", filter_mode);
	...
	wake_lock_init(&uscp_wakelock, WAKE_LOCK_SUSPEND,
