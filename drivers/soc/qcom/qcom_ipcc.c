...
#define IPC_LOG_PAGE_CNT 100
#define debug_client_id 0
...
static irqreturn_t qcom_ipcc_irq_fn(int irq, void *data)
{
	...
	for (;;) {
		...
		IPCC_INFO(ipcc_ipclog, "IRQ for client_id: %u; signal_id: %u; virq: %d\n", qcom_ipcc_get_client_id(packed_id), qcom_ipcc_get_signal_id(packed_id), virq);
...
static void qcom_ipcc_mask_irq(struct irq_data *irqd)
{
	...
	IPCC_INFO(ipcc_ipclog, "%s: Disabling interrupts for: client_id: %u; signal_id: %u\n", __func__, sender_client_id, sender_signal_id);
...
static void qcom_ipcc_unmask_irq(struct irq_data *irqd)
{
	...
	dev_dbg(proto_data->dev,
		...
		__func__, sender_client_id, sender_signal_id);
	IPCC_INFO(ipcc_ipclog, "%s: Enabling interrupts for: client_id: %u; signal_id: %u\n", __func__, sender_client_id, sender_signal_id);
...
static int qcom_ipcc_mbox_send_data(struct mbox_chan *chan, void *data)
{
	...
	if (ipcc_mbox_chan->client_id==debug_client_id) {
		IPCC_INFO(ipcc_ipclog, "%s: Generating IRQ for client_id: %u; signal_id: %u\n", __func__, ipcc_mbox_chan->client_id, ipcc_mbox_chan->signal_id);
	}
...
static int qcom_ipcc_probe(struct platform_device *pdev)
{
	...
	ipcc_ipclog = ipc_log_context_create(IPC_LOG_PAGE_CNT, name, 0);
	return 0;
