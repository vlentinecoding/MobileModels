				...
				const char *name)
{
	...
	return scnprintf(prvbuf, length, "%s\n\tVersion:0x%x\n"
			...
			accumulated_duration, is_sleep);
}

static void honor_msm_rpmh_master_stats_print_data(struct msm_rpmh_master_stats *record,
				const char *name)
{
	uint64_t accumulated_duration = record->accumulated_duration;
	uint8_t is_sleep = 0;
...
void honor_msm_rpmh_master_stats_show(void)
{
	int i = 0;
	struct msm_rpmh_master_stats *record = NULL;
	bool skip_apss = false;
	...
	if (rpmh_unit_base) {
		honor_msm_rpmh_master_stats_print_data(&apss_master_stats,
			"APSS");
		skip_apss = true;
	}
	/* Read SMEM data written by other masters */
	...
	for (i = 0; i < ARRAY_SIZE(rpmh_masters); i++) {
		...
		if (!IS_ERR_OR_NULL(record))
			honor_msm_rpmh_master_stats_print_data(
				record,
				rpmh_masters[i].master_name);
	}
...
EXPORT_SYMBOL(honor_msm_rpmh_master_stats_show);
