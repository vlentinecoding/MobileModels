	...
	struct fsa4480_priv *i2c_priv)
{
	...
	ret = request_threaded_irq(i2c_priv->handler->irq, NULL,
		handler_func, i2c_priv->handler->irq_flag | IRQF_ONESHOT,
		"fsa4480_i2c_irq", NULL);
	if (ret < 0) {
		dev_err(i2c_priv->dev, "fsa4480_i2c_irq request fail, ret = %d\n", ret);
		goto err_out;
	} else {
		ret = enable_irq_wake(i2c_priv->handler->irq);
		if (ret)
			dev_err(i2c_priv->dev, "failed to wake interrupt on irq %d, ret = %d\n",
				i2c_priv->handler->irq, ret);
	}
