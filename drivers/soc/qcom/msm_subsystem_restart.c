...
#ifdef CONFIG_RAINBOW_REASON
#ifdef CONFIG_PANIC_ON_SSR_NOTIF_TIMEOUT
        rb_sreason_set("%s_%s_ttl", timeout_data->source_name, timeout_data->dest_name);
#endif
#endif
...
#ifdef CONFIG_RAINBOW_REASON
#ifdef CONFIG_PANIC_ON_SSR_NOTIF_TIMEOUT
        rb_sreason_set("chain_%s_ttl", timeout_data->source_name);
#endif
#endif
...
#ifdef CONFIG_RAINBOW_REASON
#ifdef CONFIG_PANIC_ON_SSR_NOTIF_TIMEOUT
        rb_sreason_set("shutdown_%s_ttl", timeout_data->dest_name);
#endif
#endif
...
static void device_restart_work_hdlr(struct work_struct *work)
{
	...
	msleep(100);
#ifdef CONFIG_RAINBOW_REASON
	rb_sreason_set("subsys_%s_crashed", dev->desc->name);
#endif
