...
#ifndef CONFIG_HUAWEI_DUBAI_DDR_STATS
struct stats_entry {
	uint32_t name;
	uint32_t count;
	uint64_t duration;
...
#ifdef CONFIG_HUAWEI_DUBAI_DDR_STATS
struct stats_entry *g_stats_entry = NULL;
static struct ddr_stats_platform_data *g_pdata = NULL;
#endif
		...
		struct stats_entry *data, u64 accumulated_duration)
{
	...
	if (name == 0x0) {
		...
		*count = *count + 1;
		...
		name = data->name >> 16;
		/* changed by liudong_00017231
		 * return all support freqs, no matter is it used. */
#ifdef CONFIG_HUAWEI_DUBAI_DDR_STATS
...
#ifdef CONFIG_HUAWEI_DUBAI_DDR_STATS
int honor_ddr_freq_cnt_get(void)
{
	return ddr_stats_get_freq_count();
}
EXPORT_SYMBOL(honor_ddr_freq_cnt_get);

int honor_ddr_time_in_freq_get(void)
{
	struct ddr_freq_residency data[MAX_NUM_MODES];
	int ret = 0;
	int i = 0;
	int freq_cnt = honor_ddr_freq_cnt_get();

	ret = ddr_stats_get_residency(freq_cnt, data);
	if (ret != freq_cnt) {
		pr_err("honor_err: get residency failed, ret=%d, freq_cnt=%d\n", ret, freq_cnt);
		return ret;
	}

	if (!g_stats_entry) {
		g_stats_entry = kzalloc(freq_cnt * sizeof(struct stats_entry), GFP_KERNEL);
		if (!g_stats_entry) {
			pr_err("honor_error: failed to allocate mem\n");
			return -ENOMEM;
		}
	}

	for (i = 0; i < freq_cnt; i++) {
		g_stats_entry[i].name = data[i].freq;
		g_stats_entry[i].duration = data[i].residency;
	}

	return freq_cnt;
}
EXPORT_SYMBOL(honor_ddr_time_in_freq_get);
#if 0
/*
 * The following code is a self-developed scheme to get ddr freq information.
 * As Qcom baseline upgrade newly increased it, just annotate following code.
 */
static int ddr_freq_info_to_buf(struct stats_entry *data, struct stats_entry *temp_data)
{
	u32 cp_idx = 0;
	u32 name;
	...
	data->name = (data->name >> 16);

	temp_data->name = data->name;
	temp_data->duration = data->duration;

	return 0;
}

static int ddr_freq_info_copy_stats(void __iomem *reg, u32 entry_count)
{
	struct stats_entry data[MAX_NUM_MODES];
	struct stats_entry temp_data[MAX_NUM_MODES];
	int freq_cnt, i, ret;
	...
	for (i = 0, freq_cnt = 0; i < entry_count; i++) {
		ret = ddr_freq_info_to_buf(&data[i], &temp_data[freq_cnt]);
		if (!ret)
			freq_cnt += 1;
		else
			continue;

		if (i && (data[i].name == data[i-1].name))
			freq_cnt -= 1;
	}
	...
	for (i = 0; i < freq_cnt; i++) {
		g_stats_entry[i].name = temp_data[i].name;
		g_stats_entry[i].duration = temp_data[i].duration;
		g_stats_entry[i].count = temp_data[i].count;
	}
...
int honor_ddr_freq_cnt_get(void)
{
	...
	if (!g_pdata) {
		pr_err("honor_error: get ddr info failed\n");
		return -EINVAL;
	}

	//reg = ioremap_nocache(g_pdata->phys_addr_base, g_pdata->phys_size);
	reg = ddr_pdata->reg;
	if (!reg) {
		pr_err("ERROR could not ioremap");
		return -ENOMEM;
	}
	...
	freq_cnt = ddr_freq_info_copy_stats(reg, entry_count);
