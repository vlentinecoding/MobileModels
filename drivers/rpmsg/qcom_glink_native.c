...
do {									     \
	if (ch->glink) {							 \
			...
			if (debug_flag) \
				dev_err(ch->glink->dev, "%s[%d:%d] %s: "x, ch->name,  \
					   ch->lcid, ch->rcid, __func__, ##__VA_ARGS__); \
	} \
} while (0)
...
static irqreturn_t qcom_glink_native_intr(int irq, void *data)
{
	...
	if (should_wake) {
#ifdef CONFIG_HUAWEI_DUBAI
		dubai_log_irq_wakeup(glink->irqname);
#endif
		printk(KERN_ERR "%s: pm_system_irq_wakeup: %d triggered %s\n", __func__, glink->irq, glink->irqname);
