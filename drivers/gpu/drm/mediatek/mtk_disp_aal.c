...
static atomic_t g_aal_first_frame_leave_relay = ATOMIC_INIT(0);
	 ...
	 bool check_sram)
{
	...
	AALIRQ_LOG("[g_aal_hist_available = %d", atomic_read(&g_aal_hist_available));
...
static void mtk_aal_bypass(struct mtk_ddp_comp *comp, struct cmdq_pkt *handle, bool bypass)
{
	...
	if ((atomic_read(&g_aal_force_relay) != 1) && (bypass == true)) {
		...
		atomic_set(&g_aal_first_frame_leave_relay, 1);
		atomic_set(&g_aal_first_frame, 1);
		atomic_set(&g_aal_hist_available, 0);
		atomic_set(&g_aal_eof_irq, 0);
		AALIRQ_LOG("[g_aal_first_frame = %d", atomic_read(&g_aal_first_frame));
		AALIRQ_LOG("[g_aal_hist_available = %d",
			atomic_read(&g_aal_hist_available));
		AALIRQ_LOG("[g_aal_eof_irq = %d", atomic_read(&g_aal_eof_irq));
	}
}
...
static void ddp_aal_cabc_restore(struct mtk_ddp_comp *comp)
{
	...
	int i;
#if defined(CONFIG_MACH_MT6885)
    writel(0x0, comp->regs + DISP_AAL_CABC_00);
#else
...
void disp_aal_on_end_of_frame(struct mtk_ddp_comp *comp)
{
    ...
    if (atomic_read(&g_aal_first_frame_leave_relay) == 1) {
        AALIRQ_LOG("aal_refresh_task for relay first frame");
        atomic_set(&g_aal_first_frame_leave_relay, 0);
        queue_work(aal_refresh_wq, &g_aal_data->aal_refresh_task);
        return;
    }
