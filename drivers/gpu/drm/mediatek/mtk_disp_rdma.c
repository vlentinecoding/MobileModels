...
/* >normal_width && > normal_height need update rdma golden setting*/
#define normal_width 1080
#define normal_height 2520
...
#include "helio-dvfsrc-v3/helio-dvfsrc-qos.h"
static irqreturn_t mtk_disp_rdma_irq_handler(int irq, void *dev_id)
{
	...
	if (val & (1 << 1)) {
		...
		if (mtk_crtc && is_fence_release_at_sof()) {
	...
	struct mtk_ddp_config *cfg, unsigned int *gs)
{
	...
	const unsigned int base_width = 1080;
	const unsigned int base_height = 2520;
#if defined(CONFIG_MACH_MT6885) || defined(CONFIG_MACH_MT6893)
	...
	if (width > base_width && height > base_height) {
		pre_ultra_low_us = 180;
		pre_ultra_high_us = 190;
		ultra_low_us = 160;
		ultra_high_us = 180;
	}
	...
	if (width > normal_width && height > normal_height) {
		if (if_fps == 60) {
			gs[GS_RDMA_PRE_ULTRA_TH_LOW] = 1163;
			gs[GS_RDMA_PRE_ULTRA_TH_HIGH] = 1210;
		}
		if (if_fps == 120) {
			gs[GS_RDMA_PRE_ULTRA_TH_LOW] = 1768;
			gs[GS_RDMA_PRE_ULTRA_TH_HIGH] = 1861;
		}
	}
	...
	if (width > normal_width && height > normal_height) {
		if (if_fps == 60) {
			gs[GS_RDMA_ULTRA_TH_LOW] = 1070;
		}
		if (if_fps == 120) {
			gs[GS_RDMA_ULTRA_TH_LOW] = 1582;
		}
	}
