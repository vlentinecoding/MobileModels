...
#define LIFT_MOVE_16_BIT 16
#define LIFT_MOVE_8_BIT 8

static void update_rgb_input_data(u8 *va, int width, int height, unsigned int frameIndex, unsigned int *bufferPtr)
{
	int i, j;
	int k = 0;
	int pitch = width * SIZE_RGB;
	u8 *b_data = NULL;
	u8 *g_data = NULL;
	u8 *r_data = NULL;
	struct metadata* meta_data = NULL;

	for (i = 0; i < height; i++) {
		for (j = 0; j < pitch; j = j + SIZE_RGB) {
			b_data = va + i * pitch + j;
			g_data = b_data + 1;
			r_data = b_data + 2;
			bufferPtr[k] =
				(unsigned int)((*b_data << LIFT_MOVE_16_BIT) |
				(*g_data << LIFT_MOVE_8_BIT) | (*r_data));
			k++;
		}
	}

	// fill metadata
	meta_data = (struct metadata*)(bufferPtr + height * width);
	meta_data->isValidate = 1;
	meta_data->timeStamp = (u64)ktime_get_boot_ns();
	meta_data->frameIndex = frameIndex;

	DDPDBG("[caputre] ts = %ld, ------%u %u %u. frameIndex = %d",meta_data->timeStamp,
		bufferPtr[0], bufferPtr[1], bufferPtr[2], meta_data->frameIndex);
}
#else
static void update_rgb_input_data(u8 *va, int width, int height, unsigned int frameIndex, u8 *bufferPtr)
{
	int i;
	int pitch = width * SIZE_RGB;
	struct metadata* meta_data = NULL;
	...
	DDPDBG("[caputre] ts = %ld, ------%d %d %d. frameIndex = %d", meta_data->timeStamp,
		bufferPtr[0], bufferPtr[1], bufferPtr[2], meta_data->frameIndex);
}
#endif
			  ...
			  struct capture_info *buffer)
{
	...
	DDPMSG("[caputre] w = %d, h = %d", width, height);
#ifdef CONFIG_LCD_KIT_DRIVER
#ifdef CONFIG_MACH_MT6853
	bufferPtr = (unsigned int *)(buffer->data.image + buf_index * SHARE_MEMORY_SIZE / 2);

	// fill data
	update_rgb_input_data((u8*)va, width, height, wdma_capt_info->capture_count, bufferPtr);
#else
	bufferPtr = buffer->data.image + buf_index * SHARE_MEMORY_SIZE / 2;
	update_rgb_input_data((u8*)va, width, height, wdma_capt_info->capture_count, bufferPtr);

#endif
	DDPDBG("[caputre] buf_index:%d\n", buf_index);
	crtc_mva_unmap_kernel(addr, size, va);

	share_mem_frame_index = wdma_capt_info->capture_count;
