...
unsigned long long _layering_get_wdma_bw(struct drm_crtc *crtc,
						struct drm_display_mode *mode)
{
	static unsigned long long bw_base;
	static int fps;
					...
					struct drm_display_mode *mode)
{
	...
	/*workaround for enable wdma will lead to rdma underrun*/
	wdma_bw = _layering_get_wdma_bw(crtc, mode);
	dvfs_bw = dvfs_bw - wdma_bw;
	DDPINFO("get avail HRT BW befor:%u : after:%llu \n",
