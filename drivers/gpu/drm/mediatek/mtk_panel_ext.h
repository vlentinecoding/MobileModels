...
struct mtk_panel_params {
...
#ifdef CONFIG_LCD_KIT_DRIVER
	unsigned int esd_num;
	unsigned int read_reg_num;
	unsigned int read_reg_pos[ESD_CHECK_NUM];
#endif
	...
	unsigned int vsync_high_time;
	unsigned int hbm_high_deadline;
	unsigned int te_high_cycle;
	unsigned int te_low_cycle;
	unsigned int vsync_low_time;
	unsigned int hbm_low_deadline;
...
struct mtk_panel_funcs {
	...
	int (*early_prepare)(struct drm_panel *panel);
};
