			     ...
			     struct drm_atomic_state *state, bool async)
{
	...
	for (i = 0; i < MAX_CRTC; i++) {
		...
		if (mtk_crtc->wdma_capture_info && mtk_crtc->wdma_capture_info->enable)
			DDPDBG("[capture] atomic commit!!!");
		CRTC_MMP_EVENT_START(index, atomic_commit, 0, 0);
#ifdef CONFIG_LCD_KIT_DRIVER
		DDPMSG("%s, can_commit:%d\n", __func__, mtk_crtc->can_commit);
		if (!mtk_crtc->can_commit) {
			DDPMSG("%s:%d, slepted\n", __func__, __LINE__);
			CRTC_MMP_MARK(index, atomic_commit, 0, 1);
...
static bool fence_release_at_sof = true;

bool is_fence_release_at_sof(void) {
	DDPINFO("set_fence_release_at_sof:%d\n", fence_release_at_sof);
	return fence_release_at_sof;
}

void set_fence_release_at_sof(bool value) {
	fence_release_at_sof = value;
	DDPINFO("set_fence_release_at_sof:%d\n", value);
}
...
static int mtk_drm_kms_init(struct drm_device *drm)
{
	...
	/*6853  with vdo mode use legacy fence plan*/
	if (!is_fence_release_at_sof()) {
...
static const struct drm_ioctl_desc mtk_ioctls[] = {
	...
	DRM_IOCTL_DEF_DRV(HBM_MODE_SET_PARAM, panel_drm_hbm_set,
			DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(DISPLAY_ENGINE_SHAREMEM_GET, display_engine_share_mem_get,
			DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(DISPLAY_ENGINE_SHAREMEM_SET, display_engine_share_mem_set,
			DRM_UNLOCKED),
	DRM_IOCTL_DEF_DRV(DISPLAY_ENGINE_SHAREMEM_STATUS_GET, display_engine_share_mem_status_get,
			DRM_UNLOCKED),
#endif
#ifdef CONFIG_MTK_HDMI_SUPPORT
...
#endif
	pdata = kzalloc(sizeof(struct mtk_fb_data_type), GFP_KERNEL);

	((struct mtk_fb_data_type *)pdata)->panel_power_on = 1;
#ifdef CONFIG_TEE_TUI
	register_tui_driver(notify_dss_tui_request, "DSS", pdata);
#endif
	DDPINFO("%s-\n", __func__);
...
static int _mtk_drm_ion_create_client(void *data)
{
	struct mtk_drm_private *private;

	private = (struct mtk_drm_private *)data;
	if (private)
...
static int mtk_drm_probe(struct platform_device *pdev)
{
	...
	if (private->data->bypass_infra_ddr_control) {
		...
		if (infra_node == NULL) {
			...
			infra_mem = platform_get_resource(infra_pdev,
							IORESOURCE_MEM, 0);
			private->infra_regs_pa = infra_mem->start;
			private->infra_regs = devm_ioremap_resource(
							infra_dev, infra_mem);
			if (IS_ERR(private->infra_regs))
				DDPPR_ERR("%s: infra of_iomap err\n",
					__func__);
			else
				DDPMSG("%s, infra va:0x%p, pa:0x%lx\n",
					__func__, (void *)private->infra_regs,
					(unsigned long)private->infra_regs_pa);
		}
		of_node_put(infra_node);
	}
	...
	private->init_client_thread =
		kthread_run(_mtk_drm_ion_create_client,
				private, "init_disp_ion");

	if (IS_ERR(private->init_client_thread))
		DDPPR_ERR("kthread_run init_client_thread err = %d\n",
				IS_ERR(private->init_client_thread));
#endif
