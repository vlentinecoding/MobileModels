			   ...
			   struct drm_device *drm_dev)
{
	int disp_idx;
	bool no_disp = true;
	unsigned int rsz_num[HRT_LEVEL_NUM] = {0};
	...
	for (disp_idx = 0; disp_idx < HRT_TYPE_NUM; disp_idx++) {
		...
		if (disp_info->layer_num[disp_idx] <= 0)
			continue;
		/* clean RSZ flag if layer in GLE range */
		for (i = 0; i < disp_info->layer_num[disp_idx]; i++) {
			c = &disp_info->input_config[disp_idx][i];
			if (mtk_has_layer_cap(c, MTK_DISP_RSZ_LAYER) &&
				mtk_is_gles_layer(disp_info, disp_idx, i))
				c->layer_caps &= ~ MTK_DISP_RSZ_LAYER;
			if ((i < 2) &&
				(mtk_has_layer_cap(c, MTK_DISP_RSZ_LAYER)))
				rsz_num[disp_idx]++;
		}
		if (rsz_num[disp_idx] == 1)
			l_rule_info->addon_scn[disp_idx] = ONE_SCALING;
		else if (rsz_num[disp_idx] == 2)
			l_rule_info->addon_scn[disp_idx] = TWO_SCALING;
