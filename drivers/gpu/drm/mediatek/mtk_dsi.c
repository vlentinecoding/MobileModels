...
static void _mtk_mipi_dsi_write_gce(struct mtk_dsi *dsi,
	struct cmdq_pkt *handle, const struct mipi_dsi_msg *msg);
#endif
...
static void mtk_dsi_dual_enable(struct mtk_dsi *dsi, bool enable)
{
	...
	writel((temp & ~DSI_DUAL_EN) | (enable ? DSI_DUAL_EN : 0), dsi->regs + DSI_CON_CTRL);
}
...
static int mtk_dsi_poweron(struct mtk_dsi *dsi)
{
	...
	if (dsi->ext) {
		if (dsi->ext->params->is_cphy)
			mtk_mipi_tx_cphy_lane_config(dsi->phy, dsi->ext, !!dsi->slave_dsi);
		else
			mtk_mipi_tx_dphy_lane_config(dsi->phy, dsi->ext, !!dsi->slave_dsi);
		}
	...
	mtk_dsi_mask(dsi, DSI_SHADOW_DEBUG,
				DSI_BYPASS_SHADOW | 1, DSI_BYPASS_SHADOW | 1);
...
static void mtk_dsi_ps_control_vact(struct mtk_dsi *dsi)
{
	...
	if (!dsi->is_slave) {
		...
		width = mtk_dsi_get_virtual_width(dsi, dsi->master_dsi->encoder.crtc);
		height = mtk_dsi_get_virtual_heigh(dsi, dsi->master_dsi->encoder.crtc);
	}
...
static void mtk_dsi_config_vdo_timing(struct mtk_dsi *dsi)
{
	...
	if (!dsi->is_slave)
		...
		vact = mtk_dsi_get_virtual_heigh(dsi, dsi->master_dsi->encoder.crtc);
...
static void mtk_output_en_doze_switch(struct mtk_dsi *dsi)
{
	...
	if (!IS_ERR_OR_NULL(cmdq_handle)) {
		...
		DDPMSG("%s failed to flush ddic ops\n", __func__);
	...
	if (doze_enabled && panel_funcs->doze_enable_start) {
		...
		panel_funcs->doze_enable_start(dsi->panel, dsi,
			...
			DDPINFO("%s panel_funcs->doze_enable_start end\n", __func__);
	} else if (!doze_enabled && panel_funcs->doze_disable) {
		panel_funcs->doze_disable(dsi->panel, dsi,
			mipi_dsi_dcs_write_gce2, NULL);
		DDPINFO("%s panel_funcs->doze_disable end\n", __func__);
		mtk_disp_esd_check_switch(crtc, true);
		DDPINFO("%s mtk_disp_esd_check_switch end\n", __func__);
	}
	...
	if (panel_funcs->doze_get_mode_flags) {
		...
		DDPINFO("%s mtk_dsi_clk_hs_mode end\n", __func__);
	...
	int force_lcm_update)
{
...
#if defined(CONFIG_MACH_MT6885) || defined(CONFIG_MACH_MT6853)
	...
	if (new_doze_state && !force_lcm_update) {
		if (dsi->ext && dsi->ext->params->doze_delay) {
			doze_delay = dsi->ext->params->doze_delay;
			do {
				cmdq_pkt_wfe(handle,
...
#ifdef CONFIG_LCD_KIT_DRIVER
	/* wait esd done */
	cmdq_pkt_wait_no_clear(handle, mtk_crtc->gce_obj.event[EVENT_ESD_EOF]);
#endif
	/* wait trigger loop done */
	cmdq_pkt_wait_no_clear(handle,
...
static int mtk_dsi_wait_frame_done(struct mtk_dsi *dsi)
{
	struct cmdq_pkt *cmdq_handle = NULL, *cmdq_handle6 = NULL;
	struct mtk_ddp_comp *comp = &dsi->ddp_comp;
	struct mtk_drm_crtc *mtk_crtc = comp->mtk_crtc;
	...
	cmdq_handle6 = cmdq_pkt_create(
			mtk_crtc->gce_obj.client[CLIENT_DSI_CFG]);
	if (!IS_ERR_OR_NULL(cmdq_handle6)) {
		cmdq_pkt_flush(cmdq_handle6);
		cmdq_pkt_destroy(cmdq_handle6);
	} else
		DDPPR_ERR("%s, failed to flush ddic ops\n", __func__);

	cmdq_handle = cmdq_pkt_create(
			mtk_crtc->gce_obj.client[CLIENT_CFG]);
	if (IS_ERR_OR_NULL(cmdq_handle)) {
		DDPPR_ERR("%s, failed to wait frame done\n", __func__);
		return -1;
	}
...
static void mtk_aftconfig_dsi_enable(struct mtk_dsi *dsi)
{
	if (mtk_dsi_is_cmd_mode(&dsi->ddp_comp))
		writel(0x0001023c, dsi->regs + DSI_TXRX_CTRL);
	...
	int force_lcm_update)
{
	...
	if (dsi->panel) {
		if (!dsi->doze_enabled || force_lcm_update) {
			if(ext && ext->funcs && ext->funcs->early_prepare(dsi->panel)) {
				DDPPR_ERR("failed to early_prepare the panel\n");
				return;
			}
		}
	}
	...
	if (dsi->panel) {
		...
		if ((!dsi->doze_enabled || force_lcm_update) &&
			...
			DDPPR_ERR("panel prepare fail, reset panel\n");
			drm_panel_unprepare(dsi->panel);
			udelay(10000);
			if(ext && ext->funcs && ext->funcs->early_prepare)
				ext->funcs->early_prepare(dsi->panel);
			drm_panel_prepare(dsi->panel);
			if (lcd_kit_poweric_detect())
				DDPPR_ERR("poweric twice detect failed\n");
		}
#endif
	 ...
	 * lcm initialize.
	 */
	mtk_aftconfig_dsi_enable(dsi);

	if (dsi->slave_dsi)
		mtk_aftconfig_dsi_enable(dsi->slave_dsi);
...
static void mtk_aft_config_disable(struct mtk_dsi *dsi)
{
	/* set DSI into ULPS mode */
	mtk_dsi_reset_engine(dsi);
...
{
	bool new_doze_state = mtk_dsi_doze_state(dsi);
	bool is_disable = dsi->panel && (!new_doze_state || force_lcm_update);
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(dsi->encoder.crtc);
	...
	/* 1. If VDO mode, stop it and set to CMD mode */
	if (!mtk_dsi_is_cmd_mode(&dsi->ddp_comp)) {
		if (is_disable)
			mtk_dsi_wait_frame_done(dsi);
		else
	...
	/* 2. If not doze mode, turn off backlight */
	if (is_disable) {
	...
	mtk_aft_config_disable(dsi);
	if (dsi->slave_dsi)
		mtk_aft_config_disable(dsi->slave_dsi);
...
int mtk_dsi_esd_read(struct mtk_ddp_comp *comp, void *handle, uintptr_t slot)
{
	...
	for (i = 0; i < params->esd_num; i++) {
		if (i == params->read_reg_pos[pos]) {
			t0.CONFG = 0x04;
			...
			t0.Data1 = 0;
			DDPMSG("esd read reg i = %d pos = %d\n", i, pos);
			mtk_dsi_read_gce(comp, handle, &t0, pos, slot);
			pos++;
		} else {
			tx[0] = params->lcm_esd_check_table[i].cmd;
			memcpy(&tx[1], params->lcm_esd_check_table[i].para_list,
				params->lcm_esd_check_table[i].count - 1);
			msg.channel = 0;
			msg.flags = MIPI_DSI_MSG_USE_LPM;
			if(params->lcm_esd_check_table[i].count > 2)
				msg.type = 0x39;
			else
				msg.type = 0x15;
			msg.tx_len = params->lcm_esd_check_table[i].count;
			msg.tx_buf = tx;
			DDPMSG("msg.type = %#x, len = %d i= %d\n", msg.type, msg.tx_len, i);
			for (j = 0; j < msg.tx_len; j++) {
				DDPMSG("tx[%d] = 0x%x\n", j, tx[j]);
				DDPMSG("para_list[%d] = 0x%x\n", j,
					params->lcm_esd_check_table[i].para_list[j]);
			}
			DDPMSG("lcm esd write +\n");
...
int mtk_dsi_esd_cmp(struct mtk_ddp_comp *comp, void *handle, const void *slot)
{
	...
	if (dsi->ext && dsi->ext->params)
		...
		return 0;
#ifdef CONFIG_LCD_KIT_DRIVER
	for (i = 0; i < params->read_reg_num; i++) {
#else
	for (i = 0; i < ESD_CHECK_NUM; i++) {
#endif
...
#ifdef CONFIG_LCD_KIT_DRIVER
		reg_pos = params->read_reg_pos[i];
		lcm_esd_tb = &params->lcm_esd_check_table[reg_pos];
		DDPMSG("read reg pos = %d\n", reg_pos);
#else
...
#ifdef CONFIG_LCD_KIT_DRIVER
		DDPMSG("Esd idx(%d),read(0x%x)=0x%x, expect(0x%x)\n",
			i, lcm_esd_tb->cmd, chk_val, lcm_esd_tb->para_list[0]);
#endif
...
static void mtk_dsi_dy_fps_cmdq_cb(struct cmdq_cb_data data)
{
	...
	pr_info("%s vdo mode fps change done\n", __func__);
				     ...
				     const struct mipi_dsi_msg *msg, u8 flag)
{
	...
	if (!mtk_dsi_wait_idle(dsi, flag, 2000, NULL)) {
		...
		if (dsi->slave_dsi) {
			if (!mtk_dsi_wait_idle(dsi->slave_dsi, flag, 2000, NULL)) {
				writel(0, dsi->regs + DSI_START);
				mtk_dsi_dual_enable(dsi->slave_dsi, false);
				return -ETIME;
			}
				     ...
				     const struct mipi_dsi_msg *msg)
{
	...
	if (readl(dsi->regs + DSI_MODE_CTRL) & MODE) {
		...
		if (dsi->ext->params->lcm_cmd_if == MTK_PANEL_DUAL_PORT) {
			mtk_dsi_host_send_cmd_dual_sync(dsi, msg, irq_flag);
		} else {
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_SET_BL:
	{
		...
		break;
	case DSI_GET_DOZE_ENABLE:
	{
		struct mtk_dsi *dsi =
			container_of(comp, struct mtk_dsi, ddp_comp);

		enable = (bool *)params;
		*enable = dsi->doze_enabled;
	}
		break;
	case DSI_SET_BL_GRP:
	{
	...
	{
		struct mtk_drm_crtc *crtc = comp->mtk_crtc;
		unsigned long long *hbm_to_last_te = (unsigned long long *)params;
		if (crtc->last_te_time > 0)
			*hbm_to_last_te = local_clock() - crtc->last_te_time;
		else
			*hbm_to_last_te = 0;

		DDPINFO("%s: hbm_to_last_te = %d ns\n", __func__,
			*hbm_to_last_te);
		break;
	}
...
static int mtk_dsi_probe(struct platform_device *pdev)
{
	...
	dsi->is_slave = of_property_read_bool(dev->of_node, "mediatek,dual-dsi-slave");
	...
	dsi_mode = readl(dsi->regs + DSI_MODE_CTRL) & MODE;
#if defined(CONFIG_MACH_MT6853)
	if (dsi_mode)
		set_fence_release_at_sof(false);
#endif
	DDPINFO("%s-, dsi_mode:%d\n", __func__, dsi_mode);
