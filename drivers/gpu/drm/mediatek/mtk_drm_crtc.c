		...
		int need_lock)
{
	...
	int index;
	bool doze_enabled = false;

	DDPMSG("%s:%d\n", __func__, __LINE__);
	if (!crtc) {
		DDPPR_ERR("%s:%d Error: crtc is NULL\n", __func__, __LINE__);
		return -EINVAL;
	}
	index = drm_crtc_index(crtc);
	mtk_crtc = to_mtk_crtc(crtc);
	if (!mtk_crtc) {
		DDPPR_ERR("%s:%d Error: mtk_crtc is NULL\n",
			__func__, __LINE__);
		return -EINVAL;
	}
	comp = mtk_ddp_comp_request_output(mtk_crtc);
	if (!comp) {
		DDPPR_ERR("%s:%d Error: mtk_crtc output comp is NULL\n",
			__func__, __LINE__);
		return -EINVAL;
	}
...
#ifdef CONFIG_LCD_KIT_DRIVER
	if (!(mtk_crtc->enabled && mtk_crtc->can_commit)) {
#else
	if (!(mtk_crtc->enabled)) {
#endif
	...
	if (comp->funcs && comp->funcs->io_cmd)
		comp->funcs->io_cmd(comp, NULL,
			DSI_GET_DOZE_ENABLE, &doze_enabled);
...
static int mtk_drm_crtc_hbm_delay(struct drm_crtc *crtc, bool en)
{
	...
	if (mtk_state->prop_val[CRTC_PROP_DISP_MODE_IDX]) {
		te_cycle = panel_ext->te_high_cycle;
		vsync_time = panel_ext->vsync_high_time; //2ms
		hbm_deadline = panel_ext->hbm_high_deadline;
	} else {
		te_cycle = panel_ext->te_low_cycle;
		vsync_time = panel_ext->vsync_low_time; //9ms
		hbm_deadline = panel_ext->hbm_low_deadline;
	}
	DDPINFO("%s,hbm delay te_cycle=%d us,vsync_time=%d us,hbm_deadline=%d us\n",
		 __func__, te_cycle, vsync_time, hbm_deadline);
	...
	wait_count = vsync_time - hbm_to_te;

	if (wait_count < 0 && hbm_to_te > hbm_deadline)
		wait_count = te_cycle - hbm_to_te + vsync_time;

	DDPINFO("%s, LCM hbm %s need wait %d us, hbm_to_last_te=%d us\n",
		__func__,
		en ? "enable" : "disable",
...
static int mtk_drm_crtc_set_panel_hbm(struct drm_crtc *crtc, bool en)
{
	...
	if (is_frame_mode) {
		...
		if (crtc_id == 0) {
			struct cmdq_pkt *cmdq_handle6 =
				cmdq_pkt_create(mtk_crtc->gce_obj.client[CLIENT_DSI_CFG]);

			if (IS_ERR(cmdq_handle6)) {
				cmdq_pkt_destroy(cmdq_handle);
				return -EINVAL;
			}
			cmdq_pkt_flush(cmdq_handle6);
			cmdq_pkt_destroy(cmdq_handle6);
		}

		cmdq_handle2 = cmdq_pkt_create(mtk_crtc->gce_obj.client[CLIENT_CFG]);
		mtk_crtc_wait_frame_done(mtk_crtc, cmdq_handle2, DDP_FIRST_PATH, 0);
...
void mtk_crtc_prepare_dual_pipe(struct mtk_drm_crtc *mtk_crtc)
{
	...
	if (drm_crtc_index(&mtk_crtc->base) == 0) {
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_IDLE_MGR, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_IDLEMGR_BY_REPAINT, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_IDLEMGR_DISABLE_ROUTINE_IRQ, 0);
		//mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_USE_PQ, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_ESD_CHECK_RECOVERY, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_ESD_CHECK_SWITCH, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_OVL_EXT_LAYER, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_RPO, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_OVL_WCG, 0);
		mtk_drm_helper_set_opt(priv->helper_opt, MTK_DRM_OPT_CLEAR_LAYER, 0);
	}
...
void mtk_crtc_wdma_capt_inf_set_sec(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	wdma_capt_inf = mtk_crtc_get_wdma_capt_inf(crtc);
	mtk_crtc_wdma_capt_inf_set_sec(crtc);

	if (index != 0 || mtk_crtc_is_dc_mode(crtc) ||
		!wdma_capt_inf)
		return;

	if (wdma_capt_inf->enable) {
		if (!wdma_capt_inf->is_sec)
			return;
		else
			DDPMSG("[capture] has secure content!!");
	}
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	if (!wdma_capt_inf->enable || wdma_capt_inf->is_sec)
		return;
	...
	for (i = 0; i < addon_data->module_num; i++) {
		...
		if (addon_module->type == ADDON_AFTER &&
			...
			if (wdma_capt_inf->config_count == 0) {
				DDPDBG("[capture] first config change idx:%d->%d", buf_index, (1-buf_index));
...
void mtk_crtc_set_capt_interval(struct drm_crtc *crtc, unsigned int fps)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_wdma_capture_info *capt_info = NULL;

	if (crtc->index != 0 || !mtk_crtc->wdma_capture_info)
		return;

	capt_info = mtk_crtc->wdma_capture_info;
	if (fps == 120)
		capt_info->capture_interval = 2;
	else if (fps == 60)
		capt_info->capture_interval = 1;

	DDPDBG("[capture] %s switch to fps:%d, set interval:%d\n",
		__func__, fps, capt_info->capture_interval);
}
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	/* update wdma capture interval */
	mtk_crtc_set_capt_interval(crtc, fps_dst);
...
#if defined(MTK_DRM_DELAY_PRESENT_FENCE)

	// release present fence
	if ((drm_crtc_index(crtc) != 2) &&
			(!is_fence_release_at_sof())) {
		struct cmdq_pkt_buffer *cmdq_buf = &(mtk_crtc->gce_obj.buf);
...
void mtk_crtc_start_trig_loop(struct drm_crtc *crtc)
{
	...
	if (mtk_crtc_is_frame_trigger_mode(crtc)) {
		...
		*/
		...
		/* The EVENT_TRIG_LOOP_DONE is used for wait trigger loop done
		 * before display disable
		 */
		cmdq_pkt_wfe(cmdq_handle,
			mtk_crtc->gce_obj.event[EVENT_TRIG_LOOP_DONE]);
...
#ifdef CONFIG_LCD_KIT_DRIVER
	if (!mtk_crtc->can_commit) {
		DDPPR_ERR("%s:%d Error: lcm reset, just return\n",
			__func__, __LINE__);
		return -EINVAL;
	}
#endif
		...
		bool need_lock)
{
	...
	if (comp->funcs && comp->funcs->io_cmd)
		comp->funcs->io_cmd(comp, NULL, DSI_GET_DOZE_ENABLE, &doze_enabled);

	panel_ext = mtk_crtc->panel_ext;
	if (mtk_crtc_is_frame_trigger_mode(crtc) && doze_enabled &&
...
void mtk_drm_crtc_enable(struct drm_crtc *crtc)
{
	...
	if (!crtc_id) {
		if (!mtk_crtc->need_esd_recovery) {
			mtk_crtc->qos_ctx->last_hrt_req = 0;
		} else {
		/*. restore HRT BW */
#ifdef MTK_FB_MMDVFS_SUPPORT
		DDPINFO("%s,esd recovery need restore HRT BW,last_hrt_req=%d\n",
...
void mtk_drm_crtc_disable(struct drm_crtc *crtc, bool need_wait)
{
	...
	/*Enable DISP_PQ before PQ unprepare*/
	if (crtc_id == 0)
		mtk_crtc_bypass_pq_for_aod(mtk_crtc, NULL, false);
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if (index == 0 &&
		((bool)state->prop_val[CRTC_PROP_HBM_ENABLE] == false)) {
...
#if defined(MTK_DRM_DELAY_PRESENT_FENCE) || defined(CONFIG_MACH_MT6885) || \
		defined(CONFIG_MACH_MT6893) || defined(CONFIG_MACH_MT6853)
	/* backup present fence */
	if (is_fence_release_at_sof() &&
		(state->prop_val[CRTC_PROP_PRES_FENCE_IDX] != (unsigned int)-1)) {
		struct cmdq_pkt_buffer *cmdq_buf = &(mtk_crtc->gce_obj.buf);
		dma_addr_t addr =
			cmdq_buf->pa_base +
				    ...
				    int buf_len, int event_id)
{
	...
	case EVENT_STREAM_BLOCK:
		len = snprintf(buf, buf_len, "disp_token_stream_block%d",
			       drm_crtc_index(&mtk_crtc->base));
		break;
	case EVENT_TRIG_LOOP_DONE:
		len = snprintf(buf, buf_len, "disp_token_trig_loop_done%d",
			drm_crtc_index(&mtk_crtc->base));
		break;
	case EVENT_CABC_EOF:
...
int mtk_crtc_enter_tui(struct drm_crtc *crtc)
{
	...
	DDPPR_ERR("zp start mtk_crtc_enter_tui entry 5 %p\n", mtk_crtc->state_wait_queue);
	DDP_MUTEX_UNLOCK(&mtk_crtc->blank_lock, __func__, __LINE__);
	wake_up(&mtk_crtc->state_wait_queue);

	DDPPR_ERR("mtk_crtc_enter_tui entry 6\n");
