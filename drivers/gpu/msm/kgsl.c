...
static int kgsl_close_device(struct kgsl_device *device)
{
	 ...
	 */
	device->open_count--;
	pr_err("kgsl_close_device %d,pid: %d,comm: %s, tgid: %d,task_comm: %s \n",
	            device->open_count, current->pid, current->comm, tgid, task_comm);
...
static int kgsl_open_device(struct kgsl_device *device)
{
	...
	device->open_count++;
	pr_err("kgsl_open_device %d,pid: %d,comm :%s,tgid: %d, task_comm: %s \n",
	            device->open_count,current->pid,current->comm, tgid, task_comm);
out:
	mutex_unlock(&device->mutex);
	return result;
