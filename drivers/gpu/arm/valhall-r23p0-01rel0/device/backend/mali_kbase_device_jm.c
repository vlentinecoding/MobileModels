...
int kbase_device_init(struct kbase_device *kbdev)
{
	...
	mutex_init(&(kbdev->hisi_dev_data.mmu_notifier_list_lock));
	INIT_LIST_HEAD(&(kbdev->hisi_dev_data.mmu_notifier_list));
	INIT_LIST_HEAD(&(kbdev->hisi_dev_data.mmu_notifier_free_list));
#endif
	return err;
}
