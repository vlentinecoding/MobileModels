...
static void kbasep_show_kctx_overall_native_memory(
	struct seq_file *sfile, struct kbase_context *kctx)
{
	...
	for (i = 0; i < 2 ; i++) {
		...
		for_each_rb_node(rb_root, node) {
			...
			if (reg->cpu_alloc != reg->gpu_alloc)
				compressed_pages_overall +=
					count_compressed_pages(reg->gpu_alloc);
		}
	}
	...
	struct kbase_device *kbdev, int group_id)
{
	...
	err = gmc_storage_get_page(
		kbdev->hisi_dev_data.kbase_gmc_device.storage, page, handle);
	if (err) {
				...
				struct kbase_device *kbdev)
{
	...
	handle = gmc_storage_put_page(
		kbdev->hisi_dev_data.kbase_gmc_device.storage, page);
	if (IS_ERR(handle)) {
		dma_addr_t dma_addr;
					...
					size_t nr)
{
	...
	if (kctx->hisi_ctx_data.set_pt_flag) {
		...
		ret_insert = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu,
			...
			nr, reg->flags, kctx->as_nr, reg->gpu_alloc->group_id,
			header_page_number);
	}
			...
			long compress_size)
{
	struct kbase_hisi_device_data *hisi_dev = container_of(gmc_dev,
		struct kbase_hisi_device_data,kbase_gmc_device);
...
/*
 * kbase_gmc_device_init - Register gmc device in init
 *
 * @kbdev:      Kbase_device
 *
 * Return: None.
...
void kbase_gmc_device_init(struct kbase_device *kbdev)
{
	if (kbdev == NULL)
		return;
	if (gmc_register_device(&kbase_gmc_ops,
		&kbdev->hisi_dev_data.kbase_gmc_device)) {
...
/*
 * kbase_gmc_device_term - gmc terminate
 *
 * @kbdev:      Kbase_device
 *
 * Return: None.
