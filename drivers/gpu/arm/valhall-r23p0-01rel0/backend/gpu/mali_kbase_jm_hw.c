				...
				int js)
{
	...
	if (kbase_hw_has_feature(kbdev,
		...
		KBASE_TRACE_ADD_SLOT_INFO(kbdev, JM_SUBMIT, kctx, katom, jc_head, js,
			(u32)katom->atom_flags);
		KBASE_TRACE_ADD_SLOT_INFO(kbdev, JS_RETAIN_CTX_NOLOCK,
			katom->kctx, katom, 0xaaa, kbdev->hwaccess.backend.slot_rb[js].job_chain_flag,
			katom->atom_flags);
#endif
	}
...
#ifdef CONFIG_MALI_FENCE_DEBUG
			if (action == JS_COMMAND_SOFT_STOP_1)
				KBASE_TRACE_ADD_SLOT(kbdev, JM_SOFTSTOP_1, target_katom->kctx,
					target_katom, target_katom->jc, js);
			else
				KBASE_TRACE_ADD_SLOT(kbdev, JM_SOFTSTOP_0, target_katom->kctx,
					target_katom, target_katom->jc, js);
#endif
		}
#ifdef CONFIG_MALI_FENCE_DEBUG
		KBASE_TRACE_ADD_SLOT_INFO(kbdev, JS_RETAIN_CTX_NOLOCK,
			target_katom->kctx, target_katom, 0x999, js, target_katom->atom_flags);
#endif
		...
		if (katom->sched_priority > priority) {
			if (!stop_sent)
				KBASE_TLSTREAM_TL_ATTRIB_ATOM_PRIORITIZED(
						kbdev,
						target_katom);
			if ((katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
				(katom->event_code != BASE_JD_EVENT_STOPPED)) {
				KBASE_TRACE_ADD_REFCOUNT(kbdev, JS_RETAIN_CTX_NOLOCK,
					kctx, katom, 0x888, katom->event_code);
				continue;
			}
