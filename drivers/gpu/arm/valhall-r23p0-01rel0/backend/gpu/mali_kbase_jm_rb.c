				...
				ktime_t *end_timestamp)
{
	...
	if ((katom != NULL) && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		...
		katom->x_post_dep) {
		dev_err(kbdev->dev, "Track atom info in kbase_gpu_complete_hw first\n");
		dev_err(kbdev->dev, "atom_id %d, core_req %d, event_code = %d, jobslot = %d, sched_priority = %d, status = %d\n",
			kbase_jd_atom_id(kctx, katom), katom->core_req, katom->event_code, katom->jobslot,
			katom->sched_priority, katom->status);
	...
	if (completion_code == BASE_JD_EVENT_STOPPED) {
		...
		if (next_katom && katom->kctx == next_katom->kctx &&
			...
			WARN_ON(next_katom->gpu_rb_state ==
					KBASE_ATOM_GPU_RB_SUBMITTED);
#ifdef CONFIG_MALI_FENCE_DEBUG
			if ((next_katom->core_req == 0x4e) &&
				!(next_katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
				next_katom->x_post_dep) {
				dev_err(kbdev->dev, "kctx %p,atom_id %d, event_code = %d, jobslot = %d, sched_priority = %d, status = %d\n",
					(void *)kctx, kbase_jd_atom_id(kctx, next_katom), next_katom->event_code, next_katom->jobslot,
					next_katom->sched_priority, next_katom->status);
			}
#endif
...
#ifdef CONFIG_MALI_FENCE_DEBUG
					dev_err(kbdev->dev, "Track0 atom_id = %d, kctx = %p,"
						"jobslot = %d, gpu_rb_state = %d,"
						"sched_priority = %d, status = %d\n",
						kbase_jd_atom_id(kctx, katom_idx1),
						kctx, katom_idx1->jobslot,
						katom_idx1->gpu_rb_state,
						katom_idx1->sched_priority,
						katom_idx1->status);
#endif
					kbase_jm_return_atom_to_js(kbdev,
								katom_idx1);
...
#ifdef CONFIG_MALI_FENCE_DEBUG
				dev_err(kbdev->dev, "Track1 atom_id = %d, kctx = %p,"
					"jobslot = %d, gpu_rb_state = %d,"
					"sched_priority = %d, status = %d\n",
					kbase_jd_atom_id(kctx, katom_idx0),
					kctx, katom_idx0->jobslot,
					katom_idx0->gpu_rb_state,
					katom_idx0->sched_priority,
					katom_idx0->status);
#endif
...
#ifdef CONFIG_MALI_FENCE_DEBUG
				dev_err(kbdev->dev, "Track2 atom_id = %d, kctx = %p,"
					"jobslot = %d, gpu_rb_state = %d,"
					"sched_priority = %d, status = %d\n",
					kbase_jd_atom_id(kctx, katom_idx1),
					kctx, katom_idx1->jobslot,
	...
	if ((katom != NULL) && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		!(katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
		katom->x_post_dep) {
		dev_err(kbdev->dev, "Track atom info in kbase_gpu_complete_hw second\n");
		dev_err(kbdev->dev, "atom_id %d, event_code = %d, jobslot = %d, sched_priority = %d, status = %d\n",
			kbase_jd_atom_id(kctx, katom), katom->event_code, katom->jobslot, katom->sched_priority, katom->status);
		dev_err(kbdev->dev, "slot_nr = %d, atom_flags = 0x%x, retry_count = %d, gpu_rb_state = %d\n",
			katom->slot_nr, katom->atom_flags, katom->retry_count, katom->gpu_rb_state);
		dev_err(kbdev->dev, "retained = %d, blocked = %d, flush_id = %d, age = %d, will_fail_event_code = %d\n",
			katom->need_cache_flush_cores_retained, atomic_read(&katom->blocked),
	...
	if ((katom != NULL) && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		!(katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
		katom->x_post_dep) {
		dev_err(kbdev->dev, "Track atom info in kbase_gpu_complete_hw third\n");
		dev_err(kbdev->dev, "atom_id %d, event_code = %d, jobslot = %d, sched_priority = %d, status = %d\n",
			kbase_jd_atom_id(kctx, katom), katom->event_code, katom->jobslot, katom->sched_priority, katom->status);
		dev_err(kbdev->dev, "slot_nr = %d, atom_flags = 0x%x, retry_count = %d, gpu_rb_state = %d\n",
			katom->slot_nr, katom->atom_flags, katom->retry_count, katom->gpu_rb_state);
