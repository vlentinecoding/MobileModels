...
static void kbase_pm_shaders_update_state(struct kbase_device *kbdev)
{
	...
	if (!corestack_driver_control) {
		...
		while (kbase_pm_get_state(kbdev, KBASE_PM_CORE_SHADER,
			ACTION_PWRTRANS) && (timeout < 100000)) {
			timeout++;
			udelay(10);
		}
		if (timeout == 100000) {
			dev_err(kbdev->dev,
				"Waiting for transition done timeout!!!\n");
		}
	}
...
void kbase_pm_update_state(struct kbase_device *kbdev)
{
	...
	if (!kbdev->pm.backend.gpu_ready &&
		(atomic_read(&kbdev->hisi_dev_data.regulator_refcount) == 0)) {
...
extern void print_atom_info(struct kbase_jd_atom *katom);
extern struct kbase_jd_atom *kbase_gpu_inspect(struct kbase_device *kbdev, int js, int idx);
static void dump_slot_atoms_by_kctx(struct kbase_context *context)
{
	struct kbase_device *kbdev = context->kbdev;
	int js;
	unsigned long flags;
	...
	for (js = 0; js < kbdev->gpu_props.num_job_slots; js++) {
		...
		if (katom != NULL && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED)) {
			...
			dev_err(kbdev->dev, "Dump js0 because of abnormal poweroff when context %llx still has jobs\n",
				(unsigned long long)kctx);
			if (kctx != NULL) {
				dev_err(kbdev->dev, "kctx %p, tgid = %d\n", kctx, kctx->tgid);
			...
			dev_err(kbdev->dev, "abnormal_poweroff dump js0 end\n");
		}

		katom = kbase_gpu_inspect(kbdev, js, 1);
		if (katom != NULL && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED)) {
			struct kbase_context *kctx = katom->kctx;
			dev_err(kbdev->dev, "Dump js1 because of abnormal poweroff when context %llx still has jobs\n",
				(unsigned long long)kctx);
			if (kctx != NULL) {
				dev_err(kbdev->dev, "kctx %p, tgid = %d\n", kctx, kctx->tgid);
				dev_err(kbdev->dev, "atoms_pulled = %d, slots_pullable = %d, as_nr = %d, refcount = %d, flags = 0x%x\n",
			...
			print_atom_info(katom);
			kbasep_trace_dump(kbdev);
			dev_err(kbdev->dev, "abnormal_poweroff dump js1 end\n");
		}
...
bool kbase_pm_clock_off(struct kbase_device *kbdev, bool is_suspend)
{
	...
	if (!kbdev->pm.suspending) {
		...
		for (i = 0; i != kbdev->nr_hw_address_spaces; ++i) {
			...
			if (kctx && kctx->jctx.job_nr && kctx->jctx.sched_info.ctx.nr_jobs) {
				dump_slot_atoms_by_kctx(kctx);
				break;
			}
		}
	}
...
static int kbase_pm_do_reset(struct kbase_device *kbdev)
{
	...
	if (dsm_report_enable(kbdev->hisi_dev_data.error_num.hard_reset, 5,
		kbdev->hisi_dev_data.factory_mode))
...
int kbase_pm_init_hw(struct kbase_device *kbdev, unsigned int flags)
{
	...
	/* Soft reset the GPU */
	if (kbdev->hisi_dev_data.as_stuck_hard_reset)
	...
	kbdev->hisi_dev_data.as_stuck_hard_reset = false;
