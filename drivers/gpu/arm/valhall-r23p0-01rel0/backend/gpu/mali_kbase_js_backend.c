...
#define FENCE_SOFTSTOP_TICKS 2
#define BASE_JD_REQ_FENCE (BASE_JD_REQ_CS | BASE_JD_REQ_T | BASE_JD_REQ_CF | BASE_JD_REQ_COHERENT_GROUP)
				/* non-fragment need more time to execute */
				if (atom->core_req == BASE_JD_REQ_FENCE)
					soft_stop_ticks = FENCE_SOFTSTOP_TICKS;
#endif
...
#ifdef CONFIG_MALI_FENCE_DEBUG
					atom->softstop_timer_triggered = 2;
#endif
