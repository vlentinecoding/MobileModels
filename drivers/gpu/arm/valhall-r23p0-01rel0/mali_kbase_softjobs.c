...
static void validate_list(struct kbase_jd_atom *katom)
{
	if (katom->queue.next == LIST_POISON1)
		dev_err(katom->kctx->kbdev->dev,
			"kctx %pK atom %pK next pointer corrupted\n",
			(void *)katom->kctx, katom);

	if (katom->queue.prev == LIST_POISON2)
		dev_err(katom->kctx->kbdev->dev,
			"kctx %pK atom %pK prev pointer corrupted\n",
			(void *)katom->kctx, katom);
}
...
void kbasep_remove_waiting_soft_job(struct kbase_jd_atom *katom)
{
	...
	if (!katom->soft_job_queued)
		dev_err(katom->kctx->kbdev->dev,
			"!!!!!!!kctx %pK atom %pK job haven't be queued \n",
			(void *)katom->kctx, katom);
	validate_list(katom);
...
void validate_fence_wait_katom(struct kbase_jd_atom *katom)
{
	...
	if ((katom->status == KBASE_JD_ATOM_STATE_UNUSED) ||
		...
		dev_err(kctx->kbdev->dev,
			"kctx %pK unexpected status (%x) for katom\n",
...
void kbase_soft_event_wait_callback(struct kbase_jd_atom *katom)
{
	...
	if (kbase_ctx_flag(katom->kctx, KCTX_DYING))
		dev_err(kctx->kbdev->dev,
			"kctx %pK atom %pK fence callback invoked for dying context\n",
			(void *)kctx, katom);
...
void kbase_sync_fence_debug_dump(struct dma_fence *fence, bool show_atoms);
static void kbase_jd_debug_fence_info(struct kbase_jd_atom *atom)
{
#if defined(CONFIG_SYNC) || defined(CONFIG_SYNC_FILE)
	struct kbase_sync_fence_info info;
	int res;
...
void print_atom_info(struct kbase_jd_atom *katom)
{
	if (katom == NULL || katom->kctx == NULL) {
		pr_err("mali gpu: katom or kctx is null\n");
		return;
	}
	pr_err("mali gpu: ctx id %d_%d (nr_jobs %u pulled %u flags 0x%x)\n",
		katom->kctx->tgid, katom->kctx->id, katom->kctx->jctx.job_nr, atomic_read(&katom->kctx->atoms_pulled),
		atomic_read(&katom->kctx->flags));
	pr_err("mali gpu: atom info: id %d, age 0x%x, core_req 0x%x, flags 0x%x\n",
		kbase_jd_atom_id(katom->kctx, katom), katom->age, katom->core_req, katom->atom_flags);
	pr_err("mali gpu: jc %llx, ec 0x%x, (will_fail 0x%x), status 0x%x, rb_state = 0x%x, protect_state 0x%x, slot %d, prio %d, tick %u, blocked %d\n",
...
static int kbasep_jd_atoms_show(struct kbase_context *kctx)
{
	...
	dev_err(kbdev->dev, "dump kctx %p, tgid = %d\n", kctx, kctx->tgid);
	dev_err(kbdev->dev, "atoms_pulled = %d, slots_pullable = %d, as_nr = %d, refcount = %d, flags = 0x%x\n",
	...
	dev_err(kbdev->dev, "\n");
	dev_err(kbdev->dev, " poweroff_wait_in_progress invoke_poweroff_wait_wq_when_l2_off poweron_required poweroff_is_suspend\n");
	dev_err(kbdev->dev, "%3u, %3u, %3u , %3u",
...
extern struct fence_ops kbase_fence_ops;
void kbase_sync_fence_debug_dump(struct dma_fence *fence, bool show_atoms)
{
	unsigned long flags;
...
void kbase_fence_dump_in_display(void)
{
	struct kbase_device *kbdev = kbase_platform_get_device();
	if (kbdev) {
		kbase_fence_debug_dump_all_kctx(kbdev);
	} else {
		pr_err("mali gpu: kbdev is null\n");
	}
}
EXPORT_SYMBOL(kbase_fence_dump_in_display);
...
static void kbase_fence_debug_wait_timeout(struct kbase_jd_atom *katom)
{
	...
	if (fence->ops && fence->ops->get_driver_name)
		dev_err(dev, "fence driver name %s\n", fence->ops->get_driver_name(fence));
...
{
#else
void kbasep_soft_job_timeout_worker(unsigned long addr)
{
...
int kbase_process_soft_job(struct kbase_jd_atom *katom)
{
	...
	case BASE_JD_REQ_SOFT_FENCE_WAIT:
	{
		...
		if (ret == 1) {
			...
			hrtimer_start(&kctx->hisi_ctx_data.fence_wait_timer,
				HR_TIMER_DELAY_MSEC(FENCE_TIMEOUT_TIME),
				HRTIMER_MODE_REL);
			kctx->hisi_ctx_data.timer_started = 1;
...
void kbase_cancel_soft_job(struct kbase_jd_atom *katom)
{
	...
	default:
		...
		pr_err("%s soft-job cancel request katom is %pK,job req is %x",
			__func__, katom, katom->core_req);
...
void kbase_finish_soft_job(struct kbase_jd_atom *katom)
{
	...
	case BASE_JD_REQ_SOFT_FENCE_WAIT:
		...
		/* BASE_DEBUG_FENCE_TIMEOUT */
		if (katom->kctx && katom->kctx->hisi_ctx_data.timer_started == 1) {
			hrtimer_cancel(&katom->kctx->hisi_ctx_data.fence_wait_timer);
			katom->kctx->hisi_ctx_data.timer_started = 0;
		}
