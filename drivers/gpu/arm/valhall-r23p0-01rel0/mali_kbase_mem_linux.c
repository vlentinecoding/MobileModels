		...
		u64 *gpu_va)
{
	...
	if (kctx->kbdev->hisi_dev_data.lb_enabled)
		group_id = base_mem_group_id_get(*flags);

	if (kbase_reg_prepare_native(reg, kctx, group_id) != 0) {
		...
		u64 new_pages, u64 old_pages)
{
	...
	ret = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu,
		...
		reg->flags, kctx->as_nr, reg->gpu_alloc->group_id, delta);
...
static void kbase_cpu_vm_close(struct vm_area_struct *vma)
{
	...
	if (map->free_on_close) {
		...
		if (!(task && (task->flags & PF_EXITING))) {
			...
			/*
			 * when task = NULL only when mm_users > 0 need to free region or
			 * will let it free in destry context
			 */
			if ((task != NULL) || (!IS_ERR(mm) &&
...
static vm_fault_t kbase_cpu_vm_fault(struct vm_fault *vmf)
{
	...
	struct kbase_aliased *aliased = NULL;
	int err = 0;
	...
	if (unlikely(map->region->cpu_alloc->type == KBASE_MEM_TYPE_ALIAS)) {
		aliased = get_aliased_alloc(vma, map->region, &map_start_pgoff, 1);
	...
	/* For the region be aliased, we have to decompress it before mmapped
	 * by alias region.
	 */
	if (aliased)
		err = kbase_get_compressed_alloc(aliased->alloc, 0, aliased->alloc->nents);
	else
		err = kbase_get_compressed_alloc(map->alloc, map_start_pgoff,
			map->alloc->nents - map_start_pgoff);
	if (err)
		goto exit;
		...
		struct kbase_vmap_struct *map)
{
	...
	kbase_get_compressed_region(reg,
		(offset_bytes >> PAGE_SHIFT) + reg->start_pfn, page_count);
	...
	cpu_addr = kbdev->mgm_dev->ops.vmap(pages, page_count, offset, VM_MAP, prot);
...
static void kbase_vunmap_phy_pages(struct kbase_context *kctx,
		struct kbase_vmap_struct *map)
{
	bool may_compressed = (map->gpu_alloc != NULL) &&
			(map->gpu_alloc->reg != NULL);

	void *addr = (void *)((uintptr_t)map->addr & PAGE_MASK);
	vunmap(addr);

	/* The region may compressed by GMC, in this case, we have to decompress it
	 * before unmap physical pages.
	 */
	if (may_compressed) {
		struct kbase_va_region *reg = map->gpu_alloc->reg;
		struct tagged_addr *start_page = kbase_get_gpu_phy_pages(reg);
		size_t const page_count = PFN_UP(map->offset_in_page + map->size);
		unsigned long page_index = ((size_t)map->gpu_pages - (size_t)start_page) /
				sizeof(*start_page);

		kbase_gpu_vm_lock(kctx);
		kbase_get_compressed_region(reg, page_index + reg->start_pfn, page_count);
	}
