...
const int kbasep_js_atom_throttle_to_relative[BASE_JD_NR_THRO_LEVELS] = {
	KBASE_JS_ATOM_SCHED_THRO_MED, /* BASE_JD_THRO_MEDIUM */
	KBASE_JS_ATOM_SCHED_THRO_HIGH, /* BASE_JD_THRO_HIGH */
	KBASE_JS_ATOM_SCHED_THRO_LOW  /* BASE_JD_THRO_LOW */
};
#endif
						...
						int js)
{
	...
	/* use JS_RETAIN_CTX_NOLOCK and gpu_addr 111 simulate this case */
	KBASE_TRACE_ADD_REFCOUNT(kbdev, JS_RETAIN_CTX_NOLOCK, kctx, NULL, 111,
...
#ifdef CONFIG_MALI_FENCE_DEBUG
	KBASE_TRACE_ADD_REFCOUNT(kbdev, JS_RETAIN_CTX_NOLOCK, kctx, NULL, 222,
		atomic_read(&kctx->atoms_pulled));
#endif
...
#ifdef CONFIG_MALI_FENCE_DEBUG
	KBASE_TRACE_ADD_REFCOUNT(kbdev, JS_RETAIN_CTX_NOLOCK, kctx, NULL, 333,
		atomic_read(&kctx->atoms_pulled));
#endif
					...
					bool is_scheduled)
{
...
#ifdef CONFIG_MALI_FENCE_DEBUG
		if ((katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED))
			dev_err(kctx->kbdev->dev, "ctx_pullable1 blocked_js flags = 0x%x\n", katom->atom_flags);
		reason = 2;
		result = false;
		goto out;
#else
	...
	if (atomic_read(&katom->blocked)) {
		...
		if ((katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED)) {
			WARN_ON(1);
			dev_err(kctx->kbdev->dev, "ctx_pullable2:ctx(%llx), atom(%d), atom_flags(0x%x), kctx_flags(0x%x), priority(%d)\n",
				(unsigned long long)kctx, kbase_jd_atom_id(kctx, katom), katom->atom_flags,
			       	atomic_read(&kctx->flags), kctx->priority);
			dev_err(kctx->kbdev->dev, "kctx_pulled_slot_pri[%d][%d]=%d, status=%d\n", js, katom->sched_priority,
				kctx->atoms_pulled_slot_pri[js][katom->sched_priority], katom->status);
			dev_err(kctx->kbdev->dev, "atoms_pulled = %d, slots_pullable = %d, as_nr = %d, refcount = %d, flags = 0x%x\n",
				atomic_read(&kctx->atoms_pulled), kctx->slots_pullable, kctx->as_nr,
				atomic_read(&kctx->refcount), atomic_read(&kctx->flags));
			dev_err(kctx->kbdev->dev, "submit_allowed 0x%x nr_user_contexts_running %d"
				"nr_all_contexts_running %d nr_contexts_pullable %d nr_contexts_runnable %d\n",
				js_devdata->runpool_irq.submit_allowed, js_devdata->nr_user_contexts_running,
				js_devdata->nr_all_contexts_running, js_devdata->nr_contexts_pullable,
				js_devdata->nr_contexts_runnable);
		}
		reason = 3;
		result = false;
		goto out;
...
#ifdef CONFIG_MALI_FENCE_DEBUG
			if ((katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED))
				dev_err(kctx->kbdev->dev, "ctx_pullable3 flags = 0x%x, x_state = %d, failed_event = %d\n",
					katom->atom_flags, katom->x_pre_dep->gpu_rb_state,
					katom->x_pre_dep->will_fail_event_code);
			reason = 4;
			result = false;
			goto out;
...
#ifdef CONFIG_MALI_FENCE_DEBUG
			if ((katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED))
				dev_err(kctx->kbdev->dev, "ctx_pullable4 flags = 0x%x\n",
					katom->atom_flags);
			reason = 5;
			result = false;
			goto out;
...
out:
	if (!result) {
		if (katom)
			KBASE_TRACE_ADD_REFCOUNT(kctx->kbdev, JS_RETAIN_CTX_NOLOCK, kctx, katom, 666, reason);
		else
			KBASE_TRACE_ADD_REFCOUNT(kctx->kbdev, JS_RETAIN_CTX_NOLOCK, kctx, NULL, 666, reason);
	}
	return result;
#else
	return true;
...
static void js_return_worker(struct work_struct *data)
{
...
#ifdef CONFIG_MALI_FENCE_DEBUG
	if ((katom != NULL) && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		!(katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
		katom->x_post_dep) {
		dev_err(kbdev->dev, "Track atom info in js_return_worker first\n");
		dev_err(kbdev->dev, "core_req %d, event_code = %d, jobslot = %d, sched_priority = %d, status = %d\n",
			katom->core_req, katom->event_code, katom->jobslot, katom->sched_priority, katom->status);
		dev_err(kbdev->dev, "slot_nr = %d, atom_flags = 0x%x, retry_count = %d, gpu_rb_state = %d\n",
			katom->slot_nr, katom->atom_flags, katom->retry_count, katom->gpu_rb_state);
		dev_err(kbdev->dev, "retained = %d, blocked = %d, flush_id = %d, age = %d, will_fail_event_code = %d\n",
			katom->need_cache_flush_cores_retained, atomic_read(&katom->blocked),
			katom->flush_id, katom->age, katom->will_fail_event_code);
#ifdef CONFIG_HISI_DEBUG_FS
		dev_err(kbdev->dev, "fault_event.event_code = %d\n", katom->fault_event.event_code);
#endif
		dev_err(kbdev->dev, "kctx=%p, tgid=%d, atoms_pulled=%d, slots_pullable=%d, as_nr=%d, refcount=%d, flags=0x%x\n",
			kctx, kctx->tgid, atomic_read(&kctx->atoms_pulled), kctx->slots_pullable, kctx->as_nr,
			atomic_read(&kctx->refcount), atomic_read(&kctx->flags));
	...
	if ((katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		...
		katom->x_post_dep) {
		int i;
		dev_err(kbdev->dev, "Return worker:ctx(%llx), atom(%d), atom_flags(0x%x), kctx_flags(0x%x)\n",
			(unsigned long long)kctx, kbase_jd_atom_id(kctx, katom), katom->atom_flags, atomic_read(&kctx->flags));
		dev_err(kbdev->dev, "Return worker:pull(%d), blocked_js(%d), atoms_pulled(%d), context_pullable(%d)\n",
			kctx->atoms_pulled_slot_pri[js][prio], kctx->blocked_js[js][prio], atomic_read(&kctx->atoms_pulled),
			kbase_js_ctx_pullable(kctx, js, true));
		KBASE_TRACE_ADD_REFCOUNT(kbdev, JS_RETAIN_CTX_NOLOCK, kctx, katom, 555, atomic_read(&katom->blocked));
		dev_err(kbdev->dev, "atoms_pulled_slot[%d] = %d\n", js, atomic_read(&kctx->atoms_pulled_slot[js]));
		/* Print all pullable list and unpullable list */
		for (i = 0; i < KBASE_JS_ATOM_SCHED_PRIO_COUNT; i++) {
			struct kbase_context *pullable_kctx = NULL;
			struct kbase_context *unpullable_kctx = NULL;
			struct kbase_context *n = NULL;
			spin_lock_irqsave(&kbdev->hwaccess_lock, flags);
			list_for_each_entry_safe(pullable_kctx, n, &kbdev->js_data.ctx_list_pullable[js][i],
				jctx.sched_info.ctx.ctx_list_entry[js]) {
				dev_err(kbdev->dev, "Print (%llx) from the pullable queue (js:%d), prio(%d)\n",
					(unsigned long long)pullable_kctx, js, i);
			}
			list_for_each_entry_safe(unpullable_kctx, n, &kbdev->js_data.ctx_list_unpullable[js][i],
				jctx.sched_info.ctx.ctx_list_entry[js]) {
				dev_err(kbdev->dev, "Print (%llx) from the unpullable queue (js:%d), prio(%d)\n",
					(unsigned long long)pullable_kctx, js, i);
			}
			spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);
		}
	...
	if ((katom != NULL) && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		!(katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
		katom->x_post_dep) {
		dev_err(kbdev->dev, "Track atom info in js_return_worker second\n");
		dev_err(kbdev->dev, "core_req %d, event_code = %d, jobslot = %d, sched_priority = %d, status = %d\n",
			katom->core_req, katom->event_code, katom->jobslot, katom->sched_priority, katom->status);
		dev_err(kbdev->dev, "slot_nr = %d, atom_flags = 0x%x, retry_count = %d, gpu_rb_state = %d\n",
			katom->slot_nr, katom->atom_flags, katom->retry_count, katom->gpu_rb_state);
	...
	if ((katom != NULL) && (katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		!(katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
		katom->x_post_dep) {
		dev_err(kbdev->dev, "Track atom info in js_return_worker third\n");
		dev_err(kbdev->dev, "core_req %d, event_code = %d, jobslot = %d, sched_priority = %d, status = %d\n",
			katom->core_req, katom->event_code, katom->jobslot, katom->sched_priority, katom->status);
		dev_err(kbdev->dev, "slot_nr = %d, atom_flags = 0x%x, retry_count = %d, gpu_rb_state = %d\n",
			katom->slot_nr, katom->atom_flags, katom->retry_count, katom->gpu_rb_state);
		...
		dev_err(kbdev->dev, "kctx=%p, tgid=%d, atoms_pulled=%d, slots_pullable=%d, as_nr=%d, refcount=%d, flags=0x%x\n",
			...
			atomic_read(&kctx->refcount), atomic_read(&kctx->flags));
		dev_err(kbdev->dev, "kbdev->js_data.runpool_irq.submit_allowed = %d\n", kbdev->js_data.runpool_irq.submit_allowed);
		katom->softstop_timer_triggered -= 1;
	}
#endif
		...
		ktime_t *end_timestamp)
{
	...
	if ((katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
		!(katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
		katom->x_post_dep) {
		dev_err(kbdev->dev,"kctx %p,Find target stopped atom id %d, %d\n",
			(void *)kctx, kbase_jd_atom_id(kctx, katom), katom->will_fail_event_code);
	}
#endif
...
#ifdef CONFIG_MALI_HISI_CL_BOOST
