		...
		struct kbase_fault *fault)
{
...
#if (defined(CONFIG_MALI_GPU_BUG_ON) && defined(CONFIG_HISI_BB)) || defined(CONFIG_HUAWEI_DSM)
	...
	if (!pool->dying) {
		...
		if (dsm_report_enable(kbdev->hisi_dev_data.error_num.page_fault,
			30, kbdev->hisi_dev_data.factory_mode))
...
#endif
#if defined(CONFIG_MALI_GPU_BUG_ON) && defined(CONFIG_HISI_BB)
	kbase_mem_pool_lock(pool);
	if (!pool->dying) {
		if (kbase_has_hi_feature(kbdev,
			KBASE_FEATURE_MMU_FAULT_FILTERING) &&
			!kbdev->hisi_dev_data.factory_mode) {
			kbdev->hisi_dev_data.unhandled_page_fault_count++;
			if (kbdev->hisi_dev_data.unhandled_page_fault_count >= 3)
