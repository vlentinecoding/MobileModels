	...
	struct kbase_va_region *region)
{
	...
	kbdev = kctx->kbdev;
	fault_rel_pfn = (faulting_as->pf_data.addr >> PAGE_SHIFT) -
		region->start_pfn;
	...
	if (!err) {
		...
		if (region->flags & KBASE_REG_SCRAMBLE_BIT) {
			...
			/*
			 * No scramble needed for this buffer,
			 * set header_page_number to allocated pages.
			 */
		...
		err = kbase_mmu_insert_pages(kbdev, &kctx->mmu,
			...
			region->flags, kctx->as_nr, region->gpu_alloc->group_id,
			header_page_number);
	}
...
void page_fault_worker(struct work_struct *data)
{
	...
	struct kbase_sub_alloc *prealloc_sas[4] = { NULL, NULL, NULL, NULL };
	int i;
...
page_fault_retry:
	 ...
	 * validating the fault_adress to be within a size_t
	 * from the start_pfn
	 */
	...
	if (grown) {
		...
		err = kbase_mmu_insert_pages_no_flush(kbdev, &kctx->mmu,
			...
			&kbase_get_gpu_phy_pages(region)[pfn_offset],
			new_pages, region->flags, region->gpu_alloc->group_id, new_pages);
		if (err) {
			kbase_free_phy_pages_wraper(region->gpu_alloc,
					new_pages);
			if (region->gpu_alloc != region->cpu_alloc)
	...
	int const level, int const group_id)
{
	...
	kbdev->mmu_mode->entry_set_ate_scramble_bit(kbdev, &entry, phy, flags, level);
	return entry;
}
...
int kbase_mmu_insert_pages_no_flush(struct kbase_device *kbdev,
				    ...
				    struct tagged_addr *phys, size_t nr,
				    unsigned long flags,
				    int const group_id, u64 header_page_number)
{
	phys_addr_t pgd;
	u64 *pgd_page;
		...
		if (cur_level == MIDGARD_MMU_LEVEL(2)) {
			...
			*target = kbase_mmu_create_ate(kbdev, *phys, flags,
			...
			for (i = 0; i < count; i++) {
				 ...
				 * If GMC is enabled, we bypass this check as partial of the
				 * pages in region are not compressed, so these pages's ATE is valid.
				 */
#ifndef CONFIG_GPU_GMC_GENERIC
		...
		u64 header_page_number)
{
	...
	err = kbase_mmu_insert_pages_no_flush(kbdev, mmut, vpfn,
			phys, nr, flags, group_id, header_page_number);
		...
		u64 vpfn, size_t nr, bool sync)
{
	...
	if (err) {
		...
		kbdev->hisi_dev_data.error_num.ts = hisi_getcurtime();
#if defined(CONFIG_LP_ENABLE_HPM_DATA_COLLECT) && defined(CONFIG_HISI_BB)
...
static void kbase_mmu_notifier_invalidate_range_start_vmcpy(
	struct mmu_notifier *mn, struct mm_struct *mm,
	struct vm_area_struct *vma, unsigned long start, unsigned long end)
{
	if (!(vma->ext_flags & VM_COPY_DONE))
		return;

	struct kbase_cpu_mapping *map =
		(struct kbase_cpu_mapping *)vma->vm_private_data;
	struct kbase_context *kctx = map->kctx;

	kbase_gpu_vm_lock(kctx);
}

static void kbase_mmu_notifier_change_pte_vmcpy(struct mmu_notifier *mn,
	...
	pte_t pte)
{
	...
	struct kbase_cpu_mapping *map =
		(struct kbase_cpu_mapping *)vma->vm_private_data;
	struct kbase_va_region *reg = map->region;
	u64 page_idx = (address >> PAGE_SHIFT) - reg->start_pfn;
	struct tagged_addr *reg_pages = kbase_get_gpu_phy_pages(reg);

	phys_addr_t phy_addr = as_phys_addr_t(reg_pages[page_idx]);
	struct page *page = phys_to_page(phy_addr);
	dma_addr_t dma_addr = kbase_dma_addr(page);

	dma_unmap_page(map->kctx->kbdev->dev, dma_addr, PAGE_SIZE,
		DMA_BIDIRECTIONAL);
	put_page(page);

	phy_addr = PFN_PHYS(pte_pfn(pte));
	page = phys_to_page(phy_addr);
	get_page(page);
	dma_addr = dma_map_page(map->kctx->kbdev->dev, page, 0, PAGE_SIZE,
		DMA_BIDIRECTIONAL);
	if (dma_mapping_error(map->kctx->kbdev->dev, dma_addr)) {
		dump_page(page, "dma map page file");
		return;
	}
	kbase_set_dma_addr(page, dma_addr);

	reg->cpu_alloc->vm_copy_happed = 1;
	reg_pages[page_idx] = as_tagged(phy_addr);
	return;
}


static void kbase_mmu_notifier_invalidate_range_end_vmcpy(
	struct mmu_notifier *mn, struct mm_struct *mm,
	struct vm_area_struct *vma, unsigned long start, unsigned long end)
{
	if (!(vma->ext_flags & VM_COPY_DONE))
	...
	group_id = reg->gpu_alloc->group_id;
	...
	(void)kbase_mmu_update_pages(kctx, reg->start_pfn,
		kbase_get_gpu_phy_pages(reg), kbase_reg_current_backed_size(reg),
		reg->flags & gwt_mask, group_id);
	kbase_gpu_vm_unlock(kctx);
}

static void kbase_mmu_notifier_release(struct mmu_notifier *mn,
	struct mm_struct *mm)
{
	struct kbase_mmu_notifier *notifer =
		container_of(mn, struct kbase_mmu_notifier, notifier);
	struct kbase_device *kbdev = notifer->kbdev;
	struct kbase_hisi_device_data *hisi_dev = &kbdev->hisi_dev_data;
	mutex_lock(&hisi_dev->mmu_notifier_list_lock);
	notifer->mm = NULL;
	list_move(&notifer->link,
		&hisi_dev->mmu_notifier_free_list);
	mutex_unlock(&hisi_dev->mmu_notifier_list_lock);
}

static const struct mmu_notifier_ops kbase_mmu_notifier_ops = {
	.release                      = kbase_mmu_notifier_release,
	.change_pte_vmcpy             = kbase_mmu_notifier_change_pte_vmcpy,
	.invalidate_range_end_vmcpy   =
		kbase_mmu_notifier_invalidate_range_end_vmcpy,
	.invalidate_range_start_vmcpy =
		kbase_mmu_notifier_invalidate_range_start_vmcpy,
};

const struct mmu_notifier_ops* kbase_get_mmu_notifier_ops(void)
{
	return &kbase_mmu_notifier_ops;
}
#endif
