...
/* Map to fixed virtual address in kernel. */
#define KBASE_REG_MAP_FIXED (1ul << 27)
...
/* All the flags which are only valid for regions allocated within kbase,
 * and may not be passed from user space.
 */
#define KBASE_REG_KERNEL_ONLY (KBASE_REG_MAP_FIXED | KBASE_REG_PERMANENT_KERNEL_MAPPING | KBASE_REG_NO_USER_FREE)

#ifdef CONFIG_GPU_GMC_GENERIC
/* Flag for the regions without graphic memory compress. */
#define KBASE_REG_NO_GMC (KBASE_REG_KERNEL_ONLY | KBASE_REG_DONT_NEED | KBASE_REG_TRANSIENT_KERNEL_MAPPING)
#endif
/* Scramble memory */
		...
		enum kbase_memory_type type, int group_id)
{
	...
	alloc->lb_pages = kctx->kbdev->hisi_dev_data.smart_cache_capacity;
#else
	alloc->lb_pages = 0;
#endif
