		...
		struct kbase_ioctl_render_prio_values *data)
{
	...
	for (i = 0; i < size; i++) {
		...
		if ((param_task.sched_priority >= 0) &&
			(param_task.sched_priority <= 90)) {
			rcu_read_lock();
			p_task = find_task_by_vpid(info->task[i].pid);
			if (p_task != NULL) {
				if (info->task[i].sched_policy == SCHED_RR) {
					retval = sched_setscheduler_nocheck(
						p_task,
						info->task[i].sched_policy,
						&param_task);
...
/*
 * show_runtime_pm_delay_time - Show callback for the runtime_pm_delay sysfs file.
 *
 * This function is called to get the contents of the runtime_pm_delay sysfs file.
 *
 * @dev: The device this sysfs file is for
 * @attr: The attributes of the sysfs file
 * @buf: The output buffer for the sysfs file contents
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t show_runtime_pm_delay(struct device *dev,
...
/**
 * set_runtime_pm_delay_time - Store callback for the runtime_pm_delay sysfs
 * file.
 *
 * @dev: The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 ...
 * @count: The number of bytes written to the sysfs file.
 *
 * This allows setting the delay time for runtime poweroff when call pm_schedule_suspend.
 *
 * This is expressed in milliseconds.
 *
 * Return: count if the function succeeded. An error code on failure.
 */
static ssize_t set_runtime_pm_delay(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct kbase_device *kbdev = NULL;
	int runtime_pm_delay_ms = 0;

	kbdev = to_kbase_device(dev);
	if (!kbdev)
		return -ENODEV;

	if ((kstrtoint(buf, 0, &runtime_pm_delay_ms) != 0) ||
		(runtime_pm_delay_ms < 0))
		return -EINVAL;

	if (runtime_pm_delay_ms != RUNTIME_PM_DELAY_SHORT &&
		runtime_pm_delay_ms != RUNTIME_PM_DELAY_LONG)
		return -EINVAL;
	atomic_set(&kbdev->hisi_dev_data.runtime_pm_delay_ms,
		runtime_pm_delay_ms);
...
/*
 * The sysfs file runtime poweroff delay time.
 *
 * This is used to config runtime poweroff delay time period.
 * Reading it will show the current delay time when call pm_schedule_suspend
 * Writing to it will set the current delay time.
 */
static DEVICE_ATTR(runtime_pm_delay, S_IRUGO | S_IWUSR,
	show_runtime_pm_delay, set_runtime_pm_delay);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(gpu_product_id_names); ++i) {
		...
		if ((p->id & product_id_mask) ==
			...
			if ((p->id == GPU_ID2_PRODUCT_TTRX >> GPU_ID_VERSION_PRODUCT_ID_SHIFT)
				&& (kbdev->gpu_props.num_cores <= TRYM_GPU_CORE_NUM))
				product_name = "Mali-G57";
			else
				product_name = p->name;
			break;
		}
		...
		struct device_attribute *attr, char * const buf)
{
	...
	total_error_num = kbdev->hisi_dev_data.error_num.page_fault +
		kbdev->hisi_dev_data.error_num.gpu_fault +
		kbdev->hisi_dev_data.error_num.soft_reset +
		kbdev->hisi_dev_data.error_num.hard_reset;

	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"page_fault num:%d\n",
		kbdev->hisi_dev_data.error_num.page_fault);
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"gpu_fault num:%d\n",
		kbdev->hisi_dev_data.error_num.gpu_fault);
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
	...
	ret += scnprintf(buf + ret, PAGE_SIZE - ret,
		"last_time:%d\n",
		kbdev->hisi_dev_data.error_num.ts/1000000000 + 1);
		...
		const char *buf, size_t count)
{
	...
	kbdev->hisi_dev_data.error_num.gpu_fault = 0;
	kbdev->hisi_dev_data.error_num.soft_reset = 0;
	kbdev->hisi_dev_data.error_num.page_fault = 0;
	kbdev->hisi_dev_data.error_num.hard_reset = 0;
	kbdev->hisi_dev_data.error_num.ts = 0;
	return count;
}
...
/*
 * show_vfreq - Show vfreq info for GPU virtual devfreq.
 *
 * This function is called to get the virtual devfreq sysfs file.
 *
 * @dev: The device this sysfs file is for
 * @attr: The attributes of the sysfs file
 * @buf: The output buffer for the sysfs file contents
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t show_vfreq(struct device *dev,
	struct device_attribute *attr, char * const buf)
{
	u64 opp_freq = 0;
	u64 core_mask = 0;
	...
	num = hisi_gpu_get_vfreq_num();
	for (i = 0; i < num; i++) {
		hisi_gpu_get_vfreq_val(i, &opp_freq, &core_mask);
		ret += scnprintf(buf + ret, PAGE_SIZE - ret,
			"%d, %llu, 0x%llX\n",
...
/*
 * The sysfs file gpu virtual devfreq info.
 *
 * This is used to show gpu virtual devfreq info.
 */
static DEVICE_ATTR(vfreq, 0440, show_vfreq, NULL);
#endif
...
int power_control_init(struct kbase_device *kbdev)
{
	...
	kbdev->hisi_dev_data.clock = clk_get(kbdev->dev, "clk_g3d");
	if (IS_ERR_OR_NULL(kbdev->hisi_dev_data.clock)) {
		err = PTR_ERR(kbdev->hisi_dev_data.clock);
		kbdev->hisi_dev_data.clock = NULL;
		if (err == -EPROBE_DEFER) {
		...
		err = clk_prepare_enable(kbdev->hisi_dev_data.clock);
		if (err) {
			dev_err(kbdev->dev,
...
fail:
	...
	if (kbdev->hisi_dev_data.clock != NULL) {
		clk_put(kbdev->hisi_dev_data.clock);
		kbdev->hisi_dev_data.clock = NULL;
	}
...
void power_control_term(struct kbase_device *kbdev)
{
	...
	if (kbdev->hisi_dev_data.clock) {
		clk_disable_unprepare(kbdev->hisi_dev_data.clock);
		clk_put(kbdev->hisi_dev_data.clock);
		kbdev->hisi_dev_data.clock = NULL;
	}
}
...
#ifdef CONFIG_GPU_VIRTUAL_DEVFREQ
	kbase_virtual_devfreq_init(kbdev);
#endif
	return 0;
}
