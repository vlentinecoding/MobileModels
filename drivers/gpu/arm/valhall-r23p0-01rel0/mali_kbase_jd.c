					...
					u8 d, bool ctx_is_dying)
{
	...
	while (!list_empty(&katom->dep_head[d])) {
		...
		if (katom->event_code != BASE_JD_EVENT_DONE &&
			...
			dep_atom->dying_dependence_fault = katom->dying_dependence_fault;
		...
		while (!list_empty(&runnable_jobs)) {
			...
			if (node->status != KBASE_JD_ATOM_STATE_COMPLETED &&
				...
				if ((katom->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
					...
					katom->x_post_dep) {
					dev_err(kctx->kbdev->dev,"kctx %p,Find target katom id %d\n",
						(void *)kctx, kbase_jd_atom_id(kctx, katom));
				}
				if ((node->core_req == 0x4e) && (katom->event_code == BASE_JD_EVENT_STOPPED) &&
					!(node->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
					node->x_post_dep) {
					dev_err(kctx->kbdev->dev,"kctx %p,Find target node atom id %d, katom %d\n",
						(void *)kctx, kbase_jd_atom_id(kctx, node), kbase_jd_atom_id(kctx, katom));
				}
#endif
...
bool jd_submit_atom(struct kbase_context *kctx, const struct base_jd_atom_v2 *user_atom, struct kbase_jd_atom *katom)
{
	...
	/* detecte whether in context dying we still can submit job from user space */
	if (kbase_ctx_flag(kctx, KCTX_DYING))
		dev_err(kbdev->dev, "After context destroy called user space still can submit job to kernel, it's Abormal!!!\n");
	...
	katom->dying_dependence_fault = false;
#ifdef CONFIG_MALI_FENCE_DEBUG
	katom->softstop_timer_triggered = 0;
#endif
...
#ifdef CONFIG_MALI_FENCE_DEBUG
			if ((katom->event_code == BASE_JD_EVENT_STOPPED) && (katom->core_req == 0x4e)) {
				dev_err(kbdev->dev, "submit_atom:atom_id %d, jobslot = %d, sched_priority = %d, status = %d\n",
					kbase_jd_atom_id(kctx, katom), katom->jobslot, katom->sched_priority, katom->status);
...
void kbase_jd_done_worker(struct work_struct *data)
{
	...
	if (katom->event_code == BASE_JD_EVENT_STOPPED) {
		...
		katom->status = KBASE_JD_ATOM_STATE_IN_JS;
#ifdef CONFIG_MALI_FENCE_DEBUG
		if ((katom->core_req == 0x4e) && !(katom->atom_flags & KBASE_KATOM_FLAG_BEEN_SOFT_STOPPPED) &&
			katom->x_post_dep)
			dev_err(kctx->kbdev->dev, "kctx %p Atom %p status to in JS, id=%d\n",
				(void *)kctx, (void *)katom, kbase_jd_atom_id(kctx, katom));
#endif
	...
	if ((katom->event_code != BASE_JD_EVENT_DONE) &&
		...
		if(dsm_report_enable(kbdev->hisi_dev_data.error_num.gpu_fault,
			20, kbdev->hisi_dev_data.factory_mode))
