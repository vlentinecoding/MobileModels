 ...
 * Description: platform config file for hisp300
 * Author: sunshaodan
 * Create: 2018-08-20
 *
...
#define i_2_hi(i) container_of(i, hisp300_t, intf)
 ...
 *********************************************/
void hisp300_set_timestamp(unsigned int *timestamp_high, unsigned int *timestamp_low)
{
#define MICROSECOND_PER_SECOND 1000000
	...
	fw_micro_second =
		...
		s_timeval.tv_sec) * MICROSECOND_PER_SECOND +
...
void hisp300_handle_msg(hisp_msg_t *msg)
{
	...
	case BATCH_REQUEST_RESPONSE:
		...
		cam_info("%s batch h:0x%x l:0x%x, rate %d",
			...
			msg->u.ack_batch_request.system_couter_rate);
		hisp300_set_timestamp(&(msg->u.ack_batch_request.timestampH),
			&(msg->u.ack_batch_request.timestampL));
		break;
	case REQUEST_RESPONSE:
		hisp300_set_timestamp(&(msg->u.ack_request.timestampH),
			&(msg->u.ack_request.timestampL));
		break;
	case MSG_EVENT_SENT:
		hisp300_set_timestamp(&(msg->u.event_sent.timestampH),
 ...
 *********************************************/
static int hisp300_rpmsg_ept_cb(struct rpmsg_device *rpdev,
...
static int find_suitable_mem_pool(struct hisp_cfg_data *pcfg)
{
	...
	if (pcfg->param.type == MAP_TYPE_RAW2YUV) {
		...
		for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
			if (s_hisp300.mem_pool[ipool].prot ==
				pcfg->param.prot) {
				break;
			}
		}
...
static int hisp300_init_r8isp_memory_pool(void *cfg)
{
	...
	s_hisp300.mem_pool[ipool].security_isp_mode =
		pcfg->param.security_isp_mode;
	s_hisp300.mem_pool[ipool].shared_fd = pcfg->param.sharedFd;
	s_hisp300.mem_pool[ipool].is_ap_cached = pcfg->param.isApCached;
	...
	cam_info("%s: r8_iova_pool_base=0x%x",
		__FUNCTION__, s_hisp300.mem_pool[ipool].r8_iova);
	mutex_unlock(&hisi_rpmsg_service_mutex);
	return 0;
}
...
static int hisp300_deinit_r8isp_memory_pool(void *cfg)
{
	...
	prot = s_hisp300.mem_pool[ipool].prot; // prot should not be memset to 0
	memset_s(&(s_hisp300.mem_pool[ipool]), sizeof(struct hisp300_mem_pool),
		0, sizeof(struct hisp300_mem_pool));
	s_hisp300.mem_pool[ipool].prot = prot;
	mutex_unlock(&hisi_rpmsg_service_mutex);
...
static int hisp300_alloc_isp_cpu_addr(void *cfg)
{
	...
	fdmap->sgt = sgt;
	fdmap->shared_fd = pcfg->param.sharedFd;
	fdmap->iova = r8va;
...
static int hisp300_deinit_r8isp_memory_pool_force(void)
{
	...
	for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
		...
		memset_s(&(s_hisp300.mem_pool[ipool]),
			sizeof(struct hisp300_mem_pool),
			0,
			sizeof(struct hisp300_mem_pool));
	}
...
static int hisp300_alloc_r8isp_addr(void *cfg)
{
	...
	pcfg->param.vaddr = (void*)(
		((unsigned char*)s_hisp300.mem_pool[ipool].ap_va) + offset);
	pcfg->param.ispCoreIova = s_hisp300.mem_pool[ipool].ion_iova + offset;
	pcfg->param.iova = r8_iova;
	pcfg->param.offset_in_pool = offset;
	pcfg->param.sharedFd = s_hisp300.mem_pool[ipool].shared_fd;
	pcfg->param.isApCached = s_hisp300.mem_pool[ipool].is_ap_cached;
	mutex_unlock(&hisi_rpmsg_service_mutex);
	return rc;
...
static int hisp300_set_clk_rate(int clk_level)
{
	...
	case ISP_CLK_LEVEL_TURBO:
		...
		ret2 = hisp_set_clk_rate(ISPFUNC3_CLK,
			ISP_TURBO_ISPFUNC3_CLK_RATE);
		ret3 = hisp_set_clk_rate(ISPFUNC4_CLK,
			ISP_TURBO_ISPFUNC4_CLK_RATE);
		ret4 = hisp_set_clk_rate(ISPFUNC5_CLK,
			ISP_TURBO_ISPFUNC5_CLK_RATE);
	...
	case ISP_CLK_LEVEL_LOWPOWER:
		...
		ret4 = hisp_set_clk_rate(ISPFUNC5_CLK,
			ISP_LOWPOWER_ISPFUNC5_CLK_RATE);
	...
	case ISP_CLK_LEVEL_ULTRALOW:
		...
		ret2 = hisp_set_clk_rate(ISPFUNC3_CLK,
			ISP_ULTRALOW_ISPFUNC3_CLK_RATE);
		ret3 = hisp_set_clk_rate(ISPFUNC4_CLK,
			ISP_ULTRALOW_ISPFUNC4_CLK_RATE);
		ret4 = hisp_set_clk_rate(ISPFUNC5_CLK,
			ISP_ULTRALOW_ISPFUNC5_CLK_RATE);
	...
	if (ret0 < 0 || ret1 < 0 || ret2 < 0 || ret3 < 0 || ret4 < 0) {
		cam_err("%s: set clk fail, ret:%d, %d, %d, %d, %d",
			__FUNCTION__, ret0, ret1, ret2, ret3, ret4);
		ret = -EFAULT;
	}
	return ret;
}
...
static int hisp300_config_power_on(hisp_intf_t *i, struct hisp_cfg_data *pcfg)
{
	...
	if (!remote_processor_up) {
		...
		cam_notice("%s power on the hisp300", __FUNCTION__);
		...
		cam_warn("%s begin to power on the hisp300", __FUNCTION__);
		rc = hisp300_power_on(i);
	}
	mutex_unlock(&hisp_power_lock_mutex);
	return rc;
...
static int hisp300_config_power_off(hisp_intf_t *i)
{
	...
	mutex_lock(&hisp_power_lock_mutex);
	if (remote_processor_up) {
		cam_notice("%s power off the hisp300", __FUNCTION__);
		rc = hisp300_power_off(i);
	}
	mutex_unlock(&hisp_power_lock_mutex);
	return rc;
...
static int hisp300_config_op_isp(void *cfg)
{
	...
	case HISP_CONFIG_ISP_TURBO:
		...
		rc = hisp300_set_clk_rate_self_adapt(
			ISP_CLK_LEVEL_TURBO);
		break;
...
static int hisp300_open(hisp_intf_t *i)
{
	cam_info("%s hisp300 device open", __FUNCTION__);
...
{
	int rc = 0;
	cam_info("%s hisp300 device close", __FUNCTION__);
	mutex_lock(&hisp_power_lock_mutex);
...
#pragma GCC visibility push(default)
	hisi_serv->ept =
		rpmsg_create_ept(hisi_serv->rpdev,
			hisp300_rpmsg_ept_cb, hisi_serv, chinfo);
...
static int hisp300_rpmsg_driver_cb(struct rpmsg_device *rpdev,
	void *data, int len, void *priv, u32 src)
{
	cam_info("%s enter ... ", __FUNCTION__);
...
static int hisp300_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
	int rc = 0;
	hisp300_t *hi = NULL;
	struct rpmsg_hisp300_service *hisi_serv = NULL;
...
static int hisp300_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
	int rc = len;
	hisp300_t *hi = NULL;
	struct rpmsg_hisp300_service *hisi_serv = NULL;
...
static int32_t hisp300_platform_probe(struct platform_device* pdev)
{
	int32_t ret = 0;
	cam_info("%s: enter", __FUNCTION__);
...
static struct platform_driver s_hisp300_driver = {
...
static int __init hisp300_init_module(void)
{
	cam_notice("%s enter", __FUNCTION__);
	return platform_driver_register(&s_hisp300_driver);
}

static void __exit hisp300_exit_module(void)
{
	cam_notice("%s enter", __FUNCTION__);
	hisp_unregister(s_hisp300.pdev);
