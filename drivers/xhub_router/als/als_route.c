...
static DECLARE_WAIT_QUEUE_HEAD(xhub_als_block_waitq);

void wake_up_als_ud_block(void)
{
	wake_up_interruptible(&xhub_als_block_waitq);
}
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = sscanf_s(buf, "%hd,%hd,%hd,%hd,%hd,%hd,%hd,%hd", &dbg[0], &dbg[1],
		&dbg[2], &dbg[3], &dbg[4], &dbg[5], &dbg[6],
		&dbg[7]);
	if (ret != ALS_DBG_PARA_SIZE)
		hwlog_err("%s: get parameter error, ret valus: %d\n", __func__, ret);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = wait_event_freezable_timeout(xhub_als_block_waitq,
