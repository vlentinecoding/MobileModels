...
static struct als_platform_data als_data[ALS_DEV_COUNT_MAX] = {
	...
	.is_dc_supported = 0,
	.gpio_func_num = 2,
	.aod_support_xlevel_brightness = 0,
	},
};
	...
	struct sensor_detect_manager *sm)
{
	...
	if (of_property_read_u32(dn, "aod_support_xlevel_brightness", &temp))
		hwlog_err("%s:read aod_support_xlevel_brightness fail\n", __func__);
	else
		pf_data->aod_support_xlevel_brightness = (uint8_t)temp;
}
