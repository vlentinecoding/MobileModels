	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (kstrtoul(buf, 10, &sar_service))
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	for (; (buf = get_str_begin(buf)) != NULL; buf = get_str_end(buf)) {
		if (get_arg(buf, &arg)) {
			if (argc < (ARRAY_SIZE(event.value)))
	...
	if (val == 1) {
		...
		cpkt.hd.length = SUBCMD_LEN;
		{
			...
			ret = WAIT_FOR_MCU_RESP_DATA_AFTER_SEND(&cpkt,
				xhub_mcu_write_cmd(&cpkt, sizeof(cpkt)),
...
static void get_docom_step_counter(void)
{
	...
	g_stepcountdocm = 0;
	sensorhubnode = of_find_compatible_node(NULL, NULL, "huawei,xhub");
	if (of_property_read_string(sensorhubnode, "docom_step_counter",
			(const char **)&stepcountty) == 0) {
		hwlog_info("%s step_count_ty is %s\n",
			__func__, stepcountty);
