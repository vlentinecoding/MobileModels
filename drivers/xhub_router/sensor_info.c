...
#if 1//def CONFIG_HUAWEI_DSM //hanping
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = xhub_sensor_enable(TAG_CONNECTIVITY, enable);
	if (ret) {
	...
	if (enable == 1) {
		ret = xhub_sensor_setdelay(TAG_CONNECTIVITY, &delay_param);
		if (ret) {
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	xhub_route_write(ROUTE_MOTION_PORT, dt_motion_value,
...
ssize_t attr_set_stop_auto_data(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	if (kstrtoul(buf, 16, &stop_auto_accel))
		return -EINVAL;
...
int tell_cts_test_to_mcu(int status)
{
	...
	if (status == 1) {
		xhub_sensor_enable(TAG_AR, false);
