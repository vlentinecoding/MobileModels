...
void read_cap_prox1_info(struct device_node *dn)
{
	...
	if (!strncmp(chip_info, "huawei,semtech-sx9335",
		...
		hwlog_debug("sar sensor from dts is semtech-sx9335\n");
		...
		strlen("huawei,semtech-sx9323"))) {
		hwlog_debug("cap_prox1 sensor from dts is semtech-sx9323\n");
		...
		strlen("huawei,abov-a96t3x6"))) {
		hwlog_debug("cap_prox1 sensor from dts is abov-a96t3x6\n");
...
void read_cap_prox_info(struct device_node *dn)
{
	...
	if (!strncmp(chip_info, "huawei,semtech-sx9323",
	    strlen("huawei,semtech-sx9323"))) {
		hwlog_debug("sar sensor from dts is semtech-sx9323\n");
		...
		strlen("huawei,abov-a96t3x6"))) {
		hwlog_debug("sar sensor from dts is abov-a96t3x6\n");
...
{
	unsigned int semtech_detect_result = 0;
	unsigned int abov_detect_result = 0;
	...
	sar_detect_result = semtech_detect_result | abov_detect_result;
	hwlog_info("%s: semtech_9323=%u abov_result=%u\n",
		__func__, semtech_detect_result, abov_detect_result);
...
{
	unsigned int semtech_9335_result = 0;
	unsigned int abov_aux_detect_result = 0;
	unsigned int semtech_aux_detect_result = 0;
	...
	if (g_semtech_sar_detect_aux.detect_flag == 1) {
		...
		semtech_aux_detect_result =
			sar_sensor_i2c_detect(g_semtech_sar_detect_aux);
	}
	if (g_abov_sar_detect_aux.detect_flag == 1) {
		hwlog_info("abov_sar_aux_detect\n");
		abov_aux_detect_result = sar_sensor_i2c_detect(g_abov_sar_detect_aux);
	}
	sar_aux_detect_result = semtech_9335_result |
		semtech_aux_detect_result | abov_aux_detect_result;
	hwlog_info("%s: semtech_9323_aux=%u abov_aux=%u semtech_9335=%u\n",
		__func__, semtech_aux_detect_result, abov_aux_detect_result,
		semtech_9335_result);
