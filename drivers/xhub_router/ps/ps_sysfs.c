	...
	unsigned long val, int32_t *ps_cali_data, int length)
{
	...
	hwlog_info("ps calibrate success, ps_cali_data %d, %d, %d, %d, %d, %d\n",
		ps_cali_data[0], ps_cali_data[1], ps_cali_data[2],
		ps_cali_data[3], ps_cali_data[4], ps_cali_data[5]);
}
	...
	struct ps_device_info *dev_info, unsigned long val)
{
	...
	uint32_t offset_xtalk = ps_cali_data[3];
	struct sensor_eng_cal_test ps_test = { 0 };
	int32_t i;
	int32_t digital_offset;
	int32_t min_threshold_ps[PS_CAL_NUM] = {
	...
	if (dev_info->chip_type == PS_CHIP_APDS9253_006) {
		digital_offset = ps_get_digital_offset(offset_xtalk);
		ps_cali_data[3] = ps_get_analog_offset(offset_xtalk);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	xhub_route_write(ROUTE_SHB_PORT, (char *)&event, 8);
	return size;
}
