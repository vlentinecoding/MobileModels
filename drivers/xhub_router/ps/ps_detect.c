...
static void read_ps_chip_info(void)
{
	...
	if (!strncmp(sensor_chip_info[PS], "huawei,txc-pa224",
		...
		dev_info->chip_type = PS_CHIP_TCS3701;
	} else if (!strncmp(sensor_chip_info[PS], "ps,s001002,proximity",
		sizeof("ps,s001002,proximity"))) {
		dev_info->chip_type = PS_CHIP_TCS3701;
	} else if (!strncmp(sensor_chip_info[PS], "ps,ps_s001_003,proximity",
		sizeof("ps,ps_s001_003,proximity"))) {
		dev_info->chip_type = PS_CHIP_TCS3718;
	} else {
		return;
	}
