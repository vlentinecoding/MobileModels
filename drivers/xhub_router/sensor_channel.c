...
static void init_hash_tables(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(sensors_cmd_map_tab); ++i) {
...
static bool ap_sensor_flush(int tag)
{
	...
	if (work_on_ap) {
		...
		return xhub_route_write_batch(ROUTE_SHB_PORT,
	...
	struct ioctl_para *para, int tag)
{
	...
	if (cmd == SHB_IOCTL_APP_SENSOR_BATCH) {
		...
		return xhub_sensor_setdelay(tag, &batch_param);
...
static int xhub_app_delay_sensor(int tag, uint8_t *app_config,
	interval_param_t *delay_param)
{
	...
	if (switch_sensor(tag, true)) {
		if (tag == TAG_STEP_COUNTER)
			ret = xhub_sensor_enable_stepcounter(false,
				TYPE_STANDARD);
		else
	...
	if (tag == TAG_STEP_COUNTER) {
		...
		return xhub_sensor_setdelay(tag, delay_param);
	}
}
...
static int send_sensor_cmd(unsigned int cmd, unsigned long arg)
{
	...
	case SHB_IOCTL_APP_DISABLE_SENSOR:
		...
		if (tag == TAG_STEP_COUNTER)
			return xhub_sensor_enable_stepcounter(false,
				TYPE_STANDARD);
		else
			return xhub_sensor_enable(tag, false);
	case SHB_IOCTL_APP_DELAY_SENSOR:
		return xhub_app_delay_sensor(tag, app_config, &delay_param);
...
static ssize_t shb_read(struct file *file, char __user *buf, size_t count,
	loff_t *pos)
{
	return xhub_route_read(ROUTE_SHB_PORT, buf, count);
}
...
static void get_gyro1_calibrate_data(void)
{
	...
	if (dev_info->gyro_first_start_flag == 0) {
		...
		if (ret)
			hwlog_err("%s read from nv fail,ret=%d",
				__func__, ret);
		else
...
static long shb_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SHB_IOCTL_APP_GET_SENSOR_MCU_MODE:
		...
		return 0;
	case SHB_IOCTL_APP_SENSOR_BATCH:
		hwlog_info("%s cmd : batch flush SHB_IOCTL_APP_SENSOR_BATCH\n", __func__);
		break;
	case SHB_IOCTL_APP_SENSOR_FLUSH:
		hwlog_info("%s cmd : batch flush SHB_IOCTL_APP_SENSOR_FLUSH\n", __func__);
		break;
	default:
...
static void enable_sensors_when_recovery_iom3(void)
{
	...
	for (tag = TAG_SENSOR_BEGIN; tag < TAG_SENSOR_END; ++tag) {
		if (sensor_status.status[tag]) {
			...
			xhub_sensor_enable_nolock(tag, true);
			xhub_sensor_setdelay_nolock(tag, &interval_param);
		}
	}
}
...
static int __init xhub_init(void)
{
	int ret;
	...
	if (!is_scp_ready(SCP_A_ID))
		return -1;
	init_hash_tables();
	ret = xhub_route_open(ROUTE_SHB_PORT);
	if (ret != 0) {
		hwlog_err("cannot open xhub route err=%d\n", ret);
		return ret;
	}
	ret = misc_register(&senorhub_miscdev);
	if (ret != 0) {
		xhub_route_close(ROUTE_SHB_PORT);
		hwlog_err("cannot register miscdev err=%d\n", ret);
		return ret;
	}
	register_xhub_recovery_notifier(&sensor_recovery_notify);
	hwlog_info("%s ok\n", __func__);
	return 0;
}

static void __exit xhub_exit(void)
{
	xhub_route_close(ROUTE_SHB_PORT);
...
late_initcall_sync(xhub_init);
module_exit(xhub_exit);
