...
static struct workqueue_struct *g_nv_workqueue;
static struct work_struct g_write_nv_work;
static struct hisi_nve_info_user g_local_user_info;
static int g_should_work = 1;
...
void write_calibrate_data_to_nv_work(struct work_struct *work)
{
	int ret = hisi_nve_direct_access(&g_local_user_info);

	if (ret != 0)
		hwlog_err("hisi_nve_direct_access read %d error %d\n",
			g_local_user_info.nv_number, ret);
	g_should_work = 1;
}
...
{
	while (g_should_work == 0)
		mdelay(10);
	g_should_work = 0;
	memset(&g_local_user_info, 0, sizeof(g_local_user_info));
	g_local_user_info.nv_operation = NV_WRITE_TAG;
	g_local_user_info.nv_number = nv_number;
	g_local_user_info.valid_size = nv_size;
	strncpy(g_local_user_info.nv_name, nv_name,
		sizeof(g_local_user_info.nv_name));
	g_local_user_info.nv_name[sizeof(g_local_user_info.nv_name) - 1] = '\0';
	/* copy to nv by pass */
	memcpy(g_local_user_info.nv_data, temp,
		sizeof(g_local_user_info.nv_data) <
		g_local_user_info.valid_size ?
		sizeof(g_local_user_info.nv_data) :
		g_local_user_info.valid_size);
	if (g_nv_workqueue) {
		queue_work(g_nv_workqueue, &g_write_nv_work);
		return 0;
	}
	return -1;
}
	...
	uint32_t length, int32_t *err_no)
{
	...
	hwlog_info("%s g_xhub_state=%d,tag=%d,subcmd=%u\n",
		__func__, g_xhub_state, pkg_ap.tag, subcmd);
...
void reset_calibrate_data(void)
{
	...
	//reset_therm_calibrate_data();//hanping
}
...
{
	g_nv_workqueue = alloc_workqueue("g_nv_workqueue", 0, 0);
	if (!g_nv_workqueue) {
		hwlog_err("%s alloc workqueue error\n", __func__);
		return;
	}
	INIT_WORK(&g_write_nv_work, write_calibrate_data_to_nv_work);
}

void close_nv_workqueue(void)
{
	if (g_nv_workqueue) {
		destroy_workqueue(g_nv_workqueue);
		g_nv_workqueue = NULL;
	}
}
