				...
				struct arm_smmu_device *smmu)
{
	...
	if (!dynamic) {
		...
		if (strstr(dev_name(smmu->dev), "kgsl-smmu") != NULL && ret >= 0) {
			kgsl_smmu_num++;
			tgid = task_tgid_nr(current);
			get_task_comm(task_comm, current->group_leader);
			pr_err("arm_smmu_init_asid, after kgsl_smmu_num++, kgsl_smmu_num: %d, pid :%d, comm: %s, tgid: %d, leader comm :%s \n",
				    kgsl_smmu_num, current->pid, current->comm, tgid, task_comm);
		}

		mutex_unlock(&smmu->idr_mutex);
		if (ret < 0) {
			dev_err(smmu->dev, "%s dynamic ASID allocation failed: %d\n", dev_name(smmu->dev), ret);
			if (strstr(dev_name(smmu->dev), "kgsl-smmu") != NULL) {
				dev_err(smmu->dev, "dynamic ASID allocation failed on kgsl-smmu: kgsl_smmu has been alloced %d times, \
				        smmu->num_context_banks = %d, MAX_ASID =%d \n", kgsl_smmu_num, smmu->num_context_banks, MAX_ASID);
			}
			return ret;
		}
		cfg->asid = ret;
...
static void arm_smmu_free_asid(struct iommu_domain *domain)
{
	...
	if (strstr(dev_name(smmu->dev), "kgsl-smmu") != NULL) {
		...
		tgid = task_tgid_nr(current);
		get_task_comm(task_comm, current->group_leader);
		pr_err("arm_smmu_free_asid, after kgsl_smmu_num--, kgsl_smmu_num: %d, pid :%d, comm: %s, tgid: %d, leader comm :%s \n",
				kgsl_smmu_num,current->pid, current->comm, tgid, task_comm);
	}
	mutex_unlock(&smmu->idr_mutex);
}
