				...
				struct arm_smmu_device *smmu)
{
	...
	if (!dynamic) {
		...
		if (strstr(dev_name(smmu->dev), "smmu-kgsl") != NULL && ret >= 0) {
			kgsl_smmu_num++;
		...
		if (ret < 0) {
			...
			if (strstr(dev_name(smmu->dev), "smmu-kgsl") != NULL) {
				dev_err(smmu->dev, "dynamic ASID allocation failed on smmu-kgsl: kgsl_smmu has been alloced %d times, \
