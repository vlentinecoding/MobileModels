...
static int aw869xx_haptic_cont_get_f0(struct aw869xx *aw869xx)
{
	...
	/* recover auto break config */
	aw869xx_i2c_write_bits(aw869xx, AW869XX_REG_PLAYCFG3,
		AW869XX_BIT_PLAYCFG3_BRK_EN_MASK, brk_en_temp << 2);
	return ret;
}
...
static void aw869xx_haptic_duraton_ram_config(struct aw869xx *aw869xx,
	unsigned int total_wavloop)
{
	...
	if (total_wavloop <= AW869xx_LONG_SEQ_MAX_TIME) {
		long_seq_type = AW869xx_LONG_WAV_1_CYL;
	} else if (total_wavloop <= SEQ_LONG_TWO_CYCLE * AW869xx_LONG_SEQ_MAX_TIME) {
		total_wavloop = ((aw869xx->duration - 20) * 100) /
			(588 * SEQ_LONG_TWO_CYCLE);
		long_seq_type = AW869xx_LONG_WAV_2_CYL;
	} else if (total_wavloop <= SEQ_LONG_FOUR_CYCLE * AW869xx_LONG_SEQ_MAX_TIME) {
		total_wavloop = ((aw869xx->duration - 20) * 100) /
			(588 * SEQ_LONG_FOUR_CYCLE);
		long_seq_type = AW869xx_LONG_WAV_4_CYL;
	} else if (total_wavloop <= SEQ_LONG_EIGHT_CYCLE * AW869xx_LONG_SEQ_MAX_TIME) {
		total_wavloop = ((aw869xx->duration - 20) * 100) /
			(588 * SEQ_LONG_EIGHT_CYCLE);
		long_seq_type = AW869xx_LONG_WAV_8_CYL;
	}
	seq_num = total_wavloop / 15;
	seq_end_loop = total_wavloop % 15;
	aw869xx_haptic_set_wav_seq(aw869xx, 0, AW869xx_DRV_WAV); // driver_seq_cfg
	aw869xx_haptic_set_wav_loop(aw869xx, 0, 0); // play one time

	for (seq_idx = 0; seq_idx < seq_num; seq_idx++) {
		aw869xx_haptic_set_wav_seq(aw869xx, seq_idx + 1, long_seq_type); // long_seq_cfg
		aw869xx_haptic_set_wav_loop(aw869xx, seq_idx + 1, AW869xx_MAX_PLAY_TIME); // play 15 times
	}
	if (seq_end_loop == 0) {
		aw869xx_haptic_set_wav_seq(aw869xx, seq_idx + 1, AW869xx_BRK_WAV); // brk_seq_cfg
		aw869xx_haptic_set_wav_loop(aw869xx, seq_idx + 1, 0); // play one time
	} else {
		aw869xx_haptic_set_wav_seq(aw869xx, seq_idx + 1, long_seq_type); // long_seq_cfg
		aw869xx_haptic_set_wav_loop(aw869xx, seq_idx + 1, seq_end_loop - 1); // play last time
		aw869xx_haptic_set_wav_seq(aw869xx, seq_idx + 2, AW869xx_BRK_WAV); // brk_seq_cfg
		aw869xx_haptic_set_wav_loop(aw869xx, seq_idx + 2, 0); // play one time
	}

}

static void aw869xx_haptic_long_duraton_ram_config(struct aw869xx *aw869xx)
{
	unsigned char seq_idx = 0;
	unsigned char seq_num = 0;
	unsigned char seq_end_loop = 0;
	...
	for (seq_idx = 0; seq_idx < 6; seq_idx++) {
		aw869xx_haptic_set_wav_seq(aw869xx, seq_idx + 1, AW869xx_LONG_WAV_8_CYL); // long_seq_cfg
		aw869xx_haptic_set_wav_loop(aw869xx, seq_idx + 1, AW869xx_MAX_PLAY_TIME); // play 15 times
	}
...
static int aw869xx_haptic_ram_config(struct aw869xx *aw869xx)
{
	...
	if(aw869xx->effect_mode == SHORT_VIB_RAM_MODE){
		...
		if (aw869xx->duration <= 30) { // just play brk_wav one_time
			aw869xx_haptic_set_wav_seq(aw869xx, 0, AW869xx_BRK_WAV);
			aw869xx_haptic_set_wav_loop(aw869xx, 0, 0);
			aw869xx_haptic_set_wav_seq(aw869xx, 1, 0);
			aw869xx_haptic_set_wav_loop(aw869xx, 1, 0);
		} else {
			total_wavloop = ((aw869xx->duration - 20) * 100) / 588;
			if (total_wavloop <= SEQ_LONG_EIGHT_CYCLE * AW869xx_LONG_SEQ_MAX_TIME)
				aw869xx_haptic_duraton_ram_config(aw869xx, total_wavloop);
			else // duration > 20+8*5.88*6*15 = 4253.6ms
				aw869xx_haptic_long_duraton_ram_config(aw869xx);
		}
	}
...
static int aw869xx_vibrator_init(struct aw869xx *aw869xx)
{
	...
	aw869xx->vib_dev.name = "vibrator1";
