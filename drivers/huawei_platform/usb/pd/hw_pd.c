...
static void fb_unblank_work(struct work_struct *work)
{
	...
	power_event_notify(POWER_NT_WD, POWER_NE_WD_REPORT_UEVENT, &flag);
...
{
	return hihonor_usb_glink_get_cable_type();
}
#else
static int direct_charge_cable_detect(void)
{
...
void pd_dpm_set_cc_orientation(bool cc_orientation)
{
...
void pd_set_product_id_info(unsigned int vid, unsigned int pid,
	unsigned int bcd)
{
	int pd_product_type;
...
#ifdef CONFIG_HIHONOR_OEM_GLINK
	if (hihonor_usb_glink_check_cc_vbus_short()) {
		hwlog_info("%s: glink test cc vbus short\n", __func__);
		return true;
	}
#endif
...
bool pd_dpm_check_sbu_vbus_short(void)
{
#ifdef CONFIG_HIHONOR_OEM_GLINK
	int ret;
	char dsm_buf[PD_DPM_CC_DMD_BUF_SIZE] = {0};
	batt_mngr_get_usb_typec_info typec_info = {0};

	ret = hihonor_oem_glink_oem_get_prop(USB_TYPEC_OEM_CHECK_SBU_VBUS_SHORT, &typec_info, sizeof(typec_info));
	if (ret) {
		hwlog_info("%s: glink get sbu vbus short fail\n", __func__);
		return false;
	}
	hwlog_err("sbu_vbus_status:%d, vsbu1:%d, vsbu2:%d\n", typec_info.sbu_vbus_status, typec_info.vsbu1, typec_info.vsbu2);
	if (typec_info.sbu_vbus_status) {
		snprintf(dsm_buf, PD_DPM_CC_DMD_BUF_SIZE - 1, "sbu vbus short happend:sbu1 %dmv, sbu2 %dmv,\n",
			typec_info.vsbu1, typec_info.vsbu2);
		power_dsm_dmd_report(POWER_DSM_BATTERY, ERROR_NO_TYPEC_VBUS_SBU_SHORT, dsm_buf);
		return true;
	}
#endif
	return false;
...
static void pd_dpm_cc_short_action(void)
{
	...
	enum dc_working_mode mode = direct_charge_get_working_mode();
...
static int pd_dpm_probe(struct platform_device *pdev)
{
	...
	enum hisi_charger_type type = CHARGER_TYPE_SDP;
