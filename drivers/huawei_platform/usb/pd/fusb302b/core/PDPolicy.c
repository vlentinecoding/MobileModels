...
#include "TypeC.h"
#include "fusb30X.h"
#include "../Platform_Linux/platform_pd_status.h"
...
#define USB_FORCE_SINK_SOURCE
#define PD_CAP_VOL_CUR_FLAG        0x3FF
#define PD_CAP_MAX_SIZE            7
...
static void pd_get_cap(struct Port *port)
{
	int i;
	uint32_t pdo, vol;
	struct fusb30x_chip *chip = fusb30x_GetChip();

	if (!port || !chip || !chip->tcpc) {
		pr_err("FUSB %s - Error: port or chip structure is NULL\n", __func__);
		return;
	}
	if (port->SrcCapsHeaderReceived.NumDataObjects > PD_CAP_MAX_SIZE) {
		pr_err("FUSB %s - Error: cap size is invalid\n", __func__);
		return;
	}

	chip->tcpc->pd_port.pe_data.remote_src_cap.nr =
		port->SrcCapsHeaderReceived.NumDataObjects;
	for (i = 0; i < port->SrcCapsHeaderReceived.NumDataObjects; i++) {
		switch (port->SrcCapsReceived[i].PDO.SupplyType) {
		case pdoTypeFixed:
			pdo = port->SrcCapsReceived[i].FPDOSupply.MaxCurrent & PD_CAP_VOL_CUR_FLAG;
			vol = port->SrcCapsReceived[i].FPDOSupply.Voltage & PD_CAP_VOL_CUR_FLAG;
			/* bit 10 to 19 indicate voltage */
			vol = vol << 10;
			pdo |= vol;
			chip->tcpc->pd_port.pe_data.remote_src_cap.pdos[i] = pdo;
			pr_info("FUSB %s - cap[%d]: vol = %u, cur = %u, pdo = %u\n", __func__, i,
				port->SrcCapsReceived[i].FPDOSupply.Voltage,
				port->SrcCapsReceived[i].FPDOSupply.MaxCurrent, pdo);
			break;
		default:
			/* Ignore other supply types for now */
			pr_err("FUSB %s - get not supply type %u\n", __func__,
				port->SrcCapsReceived[i].PDO.SupplyType);
			break;
		}
	}
}
