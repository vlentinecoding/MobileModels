...
#define FUSB_CURRENT_SET_500MA          500
...
static bool g_fusb_is_pd_ready;
static bool g_fusb_is_pd_sink_vbus_ready;
static int g_fusb_sink_cur;

void fusb_get_sink_ibus(int *cur)
{
	if (!cur)
		return;
	*cur = g_fusb_sink_cur;
		...
		enum dual_role_property prop, const unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_PR:
		...
		if (*val != fusb_get_dual_role_power()) {
...
{
	return g_fusb_is_pd_ready;
}

bool fusb_get_pd_sink_vbus_flag(void)
{
	return g_fusb_is_pd_sink_vbus_ready;
}
		...
		void *usr_ctx, void *app_ctx)
{
	...
	static FSC_U32 set_voltage = 0;
	static FSC_U32 op_current = 0;
	int type;
	static int polarity;
	...
	case CC2_ORIENT:
		...
		if (chip->port.sourceOrSink == SINK) {
			...
			tcpci_sink_vbus(chip->tcpc, TCP_VBUS_CTRL_TYPEC, TCPC_VBUS_SOURCE_5V,
				FUSB_CURRENT_SET_500MA);
	...
	case CC_NO_ORIENT:
		...
		dual_role_instance_changed(chip->dual_role);
		g_fusb_is_pd_sink_vbus_ready = false;
		g_fusb_sink_cur = 0;
		g_fusb_is_pd_ready = false;
		__pm_relax(&chip->fusb302_wakelock);
		break;
	...
	case PD_NEW_CONTRACT:
		...
		vbus_state.mv = set_voltage * FUSB_VOLTAGE_UNIT;
		vbus_state.ma = op_current * FUSB_CURRENT_UNIT;
		...
		g_fusb_is_pd_ready = true;
		g_fusb_is_pd_sink_vbus_ready = true;
		g_fusb_sink_cur = op_current * FUSB_CURRENT_UNIT;
		tcpci_notify_pd_state(chip->tcpc, PD_CONNECT_PE_READY_SNK);
		dual_role_instance_changed(chip->dual_role);
		break;
	case DATA_ROLE:
