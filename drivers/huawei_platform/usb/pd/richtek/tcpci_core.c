	...
	unsigned long event, void *data)
{
	...
	case PD_WAKE_LOCK:
		pr_info("%s=en\n", __func__);
		power_wakeup_lock(tcpc->attach_wake_lock, false);
		break;
	case PD_WAKE_UNLOCK:
		pr_info("%s=dis\n", __func__);
		power_wakeup_unlock(tcpc->attach_wake_lock, false);
		break;
	default:
	...
	struct tcpc_desc *tcpc_desc, struct tcpc_ops *ops, void *drv_data)
{
	...
	tcpc->attach_wake_lock = power_wakeup_source_register(&tcpc->dev, "tcpc_attach_wakelock");
	tcpc->dettach_temp_wake_lock = power_wakeup_source_register(&tcpc->dev, "tcpc_detach_wakelock");
...
void tcpc_device_unregister(struct device *dev, struct tcpc_device *tcpc)
{
	...
	power_wakeup_source_unregister(tcpc->dettach_temp_wake_lock);
	power_wakeup_source_unregister(tcpc->attach_wake_lock);
...
#ifndef CONFIG_HUAWEI_PD_THIRDPARTY
