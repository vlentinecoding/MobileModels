...
static void rt1711h_wake_lock(struct rt1711_chip *chip)
{
	...
	power_wakeup_lock(chip->rt1711h_wakelock, false);
}
...
static void rt1711h_wake_unlock(struct rt1711_chip *chip)
{
	...
	power_wakeup_unlock(chip->rt1711h_wakelock, false);
}
	...
	const struct i2c_device_id *id)
{
	...
	chip->rt1711h_wakelock = power_wakeup_source_register(chip->dev, "rt1711h_wakelock");
	...
	if (ret < 0) {
		power_wakeup_source_unregister(chip->rt1711h_wakelock);
