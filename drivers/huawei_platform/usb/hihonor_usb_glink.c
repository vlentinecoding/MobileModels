...
int hihonor_usb_glink_get_cable_type()
{
	...
	if (hihonor_oem_glink_oem_get_prop(USB_TYPEC_OEM_CABLE_TYPE, (void *)&type, sizeof(type))) {
...
int hihonor_usb_glink_check_cc_vbus_short()
{
	...
	if (hihonor_oem_glink_oem_get_prop(USB_TYPEC_OEM_CHECK_CC_VBUS_SHORT, (void *)&type, sizeof(type))) {
...
int hihonor_usb_glink_get_typec_sm_status()
{
	...
	if (hihonor_oem_glink_oem_get_prop(USB_TYPEC_OEM_TYPEC_SM, (void *)&typec_sm_status, sizeof(typec_sm_status))) {
...
static int hihonor_get_usb_role_sw(struct hihonor_usb_glink_dev_info *info)
{
	if (info->usb_role_sw)
		return 0;

	info->usb_role_sw = fwnode_usb_role_switch_get(dev_fwnode(info->dev));
	if (IS_ERR_OR_NULL(info->usb_role_sw)) {
		hwlog_err("%s(): Unable to find usb role switch\n", __func__);
		info->usb_role_sw = NULL;
		return -EINVAL;
	}
...
static void hihonor_set_usb_role(struct hihonor_usb_glink_dev_info *info, enum usb_role role)
{

	if (hihonor_get_usb_role_sw(info)) {
		return;
	}

	if (info->cur_usb_role == role)
		return;

	hwlog_info("%s: set usb role %d", __func__, role);
	info->cur_usb_role = role;
	usb_role_switch_set_role(info->usb_role_sw, role);
}

static void hihonor_usb_glink_notification(void *dev_data, u32 notification, void *data)
{
	...
	} else if (notification == OEM_NOTIFY_ICON_TYPE_QUICK) {
		charge_send_icon_uevent(ICON_TYPE_QUICK);
	} else if (notification == OEM_NOTIFY_USB_PLUG_IN) {
		hihonor_set_usb_role(info, USB_ROLE_DEVICE);
	} else if (notification == OEM_NOTIFY_CHARGER_PLUG_OUT) {
		hihonor_set_usb_role(info, USB_ROLE_NONE);
	}
...
static int hihonor_usb_glink_probe(struct platform_device *pdev)
{
	...
	dev_info = info;
	info->cur_usb_role = USB_ROLE_NONE;
