...
static inline int rt8979_toggle_bc12(struct fsa9685_device_info *chip)
{
	...
	for (i = 0; i <= READ_REG_MAX_TIMES; i++) {
		if (i == READ_REG_MAX_TIMES) {
			hwlog_err("%s read reg 3 times, fail\n", __func__);
			goto out;
		}
		...
		if (ret < 0) {
			msleep(50);
			hwlog_err("%s %dth read reg fail %d\n", __func__, i, ret);
		} else {
			hwlog_err("%s read reg succ\n", __func__);
			break;
		}
	}
...
static void rt8979_enable_bc12(struct fsa9685_device_info *chip, bool en)
{
	...
	/* when inster ,clear interrupt */
	ret = rt8979_i2c_read_byte(chip, RT8979_REG_INT1, &int_data);

	hwlog_info("%s ret =%d en = %d status %d\n", __func__, ret, en, int_data);
	mutex_lock(&chip->bc12_en_lock);
...
static int rt8979_inform_psy_chg_type_changed(struct fsa9685_device_info *di)
{
	int ret = 0;
	...
	psy = power_supply_get_by_name("huawei_charge");
	if (!psy) {
	...
	ret = power_supply_set_property(psy, POWER_SUPPLY_PROP_CHG_TYPE, &propval);
	if (ret < 0)
		hwlog_err("%s: psy type failed, ret = %d\n", __func__, ret);
	else
		hwlog_info("%s: chg_type = %d, di->chg_type = %d\n", __func__, propval.intval, di->chg_type);

	return ret;
}
#endif
#ifdef CONFIG_TYPEC_UCSI
static int rt8979_get_usb_role_sw(struct fsa9685_device_info *di)
{
	if (di->usb_role_sw) {
		hwlog_err("%s(): usb_role_sw is not null\n", __func__);
		return 0;
	}

	di->usb_role_sw = fwnode_usb_role_switch_get(dev_fwnode(di->dev));
	if (IS_ERR_OR_NULL(di->usb_role_sw)) {
		hwlog_err("%s(): Unable to find usb role switch\n", __func__);
		di->usb_role_sw = NULL;
		return -EINVAL;
	}
...
static void rt8979_inform_psy_dwork_handler(struct work_struct *work)
{
	...
	bool attach = false;
	enum charger_type chg_type = CHARGER_REMOVED;
	...
	/* Get chg type det power supply */
	if (!chip->psy)
		chip->psy = power_supply_get_by_name("usb");
	if (!chip->psy) {
...
#ifdef CONFIG_TYPEC_UCSI
	if (!rt8979_get_usb_role_sw(chip)) {
		hwlog_info("%s: attemp to set chg_type:%d\n", __func__, chip->chg_type);
		if (chip->chg_type == CHARGER_TYPE_USB
			|| chip->chg_type == CHARGER_TYPE_BC_USB)
			ret = usb_role_switch_set_role(chip->usb_role_sw, USB_ROLE_DEVICE);
		else if (chip->chg_type == CHARGER_REMOVED)
			ret = usb_role_switch_set_role(chip->usb_role_sw, USB_ROLE_NONE);
		if (ret)
			hwlog_err("failed to set role(%d):%d\n", __func__, ret);
	}
#endif
	...
	hihonor_charger_glink_set_charger_type(chip->chg_type);
#ifndef CONFIG_PMIC_AP_CHARGER
	rt8979_inform_psy_chg_type_changed(chip);
#endif
}
...
static irqreturn_t rt8979_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < 50; i++) { /* delay 500ms max */
		ret = rt8979_i2c_read_byte(chip, RT8979_REG_INT1, &int_data);
		if (ret == 0)
			break;
		mdelay(10);
...
int rt8979_init_early(struct fsa9685_device_info *chip)
{
	...
	usleep_range(5000, 5100); /* chip i2c ready */
...
int rt8979_init_later(struct fsa9685_device_info *chip)
{
	...
	chip->bc12_en_ws = wakeup_source_register(chip->dev, "rt8979_bc12_en_ws");
#else
