...
static inline bool is_adp_plugout(void)
{
	...
	return g_fsa9685_dev->chg_type == CHARGER_REMOVED;
}
...
static int rt8979_psy_online_changed(struct fsa9685_device_info *di)
{
	int ret = 0;
...
static int fsa9685_psy_chg_type_changed(struct fsa9685_device_info *di)
{
	int ret = 0;
	union power_supply_propval propval;
...
static int fsa9685_get_usb_role_sw(struct fsa9685_device_info *di)
{
	if (di->usb_role_sw)
		return 0;
...
static int rt8979_psy_chg_type_changed(struct fsa9685_device_info *di)
{
	...
	if (((di->chg_type == CHARGER_TYPE_USB) || (di->chg_type == CHARGER_TYPE_BC_USB))
		&& !fsa9685_get_usb_role_sw(di)) {
		ret = usb_role_switch_set_role(di->usb_role_sw, USB_ROLE_DEVICE);
		if (ret)
...
#ifndef CONFIG_PMIC_AP_CHARGER
	hihonor_charger_glink_set_charger_type(di->chg_type);
	fsa9685_psy_chg_type_changed(di);
#endif
	return ret;
}
...
#ifdef CONFIG_TYPEC_UCSI
	if (!fsa9685_get_usb_role_sw(di)) {
		ret = usb_role_switch_set_role(di->usb_role_sw, USB_ROLE_NONE);
		if (ret)
			hwlog_err("%s: set role err %d\n", __func__, ret);
	}
#endif
...
static int fsa9685_is_water_intruded(void *dev_data)
{
	u8 usb_id;
	int i;
	enum charger_type charger_type = mt_get_charger_type();
	...
	if ((NULL == di) || (NULL == di->client)) {
 	...
 	for (retry_cnt = 0; retry_cnt < 10; retry_cnt++) {
 		if (atomic_read(&di->pm_suspend)) {
 			hwlog_info(" %s irq_thread wait resume\n",__func__);
...
static int rt8979_accp_adapter_detect(void)
{
	...
	for (j = 0; j < ACCP_MAX_TRYCOUNT && vbus_present; j++) {
		...
		if (i == ACCP_DETECT_MAX_COUT) {
			...
			vbus_present = (fsa9685_read_reg(RT8979_REG_MUIC_STATUS1) &
				RT8979_REG_MUIC_STATUS1_DCDT) ? false : true;
			if (vbus_present && !is_adp_plugout())
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 157))
	di->usb_switch_lock = wakeup_source_register(di->dev, "usb_switch_wakelock");
#else
...
fail_free_wakelock:
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 157))
	wakeup_source_unregister(di->usb_switch_lock);
#else
...
static void fsa9685_complete(struct device *dev)
{
	struct fsa9685_device_info *di = dev_get_drvdata(dev);
...
static const struct dev_pm_ops fsa9685_pm_ops = {
	.suspend = fsa9685_suspend,
	.resume = fsa9685_resume,
	.complete = fsa9685_complete,
};
#else
static int fsa9685_resume(struct device *dev)
{
