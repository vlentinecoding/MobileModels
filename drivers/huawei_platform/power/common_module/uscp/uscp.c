...
static void uscp_protect_wake_lock(struct uscp_device_info *di)
{
	if (!di->protect_wakelock.active) {
		hwlog_info("protect wake lock\n");
		__pm_stay_awake(&di->protect_wakelock);
	}
}

static void uscp_protect_wake_unlock(struct uscp_device_info *di)
{
	if (di->protect_wakelock.active) {
		hwlog_info("protect wake unlock\n");
		__pm_relax(&di->protect_wakelock);
	}
}
...
static int uscp_dmd_report(unsigned int dmd_id, struct uscp_temp_info *temp_info)
{
	...
	hwlog_info("%s\n", dsm_buff);
	return power_dsm_dmd_report(POWER_DSM_USCP, dmd_id, dsm_buff);
}
#endif
static int uscp_get_usb_temp(struct uscp_device_info *di)
{
	return power_temp_get_average_value(di->usb_port_name) / POWER_TEMP_UNIT;
}
#ifndef CONFIG_HLTHERM_RUNTEST
...
#endif
#ifndef CONFIG_HLTHERM_RUNTEST
static int uscp_check_usb_temp(struct uscp_device_info *di)
{
	int temp = uscp_get_usb_temp(di);
	...
	struct uscp_temp_info *temp_info)
{
	...
	if ((tusb >= di->uscp_threshold_tusb) && (tdiff >= di->open_hiz_temp)) {
		...
		power_platform_set_charge_hiz(USCP_HIZ_ENABLE);
	}

	/* uscp happened: pull up mosfet */
	if ((tusb >= di->uscp_threshold_tusb) && (tdiff >= di->open_mosfet_temp)) {
		uscp_protect_wake_lock(di);
		...
		if (di->protect_mode) {
			...
			power_usleep(DT_USLEEP_10MS);
			power_platform_set_charge_hiz(USCP_HIZ_DISABLE);
			di->hiz_mode = false;
...
static void uscp_check_work(struct work_struct *work)
{
	...
	temp_info.usb_temp = uscp_get_usb_temp(di);
	temp_info.bat_temp = uscp_get_bat_temp();
...
static void uscp_charge_type_handler(struct uscp_device_info *di)
{
	...
	type = power_platform_get_charger_type();
...
static int uscp_parse_dts(struct device_node *np, struct uscp_device_info *di)
{
	...
	if (power_dts_read_string(power_dts_tag(HWLOG_TAG), np,
			"usb_port_name", &name)) {
		strncpy(di->usb_port_name, POWER_TEMP_USB_PORT,
			(USB_PORT_NAME_MAX_LEN - 1));
	} else {
		strncpy(di->usb_port_name, name,
			(USB_PORT_NAME_MAX_LEN - 1));
	}
	hwlog_info("get usb port temp from %s\n", di->usb_port_name);
...
static int uscp_probe(struct platform_device *pdev)
{
	...
	wakeup_source_init(&di->protect_wakelock, "uscp_protect_wakelock");
...
fail_free_wakelock:
	wakeup_source_trash(&di->protect_wakelock);
fail_free_gpio:
