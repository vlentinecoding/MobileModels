...
{
	struct mtk_battery_temp_info *info = (struct mtk_battery_temp_info *)dev_data;

	if (!data)
		return -1;

	mtk_battery_temp_get_batt_temp(info, adc_chnnel, data);
...
static struct platform_driver huawei_temp_battery_driver = {
...
{
	return platform_driver_register(&huawei_temp_battery_driver);
}

static void __exit mtk_battery_temp_exit(void)
{
	platform_driver_unregister(&huawei_temp_battery_driver);
}
