...
#define DYNAMIC_EOC_MAX_LEVEL    10
#define DYNAMIC_EOC_BUF_LIMIT    100
...
static bool g_dynamic_eoc_enable;
...
static int g_dynamic_eoc_buf_limit = DYNAMIC_EOC_BUF_LIMIT;
static int g_dynamic_eoc_level = 0;
static bool g_vdpm_first_run = true;
static struct charge_vdpm_data g_vdpm_data[VDPM_PARA_LEVEL];
static struct dynamic_eoc_data g_dynamic_eoc_data[DYNAMIC_EOC_MAX_LEVEL];
...
static int set_dynamic_eoc_data(int indx, long data)
{
	switch (indx % DYNAMIC_EOC_PARA_TOTAL) {
	case DYNAMIC_EOC_PARA_TEMP_MIN:
		g_dynamic_eoc_data[indx / DYNAMIC_EOC_PARA_TOTAL].min_temp = data;
		break;
	case DYNAMIC_EOC_PARA_TEMP_MAX:
		g_dynamic_eoc_data[indx / DYNAMIC_EOC_PARA_TOTAL].max_temp = data;
		break;
	case DYNAMIC_EOC_PARA_CC:
		g_dynamic_eoc_data[indx / DYNAMIC_EOC_PARA_TOTAL].cc= data;
		break;
	case DYNAMIC_EOC_PARA_CV:
		g_dynamic_eoc_data[indx / DYNAMIC_EOC_PARA_TOTAL].cv = data;
		break;
	case DYNAMIC_EOC_PARA_ITERM:
		g_dynamic_eoc_data[indx / DYNAMIC_EOC_PARA_TOTAL].iterm = data;
		break;
	default:
		chr_err("set dynamic_eoc_data failed\n");
		return -EINVAL;
	}
...
static int dynamic_eoc_para_parse(struct device_node *np)
{
	int ret;
	int i;
	int array_len;
	...
	ret = of_property_read_u32(np, "dynamic_eoc_buf_limit", &g_dynamic_eoc_buf_limit);
	if (ret) {
		chr_err("get dynamic_eoc_buf_limit failed\n");
		g_dynamic_eoc_buf_limit = VDPM_DELTA_LIMIT;
	}
	chr_info("dynamic_eoc_buf_limit = %d\n", g_dynamic_eoc_buf_limit);

	array_len = of_property_count_strings(np, "dynamic_eoc_para");
	if ((array_len <= 0) || ((array_len % DYNAMIC_EOC_PARA_TOTAL) != 0)) {
		chr_err("dynamic_eoc_para is invaild, please check dynamic_eoc_para number\n");
		return -EINVAL;
	}
	g_dynamic_eoc_level = array_len / DYNAMIC_EOC_PARA_TOTAL;

	for (i = 0; i < array_len; i++) {
		ret = of_property_read_string_index(np, "dynamic_eoc_para",
			i, &chrg_data_string);
		if (ret) {
			chr_err("get dynamic_eoc_para failed\n");
			return -EINVAL;
		}
		/* 10 means decimalism */
		ret = kstrtol(chrg_data_string, 10, &data);
		...
		ret = set_dynamic_eoc_data(i, data);
		if (ret)
			break;

		chr_info("dynamic_eoc_para %d:%d = %ld\n",
			(i / DYNAMIC_EOC_PARA_TOTAL), (i % DYNAMIC_EOC_PARA_TOTAL), data);
	}

	return ret;
}

void set_eoc_by_temp(struct charger_manager *info)
{
	int i;
	int charge_current;
	int iterm, cv_value;
	...
	if (!g_dynamic_eoc_enable) {
		return;
	}
	if (charger_dev_get_charging_current(info->chg1_dev, &charge_current)) {
		chr_err("get charging current fail!");
		return;
	}
	for (i = 0; i < g_dynamic_eoc_level; i++) {
		if (batt_temp < g_dynamic_eoc_data[i].min_temp
			|| batt_temp >= g_dynamic_eoc_data[i].max_temp) {
			continue;
		}
		if (charge_current <= g_dynamic_eoc_data[i].cc) {
			iterm = g_dynamic_eoc_data[i].iterm;
			cv_value = g_dynamic_eoc_data[i].cv;
			charger_dev_set_eoc_current(info->chg1_dev, iterm);
			charger_dev_set_constant_voltage(info->chg1_dev, cv_value);
			chr_info("set dynamic eoc, temp: %d, cc: %d, iterm: %d, cv: %d\n",
				batt_temp, charge_current, iterm, cv_value);
		}
		break;
	}
}
...
void charger_parse_dt(struct device_node *np, struct charger_manager *info)
{
	...
	g_dynamic_eoc_enable = of_property_read_bool(np, "dynamic_eoc_enable");
	chr_err("read dynamic_eoc_enable: %d\n", g_dynamic_eoc_enable);
	if (g_dynamic_eoc_enable) {
		if (dynamic_eoc_para_parse(np)) {
			g_dynamic_eoc_enable = false;
			chr_info("call dynamic_eoc_para_parse failed\n");
		}
	}
