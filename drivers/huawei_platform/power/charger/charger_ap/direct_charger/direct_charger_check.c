...
/* for OB adapter */
#define OB_VOLTAGE_ERROR_THRESHOLD  410
	...
	struct direct_charge_device *di)
{
	...
	/* Set vol_err_th to 410, if adapter vendor is OB */
	if (di->adaptor_vendor_id == SCP_PROTOCOL_VENDOR_ID_ON_BRIGHT)
		vol_err_th = OB_VOLTAGE_ERROR_THRESHOLD;
...
void direct_charge_check(int hv_flag)
{
	...
	dc_set_adapter_usbpd_enable(false);
