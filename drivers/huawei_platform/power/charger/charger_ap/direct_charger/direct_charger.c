...
int direct_charge_get_bat_current(int *value)
{
	...
	if (dc_get_bat_current_with_calibration(l_di->working_mode, l_di->cur_mode, value)) {
		dc_fill_eh_buf(l_di->dsm_buff, sizeof(l_di->dsm_buff),
			DC_EH_GET_IBAT, NULL);
		direct_charge_set_stop_charging_flag(1);
		*value = 0;
...
void direct_charge_select_charging_param(void)
{
	...
	bat_temp_get_temperature(l_di->tbat_id, &tbat);
...
static int direct_charge_get_vstep(struct direct_charge_device *di)
{
	...
	if (dc_get_bat_current_with_calibration(di->working_mode, di->cur_mode, &ibat))
		return di->vstep;
...
static void direct_charge_get_ffc_para(struct direct_charge_device *di)
{
	...
	vbat_main = dc_get_bat_btb_voltage_with_comp(di->working_mode, CHARGE_IC_MAIN, di->comp_para.vbat_comp);
	vbat_aux = dc_get_bat_btb_voltage_with_comp(di->working_mode, CHARGE_IC_AUX, di->comp_para.vbat_comp);
...
void cam_stop_charging(void)
{
	...
	dc_set_adapter_voltage(5000); /* reset to 5000mv */
