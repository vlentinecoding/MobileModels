...
void direct_charge_set_start_time(void)
{
	...
	if (hsc_di)
		hsc_di->direct_charge_start_time = cur_time;
}
...
unsigned int direct_charge_update_local_mode(unsigned int mode)
{
	...
	if (hsc_di) {
		if ((hsc_di->sysfs_enable_charger == 0) &&
			!(hsc_di->sysfs_mainsc_enable_charger ^ hsc_di->sysfs_auxsc_enable_charger)) {
			local_mode &= ~HSC_MODE;
			hwlog_info("disable sc mode, local_mode %u\n",
				local_mode);
		}
...
int direct_charge_is_failed(void)
{
	...
	if (hsc_di) {
		if (hsc_di->error_cnt >= DC_ERR_CNT_MAX) {
			hwlog_info("sc error exceed retry max\n");
			return 1;
		} else {
			return 0;
...
static int direct_charge_get_bat_voltage(int *value)
{
	...
	if (l_di->dummy_vbat) {
		*value = l_di->dummy_vbat;
		hwlog_err("set dummy vbat is %d", l_di->dummy_vbat);
		return 0;
	}

	/* last cc&cv use the higher vbat */
	if (l_di->use_higher_vbat && (l_di->cur_stage >= (DOUBLE_SIZE * l_di->stage_size - 2)))
		btb_vol = dc_get_bat_max_btb_voltage_with_comp(l_di->working_mode,
...
int direct_charge_get_bat_current(int *value)
{
	...
	if (dc_get_bat_current_with_calibration(l_di->working_mode, l_di->cur_mode,
		value, l_di->multi_ic_check_info.ibat_comb)) {
		dc_fill_eh_buf(l_di->dsm_buff, sizeof(l_di->dsm_buff),
			DC_EH_GET_IBAT, NULL);
		direct_charge_set_stop_charging_flag(1);
		*value = 0;
...
int direct_charge_switch_path_to_normal_charging(void)
{
	...
	return dc_open_wired_channel();
}

/* switch charging path to lvc or sc charging path */
int direct_charge_switch_path_to_dc_charging(int hv_flag)
{
	struct direct_charge_device *l_di = direct_charge_get_di();
...
static int direct_charge_redetect_adapter_by_voltage(unsigned int mode)
{
	...
	if (!(mode & (LVC_MODE | SC_MODE | HSC_MODE)))
		return mode;

	/* we return the raw mode if local mode not support lvc or sc */
	if (!(local_mode & (LVC_MODE | SC_MODE | HSC_MODE)))
		return mode;
...
static void direct_charge_disable_hiz_in_exit(struct direct_charge_device *di)
{
	if (!di || !di->sc_to_bulk_delay_hiz)
		return;

	cancel_delayed_work_sync(&di->hiz_mode_work);
...
void direct_charge_exit(void)
{
	...
	direct_charge_reset_para_in_exit(hsc_di);

	direct_charge_disable_hiz_in_exit(lvc_di);
	direct_charge_disable_hiz_in_exit(sc_di);
	direct_charge_disable_hiz_in_exit(hsc_di);
}
...
void direct_charge_update_cutoff_normal_flag(void)
{
	...
	if (hsc_di)
		direct_charge_reset_para_in_stop(hsc_di);
}
...
void direct_charge_select_comp_param(struct direct_charge_device *di)
{
	int main_ic_id;
	int aux_ic_id;
	int ic_mode = di->cur_mode == CHARGE_MULTI_IC ? 1 : 0;
	...
	if (di->multi_ic_mode_para.support_multi_ic) {
		...
		dc_get_vbat_comp_para(main_ic_id, aux_ic_id, ic_mode, &di->comp_para);
		memcpy(di->multi_ic_check_info.vbat_comp, di->comp_para.vbat_comp,
			sizeof(di->comp_para.vbat_comp));
	}
}

static void direct_charge_get_cur_ibat_th_low(struct direct_charge_device *di, int tbat)
{
	int i;

	di->cur_ibat_th_low = di->volt_para[di->cur_stage / 2].cur_th_low;

	if (di->cur_stage != (DOUBLE_SIZE * di->stage_size - 1) ||
		(di->temp_cv_para[0].cv_curr == 0)) {
		return;
	}

	for (i = 0; i < DC_TEMP_LEVEL; ++i) {
		if (tbat >= di->temp_cv_para[i].temp_min &&
			tbat < (di->temp_cv_para[i].temp_max) &&
			di->temp_cv_para[i].cv_curr > di->cur_ibat_th_low)
			di->cur_ibat_th_low = di->temp_cv_para[i].cv_curr;
	}
}
...
void direct_charge_select_charging_param(void)
{
	...
	direct_charge_get_cur_ibat_th_low(l_di,tbat);
...
static int direct_charge_get_vstep(struct direct_charge_device *di)
{
	...
	if (dc_get_bat_current_with_calibration(di->working_mode, di->cur_mode,
	...
	if (!power_cmdline_is_factory_mode() &&
		...
		dc_get_adapter_type() != ADAPTER_TYPE_20V5A))
		return di->vstep;
...
static void direct_charge_regulation(void)
{
	...
	iadp = iadp > l_di->ls_ibus ? iadp : l_di->ls_ibus;
...
bool direct_charge_is_priority_inversion(void)
{
	...
	if (lvc_di->pri_inversion || sc_di->pri_inversion || hsc_di->pri_inversion)
		return true;
...
static void direct_charge_update_multi_info(struct direct_charge_device *di)
{
	...
	for (i = 0; i < CHARGE_IC_MAX_NUM; i++) {
		...
		di->curr_info.vbat[i] = dc_get_bat_btb_voltage_with_comp(di->working_mode,
...
void direct_charge_control_work(struct work_struct *work)
{
	...
	if (di->is_align_caculation_work) {
...
void direct_charge_stop_charging(void)
{
	...
	if (l_di->scp_work_on_charger) {
		...
		if (l_di->use_force_sleep) {
			charge_enable_force_sleep(HIZ_MODE_DISABLE);
		} else if (l_di->sc_to_bulk_delay_hiz &&
				(direct_charge_get_stage_status() == DC_STAGE_CHARGE_DONE)) {
			hwlog_info("sc to bulk, delay %d to close hiz mode\n", l_di->delay_hiz_time);
			schedule_delayed_work(&l_di->hiz_mode_work,
				msecs_to_jiffies(l_di->delay_hiz_time));
		} else {
			charge_enable_hz(HIZ_MODE_DISABLE);
		}
	}
	...
	dc_switch_charging_path(PATH_NORMAL, 1);
	...
	dc_set_adapter_usbpd_enable(true);
	...
	power_event_notify(POWER_NT_DC, POWER_NE_DC_STOP_CHARGE, NULL);
