...
static int dummy_ops_register2(struct dc_ic_ops *ops)
{
	return 0;
}
static int dummy_ops_register3(struct dc_batinfo_ops *ops)
{
	return 0;
}
#endif
...
int hi6526_set_input_current(u32 cin_limit)
{
	u8 i;
	int max;
	u8 i_in_limit = 0;
...
static int hi6526_get_charge_current(u32 *ichg)
{
	int ret;
	u8 reg = 0;
	int charge_current;
...
int hi6526_set_charge_current(u32 charge_current)
{
	u8 i_chg_limit;
	/* Chip limit */
 ...
 */
int hi6526_set_terminal_voltage(u32 charge_voltage)
{
	u8 data;
	struct hi6526_device_info *di = g_hi6526_dev;
 ...
 */
int hi6526_set_terminal_current(u32 term_current)
{
	u8 i_term;
...
int hi6526_set_charge_enable_execute(bool enable)
{
	struct hi6526_device_info *di = g_hi6526_dev;
...
int hi6526_set_charge_enable(bool enable)
{
...
int hi6526_set_otg_current(u32 value)
{
...
int hi6526_set_otg_enable(bool enable)
{
	return hi6526_set_otg_enable_exeute(enable);
}

int hi6526_set_term_enable(bool enable)
{
	...
	char *reg_value, int size)
{
	...
	if (hi6526_get_vbus_mv((unsigned int *)&vbus))
	...
	val_array[5] = hi6526_get_vbat();
	hi6526_get_ibus_ma(&ibus);
...
static int hi6526_dump_reg_value(char *reg_value, int size)
{
	u8 reg_val = 0;
	int i;
	int ret = 0;
...
static int hi6526_reg_head(char *reg_head, int size)
{
...
{
	return  hi6526_reg_head(buffer, size);
}

static int hi6526_dump_reg(char *buffer, int size, void *dev_data)
{
	return hi6526_dump_reg_value(buffer, size);
}
#if 0
...
static int hi6526_set_charger_hiz(bool enable)
{
...
int hi6526_dummy_fun_2(u32 val)
{
...
static void hi6526_fcp_scp_ops_register(void)
{
	...
	if (hi6526_is_support_scp() == 0) {
		 ...
		 */
		ret[0] = 0;
		ret[1] = dc_ic_ops_register(LVC_MODE, CHARGE_IC_TYPE_MAIN,
			&hi6526_lvc_ops);
		ret[2] = dc_ic_ops_register(SC_MODE, CHARGE_IC_TYPE_MAIN,
			&hi6526_sc_ops);
			...
			const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		scharger_err("honor_charger_type_notifier_register failed\n");
	...
	hi6526_log_ops.dev_data = g_hi6526_dev;
	...
	ret = hi6526_chip_init(&init_crit);
	if (ret)
...
hi6526_create_sysfs_fail:
	...
	if (ret < 0)
		scharger_err("honor_charger_type_notifier_unregister failed\n");
