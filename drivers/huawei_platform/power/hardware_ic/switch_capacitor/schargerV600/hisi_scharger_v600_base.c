...
int hi6526_get_adc_value(struct hi6526_device_info *di, u32 chan, u32 *data)
{
	...
	if (lvc_mode || sc_mode) {
		...
		ret0 = hi6526_read(di, CHG_ADC_DATA_L_REG + (chan * 2), &adc_data[0]);
		ret1 = hi6526_read(di, CHG_ADC_DATA_H_REG + (chan * 2), &adc_data[1]);
		/* 8: reg len is 8 */
	...
	ret2 = hi6526_adc_loop_enable(di, CHG_ADC_DIS);
	ret3 = hi6526_set_adc_channel(di, chan);
	ret4 = hi6526_adc_enable(di, CHG_ADC_EN);

	ret5 = hi6526_write_mask(di, CHG_ADC_START_REG, CHG_ADC_START_MSK,
	...
	/* 8: reg len */
	*data = (u32)adc_data[0] | ((u32)(adc_data[1] & 0x3f) << 8);
	ret2 = hi6526_adc_enable(di, CHG_ADC_DIS);
	ret = (ret0 || ret1 || ret2);
	if (ret) {
...
void parse_dts(struct device_node *np, struct hi6526_device_info *di)
{
	...
	if (ret) {
		di->param_dts.ic_role = CHARGE_IC_TYPE_MAIN;
...
static int hi6526_fcp_cmd_transfer_check(struct hi6526_device_info *di)
{
	...
	do {
		...
		ret0 = hi6526_read(di, CHG_FCP_ISR1_REG, &reg_val1);
		ret1 = hi6526_read(di, CHG_FCP_ISR2_REG, &reg_val2);
		ret2 = hi6526_read(di, CHG_FCP_IRQ3_REG, &reg_val3);
		ret3 = hi6526_read(di, CHG_FCP_IRQ4_REG, &reg_val4);
	...
	void *dev_data)
{
	...
	for (i = 0; i < FCP_RETRY_TIME; i++) {
		...
		ret1 = hi6526_read(di, CHG_FCP_ISR2_REG, &reg_val2);
		if (reg_val1 != 0)
			ret2 = hi6526_write(di, CHG_FCP_ISR1_REG, reg_val1);
		if (reg_val2 != 0)
			ret3 = hi6526_write(di, CHG_FCP_ISR2_REG, reg_val2);

		ret4 = hi6526_write(di, CHG_FCP_CMD_REG, CHG_FCP_CMD_MBRRD);
		ret5 = hi6526_write(di, CHG_FCP_ADDR_REG, reg);
		ret6 = hi6526_write(di, CHG_FCP_LEN_REG, data_len);
		ret7 = hi6526_write_mask(di, CHG_FCP_CTRL_REG, CHG_FCP_SNDCMD_MSK,
...
int hi6526_fcp_adapter_reg_read(u8 *val, u8 reg)
{
	...
	for (i = 0; i < FCP_RETRY_TIME; i++) {
		...
		ret4 = hi6526_write(di, CHG_FCP_CMD_REG, CHG_FCP_CMD_SBRRD);
		ret5 = hi6526_write(di, CHG_FCP_ADDR_REG, reg);
		ret6 = hi6526_write_mask(di, CHG_FCP_CTRL_REG, CHG_FCP_SNDCMD_MSK,
			CHG_FCP_SNDCMD_SHIFT, CHG_FCP_EN);
...
int hi6526_fcp_adapter_reg_write(u8 val, u8 reg)
{
	...
	for (i = 0; i < FCP_RETRY_TIME; i++) {
		...
		ret4 = hi6526_write(di, CHG_FCP_CMD_REG, CHG_FCP_CMD_SBRWR);
		ret5 = hi6526_write(di, CHG_FCP_ADDR_REG, reg);
		ret6 = hi6526_write(di, CHG_FCP_WDATA_REG, val);
		ret7 = hi6526_write_mask(di, CHG_FCP_CTRL_REG, CHG_FCP_SNDCMD_MSK,
			CHG_FCP_SNDCMD_SHIFT, CHG_FCP_EN);
...
static int hi6526_fcp_ping_success(struct hi6526_device_info *di)
{
	...
	for (i = 0; i < CHG_FCP_DETECT_MAX_COUT; i++) {
		ret = hi6526_read(di, CHG_FCP_STATUS_REG, &reg_val);
		vbus_uvp = hi6526_get_vbus_uvp_state(di);
...
int hi6526_chip_init(struct charger_device *dev, struct chip_init_crit *init_crit)
{
	...
	case CHARGER_TYPE_WIRELESS:
		hi6526_vusb_uv_det_enable(0, di);
		break;
	default:
		hi6526_vusb_uv_det_enable(1, di);
		break;
	}
	/*
