...
#include "bq27z561_fg.h"

#ifdef HWLOG_TAG
#undef HWLOG_TAG
#endif

#define HWLOG_TAG bq27z561_fg
HWLOG_REGIST();
...
static int bq27z561_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	struct bq27z561_fg_display_data g_dis_data;
...
static void bq27z561_get_single_float_body(struct spf *spf, struct fep *fe, char *res, int size)
{
	...
	t = fe->first_one_pos - fe->dec_point_pos;
	if (t > 0) {
		spf->exponent = 0x7F & ~t; /* set the exponential byte */
	} else {
		t = -t;
		if (t == 1)
			spf->exponent = 0x7F; /* set default exponential byte */
		else
			spf->exponent = 0x80 | (0x7F & (t - 2)); /* set exponential byte */
	}
...
static ssize_t bq27z561_fg_show_attrs(struct device *,
	struct device_attribute *, char *);
static ssize_t bq27z561_fg_store_attrs(struct device *,
	struct device_attribute *, const char *, size_t count);

#define bq27z561_fg_attr(_name) \
{ \
	.attr = { .name = #_name, .mode = 0664 }, \
	.show = bq27z561_fg_show_attrs, \
	.store = bq27z561_fg_store_attrs, \
}

static struct device_attribute bq27z561_fuelgauge_attrs[] = {
	bq27z561_fg_attr(flash_mode),
	bq27z561_fg_attr(rom_mode),
	bq27z561_fg_attr(curr_gain),
	bq27z561_fg_attr(vol_gain),
};
...
static ssize_t bq27z561_fg_show_attrs(struct device *dev,
...
static ssize_t bq27z561_fg_store_attrs(struct device *dev,
...
static int bq27z561_fg_create_attrs(struct device *dev)
{
	int i;
	int rc = 0;
	...
	struct bq27z561_device_info *di)
{
	...
	batt_ntc_parse_para(dev->of_node);
...
static int bq27z561_fg_probe(struct i2c_client *client,
	const struct i2c_device_id *id)
{
	...
	bq27z561_fg_create_attrs(di->dev);
...
static int bq27z561_fg_remove(struct i2c_client *client)
{
...
static int bq27z561_fg_suspend(struct device *dev)
{
...
static int bq27z561_fg_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct bq27z561_device_info *di = i2c_get_clientdata(client);
...
static const struct dev_pm_ops bq27z561_fg_pm_ops = {
	.suspend = bq27z561_fg_suspend,
	.resume = bq27z561_fg_resume,
};
...
static const struct i2c_device_id bq27z561_fg_id[] = {
	{ "bq27z561", BQ27Z561 }, {},
};

static struct i2c_driver bq27z561_fg_driver = {
	.probe = bq27z561_fg_probe,
	.remove = bq27z561_fg_remove,
	.id_table = bq27z561_fg_id,
	.driver = {
		   .owner = THIS_MODULE,
		   .name = "bq27z561_fg",
		   .of_match_table = of_match_ptr(bq27z561_of_match),
		   .pm = &bq27z561_fg_pm_ops,
	},
};

module_i2c_driver(bq27z561_fg_driver);
