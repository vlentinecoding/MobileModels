...
#endif
	struct work_struct fcc_check_work;
	struct delayed_work register_pd_nb_work;
	int alert_irq;
	int capacity;
	int soc_offset;
	u8 online:1;
...
static void set_ocv_cali_cur_threshold(struct rt9426_chip *chip)
{
	int regval;

	if (rt9426_unseal_wi_retry(chip) == RT9426_UNSEAL_FAIL) {
		dev_err(chip->dev, "%s: Unseal fail. Unable to set_ocv_cali_cur_threshold\n", __func__);
		return;
	}
	if (chip->pdata->ocv_cali_cur_threshold &&
		(chip->pdata->ocv_cali_cur_threshold != RT9426_OCV_CALI_CUR_THRESHOLD)) {
		rt9426_write_page_cmd(chip, RT9426_PAGE_1);
		rt9426_reg_write_word(chip->i2c, RT9426_REG_SWINDOW8,
			chip->pdata->ocv_cali_cur_threshold);
	}

	usleep_range(5000, 5100); /* delay 5ms */
	rt9426_read_page_cmd(chip, RT9426_PAGE_1);
	regval = rt9426_reg_read_word(chip->i2c, RT9426_REG_SWINDOW8);
	dev_info(chip->dev, "%s: page1 ExtReg4E is 0x%x\n", __func__, regval);
	return;
}
...
out_apply:
	set_ocv_cali_cur_threshold(chip);
	chip->online = 1;
	return 0;
}
...
static int rt_parse_dt(struct device *dev, struct rt9426_platform_data *pdata)
{
	...
	if (ret < 0) {
        ...
        ret = of_property_read_u32(child_node, "rt,ocv_cali_cur_threshold", &pdata->ocv_cali_cur_threshold);
        if (ret < 0) {
                dev_notice(dev, "no ocv_cali_cur_threshold, use default 0\n");
                pdata->ocv_cali_cur_threshold = RT9426_OCV_CALI_CUR_THRESHOLD;
        }
...
static void rt9426_fcc_check_work(struct work_struct *work)
{
	int i;
	int fcc;
	int dc;
	...
	for (i = 0; i < 3; i++) {
		dc = rt9426_reg_read_word(chip->i2c, RT9426_REG_DSNCAP);
		fcc = rt9426_reg_read_word(chip->i2c, RT9426_REG_FCC);

		if (fcc < ((u64)dc * 103 / 100)) {
			dev_info(chip->dev, "FCC normal\n");
			break;
		}
		...
		if (i >= 2) {
			...
			rt9426_read_page_cmd(chip, RT9426_PAGE_2);
			fcc_gain = rt9426_reg_read_word(chip->i2c, RT9426_REG_SWINDOW5);

			if (fcc_gain > 0x41EB) { /* fcc gain: 1.03 * 16384 */
				rt9426_write_page_cmd(chip, RT9426_PAGE_1);
				rt9426_reg_write_word(chip->i2c, RT9426_REG_SWINDOW4,
					((chip->pdata->op_config[3]) | 0x0040));
				rt9426_write_page_cmd(chip, RT9426_PAGE_2);
				rt9426_reg_write_word(chip->i2c, RT9426_REG_SWINDOW5, 0x4000);
				rt9426_write_page_cmd(chip, RT9426_PAGE_1);
				rt9426_reg_write_word(chip->i2c, RT9426_REG_SWINDOW4,chip->pdata->op_config[3]);
				rt9426_reg_write_word(chip->i2c, RT9426_REG_DUMMY, 0x0000);
				usleep_range(5000, 5100); /* delay 5ms */
				dev_info(chip->dev, "FCC Gain abnormal, Correct FCC Gain\n");
			} else {
				dev_info(chip->dev, "FCC Gain normal, FCC abnormal\n");
			}
		}
	}
}
...
#ifdef CONFIG_TCPC_CLASS
static bool rt9426_is_charger_plug_in(struct tcp_ny_typec_state typec_state)
{
	if ((typec_state.old_state == TYPEC_UNATTACHED ||
		typec_state.old_state == TYPEC_ATTACHED_NORP_SRC) &&
		(typec_state.new_state == TYPEC_ATTACHED_SNK ||
		    typec_state.new_state == TYPEC_ATTACHED_CUSTOM_SRC))
		    return true;

	if (typec_state.old_state == TYPEC_UNATTACHED &&
		typec_state.new_state == TYPEC_ATTACHED_NORP_SRC) {
		return true;
	}
...
static int rt9426_pd_tcp_notifier_call(struct notifier_block *pnb,
	unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_TYPEC_STATE:
		if (rt9426_is_charger_plug_in(noti->typec_state)) {
			pr_info("%s USB Plug in, pol = %d\n", __func__,
				noti->typec_state.polarity);
			schedule_work(&chip->fcc_check_work);
		}
		break;
	}
	return NOTIFY_OK;
...
static void rt9426_register_pd_nb_work(struct work_struct *work)
{
	int ret;
	struct rt9426_chip *chip = g_rt9426_chip;

	if (!chip) {
		hwlog_info("%s: di null\n", __func__);
		return;
	}

	chip->tcpc_dev = tcpc_dev_get_by_name("type_c_port0");
	if (chip->tcpc_dev == NULL) {
		hwlog_info("%s: tcpc device not ready fail\n", __func__);
		return;
	}

	chip->pd_nb.notifier_call = rt9426_pd_tcp_notifier_call;
	ret = register_tcp_dev_notifier(chip->tcpc_dev,
		&chip->pd_nb, TCP_NOTIFY_TYPE_ALL);
	if (ret < 0) {
		hwlog_info("%s: register tcpc notifer fail, schedule register work\n", __func__);
	}
}

static void rt9426_init_fcc_check_work(struct rt9426_chip *chip)
{
	INIT_DELAYED_WORK(&chip->register_pd_nb_work, rt9426_register_pd_nb_work);
	INIT_WORK(&chip->fcc_check_work, rt9426_fcc_check_work);
	schedule_delayed_work(&chip->register_pd_nb_work, msecs_to_jiffies(30000)); /* delay 30s */
}
...
#ifdef CONFIG_TCPC_CLASS
	rt9426_init_fcc_check_work(chip);
#endif /* CONFIG_TCPC_CLASS */
