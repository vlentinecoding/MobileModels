...
{
	rt9426_reg_write_word(chip->i2c, RT9426_REG_TEMP, 0x0041); /* 6.5 K */
	rt9426_reg_write_word(chip->i2c, RT9426_REG_DUMMY, 0); /* reset */
}
...
static int rt9426_fg_get_offset(struct rt9426_chip *chip, int soc_val, int temp)
{
	...
	struct data_point *sub_mesh = (struct data_point *)malloc(sizeof(struct data_point) * ip_x * ip_y);
	if (!sub_mesh) {
		dev_notice(chip->dev, "malloc return null\n");
		return -1;
	}
	memset(sub_mesh, 0, sizeof(struct data_point) * ip_x * ip_y);
...
static struct coul_interface_ops rt9426_ops = {
	...
	.get_battery_last_capacity = rt9426_read_battery_soc,
};
