...
#define RT9426_COUL_OFFSET_VAL     1000
#define RT9426_TBATICAL_MIN_B      (-20000)
#define RT9426_TBATICAL_MAX_B      20000
...
static int rt9426_block_read(struct i2c_client *i2c, u8 reg, int len, void *dst)
{
	...
	int ret;
	char info[RT9426_WRITE_INFO_LEN] = {0};

	ret = rt9426_read_device(i2c, reg, len, dst);
	if (ret < 0) {
		dev_notice(chip->dev, "rt9426 block read 0x%02x fail\n", reg);
		snprintf(info, RT9426_WRITE_INFO_LEN, "RT9426 fuel gauge communication fail");
...
static int rt9426_reg_read_word(struct i2c_client *i2c, u8 reg)
{
	...
	if (!power_i2c_get_status()) {
		pr_err("reg_read_word, i2c not ready\n");
		return -1;
	}
...
{
	if (!power_i2c_get_status()) {
		pr_err("reg_write_word, i2c not ready\n");
		return -1;
	}
...
static int rt9426_dump_log_data(char *buffer, int size, void *dev_data)
{
	...
	fcc = rt9426_reg_read_word(di->i2c, RT9426_REG_FCC) * di->pdata->rs_ic_setting / RT9426_FULL_CAPCACITY;
	flag2 = rt9426_reg_read_word(di->i2c, RT9426_REG_FLAG2);
	val70 = rt9426_reg_read_word(di->i2c, 0x70);
	val71 = rt9426_reg_read_word(di->i2c, 0x71);
...
static irqreturn_t rt9426_irq_handler(int irqno, void *param)
{
	...
	for (retry_cnt = 0; retry_cnt < 10; retry_cnt++) { /* 10: try times */
		if (atomic_read(&chip->pm_suspend)) {
			msleep(10); /* 10: wait resume 10 ms */
		} else {
			break;
		}
	}
...
static bool rt9426_is_need_update_pdata(struct rt9426_chip *chip)
{
	...
	/* mask high bits */
	if ((regval & 0x7FFF) != chip->pdata->rt9426_config_ver)
		return true;
	/* 2nd check dc */
...
static int rt9426_force_apply_data(struct rt9426_chip *chip)
{
	int i;
	int j;
	int ret;
...
init_start:
	...
	if (regval < 0)
		return -1;
	if ((regval & 0x7FFF) != chip->pdata->rt9426_config_ver) {
		regval = (regval & 0x8000) | chip->pdata->rt9426_config_ver;
...
static int rt9426_apply_pdata(struct rt9426_chip *chip)
{
	...
	ret = rt9426_force_apply_data(chip);
	if (ret)
		return ret;
...
static int rt9426_parse_dt(struct device *dev, struct rt9426_platform_data *pdata)
{
	...
	ret = of_property_read_u32_array(child_node, "rt,dtsi_version", pdata->dtsi_version, RT9426_DTSI_VER_SIZE);
	if (ret < 0)
	...
	ret = of_property_read_u32_array(child_node, "rt,offset_interpolation_order",
	...
	sizes[0] = sizes[1] = 0;
	ret = of_property_read_u32_array(child_node, "rt,soc_offset_size", sizes, RT9426_SOC_OFFSET_SIZE);
	if (ret < 0)
	...
	ret = of_property_read_u32(child_node, "rt,extreg_size", &pdata->extreg_size);
	if (ret < 0)
	...
	if (pdata->extreg_update.extreg_update_data) {
		...
		of_property_read_u32_array(child_node, "rt,extreg_data",
	...
	ret = of_property_read_u32(child_node, "rt,battery_type", &pdata->battery_type);
	if (ret < 0) {
		dev_info(dev, "uset default battery_type 4350mV, EDV=3200mV\n");
		pdata->battery_type = 4352;
	}

	ret = of_property_read_u32(child_node, "rt,volt_source", &pdata->volt_source);
	if (ret < 0)
	...
	ret = of_property_read_u32(child_node, "rt,temp_source", &pdata->temp_source);
	if (ret < 0)
		pdata->temp_source = 0;
	...
	ret = of_property_read_u32(child_node, "rt,fg_otc_tth", &pdata->otc_tth);
	if (ret < 0) {
	...
	ret = of_property_read_u32(child_node, "rt,fg_otd_tth", &pdata->otd_tth);
	if (ret < 0) {
		dev_notice(dev, "no otd_tth property, use default 0x0064\n");
		pdata->otd_tth = RT9426_OTD_ITH_DEFAULT_VAL;
	}

	ret = of_property_read_u32(child_node, "rt,fg_otd_dchg_ith",
		&pdata->otd_dchg_ith);
	if (ret < 0) {
	...
	ret = of_property_read_u32(child_node, "rt,fg_uvov_threshold",
	...
	ret = of_property_read_u32(child_node, "rt,fg_us_threshold",
		&pdata->us_threshold);
	if (ret < 0) {
	...
	ret = of_property_read_u32(child_node, "rt,design_capacity",
	...
	ret = of_property_read_u32(child_node, "rt,fcc", &pdata->fcc);
	if (ret < 0) {
		dev_notice(dev, "no FCC property, use defaut 2000\n");
		pdata->fcc = RT9426_DESIG_FCC_VAL;
	}

	ret = of_property_read_u32_array(child_node, "rt,fg_ocv_table",
	...
	ret = of_property_read_u32_array(child_node, "rt,fg_op_config",
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "rt9426_config_ver",
		&pdata->rt9426_config_ver, RT9426_DRIVER_VER);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "rt9426_flag",
		&pdata->flag, RT9426_DEFAULT_FLAG);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "cutoff_vol",
		&pdata->cutoff_vol, RT9426_DEFAULT_FLAG);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np, "cutoff_cur",
		&pdata->cutoff_cur, RT9426_DEFAULT_FLAG);
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
void rt9426_update_batt_param(int type, const char *brand)
{
	int ret = 0;

	if (!g_rt9426_chip) {
		hwlog_err("g_rt9426_chip is null, return fail\n");
		return;
	}

	ret |= rt9426_parse_dt(g_rt9426_chip->dev, g_rt9426_chip->pdata);
	ret |= rt9426_force_apply_data(g_rt9426_chip);
	if (ret)
...
static int rt9426_read_battery_temperature(void *dev_data)
{
	...
	return rt9426_set_ntc_compensation_temp(g_rt9426_chip->pdata,
		temp, bat_curr);
}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	.update_batt_param = rt9426_update_batt_param,
...
static int rt9426_set_current_gain(unsigned int val, void *dev_data)
{
	...
	rt9426_apply_calibration_para(g_rt9426_chip, RT9426_GAIN_DEFAULT_VAL,
...
{
	if (!g_rt9426_chip || !g_rt9426_chip->pdata->rs_ic_setting) {
		hwlog_err("invalid g_rt9426_chip\n");
		return -1;
	}

	val = RT9426_GAIN_DEFAULT_VAL +
		((s64)(val) * g_rt9426_chip->pdata->rs_schematic /
		g_rt9426_chip->pdata->rs_ic_setting) / RT9426_COUL_OFFSET_VAL;

	g_rt9426_chip->c_offset = val;
...
static int rt9426_get_offset_from_int(int val)
{
	if (!g_rt9426_chip->pdata->rs_ic_setting) {
		dev_notice(g_rt9426_chip->dev,"%s %d \n", __func__, g_rt9426_chip->pdata->rs_ic_setting);
...
static void rt9426_init_calibration_para(struct rt9426_chip *chip)
{
	...
	if (chip->pdata->force_use_aux_cali_para && (c_a != 0) && (v_a != 0)) {
		dev_notice(g_rt9426_chip->dev, "force_use_aux_cali_para\n");
		...
		if (c_offset < RT9426_TBATICAL_MIN_B || c_offset > RT9426_TBATICAL_MAX_B)
			c_offset = 0;
	...
	if (rt9426_calibration_para_invalid(c_a, v_a, c_offset)) {
	...
	chip->c_offset = rt9426_get_offset_from_int(c_offset);

	dev_notice(chip->dev, "c_gain %d, v_gain %d, c_offset %d\n",
	...
	const struct i2c_device_id *id)
{
	...
	rt9426_create_attrs(&chip->fg_psy->dev);
	...
	if (pdata->flag) {
		rt9426_write_page_cmd(chip, RT9426_PAGE_0);
		rt9426_reg_write_word(chip->i2c, RT9426_REG_SWINDOW3, RT9426_NTC_CONFIG_CMD);
	}
	if (pdata->cutoff_vol && pdata->cutoff_cur) {
		...
		rt9426_write_page_cmd(chip, RT9426_PAGE_5);
		rt9426_reg_write_word(chip->i2c, RT9426_REG_SWINDOW3, fc_target);
...
static void rt9426_i2c_complete(struct device *dev)
{
	struct rt9426_chip *chip = dev_get_drvdata(dev);
...
static const struct dev_pm_ops rt9426_pm_ops = {
	.suspend = rt9426_i2c_suspend,
	.resume = rt9426_i2c_resume,
	.complete = rt9426_i2c_complete,
};
#else
static int rt9426_i2c_resume(struct device *dev)
{
