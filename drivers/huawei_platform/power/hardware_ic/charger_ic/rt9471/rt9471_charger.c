...
#if 0
static int rt9471_is_charging_done(bool *done)
{
	int ret;
...
static int rt9471_get_min_ichg(u32 *curr)
{
	if (!curr)
		return -EINVAL;
...
#if 0
static int rt9471_enable_chg(bool en)
{
	int ret;
	struct rt9471_chip *chip = NULL;
...
static int rt9471_is_charging_enable(bool *en)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_plug_in()
{
	/* enable WDT when plug in */
	rt9471_set_wdt(true);

	pr_info("plug in set enable true\n");
	return rt9471_enable_chg(true);
}

static int rt9471_plug_out(void)
{
	/* disable WDT when plug out */
	rt9471_set_wdt(false);

	pr_info("plug out set enable false\n");
	return rt9471_enable_chg(false);
}

static int rt9471_get_ichg(u32 *ichg)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_set_ichg(u32 ichg)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_get_aicr(u32 *aicr)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_set_aicr(u32 aicr)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_get_cv(u32 *cv)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_set_ieoc(u32 uA)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_set_mivr(u32 mivr)
{
	struct rt9471_chip *chip = NULL;
...
static int rt9471_enable_otg(bool en)
{
	struct rt9471_chip *chip = NULL;
	int ret;
...
static int rt9471_charger_do_event(u32 event, u32 args)
{
	struct rt9471_chip *chip = NULL;
	...
	case EVENT_EOC:
		...
		//charger_dev_notify(CHARGER_DEV_NOTIFY_EOC);
		break;
	case EVENT_RECHARGE:
		pr_info("do recharge event\n");
		//charger_dev_notify(CHARGER_DEV_NOTIFY_RECHG);
		break;
	default:
		break;
	}
...
static int rt9471_enable_hz(bool en)
{
	struct rt9471_chip *chip = NULL;
	struct rt9471_desc *desc = NULL;
	int ret;
...
static int rt9471_get_register_head(char *reg_head, int size, void *dev_data)
{
	...
	int i;
	int len = 0;
	struct rt9471_chip *chip = (struct rt9471_chip *)dev_data;
