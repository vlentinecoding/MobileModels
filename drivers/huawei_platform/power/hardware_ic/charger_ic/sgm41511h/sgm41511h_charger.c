...
#define BC12_RESULT_DELAY            400
#define BC12_RESULT_RETRY            32
#define BC12_DELAY_TIME              50
...
static int sgm41511h_set_watchdog_timer(struct charger_device *chg_dev, u32 value);
	...
	u32 value)
{
	...
	if (!g_hiz_mode) {
		ret = sgm41511h_write_mask(SGM41511H_REG_ISC,
			SGM41511H_REG_ISC_IINLIM_HIZ_MASK,
			SGM41511H_REG_ISC_IINLIM_SHIFT,
			(val & ~(1 << SGM41511H_REG_ISC_EN_HIZ_SHIFT)));
		if (ret < 0) {
			pr_err("%s, ret = %d\n", __func__, ret);
			return ret;
		}
	...
	pr_info("%s, val=0x%x, g_hiz_mode=%d\n", __func__, val, g_hiz_mode);
	return ret;
}
...
static int sgm41511h_reused_set_cv(u32 value)
{
	...
	return sgm41511h_write_mask(SGM41511H_REG_CVC,
		SGM41511H_REG_CVC_TOPOFF_MASK,
		SGM41511H_REG_CVC_TOPOFF_SHIFT,
		(val << SGM41511H_REG_CVC_NEW_SHIFT) | SGM41511H_REG_TOPOFF_TIMER_0M);
}
	...
	bool enable)
{
	...
	if (enable) {
		ret = sgm41511h_set_watchdog_timer(chg_dev,
			WDT_STEP1);
		if (ret)
			pr_err("set wdt fail\n");
	}
	...
	if (enable) {
		ret = sgm41511h_write_mask(SGM41511H_REG_POC,
			SGM41511H_REG_POC_CHG_ENABLE_CONFIG_MASK,
			SGM41511H_REG_POC_CHG_CONFIG_SHIFT, true);
		if (ret < 0) {
			pr_err("%s, ret = %d\n", __func__, ret);
		}
	} else {
		ret = sgm41511h_write_mask(SGM41511H_REG_POC,
			...
			SGM41511H_REG_POC_CHG_CONFIG_SHIFT, false);
		if (ret < 0) {
			pr_err("%s, ret = %d\n", __func__, ret);
		}
	}
...
static int sgm41511h_set_otg_enable(struct charger_device *chg_dev, bool enable)
{
	...
	if (!di->chg_det_enable) {
		if ((!di->irq_active) && (enable)) {
			di->irq_active = true; /* active */
			enable_irq(di->irq_int);
		} else if ((di->irq_active) && (!enable)) {
			di->irq_active = false; /* inactive */
...
static int sgm41511h_get_register_head(char *reg_head,
	int size, void *dev_data)
{
	char buff[BUF_LEN] = {0};
	int i;
	int len = 0;
	struct sgm41511h_device_info *di = (struct sgm41511h_device_info *)dev_data;
...
static int sgm41511h_hw_dump_register(char *reg_value,
	int size, void *dev_data)
{
	int i;
	int ret;
	...
	bool enable)
{
	...
	if (enable == true) {
		...
		if (ret_wdt)
			pr_err("set_charger_hiz write fail\n");
	} else {
		ret = sgm41511h_write_mask(SGM41511H_REG_ISC,
			SGM41511H_REG_ISC_EN_HIZ_MASK,
...
static void sgm41511_set_usbsw_state(int state)
{
	pr_info("%s: state = %d\n", __func__, state);

	/* Switch D+D- to AP/41511 */
	if (state == 1)
		Charger_Detect_Init();
	else
...
static int sgm41511_enable_usbchg(bool attach)
{
	int i;
	int ret = 0;
	u8 val = 0;
	...
	if (!attach) {
		sgm41511_set_usbsw_state(0);
		return ret;
	}
	sgm41511_set_usbsw_state(1);
	mdelay(BC12_RESULT_DELAY);
	for (i = 0; i < BC12_RESULT_RETRY; i++) {
		ret = sgm41511h_read_byte(SGM41511H_REG_SS, &val);
		if (ret < 0)
			pr_err("%s, ret = %d, val = %d\n", __func__, ret, val);
		if (((val & SGM41511H_REG_SS_PG_STAT_MASK) == SGM41511H_REG_SS_PG_STAT_MASK) &&
			((val & SGM41511H_REG_SS_VBUS_STAT_MASK) != 0x0))
			break;

		mdelay(BC12_DELAY_TIME);
	}

	if (di->do_bc12_second) {
		vbus_state = (val & SGM41511H_REG_SS_VBUS_STAT_MASK) >>
			SGM41511H_REG_SS_VBUS_STAT_SHIFT;

		if ((vbus_state == REG08_VBUS_TYPE_NOT_STD) || (vbus_state == REG08_VBUS_TYPE_UNKNOWN)) {
			ret = sgm41511h_write_mask(SGM41511H_REG_MOC, SGM41511H_REG_MOC_DPDM_EN_MASK,
				SGM41511H_REG_MOC_DPDM_EN_SHIFT, REG07_FORCE_DPDM);
			if (ret < CHARGE_IC_GOOD)
				pr_err("REG SGM41511H_REG_MOC set fail,ret = %d\n", ret);

			for (i = 0; i < BC12_RESULT_RETRY; i++) {
				ret = sgm41511h_read_mask(SGM41511H_REG_MOC, SGM41511H_REG_MOC_DPDM_EN_MASK,
					SGM41511H_REG_MOC_DPDM_EN_SHIFT, &val);
				if (ret < CHARGE_IC_GOOD)
					pr_err("REG SGM41511H_REG_MOC get fail, ret = %d\n", ret);
				if (!val) {
					mdelay(BC12_DELAY_TIME);
					ret = sgm41511h_read_byte(SGM41511H_REG_SS, &val);
					if (ret < 0)
	...
	if (di->do_bc12_not_std) {
		vbus_state = (val & SGM41511H_REG_SS_VBUS_STAT_MASK) >>
			SGM41511H_REG_SS_VBUS_STAT_SHIFT;

		if ((vbus_state == REG08_VBUS_TYPE_USB) || (vbus_state == REG08_VBUS_TYPE_NOT_STD) ||
		    (vbus_state == REG08_VBUS_TYPE_UNKNOWN)) {
			ret = sgm41511h_write_mask(SGM41511H_REG_MOC, SGM41511H_REG_MOC_DPDM_EN_MASK,
				SGM41511H_REG_MOC_DPDM_EN_SHIFT, REG07_FORCE_DPDM);
			if (ret < CHARGE_IC_GOOD)
				pr_err("REG SGM41511H_REG_MOC set fail,ret = %d\n", ret);
	...
	ret = sgm41512_chg_type_det();
	sgm41511_set_usbsw_state(0);
	return ret;
}

static int sgm41511_chgdet_pre_process(struct sgm41511h_device_info *di,
	bool attach)
{
	if (!attach) {
		di->chg_type = CHARGER_UNKNOWN;
		(void)sgm41512_inform_psy_changed();
	}
	return sgm41511_enable_usbchg(attach);
}

static int sgm41511h_enable_chg_type_det(struct charger_device *chg_dev, bool en)
{
	int ret = 0;
	struct sgm41511h_device_info *di = g_sgm41511h_dev;

	dev_info(di->dev, "%s\n", __func__);
	mutex_lock(&di->chgdet_lock);
	if (di->tcpc_attach == en) {
		dev_info(di->dev, "%s attach(%d) is the same\n",
			__func__, di->tcpc_attach);
		goto out;
	}
	di->tcpc_attach = en;
	ret = sgm41511_chgdet_pre_process(di, en);
out:
	mutex_unlock(&di->chgdet_lock);
	return ret;
}
...
static void sgm41511h_irq_work(struct work_struct *work)
{
	...
	if ((!di->pd_det_enable) && (di->chg_det_enable) && (!g_hiz_mode)) {
		ret = sgm41511h_read_byte(SGM41511H_REG_SS, &vbus_reg);
		if (ret < CHARGE_IC_GOOD) {
			pr_err("read vbus_reg fail\n");
			sgm41512_chg_type_det();
		} else {
			vbus_stat = (vbus_reg & SGM41511H_REG_SS_VBUS_STAT_MASK) >>
				SGM41511H_REG_SS_VBUS_STAT_SHIFT;
...
static irqreturn_t sgm41511h_interrupt(int irq, void *_di)
{
	...
	pr_info("sgm41511h int happened %d\n", di->irq_active);
	...
	struct sgm41511h_device_info *di)
{
	...
	di->pd_det_enable = of_property_read_bool(np, "pd-det-enable");
	di->do_bc12_not_std = of_property_read_bool(np, "do-bc12-non-std");
	di->do_bc12_second = of_property_read_bool(np, "do-bc12-second");
	di->gpio_int = of_get_named_gpio(np, "gpio_int", 0);
	pr_info("gpio_int=%d chg_det_enable=%d pd_det_enable =%d\n", di->gpio_int,
		di->chg_det_enable, di->pd_det_enable);
	...
	struct sgm41511h_device_info *di)
{
	...
	if (di->pd_det_enable)
		sgm41511h_ops.enable_chg_type_det = sgm41511h_enable_chg_type_det;
	else
	...
	struct sgm41511h_device_info *di)
{
	...
	di->pd_ctc_detect = of_property_read_bool(np, "pd_ctc_detect");
