...
#endif
static int sgm41511h_set_charge_current(u32 value)
{
	int val;
...
#endif
static int sgm41511h_set_input_current(u32 value)
{
	int val;
...
static int sgm41511h_set_dpm_voltage(u32 value)
{
	int val;
...
static int sgm41511h_reset_watchdog_timer(void)
{
...
static int sgm41511h_set_watchdog_timer(u32 value)
{
	int ret;
	int val;
	...
	if (value > WDT_BASE)
		ret = sgm41511h_reset_watchdog_timer();

	return ret;
}

static int sgm41511h_set_charger_hiz(bool enable)
{
	int ret;
	int ret_wdt;
	struct sgm41511h_device_info *di = g_sgm41511h_dev;
	...
	if (enable == true) {
		...
		ret_wdt = sgm41511h_set_watchdog_timer(WATCHDOG_TIMER_DISABLE);
		if (ret_wdt)
...
static int sgm41511h_5v_chip_init(struct sgm41511h_device_info *di)
{
	...
	ret = sgm41511h_set_dpm_voltage(VINDPM_INIT);
	if (ret < CHARGE_IC_GOOD) {
		pr_info("set_dpm_voltage fail,ret = %d\n", ret);
		return ret;
	}

	ret = sgm41511h_set_input_current(IINLIM_INIT);
	if (ret < CHARGE_IC_GOOD) {
		pr_info("set_input_current fail,ret = %d\n", ret);
		return ret;
	}

	ret = sgm41511h_set_charge_current(ICHG_INIT);
	if (ret < CHARGE_IC_GOOD) {
...
static void sgm41511h_shutdown(struct i2c_client *client)
{
	...
	if (part) {
		...
		ret = sgm41511h_set_charger_hiz(false);
		if (ret < CHARGE_IC_GOOD)
