...
#define PMK_BATTERY_PLUG       0xA0  /* pmk 0x71a0 reg for battery plug out */
...
/* for qcom by sreason xvdd reset */
static int hisi_battery_removed_before_boot(struct batt_info *data)
{
	if (!data)
		return 0;

	return batt_get_battery_plugin() || !data->pmic_reg_val;
}
...
static void get_sn_from_nv(struct batt_info *drv_data)
{
	...
	if (bbinfo.version != drv_data->sn_version) {
		...
		result.oo_rematch = drv_data->result.oo_rematch;
...
static void check_func(struct work_struct *work)
{
	...
	if (ret)
		...
		hwlog_info("last result ic:%d key:%d sn:%d mode:%d, oo_rematch:%d\n",
			   last_result.ic_status, last_result.key_status,
			   last_result.sn_status, last_result.check_mode, last_result.oo_rematch);
	...
	if (!(ret || !is_legal_result(&last_result) ||
		...
		hisi_battery_removed_before_boot(drv_data))) {
	...
	list_for_each_entry(temp, &batt_checkers_head, node) {
		checker_data = platform_get_drvdata(temp->pdev);
		checker_data->oo_rematch = last_result.oo_rematch;
		hwlog_info("checker data oo rematch =%d\n", checker_data->oo_rematch);
	...
	if (drv_data->nvmem) {
		ret = nvmem_device_read(drv_data->nvmem, PMK_BATTERY_PLUG, 1, &val);
		if (ret > 0) {
			val = val | 0x01; /* bit 0 set 1 */
			ret = nvmem_device_write(drv_data->nvmem, PMK_BATTERY_PLUG, 1, &val);
			if (ret < 0)
				hwlog_err("%s write pmic reg val fail ret = %d\n", __func__, ret);
		} else
			hwlog_err("%s read  pmic reg val fail ret = %d\n", __func__, ret);
	}

process_result_status:
	if (memcmp(&last_result, final_result, sizeof(struct batt_chk_rslt))) {
		last_result.oo_rematch = 1;
		final_result->oo_rematch = 1;
		record_final_check_result(final_result);
	}
	schedule_delayed_work(&drv_data->dmd_report_dw, 5 * HZ);
skip_result_status:
	if (last_result.oo_rematch == 0) {
		last_result.oo_rematch = 1;
		record_final_check_result(&last_result);
	}
...
static struct batt_info *batt_info_data_init(struct platform_device *pdev)
{
	...
	if (drv_data->total_checkers == 0) {
		hwlog_err("no valid checker found under honor_batt_info\n");
		return NULL;
	}
...
static int battery_info_probe(struct platform_device *pdev)
{
	...
	/* get pmic reg for battery */
	drv_data->nvmem = devm_nvmem_device_get(&pdev->dev, "battery_plugout");
	if (IS_ERR(drv_data->nvmem)) {
		ret = PTR_ERR(drv_data->nvmem);
		if (ret != -EPROBE_DEFER)
			dev_err(&pdev->dev, "failed to get nvmem device, ret=%d\n", ret);
		hwlog_err("%s nvmem no config\n", __func__);
		drv_data->nvmem = NULL;
		drv_data->pmic_reg_val = 1; /* no config pmk, so can not support the function */
	} else {
		ret = nvmem_device_read(drv_data->nvmem, PMK_BATTERY_PLUG, 1, &buf);
		drv_data->pmic_reg_val = buf & 0x01;
		hwlog_info("%s pmic reg val =%d, ret =%d\n", __func__, buf, ret);
	}
	/* start checking work */
	schedule_work(&drv_data->check_work);
