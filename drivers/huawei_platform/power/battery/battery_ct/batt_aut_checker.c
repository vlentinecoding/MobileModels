...
static char batt_data[BATT_MAX_SIGN_LEN] = { 0 };
static struct batt_res battery_res;
static bool act_write_flag;
...
static int convert_act_signature(struct batt_chk_data *drv_data,
	const char *buf, size_t count)
{
	...
	for (i = 1; i < bytes + 1; i++) {
		...
		temp[2] = '\0';
		/* 16 = hexadecimal */
		if (kstrtou8(temp, 16, batt_data + i)) {
			hwlog_err("%s kstrtou8 failed\n", temp);
			return -EINVAL;
		}
	}

	battery_res.data = batt_data;
	battery_res.len = bytes + 1;
	return 0;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (convert_act_signature(drv_data, buf, count)) {
		hwlog_err("convert act signature failed %d\n",
			drv_data->act_state);
		return -EINVAL;
	}

	queue_work(system_highpri_wq, &drv_data->act_write_work);
	hwlog_info("set act %lu signature success\n", count);
	return count;
}

static ssize_t act_write_flag_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct batt_chk_data *drv_data = NULL;

	DEV_GET_DRVDATA(drv_data, dev);
	if (drv_data->act_state != ACT_PASS)
		return  snprintf(buf, PAGE_SIZE, "1");
	return  snprintf(buf, PAGE_SIZE, "0");
}

static void act_write_work(struct work_struct *work)
{
 	struct batt_chk_data *drv_data = container_of(work,
 		struct batt_chk_data, act_write_work);

 	hwlog_info("%s begin\n", __func__);

	if (drv_data->bco.set_act_signature(drv_data, RES_ACT, &battery_res)) {
...
static const DEVICE_ATTR_RO(act_write_flag);
#ifdef BATTERY_LIMIT_DEBUG
...
static int battery_all_work_init(struct batt_chk_data *drv_data)
{
	...
	INIT_WORK(&drv_data->act_write_work, act_write_work);
