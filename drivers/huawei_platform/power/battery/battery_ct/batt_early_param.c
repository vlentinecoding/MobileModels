...
#undef HWLOG_TAG
#endif
#define HWLOG_TAG battery_cmdline
HWLOG_REGIST();

static struct batt_ic_para g_ic_para;
static struct batt_info_para g_info_para;
static struct batt_cert_para g_cert_para;
static char g_nv_sn[MAX_SN_LEN];

static int g_xvdd_reset;
...
static int __init get_pmic_xvdd(char *p)
{
	int ret;
	int pmic_index;
	int pmic_type;
	...
	ret = sscanf(p, "%d,%d,%d,%d", &pmic_index, &pmic_type, &pmic_number, &g_xvdd_reset);
	hwlog_info("%s ret=%d xvdd:%d\n", __func__, ret, g_xvdd_reset);

	return ret;
}
early_param("OCP", get_pmic_xvdd);
...
int batt_get_battery_plugin(void)
{
	return g_xvdd_reset;
}
