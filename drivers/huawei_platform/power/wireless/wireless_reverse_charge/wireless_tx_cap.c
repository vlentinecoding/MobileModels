...
static void wltx_cap_set_tx_imax(u8 *tx_cap, int imax)
{
	...
	if (((pwr_type == WL_TX_PWR_5VBST_VBUS_OTG_CP) ||
		(pwr_type == WL_TX_PWR_5VBST_OTG_PMU_VBUS_CP) ||
		(pwr_type == WL_TX_PWR_5VBST_OTG_VBUS_CP)) &&
		!strstr(src_name, "CP"))
		imax = 500; /* 500mA imax for 5vbst/vbus/otg pwr_src */
	if (strstr(src_name, "PWR_SRC_PMU")) {
		hwlog_info("%s: pmu bypass\n", __func__);
        	imax = l_imax;
	}
	if (di->limit_power_flag) {
		hwlog_info("%s: high temp limit power\n", __func__);
		imax = 500; /* 500mA imax for high temp */
	}
...
static void wltx_set_high_pwr_cap_id(struct wireless_tx_device_info *di)
{
	...
	if (info->charger_type == WLTX_SC_HI_PWR_CHARGER)
		...
		di->tx_cap.exp_id = WLTX_HIGH_PWR2_CAP;
	else
		di->tx_cap.exp_id = WLTX_DFLT_CAP;
}
...
static void wltx_check_high_pwr_cap(struct wireless_tx_device_info *di)
{
	int soc;
...
void wltx_update_charge_type_by_wired(void)
{

	struct wltx_pwr_ctrl_info *info= wltx_get_pwr_ctrl_info();
	if (!info)
		return;
	info->charger_type = WLTX_UNKOWN_CHARGER;
}
