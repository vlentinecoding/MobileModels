...
static void wltx_set_tx_vset_id(struct wireless_tx_device_info *di, int id)
{
	di->tx_vset.cur = id;
...
static int wltx_set_bridge(int ask_vset, enum wltx_bridge_type type)
{
	...
	return di->tx_ops->set_bridge(ask_vset, type);
}
...
static void wltx_config_adap_vout(struct wireless_tx_device_info *di, int vout)
{
	int adap_vout;

	if (vout < WIRELESS_ADAPTER_12V)
		adap_vout = WLTX_SC_ADAP_VSET;
	else if (vout <= WIRELESS_ADAPTER_15V)
		adap_vout = vout / CP_CP_RATIO + WLTX_SC_ADAP_V_DELTA;
	else
		adap_vout = vout / CP_CP_RATIO;

	if ((vout > WIRELESS_ADAPTER_15V) &&
