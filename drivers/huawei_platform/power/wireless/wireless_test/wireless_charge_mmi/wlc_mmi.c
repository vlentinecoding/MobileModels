...
static int wlc_mmi_notifier_call(struct notifier_block *nb,
...
static void wlc_mmi_init(struct device *dev)
{
	...
	ret = power_event_nc_register(POWER_NT_WLC, &di->wlc_mmi_nb);
	if (ret)
		goto free_mem;
...
unregister_notifier:
	(void)power_event_nc_unregister(POWER_NT_WLC, &di->wlc_mmi_nb);
free_mem:
	devm_kfree(dev, di);
	g_wlc_mmi_di = NULL;
}
...
static void wlc_mmi_exit(struct device *dev)
{
	...
	(void)power_event_nc_unregister(POWER_NT_WLC, &g_wlc_mmi_di->wlc_mmi_nb);
