...
static int wlc_wp_notifier_call(struct notifier_block *nb,
...
static void wlc_wp_init(struct device *dev)
{
	...
	ret = power_event_nc_register(POWER_NT_WLC, &di->wlc_wp_nb);
	if (ret)
		goto free_mem;
...
static void wlc_wp_exit(struct device *dev)
{
	...
	(void)power_event_nc_unregister(POWER_NT_WLC, &g_wlc_wp_di->wlc_wp_nb);
