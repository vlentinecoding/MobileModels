	...
	struct wireless_charge_device_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sample_delay_time", &di->sample_delay_time,
		RX_SAMPLE_DFT_DELAY_TIME);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"support_high_pwr_wltx", &di->support_high_pwr_wltx, 0);
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"ignore_otg_mode", &di->ignore_otg_mode, 0);
	 (void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
 		"motorPosition", &di->motorPosition, 0);
	di->fod_status_default = di->fod_status;
	return 0;
}
	...
	struct wireless_charge_device_info *di)
{
	...
	np = wlrx_get_dev_node(WLTRX_IC_MAIN);
	if (!np) {
...
static int wlc_parse_rx_iout_limit(struct device_node *np,
	struct wireless_charge_device_info *di)
{
	u32 tmp_para[WLC_RX_IOUT_LIMIT_INFO_MAX] = { 0 };

	if (power_dts_read_u32_array(power_dts_tag(HWLOG_TAG), np,
		"rx_iout_limit", tmp_para, WLC_RX_IOUT_LIMIT_INFO_MAX)) {
		return -WLC_ERR_PARA_WRONG;
	}

	di->rx_iout_limit_para.rx_vout_min = (int)tmp_para[WLC_RX_VOUT_MIN];
	di->rx_iout_limit_para.rx_vout_max = (int)tmp_para[WLC_RX_VOUT_MAX];
	di->rx_iout_limit_para.current_step = (int)tmp_para[WLC_CURRENT_STEP];
	hwlog_info("%s: rx_vout_min:%dmV, rx_vout_max = %dmV, current_step = %dmA\n", __func__,
		di->rx_iout_limit_para.rx_vout_min, di->rx_iout_limit_para.rx_vout_max,
		di->rx_iout_limit_para.current_step);
	return 0;
}

static int wlc_parse_fod_status(struct device_node *np,
	struct wireless_charge_device_info *di)
{
	int i;
	int arr_len;
	u32 tmp_para[WLC_FOD_STATUS_PARA_LEVEL * WLC_FOD_STATUS_INFO_MAX] = { 0 };

	arr_len = wlc_parse_u32_array(np, "fod_status_para",
		WLC_FOD_STATUS_PARA_LEVEL, WLC_FOD_STATUS_INFO_MAX, tmp_para);
	if (arr_len <= 0) {
		di->fod_status_para_size = 0;
		return -WLC_ERR_PARA_WRONG;
        }

	di->fod_status_para_size = arr_len / WLC_FOD_STATUS_INFO_MAX;
	for (i = 0; i < arr_len / WLC_FOD_STATUS_INFO_MAX; i++) {
		di->fod_status_para[i].tx_type = tmp_para[WLC_FOD_STATUS_INFO_MAX * i +
			WLC_TX_TYPE];
		di->fod_status_para[i].fod_status = tmp_para[WLC_FOD_STATUS_INFO_MAX * i +
			WLC_FOD_STATUS];

		hwlog_info("fod_status_para[%d] tx_type:%d, fod_status:0x%x\n",
			i, di->fod_status_para[i].tx_type, di->fod_status_para[i].fod_status);
	}
	return 0;
}
	...
	struct wireless_charge_device_info *di)
{
	...
	ret = wlc_parse_fod_status(np, di);
	if (ret) {
		hwlog_err("parse_dts: get fod_status_para failed\n");
		return -EINVAL;
	}

	ret = wlc_parse_rx_iout_limit(np, di);
	if (ret) {
		hwlog_err("parse_dts: get rx iout limit para failed\n");
		return -EINVAL;
	}
