...
static void wlc_update_fod_status(struct wireless_charge_device_info *di)
{
	int index;
	for (index = 0; index < di->fod_status_para_size; index++) {
		if (di->tx_type == di->fod_status_para[index].tx_type) {
			di->fod_status = di->fod_status_para[index].fod_status;
			return;
		}
	}
	di->fod_status = di->fod_status_default;
}
	...
	struct wireless_charge_device_info *di)
{
	...
	if (di->fod_status <= 0) {
		hwlog_err("%s: fod_status invalid\n", __func__);
		return;
	}
...
static void wireless_charge_get_tx_prop(struct wireless_charge_device_info *di)
{
	...
	if (!di->tx_cap->iout_max) {
		...
		hwlog_info("%s: tx_cap_vout_max = %d, tx_cap_iout_max = %d\n", __func__, di->tx_cap->vout_max, di->tx_cap->iout_max);
	}
}
...
static void wlc_revise_icon_display(struct wireless_charge_device_info *di, int *icon_type)
{
	...
	if (wlc_need_ignore_fod_status(di)) {
		if (di->tx_cap->vout_max * di->tx_cap->iout_max <=
                         NORMAL_CHARGE_MAX_POWER)
			*icon_type = WIRELESS_NORMAL_CHARGE_FLAG;
		else
			*icon_type = WIRELESS_FAST_CHARGE_FLAG;
	}
...
void wireless_charge_update_max_vout_and_iout(bool ignore_cnt_flag)
{
	...
	for(i = 0; i < di->segment_data.segment_para_level; i++) {
		if((soc >= di->segment_data.segment_para[i].soc_min) &&
			(soc <= di->segment_data.segment_para[i].soc_max) &&
...
static void wlc_update_iout_ctrl_para(struct wireless_charge_device_info *di)
{
	...
	if (rx_vout < di->rx_iout_limit_para.rx_vout_min) {
		hwlog_info("%s: %d < %d\n", __func__, rx_vout, di->rx_iout_limit_para.rx_vout_min);
		di->rx_iout_limit -= di->rx_iout_limit_para.current_step;
		return;
	} else if ((rx_vout >= di->rx_iout_limit_para.rx_vout_min) &&
				(rx_vout <= di->rx_iout_limit_para.rx_vout_max)) {
		hwlog_info("%s: %d < %d < %d\n", __func__, di->rx_iout_limit_para.rx_vout_min, rx_vout,
			di->rx_iout_limit_para.rx_vout_max);
		return;
	}
	...
	struct wireless_charge_device_info *di)
{
	...
	if ((g_wireless_charge_stage != WIRELESS_STAGE_CHARGING) ||
	    (g_wireless_channel_state == WIRELESS_CHANNEL_OFF))
		return;
	di->charge_type_check = hihonor_charger_glink_set_charger_type(CHARGER_TYPE_STANDARD);
	msleep(50); /* solve current limit failure */
	wlc_ictrl_set_input_current(WLC_CHARGER_ILIMIT);
	hihonor_charger_glink_enable_identify_insert(WLC_INSERT_IDENTIFY);
...
static void wireless_charge_check_tx_ability(struct wireless_charge_device_info *di)
{
	...
	if (di->tx_cap->support_motor_move)
		wlc_set_motor_position(WIRELESS_PROTOCOL_QI, di->motorPosition);
	hwlog_info("%s:di->motorPosition = 0x%x\n", __func__, di->motorPosition);
	wlc_get_ept_type(di);
	wireless_charge_set_charge_stage(WIRELESS_STAGE_CABLE_DETECT);
	wireless_charge_get_tx_info(di);
	wireless_charge_send_fod_status(di);
...
static void wireless_charge_stop_charging(struct wireless_charge_device_info *di)
{
	...
	wired_chsw_set_wired_channel(WIRED_CHANNEL_MAIN, WIRED_CHSW_CLIENT_WLC_RX, WIRED_CHANNEL_RESTORE);
	hihonor_charger_glink_set_charger_type(CHARGER_REMOVED);
	hwlog_info("%s --\n", __func__);
}
	...
	enum wireless_charge_stage stage_from)
{
	...
	if (g_wireless_channel_state == WIRELESS_CHANNEL_ON) {
		/* limit current for wireless revserse pause charge */
		hihonor_charger_glink_set_charger_type(CHARGER_TYPE_USB);
		msleep(50);
		wlc_ictrl_set_input_current(100);
...
static void wireless_charge_wireless_vbus_disconnect_handler(void)
{
	...
	hihonor_charger_glink_enable_identify_insert(!WLC_INSERT_IDENTIFY);
	hihonor_charger_glink_enable_usbsuspend_collapse(true);
}
...
static void wireless_charge_wired_vbus_connect_work(struct work_struct *work)
{
	...
	for (i = 0; i < 10; i++) {
		if (wldc_is_stop_charging_complete()) {
			wired_chsw_set_wired_channel(WIRED_CHANNEL_MAIN, WIRED_CHSW_CLIENT_USB, WIRED_CHANNEL_RESTORE);
			break;
		}
		msleep(50);
	}
...
void wireless_charge_wired_vbus_connect_handler(void)
{
	...
	if (!wireless_is_in_tx_mode())
		wireless_charge_switch_off(); // here is the true reason of disconnect and connect
...
void wireless_charge_wired_vbus_disconnect_handler(void)
{
	...
	wired_chsw_set_wired_channel(WIRED_CHANNEL_ALL, WIRED_CHSW_CLIENT_USB, WIRED_CHANNEL_RESTORE);
	wltx_update_charge_type_by_wired();
	schedule_work(&di->wired_vbus_disconnect_work);
}

bool wireless_charge_is_pwr_good(void)
{
	if (!g_wireless_ops || !g_wireless_ops->is_pwr_good) {
		hwlog_err("%s: g_wireless_ops or is_pwr_good is null\n", __func__);
		return false;
	}
	return g_wireless_ops->is_pwr_good();
}
...
void direct_charger_disconnect_event(void)
{
	...
	wireless_charge_set_wired_channel_state(WIRED_CHANNEL_OFF);
	wired_chsw_set_wired_channel(WIRED_CHANNEL_ALL, WIRED_CHSW_CLIENT_USB, WIRED_CHANNEL_CUTOFF);
	schedule_work(&di->wired_vbus_disconnect_work);
}
#endif
...
static void wireless_charge_monitor_work(struct work_struct *work)
{
	...
	if (di->usb_psy_check)
		di->usb_psy_check = wlc_psy_chg_type_changed(true);
	if (di->charge_type_check)
		di->charge_type_check = hihonor_charger_glink_set_charger_type(CHARGER_TYPE_STANDARD);
	...
	struct wireless_charge_device_info *di)
{
	...
	if (((di->rx_evt.type == POWER_NE_WLRX_PWR_ON) && rx_pwr_good) ||
		...
		di->usb_psy_check = wlc_handle_sink_event(true);
	}
...
static int wireless_charge_probe(struct platform_device *pdev)
{
	...
	ret = wlc_power_supply_register(pdev, di);
	if (ret) {
