...
#include <chipset_common/hwpower/wireless_dc_cp.h>
...
static int wldc_get_bat_current(void)
{
	...
	ret = dc_get_bat_current_with_calibration(SC_MODE, di->ic_data.cur_type, &bat_curr, di->ic_check_info.ibat_comb);
	if (ret) {
		di->stop_flag_error = true;
		hwlog_err("%s: error\n", __func__);
		dc_fill_eh_buf(di->wldc_err_dsm_buff,
...
void wldc_select_comp_param(struct wldc_dev_info *di)
{
	int main_ic_id;
	int aux_ic_id;
	int ic_mode = (di->ic_data.cur_type == CHARGE_MULTI_IC) ? 1 : 0;
...
static int wldc_retore_normal_charge_path(void)
{
	...
	/* USBIN does not suspend on AICL reduces to ICL_MIN */
	hihonor_charger_glink_enable_usbsuspend_collapse(false);
	hwlog_info("%s: sc_to_bulk_delay_hiz = %d, wldc_stage = %d\n", __func__, di->sc_to_bulk_delay_hiz, di->wldc_stage);
	if (di->sc_to_bulk_delay_hiz && di->wldc_stage == WLDC_STAGE_CHARGE_DONE) {
		hwlog_info("sc to bulk, delay %d to close hiz mode\n", di->delay_hiz_time);
		return schedule_delayed_work(&di->hiz_mode_work,
				msecs_to_jiffies(di->delay_hiz_time));
	}
...
static int wldc_open_direct_charge_path(struct wldc_dev_info *di)
{
	...
	if (ret)
		return ret;
	dc_ic_vac_insert_control(SC_MODE, di->ic_data.cur_type, DC_IC_VAC_INSERT_DISABLE);
...
static void wldc_notify_start_charging_evt(struct wldc_dev_info *di)
{
	...
	power_event_notify(POWER_NT_WLC, POWER_NE_WLC_DC_START_CHARGING, &dc_type);
}
...
void wldc_stop_charging(struct wldc_dev_info *di)
{
	...
	if (ret)
		hwlog_err("%s: ls enable fail\n", __func__);
	ret = dc_ic_vac_insert_control(SC_MODE, di->ic_data.cur_type, DC_IC_VAC_INSERT_ENABLE);
	if (ret)
		hwlog_err("%s: ls vac insert enable fail\n", __func__);
...
void hiz_mode_set_work(struct work_struct *work)
{
	hwlog_info("%s disable hiz mode\n", __func__);
	charge_enable_hz(false);
	hihonor_charger_glink_enable_charge(1);
}

static void wldc_disable_hiz(struct wldc_dev_info *di)
{
	...
	struct wldc_dev_info *di)
{
	...
	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"sc_to_bulk_delay_hiz", (u32 *)&di->sc_to_bulk_delay_hiz, 0);

	(void)power_dts_read_u32(power_dts_tag(HWLOG_TAG), np,
		"delay_hiz_time", (u32 *)&di->delay_hiz_time, 30000);
