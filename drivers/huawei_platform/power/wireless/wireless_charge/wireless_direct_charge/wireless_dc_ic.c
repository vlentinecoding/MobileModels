...
static int wldc_ic_enable_multi_ic(struct wldc_dev_info *di, int type, int enable)
{
	...
	if (enable) {
		ret = dc_ic_vac_insert_control(SC_MODE, type, DC_IC_VAC_INSERT_DISABLE);
		if (ret)
			hwlog_err("%s: ls vac insert enable %d fail\n", __func__, enable);
	}

	if (dc_ic_enable(SC_MODE, type, enable)) {
		di->ic_data.multi_ic_err_cnt++;
		if (enable)
				dc_ic_vac_insert_control(SC_MODE, type, DC_IC_VAC_INSERT_ENABLE);
		return -1;
	}

	if (!enable) {
		ret = dc_ic_vac_insert_control(SC_MODE, type, DC_IC_VAC_INSERT_ENABLE);
		if (ret)
			hwlog_err("%s: ls vac insert enable %d fail\n", __func__, enable);
	}
...
static void wldc_ic_switch_to_singlepath(struct wldc_dev_info *di)
{
	...
	(void)dc_ic_vac_insert_control(SC_MODE, CHARGE_IC_AUX, DC_IC_VAC_INSERT_ENABLE);
...
static bool wldc_ic_is_need_singlepath(struct wldc_dev_info *di)
{
	...
	if (dc_get_ic_ibus(SC_MODE, CHARGE_MULTI_IC, &ibus))
		return false;
