	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	fill_mtk_sensor_info(scenario_id, sensor_info, &kit_sensor_info);
...
static uint32 feature_streaming_resume(uint32 sensor_idx,
	uint8 *feature_para, uint32 *feature_para_len)
{
	feature_set_shutter(sensor_idx, feature_para, feature_para_len);
	feature_streaming_control(sensor_idx, TRUE);
