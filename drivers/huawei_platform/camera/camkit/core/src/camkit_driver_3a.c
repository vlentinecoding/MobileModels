...
#define DGAIN_RATIO_BASE 0x10000
	...
	struct aec_ctrl_cfg *aec_ctrl)
{
	...
	uint16 i;
	uint16 gain;
	uint32 dgain_ratio = 0;
	uint32 gc5035_shutter = 0;
	...
	if (aec_info->again_type == CAMKIT_AGAIN_GC5035 &&
		params->sensor_ctrl.shutter != 0) {
		gc5035_shutter = (params->sensor_ctrl.shutter) >> 2;
		gc5035_shutter = gc5035_shutter << 2;
		dgain_ratio = (DGAIN_RATIO_BASE *
			params->sensor_ctrl.shutter) / (gc5035_shutter);
		// use DGAIN to compensation for loss of exposure
		aec_ctrl->dgain = (dgain_ratio * (uint32)reg_dgain) /
			DGAIN_RATIO_BASE;
		aaa_dbg("reg_dgain = %d, gc5035_gain = %d, dgain_ratio = %d\n",
			reg_dgain, aec_ctrl->dgain, dgain_ratio);
		aec_ctrl->dgain_valid = true;
...
uint32 camkit_set_gain(struct camkit_sensor *sensor, const uint16 gain)
{
	...
	if (aec_info->again_type == CAMKIT_AGAIN_GC ||
		aec_info->again_type == CAMKIT_AGAIN_GC5035)
