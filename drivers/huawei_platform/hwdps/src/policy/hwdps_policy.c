...
static bool parse_ruleset_jduge(struct ruleset_cache_entry_t *parsed_rule)
{
	...
	const s8 *path_node, u32 path_node_len)
{
	...
	u32 master_path_node_len = strlen(MASTER_PATH_NODE);
	const s8 *p = NULL;
...
static bool path_node_matches_rule(const s8 *filename, u32 filename_len,
	const s8 *path_node, u32 path_node_len)
{
	u32 master_user_file_path_len = strlen(MASTER_USER_FILE_PATH);
	u32 sub_user_file_path_len = strlen(SUB_USER_FILE_PATH);
	u32 master_path_node_len = strlen(MASTER_PATH_NODE);
	...
	bool master_check_filename =
		(filename_len > master_user_file_path_len) &&
		(strncmp(MASTER_USER_FILE_PATH, filename,
			master_user_file_path_len) == 0);
	bool master_check_path = (path_node_len > master_path_node_len) &&
	...
	if (master_check_filename && master_check_path) {
		if (strncmp(path_node + master_path_node_len, filename +
			master_user_file_path_len,
			path_node_len - master_path_node_len) != 0) {
			hwdps_pr_warn("master user not match filename %s %s\n",
				path_node, filename);
			return false;
		}
		return true;
