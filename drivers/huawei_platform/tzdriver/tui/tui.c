...
#else
static struct mtk_fb_data_type *dss_fd;
#endif
	...
	void *pdata)
{
...
#ifdef CONFIG_TEE_TUI_MTK
			dss_fd = (struct mtk_fb_data_type *)pdata;
#else
			dss_fd = (struct hisi_fb_data_type *)pdata;
#endif
	}

	if (!strncmp(name, TUI_TP_NAME, (size_t)TUI_DRV_NAME_MAX) && pdata == NULL)
		return -1;
	...
	if (memset_s(tui_drv, sizeof(struct tui_drv_node), 0, sizeof(struct tui_drv_node))) {
...
int tui_send_event(int event, struct teec_tui_parameter *tui_param)
{
	...
	if (check_value == true) {
		...
		case TUI_POLL_FOLD:
			...
			mb_pack->operation.params[0].value.b = make32(dss_fd->panel_info.xres, dss_fd->panel_info.yres);
...
next:
	...
	case TUI_POLL_CFG_OK:
		if (DSS_CONFIG_INDEX == g_tui_ctl->s2n.value) {
			...
			g_tui_ctl->n2s.phy_size = tui_display_mem.len;
