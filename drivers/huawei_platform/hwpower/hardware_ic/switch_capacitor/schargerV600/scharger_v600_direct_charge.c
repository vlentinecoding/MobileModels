...
static size_t printk_time(u64 ts_nsec, char *buf, int len_buf)
{
	...
	temp = snprintf(buf, (unsigned long)BUF_LEN,
...
{
	hi6526_set_watchdog_timer(NULL, WATCHDOG_TIMER_40_S);
	return 0;
}
...
int hi6526_sc_enable(int enable, void *dev_data)
{
	...
	(void)hi6526_read_mask(di, CHG_BUCK_MODE_CFG, CHG_BUCK_MODE_CFG_MASK,
		CHG_BUCK_MODE_CFG_SHIFT, &buck_mode);
	(void)hi6526_read_mask(di, CHG_HIZ_CTRL_REG, CHG_HIZ_ENABLE_MSK,
		CHG_HIZ_ENABLE_SHIFT, &hiz_status);
	scharger_inf("[%s] %d role:%d, buck_mode:%u, hiz_status:%u\n", __func__,
		enable, di->param_dts.ic_role, buck_mode, hiz_status);
	if (enable && (buck_mode || !hiz_status))
		(void)hi6526_set_buck_enable(0, di); /* 0: disable buck mode */
