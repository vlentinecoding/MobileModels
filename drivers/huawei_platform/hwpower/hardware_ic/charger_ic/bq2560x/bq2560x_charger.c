...
static struct delayed_work bq2560x_redo_bc12_work;
...
static int sgm41512_inform_psy_changed(struct bq2560x_device_info *di)
{
	hwlog_info("%s enter\n", __func__);
	power_supply_set_int_property_value("charge_manager",
		POWER_SUPPLY_PROP_CHG_TYPE, di->chg_type);
	g_bq2560x_dev->chg_type = di->chg_type;

	return 0;
}

static void sgm41512_get_bc12_result(struct bq2560x_device_info *di)
{
	int ret;
	u8 reg = 0;
	int vbus_stat;
	...
	if (ret < 0)
		hwlog_err("%s: reg read fail\n", __func__);

	vbus_stat = (reg & BQ2560X_REG_SS_VBUS_STAT_MASK) >>
		BQ2560X_REG_SS_VBUS_STAT_SHIFT;
	switch (vbus_stat) {
	...
	case REG08_VBUS_TYPE_CDP:
		di->chg_type = CHARGER_TYPE_BC_USB;
		break;
...
static int sgm41512_chg_type_det(struct bq2560x_device_info *di)
{
	int ret;
	u8 reg = 0;
	int i;
	...
	bq2560x_read_byte(BQ2560X_REG_MOC, &reg);
	hwlog_info("%s: before write ,reg = 0x%x\n", __func__, reg);
	ret = bq2560x_write_mask(BQ2560X_REG_MOC, BQ2560X_REG_MOC_DPDM_EN_MASK,
		BQ2560X_REG_MOC_DPDM_EN_SHIFT, REG07_FORCE_DPDM);
	if (ret < 0)
		hwlog_err("%s: bq2560x_write_mask fail\n", __func__);
	bq2560x_read_byte(BQ2560X_REG_MOC, &reg);
	hwlog_info("%s: after write ,reg = 0x%x\n", __func__, reg);

	for (i = 0; i < retry_cnt; i++) {
		ret = bq2560x_read_byte(BQ2560X_REG_MOC, &reg);
		hwlog_info("reg[%d]=0x%x\n", BQ2560X_REG_MOC, reg);
		if (ret < 0)
			hwlog_err("bq2560x_read_byte fail\n");
		bc12_done = (reg & BQ2560X_REG_MOC_DPDM_EN_MASK) >>
			BQ2560X_REG_MOC_DPDM_EN_SHIFT;
		if (bc12_done == 0)
			break;
		hwlog_info("input detection is not done,wait...\n");
		msleep(200); /* wait bc1.2 done */
	}
	if (i == retry_cnt)
		hwlog_err("input detection never done\n");
	...
	hwlog_info("reg[%d]=0x%x, chg_type=%d\n", BQ2560X_REG_SS, reg, di->chg_type);
	sgm41512_inform_psy_changed(di);
...
static void do_bq2560x_redo_bc12_work(struct work_struct *data)
{
	...
	sgm41512_chg_type_det(g_bq2560x_dev);
...
static int bq2560x_switch_chg_type_det(bool en)
{
	...
	if (g_bq2560x_dev->chg_det_enable)
		sgm41512_chg_type_det(g_bq2560x_dev);
...
static struct usbswitch_common_ops bq2560x_switch_extra_ops = {
	.enable_chg_type_det = bq2560x_switch_chg_type_det,
};
	...
	const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&bq2560x_redo_bc12_work, do_bq2560x_redo_bc12_work);
	g_redo_bc12_flag = false;
	...
	if (di->chg_det_enable)
		ret = usbswitch_common_ops_register(&bq2560x_switch_extra_ops);
