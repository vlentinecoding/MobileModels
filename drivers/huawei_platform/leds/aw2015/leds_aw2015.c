...
#define WR_RETRY_TIMES                  5
#define REG_CALCULATE                   5
#define KSTRTOUL_LEN                    10
#define TIME_STORE_COUNT                4
#define REG_STORE_COUNT                 2
#define REG_DISPLACEMENT                4

//static unsigned int g_blink;
...
const unsigned char aw2015_reg_access[AW2015_REG_MAX] = {
	...
	[AW2015_REG_LCTR]    = REG_RD_ACCESS|REG_WR_ACCESS,
...
static int aw2015_find_time_pattern(unsigned long delay)
{
	int i;

	for (i = 1; i < sizeof(aw2015_pattern_time_table) / sizeof(aw2015_pattern_time_table[0]); i++) {
		if (delay <= aw2015_pattern_time_table[i].period_ms) {
			if (abs(aw2015_pattern_time_table[i].period_ms - delay) <=
				abs(delay - aw2015_pattern_time_table[i - 1].period_ms))
				return aw2015_pattern_time_table[i].value;
			else
				return aw2015_pattern_time_table[i - 1].value;
		}
	}
...
static void aw2015_brightness_work(struct work_struct *work)
{
	...
	struct aw2015_led *led = container_of(work, struct aw2015_led, brightness_work);
	if (led == NULL)
		return;
	...
	pr_err("%s: brightness:%d, led->id:%d, led->blinking:%d\n", __func__,
		led->cdev.brightness, led->id, led->blinking);

	if (led->cdev.brightness == 0) {
		pr_err("%s: close led brightness, led->id:%d, led->blinking:%d\n", __func__, led->id, led->blinking);
		if (led->blinking == 1) {
			aw2015_write(led, AW2015_REG_ILED1 + led->id, 0);
		} else {
			aw2015_read(led, AW2015_REG_LEDEN, &val);
			aw2015_write(led, AW2015_REG_LEDEN, val & (~(1 << led->id)));
		}
	} else if (led->blinking > 0) {
		for (i = 0; i < 3; i++)
			aw2015_write(led, AW2015_REG_LCFG1 + i, AW2015_LED_BREATHE_MODE_MASK);

		aw2015_write(led, AW2015_REG_IMAX, led->pdata->imax);
		aw2015_write(led, AW2015_REG_PWM1, AW2015_LED_BREATHE_PWM_MASK);

		aw2015_write(led, AW2015_REG_LEDEN, 0x07);

		aw2015_write(led, AW2015_REG_LCTR, 0x08);

		aw2015_write(led, AW2015_REG_PAT1_T1,
			(led->pdata->rise_time_ms << REG_DISPLACEMENT | led->pdata->hold_time_ms));
		aw2015_write(led, AW2015_REG_PAT1_T2,
			(led->pdata->fall_time_ms << REG_DISPLACEMENT | led->pdata->off_time_ms));

		if (first_blink == 1) {
			for (i = 0; i < 3; i++)
				aw2015_write(led, AW2015_REG_ILED1 + i, 0);
			first_blink = 0;
		}
		aw2015_write(led, AW2015_REG_ILED1 + led->id, led->pdata->led_current);
		aw2015_write(led, AW2015_REG_PAT_RUN, 0x07);
	} else if (led->cdev.brightness > 0 && led->blinking == 0) {
		first_blink = 1;
		if (led->cdev.brightness > led->pdata->led_current)
			led->cdev.brightness = led->pdata->led_current;

		for (i = 0; i < 3; i++) {
			aw2015_read(led, AW2015_REG_LCFG1 + i, &val);
			if (val & AW2015_LED_BREATHE_MODE_MASK) {
				aw2015_read(led, AW2015_REG_LEDEN, &val);
				aw2015_write(led, AW2015_REG_LEDEN, val & (~(1 << i)));
			}
			aw2015_write(led, AW2015_REG_LCFG1 + i, AW2015_LED_ON_MODE_MASK);
		}
...
static void aw2015_led_blink_set(struct aw2015_led *led, unsigned long blinking)
{
	...
	if (blinking > 0) {
		aw2015_write(led, AW2015_REG_LCTR, reg_ltrl);
		...
		aw2015_write(led, AW2015_REG_PAT1_T1 + led->id * REG_CALCULATE,
			(led->pdata->rise_time_ms << REG_DISPLACEMENT | led->pdata->hold_time_ms));
		aw2015_write(led, AW2015_REG_PAT1_T2 + led->id * REG_CALCULATE,
			(led->pdata->fall_time_ms << REG_DISPLACEMENT | led->pdata->off_time_ms));

		aw2015_read(led, AW2015_REG_LEDEN, &val);
		aw2015_write(led, AW2015_REG_LEDEN, val | (1 << led->id));

		aw2015_write(led, AW2015_REG_PAT_RUN, 0x01);
	} else {
		pr_err("%s: close led blink\n", __func__);
		aw2015_read(led, AW2015_REG_LEDEN, &val);
		aw2015_write(led, AW2015_REG_LEDEN, val & (~(1 << led->id)));
	}
...
static int aw2015_set_blink(struct led_classdev *cdev,
	unsigned long *delay_on, unsigned long *delay_off)
{
	...
	pr_err("%s: delay_on=%d delay_off=%d blink=%d led->id:%d\n",
		__func__, *delay_on, *delay_off, led->blinking, led->id);

	led->pdata->rise_time_ms = 0;
	led->pdata->hold_time_ms = aw2015_find_time_pattern(*delay_on);
	led->pdata->fall_time_ms = 0;
	led->pdata->off_time_ms = aw2015_find_time_pattern(*delay_off);
	...
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	ret = kstrtoul(buf, KSTRTOUL_LEN, &blinking);
	if (ret)
		return ret;
	mutex_lock(&led->pdata->led->lock);
	...
	struct device_attribute *attr, char *buf)
{
	...
	return snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%d %d %d %d\n",
	...
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	rc = sscanf_s(buf, "%d %d %d %d",
		&rise_time_ms, &hold_time_ms,
		&fall_time_ms, &off_time_ms);
	if (rc != TIME_STORE_COUNT) {
		dev_err(&led->client->dev, "%s sscanf_s failed! rc=%d", __func__, rc);
		return -EINVAL;
	}
	...
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (sscanf_s(buf, "%x %x", &databuf[0], &databuf[1]) == REG_STORE_COUNT)
...
static int aw2015_check_chipid(struct aw2015_led *led)
{
	...
	for (cnt = WR_RETRY_TIMES; cnt > 0; cnt--) {
	...
	struct device_node *node)
{
	int rc = 0;
	int parsed_leds = 0;
	struct aw2015_led *led = NULL;
	struct device_node *temp = NULL;
	struct aw2015_platform_data *pdata = NULL;
	...
	for_each_child_of_node(node, temp) {
		...
		pdata = devm_kzalloc(&led->client->dev,
			sizeof(struct aw2015_platform_data), GFP_KERNEL);
		if (!pdata) {
		...
		rc = of_property_read_string(temp, "aw2015,name", &led->cdev.name);
		if (rc < 0) {
		...
		rc = of_property_read_u32(temp, "aw2015,imax", &led->pdata->imax);
		if (rc < 0) {
			dev_err(&led->client->dev,
		...
		rc = of_property_read_string(temp, "aw2015,led-default-trigger",
		...
		led->cdev.blink_set = aw2015_set_blink;
		...
		rc = sysfs_create_group(&led->cdev.dev->kobj, &aw2015_led_attr_group);
		if (rc) {
	...
	const struct i2c_device_id *id)
{
	int ret = -EINVAL;
	int num_leds = 0;
	struct aw2015_led *led_array = NULL;
...
static const struct of_device_id aw2015_match_table[] = {
