...
static int aw2016_find_time_pattern(unsigned long delay)
{
	int i;

	for (i = 1; i < sizeof(aw2016_pattern_time_table) / sizeof(aw2016_pattern_time_table[0]); i++) {
		if (delay <= aw2016_pattern_time_table[i].period_ms) {
			if (abs(aw2016_pattern_time_table[i].period_ms - delay) <=
				abs(delay - aw2016_pattern_time_table[i - 1].period_ms))
				return aw2016_pattern_time_table[i].value;
			else
				return aw2016_pattern_time_table[i - 1].value;
		}
	}
...
{
	struct aw2016_led *led = container_of(work,
	struct aw2016_led, brightness_work);
	u8 val;
	...
	if (val == 0) {
		aw2016_write(led, AW2016_REG_GCR1,
			AW2016_CHARGE_DISABLE_MASK | AW2016_CHIP_DISABLE_MASK);
...
static void aw2016_blink_work(struct work_struct *work)
{
	...
	if (!(val & AW2016_CHIP_ENABLE_MASK)) {
		aw2016_write(led, AW2016_REG_GCR1,
			AW2016_CHARGE_DISABLE_MASK | AW2016_CHIP_ENABLE_MASK);
		msleep(2);
	}

	pr_err("%s: led->blinking:%d, led->brightness:%d, led->id:%d\n",
		__func__, led->blinking, led->cdev.brightness, led->id);

	if (led->cdev.brightness == 0) {
			aw2016_write(led, AW2016_REG_LCFG1 + led->id, 0x00);
			aw2016_read(led, AW2016_REG_LEDEN, &val);
			aw2016_write(led, AW2016_REG_LEDEN, val & (~(1 << led->id)));
	} else if (led->blinking > 0) {
		aw2016_write(led, AW2016_REG_LCFG1, 0x90);
		aw2016_write(led, AW2016_REG_GCR2, led->pdata->imax);
		aw2016_write(led, AW2016_REG_PWM1, led->cdev.max_brightness);
		...
		aw2016_read(led, AW2016_REG_LCFG1 + led->id, &val);
		aw2016_write(led, AW2016_REG_LCFG1 + led->id,
			(val | AW2016_LED_BREATH_MODE_MASK | led->pdata->led_current));

		aw2016_write(led, AW2016_REG_LEDEN, 0x07);
	} else if (led->cdev.brightness > 0 && led->blinking == 0) {
		aw2016_read(led, AW2016_REG_LCFG1, &val);
		aw2016_write(led, AW2016_REG_LCFG1, val & 0x7F);
	...
	enum led_brightness brightness)
{
	...
	pr_err("%s: set brightness:%d, led->blinking:%d, led->id:%d", __func__,
		led->cdev.brightness, led->blinking, led->id);

	schedule_work(&led->blink_work);
}

static int aw2016_set_blink(struct led_classdev *cdev,
	unsigned long *delay_on, unsigned long *delay_off)
{
	...
	pr_err("%s: delay_on:%d, delay_off:%d, led->blinking:%d, led->id:%d\n",
		__func__, *delay_on, *delay_off, led->blinking, led->id);

	led->pdata->rise_time_ms = 0;
	led->pdata->hold_time_ms = aw2016_find_time_pattern(*delay_on);
	led->pdata->fall_time_ms = 0;
	led->pdata->off_time_ms = aw2016_find_time_pattern(*delay_off);
	...
	struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if(2 == sscanf_s(buf, "%x %x", &databuf[0], &databuf[1]))
	...
	struct device_node *node)
{
	...
	for_each_child_of_node(node, temp) {
		...
		pdata = devm_kzalloc(&led->client->dev,
			sizeof(struct aw2016_platform_data), GFP_KERNEL);
		if (!pdata) {
		...
		rc = of_property_read_string(temp, "aw2016,led-default-trigger",
		...
		led->cdev.blink_set = aw2016_set_blink;
...
static int aw2016_get_real_i2c_addr(struct i2c_client *client)
{
	int ret;
	unsigned int addr_dts;
	struct device_node *number_node = NULL;

	number_node = of_find_compatible_node(NULL, NULL, "awinic,aw2016_led");
	if (number_node == NULL) {
		pr_err("Cannot find awinic,aw2016_led from dts\n");
		return -EINVAL;
	}

	ret = of_property_read_u32(number_node, "real_reg", &addr_dts);
	if (!ret) {
		pr_info("%s, real_reg:%x\n", __func__, addr_dts);
		client->addr = addr_dts;
	} else {
		pr_err("%s, Cannot find real_reg from dts\n", __func__);
		return -EINVAL;
	}
	...
	const struct i2c_device_id *id)
{
	...
	ret = aw2016_get_real_i2c_addr(client);
	if (ret != 0)
		pr_err("can't find i2c address from dts\n");
