	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	CHECK_SENSOR_COOKIE(data);
	return als_rgb_status_store(data->sensor_type, dev, attr, buf, size);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	CHECK_SENSOR_COOKIE(data);
	return als_ud_rgbl_status_show(data->sensor_type, dev, attr, buf);
}

static DEVICE_ATTR(als_ud_rgbl, 0660, show_als_ud_rgbl_status, store_rgb_status);

static ssize_t store_als_always_on(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct sensor_cookie *data =
		(struct sensor_cookie *)dev_get_drvdata(dev);
...
static DEVICE_ATTR(always_on, 0660, NULL, store_als_always_on);
	...
	struct device_attribute *attr, char *buf)
{
	...
	CHECK_SENSOR_COOKIE(data);
	return als_calibrate_after_sale_show(data->sensor_type, dev, attr, buf);
}
