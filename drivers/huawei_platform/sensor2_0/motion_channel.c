	...
	enum obj_sub_cmd_t subcmd, enum motion_type_t type)
{
	...
	if ((type == MOTION_TYPE_PICKUP) && (cmd == CMD_CMN_OPEN_REQ))
		pick_up_status = true;
	else if ((type == MOTION_TYPE_PICKUP) && (cmd == CMD_CMN_CLOSE_REQ))
...
void honor_motion_cmd_internal(void)
{
	send_motion_cmd_internal(CMD_CMN_OPEN_REQ, 0, MOTION_TYPE_PICKUP);
}
EXPORT_SYMBOL_GPL(honor_motion_cmd_internal);
bool honor_pick_up_status(void)
{
	return pick_up_status;
}
EXPORT_SYMBOL_GPL(honor_pick_up_status);
