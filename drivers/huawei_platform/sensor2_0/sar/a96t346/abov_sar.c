...
{
	int test_result = 1; // default fail
	uint8_t irq_reg_val;
	int target_val;
	struct hf_manager_event event;
	...
	switch (test_type) {
	case FAR_TEST:
		target_val = 0; // 0 means far state
		break;

	case NEAR_TEST:
		target_val = 1; // 1 means near state
		break;
	...
	read_register(abov_sar_ptr, ABOV_IRQSTAT_REG, &irq_reg_val);
	pr_info("%s : cur state=%d\n", __func__, irq_reg_val);

	if (irq_reg_val == target_val)
	...
	struct custom_cmd *cust_cmd)
{
	...
	if ((cust_cmd->data[0] != NEAR_TEST) && (cust_cmd->data[0] != FAR_TEST))
		test_type = UNKNOW_TEST;
	else
		test_type = cust_cmd->data[0];
...
static int abov_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = write_register(ppdev, ABOV_CTRL_MODE_REG, ABOV_CTRL_MODE_REG_STOP);
	if (ret < 0) {
