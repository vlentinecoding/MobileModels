...
#define OEM_OPCODE_UPDATE_CONFIG   0x10003
...
struct hihonor_oem_glink_notify_msg {
	...
	u32 notification;
	u32 data[MAX_OEM_PROPERTY_DATA_SIZE];
};

struct hihonor_oem_glink_config_msg {
	struct pmic_glink_hdr hdr;
	struct hihonor_oem_glink_config_buffer config_buff;
};
...
static int hihonor_oem_glink_oem_write(void *msg, int len)
{
	...
	if (!rc) {
		...
		hwlog_err("pmic_glink_write fail, rc: %d\n", rc);
	}
...
{
	int ret;
	struct hihonor_oem_glink_config_msg *req_msg = NULL;

	if (!data || !data_size)
		return -1;

	req_msg = kzalloc(sizeof(*req_msg), GFP_KERNEL);
	if (!req_msg)
		return -ENOMEM;

	hwlog_info("%s, oem_config_id: %u\n", __func__, oem_config_id);
	req_msg->hdr.owner = MSG_OWNER_OEM;
	req_msg->hdr.type = MSG_TYPE_OEM_REQ_RESP;
	req_msg->hdr.opcode = OEM_OPCODE_UPDATE_CONFIG;
	req_msg->config_buff.oem_config_id = oem_config_id;
	req_msg->config_buff.data_size = data_size;
	if (data_size > sizeof(req_msg->config_buff)) {
		hwlog_err("config_buff is too big, size: %d\n", data_size);
		return -1;
	}

	memcpy(req_msg->config_buff.data_buffer, data, data_size);
...
{
	struct hihonor_oem_glink_notify_msg notify_msg = {0};
	struct hihonor_oem_glink_dev_info *info = g_dev_info;

	if (!info || data_size > sizeof(notify_msg.data))
		return 0;

	notify_msg.hdr.owner = MSG_OWNER_OEM;
	notify_msg.hdr.type = MSG_TYPE_OEM_REQ_RESP;
	notify_msg.hdr.opcode = OEM_OPCODE_NOTIFY_IND;
	notify_msg.notification = notification;
	if (data && data_size)
	{
		memcpy(notify_msg.data, data, data_size);
	}
	...
	return pmic_glink_write(info->client, &notify_msg, sizeof(notify_msg));
}
...
int hihonor_oem_glink_oem_set_prop(u32 oem_property_id, void *data, size_t data_size)
{
	...
	if (data_size > sizeof(req_msg.msg_buff)) {
		hwlog_err("data_size is too big, size: %d\n", data_size);
		return -1;
	}
	...
	size_t len)
{
	...
	list_for_each_entry(pos, &glink_ops_listhead, node) {
		if (pos->ops && pos->ops->notify_event)
			pos->ops->notify_event(pos->ops->dev_data, notify_msg->notification, notify_msg->data);
	}
}
