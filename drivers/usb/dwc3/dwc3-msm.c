					...
					bool enable_wakeup)
{
	...
	if (!mdwc->in_restart) {
		...
		if (mdwc->lpm_to_suspend_delay) {
			...
			dev_info(mdwc->dev, "%s ssusb dev pm_relax\n", __func__);
			pm_relax(mdwc->dev);
		}
	}
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	mdwc->new_role = role;
	dev_info(mdwc->dev, "cur_role = %d, dp_state = %d, new_role = %d\n",
		cur_role, mdwc->dp_state, role);
	if(cur_role == USB_ROLE_HOST && (mdwc->dp_state)) {
		dev_info(mdwc->dev, "dp connect and already set to host, jump other role set\n");
		return 0;
	}
#endif
	switch (role) {
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
int dwc3_msm_set_dp_state(struct device *dev, bool dp_state)
{
	struct dwc3_msm *mdwc = dev_get_drvdata(dev);
	enum usb_role cur_role = USB_ROLE_NONE;
	...
	mdwc->dp_state = dp_state;
	cur_role = dwc3_msm_usb_get_role(dev);
	dev_info(dev, "%s:  set dp_state to %d, cur_role=%d new_role=%d\n",
		__func__, mdwc->dp_state, cur_role, mdwc->new_role);

	if (dp_state == 0 && mdwc->new_role == USB_ROLE_NONE && mdwc->new_role != cur_role)
		dwc3_msm_usb_set_role(dev, mdwc->new_role);

	return 0;
}
EXPORT_SYMBOL(dwc3_msm_set_dp_state);
#endif
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	dev_dbg(mdwc->dev, "%s state, drd_state=%d\n", state, mdwc->drd_state);
