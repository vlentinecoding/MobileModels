...
struct ucsi_dev {
	...
	atomic_t			state;
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	struct wakeup_source	*ucsi_wake_lock;
#endif
};
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
void stay_ucsi_wakelock(void *driver_data)
{
	struct ucsi_dev *udev = (struct ucsi_dev *)driver_data;

	if (!udev || !udev->ucsi_wake_lock)
		return;

	if (!udev->ucsi_wake_lock->active) {
		pr_info("%s():ucsi wake lock\n", __func__);
		__pm_stay_awake(udev->ucsi_wake_lock);
	}
}

void relax_ucsi_wakelock(void *driver_data)
{
	struct ucsi_dev *udev = (struct ucsi_dev *)driver_data;
	...
	if (udev->ucsi_wake_lock->active) {
		pr_info("%s():ucsi wake unlock\n", __func__);
		__pm_relax(udev->ucsi_wake_lock);
	}
}

static void hihonor_ucsi_glink_notification(void *dev_data, u32 notification, void *data)
{
	struct ucsi_dev *udev = (struct ucsi_dev *)dev_data;

	if (!udev)
		return;

	if (notification == OEM_NOTIFY_CHARGER_DONE) {
		pr_info("%s():glink notify charger done\n", __func__);
		relax_ucsi_wakelock(udev);
	} else if (notification == OEM_NOTIFY_WIRED_CHANNEL_RESTORE) {
		pr_info("%s():glink notify wired channel restore\n", __func__);
		wired_chsw_set_wired_channel(WIRED_CHANNEL_MAIN, WIRED_CHSW_CLIENT_OTG, WIRED_CHANNEL_RESTORE);
	} else if (notification == OEM_NOTIFY_WIRED_CHANNEL_CUTOFF) {
		pr_info("%s():glink notify wired channel cutoff\n", __func__);
		wired_chsw_set_wired_channel(WIRED_CHANNEL_MAIN, WIRED_CHSW_CLIENT_OTG, WIRED_CHANNEL_CUTOFF);
	} else if (notification == OEM_NOTIFY_UCSI_RELEASE_LOCK) {
		pr_info("%s():glink notify ucsi release wake lock\n", __func__);
		relax_ucsi_wakelock(udev);
	}
	return;
}

static struct hihonor_glink_ops ucsi_glink_ops = {
	.notify_event = hihonor_ucsi_glink_notification,
};
#endif
...
static int ucsi_probe(struct platform_device *pdev)
{
	...
	int rc;
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	char udev_wakelock_name[NAME_MAX] = {0};
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	snprintf(udev_wakelock_name, sizeof(udev_wakelock_name),
			"udev_wakelock_%s", dev_name(udev->dev));
	udev->ucsi_wake_lock = wakeup_source_register(udev->dev, udev_wakelock_name);
	if (!udev->ucsi_wake_lock) {
		pmic_glink_unregister_client(udev->client);
		return -ENOMEM;
	}

	ucsi_glink_ops.dev_data = udev;
	(void)hihonor_oem_glink_ops_register(&ucsi_glink_ops);
#endif
	...
	if (rc) {
		...
		pmic_glink_unregister_client(udev->client);
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		(void)hihonor_oem_glink_ops_unregister(&ucsi_glink_ops);
		wakeup_source_unregister(udev->ucsi_wake_lock);
		udev->ucsi_wake_lock = NULL;
#endif
	}
