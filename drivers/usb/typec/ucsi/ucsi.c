...
#include <huawei_platform/usb/hihonor_usb_glink.h>
...
static enum usb_role pre_role = USB_ROLE_NONE;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		pd_dpm_set_pd_finish_flag(true);
#endif
		break;
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	if ((pre_role != USB_ROLE_NONE) && (pre_role != u_role)) {
		dev_info(ucsi->dev, "%s, set u_role = %d\n", __func__, u_role);
#endif
...
static void ucsi_handle_connector_change(struct work_struct *work)
{
	...
	if (!ucsi) {
		pr_err("%s():ucsi is NULL\n", __func__);
		return;
	}
#endif

	dev_info(ucsi->dev, "%s():start\n", __func__);
	mutex_lock(&con->lock);
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
	dev_info(ucsi->dev, "%s ,con->status.flags = 0x%04x, con->status.change = 0x%04x\n",
		__func__, con->status.flags, con->status.change);
#endif
	...
	if (con->status.change & UCSI_CONSTAT_CONNECT_CHANGE) {
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
			is_host = true;
			dev_info(ucsi->dev, "%s():constat partner type host\n", __func__);
			if (!wireless_charge_is_pwr_good()) {
				power_event_notify(POWER_NT_CONNECT, POWER_NE_USB_DISCONNECT, NULL);
				power_event_notify(POWER_NT_CHARGING, POWER_NE_STOP_CHARGING, NULL);
			}
		...
		case UCSI_CONSTAT_PARTNER_TYPE_AUDIO:
			if (con->status.change & UCSI_CONSTAT_PARTNER_CHANGE) {
				dev_info(ucsi->dev, "%s():constat partner type audio insert\n", __func__);
				is_audio = true;
				/**
				 * When otg disconnect, the data role will be TYPEC_HOST
				 * and nobody clear it until the next device connection,
				 * so we should set it to TYPEC_DEVICE when audio connect.
				 */
				typec_set_data_role(con->port, TYPEC_DEVICE);
			}
#endif
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
			if (!is_audio)
				hihonor_usb_glink_set_cc_insert(true);
			if ((u_role != USB_ROLE_HOST) && !is_audio) {
				stay_ucsi_wakelock(ucsi_get_drvdata(ucsi));
				power_event_notify(POWER_NT_CONNECT, POWER_NE_USB_CONNECT, NULL);
				power_event_notify(POWER_NT_CHARGING, POWER_NE_START_CHARGING, NULL);
...
#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
			hihonor_usb_glink_set_cc_insert(false);
			if (is_audio) {
				is_audio = false;
				dev_info(ucsi->dev, "%s():constat partner type audio disconnect\n", __func__);
			} else if (is_host) {
				is_host = false;
				wired_chsw_set_wired_channel(WIRED_CHANNEL_MAIN, WIRED_CHSW_CLIENT_OTG, WIRED_CHANNEL_RESTORE);
				dev_info(ucsi->dev, "%s():host disconnect\n", __func__);
				ret = usb_role_switch_set_role(ucsi->usb_role_sw, u_role);
				if (ret)
					dev_err(ucsi->dev, "%s(): failed to set role(%d):%d\n",
						__func__, u_role, ret);
				/* disable charging icon when PD data role is always host, ignore of wierless charging */
				if (!wireless_charge_is_pwr_good()) {
					power_event_notify(POWER_NT_CONNECT, POWER_NE_USB_DISCONNECT, NULL);
					power_event_notify(POWER_NT_CHARGING, POWER_NE_STOP_CHARGING, NULL);
				}
				...
				/* ignore notify disconnect event in wireless charger */
				...
				wireless_charge_wired_vbus_disconnect_handler();
				relax_ucsi_wakelock(ucsi_get_drvdata(ucsi));
			}
#endif
		}

#ifdef CONFIG_HUAWEI_POWER_EMBEDDED_ISOLATION
		if (u_role == USB_ROLE_HOST) {
#endif
			ret = usb_role_switch_set_role(ucsi->usb_role_sw, u_role);
			if (ret)
				dev_err(ucsi->dev, "%s(): failed to set role(%d):%d\n",
	...
	case UCSI_CONSTAT_PARTNER_TYPE_CABLE_AND_UFP:
		u_role = USB_ROLE_HOST;
		dev_info(ucsi->dev, "%s():USB_ROLE_HOST\n", __func__);
		break;
	case UCSI_CONSTAT_PARTNER_TYPE_DFP:
		u_role = USB_ROLE_DEVICE;
		dev_info(ucsi->dev, "%s():USB_ROLE_DEVICE\n", __func__);
		break;
	default:
		break;
	}
