...
static int msm_hsphy_enable_power(struct msm_hsphy *phy, bool on)
{
	...
	if (phy->power_enabled == on) {
		dev_info(phy->phy.dev, "%s PHYs' regulators are already %s.\n", __func__, on ? "on" : "off");
		return 0;
	}
	...
	pr_info("%s(): HSUSB PHY's regulators are turned ON.\n", __func__);
	return ret;
...
err_vdd:
	...
	dev_info(phy->phy.dev, "%s HSUSB PHY's regulators are turned OFF.\n", __func__);
	return ret;
}
...
static int msm_hsphy_probe(struct platform_device *pdev)
{
	...
	if (phy->param_override_seq_host_cnt > 0) {
		...
		if (!phy->param_override_seq_host) {
			devm_kfree(dev,phy->param_override_seq_host);
			return -ENOMEM;
		}
