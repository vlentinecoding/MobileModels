...
static void switch_delay_work(struct work_struct *data)
{
	...
	state = hw_usb_port_switch_request(14); /* switch to manufacture mode */
	pr_info("switch prot\n");
	...
	const struct usb_ctrlrequest *ctrl)
{
	...
	u16 w_index, w_value, w_length;
	int value = -EOPNOTSUPP;
	struct gadget_info *gi = NULL;
	int mode;
	int state = 0;
	bool flag = false;
	int mask = USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE;
	if (!cdev || !ctrl)
		return -EOPNOTSUPP;
	...
	case (USB_DIR_OUT | USB_TYPE_VENDOR):
		if (ctrl->bRequest != USB_REQ_SEND_HOST_TIME)
			break;
		flag = true;
	...
	case (USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE):
		if ((ctrl->bRequest != USB_REQ_VENDOR_SWITCH_MODE) ||
			(ctrl->bRequestType != mask) || (w_index != 0))
			break;
		flag = true;
		...
		if (w_value == INDEX_FACTORY_REWORK)
			...
			/* delay 500ms to avoid collapse */
		value = min(w_length, (u16)(sizeof(mode) + sizeof(state)));
		memcpy(req->buf, &state, value / HALF_BASE);
		memcpy(req->buf + value / HALF_BASE, &mode,
			value / HALF_BASE);
		break;
	case USB_DIR_IN:
		if (ctrl->bRequest != USB_REQ_VENDOR_PCINFO)
			break;
		flag = true;
