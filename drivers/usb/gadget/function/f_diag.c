		...
		struct usb_request *req)
{
	...
	memcpy(req_buffer_part_out, (u8 *)req->buf,
		sizeof(req_buffer_part_out) - 1);
	/* huawei diag log printf */
	if ((HW_HEAD1 == *((u8 *)req->buf)) &&
		((HW_HEAD2 == *((u8 *)req->buf + 1)) ||
		(HW_NEW_HEAD == *((u8 *)req->buf + 1))))
		print_hex_dump(KERN_INFO, "[USB_LOGS]Data to usb: ",
			PRE_FIX_TYPE, ROW_SIZE, DUMP_PREFIX_ADDRESS,
			req_buffer_part_out, MAX_BUF_SIZE, TRUE);

	/*printf  7B 00 01 02 03 04 05 06 07 08 09 9D 5F 7E */
	if ((QCOM_HEAD1 == *((u8 *)req->buf)) &&
		(QCOM_HEAD2 == *((u8 *)req->buf + 1)))
		print_hex_dump(KERN_INFO, "[USB_LOGS]Data to usb: ",
			PRE_FIX_TYPE, ROW_SIZE, DUMP_PREFIX_ADDRESS,
			req_buffer_part_out, MAX_BUF_SIZE, TRUE);
		...
		struct usb_request *req)
{
	...
	d_req->actual = req->actual;
	d_req->status = req->status;
	memcpy(req_buffer_part_in, (u8 *)req->buf,
		sizeof(req_buffer_part_in) - 1);
	/* huawei diag log printf */
	if ((HW_HEAD1 == *((u8 *)req->buf)) &&
		((HW_HEAD2 == *((u8 *)req->buf + 1)) ||
		(HW_NEW_HEAD == *((u8 *)req->buf + 1))))
		print_hex_dump(KERN_INFO, "[USB_LOGS]Data from usb: ",
			PRE_FIX_TYPE, ROW_SIZE, DUMP_PREFIX_ADDRESS,
			req_buffer_part_in, MAX_BUF_SIZE, TRUE);
