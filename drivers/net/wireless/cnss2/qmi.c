...
static const void *cnss_get_wlan_feature(void)
{
    int wifi_pubfile_id_len;
    struct device_node *dp = NULL;

    dp = of_find_node_by_path("/honor_wifi_info");
    if(!dp) {
...
static int cnss_get_bdfelf_filename(struct cnss_plat_data *priv,
				char *filename, const char *dts_prefix)
{
	char filename_tmp[MAX_FIRMWARE_NAME_LEN] = {0};
	char *filename_elf = filename_tmp;
	int filename_len = MAX_FIRMWARE_NAME_LEN;
	int len = 0;

	if(dts_prefix != NULL) {
		len = strlen(dts_prefix);
		if (len < (MAX_FIRMWARE_NAME_LEN -1) && (len > 0)) {
			len = snprintf(filename_tmp,
				filename_len, "%s_", dts_prefix);
			filename_elf = filename_tmp + len;
			filename_len = filename_len - len;
		} else {
			cnss_pr_err("pubfile_id is invalid\n");
			return -1;
		}
	}
	snprintf(filename_elf, filename_len, ELF_BDF_FILE_NAME);

	cnss_pr_dbg("elf file = %s\n", filename_tmp);
	cnss_bus_add_fw_prefix_name(priv, filename, filename_tmp);
	return 0;
}

static int cnss_request_bdfelf_by_name(const struct firmware **fw_entry,
				struct cnss_plat_data *priv, char *filename, const char *prefix)
{
	int ret = 0;

	ret = cnss_get_bdfelf_filename(priv, filename, prefix);
	if (ret != 0) {
		cnss_pr_err("get bdfelf filename failed, str=%s\n", prefix);
		return -1;
	}
	ret = firmware_request_nowarn(fw_entry, filename, &priv->plat_dev->dev);
	if (ret == 0) {
		cnss_pr_err("request %s success\n", filename);
	} else {
		cnss_pr_err("request %s failed,try other\n", filename);
		ret = -1;
	}
	return ret;
}
static int cnss_wlfw_bdfelf_request(const struct firmware **fw_entry,
				struct cnss_plat_data *priv, char *filename, u32 len)
{
	const char *wifi_pubfile_id = NULL;
	char *temp_board = NULL;
	char wifi_pubfile_str[MAX_FIRMWARE_NAME_LEN] = {0};
	int ret = 0;

	do {
		wifi_pubfile_id = cnss_get_wlan_feature();
		if (wifi_pubfile_id == NULL || (strlen(wifi_pubfile_id) <= 0)) {
			cnss_pr_err("No pubfile_id, use default elf\n");
			break;
		}

		/* match pubfileid */
		ret = cnss_request_bdfelf_by_name(fw_entry, priv, filename, wifi_pubfile_id);
		if (ret == 0) {
			cnss_pr_dbg("requset bdwlan success\n");
			return 0;
		}

		/* match board */
		memset(filename, 0, len);
		strncpy(wifi_pubfile_str, wifi_pubfile_id, MAX_FIRMWARE_NAME_LEN - 1);
		temp_board = strchr(wifi_pubfile_str, '_');
		if (temp_board == NULL) {
			cnss_pr_err("split %s failed\n", wifi_pubfile_str);
			break;
		}
		*temp_board = '\0';
		ret = cnss_request_bdfelf_by_name(fw_entry, priv, filename, wifi_pubfile_str);
		if (ret == 0) {
			cnss_pr_dbg("requset bdwlan success\n");
			return 0;
		}
	...
	/* match default */
	memset(filename, 0, len);
	ret = cnss_request_bdfelf_by_name(fw_entry, priv, filename, NULL);
	if (ret != 0) {
		cnss_pr_err("request all bdwlan file failed!\n");
	}
				 ...
				 u32 bdf_type)
{
	...
	if (bdf_type == CNSS_BDF_ELF) {
		ret = cnss_wlfw_bdfelf_request(&fw_entry, plat_priv, filename, sizeof(filename));
		if (ret)
			goto err_req_fw;
	} else {
