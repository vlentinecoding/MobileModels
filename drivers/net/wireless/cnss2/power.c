...
static const void *cnss_get_board_name(void)
{
	struct device_node *root_node = NULL;
	root_node = of_find_node_by_path("/rootparam");
	if (!root_node) {
		cnss_pr_err("device is not available!\n");
		return NULL;
	}
	return of_get_property(root_node,"qcom,board_name", NULL);
}
...
int cnss_get_pinctrl(struct cnss_plat_data *plat_priv)
{
	...
	const char *phone_model[] = {"ELZ_AN20_VN2","ELZ_AN10_VN2","ELZ_AN00_VN2",
				"ELZ_NX9_VN2","ELZ_AN60_VN2","ELZ_AN00_VN1_F",
				"ELZ_AN00_V4_A12","ELZ_AN00_V4_A2","ELZ_AN10_VN1_G",
				"ELZ_AN20_VN1_H","ELZ_AN10_V4_B","ELZ_AN10_V4_B12",
				"ELZ_AN10_V4_B2","ELZ_AN20_V4_C2","ELZ_AN20_VN2_RTRF",
				"ELZ_AN10_VN2_RTRF","ELZ_AN00_VN2_RTRF","ELZ_NX9_VN2_RTRF",
				"ELZ_AN60_VN2_RTRF","ELZ_AN00_VN2_RTRF2"};
	dev = &plat_priv->plat_dev->dev;
	...
	board_name = cnss_get_board_name();
	if (board_name == NULL) {
		cnss_pr_err("board_name is NULL, use default\n");
		return -EINVAL;
	}
	cnss_pr_info("board_name: %s\n", board_name);
	size = ARRAY_SIZE(phone_model);
	for (index = 0; index < size; index++) {
		if (strcmp(board_name, phone_model[index]) == 0) {
			cnss_set_feature_list(plat_priv, BOOTSTRAP_CLOCK_SELECT_V01);
			cnss_pr_info("find board_name successfully\n");
			break;
		}
	}
