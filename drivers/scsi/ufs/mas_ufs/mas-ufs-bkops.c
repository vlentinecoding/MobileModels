...
#include "mas_ufs_bkops_interface.h"
...
#if 1
static int ufshcd_query_attr_safe(struct ufs_hba *hba, enum query_opcode opcode,
...
static bool mas_ufs_is_bkops_id_match(struct mas_ufs_bkops_id *bkops_id,
	uint16_t man_id, const char *ufs_model, const char *ufs_rev)
{
	if (man_id != bkops_id->manufacturer_id)
		return false;

	if (strncmp(ufs_model, bkops_id->ufs_model, strlen(bkops_id->ufs_model)) &&
	    strcmp(MAS_UFS_MODEL_ANY, bkops_id->ufs_model))
		return false;

	if (strncmp(ufs_rev, bkops_id->ufs_rev, strlen(bkops_id->ufs_rev)) &&
	    strcmp(MAS_UFS_REV_ANY, bkops_id->ufs_rev))
		return false;

	return true;
}

static bool mas_ufs_is_bkops_supported(struct scsi_device *sdev)
{
	...
	{
		bkops_id = list_entry(node, struct mas_ufs_bkops_id, p);
		if (mas_ufs_is_bkops_id_match(bkops_id, hba->manufacturer_id,
		    sdev->model, sdev->rev))
			return false;
	}
...
int mas_ufs_manual_bkops_config(struct scsi_device *sdev)
{
	...
	if (!(sdev->host->mas_dev_quirk_flag &
		    SHOST_MAS_DEV_QUIRK(SHOST_QUIRK_BKOPS_ENABLE)))
		return 0;

	if (!mas_ufs_is_bkops_supported(sdev) || hba->ufs_bkops_enabled) {
	...
	debugfs_root = debugfs_create_dir("bkops", NULL);
	if (IS_ERR(debugfs_root))
		goto free_ufs_bkops;

	if (mas_bkops_enable(q, ufs_bkops, debugfs_root))
		goto err;
