...
static struct hist_pos g_hist_pos = {
        .cpa_pos = -1,
        .cdl_pos = -1,
        .cn_pos = -1,
        .ct_pos = -1,
...
int dsm_ufs_update_uic_info(struct ufs_hba *hba, int err_code)
{
	...
	if (test_and_set_bit_lock(UFS_UIC_TRANS_ERR, &g_ufs_dsm_adaptor.err_type))
	    return -1;
	if (g_hist_pos.cpa_pos != hba->ufs_stats.pa_err.pos) {
		pos = (hba->ufs_stats.pa_err.pos - 1) % UFS_ERR_REG_HIST_LENGTH;
		g_ufs_dsm_adaptor.uic_uecpa = hba->ufs_stats.pa_err.reg[pos];
		g_hist_pos.cpa_pos = hba->ufs_stats.pa_err.pos;
	} else {
		g_ufs_dsm_adaptor.uic_uecpa = 0;
	}
	if (g_hist_pos.cdl_pos != hba->ufs_stats.dl_err.pos) {
		pos = (hba->ufs_stats.dl_err.pos - 1) % UFS_ERR_REG_HIST_LENGTH;
		g_ufs_dsm_adaptor.uic_uecdl = hba->ufs_stats.dl_err.reg[pos];
		g_hist_pos.cdl_pos = hba->ufs_stats.dl_err.pos;
	} else {
		g_ufs_dsm_adaptor.uic_uecdl = 0;
	}
	if (g_hist_pos.cn_pos != hba->ufs_stats.nl_err.pos) {
		pos = (hba->ufs_stats.nl_err.pos - 1) % UFS_ERR_REG_HIST_LENGTH;
		g_ufs_dsm_adaptor.uic_uecn = hba->ufs_stats.nl_err.reg[pos];
		g_hist_pos.cn_pos = hba->ufs_stats.nl_err.pos;
	} else {
		g_ufs_dsm_adaptor.uic_uecn = 0;
	}
	if (g_hist_pos.ct_pos != hba->ufs_stats.tl_err.pos) {
		pos = (hba->ufs_stats.tl_err.pos - 1) % UFS_ERR_REG_HIST_LENGTH;
		g_ufs_dsm_adaptor.uic_uect = hba->ufs_stats.tl_err.reg[pos];
		g_hist_pos.ct_pos = hba->ufs_stats.tl_err.pos;
	} else {
		g_ufs_dsm_adaptor.uic_uect = 0;
	}
	if (g_hist_pos.dme_pos != hba->ufs_stats.dme_err.pos) {
		pos = (hba->ufs_stats.dme_err.pos - 1) % UFS_ERR_REG_HIST_LENGTH;
		g_ufs_dsm_adaptor.uic_uedme = hba->ufs_stats.dme_err.reg[pos];
		g_hist_pos.dme_pos = hba->ufs_stats.dme_err.pos;
	} else {
		g_ufs_dsm_adaptor.uic_uedme = 0;
	}
...
int dsm_ufs_update_error_info(struct ufs_hba *hba, int code)
{
	...
	case DSM_UFS_LIFETIME_EXCCED_ERR:
		...
		break;
	case DSM_UFS_DEVICE_RESET_ERR:
		(void)test_and_set_bit_lock(UFS_DEVICE_RESET_ERR, &g_ufs_dsm_adaptor.err_type);
		break;
...
int dsm_ufs_get_log(struct ufs_hba *hba, unsigned long code, char *err_msg)
{
	...
	get_ufs_bootdevice_product_name(product_name, PRODUCT_NAME_SIZE);
...
void dsm_ufs_handle_work(struct work_struct *work)
{
	...
	if (g_ufs_dsm_adaptor.err_type & (1 << UFS_ENTER_OR_EXIT_H8_ERR)) {
		...
		clear_bit_unlock(UFS_ENTER_OR_EXIT_H8_ERR, &g_ufs_dsm_adaptor.err_type);
	}
	if (g_ufs_dsm_adaptor.err_type & (1 << UFS_UTP_TRANS_ERR)) {
	...
	if (g_ufs_dsm_adaptor.err_type & (1 << UFS_DEVICE_RESET_ERR)) {
		DSM_UFS_LOG(hba, DSM_UFS_DEVICE_RESET_ERR, "UFS occur device reset and restore");
		clear_bit_unlock(UFS_DEVICE_RESET_ERR, &g_ufs_dsm_adaptor.err_type);
	}
	if (g_ufs_dsm_adaptor.err_type & (1 << UFS_TEMP_LOW_ERR)) {
...
void ufshcd_error_linkup_check(struct ufs_hba *hba)
{
	u32 avail_lane_rx, avail_lane_tx;
	u32 host_avail_rx, device_avail_rx;
	u32 host_avail_tx, device_avail_tx;
	unsigned long flags;
	...
	ufshcd_dme_get(hba, UIC_ARG_MIB(PA_AVAILTXDATALANES),
			&host_avail_tx);
	avail_lane_rx = min(host_avail_rx, device_avail_rx);
	avail_lane_tx = min(host_avail_tx, device_avail_tx);
...
void ufshcd_error_timeout_check(struct ufs_hba *hba, struct scsi_cmnd *scmd, int tag, bool found)
{
	if( true == found ) {
