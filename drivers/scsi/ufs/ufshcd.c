...
#ifdef CONFIG_HONOR_UFS_VENDOR_MODE
#include "ufs-boot-device.h"
#include "ufs-qcom.h"
...
struct ufs_hba *ufshcd_dev_to_hba(struct device *dev)
{
...
#ifdef CONFIG_HONOR_UFS_VENDOR_MODE
	hba->wb_permanent_disabled = false;
	set_bootdevice_wb_enable(0);
	if (of_property_read_bool(hba->dev->of_node, "ufs-no-wb")) {
		dev_dbg(hba->dev, "Found ufs-no-wb set, no support wb\n");
		return;
	}
...
#ifdef CONFIG_HONOR_UFS_VENDOR_MODE
void ufshcd_print_host_ufs_stats(struct ufs_hba *hba)
{
	ufshcd_print_err_hist(hba, &hba->ufs_stats.pa_err, "pa_err");
	ufshcd_print_err_hist(hba, &hba->ufs_stats.dl_err, "dl_err");
	ufshcd_print_err_hist(hba, &hba->ufs_stats.nl_err, "nl_err");
...
#ifndef CONFIG_HONOR_UFS_VENDOR_MODE
static
#endif
...
static
#endif
void ufshcd_print_tmrs(struct ufs_hba *hba, unsigned long bitmap)
{
	int tag;
...
#ifdef CONFIG_HONOR_UFS_VENDOR_MODE
	if(enable && hba->wb_permanent_disabled)
		return 0;
#endif
...
#ifdef CONFIG_HONOR_UFS_VENDOR_MODE
static void ufshcd_err_handler_qcom(struct kthread_work *work)
{
	struct ufs_hba *hba = NULL;
 ...
 * Returns
 *  IRQ_HANDLED - If interrupt is valid
 *  IR/Q_NONE    - If invalid interrupt
 */
...
static int ufs_get_device_desc(struct ufs_hba *hba)
{
	...
	u8 serial_num_index;
	u8 prl_index;
	u8 *buf_serial_num = NULL;
	u8 *buf_prl_index = NULL;
#endif
	struct ufs_dev_info *dev_info = &hba->dev_info;
...
#ifdef CONFIG_HONOR_UFS_VENDOR_MODE
    /* Get serial number */
	err = ufshcd_read_string_desc(hba, serial_num_index, &buf_serial_num, SD_RAW);
	if (err < 0) {
	...
	ufs_set_sec_unique_number(hba, buf_serial_num, dev_info->model);
	set_ufs_bootdevice_product_name(dev_info->model);
	set_ufs_bootdevice_manfid(hba->manufacturer_id);
	set_ufs_bootdevice_specversion(dev_info->wspecversion);
	/* Getting PRL */
	err = ufshcd_read_string_desc(hba, prl_index, &buf_prl_index, true);
	if (err < 0) {
	...
	strlcpy(dev_info->prl, buf_prl_index,
		min_t(u8, desc_buf[QUERY_DESC_LENGTH_OFFSET],
			MAX_PRL_LEN));

	/* Null terminate the PRL string */
	dev_info->prl[MAX_PRL_LEN] = '\0';

	set_ufs_fw_version(dev_info->prl);
#endif
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	kthread_init_work(&hba->eh_threadwork, ufshcd_err_handler_qcom);
#endif
