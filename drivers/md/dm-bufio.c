...
#define DM_BUFIO_DEFAULT_CLIENT_RETAIN_RATIO	70
#define DM_BUFIO_DEFAULT_RECLAIM_RETAIN_RATIO	95
#define DM_VALID_RATIO(x) (((x) > 0) && ((x) <= 100))
#endif
...
static unsigned long get_retain_buffers(struct dm_bufio_client *c)
{
	...
	if ((c->clt_retain_ratio != client_retain_ratio) &&
		DM_VALID_RATIO(client_retain_ratio)) {
	...
	if (DM_VALID_RATIO(reclaim_retain_ratio)) {
