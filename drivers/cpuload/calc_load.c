...
#define NR_CPUS_ONLINE nr_cpu_ids
...
static int pidstat_init(void)
{
	...
	for (i = 0; i < HIGH_LOAD_MAX_TYPE; i++) {
		...
		g_pid_stat_mgt[i].max_count =
			(CPUS_PROC_DURING / CPUS_PROC_PERIOD) * NR_CPUS_ONLINE;
...
static void init_freqs_data(const struct cpufreq_policy *policy)
{
	...
	cpumask_big = cpu_topology[NR_CPUS_ONLINE - 1].core_sibling.bits[0];
...
static void cpus_proc_static_high(u32 type)
{
	...
	for (i = 0, curr = g_pid_stat_mgt[type].head;
		...
		if (curr->count > get_threhold_bytype(type)) {
			tsk = find_task_by_vpid(curr->key_pid);
			pr_info("cpuload: key_pid:%d,comm:%s,count:%d,type:%u",
				curr->key_pid,
				tsk == NULL ? "unknown" : tsk->comm,
				curr->count, type);
...
static ssize_t cpumask_bg_show(struct kobject *kobj,
    struct kobj_attribute *attr, char *buf)
{
    if (buf == NULL) {
        pr_err("cpumask_bg_show buf is NULL");
        return -EINVAL;
    }
...
static ssize_t cpumask_bg_store(struct kobject *kobj,
    struct kobj_attribute *attr, const char *buf, size_t count)
{
    long value;
    int ret;
    ...
    if (buf == NULL) {
        pr_err("cpumask_bg_store buf is NULL");
        return -EINVAL;
    }

    ret = kstrtol(buf, MAX_BUF_LEN, &value);
    if (ret != 0 || value < 0 || value > 255) {
        pr_err("cpumask_bg_store kstrtol err, ret = %d, value = %d", ret, value);
        return -EINVAL;
    }
    cpumask_bg = value;
...
static struct kobj_attribute cpumask_bg_attribute = \
	__ATTR(cpumask_bg, DEFAULT_FILE_MODE, cpumask_bg_show, cpumask_bg_store);
...
static void high_freqs_load_tick(void)
{
	...
	if (!policy) {
		policy = cpufreq_cpu_get(NR_CPUS_ONLINE - 1);
		if (policy)
...
#ifdef CONFIG_HISI_FREQ_STATS_COUNTING_IDLE
	ret_err = hisi_time_in_freq_get(NR_CPUS_ONLINE - 1,
		freqs_time, freqs_len);
#endif
#ifdef CONFIG_HUAWEI_FREQ_STATS_COUNTING_IDLE
	ret_err = time_in_freq_get(NR_CPUS_ONLINE - 1,
		freqs_time, freqs_len);
#endif
	if (ret_err)
		return;
...
static void get_current_task_mask(u32 type)
{
	...
	for (cpu = 0; cpu < NR_CPUS_ONLINE; cpu++) {
...
static int __init calc_load_init(void)
{
	...
	ret = sysfs_create_file(high_load_kobj, &cpumask_bg_attribute.attr);
	if (ret)
		goto err_create_cpumask;
	...
	return 0;
err_create_pidstat:
	sysfs_remove_file(high_load_kobj, &cpumask_bg_attribute.attr);
err_create_cpumask:
