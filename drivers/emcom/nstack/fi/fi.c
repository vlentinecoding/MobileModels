...
static void fi_payload_parse(struct sk_buff *skb, struct fi_pkt_parse *pktinfo)
{
	__be16 payloadlen = 0;
	char *payload = NULL;
	...
	if (skb_shinfo(skb)->frag_list != NULL) {
		...
		while (frag_list && pktinfo->frag_num < FI_SKB_MAX_FRAG_LIST_NUM) {
		...
		for (i = 0; i < skb_shinfo(skb)->nr_frags && i < MAX_SKB_FRAGS; i++) {
			...
			if (!skb_is_gso(skb)) {
				...
				if (skb_frag_size(frag) % skb_shinfo(skb)->gso_size) {
					payloadlen = skb_frag_size(frag) % skb_shinfo(skb)->gso_size;
					payload = skb_frag_address_safe(frag) +
						(skb_frag_size(frag) / skb_shinfo(skb)->gso_size) * skb_shinfo(skb)->gso_size;
					pktinfo->frag_num += (skb_frag_size(frag) / skb_shinfo(skb)->gso_size) + 1;
				} else {
					payloadlen = skb_shinfo(skb)->gso_size;
					payload = skb_frag_address_safe(frag) +
						(skb_frag_size(frag) / skb_shinfo(skb)->gso_size - 1) * skb_shinfo(skb)->gso_size;
					pktinfo->frag_num += skb_frag_size(frag) / skb_shinfo(skb)->gso_size;
				}
				if (payload - (char*)skb_frag_address_safe(frag) + payloadlen >
					skb_frag_size(frag)) {
					payloadlen = skb_frag_size(frag);
					payload = skb_frag_address_safe(frag);
				}
...
static void fi_ip_save_5tuple_info(struct fi_pkt_parse *pktinfo, uint16_t protocol, const struct sk_buff *skb)
{
  ...
  */
 void do_gettimeofday_(struct timeval *tv)
 {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0)
	do_gettimeofday_(&(pktinfo->msg.tstamp));
#else
