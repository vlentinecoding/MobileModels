	...
	enum fi_dir dir)
{
	...
	if (!pktinfo->flow_cb.flow_identify_cfg->basic_rpt_en)
		return;
	if (dir == FI_DIR_UP) {
		flow_stat->raw_up_pkt_num += pktinfo->frag_num;
		flow_stat->raw_up_pkt_byte += pktinfo->msg.payloadlen;
	} else {
		flow_stat->raw_down_pkt_num += pktinfo->frag_num;
	...
	enum fi_dir dir)
{
	...
	if (dir == FI_DIR_UP) {
		flow_stat->up_pkt_num += pktinfo->frag_num;
		flow_stat->up_pkt_byte += pktinfo->msg.payloadlen;
	} else {
		flow_stat->down_pkt_num += pktinfo->frag_num;
