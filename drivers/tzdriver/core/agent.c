	...
	bool check_agent_id)
{
	...
	tloge("skip is_allowed_agent_ca\n");
	return 0;
	if (!check_agent_id) {
		for (i = 0; i < nr; i++) {
...
static int wait_agent_response(struct smc_event_data *event_data)
{
	...
	do {
		...
		if (need_freeze && !sig_pending) {
			rc = wait_event_freezable_timeout(event_data->ca_pending_wq,
	...
	unsigned int agent_id)
{
	...
	tloge("agent process work: wakeup the agent id: 0x%x\n", agent_id);
	wake_up(&event_data->wait_event_wq);
	tloge("agent 0x%x request, goto sleep, pe->run=%d\n",
