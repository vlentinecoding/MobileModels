...
extern bool ta_exception_enable;
...
extern struct siq_request_queue *siq_queue;
extern spinlock_t g_ta_excetion_lock;
...
#define FORCE_KILL_TA_ELAPSED 50
#define DUMP_TEE_BACKTRACE 25
#define DUMP_TA_CMD_INFO 1
...
void cmd_monitor_mark_retry(bool is_retry)
{
	struct cmd_monitor *monitor = NULL;
	pid_t pid = current->tgid;
	pid_t tid = current->pid;
	...
	list_for_each_entry(monitor, &g_cmd_monitor_list, list) {
		if (monitor->pid == pid && monitor->tid == tid) {
			monitor->is_retry = is_retry;
			break;
		}
	}
	mutex_unlock(&g_cmd_monitor_lock);
}
bool cmd_monitor_get_forcekillca_flag(void)
{
	struct cmd_monitor *monitor = NULL;
	pid_t pid = current->tgid;
	pid_t tid = current->pid;
	/*
	 * if the TA is stuck for more than 50 seconds,
	 * it will be killed and quit
	 */

	mutex_lock(&g_cmd_monitor_lock);
	list_for_each_entry(monitor, &g_cmd_monitor_list, list) {
		if (monitor->pid == pid && monitor->tid == tid) {
			if (monitor->is_force_kill_ta) {
				tloge("force kill ca\n");
				mutex_unlock(&g_cmd_monitor_lock);
				return true;
			}
	...
	list_for_each_entry_safe(monitor, tmp, &g_cmd_monitor_list, list) {
		if (monitor->returned) {
			...
			tloge("[cmd_monitor_tick] pid=%d,pname=%s,tid=%d, "
				...
				monitor->pid, monitor->pname,
				monitor->tid, monitor->tname,
				monitor->lastcmdid, monitor->count,
				monitor->agent_call_count, monitor->timetotal,
				g_cmd_monitor_list_size);
		...
		if (timedif > DUMP_TEE_BACKTRACE * S_TO_MS) {
			/* threads out of white table need info dump */
			if (timedif < 30 * S_TO_MS && (!monitor->is_reported)) {
				...
				if ((!(is_thread_in_white_table(monitor->tname))) &&
					...
					sr = (struct siq_request *)kmalloc(sizeof(struct siq_request), GFP_KERNEL);
					if (sr) {
						tloge("the ta may timeout, we will dump tee backtrace\n");
						(void)memset_s(sr, sizeof(*sr), 0x0, sizeof(*sr));
						INIT_LIST_HEAD(&sr->queuelist);
						sr->info.siq_dump = 1;
						spin_lock(&g_ta_excetion_lock);
						list_add_tail(&sr->queuelist, &siq_queue->queue_head);
						spin_unlock(&g_ta_excetion_lock);
						wakeup_tc_siq();
					}
				}
			} else {
				if (monitor->is_retry || !monitor->is_try_kill_ta) {
					tloge("the ta may timeout, we will try kill ca\n");
					if (!monitor->is_try_kill_ta)
						monitor->is_try_kill_ta = true;
					if (monitor->is_retry)
						monitor->is_retry = false;
					if (timedif > FORCE_KILL_TA_ELAPSED * S_TO_MS) {
						monitor->is_force_kill_ta = true;
					}
					if (ta_exception_enable) {
						if ((!(is_thread_in_white_table(monitor->tname))) &&
							(!is_tui_in_use(monitor->tid))) {
							sr = (struct siq_request *)kmalloc(sizeof(struct siq_request), GFP_KERNEL);
							if (sr) {
								(void)memset_s(sr, sizeof(*sr), 0x0, sizeof(*sr));
								INIT_LIST_HEAD(&sr->queuelist);
								sr->info.slot_label = monitor->slot_label;
								spin_lock(&g_ta_excetion_lock);
								list_add_tail(&sr->queuelist, &siq_queue->queue_head);
								spin_unlock(&g_ta_excetion_lock);
								wakeup_tc_siq();
		...
		} else if (timedif > DUMP_TA_CMD_INFO * S_TO_MS) {
			tloge("[cmd_monitor_tick] pid=%d,pname=%s,tid=%d, "
...
static struct cmd_monitor *init_monitor_locked(void)
{
	...
	newitem->is_retry = false;
	newitem->is_try_kill_ta = false;
	newitem->is_force_kill_ta = false;
	...
	do {
		list_for_each_entry(monitor, &g_cmd_monitor_list, list) {
			if (monitor->pid == pid && monitor->tid == tid) {
				...
				monitor->is_retry = false;
				monitor->is_force_kill_ta = false;
				monitor->is_try_kill_ta = false;
