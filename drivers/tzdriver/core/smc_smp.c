...
static DEFINE_MUTEX(g_msgq_lock);
static DEFINE_MUTEX(g_msgq_ext_lock);


/* Current state of the system */
static uint8_t g_sys_crash;
extern bool slot_enable;
...
#if (!defined(CONFIG_MINI_PLATFORM) && !defined(CONFIG_MDC_PLATFORM))
...
void show_cmd_bitmap(void)
{
	...
	if (memset_s(cmd_in, sizeof(int)* MAX_SMC_CMD, -1,
		sizeof(int)* MAX_SMC_CMD) ||
		memset_s(cmd_out, sizeof(int)* MAX_SMC_CMD, -1,
		sizeof(int)* MAX_SMC_CMD)) {
		tloge("memset failed\n");
		goto error;
	}
...
{
	return (cmd_monitor_get_forcekillca_flag() || (need_kill && sigkill_pending(current) &&
...
static void send_asm_smc_cmd(struct hh_msgq_desc *msgq, struct tc_ns_smc_cmd *cmd, struct smc_in_params *in_param,
	struct smc_out_params *out_param)
{
#ifdef CONFIG_TZDRIVER_SVM
    if (msgq) {
        tz_hvc_send_messge(msgq, (struct msgq_smc_in *)in_param, (struct msgq_smc_out *)out_param, true, 0);
    } else {
        if (cmd->label == HH_MSGQ_LABEL_TZ_SVC)
        tz_hvc_send_messge(NULL, (struct msgq_smc_in *)in_param, (struct msgq_smc_out *)out_param, true, 1);
        else
        tz_hvc_send_messge(NULL, (struct msgq_smc_in *)in_param, (struct msgq_smc_out *)out_param, true, 0);
    }
...
static noinline int smp_smc_send(struct hh_msgq_desc *msgq, struct tc_ns_smc_cmd *in,  uint32_t cmd, u64 ops, u64 ca,
...
retry:
	...
	isb();
	wmb();
	send_asm_smc_cmd(msgq, in, &in_param, &out_param);
	isb();
	wmb();
	...
	if (out_param.exit_reason == SMC_EXIT_PREEMPTED) {
		tloge("preempted return from tee\n");
		cmd_monitor_mark_retry(true);
		/*
...
{
#ifdef CONFIG_TZDRIVER_SVM
	struct msgq_smc_out out;
	struct hh_msgq_slot *slot = NULL;

	struct msgq_smc_in in = {
		.x0 = cmd,
		.x1 = cmd_addr,
		.x2 = cmd_type,
		.x3 = cmd_addr >> ADDR_TRANS_NUM};

	if (slot_enable) {
		get_msgq_slot(slot);
		in.x4 = slot->msgq->label;
	} else {
		in.x4 = HH_MSGQ_LABEL_TZ_SIQ;
    }
	if (slot) {
		tz_hvc_send_messge(slot->msgq, &in, &out, (wait != 0), 2);
	} else {
		tz_hvc_send_messge(NULL, &in, &out, (wait != 0), 2);
	}
	if (slot_enable) {
		put_msgq_slot(slot);
	}
...
void ta_exception(phys_addr_t mode, enum hh_msgq_label slot_label)
{
	raw_smc_send(TSP_TA_EXCEPTION, mode, slot_label, false);
}

struct siq_request_queue *siq_queue = NULL;
static int siq_thread_fn(void *arg)
{
	int ret;
	struct siq_request_queue *sqq = (struct siq_request_queue *)arg;
	struct siq_request *sr = NULL, *tmp = NULL;
	while (1) {
		...
		spin_lock(&g_ta_excetion_lock);
		list_for_each_entry_safe(sr, tmp,  &sqq->queue_head, queuelist) {
			if (sr->info.siq_dump) {
				siq_dump((phys_addr_t)(1));
			} else if (sr->info.slot_label) {
				ta_exception((phys_addr_t)(1), (enum hh_msgq_label)sr->info.slot_label);
			}
			list_del(&sr->queuelist);
			kfree(sr);
		}
		spin_unlock(&g_ta_excetion_lock);
...
void fiq_shadow_work_func(uint64_t target)
{
	...
	smp_smc_send(NULL, NULL, TSP_REQUEST, SMC_OPS_START_FIQSHD, current->pid,
		&secret, false);
	...
	int *last_index, const struct tc_ns_smc_cmd *cmd, u64 ops)
{
	...
	tloge("submit new cmd: cmd.ca=%u cmd-id=%x ev-nr=%u "
...
static int smp_smc_send_process(struct hh_msgq_desc *msgq, struct tc_ns_smc_cmd *cmd, u64 ops,
	struct smc_cmd_ret *cmd_ret, int cmd_index)
{
	...
	ret = smp_smc_send(msgq, cmd, TSP_REQUEST, ops, current->pid, cmd_ret,
...
static bool is_ca_killed(int cmd_index)
{
	...
	if (cmd_monitor_get_forcekillca_flag() || sigkill_pending(current)) {
...
#else
static void proc_ta_pending(struct hh_msgq_desc *msg_queue, struct pending_entry *pe,
    uint32_t cmd_index, u64 *ops)
{
	struct smc_out_params out_param = {0};
	bool kernel_call = false;
    ...
    if (!current->mm) {
        kernel_call = true;
        (void)wait_tee_wakeup(msg_queue, (struct msgq_smc_out *)&out_param);
    } else {
        (void)wait_tee_wakeup(msg_queue, (struct msgq_smc_out *)&out_param);
        *ops = SMC_OPS_SCHEDTO;
...
#endif
static int smp_smc_send_func(struct hh_msgq_desc *msgq, struct tc_ns_smc_cmd *in, bool reuse)
{
	int cmd_index = 0;
	int last_index = 0;
...
#ifdef CONFIG_TEE_ARCH_TIMER
	struct timeout_step_t timeout_step = { {0, 0, 0, 0}, TO_STEP_SIZE, -1 };
...
retry:
	...
	if (smp_smc_send_process(msgq, &cmd, ops, &cmd_ret, cmd_index) == -1)
		goto clean;

	if (!is_cmd_working_done(cmd_index)) {
		tloge("not complete the cmd_index=%d, ret:0x%x\n", cmd_index, cmd_ret.exit);
		show_cmd_bitmap();
		if (cmd_ret.exit == SMC_EXIT_NORMAL) {
...
#ifdef CONFIG_TEE_ARCH_TIMER
				enum pending_t pd_ret = proc_ta_pending(pe, &timeout_step,
					cmd_ret.ta, cmd_index, &ops);

				if (pd_ret != PD_DONE) {
					if (pd_ret == PD_WAKEUP)
						timeout_reset = true;
					tloge("timeout_reset retry\n");
					goto retry;
				}
#else
                proc_ta_pending(msgq, pe, cmd_index, &ops);
#endif
			}
			...
			tloge("invalid cmd work state, ret:0x%x\n", cmd_ret.exit);
			cmd.ret_val = TEEC_ERROR_GENERIC;
			goto clean;
		}
	}
...
static int smc_svc_thread_fn(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (slot_enable) {
			...
			smc_cmd.label = HH_MSGQ_LABEL_TZ_SVC;
		}
		if (slot)
			ret = smp_smc_send_func(slot->msgq, &smc_cmd, false);
		else
			ret = smp_smc_send_func(NULL, &smc_cmd, false);
		if (slot_enable) {
			put_msgq_slot(slot);
		}
	}
...
static int proc_tc_ns_smc(struct tc_ns_smc_cmd *cmd, bool reuse)
{
	...
	if (slot_enable) {
		...
		cmd->label = HH_MSGQ_LABEL_TZ;
	}
	tloge("%s cmd_id = 0x%x, is global task: %d\n",__func__, cmd->cmd_id, cmd->global_cmd);
	mutex_lock(&g_msgq_lock);
	item = cmd_monitor_log(cmd);
	if (slot)
		ret = smp_smc_send_func(slot->msgq, cmd, reuse);
	else
		ret = smp_smc_send_func(NULL, cmd, reuse);
	cmd_monitor_logend(item);
	mutex_unlock(&g_msgq_lock);
	if (slot_enable) {
		put_msgq_slot(slot);
	}
...
int smc_context_init(const struct device *class_dev)
{
	...
	siq_queue = (struct siq_request_queue *)kmalloc(sizeof(struct siq_request_queue), GFP_KERNEL);
	if (siq_queue == NULL) {
		tloge("sq alloc failed\n");
		return -ENOMEM;
	}
	INIT_LIST_HEAD(&siq_queue->queue_head);
	spin_lock_init(&g_ta_excetion_lock);
#ifdef CONFIG_TZDRIVER_SVM
    tz_hvc_get_mem(TZ_MEMORY_CMD_BUF, &base, &size);
    g_cmd_data = (struct tc_ns_smc_queue *)base;
    tlogi("tz_hvc_get_mem for cmd buf\n");
#else
	...
	g_siq_thread = kthread_create(siq_thread_fn, siq_queue, "siqthread/%d", 0);
