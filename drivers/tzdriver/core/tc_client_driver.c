...
static int tc_client_open(struct inode *inode, struct file *file)
{
    ...
    // if htee is not ready, just return again to teecd.
    if(tz_hvc_is_htee_ready() == 0){
        return -EAGAIN;
    }
...
struct of_device_id g_tzdriver_platform_match[] = {
	{ .compatible = "qcom,htee-msgq-pair" },
	{},
};
...
static int load_hw_info(void)
{
	...
	g_dev_node = of_find_compatible_node(NULL, NULL, "qcom,htee-haven");
	if (!g_dev_node) {
		tloge("No qcom,htee-haven compatible node found\n");
#ifndef CONFIG_ACPI
		return -ENODEV;
#endif
	}else {
		tlogd("Find qcom,htee-haven compatible node.\n");
	}
...
int tc_teeos_init(struct device *class_dev)
{
	int ret;
    ...
    // for SVM, mailbox_register will process it later
...
void tc_re_init(const struct device *class_dev)
{
	int ret;
...
static __init int tc_init(void)
{
	...
	tlogd("tc client init ++++ \n");

	ret = tc_ns_client_init(&class_dev);
	if (ret)
		return ret;

	ret = tz_hvc_init(class_dev);
	if (ret){
		tloge("tz_hvc_init failed ret=%d", ret); 
		goto class_device_destroy;
	}

	ret = tc_teeos_init(class_dev);
	if (ret){
		tloge("tc_teeos_init failed ret=%d", ret); 
		goto class_device_destroy;
	}
