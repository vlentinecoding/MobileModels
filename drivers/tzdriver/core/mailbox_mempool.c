...
int mailbox_register(const void *mb_pool, unsigned int size)
{
	...
	smc_cmd->operation_phys = virt_to_phys(mb_pool);
	smc_cmd->operation_h_phys = virt_to_phys(mb_pool) >> ADDR_TRANS_NUM;
...
int mailbox_mempool_init(void)
{
    ...
    tz_hvc_set_mem_pool((uint64_t)page_address(all_pages), MAILBOX_POOL_SIZE);
#else
