...
#define antiroot_debug(mask, x...) do { \
		if (get_antiroot_loglevel_mask() & mask) \
			ar_debug(x); \
	} while (0)

#define antiroot_error(fmt, args...) pr_err("%s: %s: %d: " fmt "\n", \
		ROOTAGENT_TAG, __func__, __LINE__, ## args)

#define antiroot_warning(fmt, args...) pr_warn("%s: %s: %d: " fmt "\n", \
		ROOTAGENT_TAG, __func__, __LINE__, ## args)

#define antiroot_info(fmt, args...) pr_info("%s: %s: %d: " fmt "\n", \
		ROOTAGENT_TAG, __func__, __LINE__, ## args)
...
#ifndef CONFIG_HUAWEI_EIMA
#define EIMA_MAX_MEASURE_OBJ_CNT 20
struct m_entry {
	uint8_t type;
	uint8_t hash_len;
	uint8_t hash[HASH_LENGTH];
	uint16_t fn_len;
	uint8_t fn[FN_LENGTH];
};

struct m_list_msg {
	uint8_t usecase[USECASE_LENGTH];
	uint32_t num;
	struct m_entry m_list[EIMA_MAX_MEASURE_OBJ_CNT];
...
/*
 * @ingroup  eima_agent_challenge
 *
 * @brief: challenge information
 *
 */
struct eima_agent_challenge {
	/* nounce as key for response */
	uint8_t nounce[CHALLENGE_NOUNCE_LENGTH];
};

/*
 * @ingroup  eima_agent_response
 *
 * @brief: response information
 *
 */
struct eima_agent_response {
	/* EIMA send data information */
	uint32_t msg_type;
};
...
union ragent_content {
	...
	struct eima_agent_challenge eima_challenge; /* eima challenge */
	struct eima_agent_response eima_response; /* eima response */
};
