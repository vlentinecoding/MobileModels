...
int check_proc_selinux_access(struct task_struct *task, const char *context)
{
	...
	rc = security_context_to_sid((uintptr_t)selinux_state_addr, context, strlen(context), &tid, GFP_KERNEL);
#endif
	if (rc) {
...
#ifdef	CONFIG_TZ_DRIVER_KERNEL4
	// This is only for kernel 4.x
	desc->shash.flags = 0;
#endif
