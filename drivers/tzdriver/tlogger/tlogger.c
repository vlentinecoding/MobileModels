	...
	struct tlogger_reader *reader, u32 *log_last_pos, u32 *is_read)
{
	...
	//tloge("tlogcat max len %x, %x\n", g_log_mem_len, buffer_max_len);
...
#ifdef CONFIG_TZDRIVER_SVM
	uint64_t size = 0; 
	tz_hvc_get_mem(TZ_MEMORY_LOG_RDR, addr, &size);
	*len = (uint32_t)size; 
#else
...
static int register_tloger(void)
{
	...
	tlogi("tlogcat verison 1.1.0 +++\n");

	ret = register_mem_cfg(&mem_addr, &g_log_mem_len);
	if (ret)
		return ret;

	tlogd("tlogcat memsize %x, %x\n", mem_addr, g_log_mem_len);
	...
	tlogi("tlogcat registered ended\n");
...
{
	tlogi("tlogcat init_tlogger_service module +++\n");
...
{
	tlogi("tlogcat init_tlogger_service +++ \n");
