...
static const struct dynamic_mem_config g_dyn_mem_config[] = {
	...
	{ TEE_SERVICE_TEST_DYNION, SEC_HIAI },
#endif
...
static int unregister_from_tee(struct dynamic_mem_item *mem_item)
{
	...
	if (tc_ns_smc(&smc_cmd)) {
		ret = -EPERM;
		tlogd("unregister from tee failed\n");
	}
	mailbox_free(phys_to_virt(mem_item->memory.ion_phys_addr));
	mailbox_free(mb_pack);
...
#ifdef CONFIG_HW_SECMEM
	ion_sg_table = cma_secmem_alloc(mem_item->ddr_sec_region,
		mem_item->memory.len);
#else
	ion_sg_table = hisi_secmem_alloc(mem_item->ddr_sec_region,
		mem_item->memory.len);
#endif
	if (!ion_sg_table) {
...
#ifdef CONFIG_HW_SECMEM
	cma_secmem_free(mem_item->ddr_sec_region,
		mem_item->memory.dyn_sg_table);
#else
	hisi_secmem_free(mem_item->ddr_sec_region,
		mem_item->memory.dyn_sg_table);
#endif
