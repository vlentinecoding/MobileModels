...
static void hisi_hihdr_top_init(struct dpu_fb_data_type *dpufd, char __iomem *hdr_base)
{
	uint64_t inv_size;
...
void init_hihdr(struct dpu_fb_data_type *dpufd)
{
	...
	hisi_hihdr_top_init(dpufd, hdr_base);
	hisi_hihdr_itm_init(dpufd, hdr_base);
...
static void hisi_dss_hihdr_global_control(struct dpu_fb_data_type *dpufd, dss_hihdr_info_t *hdr_info)
{
	char __iomem *hdr_base = NULL;
	hdr_base = dpufd->dss_base + DSS_HDR_OFFSET;
...
void hisi_dss_hihdr_set_reg(struct dpu_fb_data_type *dpufd)
{
	...
	hisi_dss_hihdr_global_control(dpufd, dpufd->hdr_info);

	if (g_debug_effect_hihdr)
		DPU_FB_INFO("[hihdr] hihdr_status = %d", dpufd->hdr_info->hihdr_status);

	hisi_dss_hihdr_itm_set_reg(dpufd, &dpufd->hdr_info->itm_info);
	hisi_dss_hihdr_gtm_set_reg(dpufd, &dpufd->hdr_info->gtm_info);
}
