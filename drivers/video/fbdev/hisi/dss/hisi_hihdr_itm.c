...
static void hisi_hihdr_itm_gamma_init(struct dpu_fb_data_type *dpufd, char __iomem *hdr_base)
{
	/* step map */
...
static void hisi_hihdr_itm_gamut_init(struct dpu_fb_data_type *dpufd, char __iomem *hdr_base)
{
...
static void hisi_hihdr_itm_degamma_init(struct dpu_fb_data_type *dpufd, char __iomem *hdr_base)
{
	/* step map */
...
void hisi_hihdr_itm_init(struct dpu_fb_data_type *dpufd, char __iomem *hdr_base)
{
	dpu_check_and_no_retval(!hdr_base, ERR, "hdr_base is nullptr!\n");

	/* CSC static config: SRGB -> P3 */
	hisi_hihdr_itm_gamma_init(dpufd, hdr_base);
	hisi_hihdr_itm_gamut_init(dpufd, hdr_base);
	hisi_hihdr_itm_degamma_init(dpufd, hdr_base);
}
...
static void hihdr_itm_set_basic_reg(struct dpu_fb_data_type *dpufd,  hihdr_itm_info_t *itm_info, char __iomem *hdr_base)
{
	...
	/* itm lut */
	hisi_dss_lut_set_reg(dpufd, hdr_base + DSS_ITM_LUT_CHROMA,
		ITM_LUT_CHROMA_SIZE / 2, (uint32_t *)(&itm_info->dynamic_info.lut_chroma));

	hisi_dss_lut_set_reg(dpufd, hdr_base + DSS_ITM_LUT_CHROMA0,
		ITM_LUT_CHROMA0_SIZE / 2, (uint32_t *)(&itm_info->dynamic_info.lut_chroma0));

	hisi_dss_lut_set_reg(dpufd, hdr_base + DSS_ITM_LUT_CHROMA1,
		ITM_LUT_CHROMA1_SIZE / 2, (uint32_t *)(&itm_info->dynamic_info.lut_chroma1));

	hisi_dss_lut_set_reg(dpufd, hdr_base + DSS_ITM_LUT_CHROMA2,
		ITM_LUT_CHROMA2_SIZE / 2, (uint32_t *)(&itm_info->dynamic_info.lut_chroma2));

	hisi_dss_lut_set_reg(dpufd, hdr_base + DSS_ITM_LUT_LUMALUT0,
		ITM_LUT_LUMALUT0_SIZE / 2, (uint32_t *)(&itm_info->dynamic_info.lut_lumalut0));

	hisi_dss_lut_set_reg(dpufd, hdr_base + DSS_ITM_LUT_CHROMALUT0,
...
{
	hisi_dss_lut_set_reg(dpufd, hdr_base + DSS_ITM_LUT_LUMA,
		ITM_LUT_LUMA_SIZE / 2, (uint32_t *)(&itm_info->dynamic_info.lut_luma));
};

static void hisi_dss_hihdr_itm_sdr_enhance(struct dpu_fb_data_type *dpufd, hihdr_itm_info_t *itm_info,
...
static void hisi_hihdr_itm_csc_trigger(struct dpu_fb_data_type *dpufd, hihdr_itm_info_t *itm_info,
	uint32_t itm_ctl, uint32_t itm_roi)
{
	char __iomem *hdr_base = dpufd->dss_base + DSS_HDR_OFFSET;
...
static void hisi_dss_hihdr_itm_ctl(hihdr_itm_info_t *itm_info, uint32_t *out_itm_ctl, uint32_t *out_itm_roi)
{
...
void hisi_dss_hihdr_itm_set_reg(struct dpu_fb_data_type *dpufd, hihdr_itm_info_t *itm_info)
{
	uint32_t itm_ctl = 0;
	uint32_t itm_roi = 0;
	...
	hisi_dss_hihdr_itm_ctl(itm_info, &itm_ctl, &itm_roi);
	hisi_hihdr_itm_csc_trigger(dpufd, itm_info, itm_ctl, itm_roi);
	hisi_dss_hihdr_itm_sdr_enhance(dpufd, itm_info, itm_ctl, itm_roi);
}
