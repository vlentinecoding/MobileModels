...
void hisi_alsc_set_reg(struct dpu_fb_data_type *dpufd)
{
	char __iomem *alsc_base = NULL;
	struct dss_alsc *alsc = NULL;
...
void hisi_alsc_update_dirty_region_limit(struct dpu_fb_data_type *dpufd)
{
	struct dss_alsc *alsc = NULL;
...
void hisi_alsc_store_data(struct dpu_fb_data_type *dpufd, uint32_t isr_flag)
{
...
void hisi_alsc_blank(struct dpu_fb_data_type *dpufd, int blank_mode)
{
	struct dss_alsc *alsc = NULL;
...
void hisi_alsc_init(struct dpu_fb_data_type *dpufd)
{
	dpu_check_and_no_retval(!dpufd, ERR, "dpufd is NULL!\n");
	...
	if (!g_is_alsc_init) {
		...
		INIT_WORK(&dpufd->alsc_send_data_work, hisi_alsc_send_data_work_func);
...
void hisi_alsc_deinit(struct dpu_fb_data_type *dpufd)
{
	dpu_check_and_no_retval(!dpufd, ERR, "dpufd is NULL!\n");
...
int dss_alsc_update_degamma_coef(const struct hisi_dss_alsc* const in_alsc)
{
	struct dpu_fb_data_type *fb0 = dpufd_list[PRIMARY_PANEL_IDX];
	dpu_check_and_return(!in_alsc, -1, ERR, "in_alsc is NULL!\n");
