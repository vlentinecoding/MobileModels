...
int hisi_fb_blank_sub(int blank_mode, struct fb_info *info)
{
	...
	hisi_alsc_blank(dpufd, blank_mode);
#endif
...
#ifdef CONFIG_HISI_EFFECT_HIHDR
	set_flag_bits_if_cond(dpu_dss_check_hihdr_active(dpufd), flag, HIHDR_ENABLED_DIRTY_UPDT_DISABLE);
#endif
...
#ifdef CONFIG_HISI_EFFECT_HIHDR
	case HISIFB_HIHDR_MEAN_GET:
		ret = dpufb_hihdr_get_mean(dpufd, argp);
		break;

	case HISIFB_HIHDR_BASIC_CONFIG_GET:
...
static int hisi_fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	if (dpufd->index == MEDIACOMMON_PANEL_IDX)
		return hisi_mediacommon_ioctl_handler(dpufd, cmd, argp);
#endif
