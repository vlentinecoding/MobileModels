...
int dpe_set_clk_rate(struct platform_device *pdev)
{
	...
	if (!is_clk_match_low_temp_cstr(dpufd, tmp.dss_pri_clk_rate)) {
		DPU_FB_INFO("cur_freq %llu > low_temp_freq %llu", tmp.dss_pri_clk_rate, MAX_DSS_CORE_CLK_RATE_LOW_TEMP);
		tmp.dss_pri_clk_rate = MAX_DSS_CORE_CLK_RATE_LOW_TEMP;
		dpufd->dss_vote_cmd.dss_pri_clk_rate = tmp.dss_pri_clk_rate;
	}
#endif
