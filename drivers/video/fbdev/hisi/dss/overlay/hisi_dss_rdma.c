		...
		char __iomem *dma_base, dss_rdma_t *s_dma)
{
	if (s_dma->afbc_used) {
		...
		dpufd->set_reg(dpufd, dma_base + AFBCD_CREG_FBCD_CTRL_MODE, 0x0, 32, 0);
#endif
	...
	char __iomem *dma_base, dss_rdma_t *s_dma)
{
	...
	if (dpufd->video_idle_ctrl.idle_frame_display)
		DPU_FB_DEBUG("data_addr0=0x%x, header=0x%x, payload=0x%x\n", s_dma->data_addr0,
	...
	if (s_dma->cld_used == 1) {
		dpufd->set_reg(dpufd, dma_base + DMA_OFT_X0, s_dma->oft_x0, 32, 0);
		dpufd->set_reg(dpufd, dma_base + DMA_OFT_Y0, s_dma->oft_y0, 32, 0);
		dpufd->set_reg(dpufd, dma_base + DMA_OFT_X1, (s_dma->cld_width - 1) / 4, 32, 0);
		dpufd->set_reg(dpufd, dma_base + DMA_OFT_Y1, (s_dma->cld_height - 1), 32, 0);
		dpufd->set_reg(dpufd, dma_base + DMA_STRETCH_SIZE_VRT, (s_dma->cld_height - 1), 32, 0);
	}
#endif
	...
	if (s_dma->hfbcd_used)
		hisi_dss_hfbcd_set_reg(dpufd, dma_base, s_dma);
#endif

#ifdef SUPPORT_DSS_HEBCD
	if (s_dma->hebcd_used)
		hisi_dss_hebcd_set_reg(dpufd, dma_base, s_dma);
#endif
}
	...
	dss_rect_ltrb_t afbc_rect, struct rdma_config_afbcd_param *afbcd_param)
{
	...
	if (afbcd_param->header_start_pos != 0) {
		DPU_FB_ERR("aligned_rect.left[%d] no equal to afbc_rect.left[%d] is err.\n",
			aligned_rect.left, afbc_rect.left);
		return -EINVAL;
	}
#endif
...
#if defined(CONFIG_RCH_CLD)
static void rdma_cld_config(struct dpu_fb_data_type *dpufd, dss_layer_t *layer)
{
	dss_rdma_t *dma = &(dpufd->dss_module.rdma[layer->chn_idx]);
...
#ifdef SUPPORT_DSS_HFBCD
		ret = hisi_dss_hfbcd_config(dpufd, ovl_idx, layer, ov_compose_rect, ov_compose_flag);
		dpu_check_and_return((ret != 0), -EINVAL, ERR, "hisi_dss_hfbcd_config failed! ret = %d\n", ret);
#endif
		return 0;
	}
...
#ifdef SUPPORT_DSS_HEBCD
		ret = hisi_dss_hebcd_config(dpufd, ovl_idx, layer, ov_compose_rect, ov_compose_flag);
		 dpu_check_and_return((ret != 0), -EINVAL, ERR, "hisi_dss_hebcd_config failed! ret = %d\n", ret);
#endif
		return 0;
	}
