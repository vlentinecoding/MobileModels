...
static int check_nve_partition_name(const struct nve_partition_header_struct *nve_partition_header)
{
	if (!nve_partition_header) {
		return -EINVAL;
	}

	if (strstr(nve_partition_header->nve_partition_name, NVE_MAGIC) == NULL) {
		return -EINVAL;
	}
			       ...
			       uint32_t index)
{
	 ...
	 */
	pr_info("[%s]line:%d  Checking partition[%u] name!\n", __func__, __LINE__, index);
	ret = check_nve_partition_name(nve_partition_header);
	if (ret) {
		pr_err("[%s]line:%d  Checking name failed!\n");
		return ret;
	}
...
static int nve_restore(struct nve_ramdisk_struct *nvep)
{
	...
	if (nvep->nve_current_id == NVE_INVALID_NVM) {
		 ...
		 */
		if (check_nve_partition_name(nve_store_header)) {
...
static long nve_ioctl(struct file *file, u_int cmd, u_long arg)
{
...
#else
		if (!access_ok(VERIFY_READ, (const void *)arg, size)) {
#endif
	...
	if (cmd & IOC_OUT) {
		...
		if (!access_ok((const void *)arg, size)) {
#else
		if (!access_ok(VERIFY_WRITE, (const void *)arg, size)) {
#endif
