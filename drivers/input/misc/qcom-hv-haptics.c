...
#ifdef CONFIG_HN_VIBRATOR_CALIB
...
struct haptics_chip {
	...
	u32				calib_freq_max;
#endif
#ifdef CONFIG_HN_HAPTICS_WRAPPER
	struct haptics_effect		firmware_effect;
#endif
};
...
#define HBOOST_WAIT_READY_COUNT		1000
...
static int haptics_load_constant_effect(struct haptics_chip *chip, u8 amplitude)
{
	...
	/* enable auto resonance for LRA unless disabled deliberately */
	rc = haptics_enable_autores(chip,
		!chip->config.is_erm && !chip->config.auto_res_disable);
	if (rc < 0)
		goto unlock;
...
#ifdef CONFIG_HN_HAPTICS_WRAPPER
#define HN_MAX_VMAX_MV		10000
#define HN_MIN_VMAX_MV		1000

static void hn_init_firmware_effect(struct haptics_chip *chip)
{
	/* firmware effect will be played in FIFO mode without brake */
	chip->firmware_effect.pattern = NULL;
	chip->firmware_effect.brake = NULL;
	chip->firmware_effect.id = UINT_MAX - 1;
	chip->firmware_effect.vmax_mv = chip->config.vmax_mv;
	chip->firmware_effect.t_lra_us = chip->config.t_lra_us;
	chip->firmware_effect.src = FIFO;
	chip->firmware_effect.auto_res_disable = true;
	if (chip->clamp_at_5v)
		chip->firmware_effect.vmax_mv = CLAMPED_VMAX_MV;
	else
		chip->firmware_effect.vmax_mv = HN_MAX_VMAX_MV;
}

static int hn_haptics_load_constant(struct haptics_chip *chip, s16 magnitude)
{
	u32 tmp;
	u8 amplitude;

	tmp = get_direct_play_max_amplitude(chip);
	amplitude = tmp * magnitude / 0x7fff;
	dev_dbg(chip->dev, "load constant effect, amplitude=%#x\n", amplitude);
	return haptics_load_constant_effect(chip, amplitude);
}

static int hn_haptics_load_fw_effect(struct haptics_chip *chip,
	struct fifo_cfg *fifo, s16 magnitude)
{
	struct haptics_play_info *play = &chip->play;
	int rc = 0;

	mutex_lock(&play->lock);
	if (play->in_calibration) {
		dev_err(chip->dev, "calibration in progress, skipping\n");
		mutex_unlock(&play->lock);
		return -EBUSY;
	}
	dev_dbg(chip->dev, "load firmware effect\n");
	chip->firmware_effect.fifo = fifo;
	play->vmax_mv = (magnitude * chip->firmware_effect.vmax_mv) / 0x7fff;
	rc = haptics_load_predefined_effect(chip, &chip->firmware_effect);
	mutex_unlock(&play->lock);
	return rc;
}

int hn_haptics_set_effects_vmax(void *priv, u32 vmax_mv)
{
	struct haptics_chip *chip = (struct haptics_chip *)priv;

	if (priv == NULL) {
		pr_err("%s: illegal input\n", __func__);
		return -EINVAL;
	}

	if (vmax_mv > HN_MAX_VMAX_MV)
		vmax_mv = HN_MAX_VMAX_MV;
	if (chip->clamp_at_5v && (vmax_mv > CLAMPED_VMAX_MV))
		vmax_mv = CLAMPED_VMAX_MV;
	if (vmax_mv < HN_MIN_VMAX_MV)
		vmax_mv = HN_MIN_VMAX_MV;
	if (chip->custom_effect != NULL)
		chip->custom_effect->vmax_mv = vmax_mv;
	chip->firmware_effect.vmax_mv = vmax_mv;

	return 0;
}
EXPORT_SYMBOL(hn_haptics_set_effects_vmax);

int hn_haptics_get_effects_vmax(void *priv, u32 *p_vmax_mv)
{
	struct haptics_chip *chip = (struct haptics_chip *)priv;

	if ((priv == NULL) || (p_vmax_mv == NULL)) {
		pr_err("%s: illegal input\n", __func__);
		return -EINVAL;
	}
	*p_vmax_mv = chip->firmware_effect.vmax_mv;
	return 0;
}
EXPORT_SYMBOL(hn_haptics_get_effects_vmax);

int hn_haptics_get_calib_f0(void *priv, u32 *p_f0)
{
	struct haptics_chip *chip = (struct haptics_chip *)priv;

	if ((priv == NULL) || (p_f0 == NULL)) {
		pr_err("%s: illegal input\n", __func__);
		return -EINVAL;
	}
	if (chip->config.cl_t_lra_us == 0) {
		dev_err(chip->dev, "%s: cl_t_lra_us is 0\n", __func__);
		return -EINVAL;
	}
	/* multipled by 10 to keep tenths place */
	*p_f0 = USEC_PER_SEC * 10 / chip->config.cl_t_lra_us;
	return 0;
}
EXPORT_SYMBOL(hn_haptics_get_calib_f0);
...
int hn_haptics_load_effect(void *priv, struct fifo_cfg *fifo, s16 magnitude)
{
	...
	if (atomic_read(&play->is_playing) > 0) {
		pr_err("%s: is playing, skipping\n", __func__);
		return -EBUSY;
	}
	atomic_set(&play->is_playing, 1);

	if (fifo == NULL)
		rc = hn_haptics_load_constant(chip, magnitude);
	else
		rc = hn_haptics_load_fw_effect(chip, fifo, magnitude);

	if (rc < 0) {
		dev_err(chip->dev, "load effect failed, rc=%d\n", rc);
		atomic_set(&play->is_playing, 0);
		return rc;
	}
	...
	if (rc < 0) {
		dev_err(chip->dev, "enable hboost failed, rc=%d\n", rc);
		atomic_set(&play->is_playing, 0);
	}
	return rc;
}
EXPORT_SYMBOL(hn_haptics_load_effect);
...
int hn_haptics_enable_play(void *priv, bool en)
{
	...
	play = &chip->play;
	dev_dbg(chip->dev, "enable_play %d\n", en);
	...
	if ((play->pattern_src == FIFO) &&
			atomic_read(&play->fifo_status.is_busy)) {
		if (atomic_read(&play->fifo_status.written_done) == 0) {
			dev_dbg(chip->dev, "cancelling FIFO\n");
			atomic_set(&play->fifo_status.cancelled, 1);
		}
		...
		atomic_set(&play->is_playing, 0);
		if (rc < 0)
			dev_err(chip->dev, "stop FIFO failed, rc=%d\n", rc);
...
EXPORT_SYMBOL(hn_haptics_enable_play);

static int hn_haptics_upload_effect(struct input_dev *dev,
		struct ff_effect *effect, struct ff_effect *old)
{
	struct haptics_chip *chip = input_get_drvdata(dev);
	int rc;
	...
	if (atomic_read(&chip->play.is_playing) > 0) {
		pr_err("%s: is playing, skipping\n", __func__);
		return -EBUSY;
        }
	atomic_set(&chip->play.is_playing, 1);
	rc = haptics_upload_effect(dev, effect, old);
	if (rc < 0) {
		atomic_set(&chip->play.is_playing, 0);
		return rc;
	}
	return 0;
}
...
#ifdef CONFIG_HN_VIBRATOR_CALIB
#define VIBRATOR_CALIB_RANGE_SIZE 2
#define DEFAULT_CALIB_FREQ_MIN (170 - 15)
#define DEFAULT_CALIB_FREQ_MAX (170 + 15)
static void haptics_parse_calib_range(struct haptics_chip *chip)
{
	struct device_node *node = chip->dev->of_node;
	unsigned int freq_range[VIBRATOR_CALIB_RANGE_SIZE];
	int rc;

	chip->calib_freq_min = DEFAULT_CALIB_FREQ_MIN;
	chip->calib_freq_max = DEFAULT_CALIB_FREQ_MAX;
	rc = of_property_read_u32_array(node, "calib_freq_range",
		freq_range, VIBRATOR_CALIB_RANGE_SIZE);
	if (rc < 0) {
		dev_warn(chip->dev,
			"Calib_freq range not set, use default: %u~%u\n",
			chip->calib_freq_min, chip->calib_freq_max);
		return;
	}
	if ((freq_range[0] == 0) || (freq_range[0] > freq_range[1])) {
		dev_err(chip->dev,
			"Calib_freq range error, use default: %u~%u\n",
			chip->calib_freq_min, chip->calib_freq_max);
		return;
	}
	chip->calib_freq_min = freq_range[0];
	chip->calib_freq_max = freq_range[1];
	dev_info(chip->dev, "Calib_freq range set to %u~%u\n",
		chip->calib_freq_min, chip->calib_freq_max);
}
#endif
...
static int haptics_parse_dt(struct haptics_chip *chip)
{
	...
	config->auto_res_disable = of_property_read_bool(node, "auto_res_disable");
#ifdef CONFIG_HN_VIBRATOR_CALIB
	haptics_parse_calib_range(chip);
#endif
...
#ifdef CONFIG_HN_VIBRATOR_CALIB
#define CALIB_T_LRA_US_MIN (USEC_PER_SEC / (chip->calib_freq_max))
#define CALIB_T_LRA_US_MAX (USEC_PER_SEC / (chip->calib_freq_min))
	...
	struct device_attribute *attr, char *buf)
{
	...
	chip = container_of(cdev, struct haptics_chip, led_dev);
	dev_info(chip->dev, "Calib_freq range: %u~%u, cl_t_lra_us: %u\n",
		chip->calib_freq_min, chip->calib_freq_max,
		chip->config.cl_t_lra_us);
	if ((chip->config.cl_t_lra_us > CALIB_T_LRA_US_MAX) ||
		(chip->config.cl_t_lra_us < CALIB_T_LRA_US_MIN))
		cali_result = 0;
	else
		cali_result = 1;

	return scnprintf(buf, PAGE_SIZE, "%d", cali_result);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	/* multipled by 10 to fit factory threshold */
	cl_f_lra = USEC_PER_SEC * 10 / chip->config.cl_t_lra_us;
	return scnprintf(buf, PAGE_SIZE, "%d\n", cl_f_lra);
}

static struct device_attribute vib_attrs[] = {
	__ATTR_RW(vibrator_calib),
	__ATTR_RO(cali_val),
};

/* Dummy functions for brightness */
static enum led_brightness hn_vib_brightness_get(struct led_classdev *cdev)
{
	return 0;
}

static void hn_vib_brightness_set(struct led_classdev *cdev,
			enum led_brightness level)
{
}

static void hn_register_calib(struct platform_device *pdev)
{
	struct haptics_chip *chip = dev_get_drvdata(&pdev->dev);
	int i;
	int ret;

	dev_info(&pdev->dev, "Registering led_dev for calib\n");
	chip->led_dev.name = "vibrator";
	chip->led_dev.brightness_get = hn_vib_brightness_get;
	chip->led_dev.brightness_set = hn_vib_brightness_set;
	chip->led_dev.max_brightness = 100;
	ret = devm_led_classdev_register(&pdev->dev, &chip->led_dev);
	if (ret < 0) {
		dev_err(&pdev->dev, "Registering led_dev ret=%d\n", ret);
		return;
	}

	for (i = 0; i < ARRAY_SIZE(vib_attrs); i++) {
		ret = sysfs_create_file(&chip->led_dev.dev->kobj,
				&vib_attrs[i].attr);
		if (ret < 0) {
			dev_err(&pdev->dev,
				"Creating led sysfs file %d ret=%d\n",
				i, ret);
			return;
		}
	}
...
static int haptics_probe(struct platform_device *pdev)
{
	...
	ff_dev = input_dev->ff;
#ifdef CONFIG_HN_HAPTICS_WRAPPER
	ff_dev->upload = hn_haptics_upload_effect;
#else
