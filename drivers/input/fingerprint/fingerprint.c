...
void fp_set_touch_info(struct tp_to_udfp_data *tp_data)
{
	struct fp_data *fp = g_fp_data;
	if (tp_data->udfp_event != TP_EVENT_FINGER_DOWN)
		return;
	if (fp == NULL) {
		fpc_log_err("%s fp is null\n", __func__);
		return;
	}
	fp->fp_touch_info.touch_x = tp_data->tp_x;
	fp->fp_touch_info.touch_y = tp_data->tp_y;
	fp->fp_touch_info.touch_major = tp_data->tp_major;
	fp->fp_touch_info.touch_minor = tp_data->tp_minor;
	fp->fp_touch_info.touch_ori = tp_data->tp_ori;
}
...
static int fp_ioc_get_touch_info(struct fp_data *fp, void __user *argp)
{
	...
	ret = copy_to_user(argp, &fp->fp_touch_info,
		sizeof(fp->fp_touch_info));
	if (ret) {
		fpc_log_err("%s copy_to_user failed, ret=%d\n", __func__, ret);
		return -EFAULT;
	}
...
static int fingerprint_probe(struct platform_device* pdev)
{
    ...
    fingerprint->ttw_wl = power_wakeup_source_register(dev, "fpc_ttw_wl");
...
use_tp_irq_tag:
	...
	fingerprint->fp_touch_info.touch_x = 0;
	fingerprint->fp_touch_info.touch_y = 0;
	fingerprint->fp_touch_info.touch_major = 0;
	fingerprint->fp_touch_info.touch_minor = 0;
	fingerprint->fp_touch_info.touch_ori = '0';
...
static int fingerprint_remove(struct platform_device* pdev)
{
    ...
    power_wakeup_source_unregister(fingerprint->ttw_wl);
#if defined(CONFIG_FB)
	if (fingerprint->fb_notify.notifier_call != NULL) {
		fingerprint->fb_notify.notifier_call = NULL;
