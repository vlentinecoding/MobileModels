...
extern struct ud_fp_ops *fp_get_ops(void);
#endif
#endif
...
#define FINGER_UP_WAIT_TIMEOUT (50 * HZ / 1000)
#define DECIMAL               10
...
#else
	strncpy(fp->module_id_ud, buffer, module_id_len - 1);
#endif
	...
	unsigned int sensor_type, char *chip_info)
{
	...
	ret = snprintf_s(chip_info, FP_MAX_SENSOR_ID_LEN, sizeof(sensor_id) + 1,
		"%x\n", sensor_id);
#else
	ret = snprintf(chip_info, sizeof(sensor_id) + 1, "%x\n", sensor_id);
#endif
...
static int ud_fingerprint_handle_tp_msg(int udfp_event)
{
	...
	switch (udfp_event) {
	case TP_EVENT_SCREEN_ON_FINGER_DOWN:
	case TP_EVENT_SCREEN_OFF_FINGER_DOWN:
		fingerdown_event_function(&g_tp_state, fp);
		break;
	case TP_EVENT_SCREEN_ON_FINGER_UP:
	case TP_EVENT_SCREEN_OFF_FINGER_UP:
...
static irqreturn_t fingerprint_irq_handler(int irq, void* handle)
{
    ...
    if (fingerprint->wakeup_enabled) {
        ...
        __pm_wakeup_event(fingerprint->ttw_wl,
#else
        __pm_wakeup_event(&fingerprint->ttw_wl,
#endif
            msecs_to_jiffies(fingerprint->irq_hold_time));
    }
#endif
...
int fingerprint_get_dts_data(struct device *dev, struct fp_data *fingerprint)
{
    ...
    fingerprint->support_btb_pull_down = of_property_read_bool(np, "fingerprint,support-btb-pull-down");
    fpc_log_info("support_btb_pull_down = %d\n", fingerprint->support_btb_pull_down);

    fingerprint->btb_gpio = of_get_named_gpio(np, "fingerprint,btb_gpio", 0);
    if (!gpio_is_valid(fingerprint->btb_gpio))
        fpc_log_err("failed to get btb gpio from device tree\n");
    else
        fpc_log_info("btb_gpio = %d\n", fingerprint->btb_gpio);
    ...
    ret = of_property_read_u32(np, "fingerprint,poweroff_scheme", &fingerprint->poweroff_scheme);
    if (ret) {
        fingerprint->poweroff_scheme = -EINVAL;
        fpc_log_err("failed to get fingerprint->poweroff_scheme from device tree, ret = %d\n", ret);
    }
    fpc_log_info("fingerprint_poweroff = %d\n", fingerprint->poweroff_scheme);
...
static void fingerprint_btb_init(struct fp_data* fingerprint)
{
    int error = 0;

    if (!fingerprint->support_btb_pull_down)
        return;

    if (gpio_is_valid(fingerprint->btb_gpio)) {
        fpc_log_info("Assign BTB -> GPIO%d\n", fingerprint->btb_gpio);

        error = gpio_direction_output(fingerprint->btb_gpio, 0);
        if (error) {
            fpc_log_err("gpio_direction_output (irq) failed\n");
            return;
        }

        gpio_set_value(fingerprint->btb_gpio, 0);
    } else {
        fpc_log_err("invalid btb gpio\n");
        return;
    }
...
static void fp_ioc_enable_irq(struct fp_data *fp)
{
	...
	if (fp->use_tp_irq == USE_TP_IRQ) {
		g_wait_finger_up_flag = false;
		fp->tp_event = TP_EVENT_SCREEN_ON_FINGER_UP;
...
static int fp_config_wait_fp_up(void)
{
	...
	if (g_fp_data->tp_event == TP_EVENT_SCREEN_ON_FINGER_UP ||
		g_fp_data->tp_event == TP_EVENT_SCREEN_OFF_FINGER_UP) {
		fpc_log_info("finger is already up\n");
		return ret;
	}
	g_wait_finger_up_flag = true;
	if (wait_event_timeout(g_fp_data->wait_finger_up_queue,
		g_fp_data->tp_event == TP_EVENT_SCREEN_ON_FINGER_UP ||
		g_fp_data->tp_event == TP_EVENT_SCREEN_OFF_FINGER_UP,
...
static int fingerprint_probe(struct platform_device* pdev)
{
    ...
    fingerprint_btb_init(fingerprint);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
        fingerprint->ttw_wl = wakeup_source_create("fpc_ttw_wl");
        if (fingerprint->ttw_wl == NULL) {
            fpc_log_err("nfc_locker wakeup_source_create error \n");
            goto exit;
        }
        wakeup_source_add(fingerprint->ttw_wl);
#else
...
#ifndef CONFIG_HONOR_GKI_V1
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
    wakeup_source_remove(fingerprint->ttw_wl);
    wakeup_source_destroy(fingerprint->ttw_wl);
#else
