...
int mas_blk_fill_rwbs(char *rwbs, unsigned int op, int offset)
{
	int i = 0;
	unsigned long loop;
	struct op_description description[] = {
		{ REQ_TZ, 'Z' }, { REQ_CP, 'C' }, { REQ_FG, 'H' },
#ifdef CONFIG_HUAWEI_QOS_BLKIO
		{ REQ_VIP, 'V' },
#endif
		{ REQ_FAULT_OUT, 'P' },	{ REQ_BATCH_OUT, 'B' }
	};

	for (loop = 0; loop < ARRAY_SIZE(description); ++loop)
		if (op & description[loop].type)
