...
static inline void hungtask_do_gettimeofday(struct timeval *tv)
{
...
static void get_localtime(struct rtc_time *tm)
{
	...
	hungtask_do_gettimeofday(&(txc.time));
...
void check_hung_tasks_proposal(unsigned long timeout)
{
	...
	for (i = 0; i < CONCERNLIST_LENGTH; i++)
	{
		if ((strlen(whitetmplist[i].name) > 0) && (get_pid_by_name(whitetmplist[i].name) > 0)) {
			if (whitetmplist[i].pid != get_pid_by_name(whitetmplist[i].name)) {
				pid_changed = 1;
				pr_err("hungtask: whitetmplist %s pid has changed\n",
					whitetmplist[i].name);
			}
		}
	}
	if (do_refresh || (cur_heartbeat < TIME_REFRESH_PIDS) || pid_changed) {
		refresh_pids();
		check_parameters();
	}
