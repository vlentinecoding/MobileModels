...
static int hangdetect_bind_thread(void *arg)
{
	...
	for (;;) {
		...
		if (hang_cpu_dd->wk_tsk[cpu] != NULL) {
			...
			spin_unlock(&hang_cpu_dd->taskbit_lock);
			if (hang_cpu_dd->wk_tsk[cpu]->pid != current->pid) {
				pr_err("%s run task %d not in target cpu %d, current pid %d\n",
					__func__, hang_cpu_dd->wk_tsk[cpu]->pid, cpu, current->pid);
			}
		}
...
static int bind_task_check(void)
{
	...
	if (HANGDETECT_BIND_TASK) {
		if (!bind_done_flag) {
			pr_err("%s hangdetect bind task fail\n", __func__);
			pr_err("bind taskbit dump at %lld\n", sched_clock());
			pr_err("bind taskbit kick_bits: 0x%x, check_bits: 0x%x\n",
				hang_cpu_dd->cpus_kick_bit, hang_cpu_dd->cpus_online_bit);
	...
	while (!kthread_should_stop()) {
		...
		if (ret) {
			...
			if (s_hang_count >= HANGDETECT_FAIL_TIMES) {
				if (CPU_REBOOT && (get_logusertype_flag() == BETA_USER ||
