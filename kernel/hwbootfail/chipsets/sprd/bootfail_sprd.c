 ...
 * Create: 2021-2-3
 */
#include <hwbootfail/chipsets/bootfail_sprd.h>
...
struct timer_list g_sprd_pwk_timer;
...
void bootfail_pwk_press(void)
{
	...
	if (!is_boot_detector_enabled()) {
		print_err("bootfail_info close\n");
		return;
	}
	delay_time = msecs_to_jiffies(BOOTFAIL_LONGPRESS_COUNT_STEP);
	if (init_flag == 0) {
		timer_setup(&g_sprd_pwk_timer, bootfail_pwk_record, 0);
		init_flag++;
	}
	g_sprd_pwk_timer.expires = jiffies + delay_time;
	if (!timer_pending(&g_sprd_pwk_timer))
		add_timer(&g_sprd_pwk_timer);
}
...
int __init bootfail_init(void)
{
	...
	// set KERNEL_STAGE in uboot
	// adp.stage_ops.set_stage(KERNEL_STAGE);
