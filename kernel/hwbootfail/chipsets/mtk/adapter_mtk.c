...
static size_t get_kmsg_buf_size(void)
{
#ifdef CONFIG_PRINTK
	return min(get_klog_size(), BF_SIZE_1M);
#else
	return get_klog_size();
#endif
}
...
static void capture_kernel_log(char *pbuf, unsigned int buf_size)
{
	...
	if (klog_addr == NULL) {
		print_invalid_params("klog_addr is NULL\n");
		return;
	}
	ret = -1;
#ifdef CONFIG_PRINTK
	klog_size = get_kmsg_lock(pbuf, buf_size);
#else
	klog_size = get_kmsg_buf_size();
...
static void capture_app_log(char *pbuf, unsigned int buf_size)
{
	...
	ret = sys_readlink(APP_LOG_PATH, pbuf, buf_size);
