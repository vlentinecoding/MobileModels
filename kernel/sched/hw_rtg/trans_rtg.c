...
static bool is_depth_valid(const struct task_struct *from, int qos)
{
	...
	if ((g_trans_depth > 0) && (from->wts.rtg_depth != STATIC_RTG_DEPTH) &&
		(from->wts.rtg_depth >= g_trans_depth))
		return false;

	// if from is not in rtg, invalid
	if (from->wts.rtg_depth == 0 &&
	...
	struct task_struct *from, unsigned int type, int qos)
{
	...
	if (task->wts.rtg_depth == STATIC_RTG_DEPTH || task->wts.rtg_depth > 0)
		return;

	if (from->wts.rtg_depth == STATIC_RTG_DEPTH)
		task->wts.rtg_depth = 1;
	else
		task->wts.rtg_depth = from->wts.rtg_depth + 1;
...
void trans_rtg_sched_dequeue(struct task_struct *task, unsigned int type)
{
	...
	if (task->wts.rtg_depth == 0 || task->wts.rtg_depth == STATIC_RTG_DEPTH)
		return;
