...
#endif
{
	...
	if (sched_energy_enabled() || may_not_preempt ||
	    test || !task_fits_capacity(p, capacity_orig_of(cpu), cpu)) {
		 ...
		 */
		if (!test && target != -1 && !task_fits_capacity(p, capacity_orig_of(target), target))
			goto out_unlock;
...
#ifdef CONFIG_SCHED_WALT
#ifdef CONFIG_HW_RTG
static inline bool rt_skip_min(struct task_struct *p)
{
    struct walt_related_thread_group *grp;
    bool ret = false;
    bool boost = task_boost_policy(p) == SCHED_BOOST_ON_BIG;

    rcu_read_lock();

    grp = task_related_thread_group(p);
    if (grp)
        ret = grp->skip_min || boost;
...
#ifdef CONFIG_HW_RTG
	boost_on_big = boost_on_big || rt_skip_min(task) || uclamp_boosted(task);
#endif
