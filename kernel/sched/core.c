...
/*
	delta -= irq_delta;
	remove this logic because /proc/sys/kernel/sched_rt_runtime_us is not stable as expected.
*/
#endif
	       ...
	       int sibling_count_hint)
{
	...
	cpu = select_task_rq(p, p->wake_cpu, SD_BALANCE_WAKE, wake_flags,
			     sibling_count_hint);
#ifdef CONFIG_HW_ED_TASK
	note_task_waking(p, sched_ktime_clock());
#endif
	...
	__releases(rq->lock)
{
	...
	if (unlikely(prev_state == TASK_DEAD)) {
		...
		/* Move walt_task_dead() to do_exit()
		walt_task_dead(prev);
		 */
...
static void __sched notrace __schedule(bool preempt)
{
	...
	if (likely(prev != next)) {
		...
		if (trace_sched_stat_cur_and_cfs_enabled()) {
			struct sched_entity *se = __pick_first_entity(&rq->cfs);
			trace_sched_stat_cur_and_cfs(next, next->se.vruntime);
			while (se) {
				struct task_struct *curtask = container_of(se, struct task_struct, se);
				trace_sched_stat_cur_and_cfs(curtask, se->vruntime);
