...
#ifdef CONFIG_SCHED_DEBUG
			WARN_ONCE(1, "invalid CPU[%d] clamp group [%u:%u] value [%lu] refcount [%u] free_group_id [%u] free_group_value [%lu]\n",
			     cpu, clamp_id, group_id,
			     uc_cpu->group[clamp_id][group_id].value,
...
#ifndef CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL
...
#endif
#ifdef CONFIG_SCHED_TASK_UTIL_CLAMP
	p->util_req.min_util = 0;
	p->util_req.max_util = SCHED_CAPACITY_SCALE;
	INIT_LIST_HEAD(&p->util_req.min_util_entry);
#endif
#ifdef CONFIG_HW_RTG
	p->rtg_depth = 0;
#endif
