...
{
	struct hmp_domain *big_domain;
	struct hmp_domain *tmp_domain[3] = {0, 0, 0};
	int domain_cnt = 0;
	int iter_cpu;
	...
	for_each_hmp_domain_L_first(big_domain) {
		tmp_domain[domain_cnt] = big_domain;
		domain_cnt++;
	}

	if (unlikely(domain_cnt == 0 || domain_cnt > 3)) {
        	printk(KERN_WARNING "domain count is %d", domain_cnt);
        	return -1;
        }

	big_domain = tmp_domain[domain_cnt - 1];

	for_each_cpu(iter_cpu, &big_domain->possible_cpus) {
