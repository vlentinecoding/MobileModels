...
#ifdef CONFIG_VIP_PREFER_BIG
int find_vip_big_cpu(struct task_struct *p)
{
	...
	unsigned long wake_util = ULONG_MAX;
...
#else
	num_cluster = num_sched_clusters;
#endif

	if (num_cluster > 2 && cpumask_test_cpu(prime_cpu, &search_cpus)) {
		big_cpu = find_vip_prime_cpu(p);
	}

	if (big_cpu != -1) {
		trace_sched_find_vip_big_cpu_each (
			p, prime_cpu, !list_empty(&cpu_rq(prime_cpu)->vip_thread_list), -1);
		return big_cpu;
	}

	for_each_cpu(iter_cpu, &search_cpus) {

		if (num_cluster <= 2 && capacity_orig_of(iter_cpu) != 1024)
                        continue;

		if (num_cluster == 3 && iter_cpu < 4)
			continue;

		wake_util = cpu_util_without(iter_cpu, p);

		trace_sched_find_vip_big_cpu_each (
			p, iter_cpu, !list_empty(&cpu_rq(iter_cpu)->vip_thread_list), wake_util);
		...
		if(!list_empty(&cpu_rq(iter_cpu)->vip_thread_list) || cpu_rq(iter_cpu)->rt.rt_nr_running)
			continue;
...
int find_vip_prime_cpu(struct task_struct *p)
{
	int prime_cpu = 7;
#if  LINUX_VERSION_CODE < KERNEL_VERSION(4, 20, 0)
	if (!p->static_vip || schedtune_task_boost(p) % 10 != 2)
		return -1;
#else
	if (!p->static_vip || uclamp_eff_value(p, UCLAMP_MIN) % 10 != 2)
		return -1;
#endif
	if (idle_cpu(prime_cpu))
		return prime_cpu;

	// not select cpu if has vip task
	if(!list_empty(&cpu_rq(prime_cpu)->vip_thread_list))
		return -1;

	// not select cpu if has rt thread
	if(cpu_rq(prime_cpu)->rt.rt_nr_running > 0)
		return -1;

	// not select high_irqload
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 20, 0)
	if(walt_cpu_high_irqload(prime_cpu))
		return -1;
#else
	   if(sched_cpu_high_irqload(prime_cpu))
		return -1;
#endif
...
static int get_vip_target_cpu(struct task_struct *p,
		struct cpumask *search_cpus,
		struct cpumask *rtg_target,
		unsigned long target_capacity,
		bool favor_larger_capacity)
{
	int i;
	int target_cpu = -1;
	...
	unsigned int cpu_vip_prio = cpu_rq(i)->highest_vip_prio;
...
int find_vip_cpu(struct task_struct *p)
{
	...
	target_cpu = get_vip_target_cpu(p, &search_cpus, rtg_target,
		target_capacity, favor_larger_capacity);
	/*
...
static int __do_vip_balance(void *data)
{
	...
	dst_rq = get_dest_rq(src_cpu, &(p->cpus_mask), &i);
#else
...
#endif
/*lint -restore*/
#ifndef CONFIG_HUAWEI_SCHED_VIP
int set_vip_dynamic(struct task_struct *p, unsigned int prio)
{
	/*
	 * Values for prio:
	 * RESET_VIP = 0
	 ...
	 */
	if (prio == 0) {
		dynamic_vip_dequeue(p, DYNAMIC_VIP_QOS);
	} else {
		dynamic_vip_enqueue(p, DYNAMIC_VIP_QOS, 0);
	}
	return 0;
}
#endif
