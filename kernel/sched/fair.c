...
bool task_fits_capacity(struct task_struct *p,
					long capacity,
					int cpu)
{
	unsigned int margin;
...
static inline void update_misfit_status(struct task_struct *p, struct rq *rq)
{
	...
	if (rtg_target)
		...
		task_fits = task_fits_capacity(p, capacity_of(cpu), cpu_of(rq));
...
#if defined(CONFIG_HW_RTG_NORMALIZED_UTIL) || defined(CONFIG_HW_FAVOR_SMALL_CAP)
				     ...
				     int sync, int sibling_count_hint)
{
	...
	if (test_tsk_thread_flag(p, TIF_FAVOR_SMALL_CAP)) {
		...
		trace_favor_small_cap(p->pid, best_energy_cpu);
...
again:
	...
	if (p) {
		...
		trace_sched_vip_sched(p, "vip_pick_success-A");
		...
		trace_sched_vip_sched(p, "vip_pick_success-B");
		goto done;
	}
#endif
