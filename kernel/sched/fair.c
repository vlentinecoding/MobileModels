...
#if defined(CONFIG_HMP) || \
    defined(CONFIG_MTK_IDLE_BALANCE_ENHANCEMENT) || \
    defined(CONFIG_SCHED_HMP_PLUS)
static inline hw_is_key_task(struct task_struct *p)
{
#ifdef CONFIG_HW_VIP_THREAD
	if (test_task_vip(p))
		return true;
...
#endif
#ifdef CONFIG_HW_RTG
	if (task_related_thread_group(p))
		return true;
#endif
	return false;
}
...
#else
		task_fits = task_fits_capacity(p, capacity_of(cpu));
#endif
#else
	task_fits = task_fits_capacity(p, capacity_of(cpu));
#endif
 ...
 */
static unsigned long cpu_util_fair(int cpu)
{
	struct cfs_rq *cfs_rq;
	unsigned int util;
...
static inline unsigned long cpu_util_rt(int cpu)
{
	...
	return READ_ONCE(rt_rq->avg.util_avg);
}
...
#endif
	return min_t(unsigned long, cpu_util_fair(cpu) + cpu_util_rt(cpu),
		     capacity_orig_of(cpu));
}
...
static inline unsigned long cpu_util_freq(int cpu)
{
	...
	if (unlikely(walt_disabled || !sysctl_sched_use_walt_cpu_util)) {
		return min(cpu_util_fair(cpu) + cpu_util_rt(cpu),
			   capacity_orig_of(cpu));
	}
...
#else
	return min(cpu_util_fair(cpu) + cpu_util_rt(cpu), capacity_orig_of(cpu));
#endif
}
...
long schedtune_margin(int cpu, unsigned long signal, long boost)
{
	long long margin = 0;
...
#ifdef CONFIG_HW_UCLAMP_MIN_UTIL_DEPRECATED
	util_min = max_t(unsigned long, util_min, task->util_req.min_util);
#endif
				   ...
				   bool boosted, bool prefer_idle)
{
	...
	*backup_cpu = -1;
	 ...
	 */
	boosted = hw_boost_state(boosted);
...
#ifdef CONFIG_HW_FAVOR_SMALL_CAP
	if (prefer_idle && !boosted)
		favor_smaller_capacity = hw_favor_smaller_capacity(p, cpu);
#endif
	...
	do {
		for_each_cpu_and(i, &p->cpus_allowed, sched_group_span(sg)) {
...
#ifdef CONFIG_HW_EAS_SCHED
					if (boosted &&
					    capacity_orig < min_target_capacity)
						continue;
			...
			if (idle_cpu(i)) {
#ifdef CONFIG_HW_EAS_SCHED
				if (boosted && capacity_orig < best_idle_min_cap_orig)
					continue;
	...
	if (prefer_idle) {
		...
		if (best_idle_cpu != -1) {
#ifdef CONFIG_HW_EAS_SCHED
			if (boosted && target_capacity <= best_idle_min_cap_orig)
...
#ifdef CONFIG_HW_EAS_SCHED
			if (boosted && target_capacity <= backup_idle_min_cpu_cap)
		...
		int wake_flags, int sibling_count_hint)
{
	...
	if (p->vip_prio) {
		...
		if (vip_cpu != -1)
			return LB_HW_VIP | vip_cpu;
	}
#endif

#ifdef CONFIG_VIP_PREFER_BIG
	if ((p->static_vip || atomic64_read(&p->dynamic_vip)) && schedtune_task_boost(p) > 0) {
		int big_cpu = find_vip_big_cpu(p);

		if (big_cpu != -1)
			return LB_VIP_PREFER_BIG | big_cpu;
	}
#endif
	...
	if (sd && !(sd_flag & SD_BALANCE_FORK)) {
		 ...
		 * last_update_time.
		 */
#if defined(CONFIG_HW_EAS_SCHED) && !defined(CONFIG_SCHED_WALT)
			...
			int wake_flags, int sibling_count_hint)
{
    ...
    !defined(CONFIG_HW_VIP_THREAD) && \
    !defined(CONFIG_HUAWEI_SCHED_VIP) && \
    !defined(CONFIG_HW_RTG_NORMALIZED_UTIL)
...
static int detach_tasks(struct lb_env *env, struct rq_flags *rf)
{
	...
	while (!list_empty(tasks)) {
		...
		if (energy_aware() &&
			...
			p = hw_get_heaviest_task(p, env->dst_cpu);
...
#ifdef CONFIG_HUAWEI_SCHED_VIP
		if ((load / 2) > env->imbalance && !p->vip_prio)
#else
		...
		if (p->vip_prio && find_rtg_target(p)) {
			...
			if ((now - p->last_enqueued_ts < vip_task_load_balance_delay) &&
			    (capacity_orig_of(env->dst_cpu) < capacity_orig_of(env->src_cpu)))
				goto next;
		}
#endif
