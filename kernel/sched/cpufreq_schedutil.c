...
#if defined(CONFIG_HW_RTG_NORMALIZED_UTIL) || defined(CONFIG_CLUSTER_NORMALIZED_UTIL)
...
unsigned int arch_get_cpu_max_freq(unsigned int cpu)
{
				  ...
				  unsigned long util, unsigned long max)
{
	...
	freq = sched_restrict_cluster_freq(policy->cpus, freq,
...
static int find_target_boost(unsigned long util, struct sugov_policy *sg_policy)
{
		int i, ret;
		unsigned long flags;

		spin_lock_irqsave(&sg_policy->tunables->target_loads_lock, flags);
		for (i = 0; i < sg_policy->tunables->ntarget_loads - 1 &&
				util >= sg_policy->tunables->target_loads[i+1]; i += 2)
				;
		ret = sg_policy->tunables->target_loads[i];
		spin_unlock_irqrestore(&sg_policy->tunables->target_loads_lock, flags);
			      ...
			      unsigned long *max)
{
	...
	target_boost = 100 + find_target_boost(*util, sg_policy);
...
#ifdef CONFIG_HW_RTG_NORMALIZED_UTIL
	next_f = max(sg_policy->rtg_freq, next_f);
#endif
...
#ifdef CONFIG_HW_ED_TASK
#include "hw_task/hw_ed_task_sysfs.c"
#endif
#ifdef CONFIG_HW_CPU_FREQ_GOV_SCHEDUTIL_COMMON
...
static ssize_t target_loads_show(struct gov_attr_set *attr_set, char *buf)
{
	...
	for (i = 0; i < tunables->ntarget_loads; i++) {
		...
		ret += sprintf(buf + ret, "%d%s", tmp, i & 0x1 ? ":" : " ");
	}
	sprintf(buf + ret - 1, "\n");
