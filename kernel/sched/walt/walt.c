...
#include "../hw_rtg/include/rtg_pseudo.h"
#endif
...
#ifdef CONFIG_HW_ED_TASK
		if (is_ed_task_ext(rq, p, wallclock, is_new_task(p))) {
#else
...
#ifdef CONFIG_HW_ED_TASK
		} else if (is_ed_task_ext(dest_rq, p, wallclock, new_task)) {
#else
...
void walt_task_dead(struct task_struct *p)
{
	...
	/* Move free_task_load_ptrs() from here to __put_task_struct()
	 * to avoid panic ussue */
	/*
