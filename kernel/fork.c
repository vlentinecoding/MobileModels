...
#endif
	/* Move free_task_load_ptrs() from walt_task_dead() to here
	 * to avoid panic issue */
	free_task_load_ptrs(tsk);
...
#include <linux/sched/hw_vip/hwvip_fork.h>
#endif
					...
					struct kernel_clone_args *args)
{
	...
	if (clone_flags & CLONE_THREAD) {
		...
		p->ext_flags &= ~(PF_EXT_WSCG_MONITOR | PF_EXT_WSCG_PREREAD);
#endif
	}
