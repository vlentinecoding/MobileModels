...
static ssize_t castplus_resource_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)

{
    ...
    if (dev == NULL) {
        d_vpr_e("castplus_resource_show, dev is null.\n");
        return rc;
    }
    core = dev_get_drvdata(dev);

    if (core == NULL) {
        d_vpr_e("castplus_resource_show, core is null.\n");
        return rc;
    }
    ...
    max_video_load = core->resources.max_load;
    max_image_load = core->resources.max_image_load;

    if (video_load > max_video_load) {
        d_vpr_e("castplus_resource_show, overloaded.used: %d max: %d.\n",
            video_load, max_video_load);
    }

    if (video_load + image_load > max_video_load + max_image_load) {
        d_vpr_e("castplus_resource_show, overloaded.used: [video + image][%d + %d], max: [video + image][%d + %d].\n",
            video_load, image_load, max_video_load, max_image_load);
    }

    rc = scnprintf(buf, PAGE_SIZE, "%d\n", max_video_load - video_load);

    return rc;
}

// only for castplus
static DEVICE_ATTR(castplus_resource, 0444, castplus_resource_show, NULL);
...
static struct attribute *msm_vidc_core_attrs[] = {
		...
		&dev_attr_castplus_resource.attr,
		NULL
};
