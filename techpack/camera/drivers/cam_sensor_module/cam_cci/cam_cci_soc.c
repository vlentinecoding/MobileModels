	...
	if (!cci_dev->cci_master_info[master].is_initilized) {
		...
		if (cci_dev->ref_count == 1) {
			...
			if (!wait_for_completion_timeout(
				...
				CCI_TIMEOUT)) {
				CAM_ERR(CAM_CCI,
					"Failed: CCI_RESET_CMD_RMSK complete timeout for master: %d",
					master);
				rc = -ETIMEDOUT;
				cci_dev->master_active_slave[master]--;
				return rc;
			...
			for (retry_times = 0; retry_times < MAX_RETRY_CCI_TIMES; retry_times++) {
				cam_io_w_mb((master == MASTER_0) ?
					CCI_M0_RESET_RMSK : CCI_M1_RESET_RMSK,
					base + CCI_RESET_CMD_ADDR);
				if (!wait_for_completion_timeout(
					&cci_dev->cci_master_info[master].reset_complete,
					CCI_TIMEOUT_RETRY * (retry_times + 1) * (retry_times + 1))) {
					CAM_ERR(CAM_CCI,
						"Retry Failed: reset complete timeout count %d",
						retry_times+1);
					continue;
				}
				break;
			...
			if (retry_times == MAX_RETRY_CCI_TIMES) {
				CAM_ERR(CAM_CCI,
					"Failed: CCI_MX_RESET_RMSK complete timeout for master: %d",
					master);
				rc = -ETIMEDOUT;
				cci_dev->master_active_slave[master]--;
				return rc;
