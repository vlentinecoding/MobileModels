...
static void cam_sensor_regulator_update(struct cam_hw_soc_info *soc_info,
    struct cam_sensor_power_setting *power_setting, int32_t power_index, int32_t rgltr_index)
{
	if (VALIDATE_VOLTAGE(
		soc_info->rgltr_min_volt[rgltr_index],
		soc_info->rgltr_max_volt[rgltr_index],
		power_setting[power_index].config_val)) {
		soc_info->rgltr_update_min_volt[rgltr_index] =
		soc_info->rgltr_update_max_volt[rgltr_index] =
		power_setting[power_index].config_val;
	} else {
		soc_info->rgltr_update_min_volt[rgltr_index] = soc_info->rgltr_min_volt[rgltr_index];
		soc_info->rgltr_update_max_volt[rgltr_index] = soc_info->rgltr_max_volt[rgltr_index];
	}
}
	...
	struct cam_buf_io_cfg *io_cfg)
{
	...
	if (rc) {
		...
		*cmd_length_in_bytes = sizeof(struct i2c_rdwr_header) +
			(sizeof(struct cam_cmd_random_read) *
			(cmd_i2c_random_rd->header.count));
		...
		case SENSOR_VDIG:
			for (j = 0; j < num_vreg; j++) {
				if (!strcmp(soc_info->rgltr_name[j],
					...
					cam_sensor_regulator_update(soc_info, power_setting, i, j);
					break;
				}
			}
			if (j == num_vreg)
		...
		if (!strcmp(soc_info->rgltr_name[j], "cam_clk")) {
			...
			if (ps != NULL) {
				...
				rc = cam_soc_util_regulator_disable(
					...
					soc_info->rgltr_update_min_volt[j],
					soc_info->rgltr_update_max_volt[j],
		...
		case SENSOR_CUSTOM_REG2:
			...
			if (power_setting->seq_val < num_vreg) {
				...
				rc =  cam_soc_util_regulator_enable(
					soc_info->rgltr[vreg_idx],
					soc_info->rgltr_name[vreg_idx],
					soc_info->rgltr_update_min_volt[vreg_idx],
					soc_info->rgltr_update_max_volt[vreg_idx],
		...
		case SENSOR_CUSTOM_REG2:
			...
			if (ps) {
				if (pd->seq_val < num_vreg) {
					...
					soc_info->rgltr_update_min_volt[ps->seq_val],
					soc_info->rgltr_update_max_volt[ps->seq_val],
