...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_DBG(CAM_SENSOR, "probe_addr_type: %u, probe_data_type: %u",
...
int cam_module_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if ((!s_ctrl) || (!s_ctrl->sensordata) || (!s_ctrl->io_master_info.cci_client)) {
	...
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR, "camera_io_dev_read occur error");
	...
	void *arg)
{
...
#ifdef CONFIG_LOG_EXCEPTION
		check_camera_btb_gpio_info(s_ctrl);
#endif
	...
	case CAM_START_DEV: {
		...
		if (s_ctrl->i2c_data.streamon_settings.is_settings_valid &&
			...
			if (rc < 0) {
#ifdef CONFIG_LOG_EXCEPTION
				camkit_hiview_report_id(DSM_CAMERA_I2C_ERR, s_ctrl->sensordata->slave_info.sensor_id,
	...
	case CAM_CONFIG_DEV: {
		...
		if (s_ctrl->i2c_data.init_settings.is_settings_valid &&
			...
			if (rc < 0) {
#ifdef CONFIG_LOG_EXCEPTION
				camkit_hiview_report_id(DSM_CAMERA_SENSOR_INIT_FAIL, s_ctrl->sensordata->slave_info.sensor_id,
		...
		if (s_ctrl->i2c_data.config_settings.is_settings_valid &&
			...
			if (rc < 0) {
#ifdef CONFIG_LOG_EXCEPTION
				camkit_hiview_report_id(DSM_CAMERA_SENSOR_SET_RESOLUTION_FAIL, s_ctrl->sensordata->slave_info.sensor_id,
