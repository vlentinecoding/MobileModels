...
#define MAX_CSIPHY_OVERRIDE_PARAM 100
...
static int csiphy_override_cnt = 0;
static int csiphy_override[MAX_CSIPHY_OVERRIDE_PARAM];
module_param_array(csiphy_override, int, &csiphy_override_cnt, 0644);
static int csiphy_hack_rate_mb = 0;
	...
	int32_t dev_handle)
{
	...
	if ((csiphy_hack_rate_mb != 0) && ((csiphy_dev->csiphy_info[index].data_rate / 1000000) > csiphy_hack_rate_mb))
	{
		for (i = 0; i < csiphy_override_cnt; i += 2)
		{
			if (i > (MAX_CSIPHY_OVERRIDE_PARAM - 2))
			    break;
			cam_io_w_mb(csiphy_override[i + 1], csiphybase + csiphy_override[i]);
			CAM_ERR(CAM_CSIPHY, "csiphy_cfg_override [0x%x, 0x%x]", csiphy_override[i], csiphy_override[i+1]);
		}
	}
