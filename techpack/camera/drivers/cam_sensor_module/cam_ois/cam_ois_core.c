	...
	uint32_t *cmd_buf, size_t len)
{
	...
	if (o_ctrl->io_master_info.master_type == CCI_MASTER) {
		...
		o_ctrl->ois_fw_flag = ois_info->ois_fw_flag;
		o_ctrl->is_ois_calib = ois_info->is_ois_calib;
		o_ctrl->module_code_data = ois_info->module_code_data;
		memcpy(o_ctrl->ois_name, ois_info->ois_name, OIS_NAME_LEN);
		o_ctrl->ois_name[OIS_NAME_LEN - 1] = '\0';
		memcpy(&(o_ctrl->opcode), &(ois_info->opcode),
			sizeof(struct cam_ois_opcode));
...
static int cam_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(CAM_OIS, "ois_name %s", o_ctrl->ois_name);
	if (!strcmp(o_ctrl->ois_name, "ois_lc898128") ||
		!strcmp(o_ctrl->ois_name, "ois_lc898128_slave")) {
		CAM_INFO(CAM_OIS, "apply lc898128 ois_fw settings");
		return lc898128_download_ois_fw(o_ctrl);
	}
	if (!strcmp(o_ctrl->ois_name, "ois_lc898129")) {
		CAM_INFO(CAM_OIS, "apply lc898129 ois_fw settings");
		return lc898129_download_ois_fw(o_ctrl);
	}

	CAM_INFO(CAM_OIS, "apply default ois_fw settings");
	return cam_default_ois_fw_download(o_ctrl);
}

static int onsemi_write_qtimer_to_ois(struct cam_ois_ctrl_t *o_ctrl) {
	int32_t  rc = 0;
	uint64_t qtime_ns = 0;
	uint64_t qtime_ns_after = 0;
	struct cam_sensor_i2c_reg_setting i2c_req_setting;
	struct cam_sensor_i2c_reg_array *i2c_reg_array = NULL;

	i2c_reg_array =
		(struct cam_sensor_i2c_reg_array *)kzalloc(sizeof(struct cam_sensor_i2c_reg_array) * 2, GFP_KERNEL);
	if (i2c_reg_array == NULL) {
		CAM_ERR(CAM_OIS, "Alloc i2c_write_setting_gl failed");
		return -ENOMEM;
	}

	memset(i2c_reg_array, 0, sizeof(struct cam_sensor_i2c_reg_array) * 2);
	rc = cam_sensor_util_get_current_qtimer_ns(&qtime_ns);
	if (rc < 0) {
	...
	i2c_reg_array[0].reg_addr = 0xF112;
	i2c_reg_array[0].reg_data = (uint32_t)((qtime_ns>>32) & 0xffffffff);
	i2c_reg_array[0].delay = 0X00;
	i2c_reg_array[0].data_mask = 0x00;

	i2c_reg_array[1].reg_data = (uint32_t)(qtime_ns & 0xffffffff);
	i2c_reg_array[1].delay = 0x00;
	i2c_reg_array[1].data_mask = 0x00;

	i2c_req_setting.reg_setting = i2c_reg_array;
	i2c_req_setting.size = 2;
	i2c_req_setting.addr_type = CAMERA_SENSOR_I2C_TYPE_WORD;
	i2c_req_setting.data_type = CAMERA_SENSOR_I2C_TYPE_DWORD;
	i2c_req_setting.delay = 0x00;
	rc = camera_io_dev_write_continuous(&(o_ctrl->io_master_info), &i2c_req_setting, CAM_SENSOR_I2C_WRITE_BURST);
	if (rc < 0) {
	...
	if (rc < 0) {
		CAM_ERR(CAM_OIS, "Failed to get current qtime_ns_after value: %d", rc);
		return rc;
	}
	CAM_DBG(CAM_OIS,"qtime_ns_after: ms=0x%x us=0x%x",
		(uint32_t)((qtime_ns_after>>32) & 0xffffffff), (uint32_t)(qtime_ns_after & 0xffffffff));
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	case CAM_OIS_PACKET_OPCODE_INIT:
		...
		for (i = 0; i < csl_packet->num_cmd_buf; i++) {
			...
			CAM_DBG(CAM_OIS, "cmd_type %d", cmm_hdr->cmd_type);
		...
		if (o_ctrl->is_ois_calib) {
			...
			if (o_ctrl->ops && o_ctrl->ops->set_calib_data) {
				if (o_ctrl->ops->set_calib_data(o_ctrl))
					CAM_ERR(CAM_OIS, "set_calib_data fail");
				else
					CAM_INFO(CAM_OIS, "set_calib_data success");
			}
		}
	...
	case CAM_OIS_PACKET_OPCODE_OIS_CONTROL:
		if (o_ctrl->calib_data.calib_ctrl_flag == 1) {
			CAM_INFO(CAM_OIS, "calib control, skip QC control");
			break;
		}
		if (o_ctrl->cam_ois_state < CAM_OIS_CONFIG) {
			rc = -EINVAL;
	...
	case CAM_OIS_PACKET_OPCODE_WRITE_TIME: {
		...
		if (!strcmp(o_ctrl->ois_name, "ois_lc898128") ||
			!strcmp(o_ctrl->ois_name, "ois_lc898128_slave") ||
			 !strcmp(o_ctrl->ois_name, "ois_lc898129")) {
			rc = onsemi_write_qtimer_to_ois(o_ctrl);
			if (rc < 0) {
				CAM_ERR(CAM_OIS, "Failed to write qtimer to onsemi: %d", rc);
				return rc;
			}
			break;
		}
	...
	if (!rc)
		return rc;
pwr_dwn:
	if (cam_ois_power_down(o_ctrl) < 0)
...
void cam_ois_shutdown(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (o_ctrl->cam_ois_state >= CAM_OIS_ACQUIRE) {
		...
		o_ctrl->calib_data.calib_ctrl_flag = 0;
	}
...
int cam_ois_driver_cmd(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	CAM_DBG(CAM_OIS, "going to lock op_code: %d, device_hdl: %d, ois_state: %d",
		cmd->op_code, o_ctrl->bridge_intf.device_hdl, o_ctrl->cam_ois_state);
