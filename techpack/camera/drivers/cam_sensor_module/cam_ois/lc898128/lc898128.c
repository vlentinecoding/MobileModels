...
static int lc898128_get_gyro_gain(void *ctrl)
{
	...
	/* get gyro gain*/
	ret = FlashMultiRead(o_ctrl, INF_MAT0, 0, UlMAT0, 32);
	if (ret || UlMAT0[4] == 0 || UlMAT0[5] == 0) {
		CAM_ERR(CAM_OIS, "ret = %d, gain x: 0x%x, gain y: 0x%x", ret, UlMAT0[4], UlMAT0[5]);
		return -1;
	}

	o_ctrl->calib_data.gyro_gain[0] = UlMAT0[4]; /* gyro gain x */
	o_ctrl->calib_data.gyro_gain[1] = UlMAT0[5]; /* gyro gain y */
...
static int lc898128_set_ctrl_flag(void *ctrl, int32_t flag)
{
	...
	/* 3：complete calib */
	if (flag == 3) {
		XuChangWrGyroGainData(o_ctrl, 1); /* write calib gyro gain */
		WrGyroOffsetData(o_ctrl, 1); /* write calib gyro offset */
	}
