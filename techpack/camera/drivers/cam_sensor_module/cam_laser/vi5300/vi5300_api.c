...
VI5300_Error VI5300_Get_Measure_Data(VI5300_DEV dev)
{
	...
	if(noise_tmp < 10) {
		lower = 150 * noise_level / 8 + 1994;
		upper = 130 * noise_level / 8 + 3391;
	} else if(noise_tmp < 20) {
		lower = 251 * noise_level / 8 + 3192;
		upper = 333 * noise_level / 8 + 4433;
	} else if(noise_tmp < 86) {
		lower = 196 * noise_level / 8 + 5704;
		upper = 218 * noise_level / 8 + 7764;
	} else if(noise_tmp < 155) {
		lower = 209 * noise_level / 8 + 18620;
		upper = 242 * noise_level / 8 + 22127;
	} else if(noise_tmp < 317){
		lower = 192 * noise_level / 8 + 33021;
		upper = 214 * noise_level / 8 + 38834;
	} else if(noise_tmp < 636){
		lower = 202 * noise_level / 8 + 64159;
		upper = 230 * noise_level / 8 + 73464;
	} else if(noise_tmp < 1299){
		lower = 188 * noise_level / 8 + 128570;
		upper = 207 * noise_level / 8 + 146944;
	} else if(noise_tmp < 2553){
		lower = 186 * noise_level / 8 + 253454;
		upper = 205 * noise_level / 8 + 284100;
	} else {
		lower = 181 * noise_level / 8 + 486904;
		upper = 198 * noise_level / 8 + 541393;
	}
	if(peak1 < lower) {
		confidence = 0;
	} else if(peak1 > upper) {
	...
	ratio = (((uint32_t)noise_level) * 772 / 8 + 1993800) / 100000;
	lower_r = lower * 10 / ratio;
	if(lower_r < 101000) {
		dmax = 13700000 / (lower_r + 3181) - 1326 * lower_r /1000000 + 415;
	} else {
		dmax = 44620000 / (lower_r + 48830) -386 * lower_r / 10000000 + 123;
	}
	...
	if(dev->enable_debug)
	{
		vi5300_errmsg("timeUSec:%d, Obj_Range:%d, RangeStatus:%d, NumberOfObject:%d, AmbientRate:%d, nearlimit:%d, farlimit:%d, dmax:%d, confidence:%d\n", 
					...
					nearlimit, farlimit, dmax, confidence);
	}
	return Status;
}
