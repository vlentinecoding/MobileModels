...
static int proxi_status = EXT_PROX1_DEFAULT_STATUS;
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(NULL != data)
	{
		...
		if(val == 1)
		{
			...
			data->sensor_vote = 1;
			proxi_status = EXT_PROX1_DEFAULT_STATUS;
			if (enable_sensor == 0) {
				vi5300_infomsg("sensor: start Start_Continuous_Measure", val);
				Status = vi5300_func_tbl->Start_Continuous_Measure(data);
				enable_sensor = 1;
			}
		} else if (val == 0) {
			data->camera_vote = 0;
			if (!data->sensor_vote && !data->camera_vote) {
				vi5300_infomsg("camera: start Stop_Continuous_Measure", val);
			...
			data->sensor_vote = 0;
			if (!data->sensor_vote && !data->camera_vote) {
				vi5300_infomsg("sensor: start Stop_Continuous_Measure", val);
				Status = vi5300_func_tbl->Stop_Continuous_Measure(data);
				enable_sensor = 0;
				/* 0： clear input sys data when vi5300 stop capture */
				input_report_abs(data->input_dev, ABS_HAT0Y, 0);
...
static irqreturn_t vi5300_irq_handler(int vec, void *info)
{
	...
	if (data->irq == vec)
	{
		...
		if(!xtalk_mark && !offset_mark)
		{
			...
			if (data->sensor_vote) {
				/* laser data to proximity sensor */
				if (data->Rangedata.RangeStatus == 0) {
					if ((data->Rangedata.Obj_Range >> 16) > EXT_PROX1_DIS_NEAR_FAR) {
						proxi_status =  EXT_PROX1_DEFAULT_STATUS;
					} else if ((data->Rangedata.Obj_Range >> 16) < EXT_PROX1_DIS_NEAR_THR) {
						proxi_status = 0;
					}
				}

				vi5300_dbgmsg("vi5300 data to proxi, distance %d, status %d, proxi_status %d", data->Rangedata.RangeStatus, data->Rangedata.RangeStatus, proxi_status);
				laser_data_to_proxi.sensor_data_head.sensor_type = SENSOR_TYPE_EXT_PROX1;
				laser_data_to_proxi.sensor_data_head.action = STD_SENSOR_DATA_REPORT;
				memcpy(&laser_data_to_proxi.sensor_data[0], &proxi_status, sizeof(int));
				rc = ap_sensor_route_write((char*)&laser_data_to_proxi,
...
int vi5300_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	vi5300_data->Rangedata.RangeStatus = 255;
