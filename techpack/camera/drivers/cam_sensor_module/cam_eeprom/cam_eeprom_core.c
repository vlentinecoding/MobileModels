...
static struct i2c_settings_list*
	cam_eeprom_get_i2c_ptr_seq(struct i2c_settings_array *i2c_reg_settings,
		uint32_t size)
{
	...
	struct list_head **list)
{
	...
	*cmd_length_in_bytes = (sizeof(struct i2c_rdwr_header) +
	...
	CAM_DBG(CAM_EEPROM, "Write Address: 0x%x",
...
static int32_t cam_eeprom_handle_random_write(
	...
	struct cam_eeprom_ctrl_t *e_ctrl)
{
	...
	for (i = 0; i < csl_packet->num_cmd_buf; i++) {
		...
		while (processed_cmd_buf_in_bytes < total_cmd_buf_in_bytes) {
			...
			/* 		sizeof(uint32_t); */
			/* 	CAM_ERR(CAM_EEPROM, "[debugTof] eeprom power setting"); */
			/* 	if (rc) { */
			...
			case CAMERA_SENSOR_CMD_TYPE_I2C_RNDM_WR: {
				...
				rc = cam_eeprom_handle_random_write(
					cam_cmd_i2c_random_wr,
					&(e_ctrl->wr_settings),
					&cmd_length_in_bytes, &off, &list);
				if (rc < 0) {
	...
	if (i2c_set->is_settings_valid == 1) {
		list_for_each_entry(i2c_list, &(i2c_set->list_head), list) {
			if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_RANDOM) {
				rc = camera_io_dev_write(&e_ctrl->io_master_info, &i2c_list->i2c_settings);
			} else {
				rc = camera_io_dev_write_continuous(&e_ctrl->io_master_info,
					&i2c_list->i2c_settings, 1); // normal: i2c_list->seq_settings
			}
			if (rc < 0) {
				CAM_ERR(CAM_EEPROM, "Error in EEPROM write");
				goto del_req;
			}
		}
	}
...
static int32_t cam_eeprom_pkt_parse(struct cam_eeprom_ctrl_t *e_ctrl, void *arg)
{
	...
	case CAM_EEPROM_PACKET_OPCODE_INIT:
		...
		for (retry_times = 0; retry_times < MAX_RETRY_TIMES; retry_times++) {
			...
			rc = cam_eeprom_read_memory(e_ctrl, &e_ctrl->cal_data);
			if (rc) {
				CAM_ERR(CAM_EEPROM, "retry read eeprom %d time", retry_times);
				rc = cam_eeprom_power_down(e_ctrl);
				usleep_range(10 * 1000, 11 * 1000); /* sleep 10~11ms */
				continue;
			}
			break;
		}
		...
		if (retry_times == MAX_RETRY_TIMES) {
			CAM_ERR(CAM_EEPROM, "retry read eeprom DMD");
			CAM_ERR(CAM_EEPROM, "retry read eeprom failed, id %d",
				e_ctrl->soc_info.index);
#ifdef CONFIG_LOG_EXCEPTION
			camkit_hiview_report_id(DSM_CAMERA_OTP_I2C_ERR,
				e_ctrl->soc_info.index,
