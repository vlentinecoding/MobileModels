...
bool g_flash_thermal_protect_status = false;

static ssize_t cam_flash_thermal_protect_show(struct device *dev,
	struct device_attribute *attr,char *buf)
{
	int rc;
	if (dev == NULL || attr == NULL || buf == NULL) {
		CAM_ERR(CAM_FLASH, "NULL input para");
		return -1;
	}
...
static ssize_t cam_flash_thermal_protect_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (length != 1) {
		CAM_ERR(CAM_FLASH, "failed to check para");
		return -1;
	}
	fctrl = platform_get_drvdata(to_platform_device(dev));
	mutex_lock(&(fctrl->flash_mutex));
	if (!op_code) {
		CAM_INFO(CAM_FLASH, "disable thermal protect");
		g_flash_thermal_protect_status = false;
	} else {
		CAM_INFO(CAM_FLASH, "enable thermal protect");
		g_flash_thermal_protect_status = true;
		cam_flash_off(fctrl);
	}
...
static struct device_attribute cam_flash_thermal_protect =
	__ATTR(flash_thermal_protect, 0660, cam_flash_thermal_protect_show, cam_flash_thermal_protect_store);
#endif
...
{
#ifndef CONFIG_HONOR_GKI_V1
	if (of_find_property(pdev->dev.of_node, "flash_thermal", NULL)) {
		device_remove_file(&pdev->dev, &cam_flash_thermal_protect);
	}
#endif
...
#ifndef CONFIG_HONOR_GKI_V1
	if (of_find_property(pdev->dev.of_node, "flash_thermal", NULL)) {
		if (device_create_file(&pdev->dev, &cam_flash_thermal_protect)) {
			CAM_ERR(CAM_FLASH,"Failed to create device file flash_thermal_protect \n");
			device_remove_file(&pdev->dev, &cam_flash_thermal_protect);
		}
	}
#endif
