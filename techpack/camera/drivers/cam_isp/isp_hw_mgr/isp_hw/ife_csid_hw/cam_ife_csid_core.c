...
#ifdef CONFIG_LOG_EXCEPTION
struct cam_hievent_info g_ife_err_info;
void cam_ife_hiview_handle(int error_no, struct cam_ife_csid_hw *csid_hw, struct cam_csid_evt_payload *evt_payload)
{
	char error_info[HIVIEW_MAX_CONTENT_LEN] = { 0 };
	errno_t ret;
	static uint32_t err_num;

	if (!csid_hw || !evt_payload || !csid_hw->hw_intf) {
		CAM_ERR(CAM_ISP,
			"Invalid Param csid_hw %pK evt_payload %pK", csid_hw, evt_payload);
		return ;
	}
	ret = memset_s(&g_ife_err_info, sizeof(g_ife_err_info), 0, sizeof(g_ife_err_info));
	if (ret != EOK) {
		CAM_ERR(CAM_ISP,"cam_isp_info memset_s fail, ret = %d\n", ret);
		return;
	}
	err_num++;
	g_ife_err_info.error_no = error_no;
	if (snprintf_s(error_info, sizeof(error_info),
		sizeof(error_info) - 1, "Erro info[err num %d CSID %d err 0x%x phy %d irq status TOP:0x%x RX:0x%x IPP:0x%x PPP:0x%x RDI0:0x%x RDI1:0x%x RDI2:0x%x RDI3:0x%x UDI0:0x%x UDI1:0x%x UDI2:0x%x]",
			err_num,
			...
			evt_payload->irq_status[CAM_IFE_CSID_IRQ_REG_UDI_2]) < 0)
		CAM_ERR(CAM_ISP,"error_info snprintf_s fail");

	cam_hiview_get_content(error_no, error_info, &g_ife_err_info);
	csid_hw->dmd_detected = true;
	CAM_INFO(CAM_ISP,"dmd_detected %d", csid_hw->dmd_detected);
}
#endif
...
#ifdef CONFIG_LOG_EXCEPTION
	if (csid_hw->dmd_detected) {
		cam_hiview_report(&g_ife_err_info);
		csid_hw->dmd_detected = false;
	}
#endif
	csid_hw->error_irq_count = 0;
...
#ifdef CONFIG_LOG_EXCEPTION
		cam_ife_hiview_handle(DSM_CAMERA_SOF_ERR, csid_hw, evt_payload);
#endif
	}
