...
static const struct cam_hievent_content g_content[] = {
	...
	{ DSM_CAMERA_SOF_ERR, "isp csi error" },
	{ DSM_CAMERA_SENSOR_INIT_FAIL, "set sensor init setting fail" },
	{ DSM_CAMERA_SENSOR_SET_RESOLUTION_FAIL, "set sensor resolution setting fail" },
};
...
static const char* cam_dsm_get_content(int error_no)
{
	unsigned int i;
	...
	struct cam_hievent_info *cam_info)
{
	...
	return_on_null(cam_info);
	dsm_content = cam_dsm_get_content(error_no);
	...
	strcat(total_info, dsm_content);

	if (error_info)
		strcat(total_info, error_info);
