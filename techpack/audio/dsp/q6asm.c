					      ...
					      struct param_hdr_v3 *param_hdr)
{
	...
	case SOFT_VOLUME_INSTANCE_2:
		...
		return 0;
#ifdef DTS_AUDIO_QDSP
	case DTS_VOLUME_CMD:
		param_hdr->module_id = DTS_VOLUME_MODULE_ID;
		param_hdr->param_id = DTS_VOLUME_PARAM_ID;
		param_hdr->instance_id = INSTANCE_ID_0;
		pr_debug("%s DTS: module_id = %x, param_id =%x",__func__,param_hdr->module_id,param_hdr->param_id);
		return 0;
#endif
...
#ifdef DTS_AUDIO_QDSP
int q6asm_set_volume_dts(struct audio_client *ac, int volume_l, int volume_r)
{
	struct asm_volume_ctrl_dts vol;
	struct param_hdr_v3 param_info;
	int rc = 0;
	...
	rc = q6asm_set_soft_volume_module_instance_ids(DTS_VOLUME_CMD, &param_info);
	if (rc) {
	    pr_err("%s: Failed to pack DTS volume module and instance IDs, error %d\n",
		       __func__, rc);
	    return rc;
	}

	param_info.param_size = sizeof(vol);
	vol.volume_left = volume_l;
	vol.volume_right = volume_r;
	vol.dtsVolFlag = 1;
...
EXPORT_SYMBOL(q6asm_set_volume_dts);
#endif
