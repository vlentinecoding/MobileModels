...
static uint32_t g_cirrus_sp_id = CIRRUS_SP;
		...
		int param_id, void *data, uint32_t length)
{
	...
	param_hdr.instance_id = (param_id > 0xA1B000) ? 0x8000 : INSTANCE_ID_0;
		...
		int param_id, void *data, uint32_t length)
{
	...
	param_hdr.param_id = param_id;
	param_hdr.param_size = length;
	pr_debug("CRUS: param_size %d, Instance ID = %d\n", length, param_hdr.instance_id);
...
static int crus_afe_send_delta(const char *data, uint32_t length)
{
	...
	uint32_t module = g_cirrus_sp_id;
	int chars_to_send, mem_size, sent = 0, ret = 0;
	int index = afe_get_port_index(port);
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (this_ctrl.fb_port_index) {
		...
		case 6:
			this_ctrl.fb_port = AFE_PORT_ID_QUINARY_MI2S_TX;
			this_ctrl.ff_port = AFE_PORT_ID_QUINARY_MI2S_RX;
			break;
		default:
...
int msm_routing_cirrus_fbport_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: this_ctrl.fb_port = 0x%x", __func__, this_ctrl.fb_port);
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	//feedback enable
	crus_set_param(this_ctrl.ff_port, g_cirrus_sp_id, CIRRUS_SP_ENABLE,
			   (void *)&this_ctrl.enable,
			   sizeof(struct crus_single_data_t));

	if (this_ctrl.prot_en) {
		crus_set_param(this_ctrl.fb_port, g_cirrus_sp_id, CIRRUS_SP_ENABLE,
			   (void *)&this_ctrl.enable,
			   sizeof(struct crus_single_data_t));
	}
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	crus_get_param(this_ctrl.ff_port, g_cirrus_sp_id,
	...
	if (this_ctrl.prot_en) {
		crus_set_param(this_ctrl.fb_port, g_cirrus_sp_id,
	...
	crus_set_param(this_ctrl.ff_port, g_cirrus_sp_id,
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	case 1: /* Load RX Config */
		...
		crus_afe_send_config(firmware->data, firmware->size,
				     this_ctrl.ff_port, g_cirrus_sp_id);
	...
	case 2: /* Load TX Config */
		...
		crus_afe_send_config(firmware->data, firmware->size,
				     this_ctrl.fb_port, g_cirrus_sp_id);
		release_firmware(firmware);
		break;
	default:
		return -EINVAL;
...
static const struct soc_enum cirrus_fb_controls_enum[] = {
	SOC_ENUM_SINGLE_EXT(7, cirrus_fb_port_text),
};
				 ...
				 void __user *arg)
{
	...
	case CRUS_SP_IOCTL_GET:
		...
		crus_get_param(port, g_cirrus_sp_id, crus_sp_hdr.param_id,
				io_data, bufsize);
	...
	case CRUS_SP_IOCTL_SET:
		...
		crus_set_param(port, g_cirrus_sp_id,
...
{
	struct device_node* cirrus_node = NULL;
	uint32_t val = 0;
	int ret;
	...
	cirrus_node = of_find_compatible_node(NULL, NULL, "cirrus,cs35l36");
	if (cirrus_node == NULL) {
		pr_err("%s: Cannot find cirrus from dts\n", __func__);
	} else {
		ret = of_property_read_u32(cirrus_node, "cirrus,sp-id", &val);
		if (!ret) {
			g_cirrus_sp_id = val;
			pr_err("%s: sp-id =0x%x\n", __func__, g_cirrus_sp_id);
		} else {
			g_cirrus_sp_id = CIRRUS_SP;
			pr_err("%s: get fail sp-id =0x%x\n", __func__, g_cirrus_sp_id);
		}
	}
