...
static const char * const rcv_type_texts[] = {"None", "Ana", "I2s"};

static const struct soc_enum rcv_type_enum[] = {
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(rcv_type_texts),
    rcv_type_texts),
};
...
static uint32_t ultrasound_rcv_type = 0;

int elliptic_ultrasound_rcvtype_get(struct snd_kcontrol *kcontrol,
                      struct snd_ctl_elem_value *ucontrol)
{
    ucontrol->value.integer.value[0] = ultrasound_rcv_type;
    return 0;
}

int elliptic_ultrasound_rcvtype_set(struct snd_kcontrol *kcontrol,
                      struct snd_ctl_elem_value *ucontrol)
{
    uint32_t msg[2] = {0, 0};
    ultrasound_rcv_type = ucontrol->value.integer.value[0];

    msg[0] = ultrasound_rcv_type; // 1 = ana rcv; 2 = i2s rcv
    EL_PRINT_I("%s: msg = %d\n", __func__, msg[0]);

    return elliptic_data_write(
        ELLIPTIC_ULTRASOUND_SET_RCV_TYPE,
        (const char *)msg, sizeof(msg));
}
...
static const struct snd_kcontrol_new ultrasound_filter_mixer_controls[] = {
    ...
    SOC_ENUM_EXT("Ultrasound RcvType",
    rcv_type_enum[0],
    elliptic_ultrasound_rcvtype_get,
    elliptic_ultrasound_rcvtype_set),
