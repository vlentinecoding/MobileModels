...
#if IS_ENABLED(CONFIG_TAS25XX_ALGO)
	{
		pr_info ("TI-SmartPA: payload1 = 0x%x, 0x%x", payload[0], payload[1]);
		if((payload[1] == this_afe.tas_module_rx) ||
			(payload[1] == this_afe.tas_module_tx)) {
#ifdef INSTANCE_ID_0 /*APR Version 3*/
            ...
            if (afe_token_is_valid(data->token)) {
                wake_up(&this_afe.wait[data->token]);
				pr_debug("[TI-SmartPA:%s] waking up", __func__);
				return 0;
			} else {
				pr_err("[TI-SmartPA:%s] Invalid token", __func__);
                return -EINVAL;
			}
		}
	}
				    ...
				    uint32_t payload_size)
{
	...
	if (param_hdr.param_id == CAPI_V2_TAS_SA_DC_DETECT) {
		pr_info("[TI-SmartPA:%s] Detected DC, Calling TAS256X Software Reset \n", __func__);
...
static int32_t tas_smartamp_algo_callback(uint32_t *payload, uint32_t payload_size)
{
	...
	if(resp->pdata.param_id == CAPI_V2_TAS_SA_DC_DETECT) {
		...
		pr_info("[TI-SmartPA:%s] Detected DC, Calling TAS256X Software Reset Channel-%s\n", __func__,
...
#ifdef CONFIG_AUDIO_QGKI
#ifdef CONFIG_ELUS_ENABLE
EXPORT_SYMBOL(afe_start_pseudo_port);
#endif /* CONFIG_ELUS_ENABLE */
#endif /* CONFIG_AUDIO_QGKI */
...
#ifdef CONFIG_SND_SOC_AWINIC_AW882XX
     pr_info("%s[Awinic] enter, afe_get_topology: %x, send_afe_cal_apr:%x, send_afe_rx_module_enable:%x", __func__, afe_get_topology,
                     aw_send_afe_cal_apr, aw_send_afe_rx_module_enable);

	aw_get_afe_callback(afe_get_topology, aw_send_afe_cal_apr, aw_send_afe_rx_module_enable,
                         aw_send_afe_tx_module_enable);
#endif
