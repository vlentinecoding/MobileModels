...
#define WCD937X_SYSFS_ENTRY_MAX_LEN 11
	...
	struct class_attribute *attr, char *buf)
{
	...
	struct wcd937x_priv *wcd937x = container_of(c, struct wcd937x_priv, wcd_class);
	if (wcd937x == NULL) {
		dev_err(wcd937x->dev, "%s: NULL data\n", __func__);
		return 0;
	}

	ret = snprintf(buf, WCD937X_SYSFS_ENTRY_MAX_LEN, "%d", wcd937x->codec_error);
	dev_info(wcd937x->dev, "%s: '%s'\n", __func__, buf);

	return ret;
}

static void wcd_register_check_micbias(struct wcd937x_priv *wcd937x)
{
	int result = -1;
	regmap_update_bits(wcd937x->regmap, WCD937X_DIE_CRACK_INT_DIE_CRK_DET_INT1,
		0xff, 0xC2);
	regmap_read(wcd937x->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_OUT, &result);
	if (result != 0) {
		dev_err(wcd937x->dev, "%s: register value is not zero!", __func__);
		wcd937x->codec_error = -1;
		return;
	}
	...
	struct class_attribute *attr, const char *buf, size_t count)
{
	...
	dev_info(wcd937x->dev, "%s: start check cocdec\n", __func__);
	rc = kstrtoint(buf, 10, &val);
	if (rc) {
		dev_err(wcd937x->dev, "%s: kstrtoint failed. rc=%d\n", __func__, rc);
		goto fail;
	}

	if (val != 1) {
		dev_err(wcd937x->dev, "%s: write error value", __func__);
		goto fail;
	}

	/* enable micbias */
	regmap_update_bits(wcd937x->regmap, WCD937X_ANA_MICB1, 0xc0, 0x40);
	regmap_update_bits(wcd937x->regmap, WCD937X_ANA_MICB2, 0xc0, 0x40);
	regmap_update_bits(wcd937x->regmap, WCD937X_ANA_MICB3, 0xc0, 0x40);

	/* start check codec */
	regmap_update_bits(wcd937x->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_EN, 0xff, 0x80);
	wcd_register_check_micbias(wcd937x);

	regmap_update_bits(wcd937x->regmap, WCD937X_DIE_CRACK_DIE_CRK_DET_EN, 0xff, 0xc0);
	wcd_register_check_micbias(wcd937x);

	dev_info(wcd937x->dev, "%s: end check cocdec\n", __func__);
	return (strlen(buf));
fail:
	wcd937x->codec_error = -1;
	return 0;
...
#ifdef CONFIG_AUDIO_CARD_FACTORY
	wcd937x->wcd_class.name = "qcom-wcd-codec";
	wcd937x->wcd_class.class_groups = wcd_fs_groups;
	ret = class_register(&wcd937x->wcd_class);
	if (ret < 0) {
