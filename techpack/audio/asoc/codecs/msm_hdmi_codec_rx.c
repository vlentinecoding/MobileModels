			...
			struct snd_ctl_elem_info *uinfo)
{
	...
	dev_info(component->dev, "%s: count: %d\n", __func__, uinfo->count);
...
static int msm_ext_disp_edid_get(struct snd_kcontrol *kcontrol,
	...
	if (rc >= 0) {
		...
		dev_info(component->dev, "%s: data_blk_size:%d, spk_alloc_data_blk_size:%d\n",
				       ...
				       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (disp_type >= 0) {
		...
		dev_info(component->dev, "%s: Display type: %d\n",
			__func__, disp_type);
	} else {
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_info(component->dev, "%s: control %d, ack set value 0x%x\n",
		...
		struct snd_soc_dai *dai)
{
	...
	dev_info(dai->component->dev, "%s: DP ctl id %d Stream id %d\n",
		__func__,
	...
	if (codec_data->cable_status[dai->id] < 0) {
		...
		dev_info(dai->dev, "%s() ext disp cable ready\n", __func__);
	}
