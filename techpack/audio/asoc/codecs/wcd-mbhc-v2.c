...
static bool g_need_disable_micbias;

static bool lineout_report_en = true;
#ifdef AUDIO_DEBUG_ENABLE
module_param(lineout_report_en, bool, 0664);
MODULE_PARM_DESC(lineout_report_en, "enable/disable lineout report");
#endif

void wcd_mbhc_jack_report(struct wcd_mbhc *mbhc,
			  struct snd_soc_jack *jack, int status, int mask)
{
	if (lineout_report_en) {
...
static bool is_disable_micbias(struct snd_soc_card *card)
{
	int ret = 0;
	u32 disable = 0;
	const char *bias_dt = "micbias-disable-flag";

	if (of_find_property(card->dev->of_node, bias_dt, NULL))
		ret = of_property_read_u32(card->dev->of_node, bias_dt, &disable);

	if (ret != 0 || disable == 0)
		return false;
	pr_info("%s: need disable micbias to eliminate noise\n", __func__);
	return true;
}
		      ...
		      bool impedance_det_en)
{
	...
	g_need_disable_micbias = is_disable_micbias(card);
