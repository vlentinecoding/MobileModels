...
#define WCD938X_SYSFS_ENTRY_MAX_LEN 11
...
#ifdef CONFIG_AUDIO_CARD_FACTORY
static ssize_t wcd_register_show(struct class *c,
	struct class_attribute *attr, char *buf)
{
	int ret = 0;

	struct wcd938x_priv *wcd938x = container_of(c, struct wcd938x_priv, wcd_class);
	if (wcd938x == NULL) {
		dev_err(wcd938x->dev, "%s: NULL data\n", __func__);
		return 0;
	}

	ret = snprintf(buf, WCD938X_SYSFS_ENTRY_MAX_LEN, "%d", wcd938x->codec_error);
	dev_info(wcd938x->dev, "%s: '%s'\n", __func__, buf);

	return ret;
}

static void wcd_register_check_micbias(struct wcd938x_priv *wcd938x)
{
	int result = -1;
	regmap_update_bits(wcd938x->regmap, WCD938X_DIE_CRACK_INT_DIE_CRK_DET_INT1,
		0xff, 0xC2);
	regmap_read(wcd938x->regmap, WCD938X_DIE_CRACK_DIE_CRK_DET_OUT, &result);
	if (result != 0) {
		dev_err(wcd938x->dev, "%s: register value is not zero!", __func__);
		wcd938x->codec_error = -1;
		return;
	}
...
static ssize_t wcd_register_store(struct class *c,
	struct class_attribute *attr, const char *buf, size_t count)
{
	...
	dev_info(wcd938x->dev, "%s: start check cocdec\n", __func__);
	rc = kstrtoint(buf, 10, &val);
	if (rc) {
		dev_err(wcd938x->dev, "%s: kstrtoint failed. rc=%d\n", __func__, rc);
		goto fail;
	}

	if (val != 1) {
		dev_err(wcd938x->dev, "%s: write error value", __func__);
		goto fail;
	}

	/* enable micbias */
	regmap_update_bits(wcd938x->regmap, WCD938X_ANA_MICB1, 0xc0, 0x40);
	regmap_update_bits(wcd938x->regmap, WCD938X_ANA_MICB2, 0xc0, 0x40);
	regmap_update_bits(wcd938x->regmap, WCD938X_ANA_MICB3, 0xc0, 0x40);
	regmap_update_bits(wcd938x->regmap, WCD938X_ANA_MICB4, 0xc0, 0x40);

	/* start check codec */
	regmap_update_bits(wcd938x->regmap, WCD938X_DIE_CRACK_DIE_CRK_DET_EN, 0xff, 0x80);
	wcd_register_check_micbias(wcd938x);

	regmap_update_bits(wcd938x->regmap, WCD938X_DIE_CRACK_DIE_CRK_DET_EN, 0xff, 0xc0);
	wcd_register_check_micbias(wcd938x);

	dev_info(wcd938x->dev, "%s: end check cocdec\n", __func__);
	return (strlen(buf));
fail:
	wcd938x->codec_error = -1;
	return 0;
...
struct class_attribute class_attr_wcd_check = __ATTR(wcd_register, 0664,
	wcd_register_show, wcd_register_store);

static struct attribute *wcd_fs_attrs[] = {
	&class_attr_wcd_check.attr,
	NULL,
};

ATTRIBUTE_GROUPS(wcd_fs);
#endif
...
#ifdef CONFIG_AUDIO_CARD_FACTORY
	wcd938x->wcd_class.name = "qcom-wcd-codec";
	wcd938x->wcd_class.class_groups = wcd_fs_groups;
	ret = class_register(&wcd938x->wcd_class);
	if (ret < 0) {
		dev_err(dev, "register wcd_class failed, rc=%d\n");
	}
#endif
