...
static const struct reg_sequence cs35l36_spk_revb1_errata_patch[] = {
...
static int cs35l36_pa_spk_num_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	struct cs35l36_private *cs35l36 =
			snd_soc_component_get_drvdata(component);
	pr_debug("%s: %d\n", __func__, cs35l36->pdata.multi_amp_count);

	ucontrol->value.integer.value[0] = cs35l36->pdata.multi_amp_count;

	return 0;
}
static int cs35l36_pa_spk_num_put(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
...
static const struct snd_kcontrol_new cs35l36_aud_controls[] = {
	...
	SOC_SINGLE_BOOL_EXT("LDM Select Switch", 0, cs35l36_ldm_sel_get,
		cs35l36_ldm_sel_put),
	SOC_SINGLE_EXT("Cirrus PA Count", SND_SOC_NOPM, 0, 8, 0, cs35l36_pa_spk_num_get,
		cs35l36_pa_spk_num_put),
	SOC_SINGLE_RANGE("ASPTX1 Slot Position", CS35L36_ASP_TX1_TX2_SLOT, 0,
			 0, 7, 0),
	SOC_SINGLE_RANGE("ASPTX2 Slot Position", CS35L36_ASP_TX1_TX2_SLOT, 16,
			 0, 7, 0),
	SOC_SINGLE_RANGE("ASPTX3 Slot Position", CS35L36_ASP_TX3_TX4_SLOT, 0,
			 0, 7, 0),
	SOC_SINGLE_RANGE("ASPTX4 Slot Position", CS35L36_ASP_TX3_TX4_SLOT, 16,
			 0, 7, 0),
};
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	struct cs35l36_private *cs35l36 =
			snd_soc_component_get_drvdata(component);
	u32 reg;
        dev_info(component->dev, "cs35lxx_main_amp_event event = 0x%x\n", event);

	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		regmap_register_patch(cs35l36->regmap,
						cs35l36_spk_revb1_errata_patch,
						ARRAY_SIZE(cs35l36_spk_revb1_errata_patch));
				 ...
				 struct snd_soc_dai *dai)
{
	...
	default:
        ...
        dev_info(dai->component->dev, "cs35lxx_pcm_hw_params sample rate: %d, asp_width: %d\n", global_fs, asp_width);
	//for system clock
	if (asp_width > CS35L36_ASP_WIDTH_16) {
		...
		asp_width = CS35L36_ASP_WIDTH_32;
		cs35l36_component_set_sysclk(dai->component, 0, 0, 2 * global_fs * asp_width, 0);
		regmap_update_bits(cs35l36->regmap, CS35L36_ASP_FRAME_CTRL,
			       ...
			       struct snd_soc_dai *dai)
{
	...
	cs35l36_set_dai_fmt(dai, SND_SOC_DAIFMT_CBS_CFS|SND_SOC_DAIFMT_I2S);
				...
				int dir)
{
	...
	int prev_clksrc;
	bool pdm_switch;
        dev_info(component->dev, "cs35lxx_component_set_sysclk clk_id: %d, Freq: %d, dir: %d\n", clk_id, freq, dir);
...
static irqreturn_t cs35l36_irq(int irq, void *data)
{
	...
	int ret = IRQ_NONE;
	dev_crit(cs35l36->dev, "step into cs35l36_irq\n");
	...
	regmap_read(cs35l36->regmap, CS35L36_INT1_STATUS, &status[0]);
	regmap_read(cs35l36->regmap, CS35L36_INT2_STATUS, &status[1]);
	regmap_read(cs35l36->regmap, CS35L36_INT3_STATUS, &status[2]);
	regmap_read(cs35l36->regmap, CS35L36_INT4_STATUS, &status[3]);
	
	regmap_read(cs35l36->regmap, CS35L36_INT1_MASK, &masks[0]);
	regmap_read(cs35l36->regmap, CS35L36_INT2_MASK, &masks[1]);
	regmap_read(cs35l36->regmap, CS35L36_INT3_MASK, &masks[2]);
	regmap_read(cs35l36->regmap, CS35L36_INT4_MASK, &masks[3]);
				...
				struct cs35l36_platform_data *pdata)
{
	...
	ret = of_property_read_u32(np, "cirrus,multi-amp-mode", &val);
	if (!ret) {
		if (val <= 1) {
			dev_warn(&i2c_client->dev,
				"Invalid speaker count number\n", val);
			pdata->multi_amp_mode = false;
			pdata->multi_amp_count = 1;
		} else {
			pdata->multi_amp_mode = true;
			pdata->multi_amp_count = val;
		}
	}
...
static const struct reg_sequence cs35l36_revb1_errata_patch[] = {
	{ CS35L36_TESTKEY_CTRL,	CS35L36_TEST_UNLOCK1 },
	{ CS35L36_TESTKEY_CTRL, CS35L36_TEST_UNLOCK2 },
	{ 0x00007064,		0x0929A800 },
	{ 0x00007850,		0x00002FA9 },
...
static void cs35l36_reset(struct cs35l36_private *cs35l36)
{
			      ...
			      const struct i2c_device_id *id)
{
	...
	case CS35L36_REV_B1:
		...
		ret = regmap_register_patch(cs35l36->regmap,
				cs35l36_revb1_errata_patch,
				ARRAY_SIZE(cs35l36_revb1_errata_patch));
		if (ret < 0) {
			dev_err(dev, "Failed to apply B1 errata patch %d\n",
				ret);
			goto err;
		}
		break;
	...
	//	goto err;
	//}
	gpio_set_value(i2c_client->irq, 1);
	usleep_range(2000, 2100);
	...
	//smartpa_type = set_smartpa_type("cs35l36_i2c_probe", CS35L36);
	hw_set_smartpa_type("cs35lxx",sizeof("cs35lxx"));
	hw_add_smartpa_codec(dev->driver->name, dev_name(dev), cs35l36_dai[0].name);
