...
int tas25xx_parse_algo_dt_sysfs(struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "ti,channels", &temp);
	s_smartpa_algo->mn_channels = (!ret) ? (int)temp : 1;

	ret = of_property_read_u32(np, "ti,impedance-min", &temp);
	s_smartpa_algo->imped_min[0] = (!ret) ? (int)temp : RDC_MIN_L;

	ret = of_property_read_u32(np, "ti,impedance-max", &temp);
	s_smartpa_algo->imped_max[0] = (!ret) ? (int)temp : RDC_MAX_L;

	pr_info("[SmartPA-%s]: Re(min,max) left = %d, %d\n", __func__,
			s_smartpa_algo->imped_min[0], s_smartpa_algo->imped_max[0]);

	ret = of_property_read_u32(np, "ti,frequency-min", &temp);
	s_smartpa_algo->fres_min[0] = (!ret) ? (int)temp : 500;

	ret = of_property_read_u32(np, "ti,frequency-max", &temp);
	s_smartpa_algo->fres_max[0] = (!ret) ? (int)temp : 1100;

	pr_info("[SmartPA-%s]: F(min,max) left = %d, %d\n", __func__,
			s_smartpa_algo->fres_min[0], s_smartpa_algo->fres_max[0]);

	ret = of_property_read_u32(np, "ti,Qt-min", &temp);
	s_smartpa_algo->Qt[0] = (!ret) ? (int)temp : 100;

	pr_info("[SmartPA-%s]: Qt left = %d\n", __func__, s_smartpa_algo->Qt[0]);

	if (s_smartpa_algo->mn_channels == 2) {
		ret = of_property_read_u32(np, "ti,impedance-min-r", &temp);
		s_smartpa_algo->imped_min[1] = (!ret) ? (int)temp : RDC_MIN_R;

		ret = of_property_read_u32(np, "ti,impedance-max-r", &temp);
		s_smartpa_algo->imped_max[1] = (!ret) ? (int)temp : RDC_MAX_R;

		pr_info("[SmartPA-%s]: Re(min,max) right = %d, %d\n", __func__,
				s_smartpa_algo->imped_min[1], s_smartpa_algo->imped_max[1]);

		ret = of_property_read_u32(np, "ti,frequency-min-r", &temp);
		s_smartpa_algo->fres_min[1] = (!ret) ? (int)temp : 500;

		ret = of_property_read_u32(np, "ti,frequency-max-r", &temp);
		s_smartpa_algo->fres_max[1] = (!ret) ? (int)temp : 1100;

		pr_info("[SmartPA-%s]: F(min,max) right = %d, %d\n", __func__,
				s_smartpa_algo->fres_min[1], s_smartpa_algo->fres_max[1]);

		ret = of_property_read_u32(np, "ti,Qt-min-r", &temp);
		s_smartpa_algo->Qt[1] = (!ret) ? (int)temp : 100;

		pr_info("[SmartPA-%s]: Qt right = %d\n", __func__, s_smartpa_algo->Qt[1]);
	}
