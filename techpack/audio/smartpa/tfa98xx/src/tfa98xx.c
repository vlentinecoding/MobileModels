...
static uint8_t num_of_started = 0;
#endif
...
int mtk_spk_send_ipi_buf_to_dsp(void *buffer, uint32_t size)
{
	(void)buffer;
	(void)size;
	return 0;
...
int mtk_spk_recv_ipi_buf_from_dsp(int8_t *buffer, int16_t size, uint32_t *data_len)
{
	(void)buffer;
	(void)size;
	(void)data_len;
...
#ifdef ENABLE_DYNAMIC_BATTERY_PROTECTION_FTR
			cancel_delayed_work_sync(&tfa98xx->battery_work);
			tfa98xx->battery_protection_data = 0;
#endif
...
#ifdef CONFIG_HUAWEI_DSM_AUDIO
	char report[TFA_DMD_INFO_LEN] = {0};
#endif
...
#ifdef CONFIG_HUAWEI_DSM_AUDIO
		snprintf(report, sizeof(report), "Write:i2c reg=0x%02x", tfa98xx->i2c->addr);
		audio_dsm_report_info(AUDIO_SMARTPA, DSM_SMARTPA_I2C_ERR, "%s", report);
		pr_warn("dsm report : %s\n", report);
#endif
		return Tfa98xx_Error_Fail;
	}
...
#ifdef ENABLE_DYNAMIC_BATTERY_PROTECTION_FTR
static struct tfa98xx_battery_protection_rule  protection_rule[] = {
	{-20, -10, 21, 30,  4},   /* level1: */
	{-20, -10, 0, 20,   8},   /* level2: */
	{-50, -20, 36, 50,  10},  /* level3: */
	{-50, -20, 0, 35,   14}   /* level4: */
};

static void tfa98xx_battery_protection(struct work_struct *work)
{
	struct tfa98xx *tfa98xx = container_of(work, struct tfa98xx, battery_work.work);
	uint8_t cmd[] = {0x00, 0x81, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	int16_t temperature = 0, powerenergy = 0, new_value = 0;
	int i = 0, ret = 0;
	int value = 0;

	/* get battery temperature, energy from system api */
	if (audio_get_battery_info(BATT_TEMP, &value) != 0)
		pr_err("%s: get battery temperature failed\n", __func__);
	temperature = (int16_t)value / 10;
	if (audio_get_battery_info(BATT_CAP, &value) != 0)
		pr_err("%s: get battery powerenergy failed\n", __func__);
	powerenergy = (int16_t)value;

	/* find target parameters from protection rule table, if not found, we will use default value. */
	for (i = 0;i < ARRAY_SIZE(protection_rule); i++) {
		if ((protection_rule[i].temperature_threadhold_low <= temperature) &&
		    (temperature >= protection_rule[i].temperature_threadhold_high) &&
			(protection_rule[i].powerenergy_threadhold_low <= powerenergy) &&
			(powerenergy >= protection_rule[i].powerenergy_threadhold_high)) {
			   new_value = protection_rule[i].value;
			   break;
		}
	}

	if (NULL != tfa98xx) {
		if (tfa98xx->battery_protection_data != new_value) {
			tfa98xx->battery_protection_data = new_value;
			cmd[0] = (1 == tfa98xx_device_count) ? 0x04 : 0x00;
			cmd[5] = new_value;
			cmd[8] = new_value;
			ret = tfa98xx_send_data_to_dsp(cmd, sizeof(cmd));
			pr_info("%s: apply new parameter(%d) to powersaver  returned(%d)\n", __func__, new_value, ret);
		}
		pr_info("%s: apply new parameter(%d) to powersaver  returned(%d)\n", __func__, new_value, ret);
		/* reschedule queue work */
		queue_delayed_work(tfa98xx->tfa98xx_wq, &tfa98xx->battery_work, 5 * HZ);
	} else {
		pr_err("[error] %s: tfa98xx is null\n", __func__);
	}
...
#ifdef ENABLE_DYNAMIC_BATTERY_PROTECTION_FTR
			/* start battery detection thread */
			queue_delayed_work(tfa98xx->tfa98xx_wq,
				&tfa98xx->battery_work, 0);
#endif
			list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
				mutex_lock(&tfa98xx->dsp_lock);
				tfa_dev_set_state(tfa98xx->tfa, TFA_STATE_UNMUTE, 0);
...
#endif
static void tfa98xx_algo_mute(struct tfa98xx *tfa98xx)
{
	int ret;
	uint8_t cmd[] = {0x00, 0x81, 0x04, 0x00, 0x00, 0xff, 0x00, 0x00, 0xff};
	if (NULL == tfa98xx) {
	...
	struct snd_soc_dai *dai)
{
#ifdef CONFIG_ARCH_SPRD
    int err = 0;
    dev_info(dai->codec->dev, "%s(): substream = %s  stream = %d  num_of_started = %d\n",
        __func__, substream->name, substream->stream, num_of_started);
    ...
    if (1 == num_of_started++) {
        err = tfa98xx_mute(dai, 0, substream->stream);
    }
...
#ifdef CONFIG_ARCH_SPRD
static int tfa98xx_hw_free(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
	int ret = 0;
	dev_info(dai->codec->dev, "%s(): substream = %s  stream = %d  num_of_started = %d\n",
		__func__, substream->name, substream->stream, num_of_started);
	...
	if (!substream->runtime) {
		dev_err(dai->codec->dev, "[Error] %s: substream->runtime=0\n", __func__);
		return 0;
	}
	...
	mutex_unlock(&tfa98xx_mutex);
	if (0 == num_of_started) {
		ret = tfa98xx_mute(dai, 1, substream->stream);
	}
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		if ((strstr(tfa_cont_profile_name(tfa98xx, tfa98xx->profile), "handset") == NULL) && tfa98xx->tfa->is_probus_device) {
			tfa98xx_algo_mute(tfa98xx);
			msleep(60);
		}
...
static char hw_firmware_file[64] = { 0 };
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
...
#endif
#ifdef ENABLE_DYNAMIC_BATTERY_PROTECTION_FTR
	INIT_DELAYED_WORK(&tfa98xx->battery_work, tfa98xx_battery_protection);
#endif

	tfa98xx->codec = codec;

	memset(hw_firmware_file, 0, sizeof(hw_firmware_file));
	memcpy(hw_firmware_file, "tfa98xx.cnt", strlen("tfa98xx.cnt"));
	fw_name = hw_firmware_file;
	hw_reset_smartpa_firmware("tfa98xx", "cnt",
		hw_firmware_file, sizeof(hw_firmware_file));
...
static int tfa9894_do_calibrate(struct tfa98xx *tfa98xx_pri, struct tfa98xx *tfa98xx_sec)
{
	...
	int err = 0, i = 0, spk_imp = 0, devices = 0;
...
void tfa98xx_set_vendor_info(int id, char *chip){
	(void)id;
	(void)chip;
	hw_set_smartpa_type(chip, strlen(chip));
	...
	const struct i2c_device_id *id)
{
	...
	if (hw_check_smartpa_type("tfa98xx", "none") == false) {
		dev_info(&i2c->dev, "%s other smartpa driver probe succ, no need probe\n", __func__);
		return 0;
	}

	if (hw_lock_pa_probe_state() == 0) {
		dev_info(&i2c->dev, "%s get lock fail, other smartpa driver processing, probe later\n", __func__);
		return -EPROBE_DEFER;
	}
	...
	hw_add_smartpa_codec(i2c->dev.driver->name,
		dev_name(&i2c->dev), dai[0].name);
