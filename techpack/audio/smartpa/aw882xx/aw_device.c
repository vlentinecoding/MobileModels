...
static void aw_dev_fade_in(struct aw_device *aw_dev)
{
	...
	/*volume up*/
	for (i = desc->mute_volume; i >= desc->init_volume; i -= g_fade_step) {
...
static void aw_dev_fade_out(struct aw_device *aw_dev)
{
	...
	i = start_volume;
	for (i = start_volume; i <= desc->mute_volume; i += g_fade_step) {
...
int aw_dev_get_fade_vol_step(void)
{
	return g_fade_step;
}

void aw_dev_set_fade_vol_step(unsigned int step)
{
	g_fade_step = step;
}
...
int aw_dev_get_dsp_re(struct aw_device *aw_dev)
{
	return aw_dev->cali_desc.cali_re;
}

void aw_dev_set_dsp_re(struct aw_device *aw_dev, int32_t cali_re)
{
...
int aw_dev_fw_update(struct aw_device *aw_dev, bool prof_sw)
{
	...
	if (prof_sw) {
		/*prof switch call, cur != set update*/
		if (aw_dev->cur_prof != aw_dev->set_prof)
...
static void aw_dev_cali_re_update(struct aw_device *aw_dev)
{
	...
	if (AW_ERRO_CALI_VALUE != aw_dev->cali_desc.cali_re) {
		aw_dsp_write_cali_re(aw_dev, aw_dev->cali_desc.cali_re);
	}
}
...
int aw_dev_prof_update(struct aw_device *aw_dev)
{
	...
	if (aw_dev->status == AW_DEV_PW_ON) {
		...
		ret = aw_dev_fw_update(aw_dev, true);
		if (ret) {
...
int aw_device_start(struct aw_device *aw_dev)
{
	...
	if (ret < 0) {
		...
		aw_dev_dbg(aw_dev->dev, "%s: pll check failed cannot start\n", __func__);
		return ret;
	}
