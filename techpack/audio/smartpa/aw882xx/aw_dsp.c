...
#include <sound/smartpa/aw882xx/aw_dsp_interface.h>
...
#define AFE_PORT_ID_AWDSP_TX	(AFE_PORT_ID_QUINARY_MI2S_TX)	/*INDEX_AFE_PORT_ID_PRIMARY_MI2S_TX*/
#define AFE_PORT_ID_AWDSP_RX	(AFE_PORT_ID_QUINARY_MI2S_RX)	/*INDEX_AFE_PORT_ID_PRIMARY_MI2S_RX*/
...
int (*aw_afe_get_topology_point)(int port_id);
...
{
    pr_info("%s[Awinic] enter, afe_get_topology: %x, send_afe_cal_apr:%x, send_afe_rx_module_enable:%x", __func__, afe_get_topology, send_afe_cal_apr, send_afe_rx_module_enable);

	aw_afe_get_topology_point = afe_get_topology;
	aw_send_afe_cal_apr = send_afe_cal_apr;
	aw_send_afe_rx_module_enable = send_afe_rx_module_enable;
	aw_send_afe_tx_module_enable = send_afe_tx_module_enable;
}
EXPORT_SYMBOL(aw_get_afe_callback);
#if 0
...
{
	if (aw_afe_get_topology_point != NULL) {
		return aw_afe_get_topology_point(AFE_PORT_ID_AWDSP_RX);
	} else {
		aw_pr_err("aw_afe_get_topology_point is NULL");
		return -EINVAL;
	}
...
static int aw_write_data_to_dsp(uint32_t param_id, void *data, int len)
{
	...
	while (try < AW_DSP_TRY_TIME) {
		if (aw_check_dsp_ready()) {
			if (aw_send_afe_cal_apr == NULL) {
				aw_pr_err("aw_send_afe_cal_apr is NULL");
...
int aw_dsp_set_afe_module_en(int type, int enable)
{
	...
	if (type == AW_RX_MODULE) {
		if (aw_send_afe_rx_module_enable == NULL) {
			aw_pr_err("aw_send_afe_rx_module_enable is NULL");
			return -EINVAL;
		}
		return aw_send_afe_rx_module_enable(&value, sizeof(int32_t));
	} else {
		if (aw_send_afe_tx_module_enable == NULL) {
			aw_pr_err("aw_send_afe_tx_module_enable is NULL");
			return -EINVAL;
		}
