...
#define AW882XX_DRIVER_VERSION "v0.3.0.6"
			...
			struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		aw_dev_dbg(aw882xx->dev, "requested rate: %d, sample size: %d",
	...
	aw882xx->aw_pa->ops.aw_update_rate(aw882xx->aw_pa, rate, width);
...
static void aw882xx_start(struct aw882xx *aw882xx)
{
	...
	aw_dev_fw_update(aw882xx->aw_pa, false);
	aw882xx->pstream = 1;
	...
	if (aw882xx->pstream) {
		if (ucontrol->value.integer.value[0] == 0) {
			...
			if (aw882xx->fw_status == AW_DEV_FW_OK) {
				aw_dev_fw_update(aw882xx->aw_pa, true);
...
static int aw882xx_request_firmware_file(struct aw882xx *aw882xx)
{
	aw_dev_info(aw882xx->dev, "load [%s]", aw882xx->aw_pa->acf_name);
...
static void aw882xx_fw_wrok_work(struct work_struct *work)
{
	struct aw882xx *aw882xx = container_of(work, struct aw882xx, fw_work.work);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	aw_dev_dbg(aw882xx->dev, "aw882xx_rx_enable %d", ctrl_value);
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret) {
		aw_dev_err(aw882xx->dev, "dsp_msg error, ret=%d",
				ret);
		ctrl_value = 0;
	}
	ucontrol->value.integer.value[0] = ctrl_value;
...
static int aw882xx_codec_probe(aw_snd_soc_codec_t *aw_codec)
{
    ...
    struct i2c_client *i2c = aw882xx->i2c;
	...
	if (aw882xx->chip_id == PID_1852_ID)
		hw_reset_smartpa_firmware("aw882xx_pid_1852_acf", "bin",
			aw882xx->aw_pa->acf_name, AW_NAME_MAX);
	...
	INIT_DELAYED_WORK(&aw882xx->fw_work, aw882xx_fw_wrok_work);
	...
	/* rename i2c client name */
	if (i2c->dev.of_node) {
		dev_set_name(&i2c->dev, "%x-%04x", i2c->adapter->nr, i2c->addr);
	} else {
		dev_err(&i2c->dev, "%s failed to set device name", __func__);
	}
				...
				int num_dai)
{
	...
	if ((dai_drv != NULL) && (num_dai > 0))
		for(i = 0; i < num_dai; i++) {
			snprintf(buf, 50, "%s-%d",dai_drv[i].name, aw882xx->index);
...
int aw_componet_codec_register(struct aw882xx *aw882xx)
{
	...
	hw_add_smartpa_codec(aw882xx->dev->driver->name,
		dev_name(aw882xx->dev), dai_drv[0].name);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "%d", &databuf[0])) {
		...
		aw_dev_set_fade_vol_step(databuf[0]);
	}
...
static int aw882xx_dvdd_control(struct device *dev)
{
	struct regulator *regulator = NULL;
	int ret;

	regulator = regulator_get(dev, "smartpa-vdd");
	if (regulator == NULL) {
		pr_err("%s: get smartpa-vdd failed\n", __func__);
		return -ENODEV;
	}
				...
				const struct i2c_device_id *id)
{
	...
	hw_set_smartpa_type("aw882xx", strlen("aw882xx"));
	...
	if (i2c->dev.of_node) {
		dev_set_name(&i2c->dev, "%s-%d", "aw882xx_smartpa", g_aw882xx_dev_cnt);
	} else {
		dev_err(&i2c->dev, "%s failed to set device name: %d", __func__,  g_aw882xx_dev_cnt);
	}
