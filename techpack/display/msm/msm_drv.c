...
static int msm_drm_component_init(struct device *dev)
{
...
#if defined(CONFIG_DRM_FBDEV_EMULATION) && defined(CONFIG_LCD_QCOM_DRV)
	...
	for (i = 0; i < sde_kms->dsi_display_count; ++i) {
		...
		if (!display || !display->panel) {
			DRM_ERROR("display or display->panel is null\n");
			goto fail;
		}
		panel = display->panel;
		if (panel->pdata && panel->pdata->create_sysfs) {
			fbinfo = priv->fbdev->fbdev;
			ret = panel->pdata->create_sysfs(&fbinfo->dev->kobj);
			if (ret) {
				DRM_ERROR("failed to create fbdev sysfs: %d\n", ret);
				goto fail;
			}
		}
	}
...
#ifdef CONFIG_LCD_QCOM_DRV
static int msm_ioctl_hbm_set(struct drm_device *dev, void *data,
		struct drm_file *file)
{
	struct sde_kms *sde_kms = NULL;
	struct dsi_display *display = NULL;
	struct dsi_panel *panel = NULL;
	struct msm_drm_private *priv;
	struct display_engine_ddic_hbm_param *hbm_te_data;
	int ret;
	int i;
	...
	for (i = 0; i < sde_kms->dsi_display_count; ++i) {
		...
		hbm_te_data = (struct display_engine_ddic_hbm_param *)data;
		if (hbm_te_data->type == TEIRQ_ENABLE){
			enable_te_irq (display);
			DRM_INFO("msm_ioctl set teirq: %d\n",display->is_te_irq_enabled);
			return 0;
		}else if (hbm_te_data->type == TEIRQ_DISABLE){
			disable_te_irq (display);
			DRM_INFO("msm_ioctl reset teirq: %d\n",display->is_te_irq_enabled);
			return 0;
		}
		mutex_lock(&display->display_lock);
		if (!display->enabled) {
			mutex_unlock(&display->display_lock);
			DRM_INFO("display is suspend\n");
			return 0;
		}
		panel = display->panel;
		if (panel->pdata && panel->pdata->panel_hbm_set) {
			ret = panel->pdata->panel_hbm_set(panel,
				(struct display_engine_ddic_hbm_param *)data);
			if (ret) {
				DRM_ERROR("failed to set panel hbm: %d\n", ret);
				mutex_unlock(&display->display_lock);
				return -EINVAL;
			}
		}
...
static const struct drm_ioctl_desc msm_ioctls[] = {
	...
	DRM_IOCTL_DEF_DRV(MSM_DISPLAY_HINT, msm_ioctl_display_hint_ops,
			DRM_UNLOCKED),
#ifdef CONFIG_LCD_QCOM_DRV
	DRM_IOCTL_DEF_DRV(MSM_DISPLAY_HBM, msm_ioctl_hbm_set,
			DRM_UNLOCKED),
#endif
};
