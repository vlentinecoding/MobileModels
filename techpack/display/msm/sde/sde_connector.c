...
static const struct drm_prop_enum_list e_hbm_mode[] = {
	{HBM_MODE_ON,   "HBM_ON"},
	{HBM_MODE_OFF,  "HBM_OFF"},
};
#endif
	...
	struct msm_display_info *display_info)
{
	...
	msm_property_install_enum(&c_conn->property_info, "LP",
			...
			CONNECTOR_PROP_LP);
#ifdef CONFIG_LCD_QCOM_DRV
	msm_property_install_enum(&c_conn->property_info, "HBM",
			0, 0, e_hbm_mode,
			ARRAY_SIZE(e_hbm_mode), 1,
			CONNECTOR_PROP_HBM);
#endif
	return 0;
}
...
void lcd_sde_panel_recovery(void *sde_connector, bool skip_pre_kickoff)
{
	_sde_connector_report_panel_dead((struct sde_connector *)sde_connector, skip_pre_kickoff);
}
#endif
