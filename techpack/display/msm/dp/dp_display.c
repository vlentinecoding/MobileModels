...
#if !defined(CONFIG_QGKI) || !defined(CONFIG_QCOM_FSA4480_I2C_MODULE)
...
// gki: display is module, qgki: display is built-in
#if defined(CONFIG_QGKI) && defined(CONFIG_QCOM_FSA4480_I2C_MODULE)
__attribute__((weak)) int fsa4480_reg_notifier(struct notifier_block *nb,
	struct device_node *node)
{
	(void)nb;
	(void)node;
	pr_info("%s: use weak func\n", __func__);
	return 0;
}

__attribute__((weak)) int fsa4480_unreg_notifier(struct notifier_block *nb,
	struct device_node *node)
{
	(void)nb;
	(void)node;
...
static int dp_display_usbpd_configure_cb(struct device *dev)
{
	...
	reinit_completion(&dp->connect_comp);
...
static void dp_display_switch_source(struct dp_debug *debug, int mode)
{
	if (debug->is_debugging) {
		DP_INFO("currently passing edid_modes debugging\n");
		return;
	}
	...
	if (mode == DIFF_SOURCE) {
		...
		// diff mode is set to 1080p@60fps
		debug->hdisplay = DP_DIFF_SOURCE_HDISPLAY;
		debug->vdisplay = DP_DIFF_SOURCE_VDISPLAY;
		debug->vrefresh = DP_DIFF_SOURCE_VREFRESH;
...
static int dp_display_usbpd_check_source_cb(struct device *dev)
{
	...
	// check whether it is in diff mode during altmode event process
	DP_INFO("current source mode %d\n", dp->debug->source_mode);
	dp_display_switch_source(dp->debug, dp->debug->source_mode);
	return 0;
}

static int dp_display_usbpd_switch_source_cb(struct device *dev, int mode)
{
	struct dp_display_private *dp = NULL;
	...
	DP_INFO("set source mode %d\n", mode);
	dp_display_switch_source(dp->debug, mode);
	dp->debug->source_mode = mode;
	return 0;
}

static int dp_display_usbpd_connect_comp_cb(struct device *dev)
{
	struct dp_display_private *dp = NULL;
	unsigned long wait_timeout_ms;
	unsigned long t;
	...
	// The return value is 0 if timed out, and positive (at least 1,
	// or number of jiffies left till timeout) if completed.
	wait_timeout_ms = DP_CONNECT_COMP_DELAY_MS;
	t = wait_for_completion_timeout(&dp->connect_comp, msecs_to_jiffies(wait_timeout_ms));
	DP_INFO("dp connect wait_timeout %lums time_waited %ums\n",
...
static void dp_display_connect_work(struct work_struct *work)
{
	...
	dp_cust_set_param(DP_PARAM_HOTPLUG_RETVAL, &rc, sizeof(rc));
...
static int dp_init_sub_modules(struct dp_display_private *dp)
{
	...
	cb->check_source = dp_display_usbpd_check_source_cb;
	cb->switch_source = dp_display_usbpd_switch_source_cb;
	cb->connect_comp = dp_display_usbpd_connect_comp_cb;
	...
	// check whether it is in diff mode during startup
	dp->debug->source_mode = dp_source_get_current_mode();
	DP_INFO("get source mode %d\n", dp->debug->source_mode);
	dp_display_switch_source(dp->debug, dp->debug->source_mode);
		...
		struct dp_display_mode *mode)
{
	...
	DP_INFO("timing pixel_clk_khz %u, bpp %u\n",
...
static int dp_display_post_enable(struct dp_display *dp_display, void *panel)
{
	...
	DP_INFO("dp_panel audio supported %d\n", dp_panel->audio_supported);
	if (dp_panel->audio_supported) {
		dp_panel->audio->bw_code = dp->link->link_params.bw_code;
		dp_panel->audio->lane_count = dp->link->link_params.lane_count;
		dp_panel->audio->on(dp_panel->audio);
		dp_cust_set_param(DP_PARAM_AUDIO_SUPPORTED, NULL, 0);
	}

	// dp connect complete, stream enable & audio on
	complete_all(&dp->connect_comp);
end:
...
static int dp_display_probe(struct platform_device *pdev)
{
	...
	init_completion(&dp->connect_comp);
