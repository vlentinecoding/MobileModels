		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	edid = debug->edid;
	dp_cust_hex_dump("debug edid", edid, debug->edid_size);
bail:
	kfree(buf);
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	dpcd = debug->dpcd;
	dp_cust_hex_dump("debug dpcd", dpcd, size / char_to_nib);
bail:
	kfree(buf);
		...
		const char __user *user_buff, size_t count, loff_t *ppos)
{
	...
	debug->dp_debug.is_debugging = true;
...
clear:
	...
	debug->dp_debug.is_debugging = false;
