...
__attribute__((weak)) int fsa4480_switch_event(struct device_node *node,
	enum fsa_function event)
{
	(void)node;
	pr_info("%s: use weak func, event = %d\n", __func__, event);
	return 0;
}
#endif
...
static void dp_aux_isr(struct dp_aux *dp_aux)
{
	...
	if (aux->aux_error_num != DP_AUX_ERR_NONE)
		dp_cust_set_param(DP_PARAM_AUX_ERR_NO, &aux->aux_error_num, sizeof(aux->aux_error_num));
}
		...
		struct drm_dp_aux_msg *msg)
{
	...
	if (aux->aux_error_num == DP_AUX_ERR_NONE) {
		...
		dp_cust_get_edid(aux->native, aux->read, msg);
		dp_aux_hex_dump(drm_aux, msg);
