...
skip_dpcd_read:
	...
	dp_cust_get_dpcd_revision(&dpcd[DP_DPCD_REV]);
	...
	dp_cust_set_sink_lanes_rate(link_info->num_lanes, dpcd[DP_MAX_LINK_RATE]);
	...
	if ((dpcd[DP_DOWNSTREAMPORT_PRESENT] & DP_DWN_STRM_PORT_PRESENT)
		...
		dp_cust_set_param(DP_PARAM_DS_PORT, dp_panel->ds_ports, sizeof(dp_panel->ds_ports));
	}
	...
	struct drm_connector *connector, bool multi_func)
{
	...
	if (rc || !is_link_rate_valid(drm_dp_link_rate_to_bw_code(
		...
		dp_cust_set_param(DP_PARAM_DPCD, dp_panel->dpcd, sizeof(dp_panel->dpcd));
	}
	...
	if (dp_panel->edid_ctrl && dp_panel->edid_ctrl->edid) {
		dp_cust_set_param(DP_PARAM_EDID, dp_panel->edid_ctrl->edid,
			DP_EDID_BLOCK_LEN(dp_panel->edid_ctrl->edid));
	}
		...
		u32 mode_edid_bpp, u32 mode_pclk_khz)
{
	...
	bpp = min_t(u32, bpp, dp_cust_get_supported_bpp());
	...
	struct drm_connector *connector, struct dp_display_mode *mode)
{
	...
	memcpy(&mode->timing, &fail_safe,
		sizeof(fail_safe));
	dp_cust_set_param(DP_PARAM_SAFE_MODE, NULL, 0);
	return 1;
}
...
static void dp_panel_resolution_info(struct dp_panel_private *panel)
{
	...
	dp_cust_set_resolution_fps(pinfo->h_active, pinfo->v_active, pinfo->refresh_rate);
}
		...
		struct dp_display_mode *dp_mode)
{
	...
	if (dp_panel->dsc_en && dsc_cap) {
		...
		rc = sde_dsc_populate_dsc_config(&comp_info->dsc_info.config, 0, false);
		if (rc) {
