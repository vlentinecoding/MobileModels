...
static int dp_ctrl_update_sink_vx_px(struct dp_ctrl_private *ctrl)
{
	...
	dp_cust_get_vs_pe(&v_level, &p_level);
		...
		bool fec_mode, bool dsc_mode, bool shallow)
{
	...
	dp_cust_set_init_lanes_rate(ctrl->initial_lane_count, ctrl->initial_bw_code);

	rc = dp_ctrl_link_setup(ctrl, shallow);
	if (!rc)
		ctrl->power_on = true;

	dp_cust_set_link_lanes_rate(ctrl->link->link_params.lane_count,
		ctrl->link->link_params.bw_code);
	dp_cust_set_param(DP_PARAM_LINK_TRAIN_RETVAL, &rc, sizeof(rc));
end:
	return rc;
}
