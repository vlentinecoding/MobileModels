...
#ifdef CONFIG_LCD_QCOM_DRV
bool dsi_panel_parse_null_insertion_packet(struct dsi_panel *panel);
#endif

u8 dbgfs_tx_cmd_buf[SZ_4K];

#ifdef CONFIG_LCD_QCOM_DRV
static const uint32_t te_60_fps_us = 16667;
static const uint32_t te_90_fps_us = 11111;
static const uint32_t te_120_fps_us = 8334;
#endif
		...
		void *display, u32 bl_lvl)
{
	...
	if (panel->bl_config.type == DSI_BACKLIGHT_DCS)
		panel->async_cmd = true;
	rc = dsi_panel_set_backlight(panel, (u32)bl_temp);
	if (rc)
		DSI_ERR("unable to set backlight\n");
	if (panel->bl_config.type == DSI_BACKLIGHT_DCS)
		panel->async_cmd = false;
#else
	rc = dsi_panel_set_backlight(panel, (u32)bl_temp);
	if (rc)
		DSI_ERR("unable to set backlight\n");
...
#ifdef CONFIG_LCD_QCOM_DRV
	display->te_timestamp = ktime_get();
#endif
	return IRQ_HANDLED;
}
	...
	u32 *expect_val)
{
	...
	int8_t record_buf[DMD_RECORD_BUF_LEN * MAX_REG_READ_ESD_COUNT] = {'\0'};
	int32_t ret;
	int i = 0;
	uint8_t expect_value, judge_type;
	int start = 0;
	int len = DMD_RECORD_BUF_LEN * MAX_REG_READ_ESD_COUNT;

	ret = snprintf(record_buf, len,
		"lcd esd register status error:\n");
	if (ret < 0)
		DSI_ERR("snprintf happened error! return %d\n", ret);
	while (record_buf[start] != 0)
		++start;
	for (i = 0; i < count; i++) {
		judge_type = (expect_val[i] >> 8) & 0xFF;
		expect_value = expect_val[i] & 0xFF;
		if (judge_type == ESD_UNEQUAL)
			ret = snprintf(record_buf + start, len - start,
				"judge_type = UNEQUAL, ");
		else if (judge_type == ESD_EQUAL)
			ret = snprintf(record_buf + start, len - start,
				"judge_type = EQUAL, ");
		else if (judge_type == ESD_BIT_VALID)
			ret = snprintf(record_buf + start, len - start,
				"judge_type = ESD_BIT_VALID, ");
		if (ret < 0)
			DSI_ERR("snprintf happened error! return %d\n", ret);
		while (record_buf[start] != 0)
			++start;
...
static bool dsi_display_validate_reg_read(struct dsi_panel *panel)
{
	...
	for (j = 0; j < config->groups; ++j) {
		for (i = 0; i < len; ++i) {
			...
			if (lcd_judge_esd(judge_type, config->return_buf[i], expect_value) ==
				LCD_ESD_ERROR) {
				DSI_ERR("read_value[%d] = 0x%x, but expect_value = 0x%x!\n",
					i, config->return_buf[i], expect_value);
				rc = LCD_ESD_ERROR;
				continue;
			}
			DSI_INFO("return_buf[%d] = 0x%x, status_value[%d] = 0x%x",
				i, config->return_buf[i], group + i, expect_value);
		}
#else
	...
	#ifdef CONFIG_HUAWEI_DSM
	if (rc == LCD_ESD_ERROR)
		report_esd_check_err(len,
			config->return_buf, config->status_value);
	#endif
	return rc == LCD_ESD_OK;
#else
	return false;
#endif
}
...
static bool is_dsi_display_panel_read(struct dsi_cmd_desc *cmd)
{
	u8 type = cmd->msg.type;
...
static bool judge_cmd_transfer_result(int rc, struct dsi_cmd_desc *cmds)
{
	if (is_dsi_display_panel_read(cmds)) {
		if (rc <= 0) {
		...
		struct dsi_panel *panel)
{
	...
	for (i = 0; i < count; ++i) {
#ifdef CONFIG_LCD_QCOM_DRV
		if (is_dsi_display_panel_read(&cmds[i]))
...
#ifdef CONFIG_LCD_QCOM_DRV
		if (judge_cmd_transfer_result(rc, &cmds[i]))
			return rc;
#else
		if (rc <= 0) {
			DSI_ERR("rx cmd transfer failed rc=%d\n", rc);
		...
		int power_mode, void *disp)
{
	...
	case SDE_MODE_DPMS_LP1:
		if ((display->panel->prev_power_mode == SDE_MODE_DPMS_LP1)
			&& (display->panel->power_mode == SDE_MODE_DPMS_LP2)
			&& display->panel->skip_send_aod_enable) {
			break;
		} else {
	...
	#ifdef CONFIG_LCD_QCOM_DRV
		display->panel->enable_esd_check = true ;
	#endif
	default:
		return rc;
	}
	...
	if (display->panel->skip_send_aod_enable)
		display->panel->prev_power_mode = display->panel->power_mode;
...
static int dsi_display_res_init(struct dsi_display *display)
{
	...
	display_for_each_ctrl(i, display) {
		ctrl = &display->ctrl[i];
		ctrl->ctrl->hw.null_insertion_enabled = dsi_panel_parse_null_insertion_packet(display->panel);
		DSI_DEBUG("dsi_display_res_init_inner:null_insertion_enabled=%d\n", ctrl->ctrl->hw.null_insertion_enabled);
	}
#endif
	display_for_each_ctrl(i, display) {
		struct msm_dsi_phy *phy = display->ctrl[i].phy;
		...
		void *data)
{
	...
	if (!display->panel_node && !display->fw)
		return 0;
#ifdef CONFIG_LCD_QCOM_DRV
	if (!display->fw && display->panel)
		display->name = display->panel->name;
#else
	if (!display->fw)
...
static void dsi_display_handle_fifo_underflow(struct work_struct *work)
{
	...
	(void)lcd_dsm_client_record(lcd_dclient, "dsi fifo underflow error\n",
...
static void dsi_display_handle_fifo_overflow(struct work_struct *work)
{
	...
	mutex_unlock(&display->display_lock);
#if defined CONFIG_HUAWEI_DSM
	(void)lcd_dsm_client_record(lcd_dclient, "dsi fifo overflow error\n",
		DSM_LCD_LDI_UNDERFLOW_NO);
#endif
}
...
#ifdef CONFIG_LCD_QCOM_DRV
static int dsi_display_parse_hbm_delay_thr(struct dsi_display *display)
{
	struct dsi_parser_utils *utils = &display->panel->utils;
	int rc_left = 0;
	int rc_right = 0;

	rc_left = utils->read_u32(utils->data, "qcom,mdss-dsi-hbm-left-delaythr",
		&display->left_thr);
	if (rc_left) {
		DSI_ERR("cannot read the hbm_left_thr\n");
		return rc_left;
	}

	rc_right = utils->read_u32(utils->data, "qcom,mdss-dsi-hbm-right-delaythr",
		&display->right_thr);
	if (rc_right) {
		DSI_ERR("cannot read the hbm_right_thr\n");
		return rc_right;
	}
...
static uint64_t change_te_to_us(struct dsi_display *display)
{
	if (!display)
		return te_60_fps_us;
	if (display->panel->cur_mode->timing.refresh_rate == FPS_120HZ)
		return te_120_fps_us;
	else if (display->panel->cur_mode->timing.refresh_rate == FPS_90HZ)
		return te_90_fps_us;
	else
		return te_60_fps_us;
}

static uint64_t time_convert_with_fps(uint32_t te_interval,
	uint64_t time_60_fps)
{
	return time_60_fps * te_interval / te_60_fps_us;
}

static uint64_t get_hbm_sync_delay_time_us(struct dsi_display *display) {
	...
	te_interval = change_te_to_us(display);
	dsi_display_parse_hbm_delay_thr(display);
	if (te_interval == te_60_fps_us) {
		left_thres_us = time_convert_with_fps(te_interval, display->left_thr);
		right_thres_us = time_convert_with_fps(te_interval, display->right_thr);
	} else {
		left_thres_us = 2*time_convert_with_fps(te_interval, display->left_thr);
		right_thres_us = 2*time_convert_with_fps(te_interval, display->right_thr);
	}

	diff_time = ktime_to_us(ktime_get()) - ktime_to_us(display->te_timestamp);
	if (diff_time < left_thres_us)
		delayus = left_thres_us - diff_time;
	else if (diff_time > right_thres_us)
		delayus = te_interval - diff_time + left_thres_us;
	DSI_INFO("current_time = %lu, diff_time = %lu us",
		current_time, diff_time);
	DSI_INFO("delay = %lu us, left_threshold = %lu us",
		delayus, left_thres_us);
	DSI_INFO("right_th = %lu us, te_interval = %lu",
		right_thres_us, te_interval);

	return delayus;
}

void enable_te_irq(struct dsi_display *display)
{
	dsi_display_change_te_irq_status(display, true);
}
void disable_te_irq(struct dsi_display *display)
{
	dsi_display_change_te_irq_status(display, false);
}

static int dsi_display_set_fp_hbm(struct drm_connector *connector,
	struct dsi_display *display)
{
	int rc = 0;
	int hbm_mode;
	...
	uint64_t mask_delay_us = 0;
	static int hbm_state = HBM_MODE_OFF;
	int te_irq_status = display->is_te_irq_enabled;
	struct dsi_parser_utils *utils = &display->panel->utils;

	if (!display->panel || !connector) {
		DSI_ERR("null pointer\n");
		return -EINVAL;
	}
	hbm_mode = sde_connector_get_property(connector->state, CONNECTOR_PROP_HBM);
	if (hbm_mode == hbm_state)
		return rc;
	if (display->panel->power_mode == SDE_MODE_DPMS_LP1 ||
			display->panel->power_mode == SDE_MODE_DPMS_LP2) {
		DSI_INFO("current mode is aod and return\n");
		return rc;
	}
	if (te_irq_status == 0)
		DSI_ERR("Teirq enable failed\n");
	DSI_INFO("hbm_mode = %d\n", hbm_mode);
	mutex_lock(&display->panel->panel_lock);
	sync_time_us = get_hbm_sync_delay_time_us(display);
	DSI_INFO("SyncDelayTime = %ld us",sync_time_us);
	rc_mask_delay = utils->read_u32(utils->data, "qcom,mdss-dsi-masklayer-delay-time",
		&display->mask_delay);
	if (!rc_mask_delay) {
		mask_delay_us = display->mask_delay * 1000000 / display->panel->cur_mode->timing.refresh_rate;
	}

	if (display->panel->pdata && display->panel->pdata->panel_hbm_fp_set) {
		if(sync_time_us > 0)
			usleep_range(sync_time_us, sync_time_us);
		display->panel->pdata->panel_hbm_fp_set(display->panel, hbm_mode);
		usleep_range(mask_delay_us, mask_delay_us);
	}
...
#ifdef CONFIG_LCD_QCOM_DRV
	/* fp hbm enable */
	rc = dsi_display_set_fp_hbm(connector, display);
	if (rc)
		DSI_ERR("set fp hbm failed\n");
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
		if (display->panel && display->panel->pdata &&
			display->panel->pdata->boot_set)
			display->panel->pdata->boot_set(display->panel);
#endif
		return 0;
	}
...
#ifdef CONFIG_LCD_QCOM_DRV
static void dsi_display_dump_rx_data(struct dsi_cmd_desc *cmds)
{
	int i = 0;
	char *buf = (char *)cmds->msg.rx_buf;

	DSI_DEBUG("cmd->dtype = 0x%x\n", cmds->msg.type);
	DSI_DEBUG("cmd->channel = 0x%x\n", cmds->msg.channel);
	DSI_DEBUG("cmd->wait_ms = 0x%x\n", cmds->msg.wait_ms);
	DSI_DEBUG("cmd->rx_len = 0x%x\n", cmds->msg.rx_len);
	for (i = 0; i < cmds->msg.rx_len; i++)
		DSI_DEBUG("cmd->rx_buf[%d] = 0x%x\n", i, buf[i]);
}

#define MAX_RX_BUF_LEN 21

static int dsi_display_panel_cmd_prepare(struct dsi_cmd_desc *cmd, int state)
{
	int rc = 0;
	...
	if (!is_dsi_display_panel_read(cmd))
		return rc;
	cmd->msg.rx_len = cmd->msg.tx_len;
	if (cmd->msg.rx_len > MAX_RX_BUF_LEN) {
		DSI_INFO("[DSI] rx len more than MAX_RX_BUF_LEN\n");
		cmd->msg.rx_len = MAX_RX_BUF_LEN;
	}
	cmd->msg.rx_buf =
		kzalloc(sizeof(u8) * MAX_RX_BUF_LEN, GFP_KERNEL);
	if (!cmd->msg.rx_buf) {
		DSI_ERR("[DSI] rx buf kzalloc error\n");
		rc = -ENOMEM;
	}
	return rc;
}

int dsi_display_panel_rx(struct dsi_panel *panel,
		struct dsi_panel_cmd_set *cmd_set)
{
	...
	if (!panel || !panel->host || !cmd_set) {
		DSI_ERR("pointer is NULL\n");
		return -EINVAL;
	}

	DSI_DEBUG("dsi_display_panel_rx +\n");
	...
	mutex_lock(&display->display_lock);
	for (i = 0; i < count; i++) {
		rc = dsi_display_panel_cmd_prepare(&cmd_set->cmds[i], cmd_set->state);
		if (rc < 0)
			goto display_rx_done;

		if (is_dsi_display_panel_read(&cmd_set->cmds[i])) {
			...
			/* Avoid sending DCS commands when ESD recovery is pending */
			if (atomic_read(&display->panel->esd_recovery_pending)) {
				DSI_INFO("ESD recovery pending\n");
				rc = -EPERM;
				goto display_rx_done;
			}

			if (dsi_display_cmd_rx(display, &cmd_set->cmds[i]) <= 0) {
				DSI_ERR("[DSI] Display command receive %d failed, rc=%d\n", i, rc);
				rc = -EPERM;
				goto display_rx_done;
			}
			dsi_display_dump_rx_data(&cmd_set->cmds[i]);
		} else {
			mutex_lock(&panel->panel_lock);
			if (!display->panel->panel_initialized) {
				DSI_WARN("panel not initialized\n");
				goto display_tx_cmd_err;
			}
			rc = dsi_host_transfer(panel->host, &cmd_set->cmds[i].msg);
			if (rc < 0) {
				DSI_ERR("failed to set cmds, rc=%d\n", rc);
				goto display_tx_cmd_err;
			}
			mutex_unlock(&panel->panel_lock);
			if (cmd_set->cmds[i].post_wait_ms)
				usleep_range(cmd_set->cmds[i].post_wait_ms * 1000,
						((cmd_set->cmds[i].post_wait_ms * 1000) + 10));
		}
	}
	goto display_rx_done;
