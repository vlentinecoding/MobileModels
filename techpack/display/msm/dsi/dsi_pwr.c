				     ...
				     struct dsi_regulator_info *regs)
{
	...
	dsi_for_each_child_node(root, node) {
		...
		if (!strcmp(regs->vregs[i].vreg_name, "gpio")) {
			regs->vregs[i].gpio_num = utils->get_named_gpio(node,
				"qcom,supply-gpio-num", 0);
			if (!gpio_is_valid(regs->vregs[i].gpio_num)) {
				DSI_ERR("failed to read supply gpio num, rc = %d\n", rc);
				goto error;
			}
		}
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
static int dsi_pwr_enable_gpio(struct dsi_vreg *vreg, bool enable)
{
	int rc = 0;

	if (enable) {
		if (gpio_is_valid(vreg->gpio_num)) {
			rc = gpio_request(vreg->gpio_num, "supply_gpio");
			if (rc) {
				DSI_ERR("request for supply gpio failed, rc=%d\n", rc);
				goto en_error;
			}
			rc = gpio_direction_output(vreg->gpio_num, 1);
			if (rc) {
				DSI_ERR("unable to set dir for supply gpio rc=%d\n", rc);
				goto en_error;
			}
			gpio_set_value(vreg->gpio_num, 1);
		}
en_error:
		...
		if (gpio_is_valid(vreg->gpio_num)) {
			gpio_set_value(vreg->gpio_num, 0);
			gpio_free(vreg->gpio_num);
		}
	...
	if (enable) {
...
#ifdef CONFIG_LCD_QCOM_DRV
			if (!strcmp(vreg->vreg_name, "gpio")) {
				rc = dsi_pwr_enable_gpio(vreg, enable);
				if (rc)
					DSI_ERR("enable gpio%d fail, rc=%d\n",
						vreg->gpio_num, rc);
				continue;
			}
#endif
		...
		for (i = (regs->count - 1); i >= 0; i--) {
			...
			if (!strcmp(vreg->vreg_name, "gpio")) {
				rc = dsi_pwr_enable_gpio(vreg, enable);
				if (rc)
					DSI_ERR("disable gpio%d fail, rc=%d\n",
						vreg->gpio_num, rc);
				continue;
			}
#endif
