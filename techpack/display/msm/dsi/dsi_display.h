 ...
 * @te_timestamp:     Record the timestamp of TE signal.
 * @left_thr,right_thr:Define the hbm delay threshold.
 */
struct dsi_display {
...
#ifdef CONFIG_LCD_QCOM_DRV
/**
 * dsi_display_panel_rx() - recevice cmd data from panel
 * @panel:         Handle to panel
 * @cmd_set:       command data
 */
int dsi_display_panel_rx(struct dsi_panel *panel, struct dsi_panel_cmd_set *cmd_set);
extern void enable_te_irq (struct dsi_display *display);
extern void disable_te_irq (struct dsi_display *display);
#endif
