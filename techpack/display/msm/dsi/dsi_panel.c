...
#ifdef CONFIG_LCD_QCOM_DRV
		if (!strcmp(panel->power_info.vregs[i].vreg_name, "gpio")) {
			DSI_INFO("get power supply by gpio, by pass!\n");
			continue;
		}
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
		if (!strcmp(panel->power_info.vregs[i].vreg_name, "gpio")) {
			DSI_INFO("put power supply by gpio, by pass!\n");
			continue;
		}
#endif
...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	/*
	 * pt station test, if pt_reset_enable enabled,
	 * reset will power down and power up when panel sleep off and on
	 */
	if (panel->pdata->pinfo->fact_info->pt_flag != 0 &&
		!panel->pdata->pinfo->fact_info->pt_reset_enable) {
		DSI_INFO("pt station test\n");
		return rc;
	}
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
static int dsi_panel_reset_custom(struct dsi_panel *panel)
{
	int rc = 0;
...
#if defined(CONFIG_LCD_QCOM_DRV) && defined(CONFIG_LCD_FACTORY)
	if (!panel->pdata || !panel->pdata->pinfo ||
		!panel->pdata->pinfo->fact_info) {
		DSI_ERR("[%s] pointer is null\n", panel->name);
		return rc;
	}
	/* pt station test */
	if (panel->pdata->pinfo->fact_info->pt_flag == 0) {
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->reset_config.reset_pre_off_sleep)
		usleep_range((panel->reset_config.reset_pre_off_sleep * 1000),
			(panel->reset_config.reset_pre_off_sleep * 1000) + 10);
#endif
	 ...
	 */
	if ((panel->pdata->pinfo->fact_info->pt_flag == 0) ||
		((panel->pdata->pinfo->fact_info->pt_flag != 0) &&
		panel->pdata->pinfo->fact_info->pt_reset_enable)) {
				...
				enum dsi_cmd_set_type type)
{
...
#if defined(CONFIG_LCD_QCOM_DRV) && defined(LCD_DEBUG_ENABLE)
	rc = dbg_set_cmd(panel, type);
	if (rc) {
		DSI_ERR("set debug cmd failed\n");
		return rc;
	}
#endif
	mode = panel->cur_mode;
	...
	DSI_INFO("type = %d, count = %d, state = %d\n", type, count, state);
	...
	u32 bl_lvl)
{
	...
	if ((panel->skip_normal_bl_set_on_aod) && (panel->power_mode == SDE_MODE_DPMS_LP1))
		return rc;
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->pinfo) {
		if (panel->pdata->pinfo->hbm.enabled) {
			if (panel->pdata->pinfo->hbm.mode == HBM_ENTER &&
				bl->type == DSI_BACKLIGHT_DCS) {
				DSI_INFO("current mode is hbm\n");
				return rc;
			}
		}
		if (panel->aod_lk_bl == AOD_LK_BL &&
			bl->type == DSI_BACKLIGHT_DCS) {
			DSI_INFO("current mode is aod_on\n");
			return rc;
		}
		if (panel->pdata->print_bkl) {
			panel->pdata->print_bkl(panel, bl_lvl);
		}
	}
#endif
...
{
	struct dsi_parser_utils *utils = &panel->utils;
	bool display_null_insertion_packet = false;
	display_null_insertion_packet = utils->read_bool(utils->data,
			"qcom,null-insertion-enabled");
	DSI_ERR("[dsi_panel_parse_null_insertion_packet]:value = %d",
			display_null_insertion_packet);
	return display_null_insertion_packet;
}
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	"qcom,mdss-dsi-hbm-enable-commands",
	"qcom,mdss-dsi-hbm-disable-commands",
	"qcom,mdss-dsi-hbm-dimming-on-commands",
	"qcom,mdss-dsi-hbm-dimming-off-commands",
	"qcom,mdss-dsi-fp-hbm-enable-commands",
	"qcom,mdss-dsi-fp-hbm-disable-commands",
	"qcom,mdss-dsi-panel-lcd-check-reg-commands",
	"qcom,mdss-dsi-alpm-low-light-commands",
	"qcom,mdss-dsi-alpm-middle-light-commands",
	"qcom,mdss-dsi-alpm-high-light-commands",
	"qcom,mdss-dsi-lhbm-enable-commands",
	"qcom,mdss-dsi-lhbm-disable-commands",
	"qcom,mdss-dsi-oeminfo-read-commands",
	"qcom,mdss-dsi-batchinfo-read-commands",
	"qcom,mdss-dsi-panel-lcd-esd-check-commands",
#ifdef CONFIG_LCD_FACTORY
	"qcom,mdss-dsi-lcd-pcd-check-open-commands",
	"qcom,mdss-dsi-lcd-pcd-check-reg-commands",
	"qcom,mdss-dsi-lcd-pcd-check-close-commands",
	"qcom,mdss-dsi-vtc-line-test-commands",
	"qcom,mdss-dsi-hor-line-test-commands",
#endif
#endif
};
...
#ifdef CONFIG_LCD_QCOM_DRV
	"qcom,mdss-dsi-hbm-enable-commands-state",
	"qcom,mdss-dsi-hbm-disable-commands-state",
	"qcom,mdss-dsi-hbm-dimming-on-commands-state",
	"qcom,mdss-dsi-hbm-dimming-off-commands-state",
	"qcom,mdss-dsi-fp-hbm-enable-commands-state",
	"qcom,mdss-dsi-fp-hbm-disable-commands-state",
	"qcom,mdss-dsi-panel-lcd-check-reg-commands-state",
	"qcom,mdss-dsi-alpm-low-light-commands-state",
	"qcom,mdss-dsi-alpm-middle-light-commands-state",
	"qcom,mdss-dsi-alpm-high-light-commands-state",
	"qcom,mdss-dsi-lhbm-enable-commands-state",
	"qcom,mdss-dsi-lhbm-disable-commands-state",
	"qcom,mdss-dsi-oeminfo-read-commands-state",
	"qcom,mdss-dsi-batchinfo-read-commands-state",
	"qcom,mdss-dsi-panel-lcd-esd-check-commands-state",
#ifdef CONFIG_LCD_FACTORY
	"qcom,mdss-dsi-lcd-pcd-check-open-commands-state",
	"qcom,mdss-dsi-lcd-pcd-check-reg-commands-state",
	"qcom,mdss-dsi-lcd-pcd-check-close-commands-state",
	"qcom,mdss-dsi-vtc-line-test-commands-state",
	"qcom,mdss-dsi-hor-line-test-commands-state",
#endif
#endif
};
...
#ifdef CONFIG_LCD_QCOM_DRV
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-reset-pre-off-sleep",
		&panel->reset_config.reset_pre_off_sleep);
	if (rc)
		DSI_DEBUG("not support reset-pre-off-sleep\n");
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	panel->reset_in_lp11 = utils->read_bool(utils->data,
			"qcom,mdss-dsi-panel-reset-in-lp11");
#endif
				...
				struct dsi_parser_utils *utils)
{
	...
	change_pps = utils->read_bool(utils->data,
			"qcom,change-pps-rc-range-params");
	rc = sde_dsc_populate_dsc_config(&priv_info->dsc.config,
			priv_info->dsc.scr_rev, change_pps);
	if (rc) {
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	panel->skip_normal_bl_set_on_aod = utils->read_bool(utils->data,
		"qcom,skip-normal-bl-set-on-aod-enabled");

#if defined(CONFIG_HONOR_GKI_V1) || defined(CONFIG_LCD_FACTORY)
	esd_config->esd_enabled = 0;
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	panel->pdata = &g_panel_data;
	if (panel->pdata && panel->pdata->panel_init) {
		rc = panel->pdata->panel_init(panel);
		if (rc) {
			DSI_ERR("[%s] failed to init panel, rc=%d\n",
				panel->name, rc);
			goto error_vreg_put;
		}
	}
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->on)
		panel->pdata->on(panel, PANEL_INIT_NONE);
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	if(!panel->reset_in_lp11) {
		rc= dsi_panel_reset_custom(panel);
		if (rc)
			goto error;
	}
#endif
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->on)
		panel->pdata->on(panel, PANEL_INIT_POWER_ON);
#endif
error:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->on)
		panel->pdata->on(panel, PANEL_INIT_MIPI_LP_SEND_SEQUENCE);
#endif
error:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
int dsi_panel_switch(struct dsi_panel *panel)
{
	...
	if (panel->power_mode == SDE_MODE_DPMS_LP1 ||
		panel->power_mode == SDE_MODE_DPMS_LP2) {
		DSI_INFO("lp1 or lp2 not allowed to switch fps\n");
		mutex_unlock(&panel->panel_lock);
		return rc;
	}
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->on)
		panel->pdata->on(panel, PANEL_INIT_MIPI_HS_SEND_SEQUENCE);
#endif
error:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
int dsi_panel_disable(struct dsi_panel *panel)
{
	...
	mutex_lock(&panel->panel_lock);
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->off)
		panel->pdata->off(panel, PANEL_UNINIT_NONE);
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->off)
		panel->pdata->off(panel, PANEL_UNINIT_MIPI_HS_SEND_SEQUENCE);
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->off)
		panel->pdata->off(panel, PANEL_UNINIT_MIPI_LP_SEND_SEQUENCE);
#endif
...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->pdata && panel->pdata->off)
		panel->pdata->off(panel, PANEL_UNINIT_POWER_OFF);
#endif
error:
	mutex_unlock(&panel->panel_lock);
	return rc;
...
#ifdef CONFIG_LCD_QCOM_DRV
int dsi_panel_receive_data(struct dsi_panel *panel, int type, u8 *recv_buf, u32 recv_buf_len)
{
	int rc = 0;
	int i = 0;
	int j = 0;
	...
	struct dsi_panel_cmd_set cmd_set = {0};

	if (!panel || !panel->cur_mode || !recv_buf)
		return -EINVAL;

	mode = panel->cur_mode;
	if (!mode || !mode->priv_info) {
		DSI_ERR("[%s] display mode is not set\n", panel->name);
		return -EINVAL;
	}

	DSI_DEBUG("type = %d, command = %s\n", type, cmd_set_prop_map[type]);
	cmd_set = mode->priv_info->cmd_sets[type];
	count = cmd_set.count;

	if (count == 0) {
		DSI_ERR("[%s] No command name %s\n",
				panel->name, cmd_set_prop_map[type]);
		return -EINVAL;
	}

	rc = dsi_display_panel_rx(panel, &cmd_set);
	if (rc < 0) {
		DSI_ERR("dsi_display_panel_rx fail\n");
		return rc;
	}
	memset(recv_buf, 0, sizeof(u8) * recv_buf_len);
	for (i = 0; i < count; i++) {
		j = 0;
		if (!cmd_set.cmds[i].msg.rx_len)
			continue;
		if (pos + cmd_set.cmds[i].msg.rx_len > recv_buf_len) {
			DSI_ERR("recv_buf_len is less\n");
			return -ENOMEM;
		}
		do{
			recv_buf[pos++] = ((u8 *)cmd_set.cmds[i].msg.rx_buf)[j++];
		} while (j < cmd_set.cmds[i].msg.rx_len);
	}
	return 0;
}

int dsi_panel_set_cmd(struct dsi_panel *panel, int type)
{
	int rc = 0;
	...
	if (rc)
		DSI_ERR("[%s] failed to send type:%d cmd, rc=%d\n",
			   panel->name, type, rc);
	return rc;
}

int dsi_panel_set_hbm_level(struct dsi_panel *panel, u32 bl_lvl)
{
	int rc = 0;

	rc = dsi_panel_update_backlight(panel, bl_lvl);
	if (rc)
		DSI_ERR("[%s] failed to update backlight, rc=%d\n", panel->name, rc);
	return rc;
}
#endif
