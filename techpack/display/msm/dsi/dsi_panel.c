...
#ifdef CONFIG_LCD_QCOM_DRV
	if (panel->reset_config.reset_after_off_sleep)
		usleep_range((panel->reset_config.reset_after_off_sleep * 1000),
			(panel->reset_config.reset_after_off_sleep * 1000) + 10);
#endif
...
static int dsi_panel_parse_reset_sequence(struct dsi_panel *panel)
{
	...
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-reset-after-off-sleep",
		&panel->reset_config.reset_after_off_sleep);
	if (rc)
		DSI_DEBUG("not support reset-after-off-sleep\n");
#endif
...
static int dsi_panel_parse_esd_config(struct dsi_panel *panel)
{
	...
	panel->skip_send_aod_enable = utils->read_bool(utils->data,
		"qcom,mdss-dsi-skip-send-aod-enable");
