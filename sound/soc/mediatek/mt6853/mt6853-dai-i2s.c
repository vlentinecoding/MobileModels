...
enum {
	FAC_I2S_MUX_DISCONNECT = 0,
	FAC_I2S_MUX_CONNECT,
	FAC_I2S_MUX_MASK = 0x3,
};

static const char * const fac_i2s_mux_map[] = {
	"Disconnect", "Connect"
};

static int fac_i2s_mux_map_value[] = {
	FAC_I2S_MUX_DISCONNECT,
	FAC_I2S_MUX_CONNECT,
};

static SOC_VALUE_ENUM_SINGLE_DECL(fac_i2s_mux_map_enum,
				SND_SOC_NOPM,
				0,
				FAC_I2S_MUX_MASK,
				fac_i2s_mux_map,
				fac_i2s_mux_map_value);

static const struct snd_kcontrol_new fac_i2s3_mux_control =
	SOC_DAPM_ENUM("FAC_I2S3 Select", fac_i2s_mux_map_enum);

enum {
	FAC_I2S_LINK_DISCONNECT = 0,
	FAC_I2S_LINK_CONNECT,
	FAC_I2S_LINK_MASK = 0x3,
};

static const char * const fac_i2s_link_mux_map[] = {
	"Disconnect", "Connect",
};

static int fac_i2s_link_mux_map_value[] = {
	FAC_I2S_LINK_DISCONNECT,
	FAC_I2S_LINK_CONNECT,
};

static SOC_VALUE_ENUM_SINGLE_DECL(fac_i2s_link_mux_map_enum,
				SND_SOC_NOPM,
				0,
				FAC_I2S_LINK_MASK,
				fac_i2s_link_mux_map,
				fac_i2s_link_mux_map_value);

static const struct snd_kcontrol_new fac_i2s3_link_mux_control =
		SOC_DAPM_ENUM("FAC_I2S3 LINK", fac_i2s_link_mux_map_enum);

static const struct snd_kcontrol_new fac_i2s3_mix[] = {
		SOC_DAPM_SINGLE("FAC_I2S3_Switch", SND_SOC_NOPM, 0, 1, 0),
	};
...
static int fac_i2s3_config_power_event(struct snd_soc_dapm_widget *w,
				struct snd_kcontrol *kcontrol, int event)
{
	unsigned int rate_reg;
	int i2s_id = MT6853_DAI_I2S_3;
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		rate_reg = mt6853_rate_transform(afe->dev, rate, i2s_id);
...
static const struct snd_soc_dapm_widget mtk_dai_i2s_widgets[] = {
	...
	SND_SOC_DAPM_INPUT("FAC_I2S_IN"),

	SND_SOC_DAPM_MUX_E("FAC_I2S3_Select", SND_SOC_NOPM, 0, 0,
			&fac_i2s3_mux_control,
			fac_i2s3_config_power_event,
			SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_PRE_PMD),

	SND_SOC_DAPM_MUX("FAC_I2S3_Link", SND_SOC_NOPM,
			0, 0, &fac_i2s3_link_mux_control),

	SND_SOC_DAPM_MIXER("FAC_I2S3_MIXER", SND_SOC_NOPM, 0, 0,
			fac_i2s3_mix,
			ARRAY_SIZE(fac_i2s3_mix)),
	...
	SND_SOC_DAPM_OUTPUT("FAC_I2S_DUMMY_OUT"),
...
static const struct snd_soc_dapm_route mtk_dai_i2s_routes[] = {
	...
	/* i2s3 clk enbale for fac test */
	{"FAC_I2S3_Select", "Connect", "FAC_I2S_IN"},
	{"FAC_I2S3_MIXER", "FAC_I2S3_Switch", "FAC_I2S3_Select"},
	{"I2S3", NULL, "FAC_I2S3_MIXER"},
	{"FAC_I2S3_Link", "Connect", "I2S3"},
	{"FAC_I2S_DUMMY_OUT", NULL, "FAC_I2S3_Link"},
