...
#define SIMPLE_PA_DEFAULT_MODE 1
...
#define MAX_SMARTPA_NUM 5
#define MAX_DAI_NAME_LENGTH 64
#define MI2S_NAME_LENGTH 32

struct dai_link_name {
	char comp_name[MAX_DAI_NAME_LENGTH];
	char dai_name[MAX_DAI_NAME_LENGTH];
};

struct hw_smartpa_info {
	char smartpa_type[SMARTPA_TYPE_BUFF_SIZE];
	char i2s_rx_name[SMARTPA_TYPE_BUFF_SIZE];
	char i2s_tx_name[SMARTPA_TYPE_BUFF_SIZE];
	int smartpa_num;
	bool is_init_smartpa_type;
	bool is_load_firmware_by_product_name;

	struct dai_link_name smartpa_names[MAX_SMARTPA_NUM];
	struct snd_soc_dai_link_component smartpa_codecs[MAX_SMARTPA_NUM];
	unsigned int smartpa_codecs_count;
	struct mutex pa_probe_lock;
};
...
struct hw_audio_info {
	...
	struct hw_smartpa_info smartpa_info;
	bool is_extpa_to_hphr;
};
