	...
	unsigned char reg_addr, unsigned int mask, unsigned int reg_data)
{
	...
	int ret;
#ifdef CONFIG_HUAWEI_DSM_AUDIO
	char report[DMD_INFO_LEN] = {0};
#endif

	ret = aw882xx_i2c_write_bits(aw882xx, reg_addr, mask, reg_data);
#ifdef CONFIG_HUAWEI_DSM_AUDIO
	if (ret < 0) {
		snprintf(report, sizeof(report), "Write:i2c 0x%x", aw882xx->i2c->addr);
		audio_dsm_report_info(AUDIO_SMARTPA, DSM_SMARTPA_I2C_ERR, "%s", report);
		aw_dev_err(aw882xx->dev, "%s: dsm report, %s", __func__, report);
	}
#endif
	return ret;
}
...
static void aw_pid_2032_update_rate(struct aw_device *aw_dev,
			unsigned int rate, int width)
{
	uint32_t cco_mux_value;
	int reg_value = 0;
