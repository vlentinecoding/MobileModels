...
static const char *cali_str[CALI_STR_MAX] = {"none", "start_cali", "cali_re",
	...
	"show_te", "show_st", "dev_sel", "get_ver", "bypass", "enable", "get_re", "get_f0", "show_dev_num", "cali_en"
};
...
static int aw_cali_get_read_cali_re(int32_t *cali_re, int channel)
{
 ...
 void aw_get_re(struct aw_device *aw_dev)
 { 
	 aw_dev_info(aw_dev->dev, "%s:channel %d\n",
			 __func__, aw_dev->channel);
 
	 if (is_single_cali) {
		 aw_dev_info(aw_dev->dev, "%s:re value from oeminfo %d\n",
				 __func__, aw_dev->cali_desc.cali_re);
	 } else {
	 	aw_dev_info(aw_dev->dev, "%s:re[%s] value from oeminfo %d\n",
				 __func__, ch_name[aw_dev->channel], aw_dev->cali_desc.cali_re);
	 }
...
int aw_cali_write_re_to_nvram(int32_t cali_re, int32_t channel)
{
		...
		if (channel >= AW_DEV_CH_MAX) {
			pr_err("%s: unsupported channel [%xd] \n", __func__, channel);
			return -EINVAL;
		}
...
int aw_cali_store_cali_re(struct aw_device *aw_dev, int32_t re)
{
	...
	if ((re > AW_CALI_RE_MIN) && (re < AW_CALI_RE_MAX)) {
...
static int aw_cali_svc_cali_en(struct aw_device *aw_dev, unsigned int flag, int is_enable)
{
...
static int aw_cali_svc_cali_en_str(struct aw_device *aw_dev, unsigned int flag, const char *en_str)
{
    int is_enable;
    int ret;

    ret = sscanf(en_str, "is_enable:%d", &is_enable);
    if (ret <= 0) {
        aw_dev_err(aw_dev->dev, "en_str:%s error", en_str);
        return -EINVAL;
    }

    return aw_cali_svc_cali_en(aw_dev, flag, is_enable);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (is_single_cali) {
		...
		aw_dev->cali_desc.cali_re = data;
		aw_dev_info(aw_dev->dev, "set re [%d]", data);
	} else {
...
static ssize_t aw_cali_misc_read(struct file *filp, char __user *buf, size_t size, loff_t *pos)
{
	...
	aw_dev_info(aw_dev->dev, "enter g_msic_wr_flag: %x",g_msic_wr_flag);
	...
	case CALI_STR_SHOW_DEV_NUM: {
		if(aw_dev->ops.aw_get_dev_num) {
			dev_num = aw_dev->ops.aw_get_dev_num();
			len += snprintf(local_buf+len, sizeof(local_buf) - len,
					"dev_num:%d\n", dev_num);
		} else {
			aw_dev_err(aw_dev->dev, "get dev_num failed");
			return -EINVAL;
		}
	} break;
	default: {
...
static ssize_t aw_cali_misc_write(struct file *filp, const char __user *buf, size_t size, loff_t *pos)
{
	...
	if (buf == NULL || size == 0) {
		aw_dev_err(aw_dev->dev, "param error!");
		return -EINVAL;
	}
	...
	case CALI_STR_CALI_RE_F0: {
			...
			if(ret < 0) {
				...
				g_msic_wr_flag = CALI_STR_CALI_RE_F0;
			}
    ...
    case CALI_STR_CALI_EN: {
            ret = aw_cali_svc_cali_en_str(aw_dev, CALI_OPS_HMUTE,
                        kernel_buf + strlen(cali_str[CALI_STR_CALI_EN]) + 1);
    } break;
	...
	case CALI_STR_BYPASS: {
		enable = 0;
		aw_dev_info(aw_dev->dev, "Bypass Enter\n");

		ret = aw_dev_set_afe_module_en(AW_RX_MODULE, enable);
		if (ret)
			aw_dev_err(aw_dev->dev, "RX bypass failed, ret=%d", ret);
		ret = aw_dev_set_afe_module_en(AW_TX_MODULE, enable);
		if (ret)
			aw_dev_err(aw_dev->dev, "TX bypass failed, ret=%d", ret);
		aw_dev_info(aw_dev->dev, "Bypass Finished\n");
	} break;
	case CALI_STR_ENABLE: {
		enable = 1;
		aw_dev_info(aw_dev->dev, "Enable Enter\n");

		ret = aw_dev_set_afe_module_en(AW_RX_MODULE, enable);
		if (ret)
			aw_dev_err(aw_dev->dev, "RX enable failed, ret=%d", ret);
		ret = aw_dev_set_afe_module_en(AW_TX_MODULE, enable);
		if (ret)
			aw_dev_err(aw_dev->dev, "TX enable failed, ret=%d", ret);
		aw_dev_info(aw_dev->dev, "Enable Finished\n");
	} break;
	case CALI_STR_GET_RE: {
		g_msic_wr_flag = CALI_STR_SHOW_RE;
	} break;
	case CALI_STR_GET_F0: {
		g_msic_wr_flag = CALI_STR_SHOW_F0;
	} break;
	default: {
