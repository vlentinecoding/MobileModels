		...
		case REALTIME_EXTRA:
			...
			if (msg->param.raw.params[0] == TI_SMARTPA_VENDOR_ID) {
				if (msg->param.raw.params[1] == RD_DATA) {
					memcpy(&rd_data_tmp.payload[0],
						&msg->param.raw.params[4],
						min(sizeof(tisa_data_t),
						sizeof(msg->param.raw)));
					rd_avail = true;
					dbg_abox_adaptation(
						"read_avail after parital read[%d]",
						rd_avail);
					/*If rd_avail available*/
					if (rd_avail && waitqueue_active(&wq_read))
						wake_up_interruptible(&wq_read);
				} else if (msg->param.raw.params[1] == WR_DATA) {
					memcpy(&wr_data_tmp.payload[0],
						&msg->param.raw.params[4],
						min(sizeof(tisa_data_t),
						sizeof(msg->param.raw)));
					wr_avail = true;
					dbg_abox_adaptation(
						"write_avail after parital read[%d]",
						wr_avail);
					/*If wr_avail available*/
					if (wr_avail && waitqueue_active(&wq_write))
						wake_up_interruptible(&wq_write);
				} else {
					pr_err(
						"[TI-SmartPA] %s: Invalid callback, %d",
						__func__,
