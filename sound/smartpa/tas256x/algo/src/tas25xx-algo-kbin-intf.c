	...
	int channel_count)
{
	...
	if ((s_rdc_fix[0] == POISON_VAL) &&
		...
		if (dev)
			ret = request_firmware(&fw_entry, "smartamp_calib.bin",
				dev);
		...
		if (fw_entry->size < (calib_re_sz + 1)) {
			...
			memcpy(&s_tv_fix, calib_data + calib_re_sz,
				sizeof(uint32_t));
		}
...
static void query_tisa_algo(struct work_struct *wrk)
{
	...
	while (atomic_read(&algo_active)) {
		...
		pr_info("[TI-SmartPA:%s] Re value is %02d.%02d (%d), %02d.%02d (%d)\n",
			...
			(int32_t)trans_val_to_user_m(re_l, QFORMAT19), re_l,
			(int32_t)trans_val_to_user_i(re_r, QFORMAT19),
			(int32_t)trans_val_to_user_m(re_r, QFORMAT19), re_r);
...
static int tas25xx_send_kbin_params(void)
{
	...
	int per_ch_param_sz =
		bin_file_get_per_profile_per_channel_param_count();
	int number_of_ch = bin_file_get_number_of_channels();
	bool success = bin_file_get_profile_data(&profile_data,
				&profile_size_bytes);
	...
	if (success && profile_data && profile_size_bytes) {
		...
		for (i = 0; i < number_of_ch; i++) {
			...
			for (current_idx = 0; current_idx < per_ch_param_sz; ) {
				...
				param_id =
					TAS_CALC_PARAM_IDX(current_idx, size, ch);
				ret = tas25xx_smartamp_algo_ctrl((u8 *)data,
					param_id,
					TAS_SET_PARAM, size * sizeof(int),
					TISA_MOD_RX);
				if (ret)
	...
	if (ret == 0) {
		...
		if (ch == CHANNEL1) {
			param_id = TAS_CALC_PARAM_IDX(TAS_DSP_SWAP_IDX,
					1, CHANNEL1);
			ret = tas25xx_smartamp_algo_ctrl((u8 *)&user_data,
				param_id,
				TAS_SET_PARAM, sizeof(uint32_t), TISA_MOD_RX);
		}
	}
		...
		struct snd_ctl_elem_value *pUcontrol)
{
	...
	if (ret) {
		...
		if (number_of_ch == 2) {
			param_id = TAS_CALC_PARAM_IDX(TAS_SA_SET_RE,
					1, CHANNEL1);
			ret = tas25xx_smartamp_algo_ctrl(
