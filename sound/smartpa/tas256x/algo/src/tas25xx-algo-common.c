...
#if IS_ENABLED(CONFIG_TAS25XX_BAT_INFO_LIMITER)
#include "tas25xx-battery-limiter.h"
#endif
...
void tas25xx_algo_enable_common_controls(int value)
{
	...
	if (s_allow_algo_controls)
		tas25xx_start_battery_info_monitor();
	else
		tas25xx_stop_battery_info_monitor();
#endif /* CONFIG_TAS25XX_BAT_INFO_LIMITER */
}
...
#if IS_ENABLED(CONFIG_TISA_SYSFS_INTF)
	tas25xx_parse_algo_dt_sysfs(np);
#endif
}
...
#if IS_ENABLED(CONFIG_TAS25XX_BAT_INFO_LIMITER)
	tas_smartamp_add_battery_algo(codec, number_of_channels);
#endif
...
#if IS_ENABLED(CONFIG_TAS25XX_BAT_INFO_LIMITER)
	tas_smartamp_battery_algo_deinitalize();
#endif
