...
static ssize_t pause_zswapd_write(struct kernfs_open_file *of,
				char *buf, size_t nbytes, loff_t off)
{
	const unsigned int base = 10;
	unsigned int pause_zswapd_stat;
	int ret;

	buf = strstrip(buf);
	ret = kstrtouint(buf, base, &pause_zswapd_stat);
	if (ret)
		return -EINVAL;

	atomic_set(&pause_zswapd, pause_zswapd_stat);
...
static int pause_zswapd_show(struct seq_file *m, void *v)
{
	seq_printf(m, "pause_zswapd: %u\n",
...
static struct cftype memcg_policy_files[] = {
	...
	},
	{
		.name = "pause_zswapd",
		.flags = CFTYPE_ONLY_ON_ROOT,
		.write = pause_zswapd_write,
#ifdef CONFIG_HYPERHOLD_DEBUG
		.seq_show = pause_zswapd_show,
#endif
	},
	{
