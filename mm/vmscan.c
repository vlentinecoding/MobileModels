...
static DECLARE_RWSEM(shrinker_release_rwsem);
...
void unregister_shrinker(struct shrinker *shrinker)
{
	if (!shrinker->nr_deferred)
		return;
	down_write(&shrinker_release_rwsem);
	down_write(&shrinker_rwsem);
	list_del(&shrinker->list);
	up_write(&shrinker_rwsem);
	up_write(&shrinker_release_rwsem);
				 ...
				 unsigned long nr_eligible)
{
	...
	if (!down_read_trylock(&shrinker_release_rwsem)) {
		/*
	...
	if (!down_read_trylock(&shrinker_rwsem)) {
		up_read(&shrinker_release_rwsem);
		freed = 1;
		goto out;
	}
...
next:
		/*
		 * Bail out if someone want to unregister a shrinker to
		 * prevent the unregsitration from being stalled for long periods
		 * by parallel ongoing shrinking.
		 */
		if (rwsem_is_contended(&shrinker_release_rwsem) ||
				      ...
				      bool force_reclaim)
{
...
#ifdef CONFIG_HUAWEI_PAGECACHE_DEBUG
	...
	while (!list_empty(page_list)) {
		...
		if (PageDirty(page)) {
#ifdef CONFIG_HUAWEI_PAGECACHE_DEBUG
			if (unlikely(pagecache_dump &
					BIT_MM_SHRINK_INACTIVE_DUMP)) {
				if (file_map)
					pgcache_log_dentry(
						BIT_MM_SHRINK_INACTIVE_DUMP,
						cur_dentry,
						"shrink_page, page is dirty");
			}
#endif
			/*
...
keep:
	...
	if (unlikely(pagecache_dump & BIT_MM_SHRINK_INACTIVE_DUMP)) {
		if (tmp_dentry) {
			tmp_dentry->mapping_stat.shrink_page_times++;
			pgcache_log_dentry(BIT_MM_SHRINK_INACTIVE_DUMP,
					tmp_dentry,
					"shrink_page, free %d pages",
					freepages);
		}

		if (nul_dentry_pages)
			pgcache_log(BIT_MM_SHRINK_INACTIVE_DUMP,
			    "shrink_page, free %d pages which is null dentry",
			    nul_dentry_pages);
	}
#endif
	return nr_reclaimed;
		     ...
		     struct scan_control *sc, enum lru_list lru)
{
...
#ifdef CONFIG_MM_PAGE_TRACE
	etime = ktime_get();
	if (ktime_us_delta(etime, stime) > 2000000) /* 2000000 means 2s */
		pr_info("reclaim-%llu lru-%d t-%lld\n", nr_reclaimed,
			lru, ktime_us_delta(etime, stime));
#endif
	spin_lock_irq(&pgdat->lru_lock);
	...
	pgcache_log(BIT_MM_SHRINK_INACTIVE_DUMP,
			"shrink inactive list, nr_reclaimed, %ld",
			nr_reclaimed);
	return nr_reclaimed;
}
