/*
 * mm/mem_track/buddy_track.c	buddy alloc trace function
 *
...
#define page_exc(p) (!PageSKB(p) && !PageION(p) && \
	!PageVmalloc(p) && !PageZspage(p) && \
	!PageReserved(p) && !PageLRU(p))

#define buddy_pfn_track(pfn)  (PageDrv(pfn_to_page(pfn)) && \
	!PageLslub(pfn_to_page(pfn)) && page_exc(pfn_to_page(pfn)))

#define lslub_pfn_track(pfn)  (PageLslub(pfn_to_page(pfn)) && \
	PageDrv(pfn_to_page(pfn)) && page_exc(pfn_to_page(pfn)))
#define invalid_buddy_type(type) ((type) != BUDDY_TRACK && \
	(type) != LSLUB_TRACK)

static phys_addr_t g_start_pfn;
static inline struct funcmap *page_to_funcmap(struct page *page)
{
	return (func_map + page_to_pfn(page) - g_start_pfn);
}
...
{
	return page_to_funcmap(pfn_to_page(pfn))->caller;
}
...
static void buddy_track_info_dump(unsigned long pfn)
{
	...
	if (buddy_pfn_track(pfn) || lslub_pfn_track(pfn)) {
...
int buddy_track_map(int nid)
{
	...
	g_start_pfn = memblock_start_of_DRAM() >> PAGE_SHIFT;
	func_map = base;
...
static void buddy_fetch_stack(int type)
{
	...
	for_each_memblock(memory, reg) {
		...
		for (pfn = start; pfn < end && pfn_valid(pfn); pfn++) {
			/* lslub use buddy func to record stack */
			if (type == LSLUB_TRACK && lslub_pfn_track(pfn))
				buddy_add_caller(type, pfn);

			if (type == BUDDY_TRACK && buddy_pfn_track(pfn))
				buddy_add_caller(type, pfn);
		}
	}
}
