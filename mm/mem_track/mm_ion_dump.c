	...
	struct file *f, unsigned int fd)
{
	...
	pr_err("%-16.s%3s%-16d%-16u%-16zu%-16u%-16llu%-12lld%-12lld%-12lld%-12lld%-12lld\n",
		tsk->comm, "", tsk->pid, fd, dbuf->size,
		ibuf->heap->id, ibuf->magic, ibuf->buffer_trace.alloc_total_cost,
		ibuf->buffer_trace.alloc_sema_cost, ibuf->buffer_trace.heap_alloc1_cost,
		ibuf->buffer_trace.heap_drain_cost, ibuf->buffer_trace.heap_alloc2_cost);
	return 0;
}
