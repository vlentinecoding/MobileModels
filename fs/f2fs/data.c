	...
	bio_for_each_segment_all(bv, bio, iter_all) {
		...
		if (bio->bi_status || PageError(page)) {
			...
			if (f2fs_dclient && !dsm_client_ocuppy(f2fs_dclient)) {
				...
				dsm_client_notify(f2fs_dclient, DSM_F2FS_SET_SBI_NEED_FSCK);
			}
#endif
...
#ifdef CONFIG_HONOR_F2FS_DSM
				if (f2fs_dclient && !dsm_client_ocuppy(f2fs_dclient)) {
					dsm_client_record(f2fs_dclient, "F2FS mapping_set_error, type is F2FS_WB_CP_DATA: %s:%d [%d]\n",
		...
		if (f2fs_compressed_file(inode)) {
			...
			if (!f2fs_cluster_can_merge_page(&cc, page->index)) {
				ret = f2fs_read_multi_pages(&cc, &bio,
							max_nr_pages,
							&last_block_in_bio,
							is_readahead, false, &io_submited);
...
read_single_page:
		...
		ret = f2fs_read_single_page(inode, page, max_nr_pages, &map,
					&bio, &last_block_in_bio, is_readahead, &io_submited);
		if (ret) {
#ifdef CONFIG_F2FS_FS_COMPRESSION
set_error_page:
#endif
		...
		struct page **pagep, void **fsdata)
{
	...
	if (file && trace_android_fs_datawrite_start_enabled()) {
