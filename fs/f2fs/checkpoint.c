...
static int block_operations(struct f2fs_sb_info *sbi)
{
	...
	int err = 0, cnt = 0;
	unsigned long sync_quota_cycles = 0;
...
retry_flush_quotas:
	...
	if (__need_flush_quota(sbi)) {
		int locked;
		sync_quota_cycles++;
		if (unlikely(sync_quota_cycles % 1000 == 0))
			f2fs_err(sbi,
				 "%s:%d: too many sync inode cycles(%lu), maybe an infinite loop!\n",
				 __func__, __LINE__, sync_quota_cycles);
...
retry_flush_dents:
	...
	if (get_pages(sbi, F2FS_DIRTY_DENTS)) {
		sync_inode_cycles++;
		if (unlikely(sync_inode_cycles % 1000 == 0))
			f2fs_err(sbi,
				 "%s:%d: too many sync inode cycles(%lu), maybe an infinite loop!\n",
	...
	if (get_pages(sbi, F2FS_DIRTY_IMETA)) {
		sync_imeta_cycles++;
		if (unlikely(sync_imeta_cycles % 1000 == 0))
			f2fs_err(sbi,
...
retry_flush_nodes:
	...
	if (get_pages(sbi, F2FS_DIRTY_NODES)) {
		sync_nodes_cycles++;
		if (unlikely(sync_nodes_cycles % 1000 == 0))
			f2fs_err(sbi,
...
void f2fs_wait_on_all_pages(struct f2fs_sb_info *sbi, int type)
{
	...
	for (;;) {
		...
		ret = io_schedule_timeout(DEFAULT_IO_TIMEOUT);
		if (!ret) {
			...
			f2fs_warn(sbi,
				  "%s timeout %lu(jiffies), HZ= %lu, writeback type=%d pages=%lld\n",
				  __func__, DEFAULT_IO_TIMEOUT, HZ, type, get_pages(sbi, type));
			f2fs_warn(sbi,
				  "%s read io in flight %u, write io in flight %u\n",
