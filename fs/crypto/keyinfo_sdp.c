...
out:
	if (res != 0)
		sdp_pr_err("%s derive key failed, enc:%d, res:%d", __func__, enc, res);
	...
	buffer_t *fek, u8 has_pub_key)
{
	...
	if (has_pub_key != 0)
		res = fscrypt_derive_gcm_key(tfm, (const u8 *)ctx->nonce,
			fek->data, (u8 *)ctx->iv, FLAG_DECRYPT);
	else
		res = fscrypt_derive_gcm_key(tfm, fek->data, (u8 *)ctx->nonce,
