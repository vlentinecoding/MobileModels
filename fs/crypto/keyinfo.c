	...
	const struct fscrypt_mode *mode)
{
	...
	if (fscrypt_is_private_mode(crypt_info)) {
		crypt_info->ci_hw_enc_flag = (u8)(HWDPS_XATTR_ENABLE_FLAG);
	...
	const struct fscrypt_context *ctx)
{
	...
	if (res == 0) {
		pr_info("%s success\n", __func__);
		ci->ci_hw_enc_flag = (u8)(HWDPS_XATTR_ENABLE_FLAG);
	}
	goto free_out;
#endif
	...
	res = setup_crypto_transform(ci, mode, fek_data->data, inode);
	if (res != 0) {
...
static int hwdps_get_context_inner(struct inode *inode,
	struct fscrypt_info **ci, struct fscrypt_context *ctx,
	secondary_buffer_t *fek_buffer, buffer_t *encoded_wfek)
{
	...
	*ci = hwdps_get_fscrypt_info(ctx, inode);
	if (!(*ci)) {
		err = -EINVAL;
		pr_err("hwdps_get_fscrypt_info failed %d\n", err);
		return err;
	}

	err = hwdps_get_fek(ctx->master_key_descriptor, inode, encoded_wfek,
		fek_buffer);
	if (err != 0) {
...
int hwdps_update_context(struct inode *inode, uid_t new_uid)
{
	...
	if (hwdps_check_support(inode) != 0)
		return 0;
	encoded_wfek = hwdps_do_get_attr(inode, HWDPS_ENCODED_WFEK_SIZE);
	if (!encoded_wfek) {
	...
	err = hwdps_has_access(inode, &encoded_wfek_buffer);
	if (err != 0) {
...
int hwdps_get_context(struct inode *inode)
{
	...
	if (inode->i_crypt_info &&
		...
		(u8)(HWDPS_XATTR_ENABLE_FLAG)))
		return 0;
	err = hwdps_check_support(inode);

	if (err != 0)
		return err;
	encoded_wfek.data = hwdps_do_get_attr(inode, HWDPS_ENCODED_WFEK_SIZE);
	...
	if (inode->i_crypt_info) {
		err = hwdps_has_access(inode, &encoded_wfek);
		goto free_fek;
	}
	err = hwdps_get_context_inner(inode, &ci, &ctx, &fek_buffer,
		&encoded_wfek);
	if (err != 0) {
		pr_err("hwdps_get_context_inner failed, err:%d\n", err);
		goto free_fek;
	}
