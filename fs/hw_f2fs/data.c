				...
				enum page_type type, enum temp_type temp)
{
	...
	bio->mas_bio.ci_key = NULL;
	bio->mas_bio.ci_key_len = 0;
	bio->mas_bio.ci_key_index = -1;
#endif
	return bio;
}
...
#ifdef CONFIG_FS_UNI_ENCRYPTION
	if (fio->ci_key) {
		bio->mas_bio.ci_key = fio->ci_key;
		bio->mas_bio.ci_key_len = fio->ci_key_len;
		bio->mas_bio.ci_key_index = fio->ci_key_index;
		bio->mas_bio.index = page->index;
	}
#endif
...
next:
	...
	if (io->bio && (io->last_block_in_bio != fio->new_blkaddr - 1 ||
		...
		__submit_merged_bio(io);
#ifdef CONFIG_FS_UNI_ENCRYPTION
	else if ((io->bio) && ((io->bio->mas_bio.ci_key != fio->ci_key) ||
				(io->bio->mas_bio.ci_key_len != fio->ci_key_len) ||
				(fio->ci_key && io->last_index_in_bio !=
						bio_page->index - 1)))
		__submit_merged_bio(io);
#endif
#ifdef CONFIG_F2FS_TURBO_ZONE_V2
	else if ((io->bio) && (io->bio->mas_bio.flags != fio->flags))
		__submit_merged_bio(io);
#endif
...
alloc_new:
	if (io->bio == NULL) {
		...
		io->fio = *fio;
#ifdef CONFIG_FS_UNI_ENCRYPTION
		io->bio->mas_bio.ci_key = fio->ci_key;
		io->bio->mas_bio.ci_key_len = fio->ci_key_len;
		io->bio->mas_bio.ci_key_index = fio->ci_key_index;
		if (fio->ci_key)
			io->bio->mas_bio.index = bio_page->index;
#endif
#ifdef CONFIG_F2FS_TURBO_ZONE_V2
		io->bio->mas_bio.flags = fio->flags;
#endif
	}
...
#ifdef CONFIG_FS_UNI_ENCRYPTION
	if (need_key) {
		bio->mas_bio.ci_key = fscrypt_ci_key(inode);
		bio->mas_bio.ci_key_len = fscrypt_ci_key_len(inode);
		bio->mas_bio.ci_key_index = fscrypt_ci_key_index(inode);
		bio->mas_bio.index = page->index;
	}
#endif
	return bio;
