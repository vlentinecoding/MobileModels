...
#else
			f2fs_msg(sb, KERN_INFO, "inline_encrypt option not supported");
#endif
			break;
		case Opt_turbozone_v2:
#ifdef CONFIG_F2FS_TURBO_ZONE_V2
			set_opt(sbi, TURBOZONE_V2);
#endif
			break;
...
repeat:
		...
		if (IS_ERR(page)) {
			...
			f2fs_msg(sb, KERN_ERR, "Failed to read quota file "
...
retry:
		...
		if (unlikely(err)) {
			...
			f2fs_msg(sb, KERN_ERR, "Failed to write quota file "
...
int f2fs_quota_sync(struct super_block *sb, int type)
{
	...
	for (cnt = 0; cnt < MAXQUOTAS; cnt++) {
		...
		if (ret) {
			set_sbi_flag(F2FS_SB(sb), SBI_QUOTA_NEED_REPAIR);
			f2fs_msg(sb, KERN_ERR, "Failed to sync quota file "
...
{
	u32 total_blocks = le64_to_cpu(sbi->raw_super->block_count) >> 18;
	if (total_blocks > 64) { /* 64G */
		...
		sbi->hot_cold_params.hot_node_lower_limit = SSR_HN_SAPCE_LIMIT_128G;
...
#if defined(CONFIG_F2FS_FS_ENCRYPTION) && defined(CONFIG_HIE)
