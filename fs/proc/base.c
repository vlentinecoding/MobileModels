...
#ifdef CONFIG_HW_RTG
#include "../../kernel/sched/hw_rtg/include/rtg_sched.h"
#endif
			     ...
			     size_t count, loff_t *ppos)
{
	...
	put_task_struct(task);
#ifdef CONFIG_VIP_PAGE_CACHE
	if (!enable_vip_page_cache && task->static_vip == 1)
		enable_vip_page_cache = true;
	update_top_uids(task, task->static_vip == 1);
#endif
	return count;
}
...
#if defined(CONFIG_HW_VIP_THREAD) || defined(CONFIG_HW_RTG_FRAME)
void honor_pid_update_inode(struct dentry *dentry, struct task_struct *task,
		struct inode *inode)
{
	task_dump_owner(task, inode->i_mode, &inode->i_uid, &inode->i_gid);

// Blew is the content differents from FUNC : pid_update_inode()
// Different - Start
#ifdef CONFIG_HW_VIP_THREAD
...
static const struct file_operations top_uids_proc_fops = {
	.open = top_uids_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
