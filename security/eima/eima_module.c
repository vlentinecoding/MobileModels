...
int stp_eima_trigger(void)
{
	...
	if (ret != EOK) {
		eima_error("strcpy_s failed, s_ret=%d\n", ret);
		return -ENOMEM;
	}
				...
				enum eima_measure_type type)
{
	...
	ret = strcpy_s(entry->fn, (EIMA_NAME_STRING_LEN + 1),
		tmp_target->file);
	if (ret != EOK) {
		eima_error("strcpy_s failed, s_ret=%d\n", ret);
		return ret;
...
static int eima_static_do_work(struct m_list_msg *mlist_msg)
{
	...
	if (mlist_msg->num > 0) {
		ret = strcpy_s(mlist_msg->usecase, EIMA_NAME_STRING_LEN + 1,
			_EIMA_POLICY_USECASE_NAME);
		if (ret != EOK) {
			eima_error("mlist_msg strcpy_s failed, line=%d ret = %d\n", __LINE__, ret);
			return ret;
		}
...
static int eima_dynamic_do_work(struct m_list_msg *mlist_msg)
{
	...
	if (mlist_msg->num > 0) {
		ret = strcpy_s(mlist_msg->usecase, EIMA_NAME_STRING_LEN + 1,
			_EIMA_POLICY_USECASE_NAME);
		if (ret != EOK) {
			eima_error("mlisg_msg strcpy_s failed. line = %d, ret = %d\n", __LINE__, ret);
			return ret;
		}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 14, 0)
static struct security_hook_list eima_ops[] HISI_RO_LSM_HOOKS = {
...
static int eima_baselining_dynamic_do_work(struct m_list_msg *mlist_msg)
{
	...
	if (mlist_msg->num > 0) {
		(void)strcpy_s(mlist_msg->usecase, EIMA_NAME_STRING_LEN + 1,
			_EIMA_POLICY_USECASE_NAME);
		ret = send_eima_data(EIMA_MSG_BASELINE, mlist_msg);
		if (ret != 0) {
