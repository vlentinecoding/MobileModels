...
TRACE_EVENT(sugov_max_util,

	TP_PROTO(int cpu, int need_freq_update, unsigned long cpu_util, unsigned long sg_util),

	TP_ARGS(cpu, need_freq_update, cpu_util, sg_util),

	TP_STRUCT__entry(
		__field(int,	cpu)
		__field(int,	need_freq_update)
		__field(unsigned long,	cpu_util)
		__field(unsigned long,	sg_util)
		),
	...
	TP_fast_assign(
		...
		__entry->sg_util    = sg_util;
		),

	TP_printk("cpu=%d need_freq_update=%d cpu_util=%lu sg_util=%lu",
		__entry->cpu,
...
TRACE_EVENT(set_task_util,

	TP_PROTO(int pid, int id, unsigned long util, int ret,
		unsigned int min_user, unsigned int min_val,
		unsigned int max_user, unsigned int max_val),

	TP_ARGS(pid, id, util, ret, min_user, min_val, max_user, max_val),
	...
	TP_STRUCT__entry(
		...
		__field(int,	ret)
		__field(unsigned int,	min_user)
		__field(unsigned int,	min_val)
		__field(unsigned int,	max_user)
		__field(unsigned int,	max_val)
		),
	...
	TP_fast_assign(
		...
		__entry->util       = util;
		__entry->ret        = ret;
		__entry->min_user   = min_user;
		__entry->min_val    = min_val;
		__entry->max_user   = max_user;
		__entry->max_val    = max_val;
		),

	TP_printk("pid=%d id=%d util=%lu ret=%d min_user_defined=%u min_req=%u max_user_defined=%u max_req=%u",
		__entry->pid,
		__entry->id,
		__entry->util,
		__entry->ret,
...
TRACE_EVENT(favor_small_cap,

	TP_PROTO(int pid, int best_energy_cpu),

	TP_ARGS(pid, best_energy_cpu),
	...
	TP_printk("pid=%d best_energy_cpu=%d",
		__entry->pid,
		__entry->best_energy_cpu)
);

TRACE_EVENT(rtg_global_update,

	TP_PROTO(int cluster_id, u64 last_freq_update_time, unsigned int us_set_min_freq, unsigned int us_set_max_freq),

	TP_ARGS(cluster_id, last_freq_update_time, us_set_min_freq, us_set_max_freq),

	TP_STRUCT__entry(
		__field(int,	cluster_id)
		__field(u64,	last_freq_update_time)
		__field(unsigned int,	us_set_min_freq)
		__field(unsigned int,	us_set_max_freq)
		),

	TP_fast_assign(
		__entry->cluster_id   = cluster_id;
		__entry->last_freq_update_time = last_freq_update_time;
		__entry->us_set_min_freq = us_set_min_freq;
		__entry->us_set_max_freq = us_set_max_freq;
		),

	TP_printk("cluster=%d last_freq_update_time=%lu us_set_min_freq=%u us_set_max_freq=%u",
		__entry->cluster_id,
		__entry->last_freq_update_time,
		__entry->us_set_min_freq,
		__entry->us_set_max_freq)
);

TRACE_EVENT(sugov_freq_update,

	TP_PROTO(int cpu, unsigned int freq, unsigned int us_min, unsigned int us_max, unsigned int next_f, unsigned long policy_min, unsigned long policy_max),

	TP_ARGS(cpu, freq, us_min, us_max, next_f, policy_min, policy_max),

	TP_STRUCT__entry(
		__field(int,	cpu)
		__field(unsigned int,	freq)
		__field(unsigned int,	us_min)
		__field(unsigned int,	us_max)
		__field(unsigned int,	next_f)
		__field(unsigned long,	policy_min)
		__field(unsigned long,	policy_max)
		),

	TP_fast_assign(
		__entry->cpu   = cpu;
		__entry->freq   = freq;
		__entry->us_min = us_min;
		__entry->us_max = us_max;
		__entry->next_f = next_f;
		__entry->policy_min = policy_min;
		__entry->policy_max = policy_max;
		),

	TP_printk("cpu=%d freq=%u us_min=%u us_max=%u next_f=%u policy_min=%lu policy_max=%lu",
		__entry->cpu,
		__entry->freq,
		__entry->us_min,
		__entry->us_max,
...
#ifdef CONFIG_HW_RTG_TRACE
		__entry->rtg_depth= p->wts.rtg_depth;
#endif
	),
...
TRACE_EVENT(task_min_util,
	...
	TP_fast_assign(
		...
		__entry->min_util	= task->uclamp_req[UCLAMP_MIN].value;
#else
#ifdef CONFIG_SCHED_TASK_UTIL_CLAMP
		__entry->min_util	= task->util_req.min_util;
#else
		__entry->min_util	= -1;
#endif
#endif
	...
	TP_printk("pid=%d comm=%s prio=%u min_util=%d",
		...
		__entry->min_util)
#else
	TP_printk("pid=%d comm=%s min_util=%d",
...
TRACE_EVENT(sched_switch,
	...
	TP_fast_assign(
		...
		/* XXX SCHED_DEADLINE */
#ifdef CONFIG_HW_RTG_TRACE
		__entry->rtg_depth = next->wts.rtg_depth;
#endif
	...
	TP_printk("prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%s%s ==> next_comm=%s next_pid=%d next_prio=%d cpuset=%s cpuctl=%s",
		...
		__entry->next_comm, __entry->next_pid, __entry->next_prio, __entry->cpuset, __entry->cpuctl)
);
...
#ifdef CONFIG_STATICS_TASKS_TRACE
/*
 * Tracepoint for accounting runtime (statics of the current task
 * and the task in cfs_rq on a CPU).
 */
DECLARE_EVENT_CLASS(sched_stat_cur_and_cfs,
	...
	TP_fast_assign(
		...
		__entry->delay		= tsk->sched_info.run_delay;
		__entry->vruntime	= vruntime;
	),

	TP_printk("comm=%s pid=%d prio=%d, delay=%Lu [ns] vruntime=%Lu [ns]",
			__entry->comm, __entry->pid, __entry->prio,
			(unsigned long long)__entry->delay,
			(unsigned long long)__entry->vruntime)
);

DEFINE_EVENT(sched_stat_cur_and_cfs, sched_stat_cur_and_cfs,
	     TP_PROTO(struct task_struct *tsk, u64 vruntime),
	     TP_ARGS(tsk, vruntime));
#endif
/*
...
TRACE_EVENT(sched_task_util,
	...
	TP_STRUCT__entry(
		...
		__field(unsigned int,   min_uclamp_req)
		__field(unsigned int,   min_uclamp)
		__field(unsigned int,   min_uclamp_active)
	),
	...
	TP_fast_assign(
		...
		__entry->min_uclamp_req        = p->uclamp_req[UCLAMP_MIN].value;
		__entry->min_uclamp            = p->uclamp[UCLAMP_MIN].value;
		__entry->min_uclamp_active     = p->uclamp[UCLAMP_MIN].active;
	),

	TP_printk("pid=%d comm=%s util=%lu prev_cpu=%d candidates=%#lx best_energy_cpu=%d sync=%d need_idle=%d fastpath=%d placement_boost=%d latency=%llu stune_boosted=%d is_rtg=%d rtg_skip_min=%d start_cpu=%d unfilter=%u affinity=%lx task_boost=%d low_latency=%d min_uclamp_req=%u, min_uclamp=%u, min_uclamp_active=%u",
		__entry->pid, __entry->comm, __entry->util, __entry->prev_cpu,
		...
		__entry->low_latency, __entry->min_uclamp_req, __entry->min_uclamp, __entry->min_uclamp_active)
);
