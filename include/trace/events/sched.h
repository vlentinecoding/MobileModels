...
TRACE_EVENT(sched_find_vip_big_cpu_each,

	TP_PROTO(struct task_struct *task, int cpu, int has_vip, unsigned long wake_util),

	TP_ARGS(task, cpu, has_vip, wake_util),
	...
	TP_printk("task=%d(%u) cpu=%d curr=%d nr_rt=%d has_vip=%d wake_util=%lu ",
		__entry->pid, __entry->static_vip, __entry->cpu, __entry->curr,
		__entry->has_rt, __entry->has_vip, __entry->wake_util)
);
#endif
