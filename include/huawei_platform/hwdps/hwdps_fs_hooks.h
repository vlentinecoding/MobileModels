...
#define HWDPS_XATTR_ENABLE_FLAG 0x0020

#ifdef CONFIG_HWDPS

/* filesystem callback functions */
struct hwdps_fs_callbacks_t {
	hwdps_result_t (*create_fek)(u8 *desc, const u8 *fsname,
		const struct dentry *dentry, secondary_buffer_t *encoded_wfek,
		secondary_buffer_t *fek);

	hwdps_result_t (*hwdps_has_access)(encrypt_id *id,
		buffer_t *encoded_wfek);

	hwdps_result_t (*get_fek)(u8 *desc, encrypt_id *id,
		buffer_t *encoded_wfek, secondary_buffer_t *fek);

	hwdps_result_t (*update_fek)(u8 *desc, buffer_t *encoded_wfek,
