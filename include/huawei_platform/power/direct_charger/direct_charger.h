...
/* apply ratio policy for all segments */
#define BASP_RATIO_POLICY_ALL         0
/* apply ratio policy for max current */
#define BASP_RATIO_POLICY_MAX         1
...
typedef struct AGING_SAFE_POLICY_TAG {
	...
	/* 0:apply ratio policy for all segments, 1:for max current */
	unsigned int cur_ratio_policy;
...
static inline void direct_charge_select_comp_param(struct direct_charge_device *di)
{
}
