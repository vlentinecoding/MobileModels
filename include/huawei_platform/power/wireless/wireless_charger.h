...
#define RX_SAMPLE_DFT_DELAY_TIME    3000 /* 3s */
...
#define WIRELESS_INT_CNT_TH                 100
...
#define WLC_INSERT_IDENTIFY                 1

#define WLC_FOD_STATUS_PARA_LEVEL           10

#define WLC_CHARGER_ILIMIT                  1500

/* for cp39s icon display, 9v*1.4A */
#define NORMAL_CHARGE_MAX_POWER             12600000
...
enum wlc_fod_status_info {
	WLC_TX_TYPE,
	WLC_FOD_STATUS,
	WLC_FOD_STATUS_INFO_MAX,
};

struct wlc_fod_status_para {
	int tx_type;
	int fod_status;
};
...
enum wlc_rx_iout_limit_info {
	WLC_RX_VOUT_MIN,
	WLC_RX_VOUT_MAX,
	WLC_CURRENT_STEP,
	WLC_RX_IOUT_LIMIT_INFO_MAX,
};

struct wlc_rx_iout_limit_para {
	int rx_vout_min;
	int rx_vout_max;
	int current_step;
};
...
struct wireless_charge_device_info {
	...
	bool online;
	int motorPosition;
	struct wlc_rx_iout_limit_para rx_iout_limit_para;
	struct wlc_fod_status_para fod_status_para[WLC_FOD_STATUS_PARA_LEVEL];
	int fod_status_para_size;
	int usb_psy_check; /* check if obtain usb psy */
	int charge_type_check; /* check if charge type set successful */
};
...
static inline bool wireless_charge_is_pwr_good(void)
{
	return false;
}
