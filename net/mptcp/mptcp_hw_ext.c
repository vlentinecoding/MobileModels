...
#define ENABLE_BY_PID_DIP_DPORT         6
#define ENABLE_BY_SRV_PID               7
...
struct mptcp_hw_ext_pid_dport_conf {
	uint32_t flags;
	uint32_t pid;
	uint32_t dip;
	uint16_t dport;
...
struct mptcp_hw_ext_srv_pid_conf {
	uint32_t flags;
	uint32_t pid;
	struct mptcp_hw_ext_conf_value values;
};
...
struct mptcp_hw_ext_uid {
	struct list_head list;
	int32_t uid;
	int32_t pid; /* only valid for ENABLE_BY_PID_DIP_DPORT,ENABLE_BY_SRV_PID */
	...
	void *config; /* 1) if by uid, this config point to one mptcp_hw_ext_conf_value
			 ...
			 3) if by ENABLE_BY_PID_DIP_DPORT, this config point to one mptcp_hw_ext_pid_dport_conf
			 4) if by ENABLE_BY_SRV_PID, this config point to one mptcp_hw_ext_srv_pid_conf
		      */
...
static void xengine_report_uid_fallback(int32_t uid, int32_t reason)
{
	...
	emcom_xengine_mptcp_proxy_fallback(&report, sizeof(report));
}
...
void xengine_report_fallback(struct sock *meta_sk)
{
	...
	emcom_xengine_mptcp_fallback(&report, sizeof(report));
}
#endif

static struct mptcp_hw_ext_uid *mptcp_hw_ext_uid_in_list(int32_t uid, int32_t pid)
{
	struct mptcp_hw_ext_uid *node = NULL;

	list_for_each_entry(node, &mptcp_hw_ext_uid_list, list) {
		if (node->uid == uid && ((pid == 0) || (pid == node->pid)))
			return node;
	}
	...
	mptcp_hw_ext_get_isp_info_by_uid(int32_t uid)
{
	...
	uid_node = mptcp_hw_ext_uid_in_list(uid, 0);
	if (NULL == uid_node)
		return NULL;
...
static bool mptcp_hw_validate_conf_values_common(uint32_t value_type,
				union mptcp_hw_ext_value *val, const char *if_name)
{
	if (!val)
		return false;
...
{
	struct mptcp_hw_ext_dip_node *node = NULL;
	struct mptcp_hw_ext_dip_node *next = NULL;
...
{
	int i;
	struct mptcp_hw_ext_dport_node *dport_node = NULL;
...
{
	struct mptcp_hw_ext_dport_node *dport_node = NULL;
	struct mptcp_hw_ext_dport_node *dport_next = NULL;
...
static void mptcp_hw_ext_update_subflow_buf_info(struct tcp_sock *tp,
	__s32 snd_buf, __s32 rcv_buf, const char *if_name)
{
	uint8_t i = 0;
...
static void mptcp_hw_ext_update_subflow_prio_at_meta(struct tcp_sock *tp,
	const char *if_name, uint8_t low_prio)
{
	uint8_t i = 0;
	for (i = 0; i < MAX_SUPPORT_SUBFLOW_NUM; i++) {
		if ((strnlen(if_name, IFNAMSIZ) == strnlen(tp->hw_subflows[i].if_name, IFNAMSIZ)) &&
	...
	for_each_netdev_rcu(net, dev) {
		...
		for_ifa(in_dev) {
			if (saddr == ifa->ifa_address) {
				(void)strncpy(ifname, dev->name, ifname_len);
			   ...
			   char *if_name)
{
	...
	mptcp_for_each_sk(meta_tp->mpcb, sk) {
		mptcp_hw_get_ifname_fr_sock(sk, sk_ifname, IFNAMSIZ);
		if (if_name && strncmp(sk_ifname, if_name, IFNAMSIZ))
			continue;

		tp = tcp_sk(sk);
		tcp_get_info(sk, &value->subflow, true);
...
{
	struct mptcp_hw_ext_uid *uid_node = NULL;
	struct mptcp_hw_ext_dip_list *dip_list = NULL;
	...
	uint8_t key_type, uint32_t value_type, uint32_t port_to_add)
{
	...
	switch (key_type) {
	case ENABLE_BY_UID: {
			struct mptcp_hw_ext_uid_conf *conf = NULL;
			uid = config->conf.uid;
	...
	case ENABLE_BY_UID_DIP_DPORT_ISP: {
			...
			if (key_type == ENABLE_BY_UID_DIP_DPORT_ISP) {
				struct mptcp_hw_ext_uid_gid_carrier_info *isp = NULL;
		...
		break;
	case ENABLE_BY_PID_DIP_DPORT: {
		struct mptcp_hw_ext_pid_dport_conf *conf = NULL;
		uid = config->conf.pid_dip_dport.uid;
		uid_node->usr_switch = 1;
		strncpy(uid_node->prim_iface, "all", IFNAMSIZ - 1);
		uid_node->prim_iface[IFNAMSIZ - 1] = '\0';
		uid_node->pid = config->conf.srv_pid.pid;
		uid_node->config = kzalloc(sizeof(struct mptcp_hw_ext_pid_dport_conf), GFP_ATOMIC);
		if (!uid_node->config) {
			pr_err("%d kzalloc mptcp_hw_ext_pid_dport_conf fail\n", __LINE__);
			kfree(uid_node);
			return -ENOMEM;
		}
		uid_node->isp = NULL;

		conf = (struct mptcp_hw_ext_pid_dport_conf *)(uid_node->config);
		conf->flags = value_type;
		conf->pid = config->conf.pid_dip_dport.pid;
		conf->dip = config->conf.pid_dip_dport.dip;
		conf->dport = config->conf.pid_dip_dport.dport;
		/* set config for uid_node */
		ret = mptcp_hw_set_conf_values(value_type, &(conf->values),
			&(config->conf_value), config->if_name);
		if (ret) {
		...
		mptcp_info("%s: ENABLE_BY_PID_DIP_DPORT uid:%d pid %u port:%u\n", __func__, uid, conf->pid, conf->dport);
		break;
	}
	case ENABLE_BY_SRV_PID: {
		struct mptcp_hw_ext_srv_pid_conf *conf = NULL;
		uid = config->conf.srv_pid.uid;
		uid_node->usr_switch = true;
		strncpy(uid_node->prim_iface, "all", IFNAMSIZ - 1);
		uid_node->prim_iface[IFNAMSIZ - 1] = '\0';
		uid_node->pid = config->conf.srv_pid.pid;
		uid_node->config = kzalloc(sizeof(struct mptcp_hw_ext_srv_pid_conf), GFP_ATOMIC);
		if (!uid_node->config) {
			pr_err("%d kzalloc mptcp_hw_ext_srv_pid_conf fail\n", __LINE__);
			kfree(uid_node);
			return -ENOMEM;
		}
		uid_node->isp = NULL;

		conf = (struct mptcp_hw_ext_srv_pid_conf *)(uid_node->config);
		conf->flags = value_type;
		conf->pid = config->conf.srv_pid.pid;
		/* set config for uid_node */
		ret = mptcp_hw_set_conf_values(value_type, &(conf->values),
			&(config->conf_value), config->if_name);
		if (ret) {
		...
		mptcp_info("%s: ENABLE_BY_SRV_PID uid:%d listen pid:%u\n", __func__, uid, conf->pid);
		break;
	}
	...
	uint8_t key_type, uint32_t value_type, uint32_t port_to_add)
{
	...
	case ENABLE_BY_PID_DIP_DPORT: {
		...
		ret = mptcp_hw_set_conf_values(value_type, &(conf->values),
			&(config->conf_value), config->if_name);
		if (ret) {
			pr_err("%d update config fail with uid:%d pid:%u dport %u\n", __LINE__,
				uid_node->uid, conf->pid, conf->dport);
			return ret;
		}
		break;
	}
	case ENABLE_BY_SRV_PID: {
		...
		ret = mptcp_hw_set_conf_values(value_type, &(conf->values),
			&(config->conf_value), config->if_name);
		if (ret) {
			pr_err("%d update config fail with uid:%d pid:%u\n", __LINE__,
				uid_node->uid, conf->pid);
			return ret;
		}
		break;
	}
...
{
	struct mptcp_hw_ext_dport_node *node = NULL;
	struct mptcp_hw_ext_dport_node *next = NULL;
...
{
	const struct hlist_nulls_node *node = NULL;
			...
			if (mpcb && meta_sk->sk_uid.val == uid && meta_tp->user_switch != on) {
				if (mpcb->pm_ops->user_switch &&
					...
					if (sock_owned_by_user(meta_sk)) {
						...
						if (!test_and_set_bit(TCPF_USER_SWTCH_DEFERRED,
...
{
	struct mptcp_hw_ext_uid *node = NULL;
	struct mptcp_hw_ext_uid *next = NULL;
...
static bool mptcp_hw_ext_try_to_delete_dport_node(struct list_head *dport_list,
	struct mptcp_hw_ext *config)
{
	struct mptcp_hw_ext_dport_node *pdport_node = NULL;
	...
	uint32_t value_type)
{
	...
	case ENABLE_BY_UID_DIP_DPORT_ISP: {
		...
		break;
	case ENABLE_BY_PID_DIP_DPORT: {
		struct mptcp_hw_ext_pid_dport_conf *conf =
			(struct mptcp_hw_ext_pid_dport_conf *)(uid_node->config);
		if (value_type == MPTCP_HW_CONF_VALUE_MPTCP) {
			list_del_init(&uid_node->list);
			if (conf)
				kfree(conf);
		...
		mptcp_info("%d delete config flag:%u with uid:%d\n", __LINE__,
			value_type, uid_node->uid);
		break;
	}
	case ENABLE_BY_SRV_PID: {
		struct mptcp_hw_ext_srv_pid_conf *conf =
			(struct mptcp_hw_ext_srv_pid_conf *)(uid_node->config);
		if (value_type == MPTCP_HW_CONF_VALUE_MPTCP) {
			list_del_init(&uid_node->list);
			if (conf)
				kfree(conf);
...
{
	struct mptcp_hw_ext_uid_gid_carrier_info *node = NULL;
	...
	uint8_t old_key_type, uint8_t new_key_type, bool is_add)
{
	...
	case ENABLE_BY_UID: {
		if (is_add && (new_key_type == ENABLE_BY_UID_DIP_DPORT ||
			       new_key_type == ENABLE_BY_UID_DIP_DPORT_ISP ||
			       new_key_type == ENABLE_BY_PID_DIP_DPORT ||
			       new_key_type == ENABLE_BY_SRV_PID))
			ret = false;
	...
	case ENABLE_BY_PID_DIP_DPORT: {
		if (is_add && (new_key_type == ENABLE_BY_UID_DIP_DPORT ||
			       new_key_type == ENABLE_BY_UID_DIP_DPORT_ISP ||
			       new_key_type == ENABLE_BY_UID ||
			       new_key_type == ENABLE_BY_SRV_PID))
			ret = false;

		if (!is_add && new_key_type != ENABLE_BY_PID_DIP_DPORT)
			ret = false;
		break;
	}
	case ENABLE_BY_SRV_PID: {
		...
		if (!is_add && new_key_type != ENABLE_BY_SRV_PID)
			ret = false;
		break;
	}
	default:
	...
	uint8_t key_type, uint32_t value_type)
{
	...
	case ENABLE_BY_PID_DIP_DPORT:
		uid = config->conf.pid_dip_dport.uid;
		pid = config->conf.pid_dip_dport.pid;
		if (pid <= 0) {
	...
	uid_node = mptcp_hw_ext_uid_in_list(uid, pid);
	if (uid_node) {
	...
	uint8_t key_type, uint32_t value_type)
{
	...
	uid_node = mptcp_hw_ext_uid_in_list(uid, pid);
	if (NULL == uid_node) {
		pr_err("%d the node with uid:%d pid:%d not exist\n", __LINE__, uid, pid);
		return -EINVAL;
	}
...
static int mptcp_hw_user_switch(struct mptcp_hw_ext *ext)
{
	...
	if ((uid_node != NULL) && (uid_node->key_type == ENABLE_BY_PID_DIP_DPORT ||
	    uid_node->key_type == ENABLE_BY_SRV_PID)) {
		mptcp_debug("%s: key_type not supported\n", __func__);
		return 0;
	}
...
int mptcp_hw_ext_cmd_get_sockopt(struct mptcp_hw_ext *ext)
{
	...
	if (ext->cmd >= MPTCP_HW_EXT_CMD_MAX_INVALID ||
	    (((uint32_t)ext->cmd & 0xFFFFFF) >= MPTCP_HW_EXT_MAX_VALUE_TYPE)) {
...
int mptcp_hw_ext_cmd_set_sockopt(struct mptcp_hw_ext *ext)
{
	...
	case MPTCP_HW_CONF_KEY_PID_DIP_DPORT:
		if (action)
			ret = mptcp_hw_ext_add_config(ext, ENABLE_BY_PID_DIP_DPORT, value_type);
		else
			ret = mptcp_hw_ext_del_config(ext, ENABLE_BY_PID_DIP_DPORT, value_type);
		break;
	case MPTCP_HW_CONF_KEY_PID_LISTEN:
		if (action)
			ret = mptcp_hw_ext_add_config(ext, ENABLE_BY_SRV_PID, value_type);
		else
			ret = mptcp_hw_ext_del_config(ext, ENABLE_BY_SRV_PID, value_type);
		break;
...
bool check_ip_addrss_for_mptcp_available_v4(__be32 s_addr)
{
	return !ipv4_is_loopback(s_addr) && !ipv4_is_multicast(s_addr) &&
		!ipv4_is_zeronet(s_addr) && !ipv4_is_lbcast(s_addr);
}

bool check_ip_addrss_for_mptcp_available_v6(struct sockaddr_in6 *usin6)
{
	if (ipv6_addr_v4mapped(&usin6->sin6_addr)) {
		__be32 addr_v4 = usin6->sin6_addr.s6_addr32[3];
		char src_ip_str[INET_ADDRSTRLEN];

		mptcp_debug("%s: ipv6 check ip=%s\n",  __func__,
			anonymousIPv4addr(addr_v4, src_ip_str,
				INET_ADDRSTRLEN));

		return check_ip_addrss_for_mptcp_available_v4(addr_v4);
	}
	return !ipv6_addr_loopback(&usin6->sin6_addr) &&
		!ipv6_addr_is_multicast(&usin6->sin6_addr);
}
...
bool check_ip_addrss_for_mptcp_available(struct sockaddr *addr)
{
	if (addr->sa_family == AF_INET) {
		...
		__be32 s_addr = usin->sin_addr.s_addr;

		return check_ip_addrss_for_mptcp_available_v4(s_addr);
	}
#if IS_ENABLED(CONFIG_IPV6)
	else if (addr->sa_family == AF_INET6) {
		struct sockaddr_in6 *usin6 = (struct sockaddr_in6 *)addr;

		return check_ip_addrss_for_mptcp_available_v6(usin6);
	}
#endif
...
bool check_ip_addrss_public_available(struct sockaddr *addr)
{
	struct sockaddr_in *usin = (struct sockaddr_in *)addr;
...
static int32_t list_enable_by_pid_dip_dport(struct mptcp_hw_ext_uid *uid_node,
	struct sockaddr_in *daddr, struct mptcp_hw_ext_conf_value *values,
	struct tcp_sock *tp)
{
	struct mptcp_hw_ext_pid_dport_conf *conf = NULL;
	__be32	s_addr;
	__be16	sin_port;
	int32_t flags = 0;
	struct sockaddr_in6 *usin6 = (struct sockaddr_in6 *)daddr;
	if (daddr->sin_family == AF_INET) {
		s_addr = daddr->sin_addr.s_addr;
		sin_port = daddr->sin_port;
	} else if (ipv6_addr_v4mapped(&usin6->sin6_addr)) {
		s_addr = usin6->sin6_addr.s6_addr32[3];
		sin_port = usin6->sin6_port;
	} else {
		mptcp_debug("%s: daddr is not a AF_INET address\n", __FUNCTION__);
		return 0;
	}
	conf = (struct mptcp_hw_ext_pid_dport_conf *)(uid_node->config);
	if (conf && (conf->flags & MPTCP_HW_CONF_VALUE_MPTCP)) {
		if (conf->dip != s_addr) {
			mptcp_debug("%s: dip %08x s_addr %08x\n",
				__FUNCTION__, conf->dip, s_addr);
			return 0;
		}

		if (conf->dport != ntohs(sin_port)) {
			mptcp_debug("%s: dport %u sin_port %u\n",
				__FUNCTION__, conf->dport, ntohs(sin_port));
			return 0;
		}
		flags = conf->flags;
		*values = conf->values;
		tp->mptcp_cap_flag = MPTCP_CAP_PID_DIP_DPORT;
		tp->user_switch = uid_node->usr_switch;
	}
	return flags;
}
...
{
	struct mptcp_hw_ext_uid *uid_node = NULL;
	struct mptcp_hw_ext_uid *tmp = NULL;
	uint8_t key_type;
	int32_t flags = 0;
	...
	list_for_each_entry(tmp, &mptcp_hw_ext_uid_list, list) {
		if (tmp->uid != uid)
			continue;

		/* ENABLE_BY_PID_DIP_DPORT/ENABLE_BY_SRV_PID pid must be valid */
		if (tmp->key_type == ENABLE_BY_PID_DIP_DPORT ||
		    tmp->key_type == ENABLE_BY_SRV_PID) {
			if (!current) {
				mptcp_debug("%s: current is NULL\n", __FUNCTION__);
				continue;
			}
			if (tmp->pid != task_tgid_nr(current)) {
				mptcp_debug("%s: pid %d current %d\n", __FUNCTION__, tmp->pid, task_tgid_nr(current));
				continue;
			}
		}

		/* daddr must valid for ip match key type */
		if (!daddr && (tmp->key_type == ENABLE_BY_UID_DIP_DPORT ||
			tmp->key_type == ENABLE_BY_UID_DIP_DPORT_ISP ||
			tmp->key_type == ENABLE_BY_PID_DIP_DPORT)) {
			mptcp_debug("%s: daddr is NULL\n", __FUNCTION__);
			continue;
		}

		/* daddr must be NULL for server enabled */
		if (daddr && tmp->key_type == ENABLE_BY_SRV_PID)
			continue;
		uid_node = tmp;
		break;
	}
	...
	case ENABLE_BY_UID_DIP_DPORT_ISP: {
		...
		break;
	case ENABLE_BY_PID_DIP_DPORT: {
		flags = list_enable_by_pid_dip_dport(uid_node, daddr, values, tp);
		break;
	}
	case ENABLE_BY_SRV_PID: {
		struct mptcp_hw_ext_srv_pid_conf *conf;
		...
		if (conf && (conf->flags & MPTCP_HW_CONF_VALUE_MPTCP)) {
			if (sk_state_load(sk) != TCP_LISTEN) {
				mptcp_debug("%s: sk_state is not TCP_LISTEN\n", __FUNCTION__);
				return flags;
			}

			flags = conf->flags;
			*values = conf->values;
			tp->mptcp_cap_flag = MPTCP_CAP_PID_LISTEN;
			tp->user_switch = uid_node->usr_switch;
		}
		break;
	}
				       ...
				       struct sockaddr_in *daddr, int addr_len)
{
	...
	if (!tp || mptcp_init_failed || !sysctl_mptcp_enabled) {
	...
	if (sysctl_mptcp_enabled == MPTCP_TEST) {
		mptcp_enable_sock(sk);
		tp->mptcp_cap_flag = MPTCP_CAP_PID_DIP_DPORT;
		tp->user_switch = true;
		(void)strncpy(tp->prim_iface, "all", IFNAMSIZ - 1);
		tp->prim_iface[IFNAMSIZ - 1] = '\0';
		(void)mptcp_set_scheduler(sk, "redundant");
		mptcp_debug("%s: enable mptcp for all MPTCP_TEST\n", __func__);
		return;
	}
#ifdef CONFIG_MPTCP_EPC
	if (mptcp_hw_is_enable_hag_gw(sk) && check_ip_addrss_public_available((struct sockaddr *)daddr)) {
		struct sock *meta_sk = tp->meta_sk ? tp->meta_sk : sk;
		mptcp_enable_sock(sk);
		tp->mptcp_cap_flag = MPTCP_CAP_ALL_APP;
		tcp_sk(meta_sk)->user_switch = true;
		tp->user_switch = true;
	...
	if (tp->mptcp_cap_flag == 0 || tp->mptcp_cap_flag == ENABLE_BY_SRV_PID)
		return;

	mptcp_enable_sock(sk);
	mptcp_info("%s: enable mptcp for %u\n", __func__, tp->mptcp_cap_flag);
	if (sock_flag(sk, SOCK_MPTCP)) {
...
void mptcp_init_tcp_listen_sock(struct sock *sk)
{
	...
	first_path.if_name[0] = '\0';
	read_lock_bh(&mptcp_hw_ext_lock);
	flags = in_mptcp_uid_list(sk, NULL, &value, &first_path);
	read_unlock_bh(&mptcp_hw_ext_lock);
	mptcp_debug("%s: flags:%d mptcp_cap_flag %u user_switch:%d\n",
		__func__, flags, tp->mptcp_cap_flag, tp->user_switch);
	if (tp->mptcp_cap_flag != MPTCP_CAP_PID_LISTEN) {
		mptcp_debug("%s: tp->mptcp_cap_flag %u\n", __func__, tp->mptcp_cap_flag);
		tp->mptcp_cap_flag = 0;
		return;
	}
	...
	mptcp_enable_sock(sk);
	mptcp_info("%s: enable mptcp for %u\n", __func__, tp->mptcp_cap_flag);
	if ((uint32_t)flags & MPTCP_HW_CONF_VALUE_SCHEDULER) {
		err = mptcp_set_scheduler(sk, value.sched.sched_name);
		if (err) {
			pr_err("set scheduler failed, scheduler name:%s\n",
				value.sched.sched_name);
...
void mptcp_init_sub_sock(struct sock *sk, bool master_sk)
{
	...
	if (meta_tp->mptcp_cap_flag == MPTCP_CAP_PID_LISTEN) {
		return;
	}
...
bool mptcp_hw_ext_get_switch_by_uid(int32_t uid)
{
	...
	struct mptcp_hw_ext_uid *uid_node = NULL;
#ifdef CONFIG_MPTCP_EPC
	if (mptcp_hw_is_enable_hag_config())
		return true;
#endif
	read_lock_bh(&mptcp_hw_ext_lock);
	uid_node = mptcp_hw_ext_uid_in_list(uid, 0);
...
#ifdef CONFIG_MPTCP_EPC
	mptcp_teid_options_write(ptr, tp, opts, skb);
#endif
}
...
int mptcp_proxy_fallback(struct sock *sk, int reason, bool is_add)
{
	...
	uid = sk->sk_uid.val;
	write_lock_bh(&mptcp_hw_ext_lock);
	node = mptcp_hw_ext_uid_in_list(uid, 0);
