...
static void qrtr_log_resume_pkt(struct qrtr_cb *cb, u64 pl_buf)
{
	...
	if (glink_resume_pkt) {
		...
		pr_info("[QRTR RESUME PKT]:src[0x%x:0x%x] dst[0x%x:0x%x] [%08x %08x]: service[0x%x]\n",
			...
			service_id);
#ifdef CONFIG_HUAWEI_DUBAI
		snprintf(des, sizeof(des), "[QRTR RESUME PKT]: src[0x%x:0x%x] dst[0x%x:0x%x] [%08x %08x]: service[0x%x]",
			cb->src_node, cb->src_port, cb->dst_node, cb->dst_port,
			(unsigned int)pl_buf, (unsigned int)(pl_buf >> 32), service_id);
		dubai_log_irq_wakeup(des);
#endif
	}
}
#endif
